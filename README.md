...
**Claude Code 범용 SW 개발 오케스트레이션 플러그인**. 사용자 요구사항 한 건을 받아 **0 core 에이전트 (wrapper-only) + 6 lane plugin (codeforge-{requirements,design,review,develop,test,pmo}) + `role: dev` 동적 roster · 8 레인** 구조로 요구사항 해석 → 요구사항 리뷰 → 설계 → 설계 리뷰 → 구현 → 구현 리뷰 → 통합 테스트 → 보안 테스트까지 자율 실행한다.

## 주요 특징

- **0 core (wrapper-only) + 6 lane plugin + `role: dev` 동적 roster · 8 레인 · 1 Cross-cutting (PMOAgent)** 구조로 SW 개발 프로세스 전반 커버

---
key: CFP-FIXTURE-FIX-INVALID-2
title: FIX evidence fixture — 2 FIX iterations but only 1 fix-iter lane row (FAIL)
status: phase:구현-테스트
date: 2026-05-09
type: story
---

# CFP-FIXTURE-FIX-INVALID-2: Invalid fixture — 2 FIX iterations, only 1 fix-iter row

(fixture stub — CFP-298 check-fix-evidence.sh FAIL test: §10 has 2 FIX rows, §14 has only 1 fix-iter row)

## §1 메타

placeholder

## §10 FIX Ledger

| Iter | 시각 | 레인 | 트리거 | 원인 판정 | 재실행 범위 | RESET? |
|------|------|------|--------|-----------|-------------|--------|
| 1    | 2026-05-09T11:00:00Z | 설계-리뷰 | DesignReviewPL P0 §7 누락 | 설계 | Change Plan §3 재작성 | — |
| 2    | 2026-05-09T14:00:00Z | 구현-리뷰 | CodeReviewPL P0 보안 | 구현 | Phase 2 PR commit append | — |

## §14 Lane Evidence

```yaml
lane_evidence:
  - lane: 요구사항
    iteration: 1
    agent: RequirementsPLAgent
    spawned_at: 2026-05-09T10:00:00Z
    returned_at: 2026-05-09T10:15:00Z
    output_status: completed
    outcome: PASS
    spawn_id: req-001
    fix_iteration: null
  - lane: 설계
    iteration: 1
    agent: ArchitectPLAgent
    spawned_at: 2026-05-09T10:20:00Z
    returned_at: 2026-05-09T10:50:00Z
    output_status: completed
    outcome: PASS
    spawn_id: design-001
    fix_iteration: null
  - lane: 설계-리뷰
    iteration: 1
    agent: DesignReviewPL
    spawned_at: 2026-05-09T11:00:00Z
    returned_at: 2026-05-09T11:30:00Z
    output_status: completed
    outcome: FIX
    spawn_id: design-review-001
    fix_iteration: null
  - lane: 설계
    iteration: 2
    agent: ArchitectPLAgent
    spawned_at: 2026-05-09T12:00:00Z
    returned_at: 2026-05-09T12:45:00Z
    output_status: completed
    outcome: PASS
    spawn_id: design-002-fix
    fix_iteration: 1
  - lane: 설계-리뷰
    iteration: 2
    agent: DesignReviewPL
    spawned_at: 2026-05-09T13:00:00Z
    returned_at: 2026-05-09T13:20:00Z
    output_status: completed
    outcome: PASS
    spawn_id: design-review-002
    fix_iteration: null
  - lane: 구현
    iteration: 1
    agent: DeveloperPL
    spawned_at: 2026-05-09T13:30:00Z
    returned_at: 2026-05-09T15:00:00Z
    output_status: completed
    outcome: PASS
    spawn_id: develop-001
    fix_iteration: null
  - lane: 구현-리뷰
    iteration: 1
    agent: CodeReviewPL
    spawned_at: 2026-05-09T15:10:00Z
    returned_at: 2026-05-09T15:40:00Z
    output_status: completed
    outcome: FIX
    spawn_id: code-review-001
    fix_iteration: null
  # NOTE: fix_iteration: 2 행 누락 — 이것이 FAIL 원인
  - lane: 구현
    iteration: 2
    agent: DeveloperPL
    spawned_at: 2026-05-09T16:00:00Z
    returned_at: 2026-05-09T17:00:00Z
    output_status: completed
    outcome: PASS
    spawn_id: develop-002
    fix_iteration: null
```

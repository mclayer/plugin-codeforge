...
#
# 검증 대상 (ADR-058 §결정 1-3 verbatim):
#   (a) docs/adr/ADR-*.md frontmatter `is_transitional: true|false` 필드 존재
#   (b) 본문에 `## 해소 기준` 섹션 존재
#   (c) is_transitional=false 시 "N/A — permanent policy" 1줄 또는 동등 형식 허용
#   (d) is_transitional=true 시 측정성 3-tuple (metric / who / how) 존재 +
#       모달 어휘 1차 사전 4 표현 anti-pattern 매치 검사 (ADR-058 §결정 8 verbatim)
#
# Modal anti-pattern dictionary v1.0 (4 표현):
#   - "안정화되면"
#   - "임시"
#   - "한시적"
...
#   - exit 1 : violation 1건 이상 (workflow level conditional / 후속 enforce mode 시 PR block)
#
# 인자:
#   $@ : 검증 대상 ADR file path list (없으면 docs/adr/ADR-*.md glob)
#
# 명령 라인:
#   $ bash scripts/check-adr-sunset-criteria.sh docs/adr/ADR-060-evidence-enforceable-promotion-framework.md
#   $ bash scripts/check-adr-sunset-criteria.sh    # all ADRs
#
# carrier: ADR-060 §결정 4
set -euo pipefail
cd "$(dirname "$0")/.."
...
for p in paths:
    ...
    if "is_transitional" not in fm:
        # ADR-058 §결정 4 — 미선언 default = True (safe direction).
        # warning mode 에서는 explicit declaration 요구 (작성자 부담 최소화 회피).

...
#
# Agent 액션 렌더 줄 프리픽스 `[에이전트명] MM/DD HH:MM:SS - 내용` 의 컴팩트 KST 시각을 stdout 1줄로 산출.
# 시각원 pin (ADR-143 §결정 3):
#   - primary (GNU date): `date -u -d '+9 hours' '+%m/%d %H:%M:%S'` — UTC read 후 +9시간 산술 (tzdata 무의존).
...
# Usage: bash scripts/kst-render-stamp.sh   → stdout `MM/DD HH:MM:SS`
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

# primary: GNU date UTC+9 고정 산술 (offset·연도·KST 라벨 미표기, 초 표기 — ADR-143 §결정 2 + Amendment 4 컴팩트)
if STAMP="$(date -u -d '+9 hours' '+%m/%d %H:%M:%S' 2>/dev/null)" && [ -n "$STAMP" ]; then
  printf '%s\n' "$STAMP"
  exit 0
fi

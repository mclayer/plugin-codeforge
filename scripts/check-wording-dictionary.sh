...
#   별 (standalone) = docs/adr/** + archive/adr/** + docs/change-plans/** + CLAUDE.md + docs/orchestrator-playbook.md + templates/** (6-scope — archive/adr CFP-2661 D1 union-ADD, ADR-064 Amendment 15/CFP-1561 SSOT 정합. WORD_TARGETS L62 이 SSOT)
...
#   별 = 6-scope (5 영역 + archive/adr CFP-2661 D1 union-ADD; 확장 scope docs/inter-plugin-contracts 등은 배제 — fp carrier 분리, Amendment 4 §Amendment 결정 6 + Amendment 15/CFP-1561)
...
if [ "$WD_WRITE_BASELINE" -eq 1 ]; then
  {
    ...
    echo "# grandfather = D1 union(archive/adr) 노출 시점 기존 카테고리 (a) 위반(file|word|content) 동결 → new-only subtract (ADR-171 §결정6 Clean-as-You-Code)."

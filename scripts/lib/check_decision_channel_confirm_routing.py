#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""scripts/lib/check_decision_channel_confirm_routing.py

CFP-2725 Phase 2 / Change Plan §8 RTM AC-6 — 원격 결정 채널(jira-decision-channel)의
최종 확정 payload + user-final-confirmation-driven terminal routing 확정 배선 presence lint.
Python SSOT engine (thin wrapper = scripts/check-decision-channel-confirm-routing.sh, ADR-061 §결정 1).

목적:
  design-entry 확정 gate 의 원격 채널 경로(옵션 1 최종 확정 = terminal event, 재조사 미발동)가
  skills/jira-decision-channel/SKILL.md 에 landed 되어 있는지(회귀/드리프트 방어)를 정적으로 검사.
  검사 대상 anchor:
    - `최종 확정 payload`               — design-entry gate 확정 payload 절 앵커
    - `user-final-confirmation-driven`  — 순수 확정(내용 무변경) terminal trigger origin 앵커
...
CLI 계약 (QADev self-test 가 이 계약에 의존):
  python3 check_decision_channel_confirm_routing.py [--root <repo_root>] [--self-test]
...
ReDoS-safe: 리터럴 `in` substring 검사 (정규식 backtracking 부재), 문서 단위 스캔.
"""
...
LINT_NAME = "decision-channel-confirm-routing"

# REQUIRED (target_rel_path, [anchor_literals]) — Change Plan §8 RTM AC-6 확정 배선 anchor.
REQUIREMENTS = [
    ("skills/jira-decision-channel/SKILL.md", ["최종 확정 payload", "user-final-confirmation-driven"]),
]
...
def scan(root):
    ...
    for rel, anchors in REQUIREMENTS:
        ...
        for anchor in anchors:
            if anchor not in text:  # 리터럴 substring — ReDoS-safe (backtracking 부재)
...
def run(root):
    try:
        ...
        return EXIT_SETUP
    if failures:
        print(f"[{LINT_NAME}] FAIL — 원격 결정 채널 최종 확정 routing 확정 배선 presence 위반 "
              "(Change Plan §8 RTM AC-6 / target-existence + hollow-gate guard):")
        for msg in failures:
            print(f"  {msg}")

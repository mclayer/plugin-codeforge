...
DRIFT_ALLOWLIST = {
    ...
    # CFP-1336 carrier — 5 pre-existing main drift cleanup (similar to CFP-1413 amendment-ref-drift root-cleanup pattern)
    ("dialog-fidelity-effect",          "dialog-fidelity-measurement.yml"):   "Conservative no-rename — entry name semantic 'effect' (DialogFidelityAgent assertion divergence effect), workflow basename 'measurement' (KPI measurement layer). Pre-existing main drift, CFP-1336 carrier cleanup",
    ("branch-protection-sync",          "branch-protection-drift-check.yml"): "Conservative no-rename — entry name 'sync' (CODEOWNERS/branch-protection-manifest sync verify), workflow basename 'drift-check' (drift detection layer). Pre-existing main drift, CFP-1336 carrier cleanup",

...
Exit codes — `--cross-repo` 모드 (G5 — ADR-157 §결정4 failure-mode 분리, base scan 과 disjoint):
  0 = PASS (전파 확인 / vacuous(namespace 자원 0 = wrapper-self 정상) / transient-fail-open[Issue 발행]).
      non-SHA ref(tag/mutable-branch)는 WARN 방출 후 대조 진행 = exit 불변(hard-fail 아님, §결정8 (viii)).
  1 = fail-closed (content-mismatch drift = 타저장소 미전파 검출 / 403·404 hard-unavailable /
      moving-HEAD·traversal·namespace-spoof(_NS_RE) 위반). foreign transient 은 여기 아님(fail-open).
      ※ ref-pin 은 moving-HEAD 7종만 hard-fail — name-based 라 tag/branch 구분 불가 ∴ 'tag/SHA 강제'
        집행 불가. 그 외 non-SHA 는 WARN(exit 0 불변), 엄밀 idempotency 는 SHA-pin(^[0-9a-f]{7,64}$)만 보장.
...
# ref-pin: moving-HEAD hard-fail + non-SHA WARN (§결정4/§결정8 (viii)). name-based 라 tag/branch 구분
#   불가 ∴ 'tag/SHA 강제' 집행 불가 — 아래 심볼릭 ref 는 hard-fail 거부, 그 외 non-SHA(tag/mutable-branch)
#   는 수용하되 WARN(idempotent 미보장), 엄밀 idempotency 는 SHA-pin(_SHA_REF_RE)만 보장.
_MOVING_REFS = frozenset(("main", "master", "head", "trunk", "develop", "default", "latest"))
# namespace = fetch 대상 foreign owner/repo. _NS_RE 형식검증(spoof/traversal 차단) — 별도 allowlist 없음
#   (fetch 도메인 = 선언 namespace 집합 자체라 same-domain allowlist = tautology, §결정8 (vii)).
...
_SHA_REF_RE = re.compile(r"^[0-9a-f]{7,64}$")                            # SHA-pin only = 엄밀 idempotency (§결정8 (viii))
...
#   전 regex bounded capture (ReDoS-safe — nested quantifier 0, 상한 명시 quantifier).
_GH_TOKEN_RE = re.compile(r"\bgh[pousr]_[A-Za-z0-9]{16,255}\b")            # classic gh 토큰 (ghp_/gho_/…)
_GH_FINEGRAINED_PAT_RE = re.compile(r"\bgithub_pat_[A-Za-z0-9_]{20,255}\b")  # fine-grained PAT (github_pat_…)
_BEARER_RE = re.compile(r"(?i)\b(bearer)\s{1,8}[A-Za-z0-9._~+/\-]{8,512}=*")  # Authorization: Bearer <t>
_KV_SECRET_RE = re.compile(r"(?i)\b(token|secret|password|passwd|pat|api[_-]?key|authorization|bearer)\b"
                           r"(\s*[:=]\s*|\s+)\S{1,512}")
_USERPASS_RE = re.compile(r"://[^/\s:@]{1,255}:[^/\s@]{1,255}@")
...
#   실행/eval 절대 0) + secret self-redaction + fail-closed default + _NS_RE 형식검증(spoof/traversal 차단).
...
def _redact(text):
    ...
    text = _GH_FINEGRAINED_PAT_RE.sub("<redacted-token>", text)
    ...
    if proc.returncode == 0:
        try:
            ...
            if not isinstance(obj, dict):
                # contents API 가 dir 목록(JSON array) 등 non-dict 반환 = 대상 파일 아님 → `.get`
                #   AttributeError 차단 + 빈 콘텐츠(미참조 = content-mismatch drift, fail-closed 방향).
                return ("ok", "")
...
def run_cross_repo(manifest):
    ...
    hard = []         # 403/404/moving-HEAD/traversal/spoof → fail-closed
    transient = []    # 503/network/timeout → fail-open + WARN + Issue
    propagated = []
    nonsha_warn = []  # non-SHA ref(tag/mutable-branch) → WARN (exit 불변, §결정8 (viii))
    ...
    for r in cross:
        ...
        # namespace 형식 검증 (spoof/traversal 차단 — _NS_RE shape/traversal, §결정4/§결정8 (vii)).
        #   fetch 도메인 = 선언 namespace 집합(순회 대상 = manifest cross-repo 자원)이라 별도 allowlist 불요:
        #   same-domain allowlist = tautology(같은 semi-trusted project.yaml) + read-only fetch → 보안 분리 무제공.
        if not _NS_RE.match(ns):
            hard.append((rid, ns, "namespace 형식 위반/traversal (spoof — _NS_RE)"))
            print("::error::check-infra-resource-drift: CROSS-REPO HARD-FAIL — resource-id=%s "
                  "namespace 형식 위반/traversal → fetch 차단 (spoof, §결정4 _NS_RE shape/traversal)." % rid)
            continue
        # ref-pin: moving-HEAD 금지(hard-fail) + traversal(`..`) 차단 (§결정4). name-based 라 tag/branch
        #   구분 불가 ∴ 여기선 moving-HEAD·형식·traversal 만 hard-fail, non-SHA WARN 은 아래에서 방출.
        if not ref or ref.lower() in _MOVING_REFS or ".." in ref or not _CROSS_REF_RE.match(ref):
            hard.append((rid, ns, "cross_repo_ref 부재/moving-HEAD/traversal/형식위반 (ref-pin)"))
            print("::error::check-infra-resource-drift: CROSS-REPO HARD-FAIL — resource-id=%s "
                  "cross_repo_ref 미pin (moving HEAD 금지 — moving-HEAD/traversal hard-fail, §결정4 ref-pin)." % rid)
            continue
        # path traversal 차단.
        if not path or ".." in path or path.startswith("/") or not _CROSS_PATH_RE.match(path):
            ...
            continue
        # non-SHA ref = WARN (§결정8 (viii)) — moving-HEAD/traversal 통과 후, name-based 라 tag/branch
        #   구분 불가 ∴ 그 외 non-SHA(tag/mutable-branch)는 수용하되 idempotency 미보장을 surfacing.
        #   deterministic 라인(rid/ns/ref = manifest 유래 constant, timestamp/nonce 0) → idempotent 2-run byte-identical.
        if not _SHA_REF_RE.match(ref):
            nonsha_warn.append((rid, ns, ref))
            print("::warning::check-infra-resource-drift: CROSS-REPO NON-SHA REF — resource-id=%s ns=%s "
                  "ref=%s 비-SHA ref — idempotent 미보장(mutable branch/재지정 tag). 엄밀 idempotency 는 "
                  "SHA-pin(^[0-9a-f]{7,64}$)만 보장 (§결정8 (viii))." % (rid, ns, ref))
    ...
    print("check-infra-resource-drift: cross-repo census cross_resources=%d propagated=%d "
          "content_mismatch=%d hard_unavailable=%d transient_failopen=%d nonsha_warn=%d"
          % (len(cross), len(propagated), len(drift), len(hard), len(transient), len(nonsha_warn)))

...
★ **본 모듈은 핀 값을 보유하지 않는다.** `PIN_ENVELOPE_SHA256` 리터럴의 거처는
  담지 테스트 `tests/scripts/test_cfp2978_envelope_pin.py` 의 **동명 모듈 상수**
  (대조 셀 `W-16.S`)이며, 이 모듈은 **산출자이자 대조기**일 뿐이다. 고정 수치를
  이 파일에 박으면 산출자와 기대값이 같은 파일에서 자기정합을 이루어 `UM-16`
  (참조 구현이 결함을 포함한 채 정본이 되는 위험)을 **키운다**.
  ★**앵커 규약 — 파일경로 + 심볼명으로만 적는다**(행번호 리터럴 금지: 이동참조로
    썩는다). 구 문면은 형제 파일 `test_cfp2978_workflow_shape.py` 를 지목했는데,
    그 파일은 **실재하되 핀 문자열을 0회 담는다** — **지목 대상이 실재하는** 앵커
    오류가 최악이다(감사자가 「이미 정합」으로 읽고 지나간다). 재확인 명령:
        grep -rnE 'PIN_ENVELOPE_SHA256[[:space:]]*=' --include='*.py' .
    산문 언급이 아닌 **대입문**만 세는 것이 거처 판정이며, 현행 산출은 **1건**이다.

#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""scripts/lib/check_design_entry_signoff_predicate.py

CFP-2725 Phase 2 / Change Plan §8 RTM AC-5 — 설계 진입 preflight 의 design-entry sign-off
predicate 확정 배선(landed content) presence lint. Python SSOT engine (thin wrapper =
scripts/check-design-entry-signoff-predicate.sh, ADR-061 §결정 1).

목적:
  요구사항리뷰 PASS 후·설계 진입 직전 preflight 의 사용자 최종 확정 gate rule 이
  docs/orchestrator-playbook.md 에 landed 되어 있는지(회귀/드리프트 방어)를 정적으로 검사.
  검사 대상 anchor:
    - `user-final-sign-off-resolved`  — 설계 진입 preflight 확정 predicate 앵커
    - `advisory ceiling`              — 확정 gate 의 정직 tier 라벨(기계강제 아님) 앵커
...
CLI 계약 (QADev self-test 가 이 계약에 의존):
  python3 check_design_entry_signoff_predicate.py [--root <repo_root>] [--self-test]
...
LINT_NAME = "design-entry-signoff-predicate"

# REQUIRED (target_rel_path, [anchor_literals]) — Change Plan §8 RTM AC-5 확정 배선 anchor.
REQUIREMENTS = [
    ("docs/orchestrator-playbook.md", ["user-final-sign-off-resolved", "advisory ceiling"]),
]
...
def run(root):
    try:
        ...
        return EXIT_SETUP
    if failures:
        print(f"[{LINT_NAME}] FAIL — design-entry sign-off predicate 확정 배선 presence 위반 "
              "(Change Plan §8 RTM AC-5 / target-existence + hollow-gate guard):")
        for msg in failures:
            print(f"  {msg}")

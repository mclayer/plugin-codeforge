...
★정직 — 본 게이트가 **검출하지 못하는** 축 (over-claim 금지):
  (a) **의미 정합 미검증**: 필드 *설명* 문면의 정확성은 보지 않는다. 이름·타입 토큰·행
      번호·개수만 본다 (실제로 현행 계약 §2 는 `writer_key`/`artifact_key` 의 **설명이
      서로 뒤바뀌어** 있는데 — §7.5.3 은 writer_key=sha256(agent_id) — 본 게이트는 이를
      잡지 못한다. 문서 소유 밖이라 본 Story 워커는 미수정, 관찰만 declare).

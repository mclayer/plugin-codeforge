#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""scripts/lib/check_confirmation_record_schema_resume.py

CFP-2725 Phase 2 / Change Plan §8 RTM AC-7 — 사용자 최종 확정 기록 schema(Story §5.5) +
세션 재개 복원 절차의 확정 배선 presence lint. Python SSOT engine
(thin wrapper = scripts/check-confirmation-record-schema-resume.sh, ADR-061 §결정 1).

목적:
  design-entry 확정의 기록 SSOT(확정 발화 verbatim primary / 양채널 mirror) + 세션 재개 시
  확정 상태 복원(확정 여부 + 미해소 질문 목록)이 각 governance 문서에 landed 되어 있는지
  (회귀/드리프트 방어)를 정적으로 검사.
...
CLI 계약 (QADev self-test 가 이 계약에 의존):
  python3 check_confirmation_record_schema_resume.py [--root <repo_root>] [--self-test]
...
LINT_NAME = "confirmation-record-schema-resume"

# REQUIRED (target_rel_path, [anchor_literals]) — Change Plan §8 RTM AC-7 확정 배선 anchor (2 target).
REQUIREMENTS = [
    ("templates/story-page-structure.md", ["확정 발화 verbatim", "양채널 mirror"]),
    ("skills/session-recovery/SKILL.md", ["확정 상태 복원", "미해소 질문 목록"]),
]
...
def run(root):
    try:
        ...
        return EXIT_SETUP
    if failures:
        print(f"[{LINT_NAME}] FAIL — 확정 기록 schema + 세션 재개 복원 확정 배선 presence 위반 "
              "(Change Plan §8 RTM AC-7 / target-existence + hollow-gate guard):")
        for msg in failures:
            print(f"  {msg}")

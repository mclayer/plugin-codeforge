...
# (scripts/lib/measure_chief_author_span.py SSOT)
# ADR-060 §결정 5 — warning-tier (exit 0 항상 for warnings, PR merge 미차단)
#
# Chief author span telemetry mechanical lint.
# Detection scope:
#   Check (a) — chief author spawn entry telemetry marker presence (PRIMARY):
#     same entry block 안 `[chief-author-span: <minutes>, <class>]` inline marker OR
#     `chief_author_span_minutes:` YAML-like field presence 검증
#     → 부재 시 [WARN-TELEMETRY-MARKER-ABSENT]
#   Check (b) — long-span chief author spawn (≥ 10 min, env CFP_CHIEF_AUTHOR_SPAN_MAX_MIN)
#     with class `monolithic` (not `multi_step_N`) → recommendation tier warning
#     → 위반 시 [WARN-LONG-SPAN-MONOLITHIC]
#
# Bypass channel: HOTFIX_BYPASS_CHIEF_AUTHOR_SPAN_TELEMETRY=1 env
#   → 즉시 exit 0 (hotfix-bypass:chief-author-span-telemetry label 부착 시 workflow 에서 주입)
#
# 검증 대상 file 수집 (check-mid-spawn-drift-detection.sh collect_files 패턴 답습):
...
SCRIPT_NAME="[chief-author-span-telemetry-lint]"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

# ── bypass env 확인 ───────────────────────────────────────────────────────────
BYPASS="${HOTFIX_BYPASS_CHIEF_AUTHOR_SPAN_TELEMETRY:-}"
if [[ "$BYPASS" == "1" ]]; then
...
collect_files() {
  ...
  if [[ $# -gt 0 ]]; then
    ...
    # git diff 기반 자동 감지 (check-mid-spawn-drift-detection.sh 패턴 답습)
...
main() {
  ...
  exec python3 "$SCRIPT_DIR/lib/measure_chief_author_span.py" "${files[@]}"
}

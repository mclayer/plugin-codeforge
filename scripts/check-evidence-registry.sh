#!/usr/bin/env bash
# CFP-455 / ADR-060 Amendment 2 / evidence-check-registry v1.1 — registry yaml schema validation lint
#
...
#
# 인자:
#   $@ : 검증 대상 registry yaml file path (없으면 docs/evidence-checks-registry.yaml)
#
# 명령 라인:
...
# 인자 없을 시 codeforge repo root 로 이동 후 default path 사용.
if [ "$#" -eq 0 ]; then
    cd "$(dirname "$0")/.."
fi

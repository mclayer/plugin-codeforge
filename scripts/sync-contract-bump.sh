...
# CFP-478 / ADR-061 §결정 1 + Amendment 1 §결정 6.A — Python MANIFEST lookup 외부화
#   (scripts/lib/sync_contract_bump.py SSOT, mode_preserve: 100755)
#
# inter-plugin contract version bump 시 3단계 sync PR sequence 자동 생성.
# Usage / 인자 / exit code: 파일 상단 주석 참조.
set -uo pipefail
...
# CFP-478: 외부 .py 위임 (ADR-061 §결정 1 정합)
LOOKUP_RESULT=$(python3 "$SCRIPT_DIR/lib/sync_contract_bump.py" "$CONTRACT" "$MANIFEST" 2>&1)
LOOKUP_RC=$?

#!/usr/bin/env bash
# bootstrap-labels.sh — Plugin이 사용하는 GitHub label 31종 일괄 생성 (1회).
#
...
# monitoring:* (1종 — CFP-393 v2.2 신설 tier, ADR-057 Amendment 2 / ADR-060)
# KPI / metric / dashboard / alert 영역. 기존 `audit` (후처리 분류) 와 분리.
# rate-limit-fallback-kpi.yml workflow 가 sample_size_sufficient=true AND fallback_rate_percent >= 1.0% 시 자동 부착.
...
if [ $DRY_RUN -eq 0 ]; then
    echo ""
    echo "✓ 31 base label + component:* (project.yaml.labels.components[] 동적) 처리 완료. 'gh label list' 로 확인."
fi

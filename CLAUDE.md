...
> **요구사항리뷰 lane 도입 근거 (서술만)**: 요구사항 결론은 외부 개념·시장·표준 사실에 가장 자주 의존하므로, 요구사항→**요구사항리뷰**→설계 시퀀스로 외부사실 의존성을 설계 진입 전에 검증한다 (외부지식 충당 3-단계 중 깊은 검증 단계의 주 발동 lane). **lane 개수 ≠ plugin 개수** — `codeforge-review` 부품 하나가 이미 설계리뷰·구현리뷰·보안테스트 다수 lane 을 host 한다 (1 plugin 다 lane). 근거 SSOT = ADR-124 (외부지식 충당 3-단계 모델). **실 lane 시퀀스·카운트 wiring (8→9) = CFP-2325 S2** (여기선 근거 서술만, 숫자 hard-commit 미포함).

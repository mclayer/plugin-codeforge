...
- **요구사항**: Orchestrator가 사용자 요건 접수 → Jira Story 생성 → DocsAgent가 Confluence Story 페이지 생성(§1-2) → PMOAgent 아래 **순차** (PMAgent 조건부 → Analyst 필수 → Researcher 조건부) 통합 명세서 확정 → Story 페이지 §3-6 갱신
...
```
[요구사항] Orchestrator → PMOAgent → **순차** (PMAgent 도메인 해석 조건부 → RequirementsAnalyst 필수 → Researcher 조건부) → PMOAgent 통합
        · Analyst가 "Researcher 리서치 키워드" 필드를 생성 → 키워드 존재 시에만 Researcher 스폰
...
### 병렬 스폰 권장
- 요구사항: **순차 원칙** — PMAgent(조건부) → RequirementsAnalyst(필수) → Researcher(조건부). Analyst가 Researcher 키워드를 생성하므로 병렬 불가

...
- **Write queue 의뢰 권한** (`.claude-work/doc-queue/**`): RequirementsPLAgent, ArchitectAgent, ArchitectPLAgent, DomainAgent, PMOAgent, SecurityArchitectAgent, TestContractArchitectAgent, DataMigrationArchitectAgent, CodebaseMapper, Refactor, DesignReviewPL, CodeReviewPL, SecurityTestPL, ClaudeReviewAgent, CodexReviewAgent, DeveloperPLAgent, RequirementsAnalyst, Researcher, TestAgent — 기타 Edit/Write는 owner agent path-scoped에 한정 (CFP-26 Phase 0a: ArchitectAgent → docs/change-plans/** + docs/adr/**, DomainAgent → docs/domain-knowledge/**, PMOAgent → docs/retros/**). Story file 섹션 갱신·GitHub comment 등 multi-writer/lifecycle 영역은 여전히 DocsAgent 경유

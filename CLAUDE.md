...
**필수 플러그인 (6종)**:
- `codeforge-review@mclayer` (>= 1.0.0) — review subsystem (3 PL + 2 worker + base + 3 checklist). CFP-35 ζ arc retrofit 부터 v2 self-write (Story §9 + GitHub comment + gate label 직접). CFP-29 추출 (BREAKING v0.17.0) + CFP-35 retrofit (BREAKING v1.0.0). 미설치 시 설계 리뷰·구현 리뷰·보안 테스트 lane 진행 불가
- `codeforge-pmo@mclayer` (>= 0.1.0) — PMO lane plugin (PMOAgent 단독). Cross-Story 회고 감사 + Epic milestone 갱신 + ADR 후보 발의. CFP-36 ζ arc 추출. 미설치 시 PMO Cross-cutting 책임 불가 (Cross-Story 패턴 분석·ADR 후보 발의·Epic 분해 자문)
...
## Development Agent Team (18 core 에이전트 + codeforge-review plugin 5 agent + `role: dev` 동적 roster · 7 레인 + 2 Cross-cutting)
...
- **Write queue 의뢰 권한** (`.claude-work/doc-queue/**`): RequirementsPLAgent, ArchitectAgent, ArchitectPLAgent, DomainAgent, PMOAgent, SecurityArchitectAgent, TestContractArchitectAgent, DataMigrationArchitectAgent, CodebaseMapper, Refactor, DeveloperPLAgent, RequirementsAnalyst, Researcher, TestAgent — 기타 Edit/Write는 owner agent path-scoped에 한정 (CFP-26 Phase 0a: ArchitectAgent → docs/change-plans/** + docs/adr/**, DomainAgent → docs/domain-knowledge/**, PMOAgent → docs/retros/**). Story file 섹션 갱신·GitHub comment 등 multi-writer/lifecycle 영역은 여전히 DocsAgent 경유. **Review 5 agent (DesignReviewPL · CodeReviewPL · SecurityTestPL · ClaudeReviewAgent · CodexReviewAgent)는 별도 plugin codeforge-review로 추출 (CFP-29 Phase 1) — 본 listing에서 제외**, 그쪽 plugin의 frontmatter가 SSOT

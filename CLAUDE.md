...
확인·자동복구·blocking-wait 절차 SSOT: [playbook §1.1](docs/orchestrator-playbook.md) checklist 0번 (MCP `ToolSearch` 노출 / settings.json 토글 / `/mcp` 재인증·`/plugins install` 요구 / consumer `.github/` 6 workflow + 3 forms + CODEOWNERS 부재 알림 / **codeforge plugin family version drift 검사 — CFP-262 / [ADR-037](docs/adr/ADR-037-plugin-version-bump-rule.md)**: MAJOR drift = hard-stop blocking, `bash ${CLAUDE_PLUGIN_ROOT}/codeforge/scripts/check-codeforge-version-drift.sh` / **Deferred tool 스키마 선제 로드 (0i) — SessionStart hook tier 위임 ([ADR-038 Amendment 2 §결정 9](docs/adr/ADR-038-progress-visualization-todowrite.md), CFP-500)**: consumer `.claude/settings.json` `hooks.SessionStart[]` 에 `SessionStart-codeforge-prereq-check.json.sample` 등록 → 첫 turn prompt-injection 으로 `ToolSearch("select:TodoWrite")` 지시. hook 미등록 / 실행 실패 시 runtime ToolSearch 폴백 (§결정 8 retain) + 재실패 시 경고 후 계속 (§결정 7 layered defense). 초기 preload list = TodoWrite 단독 (보수적, 확장은 별도 CFP). 선행 attempt = CFP-375 / CFP-385 (runtime tier 2회 실패) → 본 격상 3rd attempt. **해소 기준 metric** = TodoWrite InputValidationError 발생률 < 5건/100세션 (Amendment 2 amendment_log `sunset_justification` 3-tuple — metric/who/how, [ADR-058](docs/adr/ADR-058-adr-sunset-criteria-mandate.md) §결정 5 정합).

**SessionStart hook actual wire — worktree-gc (CFP-427 / [ADR-040](docs/adr/ADR-040-worktree-convention.md) §결정 5)**: wrapper repo `.claude/settings.json` `hooks.SessionStart[]` array 두 번째 entry = `bash "${CLAUDE_PROJECT_DIR}/templates/scripts/check-worktree-stale.sh" || true` (CFP-427 Story 2 merge 후 effective). 매 wrapper repo Claude Code session 시작 시 stale worktree (mtime ≥ 7 days + origin branch absent) 자동 prune. session 차단 0 (`|| true` postfix). bypass = `BYPASS_WORKTREE_GC=1` env (rare debugging only — `check-worktree-stale.sh` 단독 short-circuit, ADR-040 §결정 5 SSOT). 폐쇄루프 안전망 = `scripts/check-session-start-hook-presence.sh` (CFP-427 신설, warning tier) — hook wire 미적용 자체를 lint 대상으로 검증 (Researcher Unknown 1 폐쇄루프). prereq-check entry (CFP-500 산출물, 1번째 entry) 와 disjoint scope — 양쪽 entry 모두 sequential 실행.

**Git layer 안전망 (CFP-428 / [ADR-040](docs/adr/ADR-040-worktree-convention.md) Amendment 3 §결정 7.D)**: wrapper repo `templates/.git-hooks/{pre-checkout,pre-commit-main-block}.sample` + `scripts/install-git-hooks.sh` opt-in installer 도입 (CFP-428 Story 3 merge 후 effective). main working tree 에서 lane branch (`^cfp-[0-9]+`) checkout / `src/**` 또는 `docs/**` commit 시도 시 warning 출력 (warning tier — exit 0 차단 0). `BYPASS_WORKTREE_FIRST=1` env 즉시 disable (4 evidence-check entry 공통 — `BYPASS_WORKTREE_GC` 와 disjoint scope, ADR-040 §결정 7.E). `--git-dir` vs `--git-common-dir` 분기로 worktree 내부 false-positive 회피 (Researcher Unknown 2 해소 — main = `git_dir == common_dir`, worktree = `git_dir != common_dir`). install 강제 0 (사용자 명시적 `bash scripts/install-git-hooks.sh` 호출 전까지 wrapper 자체 git hook 미활성, idempotent symlink installer). lint pattern = verification-only (sample 존재 + executable bit 검증 — Story 2 exec swap 패턴 부적용, canonical 이 git hook sample 자체). CLI-only enforcement 가정 (Pro Git book §8.3 hooks inherent — IDE/GUI git client (VSCode / IntelliJ / SourceTree / GitKraken) 우회 = warning tier scope 안 acceptable, Story 4 blocking-on-pr 승격 시 재평가). 폐쇄루프 안전망 = `scripts/check-worktree-first-{pre-checkout,pre-commit-main-block}.sh` (CFP-428 actual logic 전환, warning tier). Story 4 (CFP-429) 가 의무화 plan 명세 + ADR-060 §결정 6 promotion gate 평가.
...
- **Worktree-first mechanical enforcement (CFP-426 / ADR-040 Amendment 3 §결정 7)**: normative 선언 + mechanical action mapping 의무. 4 evidence-check entry warning tier 활성 — `worktree-first-{session-start-wire, pre-checkout, pre-commit-main-block, spawn-evidence-cwd}` (skeleton — actual wire 는 CFP-427/428/429 후속). frontmatter `mechanical_enforcement_actions[]` schema = ADR-040 Amendment 3 §결정 7.A (list[object] verbatim). retroactive 면제 명시 (§결정 7.C — 본 Amendment 3 이후 신설/Amendment ADR 만). 4 hotfix-bypass label = `hotfix-bypass:worktree-{session-start-wire, pre-checkout, pre-commit-main-block, spawn-evidence-cwd}` (per-entry namespace, ADR-024 Amendment 3 §결정 6.A 정합). `BYPASS_WORKTREE_FIRST=1` env = `BYPASS_WORKTREE_GC=1` 와 disjoint scope (§결정 7.E).
...
- [evidence-check-registry-v1.md](docs/inter-plugin-contracts/evidence-check-registry-v1.md) (v1.1 — Active) — evidence-enforceable governance check schema (CFP-389 / ADR-060). registry data SSOT = [`docs/evidence-checks-registry.yaml`](docs/evidence-checks-registry.yaml). MANIFEST `registries:` entry = `evidence_check_registry`. **CFP-390 Amendment 1 (2026-05-11)**: 18 entry 인벤토리 backfill SSOT (ADR-060 §결정 13) — Phase 1 = SSOT 만, Phase 2 = row append. **CFP-426 (2026-05-12)**: 4 worktree-first entry skeleton 추가 (warning tier, ADR-040 Amendment 3 §결정 7.D self-application 첫 사례) — `worktree-first-{session-start-wire, pre-checkout, pre-commit-main-block, spawn-evidence-cwd}`. **CFP-455 / ADR-060 Amendment 2 (2026-05-12)**: schema v1.0 → v1.1 MINOR bump — `current_tier` 필드 optional → required 전환 + 4-tier enum 본문 강조 + 기존 22 entry retroactive 분류 검증 (모두 보유 verified, mechanical regression 0건). CFP-391 (#396) / CFP-412 (#412) closed without delivery 후 재재예약 carrier. 메타 lint = Phase 2 PR scope (`scripts/check-evidence-registry.sh` + `templates/github-workflows/evidence-registry-check.yml`, warning mode 첫 도입). **CFP-481 / ADR-060 Amendment 4 (2026-05-12)**: 3rd warning-tier entry `auto-phase-label` 등록 — PR open 시 phase label 자동 부착 workflow (ADR-024 Amendment 4 §결정 6.A.1 branch → phase mapping SSOT mechanical enforcement). CFP-455 + CFP-449 retro sentinel 2 재현 2회 → mechanical enforcement 도입 timing 도달.
...
**ArchitectAgent Phase 1 mechanical sync self-check 의무 (non-marketplace 영역)** = [ADR-065](docs/adr/ADR-065-architect-phase1-mechanical-self-check.md) — codeforge-design lane 의 ArchitectAgent (chief author) Phase 1 산출물 commit 직전 7-item mechanical sync checklist (label-registry sync / doc-locations regen / workflow self-app / link target Phase 분배 / MANIFEST.yaml 갱신 / section-ownership row / doc-locations row) self-check 의무. ArchitectPLAgent verdict packet `mechanical_self_check_passed: bool` 필드 (review-verdict-v4 v4.2 MINOR bump, ADR-008 §결정 2 정합) 로 explicit marker. false 시 FIX 의무 — ArchitectAgent re-spawn. marketplace 영역 self-check 는 ADR-063 SSOT (cross-ref only — 중복 codification 회피). 3 Story 누적 결함 evidence (CFP-393 iter 1 3건 + iter 3 1건 + CFP-411 phase-gate path).
...
`templates/github-workflows/` 21종 fixture: 9 core (`story-init.yml` · `phase-label-invariant.yml` · `story-section-1-immutable.yml` · `subissue-from-impl-manifest.yml` · `phase-gate-mergeable.yml` · `fix-ledger-sync.yml` · `post-merge-followup.yml` (ADR-026 / CFP-74) · `retro-mandatory.yml` ([ADR-045](docs/adr/ADR-045-story-retro-mandatory-trigger.md) / CFP-138 — Story 완료 회고 forcing function, `gate:retro-complete` close-blocking) · `parallel-epic-conflict-check.yml` (ADR-050 — 병렬 에픽 파일 충돌 감지, non-blocking)) + 3 evidence-enforceable warning (`adr-sunset-criteria.yml` — CFP-389 / ADR-060 §결정 5, ADR-058 mechanical lint, `continue-on-error: true`, `hotfix-bypass:adr-sunset` label 부착 PR conditional skip + audit comment 자동 발의; `decision-principle-vocabulary.yml` — CFP-449 / ADR-060 §결정 5, ADR-064 §결정 2 forbid-list 8 어휘 mechanical lint, `continue-on-error: true`, `hotfix-bypass:decision-principle-vocab` label conditional skip + audit comment 자동 발의; `auto-phase-label.yml` — CFP-481 / ADR-060 Amendment 4 §결정 5, ADR-024 Amendment 4 §결정 6.A.1 branch → phase mapping SSOT mechanical enforcement, `pull_request:opened` 단일 trigger + 4-tier inference fallback (branch parse / Related Issue inherit / terminal default / unclassified+warning), `continue-on-error: true`, `hotfix-bypass:auto-phase-label` label conditional skip + audit comment 자동 발의 + PR body marker `<!-- attached_by_auto: true -->` idempotency dedup) + 1 atomic invariant blocking (`version-bump-atomic-check.yml` — CFP-441 / ADR-063 §결정 5, plugin.json mirrored field 변경 시 3-file atomic (plugin.json + CHANGELOG.md + marketplace.json) verify, `hotfix-bypass:marketplace-atomic` label conditional skip + 24h sync 의무 audit comment) + 4 worktree-first warning (`worktree-first-{session-start-wire, pre-checkout, pre-commit-main-block, spawn-evidence-cwd}.yml` — CFP-426 / ADR-040 Amendment 3 §결정 7, Story 1 skeleton — actual wire 는 CFP-427/428/429 후속, `hotfix-bypass:worktree-*` per-entry label + `check-bypass-audit-comment.sh` reuse) + 4 Live touching reusable workflow (`live-test-guard.yml` · `live-deploy-approval.yml` · `live-secret-policy.yml` · `kill-switch-integration-test.yml`, CFP-C). Issue Forms / branch protection / 버그 기록 등 상세 hierarchy + label 분류 + 코멘트 규칙 SSOT: [`docs/consumer-guide.md`](docs/consumer-guide.md) §1.3 + [`label-registry-v2`](docs/inter-plugin-contracts/label-registry-v2.md) + [`comment-prefix-registry-v1`](docs/inter-plugin-contracts/comment-prefix-registry-v1.md).

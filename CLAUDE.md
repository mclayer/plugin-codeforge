...
## 정체
codeforge = Claude Code 범용 SW 개발 오케스트레이션 플러그인. **wrapper-only (자체 에이전트 0개)** — 최상위 Claude 세션(Orchestrator)이 8개 lane plugin 의 에이전트를 spawn 해 요구사항 접수부터 배포 리뷰까지 진행한다.

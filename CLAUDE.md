...
### Verify-before-assert (ADR-073, CFP-622)

Orchestrator (또는 subagent) 가 cross-repo state / sibling plugin file path / assumption 단정 발화 시 의무: (1) `cd <repo> && git fetch origin` 선행 (2) `git show origin/main:<path>` 또는 `gh issue/pr view --json state` direct verify (3) 인용 옆 `verified-via: <method>` annotation (4) spec/plan frontmatter 안 `pre_lookup_evidence[]` PL 수동 declaration. **Inline whitelist (ADR-039 §결정 2) 영역 단순 file stat 는 inline 허용** — 단정 발화 시만 verify 의무. Mechanism enumeration: M1 same-repo working tree mutation lag (CFP-597 strike #1) + M2 cross-repo origin lag (CFP-622 strike #2). 3-layer coherence: ADR-070 (external worker) + **ADR-073 (self-assertion)** + ADR-071 (dialog convergence) — super-class "Orchestrator self-discipline ratchet" children. SSOT = [ADR-073](docs/adr/ADR-073-orchestrator-verify-before-assert.md).

...
codeforge ζ arc Design lane plugin (LAST extraction). 7 permanent + 3+1 CONDITIONAL SubAgent + 4-tuple sub-tuple + change-plan/adr templates. CFP-1086 / ADR-042 Amendment 8 — BackendArchEpic Phase 2 design lane 7+3+1 roster 재편 (AggregateArch + APIContractArch 신설 / CodeArch → ModuleArch rename / DataArch mandate 축소 RDB OLTP 영역 제거 → 빅데이터 OLAP only).
...
## 7 permanent + 3+1 CONDITIONAL SubAgent + 4-tuple sub-tuple (CFP-1086 — ADR-042 Amendment 8 / ADR-068 Amendment 2 / ADR-086 신설)

**SubAgent 수 = 7 permanent + 3+1 CONDITIONAL + 4-tuple sub-tuple**. wrapper SSOT (CLAUDE.md "Deputy mandate 매트릭스" / skills/deputy-mandate/SKILL.md) 와 byte-consistent.

**7 permanent deputy** (모든 설계 lane 진입 시 병렬 spawn — CONDITIONAL applicability 확인):
...
| **ModuleArchitectAgent** (CFP-1086 Amendment 8 — CodeArch rename + mandate 정정, 도메인 모델 invariant 영역 AggregateArch 분리) | §3 code module-level 구조 advocate — single-mandate | "module / package boundary + dependency direction + layered/hexagonal/clean module-level 어떻게 정합인가" | Sonnet |
| **AggregateArchitectAgent** (CFP-1086 Amendment 8 신설) | RDB OLTP aggregate invariant 변호자 — single-mandate | "aggregate boundary 어디인가, 트랜잭션 경계 어디까지 atomic 인가, Alembic 정책 7 원칙 (양방향 호환 / 확장-정리 분리 / reverse / smoke / cross-repo / 백업 / hard limit) 정합인가" | Sonnet |
...
| **AggregateArch CONDITIONAL applicability** (CFP-1086 P2 신설 — 3+1) | `project.yaml aggregate_arch.applicable: bool` (default `true`). non-applicable consumer = frontend-only / API-only / external-managed RDB | "본 consumer 가 RDB OLTP schema 제어권 보유하는가, AggregateArch 영역이 mandate 충족 가능한가" |
...
ArchitectPLAgent 가 7 (또는 9/10/11) SubAgent 를 **병렬 spawn**. 대립 참여 = CodebaseMapper (4-tuple) ↔ Refactor (4-tuple) ↔ SecurityArch ↔ DataArch (OLAP 영역) / AggregateArch (RDB OLTP 영역). TestContractArch / InfraOperationalArch / ModuleArch / APIContractArch = single-mandate advocacy / contributor 단일 축 — 대립 비참여. LiveOps / LiveOrdering / ProductionEvidence = CONDITIONAL contributor 단일 축 — 대립 비참여.

**CFP-1086 정합 4-way 영역 분리**:
- **RDB OLTP 영역 대립**: CodebaseMapper ↔ Refactor ↔ SecurityArch ↔ AggregateArch (4-way)
- **빅데이터 OLAP 영역 대립**: CodebaseMapper ↔ Refactor ↔ SecurityArch ↔ DataArch (4-way)
- **Cross-layer (ELT/ETL/CDC) 영역**: AggregateArch + DataArch co-author (deferred carrier — sibling Epic 산출 후 결정)
...
**DesignReviewPL 교차 체크**: ArchitectAgent 통합 판정 + ArchitectPLAgent 검수가 각 변호 근거를 근거 있게 일축·수용했는가 / 요건 범위를 넘지 않았는가 / §7 보안 설계와 §7.4 운영 리스크와 §11 데이터 마이그레이션이 충실히 반영되었는가 / AggregateArch ↔ DataArch cross-layer boundary 명시했는가.
...
| Sub-axis ↓ \\ Cross-axis → | Aggregate (R/C/I) | Data OLAP (R/C/I) | Module (R/C/I) | APIContract (R/C/I) |
|---|---|---|---|---|
| **Security** | R=SecurityArch / C=AggregateArch / I=TestContract | R=SecurityArch / C=DataArch / I=TestContract | R=SecurityArch / C=ModuleArch / I=InfraOp | R=SecurityArch / C=APIContract / I=InfraOp |
| **InfraOp** | R=InfraOp / C=AggregateArch / I=TestContract | R=InfraOp / C=DataArch / I=TestContract | R=InfraOp / C=ModuleArch / I=SecurityArch | R=InfraOp / C=APIContract / I=TestContract |
| **TestContract** | R=TestContract / C=AggregateArch / I=InfraOp | R=TestContract / C=DataArch / I=SecurityArch | R=TestContract / C=ModuleArch / I=APIContract | **R=APIContract** / C=TestContract / I=InfraOp |
...
## Sub-agent fan-out (ArchitectPL → 7 permanent + 3+1 CONDITIONAL SubAgent 병렬 + 4-tuple sub-tuple)
...
| **AggregateArchitectAgent** (deputy, CFP-1086 신설) | 설계 lane 진입 즉시 (CONDITIONAL applicability `project.yaml aggregate_arch.applicable` 확인) | **§3 aggregate (DDD aggregate boundary + 트랜잭션 경계 + persistence-bound) + §11.1-§11.6 RDB OLTP (Alembic 정책 7 원칙: 양방향 호환 / 확장-정리 분리 / reverse / smoke / cross-repo / 백업 / hard limit)** |
...
| **InfraOperationalArchitectAgent** (deputy, CFP-1026 S1 rename) | 설계 lane 진입 즉시 | **§7.4 운영 리스크 6 항목 + §11.6 idempotency consult (AggregateArch primary 영역 협업)** |
| **ModuleArchitectAgent** (deputy, CFP-1086 CodeArch rename + mandate 정정) | 설계 lane 진입 즉시 | **§3 code module-level (module boundary + dependency direction + layered/hexagonal/clean module-level + DDD bounded context module placement)** |
...
ArchitectPLAgent prompt (CFP-1086 정합):
- **Backtest/Paper-only Story (default, AggregateArch applicable=true)**: "7 permanent deputy 병렬 spawn (SecurityArch / InfraOperationalArch / TestContractArch / DataArch / ModuleArch / AggregateArch / APIContractArch) + 3 4-tuple sub-tuple (CodebaseMapper / Refactor / ArchitectAnalyst) flat spawn (ArchitectAgent chief 포함). ArchitectAgent (chief author) 가 10 (7 + 3 sub-tuple) 산출물 통합 후 Change Plan §1-§13 author."
- **Frontend-only / API-only / external-managed consumer (`project.yaml aggregate_arch.applicable: false`)**: "AggregateArch 미spawn (CONDITIONAL applicability false). 6 permanent deputy + 3 sub-tuple = 9 산출물 통합."
- **Live touching Story (CFP-77 CONDITIONAL active)**: "위 + LiveOpsDeputy + LiveOrderingDeputy spawn. ArchitectAgent chief 가 12 (또는 AggregateArch non-applicable 시 11) 산출물 통합 후 Change Plan §1-§13 + Story §13 author."
- **Production cutover Story (ADR-72 CONDITIONAL active)**: "위 + ProductionEvidenceDeputy spawn (wrapper-self-app N/A 시 미spawn). ArchitectAgent chief 가 13 (또는 AggregateArch non-applicable 시 12) 산출물 통합."

CONDITIONAL trigger 판정 (ArchitectPL 의 의무):
- AggregateArch applicability: consumer `project.yaml aggregate_arch.applicable: bool` 확인 (default `true`). 미정의 시 `true` 가정. frontend-only / API-only / external-managed RDB consumer 만 `false`

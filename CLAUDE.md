...
조건부 생략: DomainAgent(RequirementsPL 판정 — "요건 이미 명확" 명시 시), ResearcherAgent(RequirementsPL 판정 — Analyst 키워드 비어있을 때), `role: dev` 에이전트(Change Plan이 해당 에이전트 소유 경로 미변경 시).

...
**Objective SSOT 상수** (Atlassian·GitHub 좌표, label taxonomy)는 **`.claude/_overlay/project.yaml`** 에 structured로 기재. 에이전트는 해당 파일을 `Read`로 직접 참조. Schema: [`docs/project-config-schema.md`](docs/project-config-schema.md). Narrative 컨텍스트(도메인 해설·기술 스택 근거)는 `.claude/_overlay/CLAUDE.md`에 기재.
...
- **요구사항**: Orchestrator가 사용자 요건 접수 → Jira Story 생성 → DocsAgent가 Confluence Story 페이지 생성 (§1 verbatim, §2·§5·§6 placeholder) → RequirementsPLAgent 아래 **병렬** (DomainAgent · Analyst · Researcher 동시 스폰, 각자 독립 관점) → RequirementsPL이 세 결과 dedup·상충 조정 → DocsAgent 경유 §2·§5·§6 동시 채움 + §3-4 갱신
- **설계**: Architect가 CodebaseMapper(변호자) · Refactor(혁신자) **병렬 스폰** → 두 독립 관점 대립 조정 → Change Plan 확정 (§8 Test Contract 포함) → DocsAgent가 `docs/change-plans/<slug>.md` 저장 + Story 페이지 §7 미러링
- **설계 리뷰**: DesignReviewPL이 Claude/Codex 설계 리뷰 종합 → PASS 시 구현 진입 / FIX 시 Architect 회귀 (최대 3회)
- **구현**: Orchestrator가 QADev + DeveloperPL 병렬 스폰. DevPL이 프로젝트 `role: dev` roster를 동적 discover해 의존성 없는 한 **모두 병렬** 스폰. Architect는 stateless 재스폰되어 매핑표 감사
...
**Project Config Packet** (DocsAgent·RequirementsPL·DomainAgent·PMO): `.claude/_overlay/project.yaml` slice도 packet으로 주입 → Atlassian/GitHub 호출 에이전트의 반복 `Read` 회피. 상세는 playbook §12.5.

**Story 페이지 위치**: `.claude/_overlay/project.yaml` `atlassian.confluence.{space_key, stories_parent_page_id}`. 각 Story는 `<PROJECT_KEY>-N: <제목>` 페이지 1개.
...
- **Write 권한 있음**: `role: dev` 에이전트별 개별 scoping (core: DeveloperAgent `src/**` 기본, DataEng 프로젝트별 데이터 계층 경로, InfraEngineer `deploy/**`·`config/**`·`scripts/**`; preset·overlay가 경로 재정의), QADev(`tests/**`), **DocsAgent(`docs/**` + `.claude-work/doc-queue/**` + Jira/Confluence MCP 전용, `src`·`tests`·`.claude`·`config`·`deploy`·`scripts` 명시 deny)**
...
**DocsAgent만이 Jira·Confluence·docs/** write 가능**. 나머지 에이전트는 모두 문서 write 권한 없음. 문서 작업은 전원 **file-based write queue**(`.claude-work/doc-queue/<story>/`)에 의뢰 파일을 append → Orchestrator가 DocsAgent 스폰 시 drain. 상세는 playbook §11.
...
- Space: `project.yaml` `atlassian.confluence.space_key` / 루트 page pageId = `atlassian.confluence.adr_root_page_id` / 카테고리 parent 하위
...
## 버그 기록 (Jira)
- 프로젝트: `project.yaml` `atlassian.jira.project_key` (키 = 카테고리 구분 없음, label로 분류)
...
프로젝트마다 transition ID가 다름. DocsAgent가 세션 시작 시 `getTransitionsForJiraIssue(issueIdOrKey)` 호출해 동적으로 획득. `project.yaml` `atlassian.jira.transitions`에 고정 매핑이 있으면 우선 적용.
...
- **요구사항·컨텍스트·서사**: Confluence Story 페이지 (parent = `project.yaml` `atlassian.confluence.stories_parent_page_id`). 섹션 1-11
...
- Space: `project.yaml` `atlassian.confluence.space_key` / parent pageId = `stories_parent_page_id` / 각 Story `<PROJECT_KEY>-N: <제목>` 페이지 1개
- Template: consumer overlay 지정 Story Page 템플릿 복제해 신규 생성 (템플릿 pageId는 프로젝트별, consumer가 지정)

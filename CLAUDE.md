...
※ 본 구조는 v0.11.0(CFP-17 머지) 직후부터 발효. 머지 시점 이전에 진행 중이던 Story는 [`docs/superpowers/specs/2026-04-27-cfp-17-architectpl-securityarch-design.md`](docs/superpowers/specs/2026-04-27-cfp-17-architectpl-securityarch-design.md) §8.1 paradox 처리에 따라 옛 구조로 완료.
...
- **설계**: ArchitectPLAgent가 CodebaseMapper(변호자) · Refactor(혁신자) · SecurityArchitectAgent(위협 변호자) **병렬 스폰** → ArchitectAgent(chief author)가 통합 → Change Plan 확정 (§7 보안 설계 + §8 Test Contract 포함) → DocsAgent가 `docs/change-plans/<slug>.md` 저장 + Story file §7 미러링
...
모든 deputy (CodebaseMapperAgent · RefactorAgent · SecurityArchitectAgent) 공통:
...
- **ArchitectAgent (chief author)** 도 각 설계 레인 진입마다 stateless 재스폰 — 3 deputy 산출물을 입력으로 수령 후 Change Plan §1-§10 author 수행. ArchitectPLAgent RETURN 시에도 재스폰
...
- **설계**: **CodebaseMapper · Refactor · SecurityArchitectAgent 병렬** — 3 deputy 모두 원 소스 직접 읽기, 한쪽이 다른 쪽의 요약에 의존하지 않음. ArchitectAgent (chief author)가 통합, ArchitectPLAgent가 검수

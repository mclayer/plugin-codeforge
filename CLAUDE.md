...
## 필수 의존성 (세션 개시 확인)
- Orchestrator 모델 = 별칭 `opus` (최신 Opus tier). Sonnet/Haiku 세션이면 중단 후 재시작 — **단 이 '중단' 규범은 Orchestrator 세션 scope 한정** ('## opus default + ADR-141 Amendment 1 carve-out' 의 guard 참조; Amendment 1 carve-out non-opus subagent 는 정상 — self-refuse 대상 아님).
...
## opus default + ADR-141 Amendment 1 carve-out (ADR-141)
codeforge family 의 **model tier = `opus` default + ADR-141 Amendment 1 carve-out(외부위임·기계 워커 7종 haiku; 1M native 는 opus 기준)**. fable·sonnet tier·3-tier 선택 기준·rate-limit fallback 은 폐기(ADR-141 §결정1/2/3) — haiku 는 Amendment 1 carve-out 로 7종에 의도 배정. **'전 에이전트 opus 단일 tier'·'Sonnet/Haiku 세션 중단' 규범은 Orchestrator 세션·거버넌스 default scope 이다. Amendment 1 carve-out non-opus(haiku) subagent 는 정상 상태이며 self-refuse 대상 아님**(#846 정산). Orchestrator 세션 모델 opus mandate 도 본 정책의 자연 귀결(ADR-141 §결정4, 구 ADR-057 §결정1 흡수). opus rate-limit(429) 대응은 [ADR-109](archive/adr/ADR-109-in-process-429-mitigation-framework.md) 소관(fallback tier 없음). 신규 agent = `model: opus` default, 단일 tier 이탈 = ADR-141 amendment 의무. 상세 = [ADR-141](archive/adr/ADR-141-all-opus-single-tier.md).

...
**Wave 4 (CFP-582 / [ADR-059 Amendment 2](docs/adr/ADR-059-debate-protocol-v1.md) / debate-protocol-v1 v1.2)**: DesignLane internal blanket trigger 신설 — cross-module Story (`touched_top_level_paths >= 2` OR `touched_lanes >= 2`) 진입 시 dispatch_mode `blanket_cross_module_designlane` 자동 활성 (ArchitectAgent 산출물 Codex worker 검증 + Opus divergence 시 multi-round debate). 4-value enum precedence `auto_on_divergence > blanket_cross_module_designlane > mechanical_fast_path_inline > user_request_only`. touched paths 산정 = Story §1 spec_links + Change Plan §2 영향 영역 union. **convergence_quality_invariant**: 3 marker 의무 출력 — `agreement_reached: bool` / `convergence_signal: "natural" | "forced" | "escalated"` / `unresolved_concerns[]`. PL verdict 작성 시 모순 (예: `agreement_reached: true` + non-empty `unresolved_concerns`) 검출 시 force_continue. **Touchpoint #2 carry-over (§결정 9)**: ArchitectAgent §3 직후 Codex proactive check (ADR-052 #2) 결과가 동일 Story 안 blanket debate 진입 시 transcript `prior_codex_findings[]` verbatim carry-over (Round 0 input 첨부, double-spawn 회피).

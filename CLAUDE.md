...
- [evidence-check-registry-v1.md](docs/inter-plugin-contracts/evidence-check-registry-v1.md) — evidence-enforceable governance check schema (CFP-389 / ADR-060). registry data SSOT = [`docs/evidence-checks-registry.yaml`](docs/evidence-checks-registry.yaml). MANIFEST `registries:` entry = `evidence_check_registry`. **CFP-390 Amendment 1 (2026-05-11)**: 18 entry 인벤토리 backfill SSOT (ADR-060 §결정 13) — Phase 1 = SSOT 만, Phase 2 = row append. **CFP-426 (2026-05-12)**: 4 worktree-first entry skeleton 추가 (warning tier, ADR-040 Amendment 3 §결정 7.D self-application 첫 사례) — `worktree-first-{session-start-wire, pre-checkout, pre-commit-main-block, spawn-evidence-cwd}`.

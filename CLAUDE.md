...
- **진행 커밋 (progress-commit) 규범 (CFP-2966 / ADR-178)**: 작업 중 산출물은 **산출 주체 워커 자신이** 선행 적재한다(워커 자기-적재 — Orchestrator 사후수습·inline 대행 아님); 시점 = **atomic 의미 단위 경계**(파일군 1개 완결 / AC 1개 대응분 완결 / 조사 축 1개 결론 확정 중 택1 이상, 시간 주기 아님) — **신호-무관 상시 규율**이며 진행 커밋은 정지 사유가 아니다(negative control); 보존 최소 인정 단위 = **local git commit (P0)** — working tree·stash·scratch 파일·최종 메시지는 보존 단위로 불인정; 미완 표식 = 커밋 subject `[CFP-NNN][WIP] <의미 단위 요약 1줄>`(소비 시 inconclusive — PASS·완료 자동승격 금지); D2(분석·조사 텍스트)는 cheap tier `Write`/`Edit` **파일-per-워커**(`<story>-<agent>.md`, 공유 파일 직접 편집 금지)로 상시 적재하고 커밋 승격은 lane PL 배치 수행. advisory ceiling(준수는 저작 규율 — "100% 기계강제" 아님). 정책 SSOT = [ADR-178](archive/adr/ADR-178-subagent-progress-commit-preservation.md).
...
- **Agent 액션 렌더 줄 프리픽스 (model-authored primary render · hook fail-open backup, ADR-143 Amendment 2 / CFP-2770 · 값 정확성 저작 규율 Amendment 3 / CFP-2818 · 초(SS) 표기 Amendment 4 / CFP-2836)**: harness UI 가 Agent 스폰·도구호출을 렌더하는 줄에 `[<주체명>] MM/DD HH:MM:SS - <내용>` 프리픽스를 붙여 "누가·언제·무엇을 시작했는지" 를 즉시 식별(glanceability). 화면 렌더에 실제 도달하는 **유일 통로 = model-authored 프리픽스** — 에이전트/Orchestrator 가 description·prose 에 **직접 저작(primary)**. PreToolUse hook `updatedInput` 기계 주입(CFP-2587)은 도구 **실행 입력 계층**만 치환하고 화면 렌더 줄(model-emitted 원본 표시)에 미도달 → render-glanceability 목적에서 **fail-open backup 으로 강등**(실행-계층 정확성 목적 유효 잔존, 코드·G1-G5 삭제 0).
  - **범위①** Agent spawn `description` = `[<피스폰 subagent_type>] MM/DD HH:MM:SS - <task>` — model 이 `Agent.description` 에 직접 저작(subject=피스폰자; dispatcher/Orchestrator 이름이 spawned 줄 subject 로 절대 미등장 — INV-1). backup = PreToolUse(Agent) hook `pretooluse-agent-spawn-gate`.
  - **범위②** 서브에이전트 leaf Bash 호출 `description` = `[<self agent_type>] MM/DD HH:MM:SS - <action>` — model 이 `Bash.description` 에 직접 저작(subject=self). backup = sibling PreToolUse(Bash) hook `pretooluse-bash-description-inject`. **top-level Bash(Orchestrator 직속, agent_type 부재) = hook injection EXCLUDE**(dispatcher 명 미주입).
  - **범위③** Orchestrator 상태·액션 줄 = `[Orchestrator] MM/DD HH:MM:SS - <내용>` self-subject 직접 저작 허용(렌더 UI 진행 줄 한정 — INV-2 self-subject 완화; model 은 자기가 Orchestrator 임을 앎 subject-genuine, hook 판별 불가와 계층 disjoint). **prose 상태보고(ADR-170 §결정 2 whitelist entry #4 — 사용자 대상 대화 텍스트)는 prefix-exempt 유지** — 렌더 UI action/상태 LINE(display 축) ≠ prose 상태보고(mechanism 축) disjoint.
...
**tier 전순서 + ≤ Orchestrator 상한 (cap-down — ADR-141 Amendment 7)**: 모델 tier 전순서 = `haiku < sonnet < opus < fable`(fable 최상위) — tier 는 특정 모델버전에 고정되지 않은 version-agnostic **SLOT**(opus slot = 현행 Opus 5 + legacy Opus 4.8, 동 $5/$25). **≤ 상한 불변식**: 어떤 서브에이전트도 effective tier 가 Orchestrator effective tier 를 초과해 spawn 될 수 없다(동급 허용, 초과만 금지). 집행 = spawn 시 Agent tool `model:` 파라미터로 `min(frontmatter, orchestrator)` override — 초과 시에만 Orchestrator tier 명시 전달, 그 외 생략. **fresh-spawn only**(새 `Agent` spawn + `model:` override; SendMessage resume 금지 — 원본 frontmatter `model: fable` 재해석 함정 CFP-2236, Amd6 A6-2 shape 상속). **opus 세션 cap-down**: Orchestrator=opus 세션에서 fable 지정 10역할이 opus 로 cap-down(유일 live 실효). **fable 세션 = no-op**(fable 최상위라 전 배정 ≤ fable). **동급 discriminating**: opus subagent under opus Orchestrator 는 cap-down 비대상(≤ = 동급 허용, override 미발생) — SecurityTestPLAgent(opus)도 opus 세션 무변화. **정적 배정 무접촉**: frontmatter `model:` 41 파일 byte 무변경(runtime override 한정), `check-fable-roster-integrity` fable=10 GREEN 유지. **Amd6 failover 와 별개 축**: cap-down 은 사전 tier 판정(항시)·Amd6 는 사후 리밋 반응 — cap-down spawn 은 `[rate-limit-failover:fable→opus]` 태그를 사용하지 않는다(telemetry 오염 방지). **advisory ceiling 정직 라벨**: 규범 문구 presence = prompt-mandate(매턴 자기검열·grep testable=normative), Orchestrator 세션 실준수 = advisory·비-PR-enforceable(hook 기계 backup 불성립 — Orchestrator 세션모델 ground-truth env 부재). "100% 기계강제/hard-gate" over-claim 금지(ADR-143 동형). SSOT = [ADR-141](archive/adr/ADR-141-all-opus-single-tier.md) Amendment 7.

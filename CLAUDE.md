...
- **Agent 액션 렌더 줄 프리픽스 (advisory · 강제 아님, ADR-143)**: harness UI 가 Agent 스폰·도구호출을 렌더하는 줄에 `[<agent_type>] MM/DD HH:MM - <내용>` 프리픽스를 시도.
  - **범위①** Agent spawn `description` = `[<피스폰 agent_type>] MM/DD HH:MM - <task>` (subject=피스폰자, Orchestrator 이름 미등장).
  - **범위②** 서브에이전트 leaf 도구호출 `description` = `[<self agent_type>] MM/DD HH:MM - <action>`.
  - 시각 = **KST(UTC+9 고정 산술)**, dispatch/작성 시점 근사. 컴팩트(offset/연도/초/KST 라벨 미표기). helper `scripts/kst-render-stamp.sh`.
  - **TodoWrite 행 제외** + Orchestrator inline 4종(대화/TodoWrite/읽기 Q&A/상태보고, ADR-039 whitelist) 제외.
  - 3요소에 secret·token·절대경로·자격증명 임베드 금지 — `내용`은 액션 요약만.
  - 강제 상한 = advisory: 범위①만 PreToolUse(Agent) warning-tier detect(exit-0-always, rewrite 없음), 범위②는 prompt-mandate+세션시작 리마인더. 미준수 차단 없음.
  - **매턴 self-check**: "이 액션 줄에 `[agent] MM/DD HH:MM -` 프리픽스를 붙였는가? (TodoWrite·inline 4종 제외)"

...
- **Write queue 의뢰 권한** (`.claude-work/doc-queue/**`): RequirementsPLAgent, ArchitectAgent, ArchitectPLAgent, DomainAgent, SecurityArchitectAgent, TestContractArchitectAgent, DataMigrationArchitectAgent, CodebaseMapper, Refactor, DeveloperPLAgent, RequirementsAnalyst, Researcher, TestAgent — 기타 Edit/Write는 owner agent path-scoped에 한정 (CFP-26 Phase 0a: ArchitectAgent → docs/change-plans/** + docs/adr/**, DomainAgent → docs/domain-knowledge/**). Story file 섹션 갱신·GitHub comment 등 multi-writer/lifecycle 영역은 여전히 DocsAgent 경유. **Review 5 agent (codeforge-review plugin) + PMOAgent (codeforge-pmo plugin) 는 별도 plugin 으로 추출 — 본 listing 에서 제외**, 각 plugin 의 frontmatter 가 SSOT

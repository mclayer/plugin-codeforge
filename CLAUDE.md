...
**MCP 서버 (1종)**:
- `github` — Issue/PR/sub-issue/comment·label·milestone는 DocsAgent 단독 사용; `docs/{change-plans,adr,domain-knowledge,retros}/**` 직접 write는 owner agent (CFP-26 Phase 0a)

**필수 플러그인 (8종)**:
- `codeforge-review@mclayer` (>= 1.0.0) — review subsystem (3 PL + 2 worker + base + 3 checklist). CFP-35 ζ arc retrofit 부터 v2 self-write. 미설치 시 설계 리뷰·구현 리뷰·보안 테스트 lane 진행 불가
- `codeforge-pmo@mclayer` (>= 0.1.0) — PMO lane plugin (PMOAgent 단독). Cross-Story 회고 감사 + Epic milestone 갱신 + ADR 후보 발의. CFP-36 ζ arc 추출
- `codeforge-requirements@mclayer` (>= 0.1.0) — Requirements lane plugin (RequirementsPL + Domain + Analyst + Researcher). 4 sub-agent 병렬 + 도메인 KB direct write + Story §2/§5/§6 self-write. CFP-37 ζ arc 추출
- `codeforge-test@mclayer` (>= 0.1.0) — Test lane plugin (TestAgent 단독). functional + performance subset 병렬 + test_verdict v1 contract. CFP-38 ζ arc 추출. 미설치 시 구현 테스트 lane 진행 불가
...
## Development Agent Team (13 core 에이전트 + codeforge-review plugin 5 agent + `role: dev` 동적 roster · 7 레인 + 2 Cross-cutting)
...
- **설계**: ArchitectPLAgent가 CodebaseMapper(변호자) · Refactor(혁신자) · SecurityArchitectAgent(위협 변호자) · TestContractArchitectAgent(QA perspective contributor) · DataMigrationArchitectAgent(데이터 무결성 변호자) **5 deputy 병렬 스폰** → ArchitectAgent(chief author)가 통합 → Change Plan 확정 (§7 보안 설계 + §8 Test Contract + §11 데이터 마이그레이션 포함) → ArchitectAgent가 `docs/change-plans/<slug>.md` + 신규 `docs/adr/ADR-NNN-<slug>.md` direct write (CFP-26 Phase 0a 후) + DocsAgent가 Story file §7 미러링
...
- **Write queue 의뢰 권한** (`.claude-work/doc-queue/**`): ArchitectAgent, ArchitectPLAgent, SecurityArchitectAgent, TestContractArchitectAgent, DataMigrationArchitectAgent, CodebaseMapper, Refactor, DeveloperPLAgent — 기타 Edit/Write는 owner agent path-scoped에 한정 (CFP-26 Phase 0a: ArchitectAgent → docs/change-plans/** + docs/adr/**). Story file 섹션 갱신·GitHub comment 등 multi-writer/lifecycle 영역은 여전히 DocsAgent 경유. **별도 plugin 으로 추출된 agents 는 본 listing 에서 제외**: codeforge-review (5 agent), codeforge-pmo (PMOAgent), codeforge-requirements (4 agent), codeforge-test (TestAgent). 각 plugin 의 frontmatter 가 SSOT
- **외부 도구 wrapper**: SecurityTestPL(`Bash(gh api repos/*)` 1차 layer alerts fetch — codeforge-review plugin), DocsAgent(`Bash(gh api repos/*/milestones*)`, `Bash(gh api repos/*/discussions*)`, `Bash(gh api graphql*)`, `Bash(mkdir/ls/rm .claude-work/doc-queue*)`). **Review 5 agent + PMOAgent + 4 requirements agent**의 외부 도구 wrapper 는 각 plugin 으로 이전 — 본 listing 에서 제외
...
사용자 요구사항 접수부터 PR merge까지의 모든 의사결정·협업을 GitHub에 영속 기록. GitHub Issue/PR/comment·label·milestone 쓰기는 DocsAgent 단독.

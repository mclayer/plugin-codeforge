...
**Verify-before-assert 의무 (ADR-073)** = [ADR-073](docs/adr/ADR-073-orchestrator-verify-before-assert.md) — Orchestrator 가 cross-repo state 또는 assumption 단정 시 `git fetch origin` + `git show origin/main:<path>` direct verify + `verified-via` annotation 의무 (cross-repo state + assumption 기술 한정, Inline whitelist 모호성 회피). ADR-070 (Codex external worker output verify) 자매 — Orchestrator self-assertion layer. ADR-071 (dialog convergence) 와 분리 (사실 verify layer ↔ 대화 표현 layer). Sentinel #4 strike #2 carrier (Issue #607 trigger 충족).

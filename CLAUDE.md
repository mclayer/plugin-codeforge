...
- **Worktree dispatch + Worktree-first normative + mechanical enforcement (CFP-136/CFP-139/CFP-341/CFP-426 / [ADR-040](docs/adr/ADR-040-worktree-convention.md) Amendment 3·5 / CFP-531)**: 매 lane spawn 시 isolated worktree 보장 (`${HOME}/.claude/worktrees/<repo-name>/<branch-flat>`) — lane spawn·ad-hoc 구분 없이 모든 coding work 는 worktree 안에서 수행 (`git checkout` 직접 편집 금지). 4 evidence-check entry **blocking-on-pr tier 활성** (`worktree-first-{session-start-wire, pre-checkout, pre-commit-main-block, spawn-evidence-cwd}`, CFP-427/428 완료). `BYPASS_WORKTREE_FIRST=1` env = `BYPASS_WORKTREE_GC=1` 와 disjoint scope. 상세 [playbook §3.5](docs/orchestrator-playbook.md) + [ADR-040](docs/adr/ADR-040-worktree-convention.md).

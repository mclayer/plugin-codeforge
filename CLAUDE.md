...
**Wave 4 (CFP-582 / [ADR-059 Amendment 2](docs/adr/ADR-059-debate-protocol-v1.md) / debate-protocol-v1 v1.2)**: DesignLane internal blanket trigger 신설 — cross-module Story (`touched_top_level_paths >= 2` OR `touched_lanes >= 2`) 진입 시 dispatch_mode `blanket_cross_module_designlane` 자동 활성. 4-value enum precedence `auto_on_divergence > blanket_cross_module_designlane > mechanical_fast_path_inline > user_request_only`. touched paths 산정 = Story §1 spec_links + Change Plan §2 union. **convergence_quality_invariant**: 3 marker 의무 — `counterargument_present` (Round 1+, per worker) + `alternative_proposed` (cumulative ≥1) + `debate_purpose_statement_present` (Round 0). PL 이 `consensus_reached` verdict 발화 전 3-tuple AND 검증 — 미충족 시 `force_continue`. **Touchpoint #2 carry-over (§결정 9)**: ArchitectAgent §3 직후 Codex proactive check (ADR-052 #2) 결과 P0/P1 finding 을 blanket debate Round 0 input `codex_initial_position` 에 verbatim carry-over. 정책 SSOT: [ADR-059](docs/adr/ADR-059-debate-protocol-v1.md) + [debate-protocol-v1 registry](docs/inter-plugin-contracts/debate-protocol-v1.md). Sonnet decider 자동 발동 무효 (ADR-022 Deprecated / CFP-134) — debate 발동은 PL 책무. **Wave 5 (CFP-612 / [ADR-071](docs/adr/ADR-071-orchestrator-user-dialog-convergence.md) / Orchestrator-user dialog convergence)**: agent ↔ agent debate domain (§3.13 / Wave 4) 의 conceptual pair = Orchestrator ↔ user dialog domain. frame mode 4 step + 4 layer (preamble / declare / "추상" halt / N=1 + M=5 cross-Story incidents) + sub-mechanism 2 종 (이전과 다르게 한 점 / 4 차원 enum 전환). schema fit 부적합 declare — 3 marker pattern import 절대 금지 (debate transcript verification schema vs turn-by-turn cognitive frame). SSOT 본문 = [playbook §3.14](docs/orchestrator-playbook.md) + skill `codeforge:user-dialog-mode`. Layer 4 영속 file = [`docs/orchestrator-communication-incidents.md`](docs/orchestrator-communication-incidents.md) (wrapper repo cross-Story append-only, Orchestrator monopoly).
...
설계 lane 진입 시 `codeforge:deputy-mandate` 호출 (ArchitectPLAgent deputy spawn 결정 전). 6+3 deputy §7/§11/§13 sub별도 ownership 전체 테이블 포함.

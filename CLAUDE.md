...
- `codeforge-requirements@mclayer` (>= 0.1.0) — Requirements lane plugin (RequirementsPL + Domain + Analyst + Researcher). 4 sub-agent 병렬 + 도메인 KB direct write + Story §2/§5/§6 self-write. CFP-37 ζ arc 추출. 미설치 시 요구사항 lane 진행 불가
...
## Development Agent Team (14 core 에이전트 + codeforge-review plugin 5 agent + `role: dev` 동적 roster · 7 레인 + 2 Cross-cutting)
...
- **Write queue 의뢰 권한** (`.claude-work/doc-queue/**`): ArchitectAgent, ArchitectPLAgent, SecurityArchitectAgent, TestContractArchitectAgent, DataMigrationArchitectAgent, CodebaseMapper, Refactor, DeveloperPLAgent, TestAgent — 기타 Edit/Write는 owner agent path-scoped에 한정 (CFP-26 Phase 0a: ArchitectAgent → docs/change-plans/** + docs/adr/**). Story file 섹션 갱신·GitHub comment 등 multi-writer/lifecycle 영역은 여전히 DocsAgent 경유. **별도 plugin 으로 추출된 agents 는 본 listing 에서 제외**: codeforge-review (5 agent — Design/Code/Security PL + Claude/Codex worker), codeforge-pmo (PMOAgent), codeforge-requirements (4 agent — RequirementsPL + Domain + Analyst + Researcher). 각 plugin 의 frontmatter 가 SSOT

...
## 정체
codeforge = Claude Code 범용 SW 개발 오케스트레이션 플러그인. **0 core 에이전트 (wrapper-only)** — 최상위 Claude 세션(Orchestrator)이 8개 lane plugin 의 에이전트를 spawn 해 요구사항 접수부터 배포 리뷰까지 진행한다.
...
8 lane plugin: `codeforge-{requirements, design, review, develop, test, deploy, deploy-review, pmo}@mclayer`. 추가 필수: `superpowers` · `github` · `codex`. 에이전트 상세는 각 lane plugin 의 CLAUDE.md 가 SSOT(단일 출처) — 본 repo 에는 에이전트 파일 없음.
...
- **worktree**: 모든 코딩 작업은 격리된 worktree(`~/.claude/worktrees/<repo>/<branch>`) 안에서. `git checkout` 직접 편집 금지.
...
- **검증 후 단언**: cross-repo 상태는 `git fetch` 후 실제 확인하고 말한다. 외부 워커(Codex 등) 출력은 직접 Read 로 검증 후 신뢰.
...
## 문서 위치
- 결정 기록(ADR) = `docs/adr/ADR-NNN-<slug>.md`.
...
## 마켓플레이스 동기화
plugin.json 의 `name`·`version`·`description`·`author` 변경 시 `mclayer/marketplace` 의 동일 필드를 같은 Story 안 sync PR 로 맞춘다 (marketplace sync PR 선행 merge → plugin PR merge). 상세: [docs/adr/ADR-063-marketplace-atomic-invariant.md](docs/adr/ADR-063-marketplace-atomic-invariant.md).

## 브랜치 보호
**6 lane plugin branch protection contexts SSOT (CFP-1850-S2 후 상태)**: `phase-gate-mergeable.yml` workflow job ID = `check-gate` — contexts 에 `check-gate` 포함 의무. audit log: [`docs/security/branch-protection-audit.md`](docs/security/branch-protection-audit.md). CFP-1850-S2 — requirements/pmo 에 `phase-gate-mergeable` 필수 추가 (단일 chore PR 은 isChoreOnly fast-pass 로 통과) + deploy/deploy-review PROTECTED 정정 완료. review `invariant` 유지 (live check — invariant-check.yml 생성, 보호 강도 비축소).

| plugin | required_status_checks contexts | 비고 |
|--------|--------------------------------|------|
| wrapper (plugin-codeforge) | `["phase-gate-mergeable","invariant-check","doc frontmatter schema (CFP-28 — strict)","doc section schema (CFP-28 — strict)","check-gate","Verify deploy lane presence (Phase 2 wire — ADR-087 Amd 2)"]` | 6-tuple (CFP-1808 Amendment 2 — deploy-lane-presence wire). 6번째 context = workflow job 표시명 (`deploy-lane-presence.yml` 의 `jobs.*.name`) verbatim — CFP-1850-S2 drift 정정 (이전 표기 `deploy-lane-presence` 축약 = actual context명 mismatch) |
| codeforge-requirements | `["check-gate","phase-gate-mergeable"]` | CFP-1850-S2 — `phase-gate-mergeable` 필수 추가 (이전 `check-gate`만) |
| codeforge-design | `["phase-gate-mergeable","check-gate"]` | — |
| codeforge-review | `["invariant","phase-gate-mergeable","check-gate"]` | `invariant` 유지 (live check, CFP-1850-S2 결정 — 보호 강도 비축소) |
| codeforge-develop | `["phase-gate-mergeable","check-gate"]` | — |
| codeforge-test | `["phase-gate-mergeable","check-gate"]` | — |
| codeforge-pmo | `["check-gate","phase-gate-mergeable"]` | CFP-1850-S2 — `phase-gate-mergeable` 필수 추가 (이전 `check-gate`만) |
| codeforge-deploy | `["check-gate","phase-gate-mergeable"]` | CFP-1850-S2 — PROTECTED 정정 (이전 표기 NOT PROTECTED = drift) |
| codeforge-deploy-review | `["check-gate","phase-gate-mergeable"]` | CFP-1850-S2 — PROTECTED 정정 (이전 표기 NOT PROTECTED = drift) |

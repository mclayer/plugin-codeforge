...
**필수 플러그인 (4종)**:
- `codex@openai-codex` — CodexReviewAgent (3 리뷰 lane 공통) / RequirementsAnalyst 의존
...
## Development Agent Team (20 core 에이전트 + `role: dev` 동적 roster · 7 레인 + 2 Cross-cutting)
...
- **외부 도구 wrapper**: RequirementsAnalyst(`Bash(codex exec *)`), CodexReviewAgent(`Bash(node *)` codex-companion 실행), SecurityTestPL(`Bash(gh api repos/*)` 1차 layer alerts fetch), DocsAgent(`Bash(gh api repos/*/milestones*)`, `Bash(gh api repos/*/discussions*)`, `Bash(gh api graphql*)`, `Bash(mkdir/ls/rm .claude-work/doc-queue*)`)

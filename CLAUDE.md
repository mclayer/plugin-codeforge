...
- **Epic 내 Story 전환 자율 진행** (ADR-071 §결정 22): 한 세션에서 Epic 을 여러 child Story 로 진행할 때 Story N→N+1 전환(및 단일 Story Phase1→Phase2 전환)은 자동 이어서 진행이 default — 전환 지점에서 over-halt(무발화 정지)·over-ask("다음 Story 진행할까요?") 금지(정당 멈춤 3종 예외). in-session 전환 한정(세션 재개 §3.12 cold-resume·session-swap §결정 18/§결정 24 controlled-path 는 별 경로 — 세션 전환 권유 시 자족 handoff 프롬프트 동반 의무).

...
섹션 규격·단계별 책임 상세는 [`agents/DocsAgent.md`](https://github.com/mclayer/plugin-codeforge/blob/v3.0.0/agents/DocsAgent.md) 참조.
...
**§10 FIX Ledger 스키마** (DocsAgent가 관리, 상세는 [DocsAgent.md](https://github.com/mclayer/plugin-codeforge/blob/v3.0.0/agents/DocsAgent.md) §9 참조):
```
...
- **Write queue 의뢰 권한** (`.claude-work/doc-queue/**`): ArchitectAgent, ArchitectPLAgent, SecurityArchitectAgent, TestContractArchitectAgent, DataMigrationArchitectAgent, CodebaseMapper, Refactor — 기타 Edit/Write는 owner agent path-scoped에 한정 (CFP-26 Phase 0a: ArchitectAgent → docs/change-plans/** + docs/adr/**). Story file 섹션 갱신·GitHub comment 등 multi-writer/lifecycle 영역은 여전히 DocsAgent 경유. **별도 plugin 으로 추출된 agents 는 본 listing 에서 제외**: codeforge-review (5 agent), codeforge-pmo (PMOAgent), codeforge-requirements (4 agent), codeforge-test (TestAgent), codeforge-develop (5 agent + presets). 각 plugin 의 frontmatter 가 SSOT
...
형식·phase prefix(10 lane prefix + Orchestrator Preflight 1 = 총 11종)은 [`agents/DocsAgent.md`](https://github.com/mclayer/plugin-codeforge/blob/v3.0.0/agents/DocsAgent.md) SSOT 참조. 다른 에이전트는 Orchestrator에 기록 요청만 수행, DocsAgent가 실행 (`mcp__github__add_issue_comment`).
...
- 세부 규약·섹션 책임: [`agents/DocsAgent.md`](https://github.com/mclayer/plugin-codeforge/blob/v3.0.0/agents/DocsAgent.md) SSOT

...
Claude Code 범용 SW 개발 오케스트레이션 플러그인. 24 core 에이전트 · 7 레인 구조 + `role: dev` 동적 roster로 요구사항 접수부터 보안 테스트 통과까지 자율 실행. 에이전트 상세는 각 [`agents/<Name>.md`](agents/) (SSOT). 공통 문서 양식은 [`templates/`](templates/) SSOT 참조. 프로젝트 shape별 Dev 구성 preset은 [`presets/`](presets/) 참조.
...
**필수 플러그인 (4종)**:
- `codex@openai-codex` — CodexDesignReview/CodexCodeReview/CodexSecurityTest/RequirementsAnalyst 의존
...
Orchestrator                        # 최상위 Claude 세션 — 모든 스폰·토큰 예산 소유
 ...
 └── [보안 테스트] SecurityTestPLAgent
      ├── ClaudeSecurityTestAgent     # Claude 네이티브 보안 리뷰 (high-level: trust boundary·auth model)
...
- **설계**: Architect가 CodebaseMapper(변호자) · Refactor(혁신자) **병렬 스폰** → 두 독립 관점 대립 조정 → Change Plan 확정 (§8 Test Contract 포함) → DocsAgent가 `docs/change-plans/<slug>.md` 저장 + Story file §7 미러링
- **설계 리뷰**: DesignReviewPL이 Claude/Codex 설계 리뷰 종합 → PASS 시 `gate:design-review-pass` 라벨 부착 → Phase 1 PR mergeable → merge → 구현 진입 / FIX 시 Architect 회귀 (최대 3회)
- **구현**: Phase 2 PR open (DeveloperPL이 첫 commit 준비 후 DocsAgent 경유 `mcp__github__create_pull_request`). Orchestrator가 QADev + DeveloperPL 병렬 스폰. DevPL이 프로젝트 `role: dev` roster를 동적 discover해 의존성 없는 한 **모두 병렬** 스폰. Architect는 stateless 재스폰되어 매핑표 감사
...
- **보안 테스트**: 1차 layer = Dependabot/CodeQL/Secret Scanning/Push Protection (GitHub native). 2차 layer = SecurityTestPL이 Claude/Codex 보안 테스트 종합 (OWASP·CWE·CVE·trust boundary·credential). PASS 시 `gate:security-test-pass` 라벨 → Phase 2 PR mergeable → merge → Story 완료 / FAIL 시 DeveloperPL 1차 진단 → Architect 최종 판정 (무제한)
...
**Project Config Packet** (DocsAgent·RequirementsPL·DomainAgent·PMO): `.claude/_overlay/project.yaml` slice도 packet으로 주입 → GitHub 호출 에이전트의 반복 `Read` 회피. 상세는 playbook §12.5.

**Story file 위치**: `docs/stories/<KEY>.md` (KEY = `<github.story_key_prefix>-N`, 예: `PLG-7`). story-init.yml Action이 자동 생성 (요구사항 lane 진입 시).

**생성·갱신 전담**: **DocsAgent**.

**섹션 갱신 의뢰 경로**: 각 에이전트는 Orchestrator 경유 DocsAgent에 "Story file <key> 섹션 {X}에 다음 내용 추가" 의뢰. 직접 write 하는 에이전트는 DocsAgent 단독.
...
[요구사항] Orchestrator → RequirementsPLAgent → **병렬** (DomainAgent · RequirementsAnalyst · Researcher 동시 스폰) → RequirementsPLAgent 통합
        ...
        · DomainAgent 지식 공백 해소 시 write queue에 Domain Knowledge draft 제출 → DocsAgent가 docs/domain-knowledge/<area>/<topic>.md 신규/갱신
...
[설계] Orchestrator → ArchitectAgent → **병렬 스폰**
        ├── CodebaseMapperAgent (as-is 변호자 — 원 소스 직접 읽기)
        └── RefactorAgent (to-be 혁신자 — 원 소스 직접 읽기)
        · 둘 다 Architect가 공통 입력(코드 경로 + 관련 ADR + Change Plan 초안 + Story §1-7) 직접 제공 → Mapper→Refactor 선후 의존성 없음
        · 두 결과 독립적으로 반환 → Architect가 교차 검토 후 대립 조정
        · Change Plan §2 "현재 구조"에는 Mapper 분석, §3 "도입할 설계"에는 Refactor 제안 + Mapper 변호 근거 수용/반박 기록
        · **Clarification 재스폰**: Architect가 두 결과 통합 중 특정 관점의 추가 분석·재해석이 필요하면 Orchestrator에 "<에이전트> 재스폰 요청 + clarification context + 이전 출력 pointer" 전달
        → Change Plan 확정 (§8 Test Contract 포함)
        → DocsAgent 저장 (docs/change-plans/<slug>.md) + Story file §7 미러링

[설계 리뷰] Orchestrator → DesignReviewPLAgent
        ├── ClaudeDesignReviewAgent
        └── CodexDesignReviewAgent  (병렬)
        → severity 종합 → PASS or FIX (최대 3회)
        · PASS: DocsAgent가 `gate:design-review-pass` 라벨 부착 → Phase 1 PR mergeable → merge → 구현 lane
...
[구현] Phase 2 PR open (DeveloperPL → DocsAgent → mcp__github__create_pull_request)
        ...
        → Orchestrator가 Architect를 stateless 재스폰해 매핑표 감사
...
[보안 테스트] Orchestrator → SecurityTestPLAgent
        1차 layer (자동): Dependabot alerts + CodeQL findings + Secret Scanning + Push Protection (GitHub native, 워크플로우 자동 실행). PL이 결과 fetch
        2차 layer (병렬):
        ├── ClaudeSecurityTestAgent (high-level: trust boundary, auth model)
...
**프로세스**: DeveloperPL 1차 원인 진단 → Orchestrator 경유 → Architect 최종 판정 + evidence pack(Change Plan 버전 + 리뷰 findings + 테스트 로그) 첨부 의무. Story file §10 FIX Ledger에 누적.
...
- **설계 원인 판정 시**: Change Plan 갱신 (특히 §3 도입할 설계 / §6 리팩터링 선행 / §7 Test Contract 중 해당 항목) → Phase 1 follow-up PR → 설계 리뷰 레인부터 재실행
...
**Noise 분류**: ReviewPL이 1차 `valid/noise` 분류 → Architect가 noise 재배정 가능 (과정을 GitHub Issue 코멘트 의무 기록).
...
- **SecurityTest**: 대상은 코드 + 인프라 + 의존성. 보안 카테고리 전담. 1차 layer는 GitHub native 도구(Dependabot/CodeQL/Secret Scanning), 2차 layer는 Claude/Codex 보안 에이전트
...
- Architect가 Mapper·Refactor **병렬 스폰** — 둘 다 원 소스(코드 + ADR + Change Plan 초안 + Story §1-7)를 직접 읽고 각자 관점에서 분석 (한쪽이 다른 쪽 산출물에 오염되지 않도록 독립 유지)
- 두 관점 충돌 시 Architect가 결정 근거와 함께 Change Plan §2(현재 구조)와 §3(도입할 설계)에 명시. 수용·반박은 Architect가 조정 후 기록 (Refactor가 Mapper 요약을 선행 읽고 대응하는 방식 아님)
- DesignReviewPL이 "**Architect 통합 판정**이 Mapper 변호 근거를 근거 있게 일축·수용했는가 / Refactor 제안이 요건 범위를 넘지 않았는가" 교차 체크 (병렬 모델에서는 Mapper·Refactor가 상호 대응하지 않으므로, 대립 해소 품질 평가는 Architect 판정 결과 대상)
...
- **Write 권한 있음**:
  - `role: dev` 에이전트별 개별 scoping (core: DeveloperAgent `src/**` 기본, DataEng 프로젝트별 데이터 계층 경로, InfraEngineer `deploy/**`·`config/**`·`scripts/**`; preset·overlay가 경로 재정의)
  - QADev(`tests/**`)
...
- **외부 도구 wrapper**: RequirementsAnalyst(`Bash(codex exec *)`), CodexDesignReview·CodexCodeReview·CodexSecurityTest(`Bash(node *)`), DocsAgent(`Bash(gh api repos/*/milestones*)`, `Bash(gh api repos/*/discussions*)`, `Bash(gh api graphql*)`, `Bash(mkdir/ls/rm .claude-work/doc-queue*)`)
...
- **설계**: **CodebaseMapper · Refactor 병렬** — 둘 다 원 소스 직접 읽기, 한쪽이 다른 쪽의 요약에 의존하지 않음. Architect가 교차 검토
...
형식·phase prefix 10종은 [`agents/DocsAgent.md`](agents/DocsAgent.md) SSOT 참조. 다른 에이전트는 Orchestrator에 기록 요청만 수행, DocsAgent가 실행 (`mcp__github__add_issue_comment`).
...
- `phase-gate-mergeable.yml`: phase + gate 라벨 둘 다 있어야 mergeable (required status check)
- `fix-ledger-sync.yml`: docs §10 commit → Issue comment + fix:* 라벨 자동

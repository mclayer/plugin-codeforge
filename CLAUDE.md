...
Claude Code 범용 SW 개발 오케스트레이션 플러그인. 22 에이전트 · 6 레인 구조로 요구사항 접수부터 테스트 통과까지 자율 실행. 에이전트 상세는 각 [`agents/<Name>.md`](agents/) (SSOT).
...
Orchestrator                        # 최상위 Claude 세션 — 모든 스폰·토큰 예산 소유
 ...
 │
 └── [테스트] TestAgent               # Orchestrator 직속 테스트 게이트 (언어·프레임워크 중립)
```
...
- **테스트**: TestAgent (기능 → 성능 순차). ALL PASS 시 완료 / FAIL 시 DeveloperPL 1차 진단 → Architect 최종 판정 (무제한)
...
**§10 FIX Ledger 스키마** (DocsAgent가 관리, 상세는 [DocsAgent.md](agents/DocsAgent.md) §8 참조):
```
...
- FIX 대상: `project = <PROJECT_KEY> AND labels in ("fix:설계-리뷰-retry", "fix:구현-리뷰-retry", "fix:테스트-retry")`

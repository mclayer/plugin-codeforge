...
- **Forbid-list dictionary 12 어휘** (Amendment 2 — 8 → 12, CFP-610): dictionary SSOT = [`docs/wording-dictionary.md`](docs/wording-dictionary.md). lint = [CFP-449](https://github.com/mclayer/plugin-codeforge/issues/449) warning tier (scope 5 영역: `docs/adr/**` / `docs/change-plans/**` / `CLAUDE.md` / `docs/orchestrator-playbook.md` / `templates/**`. exempt channel = `hotfix-bypass:decision-principle-vocab` label). wording dictionary 카테고리 (a) 4 어휘 포함 (어휘 목록 SSOT = `docs/wording-dictionary.md` 카테고리 (a) 참조). 결정 menu 자체에서 제거 의무.

...
본 플러그인은 [`mclayer/marketplace`](https://github.com/mclayer/marketplace) 를 통해 노출 (`/plugins install codeforge@mclayer`). codeforge family 7 plugin (wrapper + 6 lane) 모두 등록 — 등록 자격 / mirrored field SSOT / sync trigger / forward-looking 정책은 [ADR-016](docs/adr/ADR-016-marketplace-registration-policy.md) (CFP-49). codeforge `.claude-plugin/plugin.json` 의 **mirrored 필드** (`name` · `version` · `description` · `author`) 변경 시 `mclayer/marketplace` 의 [`marketplace.json`](https://github.com/mclayer/marketplace/blob/main/.claude-plugin/marketplace.json) `plugins[name=codeforge]` 동일 필드를 **같은 Story 내 동기화 PR 로 반영** (codeforge PR merge 직후 즉시 sync PR open·merge). 비-mirrored 필드(`keywords` 등) 만 변경 시 면제 (Story §4·§7 예외 사유 명시). codeforge PR 본문/Story §11 에 "marketplace sync PR 후속 의무" 표기. 정식 cross-repo parity CI 는 후속 CFP (CFP-50 잠정) — 도입 전까지 author·Orchestrator 의무. drift 시 사용자가 stale version 을 install 하게 되어 marketplace 단일 진입점 의미가 무너진다.

## SSOT Boundary (ADR-012, ADR-013)
...
- Story workflow Action 4종은 internal-docs 위치, plugin-side 는 phase-gate cross-repo validation 만
- Phase 1 PR = internal-docs / Phase 2 PR = plugin repo / Story §8-11 commit = internal-docs
- Issue ↔ Story bidirectional binding (`story_uri` / `story_issues`)
- Plugin repo 의 brainstorming / writing-plans skill 작업 시 spec/plan 저장 위치는 internal-docs path — CLAUDE.md policy override (skill fork 안 함)
...
**스폰 트리거**: Epic 창설 (1회/Epic) · Story 완료 회고 (1회/Story) · 사용자 요청 (주기적 Cross-Story 패턴 보고서). 단일 Story lane 게이트에 개입 없음. 상세 동작·산출물 schema 는 [codeforge-pmo CLAUDE.md](https://github.com/mclayer/plugin-codeforge-pmo/blob/main/CLAUDE.md) SSOT.
...
매 변경 시작 시 Orchestrator 가 cutoff 분류 → 강제/면제 결정. **모호 시 강제 측 분류**. 본 정책은 plugin 자체와 consumer 프로젝트 모두에 적용.

### Story file 위치

- Story file = **`mclayer/codeforge-internal-docs/<plugin-folder>/stories/<KEY>.md`** (CFP-45 PR-I 후 — internal-docs SSOT)
- Plugin repo Issue body 와 bidirectional binding (`story_uri:` / `story_issues:`)
- 새 Story 시작: internal-docs 의 Issue Forms `story.yml` 제출 → story-init.yml Action 자동 생성 + Phase 1 PR open
...
면제 시 commit body 에 `Story 면제 사유: <이유>` 1줄 명시 의무.

### Brainstorming/writing-plans skill default override

Plugin repo (codeforge family) 에서 작업 시:
- `superpowers:brainstorming` skill 의 spec 저장 위치는 `<internal-docs-clone>/<plugin-folder>/specs/` (default `docs/superpowers/specs/` 아님)
- `superpowers:writing-plans` skill 의 plan 저장 위치는 `<internal-docs-clone>/<plugin-folder>/plans/` (default `docs/superpowers/plans/` 아님)
- Controller (Orchestrator) 가 path 명시 의무 — Skill prompt 에 명시적 override
- 자동 enforcement 없음 — Skill prompt 가 정책 인지해야 한다는 신뢰 기반

### Consumer overlay 확장

Consumer 는 `.claude/_overlay/project.yaml` 의 `story_cutoff.additional_exempt_categories[]` 로 도메인 특화 면제 항목 추가 가능. **강제 항목 축소는 불허** — 안전 방향(면제 추가) 확장만 허용. Schema 는 [`docs/project-config-schema.md`](docs/project-config-schema.md) §2 참조.

### Plugin 자체 적용 (dogfooding)

본 plugin repo 도 동일 정책 적용. KEY prefix는 `CFP` (CodeForge Plugin). Story file 은 `mclayer/codeforge-internal-docs/wrapper/stories/<KEY>.md` 위치. Plugin meta 변경 시 무의미한 lane (§8 Test Contract / §9 리뷰·테스트 등) 은 `N/A — <사유>` 명시 (lane 게이트 면제 audit trail).

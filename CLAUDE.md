...
적용 대상 Sonnet agent: DeveloperAgent · BackendDeveloperAgent · FrontendDeveloperAgent · IntegrationTestAgent · StatefulTestAgent · CodebaseMapperAgent · RefactorAgent · DeveloperPLAgent · FeasibilityAgent · ContinuityAgent · ChangeImpactAgent (현재 Sonnet 유지 agent 전체 — 11종).

> **SSOT 명시 (CFP-448 / ADR-057 Amendment 3 + CFP-264 / ADR-057 Amendment 4, CL-6 + CL-1 사용자 확정 — path B)**: 본 명단은 [ADR-057 §결정 3 표](docs/adr/ADR-057-orchestrator-opus-mandate-and-sonnet-opus-fallback.md) 의 mirror reference 임. **SSOT = ADR-057 §결정 3 표**. drift 시 ADR 본문 우선 — 본 CLAUDE.md L127 갱신은 ADR Amendment definition of done 의 part. ResearcherAgent Opus 유지 (path B 정합 — ADR-046 §결정 4·5 본문 invariant 정합 보존, Codex proactive check touchpoint #4 권장 + 사용자 CL-1 확정).

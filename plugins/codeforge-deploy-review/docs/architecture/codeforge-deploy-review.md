...
> **ProductionEvidenceDeputy ownership 이관** (ADR-088 §결정 4 + ADR-072 Amendment 4): codeforge-design lane CONDITIONAL deputy → **codeforge-deploy-review lane 정식 deputy**. parent_pl: ArchitectPLAgent → DeployReviewPLAgent / ssot_position: codeforge-design → codeforge-deploy-review. mandate body = ADR-072 §결정 1-7 verbatim 유지 (책임 영역 불변, ownership / parent_pl / ssot_position 만 변경). InfraOperationalArch (codeforge-design) 와 disjoint axis — policy SSOT (design-time) vs evidence SSOT (runtime).

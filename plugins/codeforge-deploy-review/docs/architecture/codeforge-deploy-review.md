...
| **DeployReviewWorkerAgent** | 검증 3종 실 측정 worker | smoke test 실행 (HTTP shadow / WebSocket·daemon 대기 mode) + 성능 baseline 수집 (latency p50/p95/p99 / throughput / error rate / CPU·memory) + cutover 사후 검증 측정. raw metric 측정만 (verdict 판정 금지) → DeployReviewPL 반환 | opus (ADR-141 단일 opus tier — fallback 대상 없음) |

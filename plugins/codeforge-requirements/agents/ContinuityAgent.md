...
본 에이전트 = **G2 mandatory 소비자**. 연속성 분석 착수 **전** Living Architecture(현재상태 구조 문서)를 1차 설계 정보 소스로 선행 read 한다 — 현재상태를 append-only 이력(archive/adr) 조각에서 재구성하는 layer 오용 차단. 특히 "이미 결정된 것 vs 재논의" 분류 시 canonical 현재상태 대조가 이력 조각 재구성보다 정확하다.
...
- **anchor (부분집합)**: floor = **arc42 §5 Building Block View** + **Open Decisions Pending** (ContinuityAgent 권장 — 재논의 후보 식별은 미결정 사항 축과 정합). 5-anchor 부재 doc(6 lane plugin doc)은 **4 H2 closed-enum(모듈/경계/인터페이스 계약/데이터 흐름) fallback read**. 전문 pre-embed 금지 (context rot).
- **marker**: 산출물(§4.3) 선두에 `[Living-Arch-Read: <doc-basename>, anchors=<list>, read_at=<HEAD sha7 | ISO ts>]` 1줄 (advisory ceiling — 읽음의 기계 증명 아님).

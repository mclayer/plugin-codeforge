...
> rate-limit 시 Orchestrator 가 `model: opus` fallback spawn (ADR-057). tier 분류 (Sonnet → Sonnet) 불변. agent file frontmatter `model:` 별칭 (CFP-1845 follow-up — `claude-sonnet-4-6` → `sonnet`).

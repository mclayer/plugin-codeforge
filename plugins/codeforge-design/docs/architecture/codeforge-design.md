...
**Permanent agent (8 file)** — 모든 설계 lane 진입 시 spawn (PL + chief + **6 permanent deputy** — CFP-1126 / ADR-042-agent-model-selection-policy Amendment 10: AggregateArch deprecated + ModuleArch boundary axis unified, 7+3+1 → **6+3+1**). model 열 = agents/*.md frontmatter 실측 (ADR-141 opus default + carve-out — fable 2 / sonnet 4 / haiku 1) `[verified: agents/ model frontmatter grep @ 14ac6b9b3]`:

| 모듈 (agent) | 역할 | 입장 / 책임 | model |
|---|---|---|---|
| **ArchitectPLAgent** | 설계 lane PL (supervisor + FIX 판정자) | ArchitectAgent chief + SubAgent 산출물 검수 + final pl_recommendation. **chief tie-break ladder 3 단계** (ADR-068 Amd 2) + **Deputy 신설 결정 framework** (ADR-086 §결정 1/2) 적용 | fable (ADR-141 Amd4 apex) |
| **ArchitectAgent** (chief author) | 통합 author / synthesizer | SubAgent 산출물 통합 + Change Plan §1-§13 author + ADR draft + Story §3/§7/§11 mirror write + **architecture_doc per-PR 현행화 (write monopoly — ADR-078 Amd3/ADR-112 Amd1)** | fable (ADR-141 Amd4 apex) |
| **SecurityArchitectAgent** (deputy) | 위협 — 공격자 관점 | 외부 입력 / 신뢰 경계 / 권한 위임 (Change Plan §7.1-§7.3, §7.5-§7.6 input) | opus |
| **InfraOperationalArchitectAgent** (deputy, CFP-1026 S1 rename — OperationalRiskArch → Infra...) | 운영 리스크 + infra — production-readiness 변호자 | 끊김 / 실패 / 과부하 / 스테이징-프로덕션 누설 / Docker (Change Plan §7.4 + §11.6 idempotency consult input) | opus |
...
| **DataArchitectAgent** (deputy, CFP-1086 Amendment 8 mandate 축소 — 빅데이터 OLAP only) | 빅데이터 OLAP 영역 변호자 | Parquet / 객체저장소 / DuckDB / streaming / 백필 / 시계열 집계 (Change Plan §3 OLAP + §11 OLAP input). RDB OLTP aggregate 영역 = ModuleArch (aggregate-level) 분리 | opus |
| **APIContractArchitectAgent** (deputy, CFP-1086 Amendment 8 신설) | API transport contract 변호자 — single-mandate | REST/GraphQL/gRPC/WebSocket + API versioning + DTO + OpenAPI/GraphQL schema + contract testing (Change Plan §3 API + §8 contract testing input) | sonnet |
| **ModuleArchitectAgent** (deputy — CFP-1086 CodeArch rename → **CFP-1126 / ADR-042 Amd10 + ADR-091 Amd1: AggregateArch mandate 통합 흡수, boundary axis 단일 advocate**) | module-level + aggregate-level boundary 통합 advocate | module boundary + dependency direction + layered/hexagonal/clean + DDD bounded context placement (§3 code) **+ RDB OLTP aggregate invariant + 트랜잭션 경계 + persistence-bound + Alembic 정책 (§3.A aggregate + §11.1-§11.6 RDB OLTP input — CONDITIONAL applicability `project.yaml aggregate_arch.applicable: bool` carry-over)** | sonnet |
...
| 모듈 (agent) | 역할 | 입장 / 책임 | model |
|---|---|---|---|
| **CodebaseMapperAgent** | 보수 — as-is 변호자 | 기존 패턴 유지, 변경 영향 최소화 (Change Plan §2 현재 구조 input). **ADR-166 G2 mandatory 소비자** — 작업 개시 시 Living Architecture 선행 read (CFP-2813 Phase 2 배선) | haiku |
...
| **ArchitectAnalystAgent** (CFP-1026 S1 신설 — PriorArtAgent conceptual rename) | 변경 전 기존 설계 분석 단일 축 — fact 변호자 | 변경 전 ADR / Change Plan / Story §3/§7/§11 + Living Architecture 분석 (Change Plan §2 컨텍스트 input). **ADR-166 G1 (현재상태 1차 소비자 — 기배선 dual-read 유지 + protocol v1 cross-ref)** | sonnet |
...
| **ModuleArch aggregate-level applicability** (CFP-1086 P2 신설 → CFP-1126 ModuleArch 로 carry-over — 3+1) | `project.yaml aggregate_arch.applicable: bool` (default `true`). non-applicable consumer = frontend-only / API-only / external-managed RDB | ModuleArch 의 aggregate-level mandate 활성 여부 결정 (`false` 시 module-level boundary 만 — deputy 수 무변경, AggregateArch 별도 file 은 CFP-1126 로 deprecated·삭제됨) | (CONDITIONAL flag, agent file 없음) |

> **3-way 이념 대립 축** (CFP-1126 / ADR-042 Amd10 — 4-way → 3-way 재편): RDB OLTP 영역 = CodebaseMapper ↔ Refactor ↔ SecurityArch ↔ ModuleArch(aggregate-level) / 빅데이터 OLAP 영역 = CodebaseMapper ↔ Refactor ↔ SecurityArch ↔ DataArch / Cross-layer (ELT/ETL/CDC) = ModuleArch(aggregate) + DataArch co-author deferred. chief author 가 충돌 해소 + Change Plan 명시. TestContractArch / InfraOperationalArch / APIContractArch / ArchitectAnalyst / LiveOps / LiveOrdering / ProductionEvidence = contributor / single-mandate advocacy 단일 축 (대립 비참여).
...
| §3 code (layered / hexagonal / clean / DDD bounded context / module boundary / dependency direction) + §3.A aggregate | ModuleArchitectAgent (CFP-1086 CodeArch rename → CFP-1126 aggregate-level 흡수 통합) |
| §3 data (OLAP — Parquet / 객체저장소 / streaming / 시계열) | DataArchitectAgent (CFP-1086 mandate 축소 — 빅데이터 OLAP only) |
...
| §11.1-§11.6 (RDB OLTP schema / migration / rollback / integrity / backfill / idempotency) | ModuleArchitectAgent (aggregate-level — CFP-1126 흡수) + DataArchitectAgent (OLAP 측) + InfraOperationalArch (§11.6 consult) |
...
lane 간 + lane 내부 계약 surface = `docs/inter-plugin-contracts/` (**wrapper 단일 원본 — ADR-118 D5, sibling sync 폐지**. 구 ADR-010 canonical/mirror 체계 supersede — 모노레포 통합 CFP-2151):

**Producer 계약 (kind:contract)** — 본 lane 이 생성:

| contract | 용도 | SSOT pointer |
|---|---|---|
| `design_output` | 설계 lane 산출물 핸드오프 (Change Plan + ADR + Story §3/§7/§11 mirror + `architecture_doc_updated` bool — v2.4 ADR-078 carrier) | `docs/inter-plugin-contracts/design-output-v2.md` (wrapper 단일 원본) |

**Host 계약 (kind:registry)** — 본 lane 이 발동 / 참여:
...
| `review-verdict-v4` | ArchitectPL → DesignReviewPL 핸드오프 carrier field 보유: `mechanical_self_check_passed` (ADR-065) + `boundary_completeness_self_check_passed` (ADR-068 I-1~I-4) + `dimensional_empirical_self_check_passed` (ADR-068 I-5) + `marketplace_sync_declared` (ADR-063) + `living_architecture_updated_self_check_passed` (ADR-112 — CFP-2813 이 CI actuator·DesignReviewPL 소비 로직 결박, 계약 bump 0) |
| `design-info-read-protocol-v1` | **ADR-166 (CFP-2813) 신규 kind:registry** — 본 lane 소속 G1 (ArchitectAnalyst — 기배선 dual-read) + G2 (CodebaseMapper) mandatory 선행 read 소비자 / ArchitectAgent = G4 write 주체 (읽는 쪽 아닌 쓰는 쪽). 소비 = agent .md in-band (파일 참조 아님) |
...
Orchestrator parallel spawn (ADR-039 flat spawn, ArchitectPL re귀 spawn 0):
  ...
  ├─ SecurityArchitectAgent    → §7.1-§7.3 / §7.5-§7.6 input (6 permanent deputy)
  ├─ InfraOperationalArchAgent → §7.4 + §11.6 idempotency consult input (6 permanent, CFP-1026 S1 rename)
  ├─ TestContractArchAgent     → §8 Test Contract input (6 permanent deputy)
  ├─ DataArchitectAgent        → §3 OLAP + §11 OLAP input (6 permanent, CFP-1086 mandate 축소 — 빅데이터 OLAP only)
  ├─ ModuleArchitectAgent      → §3 code module-level + §3.A aggregate + §11.1-§11.6 RDB OLTP input
  │                              (6 permanent — CFP-1126 boundary axis unified, aggregate-level = CONDITIONAL
  │                              applicability `project.yaml aggregate_arch.applicable` carry-over)
  ├─ APIContractArchitectAgent → §3 API + §8 contract testing input (6 permanent, CFP-1086 신설)
  ...
  ↓ (9-12 산출물 병렬 수신 — CFP-1126 6 permanent + 3 sub-tuple = 9 default / CONDITIONAL 모두 활성 시 최대 12)
...
- 본 doc (architecture_doc) = 누적 현재 상태 (영속, Story key 독립) — **per-PR closed-binary 현행화** (ADR-078 Amd3 / ADR-112 Amd1: 구조 표면 변경 PR 마다 본문 갱신 OR `[living-arch-no-impact]` declare, `living-architecture-update` 게이트 기계 강제 — CFP-2813. 구 per-Epic mandate 의 granularity 확정)
- **소비 (ADR-166 read protocol)**: G1+G2 소비자 (본 lane = ArchitectAnalyst·CodebaseMapper) 가 작업 개시 시 본 doc 을 canonical source 로 선행 read — append-only 이력 재구성 대체
...
- **converge** = ArchitectAgent per-PR self-write (ADR-078 Amd3 / ADR-112 Amd1 — `living-architecture-update` per-PR closed-binary 게이트, CFP-2813 재건축) + DesignReviewPL L3 review-tier (`living-architecture-not-updated` finding — 구 architecture-drift content lint (CFP-923) 는 #1972/#2110 로 제거, content-drift 재도입 금지)

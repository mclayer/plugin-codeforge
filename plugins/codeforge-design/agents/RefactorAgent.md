...
- **debate 매개**: Refactor 구조 advocacy 는 `blanket_designrefactor` debate 로 dispatch 되어 Codex(proponent — 구조 리팩터 발제)↔Claude(opponent — 필요성 게이트) 적대 토론을 거친다. dispatch 주체 = **Orchestrator top-level inline** (ADR-039 §결정18 merge-time Codex adversarial 전용 whitelist + §결정19 lead 위임 per-Story dispatch topology). RefactorAgent·ArchitectPL self-spawn 불가(**codeforge 정책** 재귀 가드 — platform inherent 아님, ADR-170 §결정 19; `subagent_recursion_blocked` silent skip 상속은 **[확인 불가]** 재현 근거 미확인 서술).

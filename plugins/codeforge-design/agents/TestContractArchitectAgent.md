...
## §8.10 dark-path activation manifest 계약 (ADR-152 §결정 4, ADR-006 authoring)

§8 Test Contract 의 **dark-path 활성화 축(discriminating-B)**을 §8 author input 에 반영한다. "코드가 default-off flag(예: mctrader `COMPACT_TIERED=0`) 뒤에 숨어 landing(repo 착지)했다는 사실만으로 done 으로 인정"하는 병을 차단한다 — **landing ≠ done**. §8.10 은 §8.8(기능 동적, robustness oracle)·§8.9(DAST, attack oracle)와 **oracle 로 축이 갈리는 독립 축** — dark-path oracle = **활성화**(default-off flag 를 실제 ON 으로 켜서 행사·검출). 같은 fuzzing/능동입력을 써도 무엇을 결함으로 보는가가 disjoint. 축 어휘: **discriminating-B**(product 활성화 축 — flag ON 행사, 본 §8.10) ⊥ **discriminating-A**(게이트 self-test mutation-kill 축 — G6 ADR-151 소유). 섞으면 중복 게이트(ADR-152 §결정1). §8.10 계약 필드는 **본 에이전트(계약 필드) author input** 으로 ArchitectAgent(chief) 통합 대상(ADR-006 §8 authoring / ADR-152 §결정1).

**§8.10.0 applicability 표 (`dark_path` 단일 축 `DO|N/A` + `g_boundary_check`)**: dark-path 는 4-기법 loop 이 아니라 **단일 `dark_path` 축 1행**(§8.8 multi-key `TECHNIQUE_8_8_META` 구조 복제 금지, ADR-152 §결정2). default-off flag 뒤 도달 가능한 product 코드 존재 여부로 DO/N/A 판정.

**§8.10 산출물 계약 필드 (DO 판정 시 author 의무)** — 6 unconditional + `g_boundary_check` + conditional `infeasibility_reason`:
- **6 unconditional presence**: `flag_identifier`(default-off flag/게이트 식별자) · `default_state`(default-off/gated 근거 — dark 성립) · `activation_test_ref`(flag 를 ON 으로 켜는 discriminating test 참조) · `on_state_assertion`(ON-state 관측 assertion — flag-OFF-only 아님) · `discriminating_basis`(flag OFF 시 test 가 fail/skip 하는 근거 — crucial-experiment 성격) · `status`(∈ `{activated, infeasible, natural_na}`).
- **`g_boundary_check`** = §8.10.0 표 region-token. "soak/restart/replay(G2)·기능 fuzz(G4)·DAST attack(G5)로 넘어가지 않음" 확인. 단 token presence ≠ boundary 실준수(천장).
- **`infeasibility_reason`** ≥30자 — `status = infeasible` 일 때만 조건부 required.

**2 cross-field 선언-정합 (declared-consistency, detection 아님 — 천장 무침범)**:
- **(a) activation-honesty**: `status = activated` ⟹ `activation_test_ref` non-empty ∧ `on_state_assertion` substantive(≥15자) (빈 stub 로 "켰다" 위장 차단).
- **(b) infeasible-reason**: `status = infeasible` ⟹ `infeasibility_reason` present.

**wrapper-self declarative**: wrapper-self 거버넌스 Story(실행코드 0줄류)는 dark_path 대개 자연 N/A — §8.10 레코드 schema 존재 + 정당화만 의무. 실 dark-path activation manifest = consumer(예: mctrader `COMPACT_TIERED`) §8. 실측 정량은 `[empirical-source: consumer, Phase 2]` defer.

**정직 천장 (과대약속 금지)**: 본 author 는 applicability 레코드 + 6 산출물 계약 필드 + status enum + 2 cross-field 선언-정합까지만 강제한다. **검출력**(discriminating-B 실행사 — flag 를 진짜 켜서 관측이 진짜 걸렸는가) = 미강제(G3-review/advisory — equivalent-mutant/Duhem underdetermination) / **모든 flag 열거 완결성**(flag-naming SSOT 부재)·infeasibility 사유 타당성 = review-tier / `g_boundary_check` presence ≠ 경계 실준수 — "완전 봉인" hard-claim 금지(ADR-152 §결정3 / ADR-119 검사연극 금지 정합). adequacy 어휘 고정(soak/생존 어휘는 G2 참조 맥락에서만 — "test liveness" 표현 금지, ADR-152 §결정10).

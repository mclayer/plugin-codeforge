...
last_update_cfp: CFP-2926  # boundaries 축 갱신 — PL roster spawn 권한(INV-1) 절 신설. ADR-170 §결정 19 / ADR-044 Amendment 7 위상 `lead → Story-teammate → lane PL → SubAgent`(depth 0→1→2)에서 DeveloperPLAgent 는 lead 가 confine 한 Story/lane scope 안에서 자기 design-time roster(role:dev worker + QADev)를 직접 spawn 한다. ADR-044 §결정 11(d) 의 구 제약 문면(전문 = archive ADR-044 Amendment 7 (1) 개정 전/후 대조 — active doc 재수록 안 함. 그 문면은 근거로 `ADR-009 wrapper-only` 를 들었다)은 §결정 19 재정의 이전 상태 정지분이고 Amendment 7 은 신규 판정 없이 정합만 수행(ADR-009 = 패키지 분해 축 ⊥ 런타임 spawn 위상). 금지 존치 3종 = scope 밖 spawn · roster 외 agent spawn · teammate→teammate spawn(nested TEAMS = platform 강제). env=0 fallback 도 PL 직접 spawn 정본화하되 구 "work-request 반환 → Orchestrator read-worker pre-spawn" 은 thin-PL READ 위임 축(§결정 11(a)~(c)) 한정 scope 축소 존치 — spawn 권한 ≠ raw 직접 read 허용(두 축 disjoint). liveness 게이트(stall 판정·force-resume·TaskStop)는 lead 고정 유지, PL spawn-then-blind-wait 금지 불변(ADR-139 INV-L4 / ADR-170 §결정 20). modules=N·boundaries=Y·interfaces=N·data_flow=N(기존 §데이터 흐름 flow 의 `DeveloperPL = parallel role:dev spawn` 서술은 신 위상과 이미 정합 — 재작성 불요). 이전: CFP-2813  # stale 해소 실갱신 — model tier 실측 반영(ADR-141: DeveloperPL fable / Developer opus / QADev·DataEng·InfraEng haiku Amd1) + doc-only fast-path(ADR-054) 폐지 정정(ADR-127 §결정 1/2 — 모든 변경 full 8 lane + 2 PR) + develop_output wrapper 단일 원본(ADR-118 D5) + ADR-166 read protocol G3 관점 + 본 doc = living-architecture-update per-PR 게이트 대상(ADR-078 Amd3/ADR-112 Amd1)
...
**PL roster spawn 권한 (INV-1 — ADR-170 §결정 19 / [ADR-044](https://github.com/mclayer/plugin-codeforge/blob/main/archive/adr/ADR-044-phase-scoped-sequential-team.md) Amendment 7, CFP-2926)**: DeveloperPLAgent 는 lead 가 confine 한 **Story/lane scope 안에서 자기 design-time roster(`role: dev` 매칭 worker + QADev)를 직접 spawn** 한다 — 위상 = `lead → Story-teammate → lane PL → SubAgent`(depth 0→1→2). ADR-044 §결정 11(d) 의 구 제약 문면(전문 = archive ADR-044 Amendment 7 (1) 개정 전/후 대조 — active doc 재수록 안 함. 그 문면은 근거로 `ADR-009 wrapper-only` 를 들었다)은 ADR-170 §결정 19 재정의 이전 상태에 정지해 있던 것이며, Amendment 7 이 신규 판정 없이 정합만 수행했다(ADR-009 = 패키지 분해 축 → 런타임 spawn 위상과 disjoint). **금지 존치 3종**: scope 밖 spawn · roster 외 agent spawn · teammate→teammate spawn(nested TEAMS = platform 강제). **env 캐리어 무관 정본화** — env=0 fallback 도 PL 직접 spawn 이 정본이고, 구 "PL work-request 반환 → Orchestrator read-worker pre-spawn" 경로는 **thin-PL READ 위임 축(ADR-044 §결정 11(a)~(c)) 한정**으로 scope 축소 존치한다. **두 축 disjoint** — spawn 권한 보유가 raw 직접 read 허용을 뜻하지 않는다. **liveness 게이트는 여전히 lead 고정** — PL 이 자기 자식을 띄워도 stall 판정 · force-resume · `TaskStop` 은 lead 소유이며(ADR-139 INV-L4 / ADR-170 §결정 20), PL 의 spawn-then-blind-wait 은 계속 금지다.

...
| 모듈 (agent) | tier | 책임 |
|---|---|---|
| **RequirementsReviewPLAgent** | PL (요구사항리뷰 lane, CFP-2326 / ADR-125) | 요구사항 산출물 (Story §1-7 + 사용자 원문 + 도메인 지식) 외부사실 의존성 검수 — 외부지식 충당 3-단계 (ADR-124) 단계③ 주 발동 lane. WebSearch/WebFetch 허용 (외부사실 의존 지점 검증). 검사연극 금지. PL 책임 = synthesis only (동일 패턴). PASS → gate:requirements-review-pass → phase:설계 |
...
**공통 base** `templates/review-pl-base.md` = 4 PL 공유 SSOT (severity 종합 / dedup / noise 분류 / 보고 형식 / escalation / FIX Ledger / 워커 의존성). 각 PL md 본문은 lane-specific 4 항목만 (checklist packet · FIX 카운터 정책 · 검증 스코프 · 다음 게이트).
...
- **Worker 호출 규약 invariant** — Packet 누락 시 즉시 `ESCALATE_PACKET_INCOMPLETE` 반환 / 워커 상호 미참조 / 워커 직접 다른 subagent spawn 불가 / WebSearch+WebFetch 는 `lane=security` + `lane=requirements-review` 만 허용 (CFP-2326 / ADR-125 — requirements-review 는 외부사실 의존 지점 검증에만) / security PL 은 worker spawn 전 GitHub native 1차 layer fetch 의무.
...
| Review lane | 검증 영역 |
|---|---|
| RequirementsReview (RequirementsReviewPLAgent, CFP-2326 / ADR-125) | 요구사항 산출물 (Story §1-7 + 사용자 원문 + 도메인 지식) 외부사실 의존성 게이트 — 외부 표준/규제 누락 / 도메인 선행사례 / AC 외부검증가능성 / 시장·벤더 단정 출처 / ADR-124 결정 6 휴리스틱. 외부지식 충당 3-단계 단계③ 주 발동 lane. 검사연극 금지 (내부근거-only 결론에 외부조사 강제 금지) |
...
- **RequirementsReview (CFP-2326 / ADR-125)**: input = 요구사항 산출물 (Story §1-6 + 사용자 원문 + 도메인 지식) / checklist = `templates/review-checklists/requirements.md` / WebSearch+WebFetch 허용 (외부사실 의존 지점 검증에만 — 외부지식 충당 3-단계 단계③) / 검사연극 금지 / upstream = `requirements_output` (codeforge-requirements RequirementsPL synthesis) / output label transition = `phase:요구사항-리뷰 → phase:설계` / FIX 회귀 = RequirementsPL (요구사항 명세 갱신)

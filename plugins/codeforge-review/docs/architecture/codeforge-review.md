...
**spawn 위상 — U-1 적용 제외 4종 경계** (`templates/review-pl-base.md` §1.6 신설 — ADR-170 §결정 19 / [ADR-044](https://github.com/mclayer/plugin-codeforge/blob/main/archive/adr/ADR-044-phase-scoped-sequential-team.md) Amendment 7, CFP-2926): family 공통 위상은 `lead → Story-teammate → lane PL → SubAgent`(depth 0→1→2) 이고 **일반 lane PL 은 confine 된 scope 안에서 자기 roster worker 를 직접 spawn** 한다. **본 lane 4 review lane 은 그 일반 규칙에 4종 제외를 진다** — 4 PL 전원 공통:

| # | 제외 | 판별 기준 | 근거 |
|---|---|---|---|
| (a) | **verdict-산출 peer dispatch = lead 소유 유지** | worker 가 `review-verdict-v4` envelope 를 산출하는가. 산출하면 **lead dispatch**, 비-verdict worker(초안 작성 · mechanical scan · 집계)는 **PL spawn 허용** | K-5 SoD + [ADR-139](https://github.com/mclayer/plugin-codeforge/blob/main/archive/adr/ADR-139-background-wait-liveness-gate.md) INV-L4 "대기 주체 ↔ 판정 주체 분리". 실이득 0 — peer 2 는 이미 lead 가 한 메시지에 co-dispatch |
| (b) | **worker(depth-2) 자가-spawn 금지 유지** | worker 계층에서 `Agent` tool 호출 발생 여부 | ADR-139 INV-L4 (본래 대상). canonical 문장 = 위 "워커 직접 다른 subagent spawn 불가" |
| (c) | **nested TEAMS(teammate→teammate) 금지** | team 계층 중첩 생성 | **platform 강제 불변** — codeforge 정책 아님 |
...
> **INV-L4 인용 자격 한정 (오귀속 차단)**: INV-L4 의 자기 축은 **liveness 게이트 개입 주체 고정**이고 그 조항이 **명시적으로 금지하는 대상은 worker 자가-spawn**(=(b))이다. (a) 는 그 조항의 **분리 원리를 peer dispatch 축으로 유추 적용**한 것이지 "INV-L4 가 peer dispatch 를 금지한다"가 아니다 — 근거 강도를 실제보다 높게 표기하지 않기 위한 명시적 한정이다.

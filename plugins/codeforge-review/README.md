...
**필수**: [`codeforge@mclayer`](https://github.com/mclayer/plugin-codeforge) (>= 0.17.0). 단독 동작 불가 — codeforge core가 review_packet을 주입하고 review_verdict v4 contract로 결과 수령.
...
## Inter-plugin Contract — review_verdict v4
...
- v5.0 BREAKING: 양쪽 plugin 동시 bump + ADR 신설
...
`codeforge-review` 자체 version은 codeforge core version과 **독립**. review packet/verdict contract version v4와 호환되는 이상 자유롭게 bump 가능.

| codeforge-review | codeforge core compat |
|---|---|
| v0.1.x | >= 0.17.0 |
| v1.x (조건부 향후) | >= 0.17.0 (v4 contract 유지 시) |

...
## 추가 체크 항목 (CFP-2813 / ADR-112 Amendment 1 — Living Architecture 최신성 L3 substance)

`living-architecture-update` 게이트(CI, L2)는 presence/shape/coupling 까지만 판정한다 — "대응 arch doc 가 touch 됐고 frontmatter-only 가 아닌가". **"doc 본문이 이번 델타를 실제로·의미 있게 반영했는가"(L3 substance)는 기계 판정 불가 → 본 lane 소관** (ADR-112 §결정 4, honest ceiling). review-verdict-v4 `living_architecture_updated_self_check_passed` field + `living-architecture-not-updated` finding type 은 v4.11(현행 v4.16)에 **기존재** — Phase 2 는 그 소비 로직 배선만 (계약 파일 무수정, bump 불요 — 껍데기 계약 해소).

**소비 로직 (design lane primary emit)**:

- **(1) self-check field 소비**: ArchitectAgent verdict 의 `living_architecture_updated_self_check_passed` 판정 —
  - `false` (5-anchor/본문 갱신 0 AND no-op `[living-arch-no-impact]` declare 부재) → `living-architecture-not-updated` finding emit (severity **P1**, FIX 의무 = ArchitectAgent re-spawn).
  - `true` / null(omit — v4.11 이전 producer) → backward-compat 무시.
- **(2) L3 substance 심사 (본 Story 핵심 — 껍데기 계약 해소)**: 게이트가 GREEN(touch presence 충족)이어도, 갱신된 arch doc 본문이 Change Plan §10.A `architecture_doc_impact`(modules/boundaries/interfaces/data_flow) 델타를 **실제로 반영**했는지 대응 doc direct Read 대조. 본문 1줄 어휘 치환(L2 통과)만 있고 델타 미반영 시 → `living-architecture-not-updated` finding emit (severity **P1**, evidence = 반영 누락 anchor + Change Plan 델타 sample, suggestion = 갱신 대상 4 H2/5-anchor 지정).
- **(3) semantic drift advisory (AC-8 승격 채널)**: arch doc 서술 ↔ 실코드/Change Plan 간 divergence(예: 삭제된 표면을 여전히 서술 / 신규 boundary·interface 미기재) 발견 시 → 동일 `living-architecture-not-updated` finding 을 **advisory(P2)** 로 emit (hard-block 아님 — ArchitectAgent 갱신 신호). arch doc ↔ 실코드 충돌 = 실측(코드) 우선 + divergence 명시.

**honest ceiling (over-claim 금지 — ADR-119)**: 본 심사는 "본문이 델타를 반영했는가"의 사람-판정이지 "모든 semantic drift 완전 검출"의 기계 보장이 아니다. AC-3 `[Living-Arch-Read: ...]` marker 는 read 추적 표면이지 read 증명 아님 — hollow "did-you-read" 게이트 배선 금지.

**Scope**: design lane only (DesignReviewPL primary emit). CodeReviewPL 은 findings[] 로 cross-validate (governance write-time anchor, code-time invariant 아님 — ADR-112 §결정 4). ADR-091 DDD finding type 3종과 disjoint axis.

...
review_packet:
  ...
  lane: design | code | security | requirements-review        # 필수 (requirements-review: CFP-2326 / ADR-125 — 10번째 lane, 요구사항 외부사실 의존성 게이트)
...
### 종합 발화 precondition — worker_outcomes 도달 (CFP-2597 / ADR-139 §결정 7)
...
- `lane=requirements-review` + `sonnet_final_status=PASS` → `gate:requirements-review-pass` + `phase:요구사항-리뷰` → `phase:설계` (CFP-2326 / ADR-125 — 10번째 lane, Phase 1 내부 sub-gate)
...
검증 floor = **≥1 independent peer (SoD: implementer ≠ certifier)** — ADR-044 Amendment 4 §결정 10 / concept `lane-verification-floor.md` (CFP-2471, Epic CFP-2468 W3). floor 충족자 = ClaudeReviewAgent (default roster 상존). Codex = floor 아닌 **ceiling** (cross-model diversity 이득). self-audit (0-peer, implementer=certifier) verdict 는 floor 위반 = 무효.

- **ClaudeReviewAgent**: 외부 의존성 없어 **항상 필수** — floor (≥1 independent peer) 의 default 충족자
- **CodexReviewAgent**: Codex = ad-hoc **ceiling** (ADR-044 §결정2 `dispatch_mode: user_request_only`, `team-spec-code-review.yaml` default 2-teammate 정합). 미가용 시 single-peer **honest degrade** 가 정식 floor 충족 (진입 불가 아님) — 단 **silent degrade 금지**. degrade 시 review-verdict-v4 `peer_degrade` block (`peer_count: 1` + `degrade_acknowledged: true` + `degrade_reason`) 으로 명시 인지·기록 의무 (ADR-094 (c) degraded+warning 동형, (a) silent harm 거부). `peer_count: 1` 인데 marker 부재 (silent 2→1 degrade) = `check-verification-floor.sh` 차단 대상.

> **floor ⊥ ceiling disjoint axis**: floor (≥1 peer) 는 Claude peer 로 충족 — "Codex 강제" 아님 (ADR-044 §결정2 ad-hoc-only 와 무충돌). dual-peer (Claude+Codex) = ceiling 강도 (있으면 정식 강도, 없으면 honest single-peer degrade). 본 §10 = ADR-044 Amendment 4 의 review-pl-base 정합 carrier (CFP-2471, "진입 불가·SKIPPED 불허" → "silent degrade 금지" 재해석).
...
**codex exec dispatch wall-clock 상한 (CFP-2545 / CFP-2828 / ADR-081 §결정 D14→D15)**: CodexReviewAgent 의 `codex exec` 직접 dispatch (companion 브로커 우회 — 단일 primitive) 는 wall-clock 상한(**option-first** `timeout --kill-after=${CODEX_REVIEW_KILL_AFTER_SEC:-30} ${CODEX_REVIEW_TIMEOUT_SEC:-300}` — GNU coreutils runnable 형태, duration-first 는 exit 127 무효) 대상 — CLI 고유 hang(#20919/#19945, stall 축 "소멸" 아닌 "이동") 시 `[codex-sandbox-fallback: dispatch_stall_or_stream_timeout]` marker + verdict=inconclusive (fail-open 금지, PASS 자동 승격 금지). SSOT = [ADR-081 §결정 D15](https://github.com/mclayer/plugin-codeforge/blob/main/archive/adr/ADR-081-codex-worker-prompt-boilerplate.md) / [`agents/CodexReviewAgent.md`](../agents/CodexReviewAgent.md) 실행 패턴.

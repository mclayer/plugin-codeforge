...
DesignReviewPLAgent가 ClaudeReviewAgent / CodexReviewAgent에 packet으로 주입하는 설계 리뷰 체크리스트. 두 워커가 **공통 입력**으로 사용. SSOT 분리는 [ADR-001](https://github.com/mclayer/plugin-codeforge/blob/main/archive/adr/ADR-001-review-agent-unification.md) 결정.

CFP-46 / [ADR-014](https://github.com/mclayer/plugin-codeforge/blob/main/archive/adr/ADR-014-operational-risk-ssot-distribution.md) 반영 — §7.4 운영 리스크 (DR / cancel-on-disconnect / clock sync / rate limit / env isolation, InfraOperationalArchitectAgent 산출물) + §11.6 Idempotency CONDITIONAL invariant 신설. §7 numbering shift: 기존 §7.4 민감 데이터 → §7.5, §7.5 위협↔완화 → §7.6, §7.6 N/A → §7.7. ADR-014 Amendment 4 (CFP-676 wrapper#1035 `abcd92bf` merged 2026-05-19) — InfraOperationalArch rename + DataArch rename (sibling sync CFP-698 carrier).
...
- **Architect 통합 판정에서 DataArch 마이그레이션 안전성 매핑 미반영** → P0 강제 (`data-migration`)
...
- 설계 의도가 ADR 변경이라면 "신규 ADR 필요" P0 지적 (신규 ADR 없이 기존 ADR 변경 금지 — [`templates/adr.md`](https://github.com/mclayer/plugin-codeforge/blob/main/plugins/codeforge-design/templates/adr.md) §파일 메타)
...
- **§8.8 동적 로스터 applicability·산출물 계약 (anchor-presence)**: 실행 코드 있는 Story §8 에 fuzz/property/load/concurrency 4기법 `DO|N/A` applicability 레코드 + `g2_boundary_check`(soak/restart/replay=G2 disjoint 확인) 존재 + DO 기법의 산출물 계약 필드(fuzz 6·property 4·load 4·concurrency 5) presence + N/A 기법의 per-technique substantive reason(≥30자) — 또는 §8.4 N/A(코드 0줄) 가 명시됐는가. **presence-only**: anchor 는 applicability·산출물 계약 레코드의 "존재"만 판별하고 완결성·검출력은 판별하지 않는다(검출력 = G3 미강제, 완결성 = review 채널, ADR-119 검사연극 금지). burden-flip 표준(feasible 동적 검증 default 수행 = do-it-unless-proven-infeasible) 준수 여부는 review-tier(AC-1b). ADR-146 / §8.8 축 (동적 검증 EXECUTION 로스터 축 — 엣지 완결성 §8.2·형상 정확성 §8.7 과 disjoint)
...
InfraOperationalArchitectAgent 산출물 검증. 5 sub-items 모두 작성 또는 N/A + 사유 1줄 (10자+) 의무.
...
## §11 데이터 마이그레이션 감사 (DataArchitectAgent 산출물 통합 결과 검증)
...
DataArchitectAgent primary + InfraOperationalArchitectAgent consult 산출물 검증.
...
## §4 API 호환 감사 (Codex audit #5, [CFP-22 spec](https://github.com/mclayer/codeforge-internal-docs/blob/main/wrapper/specs/2026-04-28-cfp-22-design-checklist-expansion.md))
...
## §3·§4 관측성 감사 (Codex audit #4, [CFP-22 spec](https://github.com/mclayer/codeforge-internal-docs/blob/main/wrapper/specs/2026-04-28-cfp-22-design-checklist-expansion.md))
...
## §3 SLO 감사 (Codex audit #6, [CFP-22 spec](https://github.com/mclayer/codeforge-internal-docs/blob/main/wrapper/specs/2026-04-28-cfp-22-design-checklist-expansion.md))

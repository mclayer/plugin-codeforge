#!/usr/bin/env bash
# codeforge-pmo plugin agent regen
#
...
if [[ ! -f "$CORE_MERGE_PY" ]]; then
  echo "⚠ codeforge core merge.py 미발견 — codeforge-pmo 단독 사용 불가" >&2
...
echo "✓ codeforge-pmo: $(ls "$PLUGIN_ROOT/agents/"*.md | wc -l) agents regenerated"

---
title: codeforge-pmo lane 구조 (Cross-cutting — Epic 창설 / Story 회고 / Git ops / Dialog fidelity)
...
codeforge-pmo = **Cross-cutting** lane plugin. 단일 Story lane 게이트에 비개입하면서 Epic 분해 보조 / Story 완료 회고 감사 / Cross-Story 패턴 분석 / git operations orchestration / Orchestrator-user dialog fidelity 검수 책임을 가로지른다. `[verified: lane plugin CLAUDE.md @ c0f26435 GitOpsAgent/DialogFidelityAgent 단락 + agents/ tree direct enumeration: PMOAgent.md + GitOpsAgent.md + DialogFidelityAgent.md]` — 3 sibling agent (책임 영역 disjoint):

| 모듈 (agent) | 라이프사이클 | 입장 / 책임 |
|---|---|---|
| **PMOAgent** | one-shot trigger-driven (Epic 창설 / Story 완료 회고 자동 trigger / 사용자 ad-hoc) | Epic 분해 보조 + Story 완료 회고 감사 + Cross-Story 패턴 분석 + 게이트 준수 감사 + ESCALATE 트렌드 축적 → ADR 후보 발의 (`pmo_output v1.adr_proposal` field) |
| **GitOpsAgent** | long-running teammate (Story 전 기간 active) | Hierarchical branch tree + Worktree lifecycle (`.claude-work/worktree-manifest.yaml`) + Sequential merge orchestration + FIX iteration 재구성 + Stale worktree cleanup + Cross-platform path handling. Orchestrator + 모든 lane PL 의 git 작업 단일 위임 대상 |
| **DialogFidelityAgent** | one-shot read-only verifier (3-anchor 발화 시점) | Orchestrator-user dialog turn fidelity 검수. 3-anchor: `post_user_turn` / `pre_architectpl_synthesis` / `pre_fix_rootcause`. verifier-narrower-than-generator (read-only) — ADR-071 §결정 13 inline whitelist 1번 entry cognitive 보강 채널 |

> 3 agent 간 직접 sub-agent spawn 불가 — Orchestrator 경유 (codeforge family ADR-009 invariant). GitOpsAgent ↔ PMOAgent ↔ DialogFidelityAgent 는 sibling (책임 disjoint, 회고 영역 vs git 영역 vs dialog 영역).
...
| `docs/architecture/<path>.md` (본 doc 영역) | PMOAgent (lane gate — ADR-078 §결정 1 4 영역 갱신 의무, 매 Cross-cutting 책임 변경 시) |

**Cross-cutting boundary (Story lane 게이트 비개입)**:
- 단일 Story lane gate (Phase 1/2 PR merge-mergeable / FIX root cause / 설계 결정 / 구현 / 보안 테스트) 영역 deny — PMOAgent / GitOpsAgent / DialogFidelityAgent 어떤 agent 도 lane 진행 결정 비참여
...
- Orchestrator ↔ user dialog 본 발화는 inline 유지 + 직전/직후 DialogFidelityAgent read-only verifier spawn 가능 (ADR-071 §결정 13)
...
본 lane 의 인터페이스 surface = `docs/inter-plugin-contracts/` (canonical = 본 plugin repo, wrapper = sibling sync mirror — ADR-010):

**Producer 계약 (kind:contract)** — 본 lane 이 생성:

| contract | producer agent | 용도 | SSOT pointer |
|---|---|---|---|
| `pmo_output` | PMOAgent | Cross-cutting 산출물 핸드오프 (retro_summary / learnings / cross_story_pattern_adr_trigger / adr_proposal) | `docs/inter-plugin-contracts/pmo-output-v1.md` (canonical) + wrapper sibling mirror (ADR-010) |
...
**DialogFidelityAgent verifier surface** (ADR-071 §결정 13, CFP-818 — read-only):
- 3-anchor 발화 시점 input = Orchestrator dialog turn payload (사용자 발화 직전/직후 + ArchitectPL synthesis 직전 + FIX root cause 판정 직전)
- output = fidelity verdict (세션 요건·누적 결정 정합 검사 결과) — 별도 contract field 신설 0 (verifier-narrower-than-generator, declaration-only)
...
[trigger 5: Orchestrator-user dialog turn — inline whitelist 1번 entry] (ADR-071 §결정 13, CFP-818)
  ↓
3-anchor 시점 (post_user_turn / pre_architectpl_synthesis / pre_fix_rootcause)
  ↓
DialogFidelityAgent spawn (Orchestrator, one-shot read-only)
  ├─ 세션 요건·누적 결정 fidelity 검수 (verifier-narrower-than-generator)
  └─ fidelity verdict return (declaration-only, contract field 신설 0)

[trigger 6: SessionStart hook] retro-alert-pickup-kpi.yml (CFP-628 / ADR-045 §D-5)
  ↓
...
[downstream]
  ...
  · DialogFidelityAgent verdict → Orchestrator (dialog turn 직후 cognitive 보강)
...
- **current state** = lane plugin agent file (`agents/PMOAgent.md` + `agents/GitOpsAgent.md` + `agents/DialogFidelityAgent.md`) + `CLAUDE.md` 의 실제 정의 상태

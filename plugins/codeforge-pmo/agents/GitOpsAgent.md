...
정리 **실행** owner = GitOpsAgent eager(§5a) **불변**. `phase:완료` worktree-clean self-check (`scripts/check-worktree-completion-clean.sh`, Orchestrator 호출) 는 "eager 가 실제로 정리했는가" 를 **검증**만 하는 별 layer (ADR-040 Amendment 9 §결정 7.K 가정 1 / ADR-045 Amendment 13 §D-12). 2-layer 책임 분리:

...
버전 체계: [Semantic Versioning 2.0.0](https://semver.org/lang/ko/). v1.0 이전은 minor bump도 breaking 가능. plugin SemVer rule SSOT: [ADR-037](archive/adr/ADR-037-plugin-version-bump-rule.md).

## [Unreleased]

### Added

- [CFP-2284] **ADR-119 Amendment 1 / §결정 9 신설 — 제안 필요성 게이트 + 완료보고 정직성 (research-before-claims 를 claim → proposal/report 로 확장, plugin.json 6.22.0 → 6.23.0 MINOR, ADR-037 결정 1 (h) additive amendment + (g) additive guidance)**. §결정 1~8 의 "substantive 주장 검증 후 단언" 을 **substantive 제안·보고** 로 확장 (behavioral directive only — hook/lint 신설 0, necessity 는 기계 비탐지). **제안 필요성 3문 게이트** (작업 제안 + GitHub Issue/follow-up 발의 공통, 발의 전): ① 깨졌나·강제 요인 ② 이득>비용·리스크 ③ 관찰자 없어도 할 일 — 셋 다 YES 아니면 발의 금지 ("관찰됨·미조치" 1줄 기록, 발견 ≠ 필요, Orchestrator = filter 아닌 firehose 거부). **완료 보고 정직성**: 작업 상태("완료"/"잔여"/"필요함") 단언 = §결정 1 ② repo·cross-repo 실측 대상 → 재검증(Read/Grep) 후에만 단언, 추측성 backlog 패딩 금지. **적용 주체** = Orchestrator + 전 lane PL/agent (PMOAgent retro PROMOTE 발의도 게이트 선통과 의무 — ADR-045 §D-11 정합). consumer 자동 상속 (§결정 7) + Phase 1 declarative-only (§결정 8). 미러: CLAUDE.md `## 작업 규칙` 형제 bullet + `docs/consumer-guide.md` §7.4 cross-ref 1줄 + `skills/user-dialog-mode/SKILL.md` (완료보고) + `skills/post-merge-closure/SKILL.md` (PROMOTE 발의) cross-ref. frontmatter `amendments[]` Amendment 1 row (is_transitional:false, sunset_justification N/A — permanent governance, ADR-064 top-down ratchet 강화 방향 only). marketplace sibling sync PR 동반 (`mclayer/marketplace` `plugins[name=codeforge].version` 6.22.0 → 6.23.0 — ADR-063 atomic ordering, Orchestrator 가 별 PR 로 처리).
- [CFP-2282] **`git-branch-delete-merge-gate` PreToolUse hook 신설 — 미머지 PR branch 삭제 하드차단 + worktree-lifecycle 정책 codify (plugin.json 6.21.0 → 6.22.0 MINOR, ADR-037 결정 1 (c) 선택 hook 추가)**. remote branch 삭제 명령(`git push <remote> --delete|-d <b>` / `git push <remote> :<b>` colon refspec)을 가로채 해당 branch 에 열린(미머지) PR 이 있으면 PreToolUse 단계에서 물리 차단(exit 2). bootstrap-first-gate launcher 패턴 미러(extensionless bash launcher → `python3 <name>.py`). **모든 경로 fail-open(exit 0) except "열린 PR 확인"** (P0 fail-safe — 자기 결함으로 정상 작업 차단 금지): 비-Bash / 비-delete / 삭제대상 없음 / tag 삭제(`--delete tag`·`:refs/tags/...`) / gh 부재·non-zero·timeout·JSON 파싱 실패 / `BYPASS_BRANCH_DELETE_MERGE_GATE=1` = 전부 통과. **network call 1** — `gh pr list --head <branch> --state open` subprocess(timeout 10s, delete 패턴 매칭 시에만), cross-repo-gh-safety 의 "network 0" 와 다른 점(사유: squash merge 는 branch commit 을 origin/main ancestry 에 안 올려 PR merge 상태를 로컬 git 으로 알 수 없음 — gh 필수). `hooks/hooks.json` PreToolUse/Bash 배열에 cross-repo-gh-safety·repo-confinement 다음 entry 등록. pytest `hooks/tests/test_git_branch_delete_merge_gate.py`(파서 단위 + fake `gh` 스텁 PATH-주입 subprocess 통합, lint.yml hook-unit-tests ubuntu job). `skills/worktree-lifecycle/SKILL.md` §3 정리 절에 순서 불변(비가역 삭제는 merge 확인 후에만 + `A; B`/`A && B` 파이프 exit-code 가림 금지 + 조건 가드 의무) + 기계적 보강 hook 명문화. **사고 박제: INCIDENT 2026-06-15 #2280** — 미머지 PR branch 선삭제로 PR auto-close + phase-gate-mergeable status 가 head SHA 에 stuck("expected") → reopen·fresh PR·admin merge 까지 BLOCKED. marketplace sibling sync PR 동반(`mclayer/marketplace` `plugins[name=codeforge].version` 6.21.0 → 6.22.0 — ADR-063 atomic ordering, Orchestrator 가 별 PR 로 처리).
- [CFP-2092] repo-confinement + stray-scratch-leak 2 hook 신설 (repo 밖 홈 루트 스크래치 누출 재발 방지). plugin.json bump 0 (marketplace atomic sync 회피 — mirrored field 변경 0).

### Changed

- [CFP-1850-S3] **ADR-024 Amendment 18 — §결정 6.A.10 신설 isChoreOnly fast-path 면제 채널 codify + CFP-1845 anti-pattern 명문화** (doc-only fast-path ADR-054 Cat 1). phase-gate-mergeable.yml 의 `isChoreOnly` 5번째 fast-pass source (CFP-1850-S1 PR #1866 MERGED + S2 PR #1867 MERGED 이미 wired) 를 ADR-024 branch protection 정책의 정식 면제 채널로 normative codify. **메커니즘 요약**: 4-조건 AND strict (isPostMergeFix 3-조건 AND 보안 패턴 동형) — (a) `phase:unclassified` label ∧ (b) Story 미연결 (`story_uri` PR-body marker 부재 + linked `type:story` Issue 0 — Orchestrator monopoly 결정적 차단원, PR author 위조 불가) ∧ (c) `sibling-pr` / `impl-manifest` label 부재 ∧ (d) diff chore-safe path (기존 isDocOnly allow-list + `agents/*.md` 승격). 조건 (b) helper catch = 404-only skip + 그 외 예외 fail-closed. **anti-pattern 명문화**: CFP-1845 incident (model 핀 → 별칭 sync) — agents/*.md touch → hasCode=true → isDocOnly=false → phase:unclassified mismatch phase:설계-리뷰 → 영구 action_required → enforce_admins:true 라 admin merge 도 거부 → 6 lane plugin repo branch protection 손작업 우회 (off → merge → restore) 반복. 이 "보안 게이트 단발 비활성 + enforce_admins:true invariant 손작업 위배" anti-pattern 의 structural 해소 (손작업 우회 불필요, invariant 보존). **ratchet 방향**: fast-pass OR-gate 4-source → 5-source 강화 (ADR-026 Amendment 4 §결정 6 3-source → 4-source 동형 답습) — frontmatter `is_transitional: false` permanent retain, sunset_justification N/A, ratchet 약화 0건 (4-조건 AND strict + 조건 (b) 결정적 차단원 위조 불가). mechanical layer 신규 신설 0건 (existing-reuse — Wave 0 이미 wired via CFP-1850-S1 + S2). frontmatter `amendments[]` row (CFP-1850 2026-05-31) + `mechanical_enforcement_actions[]` entry (action: `phase-gate-mergeable`, status: `existing-reuse`, target_section: §결정 6.A.10) + body `## Amendment 18` 섹션 (Context + CFP-1845 anti-pattern 명문화 + 4-조건 AND 표 + ratchet 정합 분석 + axis disjoint matrix § 6.A.10 row + cross-reference 11 entry) + ADR-RESERVATION amendments_reserved[] row append (adr_number: 24 / amendment_id: 18 / reserved_by_cfp: CFP-1850 / status: active 직접 전환). plugin.json bump 0 / marketplace_sync_declared: false (mirrored field 변경 0, ADR-063 atomic invariant 발효 조건 미충족). label-registry-v2 bump 0 (family member 신설 0, kind:registry sibling sync 면제). Epic CFP-1850 S3 마지막 Story (S1 inline + S2 mirror 후 S3 normative codify 3-Story chain 완결).
- [CFP-1850-S2] **8 lane plugin repo phase-gate-mergeable 워크플로 mirror 통일 + requirements/pmo 보호규칙 필수 추가 + registry/CLAUDE.md drift 정정**. (1) 워크플로 mirror — 8 lane plugin repo `.github/workflows/phase-gate-mergeable.yml` 을 wrapper 정본 (CFP-1850-S1 isChoreOnly 포함) 으로 통일. 이전 6 repo (design/develop/review/test/requirements/pmo) = stale (isPostMergeFix + isChoreOnly 누락), deploy/deploy-review = isChoreOnly 누락. 8 PR 전부 MERGED, byte-parity 확인. (2) 보호규칙 API — requirements/pmo 에 `phase-gate-mergeable` 필수 체크 추가 (이전 `check-gate`만 → `[check-gate, phase-gate-mergeable]`). isChoreOnly 가 먼저 두 repo 워크플로에 들어가(변경 1) 단일 chore PR 영구차단 재발 없음 (S1→S2 순서 의존). (3) drift 정정 — `branch-protection-context-registry-v1` v1.0 → v1.1 MINOR (pmo/deploy/deploy-review entry contexts[] 채움, deploy/deploy-review 는 2026-05-28부터 PROTECTED였으나 doc 표기 `protected:false` 가 stale drift였음) + CLAUDE.md "6 lane plugin branch protection contexts SSOT" 표에 requirements/pmo 행 추가 + deploy/deploy-review PROTECTED 정정 + MANIFEST registry version bump + audit log section append. review `invariant` 필수 체크는 live check (invariant-check.yml 생성) 이라 유지 — 보호 강도 비축소 (ADR-058 ratchet). kind:registry sibling sync 면제. 8 lane plugin 모두 phase-gate-mergeable 필수 포함 통일 완료. Epic CFP-1850 S2 (#1864).
...
- [CFP-1565] **ADR-068 Amendment 5 — I-7 chief-author cross-ADR scope/fact claim consistency invariant 신설 + review-verdict-v4 v4.12 + ADR-065 Amendment 6 (CLAUDE.md line-cap commit-time 선제-lint)** (doc-only fast-path ADR-054). chief author 가 ADR/doc 본문 §3/§7/§10/§11 에서 다른 ADR 의 SSOT 값 (scope list / count / enum / 권한 범위) 인용·단언 시 대상 ADR direct Read-verify 후 cross-adr-claim-verify-annotation 3-key (cited_adr+§결정 / cited_value / verify_status) 대조 의무. **I-4 wording SSOT (identifier 표기 동기화) 와 disjoint axis** — I-7 = cross-ADR factual/scope **값** 의 SSOT 정합 (I-6 audit-gate-pointer = pointer 실재 verify, I-7 = 인용 값 SSOT 정합). boundary completeness 6 → 7 invariants ratchet 강화 (ADR-058 §결정 5 / ADR-064 §결정 7 정합, I-1~I-6 본문 의미 변경 0건 — I-7 disjoint append only). review-verdict-v4 v4.11 → v4.12 MINOR (`chief_author_crossref_consistency_self_check_passed` 7번째 verdict-level optional bool field + `findings[].type: "chief-author-crossref-inconsistency"` 11번째 enum literal, closed-enum 10 → 11 ratchet additive only, ADR-008 §결정 2 정합) + MANIFEST review_verdict row v4.12. DesignReviewPL + CodeReviewPL dual cross-validate (sibling cross-repo PR codeforge-review canonical 필요). **CFP-1522 retro F-003 P1 evidence** (ADR-113 §결정 6 `admin:repo` scope claim ↔ ADR-066 §결정 2 PAT 6-scope SSOT mismatch, chief 미 cross-check). **ADR-065 Amendment 6 (F-005 sibling carrier same Story)** — §결정 1 row 12 신설 (CLAUDE.md 변경 시 `bash scripts/check-claude-md-line-cap.sh` commit-time 선제 실행, Amendment 4 row 10 선제-lint family 확장, F-005 evidence: CFP-1522 retro CLAUDE.md 322>320 미사전검출). evidence-checks-registry `chief-author-crossref-consistency` deferred-followup entry append (Wave 1 declaration-only, bypass_label `hotfix-bypass:boundary-wording` 재사용, 신규 label 0건). ADR-RESERVATION amendments_reserved[] 2 row append (ADR-068 Amd 5 + ADR-065 Amd 6, status active 직접 전환, CFP-1565). Wave 2 mechanical wire (scripts/check-chief-author-crossref-consistency.sh + workflow + bats) = 별 sub-CFP carrier (ADR-082 §결정 6 retain pattern). plugin.json 6.11.0 → 6.12.0 MINOR (ADR-037 §결정 1(h) — additive amendment + 신규 contract field = governance behavior change) / marketplace.json sibling sync (ADR-063 §결정 5, 별도 sibling PR, codeforge PR merge 직후 즉시 open·merge).
...
- [CFP-1735] **ADR-082/085 historical body-section backfill — 20 dual-block-parity warnings → 0** (doc-only fast-path ADR-054 Cat 1). Resolves warnings surfaced post-CFP-1734 narrow (ADR-082 Amendment 32 sub-scope 1-U): ADR-082 (18 warnings → 0) body sections for Amendments 8/9/10/11/12/13/18/20/21 (9 missing H2/H3 sections) + amendment_log[] entry 13 (CFP-1390, between 12 and 14); ADR-085 (2 warnings → 0) body section for Amendment 1 (CFP-1693, §결정 9 sibling — added H3 above existing §결정 9 content). Body sections derived verbatim from frontmatter `amendments[].summary` + `amendment_log[].note` (no fabricated content, numeric order preserved). Existing Amendment 19 (H3) + Amendment 21 H4 sub-content retained. Lint `scripts/lib/check_adr_dual_block_parity.py --mode audit` = PASS (119/119 ADR files).
- [CFP-1734] **ADR dual-block parity lint narrowed to dual-block-only ADRs** (`scripts/lib/check_adr_dual_block_parity.py`, ADR-082 Amendment 32 sub-scope 1-U — narrows Amendment 30 sub-scope 1-S Fix A). Dual-block gate: lint runs Block 1/2/3 only when `amendments_ids` AND `amendment_log_ids` both non-empty (dual-block ADR, e.g. ADR-082); single-block (either direction), amendments[]-only, parens-convention (`## §결정 N (Amendment M)`, e.g. ADR-071), and no-amendments ADRs are EXEMPT (trivial PASS). Supersedes CFP-1688 Fix A's single-block Block-2 mode. Rationale (verify-before-trust ADR census): lint body detection matched only 39/82 ADRs — 35 use the parens-convention the lint can't see, so the original lint produced convention-mismatch false positives. Corpus regression: OLD 47 ADRs/310 violations → NEW 2 ADRs/20 (45-ADR FP elimination, no genuine F-DR-001 masked — the F-DR-001 P0 sentinel fixture is dual-block, protection preserved). Fix B (H2/H3) + Fix C (scan cap) retained on dual-block path. bats 17/17 GREEN (TC-3/TC-10 breaking exit 1→0 EXEMPT, new TC-16 parens-convention). No label-registry / evidence-checks-registry changes (reuses `adr-dual-block-parity` entry + `hotfix-bypass:adr-dual-block-parity`). #1735 narrows to ADR-082-only genuine drift (separate follow-up). Amendment slot reslot 31→32 / 1-T→1-U post-CFP-1684 parallel race.
- [CFP-1688] **ADR dual-block parity lint false-positive fix — single-block exemption + H2/H3 detection + frontmatter scan cap** (`scripts/lib/check_adr_dual_block_parity.py`, ADR-082 Amendment 30 sub-scope 1-S). 3 fixes: (A) single-block mode — amendments[] absent + amendment_log[] present → Block 1 + Block 3 skip, Block 2 (F-DR-001 P0 sentinel) unconditionally retained; (B) body heading H2/H3 both-level detection — `BODY_AMENDMENT_PATTERN = re.compile(r"^#{2,3}\s+Amendment\s+([0-9]+)")` (bounded `{2,3}` = H4+ excluded, ADR-061 Amd 3 ReDoS guard); (C) frontmatter scan cap 300 → 5000 (`lines[:5000]`) — real boundary = 2nd `---` delimiter, resolves ADR-082 amendment_log[] entry truncation past line 300. 7 new bats TC (TC-9~15 + TC-15-RED) + 3 new fixtures (adr-single-block-h3-pass.md / adr-single-block-missing-body.md / adr-long-frontmatter.md). RED→GREEN stash proof: TC-9/12/13/15 genuine RED on unfixed code, 16/16 GREEN post-fix. Acceptance: ADR-045 FP count 0 (genuine drift 4 entries retained), ADR-082 Amendment 30 PASS (spurious 22-30 truncation warnings 0).

### Added

- [CFP-1755] **ADR-064 Amendment 12 — Wave 2A PreToolUse ScheduleWakeup advisory reminder hook (mechanical wire activation)** (CFP-1753 §결정 12 Wave 2 split — advisory reminder hook proportionate first step, Wave 2B blocking gate defer per signal inspection limits). `hooks/schedule-wakeup-reminder` 신규 bash shim (CFP-1738 plain-language-reminder pattern verbatim 답습) + `hooks/hooks.json` PreToolUse 2 entries (matcher: ScheduleWakeup advisory reminder). ADR-064 frontmatter `amendment_log[]` 12th entry + `mechanical_enforcement_actions[]` 2nd entry (`autonomous-loop-idle-termination-reminder`). ADR-064 body §Amendment 12 section (Amendment 결정 1-6). `tests/hooks/test_schedule_wakeup_reminder.bats` 6 TC mapping AC-1~5 (6/6 GREEN local verify). CLAUDE.md Amendment 11 citation 에 Amendment 12 Wave 2A activation 추가.
- [CFP-1742] **Story-2 Phase 2 — PreToolUse(Agent) spawn-format gate (Wave 1 warning-only)** (ADR-115 §결정 1·4·6 / ADR-061 Amendment 3 / ADR-082 §결정 11.A). `hooks/pretooluse-agent-spawn-gate` PreToolUse(Agent) hook 신설 (extensionless polyglot, Wave 1 warning-only — 4 block 누락 시 stderr warning + exit 0, deny 비활성). graceful degradation 5층 (`set -euo pipefail` + `trap 'exit 0' ERR`) + `BYPASS_CODEFORGE_PRETOOLUSE_AGENT_GATE=1` bypass (audit trail). `scripts/lib/check_spawn_prompt_format.py` 4 block presence verifier 신설 (ADR-061 Amd 3 CodeQL ReDoS guard 정합 — line-by-line parse + anchored simple regex + `SCAN_CAP_LINES=50` per-entry scan cap, 1000-line 입력 ~60ms / lazy nested quantifier · nested negated class · alternation overlap 금지). `hooks/hooks.json` PreToolUse `matcher: Agent` entry append (기존 Bash entry 병존). `tests/bats/cfp-1742-pretooluse-agent-spawn-gate.bats` 10 TC RED→GREEN stash proof (RED 9/10 FAIL hook+verifier 부재 / GREEN 10/10 PASS).
- [CFP-1753] **ADR-064 Amendment 11 §결정 12 신설 — Autonomous loop idle-without-purpose 자동 종료 mandate** (doc-only fast-path ADR-054 Cat 1). `ScheduleWakeup` 호출 결정 직전 4-tuple AND gate self-check (armed_monitor / in_progress_agent / open_pr_session / actionable_backlog 모두 0 → 호출 금지, loop 자동 종료). Anti-pattern enum 3종 차단 — (A) CI green check 단독 actionable 오해석 / (B) trivial verification 만 loop 유지 / (C) "stop in single line = 한 줄 status report + loop 유지" 오해석 ("stop" mechanical 의미 = ScheduleWakeup 호출 생략). ADR-071 §결정 15 (Amd 4 / CFP-851) frequency-axis sibling — user-facing turn frequency vs autonomous loop tick frequency disjoint scope, same governance family. ADR-064 single-block (amendment_log[] only) CFP-1734 dual-block gate EXEMPT. Wave 1 declarative — Wave 2 mechanical wire (CFP-1738 hooks 패턴 Stop hook / PreToolUse ScheduleWakeup matcher) = 별 sub-CFP carrier. META self-applied (Orchestrator behavior governance self-write, ADR-082 sub-scope 1-G 15th applied case via ADR-RESERVATION pre-claim).
- [CFP-1741] **Story-1 Phase 2 — UserPromptSubmit hook + dispatcher entry + bats fixture** (CFP-1740 Epic Phase 2 carrier, ADR-115 §결정 1·3·5 정합). `hooks/hooks.json` UserPromptSubmit entry 에 `userprompt-submit` command append (기존 `plain-language-reminder` 병존 유지). `hooks/userprompt-submit` 신규 extensionless polyglot hook — `[codeforge-wrapper-userprompt-submit]` one-channel marker / ADR-071 user-dialog-mode + ADR-073 parallel-work-sentinel directive 주입 / graceful degradation 5층 (`set -euo pipefail` + `trap ERR exit 0`) / `BYPASS_CODEFORGE_USERPROMPT_SUBMIT=1` bypass (stderr audit). `tests/bats/cfp-1741-userprompt-submit-hook.bats` 8 TC (RED phase stash verified, GREEN 7/8 PASS, TC-5 skip).
- [CFP-1750] **docs/domain-knowledge concept 신설 — orchestrator-runtime-hook-enforcement** (CFP-1738 followup, doc-only fast-path ADR-054 Cat 1). 36-line concept_definition supporting CFP-1738 (사용자 대화 품질 hook 내장 — reminder + check). Cites Claude Code official hooks reference + 3 GitHub issues. Captures the runtime-hook boundary (Stop / UserPromptSubmit / SubagentStop block-capable vs SubagentStart NO + PreToolUse `'Agent'`/`'Task'` matcher) that codeforge had previously assumed unavailable.
- [CFP-1740] **Epic Phase 1 — ADR-115 신설 Runtime hook enforcement policy + Change Plan** (PreToolUse(Agent) blocking + UserPromptSubmit / Stop / SubagentStop non-blocking ledger). 6 §결정 — §1 4-hook tier 분류 (PreToolUse:Agent = blocking `permissionDecision:deny` / 3 non-blocking ledger) / §2 plugin 배포 block 불신뢰 constraint (Stop / SubagentStop block 금지, GitHub issue #10412 / #55754 verbatim cite) / §3 단일 dispatcher 통합 + one-channel rule (`run-hook.cmd` polyglot, consumer overlay 이중발화 차단) / §4 warning → blocking 승격 path (ADR-060 evidence-gate) / §5 graceful degradation 5-layer fail-open (auto-block escalate 금지) / §6 scope boundary (inline-write detect = 후속 CFP). frontmatter: `category: governance` / `is_transitional: false` (permanent ratchet, ADR-058 §결정 7) / `mechanical_enforcement_actions: [runtime-hook-presence]` / 14 related_adrs + 16 related_files. ADR-068 6-invariant self-check PASS (I-1~I-6 audit-gate-pointer 포함). 'turn-final hook 부재 platform 한계' 미검증 가정 falsify carrier (ADR-039 §결정 9 deferred follow-up 직접 realize). ADR-082 sub-scope 1-G META applied (ADR-RESERVATION row 115 pre-claim).
- [CFP-1751] **한영 키보드 레이아웃 자동 변환 추론 hook 플러그인 내장 — UserPromptSubmit 두 번째 entry, 모든 consumer 자동 적용** (`hooks/korean-english-recovery` bash shim + `hooks/korean-english-recovery.py` python). 사용자가 한영 전환 키를 누르지 못해 잘못된 레이아웃으로 입력한 경우 (예: `dkssudgktpdy` → `안녕하세요`) 두벌식 ↔ QWERTY 매핑 변환을 시도해 의미 추정 결과를 `additionalContext` 로 주입. Orchestrator 가 변환 의미로 답변하고 첫 줄에 통보 후 진행 (사용자 정정 가능). 양방향(eng→kor / kor→eng) 지원. 품질 점수 임계치(합성 음절 비율 ≥ 70% / 영어 vowel 분포 ≥ 50%)로 false positive 회피. 설정 env: `BYPASS_KOREAN_ENGLISH_RECOVERY`. python 부재 시 fail-safe 통과. `hooks/hooks.json` UserPromptSubmit 두 번째 entry 로 등록 (기존 `plain-language-reminder` 와 병렬). 로컬 8개 시험 통과 (eng→kor 변환 / 정상 영어·한국어 비검사 / 짧은 입력 무시 / bypass / dispatch / 비-string prompt fail-safe). consumer-guide §2h.4. 근거: 사용자 directive 2026-05-27 (memory `feedback_korean_english_layout_recovery`). CFP-1738 sibling pattern.
- [CFP-1738] **사용자 대화 품질 hook 플러그인 내장 — UserPromptSubmit(reminder) + Stop(check), 모든 consumer 자동 적용** (`hooks/plain-language-reminder` + `hooks/plain-language-check` + `hooks/plain-language-check.py`, `hooks/hooks.json` 에 2 entry 등록). Orchestrator 가 사용자에게 codeforge 내부 식별자(ADR/CFP 번호·§결정·계약명)를 풀이 없이 쓰는 것을 막는다 — 일반 기술 용어(hook/worktree/schema/latency 등)는 회피 대상 아님(사용자=엔지니어 전제). reminder = 순수 bash JSON additionalContext (의존성 0). check = python 으로 transcript JSONL 파싱(이번 turn assistant text 만, python 없으면 fail-safe 통과). 설정 env 3종: `BYPASS_PLAIN_LANGUAGE`(끄기) / `PLAIN_LANG_JARGON_THRESHOLD`(기본 1 = 0 허용) / `PLAIN_LANG_EXTRA_PATTERNS`(consumer 고유 패턴). 한계: Stop hook 은 답 표시 후 작동 → "노출 즉시 재작성"(비노출 아님), 진짜 0 노출은 Orchestrator 규율 + reminder 책임. plugin-root `hooks/hooks.json` 자동 활성 (별도 등록 불필요). 로컬 9개 시험 통과 (block 동작 / 기술용어 비검사 / bypass / loop-guard / consumer 패턴 추가). consumer-guide §2h.3. 근거: 사용자 directive 2026-05-26 (memory feedback_plain_language_to_user / feedback_skip_unneeded_question / feedback_structured_delivery — over-correction 금지: 내부 식별자만 회피).
- [CFP-1685] **ADR-114 신설 — Minimal path direct invocation protocol (full-lane process escalation fallback)** (CFP-1646 retro F4 escalation_action `escalate_user` Option A 사용자 결정). full-lane process ESCALATE_PACKET_INCOMPLETE 시 사용자 directive 기반 minimal path 발동 protocol 정형화. §결정 1 When (FIX iter ≥ 3 ESCALATE + N+ ambiguity blocker + 사용자 directive 필수) / §결정 2 How (close 1st attempt PRs + 새 branch cfp-NNN-v2 + Orchestrator inline write per ADR-039 inline exception + commit + PR + admin merge) / §결정 3 Boundary (single carrier 보존 ADR-064 §결정 5 + lane evidence §10/§14 preservation + ADR-082 Amendment 29 §1-R mid-Story FIX-loop re-verification 정합) / §결정 4 exception path = norm 아님 (full-lane default 우선, 매 발동 §14 `[minimal-path-direct: ...]` marker 의무). CFP-1110 1st applied case + CFP-1646 2nd applied case codify (pattern_count ≥ 2 reach). is_transitional: false permanent governance protocol. mechanical_enforcement_actions [] declaration-only Wave 1 (pattern_count ≥ 3 재발 시 promotion). related_adrs: ADR-039 (inline exception) + ADR-064 §결정 5 (CFP scope unitary) + ADR-067 (Max FIX 3/3) + ADR-082 Amendment 29 §1-R + ADR-085 (multi-session collaboration).
- [CFP-1684] **ADR-082 Amendment 31 — §결정 1 layer 1 sub-scope (1-T) 신설 PMOAgent retro write-time verify-before-trust mandate (META recursive evidence carrier)** (CFP-1646 retro F3 forcing function; 4th parallel race — CFP-1688 Amendment 30 + sub-scope 1-S slot collision → Amendment 31 + 1-T reslot per ADR-082 Amendment 29 §1-R mid-Story FIX-loop re-verification self-apply). retro file write 시 cited fact (commit SHA + label-registry version + bypass family count + cross-Story memory pattern_count) source direct verify 의무. axis disjoint from §결정 9 (CFP-1312 Amd 7 ArchitectAgent + Orchestrator scope) — 본 1-T = PMOAgent retro write scope. META self-application direct evidence: CFP-1646 retro write-time PMOAgent 자체 stale_fact_inheritance 진입 (wrapper merge SHA claim e84f0460 vs actual 00641695 + internal-docs 0b37a71 vs actual 33fff4cf). Wave 1 declarative scope: ADR-082 Amendment 31 신설 (amendments[] + amendment_log[] + body dual-block per Amd 28 parity) + label-registry-v2 v2.81 → v2.82 (`hotfix-bypass:retro-fact-verify-mandate` 107번째 family member) + MANIFEST sync + evidence-checks-registry `retro-fact-verify-mandate` warning-tier entry + workflow Wave 1 stub pair (ADR-005). Wave 2 mechanical wire (Phase 2 PR future): scripts/check-retro-fact-verify.sh + scripts/lib/check_retro_fact_verify.py (ADR-061) + bats RED→GREEN stash proof (CFP-1334) + boundary fixture pair (CFP-963 Codex TP#4).
- [CFP-1683] **ADR-082 Amendment 29 — §결정 1 layer 1 sub-scope (1-R) 신설 mid-Story FIX-loop re-verification mandate** (CFP-1646 retro F2 forcing function 산물, `sister_session_race_in_design_lane` cumulative pattern_count ≥ 5: CFP-684 + CFP-698 + CFP-1041 + CFP-1591 + CFP-1646 3 sub-events). FIX iter ≥ 2 시점 ArchitectPL re-engage prompt 안 3-tuple 재verify 의무 (amendment_id slot + label-registry version + bypass family count) + 3 field presence (amendment_slot_revalidated / registry_version_revalidated / bypass_count_revalidated). axis disjoint from sub-scope 1-G CFP-1435 (Story-start pre-reservation) — 본 1-R = FIX-loop intra-Story window 영역. Wave 1 declarative scope: ADR-082 Amendment 29 신설 (amendments[] + amendment_log[] + body section dual-block) + label-registry-v2 v2.80 → v2.81 (`hotfix-bypass:fix-loop-reverify-mandate` 106번째 family member) + MANIFEST sync + evidence-checks-registry `fix-loop-reverify-mandate` warning-tier entry (status `deferred-followup`) + workflow Wave 1 stub pair (ADR-005). Wave 2 mechanical wire (Phase 2 PR scope, future carrier): scripts/check-fix-loop-reverify.sh + scripts/lib/check_fix_loop_reverify.py (ADR-061) + bats RED→GREEN stash proof (CFP-1334) + boundary fixture pair (CFP-963 Codex TP#4).
- [CFP-1682] **ADR-040 Amendment 7 — §결정 7.J.4 신설 filesystem Read/Grep tool layer worktree-pinned `git show HEAD:<path>` 강제 default + agent spawn prompt `git -C <worktree_abs_path>` directive 강제 expansion** (CFP-1646 retro F1 forcing function 산물, CFP-1646 ArchitectPL FIX iter 1 false premise 사건 carrier — main repo path Grep stale snapshot 차단). ADR-045 §D-9 cumulative pattern_count Mandatory reach evidence (worktree_path_mismatch ≥ 3 + stale_fact_inheritance ≥ 6 + orchestrator_initial_turn_verify_stale ≥ 2). Wave 1 declarative scope: ADR-040 Amendment 7 §결정 7.J.4 신설 + label-registry-v2 v2.79 → v2.80 MINOR (`hotfix-bypass:filesystem-worktree-pinned` 105번째 family member) + MANIFEST sync + evidence-checks-registry `filesystem-worktree-pinned-default` warning-tier entry (status `deferred-followup`, owner_adr ADR-040-Amendment-7) + workflow Wave 1 stub pair (templates + .github byte-identical, ADR-005 invariant PASS). Wave 2 mechanical wire (Phase 2 PR scope, future carrier): scripts/check-filesystem-worktree-pinned.sh (ADR-061) + scripts/lib/check_filesystem_worktree_pinned.py (ADR-061 multi-line Python external) + bats RED→GREEN stash proof (CFP-1334) + boundary fixture pair (CFP-963 Codex TP#4) + evidence-registry status `deferred-followup` → `warning` 전환.
- [CFP-1680] **ADR-045 Amendment 11 — §D-11 신설 PMOAgent retro batch closure pattern normative codify (HIGH normative consolidation carrier from batch-close 2026-05-26 retro Candidate A+B)** (doc-only fast-path per ADR-054 Cat 1, Issue #1680 HIGH carrier — 6 applied case evidence base pattern_count 6 reach threshold 2 = 3.0x: CFP-963 retro 4-batch + CFP-1339 retro 5-batch + CFP-1612 retro 3-batch + CFP-1637 retro 3-batch + CFP-1648 retro deferred (DEFER decision evidence) + batch-close 2026-05-26 본 carrier 자체 retro)
  - **§D-11 신설** (axis 분리 codify, post-batch governance status update lifecycle disjoint axis vs §D-9 post-hoc threshold escalation + §D-10 pre-publish 8-tuple verify gate)
  - **§D-11 (1) Decision enum closed-set 4-option**: `CLOSE_AS_OBVIATED` (recent carrier resolution direct merge link verify) / `CLOSE_AS_SENTINEL` (declarative monitor only, pattern_count not reached) / `PROMOTE` (pattern_count reached, active Story 발의 + label priority:P1) / `DEFER` (keep open, future carrier 대기, rationale 명시)
  - **§D-11 (2) Verify-before-trust 5 sub-scope mandate**: (a) per-Issue body verbatim cite (ADR-082 §결정 1 layer 1 sub-scope (1-C) USER-UTTERANCE-VERBATIM 패턴 답습) / (b) recent merge state direct verify gh api + git log (ADR-073 verify-before-assert primitive 답습) / (c) axis disjoint discrimination false-positive obviation 차단 (ADR-082 §결정 12 정합) / (d) sibling carrier cross-link via PR number (ADR-082 §결정 9 verify-before-cite 양방향 답습) / (e) sub-scope alphabet sequential verify pre-write 위치 확인 (ADR-082 §결정 1 sub-scope codify 패턴 답습)
  - **§D-11 (3) Closure summary table SSOT format**: 5-column `Issue / Tier / Decision / Final state / Comment URL` (ADR-068 I-4 wording SSOT invariant 정합 batch closure 영역)
  - **§D-11 (4) Cross-Story pattern_count progression**: pre-batch / post-batch / threshold / status field codify, Net escalation 0 시 `cross_story_pattern_adr_trigger` field empty (closure ≠ pattern increment, §D-9 escalation 영역 disjoint axis)
  - **§D-11 (5) Closure forcing function 3 step**: (1) `[PMO]` prefix comment + state transition / (2) Retro PR open + auto-merge closure evidence trail (ADR-045 §결정 4 정합) / (3) `gate:retro-complete` label add OR `not_planned` reason close (ADR-045 §결정 5 close-blocking 정합)
  - **Wave 1 → Wave 2 split 적용 영역 외** (declarative anchor only — batch close = governance status update mechanical wire 영역 disjoint, ADR-076 declarative reconciliation 패턴 답습 보류, pattern_count ≥ 9 reach 시 warning → blocking-on-pr 승격 Wave 3 별 carrier 분리 ADR-060 §결정 6 promotion gate AND 3/3)
  - **META 24th applied case**: ADR-082 §결정 1-K 4-step numeric claim write-time verify-before-write mandate self-execute, CFP-1632 = 21st / CFP-1648 = 22nd / CFP-1637 = 23rd / 본 CFP-1680 = 24th — pattern_count 6 reach 자체 verify-before-write self-execute recursive dogfooding self-evidence (normative consolidation Story 자체가 batch close pattern 7th applied)
  - **Files**: `archive/adr/ADR-045-story-retro-mandatory-trigger.md` (frontmatter amendment_log[] Amendment 11 entry + related_stories[] CFP-1680 entry + §D-11 본문 8 sub-section append) / `archive/adr/ADR-RESERVATION.md` (amendments_reserved[] row sequential append — adr_number 45 amendment_id 11 reserved_by_cfp CFP-1680 reservation_date 2026-05-26 KST status active per F-DR-003) / `docs/orchestrator-playbook.md` (§18 신설 paired sibling carrier — PMOAgent retro batch closure operating sequence 5 sub-section §18.1 trigger conditions / §18.2 4-option enum decision tree / §18.3 verify-before-trust mandate workflow / §18.4 closure summary table SSOT format / §18.5 retro PR auto-merge sequence) / Story file `c:/workspace/mclayer/codeforge-internal-docs/plugin-codeforge/stories/CFP-1680.md` (codeforge-internal-docs SSOT ADR-013 dogfood-out) / Change Plan `c:/workspace/mclayer/codeforge-internal-docs/plugin-codeforge/change-plans/cfp-1680-adr045-amd-11-pmo-batch-closure.md` (codeforge-internal-docs SSOT ADR-013 dogfood-out) / `CHANGELOG.md` (본 entry append per F-DR-002)
  - **Cross-ref**: Issue #1680 HIGH normative consolidation from batch-close retro Candidate A+B / ADR-045 §D-9 (post-hoc threshold escalation, disjoint axis) / ADR-045 §D-10 (pre-publish 8-tuple verify gate, disjoint axis) / ADR-082 §결정 9 (verify-before-cite 양방향) / ADR-082 §결정 12 (retro-time verify-before-trust adjacent axis) / ADR-073 (verify-before-assert primitive) / ADR-076 (declarative reconciliation Wave 1 → Wave 2 split precedent retained) / ADR-054 Cat 1 doc-only fast-path
- [CFP-1646] **ADR-024 Amendment 17 — §결정 6.A.9 신설 Issue label timing discipline (backlog `phase:*` reflex 차단) + `hotfix-bypass:backlog-issue-label` 104번째 family member + label-registry-v2 v2.78 → v2.79 MINOR bump** (minimal path direct per ADR-039 inline exception + CFP-1110 precedent invocation, Issue #1646 USER-UTTERANCE-VERBATIM CFP-1111 follow-up #1198/#1199 phase:설계 reflex incident origin)
  - **§결정 6.A.9 신설** (Issue label timing discipline) — 즉시 lane 진입하지 않는 backlog / 예약 / follow-up CFP Issue 발의 시 `phase:*` label 부착 금지. 미착수 backlog 관례 = unlabeled (Issue #1112 / #1169 prior art pattern). 본 invariant = "발의-time 에 lane spawn 자동 follow 가정 차단".
  - **Backlog 상태 식별 mechanical proxy 3-AND predicate**: (a) `state:open` (Issue 자체 열림) + (b) `assignee:none` (담당자 미할당) + (c) `linked_pr:none` (linked PR 부재). 3 AND 만족 = backlog → lint 발화 (warning tier). 한 조건이라도 false = 착수 가능성 신호 (lint 발화 부적합, false-positive 차단).
  - **Bypass channel** = `hotfix-bypass:backlog-issue-label` 104번째 (raw active concrete grep count post-append, 103 + 1 = 104) family member — §결정 6.A per-entry namespace 영구 확장. 기존 single audit lint `scripts/check-bypass-audit-comment.sh` reuse (CFP-389 prior art, 신규 audit lint 도입 0건).
  - **Behavioral directive normative 승격 첫 carrier** (CLAUDE.md "Orchestrator 정책 적용 범위 + behavioral directive (normative)" 단락 정합) — 사용자 directive 를 personal memory file 저장 만으로 갈음 금지. 본 Story = memory `feedback_no_phase_label_on_backlog_issues.md` (ephemeral + consumer 비전파 + single-session scope = structural enforcement 불가) → ADR-024 normative SSOT 승격 첫 사례.
  - **Wave 1 declarative (본 Phase 1 PR scope)**: ADR-024 Amendment 17 본문 + frontmatter `amendments[]` row + `mechanical_enforcement_actions[]` row + `amended_by/amended_date` 갱신 / label-registry-v2 v2.78 → v2.79 MINOR bump + `hotfix-bypass:backlog-issue-label` 104번째 family member append + amendment_log row / MANIFEST.yaml `"2.78"` → `"2.79"` sync / `templates/github-workflows/backlog-issue-label-forbid.yml` Wave 1 stub + `.github/workflows/` self-app byte-identical (ADR-005).
  - **Wave 2 (Phase 2 PR scope — future carrier)**: workflow body actual lint wire (3-AND predicate detection) + `scripts/check-backlog-issue-label.sh` thin-wrapper (ADR-061) + `scripts/lib/check_backlog_issue_label.py` (ADR-061 multi-line Python external file) + `tests/bats/check-backlog-issue-label.bats` RED→GREEN stash proof (CFP-1334 mandate) + `tests/fixtures/backlog-issue-label/{pass,fail}/` boundary fixture pair (CFP-963 Codex TP#4).
  - **Memory retire (post-merge별 task)**: `feedback_no_phase_label_on_backlog_issues.md` (normative 승격 완료) + `feedback_brainstorming_why_first.md` (skill `codeforge:codeforge-brainstorm` body L63/66/100/140-145 9 grep hits HIT verified, retire only) + `feedback_codex_collaboration_patterns.md` (ADR-052 + ADR-070 + ADR-081 codified, retire only).
  - **Full lane attempt audit (PRs closed #1664 + #941)**: RequirementsPL PROCEED → ArchitectPL FIX iter 1 (false premise, main repo path stale) → SendMessage premise resolution → ArchitectPL PASS → DesignReviewPL FIX iter 2 (3 findings) → ArchitectPL FIX iter 2 closure → DesignReviewPL re-review PASS → merge attempt DIRTY (CFP-1657 parallel race Amd 16 § 6.A.8 slot collision) → ArchitectPL FIX iter 3 ESCALATE (3rd parallel race CFP-1648 v2.78 + 103rd slot collision) → user minimal path 선택 (CFP-1110 precedent invocation, Orchestrator inline exception per ADR-039) → 본 PR #NEW. Audit trail = Issue #1646 + closed PR #1664/#941 + Story §10/§14 (3 parallel race + 3 FIX iter + ESCALATE_PACKET_INCOMPLETE).
  - **PMOAgent retro 의무 (post-merge auto-spawn)**: ADR-045 §D-9 escalation territory — multiple super-class pattern_count surface from full-lane attempt: `stale_fact_inheritance` (Orchestrator initial Grep stale main repo path + ArchitectPL inline verify falsified) + `worktree_path_mismatch` (ADR-040 Amd 6 §결정 7.J.3 directive violation) + `orchestrator_initial_turn_verify_stale` (SessionStart hook 18-commit-behind warning 무시) + `sister_session_race_in_design_lane` pattern_count 3 reach Mandatory (CFP-1657 + CFP-1648 dual collision) + `parallel_race_recursive_dogfooding` (3 race events on same Story) + `cross_session_state_ambiguity` (cfp-1646 vs cfp-1646-pr branch reflog). sister carrier escalation_action `adr_draft_emitted` 후보 (ADR-040 Amendment 7 or ADR-073 Amendment 17 — worktree absolute path 강제 directive mechanical enforcement lint).
  - **Files**: `archive/adr/ADR-024-story-scoped-branch-policy.md` (frontmatter amendments[] Amendment 17 row + `amended_by`/`amended_date` + `mechanical_enforcement_actions[]` row + 본문 `## Amendment 17` section) / `docs/inter-plugin-contracts/label-registry-v2.md` (frontmatter version `"2.78"` → `"2.79"` + amendment_log row append + `hotfix-bypass:backlog-issue-label` 104번째 family member entry append) / `docs/inter-plugin-contracts/MANIFEST.yaml` (label_registry row `"2.78"` → `"2.79"` sync) / `templates/github-workflows/backlog-issue-label-forbid.yml` (Wave 1 declarative stub) / `.github/workflows/backlog-issue-label-forbid.yml` (self-app byte-identical, ADR-005) / `CHANGELOG.md` (본 entry append)
  - **Cross-ref**: Issue #1646 USER-UTTERANCE-VERBATIM / closed PR mclayer/plugin-codeforge#1664 + mclayer/codeforge-internal-docs#941 (full-lane attempt audit) / CFP-1657 (parallel race carrier — Amd 16 § 6.A.8 slot occupied) / CFP-1648 (parallel race carrier — v2.78 + 103rd slot occupied) / CFP-1110 (minimal path precedent invocation source) / ADR-039 (inline exception per user directive) / prior art Amd 11 (raw count convention) / Amd 13/14 (§ 6.A.N sub-decision pattern) / Amd 16 (CFP-1657 chronological direct precedent)
- [CFP-1648] **ADR-082 Amendment 28 — §결정 1 layer 1 sub-scope (1-Q) 신설 ADR dual-block parity 3-invariant forward-prevention lint (F-DR-001 P0 origin sentinel mechanical carrier)** (Combined Phase 1+2, Issue #1648 MEDIUM follow-up from CFP-1637 retro — F-DR-001 P0 origin: Amendment N frontmatter amendment_log[] entry present but body ## Amendment N section missing pattern forward-prevention mechanical lint). 3-invariant check: Block 1 (amendments[] ↔ body H2 parity) / Block 2 (amendment_log[] ↔ body H2 parity — F-DR-001 P0 sentinel) / Block 3 (amendments[] ↔ amendment_log[] cross-count parity). Python SSOT `scripts/lib/check_adr_dual_block_parity.py` (ADR-061 thin wrapper convention, anchored simple regex CodeQL ReDoS guard per ADR-061 Amendment 3 §결정 11, line-by-line parse, PER_LANE_EVIDENCE_SCAN_CAP=30, Windows cp949 reconfigure) + bash thin wrapper `scripts/check-adr-dual-block-parity.sh` + D2 dual trigger workflow `templates/github-workflows/adr-dual-block-parity.yml` + `.github/workflows/adr-dual-block-parity.yml` byte-identical mirror (ADR-005) + bats 8 TC RED→GREEN stash proof (CFP-1334 §8.4 5 markers) + evidence-checks-registry warning-tier initial registration + label-registry-v2 MINOR bump v2.77 → v2.78 (`hotfix-bypass:adr-dual-block-parity` 103번째 hotfix-bypass:* family member) + MANIFEST.yaml label_registry version 갱신. ADR-060 §결정 5 first introduction = warning mode (continue-on-error: true). Wave 1→Wave 2 combined single Phase 1+2 PR (no split — mechanical wire included). META-self-applied: F-DR-001 P0 sentinel = the exact pattern this lint is designed to catch (ADR amendment_log[] entry present but body section missing), applied to ADR-082 Amendment 28 own amendment_log[] + body section dual-write validation.
  - **Files**: `scripts/check-adr-dual-block-parity.sh` (bash thin wrapper per ADR-061) / `scripts/lib/check_adr_dual_block_parity.py` (Python SSOT, 3-invariant check logic) / `templates/github-workflows/adr-dual-block-parity.yml` (D2 dual trigger fixture) / `.github/workflows/adr-dual-block-parity.yml` (byte-identical mirror) / `tests/scripts/check-adr-dual-block-parity/` (bats 8 TC + 4 fixtures) / `docs/evidence-checks-registry.yaml` (adr-dual-block-parity warning-tier entry) / `docs/inter-plugin-contracts/label-registry-v2.md` (v2.77 → v2.78, 103rd family member) / `docs/inter-plugin-contracts/MANIFEST.yaml` (label_registry version row 갱신) / `archive/adr/ADR-082-write-time-self-write-verification-mandate.md` (Amendment 28 sub-scope 1-Q dual-block append)
  - **Cross-ref**: Issue #1648 MEDIUM FU from CFP-1637 retro / CFP-1637 retro F-DR-001 P0 origin / ADR-082 Amendment 28 §결정 1 layer 1 sub-scope (1-Q) carrier / ADR-060 §결정 5 warning-tier first introduction / ADR-108 §결정 3 103번째 family member parity mandate
- [CFP-1647] **ADR-082 Amendment 27 — §결정 1 layer 1 sub-scope (1-P) 신설 sub-scope 1-O Wave 2 mechanical enforcement wire (declaration → mechanical enforce promotion path PR-level artifact scope)** (doc-only fast-path per ADR-054 Cat 1, Issue #1647 HIGH FU-A from CFP-1637 retro carrier — sub-scope 1-O Amd 26 CFP-1637 Wave 1 declaration-only behavioral mandate (PR commit message + PR body layer numeric claim write-time strict claim mandate) 의 mechanical enforce 부재 영역 codify). 4-quadrant matrix codify: governance docs / PR-level artifact × declaration / Wave 2 wire = (1-K Amd 22, 1-N Amd 25) governance docs pair + (1-O Amd 26, 1-P 본 Amd 27) PR-level artifact pair. 4-tuple primitive: (a) Python SSOT `scripts/lib/check_numeric_claim_write_time.py` (CFP-1612 wired, governance docs scope 활성) detection scope expansion to PR commit message body + PR description body layer + bash thin wrapper scope flag (`--scope pr-commit-msg` + `--scope pr-body` 또는 unified `--scope all` default) per ADR-061 / (b) 6 dimension detection logic 재사용 (line/file/API/pattern/commit/row count) Python dict SSOT regex+source command pattern map 재사용 / (c) PR-level source ingestion (`gh pr view --json title,body,commits` + `git log --format=%B <range>`) + FP guard 4종 재사용 (code-span / quoted / templates/** / docs/stories/§9) + PER_BLOCK_SCAN_CAP=50 ReDoS guard 재사용 / (d) workflow trigger 확장 (PR open + sync event 추가) + `.github/workflows/` byte-identical mirror per ADR-005 + 기존 `continue-on-error: true` warning tier 유지 + 기존 `hotfix-bypass:numeric-claim-write-time-verify` bypass label 재사용 (bypass label 신설 0건 — scope expansion only, label-registry MINOR 0건) + evidence-checks-registry entry `numeric-claim-write-time-verify` target_section 갱신 (`§결정 1 layer 1 sub-scope (1-K + 1-N + 1-O + 1-P)`). Wave 1 declarative-only — Wave 2 actual script/workflow/bats wire 별 sub-CFP carrier (Phase 2 PR — DeveloperPL + QADev spawn). Wave 1 → Wave 2 split precedent CFP-1437/1436/1435/1559/1578/1601/1590/1589/1612/1637 13번째 instance (CFP-1612 = 11th + CFP-1637 = 12th + 본 CFP-1647 = 13th sequential lineage extension). META-self-applied 22nd applied case (CFP-1637 = 21st post-merge → CFP-1647 = 22nd sequential carrier). pattern_count ≥ N 추가 reach 시 warning → blocking-on-pr 승격 = Wave 3 별 carrier 분리 (ADR-060 §결정 6 promotion gate AND 3/3).
  - **Files**: `archive/adr/ADR-082-write-time-self-write-verification-mandate.md` (frontmatter amendments[] Amendment 27 entry + amendment_log[] entry dual-block parity per F-DR-001 + related_stories[] CFP-1647 + CFP-1637 entries + mechanical_enforcement_actions[] entry `numeric-claim-write-time-verify` target_section 갱신 (1-K + 1-N + 1-O + 1-P 4 sub-scope) + §결정 1 본문 sub-scope 1-P section append) / `archive/adr/ADR-RESERVATION.md` (amendments_reserved[] row sequential append after row 26 — adr_number 82 amendment_id 27 reserved_by_cfp CFP-1647 reservation_date 2026-05-26 KST status active per F-DR-003) / Story file `c:/workspace/mclayer/codeforge-internal-docs/plugin-codeforge/stories/CFP-1647.md` (codeforge-internal-docs SSOT ADR-013 dogfood-out) / Change Plan `c:/workspace/mclayer/codeforge-internal-docs/plugin-codeforge/change-plans/cfp-1647-adr082-amd-27-1p.md` (codeforge-internal-docs SSOT ADR-013 dogfood-out) / `CHANGELOG.md` (본 entry append per F-DR-002)
  - **Cross-ref**: Issue #1647 HIGH FU-A from CFP-1637 retro / CFP-1637 (sub-scope 1-O Amd 26 Wave 1 declaration-only mandate origin) / CFP-1612 (1-N Wave 2 mechanical wire governance docs scope — Python SSOT base for scope expansion) / CFP-1601 (1-K Wave 1 declaration governance docs scope — original mandate) / Wave 1→Wave 2 split precedent 13th instance lineage CFP-1437/1436/1435/1559/1578/1601/1590/1589/1612/1637
- [CFP-1637] **ADR-082 Amendment 26 — §결정 1 layer 1 sub-scope (1-O) 신설 PR commit message + PR body layer numeric claim write-time strict claim mandate (3rd axis disjoint codify)** (doc-only fast-path per ADR-054 Cat 1, Issue #1637 Pattern 2 Mandatory escalation from CFP-1612 retro per ADR-045 §D-9 pattern_count 2 reach `meta-self-application-accuracy-violation` — 1st: CFP-1601 §13.C row 3 claim 11 vs actual 15, 2nd: CFP-1612 PR #1631 commit msg LOC drift "29 / ~470 / ~155" vs canonical actual "30 / 605 / 183"). Axis disjoint matrix codify: sub-scope 1-K (Amd 22 CFP-1601, write-time governance docs scope) + sub-scope 1-N (Amd 25 CFP-1612, Wave 2 mechanical wire scope) + 본 sub-scope 1-O (Amd 26 CFP-1637, PR commit msg + PR body 3rd axis). Same 6 numeric closed-set dimension (line/file/API/pattern/commit/row count) + same 4-step verify-before-write mandate inherit from 1-K. Wave 1 declarative-only — Wave 2 mechanical wire (Python SSOT `check_numeric_claim_write_time.py` scope expansion to PR commit msg + PR body detection layer) = 별 sub-CFP carrier (Wave 1 → Wave 2 split precedent 12th instance). META-self-applied 21st applied case (CFP-1612 = 20th post-merge → CFP-1637 = 21st sequential carrier). FIX iter 1 inline mechanical_fast_path_inline applied: amendment_log entry append (F-DR-001 P0) + 본 CHANGELOG entry add (F-DR-002 P0) + ADR-RESERVATION row sequential placement (F-DR-003 P1).
- [CFP-1612] **ADR-082 Amendment 25 — §결정 1 layer 1 sub-scope (1-N) 신설 numeric claim write-time verify Wave 2 mechanical enforcement wire** (doc-only fast-path per ADR-054 Cat 1, Issue #1612 HIGH FU-A from CFP-1601 retro carrier — sub-scope 1-K Amd 22 CFP-1601 Wave 1 declaration-only behavioral mandate 의 Wave 2 mechanical lint enforcement wire (declaration → mechanical enforce promotion path), mid-flight CFP-1589 collision recovery — initial Amd 24+1-M → renumber Amd 25+1-N on rebase to origin/main `5b5c9f7b`)
  ...
  - **Files**: `archive/adr/ADR-082-write-time-self-write-verification-mandate.md` (frontmatter amendments[] Amendment 25 entry + amendment_log[] entry + related_stories[] + mechanical_enforcement_actions[] entry `numeric-claim-write-time-verify` status 갱신 + §결정 1 본문 sub-scope 1-N section append) / `archive/adr/ADR-RESERVATION.md` (amendments_reserved[] row append — adr_number 82 amendment_id 25 reserved_by_cfp CFP-1612) / `docs/evidence-checks-registry.yaml` (numeric-claim-write-time-verify warning-tier initial registration entry append, ADR-060 §결정 5 정합) / `docs/inter-plugin-contracts/label-registry-v2.md` (MINOR bump v2.75 → v2.76 + hotfix-bypass:numeric-claim-write-time-verify 101st family member append) / `docs/inter-plugin-contracts/MANIFEST.yaml` (label_registry version `2.75` → `2.76` 갱신) / `CHANGELOG.md` (본 entry append)
...
- [CFP-1592] **ADR-045 Amendment 8 — §D-9 sub-decision (b) 신설 PMOAgent retro batch §6 ADR draft pre-publish verify-before-trust 3-source AND mandate (META Wave 5)** (doc-only fast-path per ADR-054 Cat 1, ADR-045 §D-9 Mandatory escalation pattern_count 6 reach `retro_batch_adr_draft_stale_premise` super-class)
  - **§D-9 sub-decision (b) 신설** (additive ratchet 강화 방향, sub-decision (a) RETAIN — pattern_count ≥ 2 → escalation_action enum 2-value invariant 보존, 본문 변경 0건): PMOAgent retro file write 시점 §6 ADR 후보 발의 sub-section 안 매 ADR draft 후보 (escalation_action: adr_draft_emitted 또는 escalate_user 모두) 작성 직전 3-source AND verify gate 통과 의무
  - **3-source AND gate**: (1) `git show origin/main:<ADR-path>` direct frontmatter amendment count verify — target ADR 존재 + amendment_log[] max 확인 / (2) `grep <feature-name> docs/evidence-checks-registry.yaml` — mechanical lint 이미 등록 여부 직접 grep / (3) `Glob scripts/check-<feature-pattern>*` — 실 script 이미 존재 여부 직접 Glob
  - **Gate fail behavior** (1+ source 부재 = stale-premise 확률 높음): `[fact-check-pending]` annotation 의무 + §6 후보 downgrade to §4 informational only OR PIVOT mark (`pmo_output v1.cross_story_pattern_adr_trigger.escalation_action: escalate_user` enum retain false positive 안전망)
  - **Gate PASS behavior** (3 source AND 모두 PASS): §6 후보 정식 발의 (`escalation_action: adr_draft_emitted`) → ArchitectAgent spawn → 신규 ADR 또는 Amendment carrier 작성
  - **pattern_count 6 reach evidence** (≫ threshold 2 = Mandatory framing 발동): (1) CFP-1006 Tier-B Wave 1 (2026-05-19) auto-resolve assumption stale → CFP-1025 root cause codify post-merge / (2) CFP-1542 CFP-FU-A/B retro batch bypass-cumulative-counter mechanical wire 신규 → CFP-845 (ADR-024 Amd 8) MERGED 2026-05-17 already shipped / (3) CFP-1558 CFP-FU-A/B retro batch amendment-number-frontmatter-verify lint 신규 → CFP-1198/1216/1312 MERGED 2026-05-21~23 Wave 1+2+bidirectional already shipped / (4)(5)(6) CFP-1604/1605/1606 Wave 4 batch 3 stale catches — 60% PIVOT rate + 100% retro-batch-origin rate
  ...
  - **forcing function self-strengthening**: sub-decision (a) (post-detection escalation enforce mechanism) ↔ sub-decision (b) (pre-publish stale-premise prevention mechanism) = temporal split disjoint axis (pre vs post detection). forcing function 약화 0건 — pre-publish gate 는 post-detection escalation 의 quality 강화 layer
  ...
  - **paired sibling carrier**: CFP-1623 = Wave 2 mechanical wire prerequisite carrier (sequential after #1592 merge — declarative anchor 발효 후 mechanical wire activation, CFP-825 → CFP-845 → CFP-1198~1216~1312 → CFP-1580 → CFP-1592 5-Story Wave 1 → Wave 2 split precedent 답습). pmo-output-v1 schema 확장 (3-source verify field) 영역 = #1623 carrier
  - **§D-9 forcing function 산물 7번째 evidence**: Amendment 6 CFP-776 ADR-082 carrier = 5번째 / Amendment 7 CFP-1580 ADR-068 Amd 4 carrier = 6번째 / 본 Amendment 8 = 7번째. 본 Amendment 8 는 **evidence + sub-decision (b) 신설** dual carrier — Amendment 6/7 (evidence-only) 대비 §D-9 본문 자체 (sub-decision layer) 강화 ratchet 동반
  - **META-self-applied**: ADR-082 §결정 1 sub-scope 1-G strict pre-reservation claim mandate 6th applied case after Amd 18 CFP-1342 + Amd 21 CFP-1578 + Amd 22 CFP-1601 + Amd 23 CFP-1590 + Amd 24 CFP-1589 + 본 carrier — ADR-RESERVATION amendments_reserved[] row pre-claim commit `4eaa3ff8` + push (Amd 17 1-G precedent 답습)
  - **Files**: `archive/adr/ADR-045-story-retro-mandatory-trigger.md` (frontmatter amendment_log[] entry Amendment 8 entry + related_stories[] CFP-1592 + CFP-1623 append + 본문 ## Amendment 8 section append) / `archive/adr/ADR-RESERVATION.md` (amendments_reserved[] row append — adr_number 45 amendment_id 8 reserved_by_cfp CFP-1592) / `CLAUDE.md` (Verify-before-trust 4-layer governance 단락 ADR-045 §D-9 layer (4) Amendment 8 cross-ref append, 320 line cap CFP-506 정합)
  - **Cross-ref**: Issue #1592 / paired sibling Issue #1623 (Wave 2 mechanical wire) / Story file `mclayer/codeforge-internal-docs/plugin-codeforge/stories/cfp-1592.md` / ADR-082 §결정 12 (RequirementsPL + retro-time Wave-defer empirical verify, adjacent disjoint axis collision NONE verified) / ADR-097 paradigm scope boundary (sub-decision 확장 = ratchet 강화 carve-out)

- [CFP-1623] **ADR-045 Amendment 9 — §D-10 신설 PMOAgent retro batch §6 ADR draft pre-publish 8-tuple verify-before-trust forcing function (paired sibling sequential second after Amendment 8 CFP-1592 §D-9 sub-decision (b) 3-source AND)** (doc-only fast-path per ADR-054 Cat 1, Wave 4 batch 6 cumulative PIVOT evidence 100% retro-batch-origin discriminator)
  - **8-tuple verify sources AND gate**: (1) `git show origin/main:<ADR-path>` frontmatter `amendment_log` direct read / (2) `grep <feature-name> docs/evidence-checks-registry.yaml` mechanical lint 등록 / (3) `Glob scripts/check-<feature-pattern>*` 실 script / (4) `gh pr list --search "<feature-name> in:title" --state merged` sibling carrier merge status / (5) `gh issue list --search "<feature-name> in:title" --state all` existing CFP carrier / (6) `git log --all --oneline -- <path>` file-level historical change / (7) `Glob archive/adr/ADR-*.md` + frontmatter `amendment_log` cross-Story scan / (8) retro §5 cross-Story pattern table `anchor_id` ↔ existing implementation 매핑. 1+ source disagree → `downgrade_action` enum 2-value mandatory (`downgrade_to_section_4_informational_only` / `pivot_mark`)
  - **axis 분리 codify (3-layer forcing function 시점 cover)**: §D-9 (Amendment 5 CFP-665) = retro write 완료 후 cross-Story pattern_count 누적 → ADR escalation (post-hoc aggregation) ↔ §D-9 sub-decision (b) (Amendment 8 CFP-1592 paired sibling) = retro batch §6 ADR draft 후보 작성 직전 3-source AND verify (declarative anchor) ↔ 본 §D-10 (Amendment 9) = §6 ADR draft section authoring 직전 8-tuple AND gate (preflight expansion forcing function)
  - **Platform 한계 영역 처리**: `[verification-out-of-scope: <사유>]` marker (ADR-052 Amendment 3 marker 5종 정합) — gh CLI rate-limit (source 4+5 skip → 6-tuple AND) / shallow clone (source 6 skip → 7-tuple AND). 단일 source 미충족 ≠ gate fail (사유 marker 의무, reverse-explicit annotation)
  - **pmo-output-v1 v1.3 optional field 신설** (Wave 1 declarative): `retro_section_6_pre_publish_verify` (3 sub-field: `verify_sources_attempted[]` enum / `verify_sources_blocked[]` 사유 / `downgrade_action` enum 2-value `null|to_section_4_informational|pivot_mark`). Wave 2 mechanical wire = 별 sub-CFP carrier defer (scripts + workflow + evidence-checks-registry entry + label-registry MINOR + codeforge-pmo sibling sync)
  - **pattern_count 6 cumulative evidence**: Wave 1 CFP-1006 1 + Wave 3 CFP-1542+CFP-1558 2 + Wave 4 CFP-1604+CFP-1605+CFP-1606 3 = **6 ≫ threshold 2 (3.0x)**. retro-batch-origin discriminator confirmed (5/5 PIVOT rate retro-batch-origin Stories vs 0/2 non-retro-batch CFP-1603/CFP-1607)
  - **paired sibling cross-ref**: Amendment 8 (CFP-1592) §D-9 sub-decision (b) 3-source AND declarative anchor — sequential dispatch convention (#1592 먼저 land → 본 §D-10 후속, Story §7 권장). Amendment 5 (CFP-665) §D-9 retain invariant 보존 (Amendment 9 = §D-10 sub-decision append only, §D-9 본문 의미 변경 0). Amendment 6 (CFP-776) §D-9 evidence Amendment precedent 답습. Amendment 7 (CFP-1580) §D-9 evidence 6번째 precedent 답습. ADR-082 §결정 12 (RequirementsPL + retro-time verify-before-trust) adjacent disjoint axis (write-time vs pre-publish discipline 명확 분리)
  - **META-self-applied** (ADR-082 §결정 1 sub-scope (1-G) strict pre-reservation claim mandate 7th applied case after Amd 18 CFP-1342 + Amd 21 CFP-1578 + Amd 22 CFP-1601 + Amd 23 CFP-1590 + Amd 24 CFP-1589 + Amd 8 CFP-1592 paired sibling + 본 carrier): ADR-RESERVATION amendments_reserved[] row pre-claim (adr_number 45 / amendment_id 9 / reserved_by_cfp CFP-1623) + ADR-082 §결정 1 sub-scope 1-K numeric claim write-time strict claim mandate 2nd applied case (META): 본 Amendment 번호(9) 가 target ADR-045 frontmatter `amendment_log:` Read verify (origin/main HEAD `8bce3647` amendment_log[] max=7 + cfp-1592-wave5 branch pre-claim Amendment 8 = paired sibling sequential precedent → 정확 next-slot for CFP-1623 = 9 sequential after #1592)
  - **Files**: `archive/adr/ADR-045-story-retro-mandatory-trigger.md` (frontmatter amendment_log[] Amendment 9 entry + Amendment 9 §D-10 본문 section append + related_stories[] CFP-1592 + CFP-1623 append) / `archive/adr/ADR-RESERVATION.md` (amendments_reserved[] row append — adr_number 45 amendment_id 9 reserved_by_cfp CFP-1623) / `CLAUDE.md` (SessionStart retro alert scan 단락 §D-9 sub-decision (b) + §D-10 paired sibling cross-ref 1줄 inline append, 320/320 line cap invariant 보존)
  - **Cross-ref**: paired sibling carrier CFP-1592 (Amendment 8 §D-9 sub-decision (b) 3-source AND declarative anchor, sequential first land) / Issue #1623 / sister carriers CFP-1006 + CFP-1542 + CFP-1558 + CFP-1604 + CFP-1605 + CFP-1606 (Wave 1 + Wave 3 + Wave 4 batch precedents pattern_count 6 cumulative evidence base) / non-retro-batch-origin discriminator CFP-1603 + CFP-1607 (0/2 PIVOT rate)
  - Story SSOT: `<internal-docs>/plugin-codeforge/stories/cfp-1623.md` §3·§7·§11 append (declarative-only Wave 1, code change 0)
  - Change Plan SSOT: `<internal-docs>/plugin-codeforge/change-plans/cfp-1623-pmo-retro-8tuple-verify.md`
...
- [CFP-1589] **ADR-082 Amendment 24 — §결정 1 layer 1 sub-scope (1-M) 신설 own-author synthesis 보고 vs actual git commit gap verify mandate (F-DR-003 carrier)** (doc-only fast-path per ADR-054 Cat 1, CFP-1523 retro carry-over — F-DR-002 P0 finding ArchitectPL verdict packet 'Artifacts written: ...' 보고 ≠ actual git commit DesignReviewPL audit FIX iter 1 dispatch)
  ...
  - **Files**: `archive/adr/ADR-082-write-time-self-write-verification-mandate.md` (frontmatter amendments[] Amendment 24 entry + amendment_log[] entry + related_stories[] + related_files[] + mechanical_enforcement_actions[] append + §결정 1 본문 sub-scope 1-M section append) / `archive/adr/ADR-RESERVATION.md` (amendments_reserved[] row append — adr_number 82 amendment_id 24 reserved_by_cfp CFP-1589) / `CLAUDE.md` (verify-before-trust 4-layer 단락 Amendment 24 cross-ref append, 320/320 line cap invariant 보존) / `docs/evidence-checks-registry.yaml` (synthesis-vs-commit-gap-check warning-tier deferred-followup entry append)
...
- [CFP-1590] **ADR-082 Amendment 23 — §결정 1 layer 1 sub-scope (1-L) 신설 spawn prompt fact verify-before-trust mandate (upstream-inherited stale fact carrier super-class)** (doc-only fast-path per ADR-054 Cat 1, PMOAgent CFP-1523 retro §5 inline ADR draft Orchestrator → ArchitectAgent spawn carry-over carrier, ADR-045 §D-9 Mandatory escalation pattern_count 3 reach, mid-flight CFP-1601 collision recovery — initial Amd 22+1-K → renumber Amd 23+1-L on rebase to origin/main `4c668913`)
  ...
  - **Files**: `archive/adr/ADR-082-write-time-self-write-verification-mandate.md` (frontmatter amendments[] Amendment 23 entry + amendment_log[] entry + related_stories[] + related_files[] + mechanical_enforcement_actions[] append + §결정 1 본문 sub-scope 1-L section append) / `archive/adr/ADR-RESERVATION.md` (amendments_reserved[] row append — adr_number 82 amendment_id 23 reserved_by_cfp CFP-1590) / `CLAUDE.md` (verify-before-trust 4-layer 단락 Amendment 22 + Amendment 23 cross-ref append) / `docs/evidence-checks-registry.yaml` (spawn-prompt-fact-verify warning-tier deferred-followup entry append)
...
- ADR-037 §결정 1(a) PATCH — additive runtime hook infrastructure (UserPromptSubmit `userprompt-submit` dispatcher, CFP-1741) + 추가 hook 2종 (plain-language reminder/check CFP-1738 / Korean-English recovery CFP-1751, prior [Unreleased]) 모두 advisory layer (Stop/SubagentStop block 금지 invariant 정합, ADR-115 §결정 2 platform constraint). 신규 ADR 2건 (ADR-114 minimal path protocol / ADR-115 runtime hook enforcement) + 다수 ADR Amendment (declarative-only Wave 1) + lint fix 3종 (CFP-1735/1734/1688) 동반. src/tests 추가 = `hooks/` 하위 신규 file (extensionless polyglot wrapper + python module + bats fixture), 기존 file behavior 변경 0건 (backward-compat invariant).
- 6.8.1 (2026-05-25) 이후 main LAND ≠ marketplace publish 영역 닫힘 (CFP-FU #1758 carrier): 7 CHANGELOG entry backfill (CFP-1735 / 1740 / 1741 / 1742 / 1750 / 1753 / 1755 — 최초 5건 + rebase 동안 parallel land CFP-1742 / 1755 추가) + plugin.json 6.8.1 → 6.8.2 atomic bump + marketplace.json sibling sync per ADR-063 §결정 2 ordering (marketplace 선행 merge → wrapper merge). consumer `claude plugin update codeforge@mclayer` 영역 cache 갱신 enable.

### Cross-ref

- Issue #1758 (CFP-FU from CFP-1738/1741 release/publish 영역) — ADR-063 atomic invariant carrier
- mclayer/marketplace sibling sync PR — codeforge entry `version: 6.8.1` → `6.8.2`
- post-merge verification: consumer cache upgrade 후 `hooks.json` (a) UserPromptSubmit 3 entry (`plain-language-reminder` + `korean-english-recovery` + `userprompt-submit`) + (b) Stop 1 entry (`plain-language-check`) + (c) PreToolUse 3 matcher (`Bash` cross-repo-gh-safety + `ScheduleWakeup` schedule-wakeup-reminder + `Agent` pretooluse-agent-spawn-gate) 모두 presence 1:1 확인 + dispatcher file (`hooks/userprompt-submit` / `hooks/korean-english-recovery{,.py}` / `hooks/plain-language-{reminder,check,check.py}` / `hooks/schedule-wakeup-reminder` / `hooks/pretooluse-agent-spawn-gate` + `scripts/lib/check_spawn_prompt_format.py`) file presence.
...
- [CFP-FU-A] **Parallel session race 11th occurrence 3-Amendment paired carrier + 12th meta-occurrence collision recovery** (escalate_user pattern_count 11 reach Mandatory ADR-045 §D-9 — sub-decisions 1+2+3 통합 + recursive dogfooding evidence for #1476)
  - `archive/adr/ADR-073-orchestrator-verify-before-assert.md` — Amendment 13 (transition trigger enum 12+13번째 entries `pre_git_operation` + `pre_push`, polling cadence 1 → 3) + Amendment 14 (§결정 1-P primitive AND aggregate composition layer, OR semantics → 3-mode AND aggregate `scripts/lib/check_parallel_work_sentinel.py:437` Wave 2 mechanical wire 별 sub-CFP carrier)
  - `archive/adr/ADR-082-write-time-self-write-verification-mandate.md` — Amendment 19 sub-scope (1-I) (§결정 1 layer 1 sub-scope (1-I) 신설 pre-spawn-prompt-finalize verify layer, worktree create 후 ~30-60s window 안 1회 추가 polling 의무 + 3-source AND aggregate verify + verified-via `pre_spawn_prompt_finalize_verified` annotation, **renumbered from Amd 18 sub-scope 1-H post CFP-1342 mid-flight collision recovery** — CFP-1342 ADR-082 Amd 18 + 1-H collision detected post-PR-#1527-open during pre-merge window, recovery via rebase on origin/main ca1c20e + renumber + 4-file cross-ref update. 4-layer temporal defense forcing function 완결 = Amd 15 pre-spawn pin + Amd 16 mid-spawn drift + Amd 18 Orchestrator §10 source-claim + Amd 19 pre-spawn-prompt-finalize)
  - `archive/adr/ADR-RESERVATION.md` — `amendments_reserved[]` 3-row pre-claim append (ADR-073 Amd 13 + Amd 14 + ADR-082 Amd 19, CFP-FU-A carrier active status, ADR-082 Amd 17 §결정 1-G strict pre-claim mandate 정합; CFP-1342 Amd 18 row preserved upstream of our Amd 19 row post-collision-recovery)
...
- [CFP-1312] **ADR-082 Amendment 7 — §결정 9 verify-before-cite scope 양방향 확장 + CFP-1216 lint Check (b) backward-staleness wire** (dual-carrier, ADR-045 §D-9 pattern_count 3 reach Mandatory escalation 산물)
  - `archive/adr/ADR-082-write-time-self-write-verification-mandate.md` Amendment 7 신설: frontmatter amendments[] / amendment_log[] entry 7 append + §결정 9 wording 양방향 확장 (forward only → forward + backward, `M = max+1` 정확 next-slot 외 모두 stale) + Amendment 7 본문 section append + related_stories CFP-1312/CFP-1293/CFP-1216 append
  ...
  - `archive/adr/ADR-RESERVATION.md` amendments_reserved[] row append (ADR-082 Amendment 7 reserved → active 직접 전환, ArchitectPL chief author precedent — sibling pattern ADR-083 Amendment 3 CFP-1293)
...
- [CFP-1243] S4 producer (`scripts/check_rollback_signal.py`) enum literal conformance — `operational-signal-v1` closed `signal_type` enum drift 해소 (Option B: contract = SSOT, producer 가 conform).
  ...
  - `archive/adr/ADR-106-operational-signal-pmo-input-circuit.md`: Amendment 3 추가 (S4 producer emit literal `burn_rate` → `latency_burn_rate` conformance 기록). §결정 3 closed enum 4-value 자체는 무변경 (이미 정규). is_transitional false 유지 (corrective conformance — additive trail, strengthening, sunset_justification null).
...
- **`archive/adr/ADR-073-orchestrator-verify-before-assert.md`** — Amendment 5 append (CFP-1102). §결정 1 transition trigger enum 4 → 5 entry `ratchet` (`fix_iter_start` 5번째). §결정 1-E main HEAD `pin` verify primitive 3-step (fetch + remote HEAD `pin` gh api + local cache cross-check). §결정 1-F Amendment 2 §결정 1-A 3-step 재실행 정합. CFP-1087 cascade race evidence + pattern_count 2 reach HIGH escalation.
...
- **`archive/adr/ADR-068-boundary-completeness-invariants.md`** — `related_adrs[]` ADR-073 append + `## 관련 ADR` 표 ADR-073 row 신설 (cross-ref only, ADR-073 본문 0건 변경).
...
- **`archive/adr/ADR-068-boundary-completeness-invariants.md`** — Amendment 2 body section 끝 §"Implementation note (CFP-1086 Story-4 — chief author body cross-ref)" subsection 추가. 4-layer 분리 명시 (declaration layer = 본 ADR / implementation layer = chief author prompt body / architecture doc layer = lane internal SSOT / skill layer = RACI matrix host). Carrier 분리 표 (Story-1 declaration / Story-3 RACI matrix / Story-4 implementation body). Body 정합성 invariant (I-4 wording SSOT 자기 적용). Mechanical enforcement 영역 unchanged (verdict field-only enforcement 유지 — 신규 lint script / workflow yml / registry entry 0건). frontmatter `amendments[]` Amendment 2 row `ref` field 갱신 (implementation note 동반 명시). 변경이력 row 추가 (`2026-05-20 Implementation note (CFP-1086 / Story-4 — ADR-068 본문 정책 0건 변경)`).
...
- **`archive/adr/ADR-RESERVATION.md`** — Row 86 `reserved` → `active` 전환 (ADR-086 신설 점유 확정).
...
- **`archive/adr/ADR-085-multi-session-collaboration-protocol.md`** NEW — 본질 선언 + 8 §결정 + 5-layer disjoint 표 (ADR-082 §결정 1 4-layer 표 verbatim 답습 + 5번째 row Multi-session coordination 신설) + `mechanical_enforcement_actions: [active-sessions-presence, lane-entry-ownership-verify]` declaration-only-Wave-1 (ADR-082 §결정 6 + ADR-070 §D5 retain pattern 답습)
- `archive/adr/ADR-RESERVATION.md` row 85 = CFP-1041 active
- `archive/adr/ADR-073-orchestrator-verify-before-assert.md` **Amendment 4** cross-ref (post-rebase amendments[] sequence [1, 2, 3 (CFP-689 worktree-first self-ownership), 4 (본 CFP-1041 ADR-085 coordination)] consecutive)
- `archive/adr/ADR-082-write-time-self-write-verification-mandate.md` **Amendment 3** cross-ref (dual amendments[]+amendment_log[] block 정합)
...
- **`archive/adr/ADR-026-post-merge-automation.md`** — frontmatter mechanical_enforcement_actions[] 2 entry `status: deferred-followup` → `active` + progress_note 갱신.
...
- **`templates/github-workflows/claude-md-amendment-ref-drift.yml`** — PR-time warning tier lint workflow (paths: CLAUDE.md + archive/adr/**). `hotfix-bypass:claude-md-amendment-ref` label bypass + audit comment 자동 발의 (ADR-024 Amendment 3 §결정 6.A).
...
- **`templates/github-workflows/evidence-registry-anomaly-check.yml`** — warning mode workflow (continue-on-error: true). PR trigger: scripts/check-*.sh + .github/workflows/*.yml + templates/github-workflows/*.yml + docs/evidence-checks-registry.yaml + archive/adr/ADR-060-*.md.
...
- **`archive/adr/ADR-036-project-key-atomic-reservation.md` Amendment 1**: Title regex precedence 명시.
...
- **`archive/adr/ADR-013-codeforge-family-dogfood-out-policy.md` Amendment 5 (CFP-596 verbatim port via CFP-671 actual integration)**: Story-init workflow cross-repo write 의무 codification.
...
- **`archive/adr/ADR-013-codeforge-family-dogfood-out-policy.md` Amendment 6 (CFP-671 신설)**: CFP-596 phantom changelog incident retrospective.
...
- ADR-063 Amendment 2 본문 — `archive/adr/ADR-063-marketplace-atomic-invariant.md` frontmatter `amendments[1]` row append + §결정 11 (description proactive lint mandate) + §결정 12 (self-application `ratchet` + 본 carrier 첫 사례 시연 의무).
...
- `archive/adr/ADR-064-decision-principle-mandate.md` (UPDATE) — `related_adrs` field 에 ADR-071 append (본문 변경 0, backward compat). ADR-064 §결정 7 top-down `ratchet` 정합 — 강화 방향 only.
- `archive/adr/ADR-RESERVATION.md` (UPDATE) — row 71 `reserved → active` 전환. ArchitectAgent inline append per CFP-578 / ADR-070 chief author precedent.
...
- `archive/adr/ADR-059-debate-protocol-v1.md` — Amendment 2 append (§결정 7: DesignLane blanket trigger + cross-module Story 정의 heuristic inline / §결정 8: convergence_quality_invariant 3 marker pattern / §결정 9: Touchpoint #2 carry-over 의무 / §결정 10: lane-agnostic registry 정합).
...
- `archive/adr/ADR-060-evidence-enforceable-promotion-framework.md` — Amendment 7 + §결정 20 신설
...
- `archive/adr/ADR-060-evidence-enforceable-promotion-framework.md` (UPDATE) — Amendment 6 + §결정 19 신설
...
- `archive/adr/ADR-066-pat-rotation-policy.md` (NEW) — 7 결정 (cadence / scope / 절차 / compromise / audit / 자동화 carrier / consumer overlay)
- `archive/adr/ADR-RESERVATION.md` (UPDATE) — ADR-066 row append
...
- `archive/adr/ADR-052-codex-proactive-check-touchpoints.md` (UPDATE) — Amendment 3 본문 append (A1~A8 결정 + 거절된 대안 H~K). amendments[] frontmatter row 추가.
...
- `archive/adr/ADR-038-progress-visualization-todowrite.md` (UPDATE — Phase 1 PR #493) — Amendment 3 §결정 10·11·12·13·14 신설:
...
- `scripts/check-decision-principle-vocabulary.sh` (NEW) — Python heredoc lint script. 8 forbid 어휘 (임시 / 단계적 / 일단 / 우선 / 잠정 / 가벼운 / minimal viable / quick win) detection in 5 scope 영역 (`archive/adr/**` / `docs/change-plans/**` / `CLAUDE.md` / `docs/orchestrator-playbook.md` / `templates/**`). Exempt = markdown blockquote + fenced code + EXEMPT_PATHS (ADR-064 self / ADR-RESERVATION / registry yaml / script self / bats fixture self). Exit code 3-tier (0=PASS / 1=violation / 2=meta-error — ADR-060 Amendment 2 §결정 15).
...
- [ADR-064](archive/adr/ADR-064-decision-principle-mandate.md) §결정 2 — forbid-list dictionary SSOT
- [ADR-060](archive/adr/ADR-060-evidence-enforceable-promotion-framework.md) §결정 5 — warning mode
- [ADR-024](archive/adr/ADR-024-story-scoped-branch-policy.md) Amendment 3 — `hotfix-bypass:*` per-entry namespace
- [ADR-061](archive/adr/ADR-061-python-script-writing-convention.md) §결정 1 — Python heredoc convention
- [ADR-063](archive/adr/ADR-063-marketplace-atomic-invariant.md) §결정 2 — marketplace sync ordering
...
- `archive/adr/ADR-064-decision-principle-mandate.md` (NEW) — 8 결정 본문
...
- `archive/adr/ADR-RESERVATION.md` — `| 64 | CFP-445 | active | 2026-05-12 |` row append
...
- [ADR-064](archive/adr/ADR-064-decision-principle-mandate.md) — `normative` 결정 SSOT
- [ADR-058](archive/adr/ADR-058-adr-sunset-criteria-mandate.md) — sunset criteria mandate (`ratchet` 차단 forcing function)
- [ADR-060](archive/adr/ADR-060-evidence-enforceable-promotion-framework.md) — evidence-enforceable framework
- [ADR-063](archive/adr/ADR-063-marketplace-atomic-invariant.md) — 3-file atomic invariant
...
- `archive/adr/ADR-060-evidence-enforceable-promotion-framework.md` — Amendment 2 append (frontmatter `amendment_log[]` row 2 + 본문 `## Amendment 2` § 신설 8 결정 — §결정 3 required 전환 / §결정 6 (c) `sibling_dependencies` append CFP-455 / §결정 14 메타 anomaly vs schema validation lint 분리 / §결정 15 exit-code 3-tier semantics / §결정 16 warning-tier bypass_label optional / §결정 17 retroactive reclassification immediate fail / §결정 18 marketplace sync 의무 명시 / Mermaid diagram 동기화)
...
- `archive/adr/ADR-063-marketplace-atomic-invariant.md` (NEW, 200L) — 8 결정 정책 본문
...
- `archive/adr/ADR-RESERVATION.md` — `| 63 | CFP-436 | active | 2026-05-12 |` row append
...
- `archive/adr/ADR-061-python-script-writing-convention.md` (NEW, 260L) — 8 결정 정책 본문
...
- `archive/adr/ADR-RESERVATION.md` — `| 61 | CFP-423 | active | 2026-05-12 |` row append
...
- `archive/adr/ADR-057-orchestrator-opus-mandate-and-sonnet-opus-fallback.md` — Amendment 2: sunset criteria 본문 강화 (CFP-388 framework 첫 적용 사례)
...
- `archive/adr/ADR-052-codex-proactive-check-touchpoints.md` — Amendment 1: touchpoint #4 (RequirementsPLAgent §1~§6 완료 직후 Codex proactive check) single-shot 검토 → multi-round adversarial debate 격상
...
- CFP-389 — Evidence-enforceable promotion framework SSOT (ADR-060 신규)
  - `archive/adr/ADR-060-evidence-enforceable-promotion-framework.md` — framework SSOT (12 §결정, is_transitional=false, ## 해소 기준 N/A — permanent policy)
  - `archive/adr/ADR-024-story-scoped-branch-policy.md` — Amendment 3: `hotfix-bypass:*` label family = audit-trailed exception channel (사용자 ESCALATE Option A)
...
### Changed
- `archive/adr/ADR-058-adr-sunset-criteria-mandate.md` — lint regex 정합 ("N/A — permanent policy" 문구)
- `archive/adr/ADR-RESERVATION.md` — row 60 추가 (CFP-389)
...
- CFP-391 / ADR-059: debate-protocol-v1 registry + DesignReview lane 적용 (Phase 1)
  ...
  - `archive/adr/ADR-059-debate-protocol-v1.md` (NEW, Accepted) — 5 결정 carrier
  - `archive/adr/ADR-044-phase-scoped-sequential-team.md` Amendment 1 — `dispatch_mode` enum 에 `auto_on_divergence` 추가 + 우선순위 룰 (`default > auto_on_divergence > user_request_only`)
...
- CFP-387 / ADR-058: ADR template 해소 기준 섹션 의무화 + `is_transitional` 분류 frontmatter
  - `archive/adr/ADR-058-adr-sunset-criteria-mandate.md` (NEW, Accepted) — declaration only policy carrier
...
### Changed
- `CLAUDE.md` "ADR (`archive/adr/` SSOT)" 섹션 — ADR-058 안전망 분류 + 해소 기준 의무 cross-ref 추가 (1 단락)
...
- `archive/adr/ADR-014-operational-risk-ssot-distribution.md` Amendment 2 추가
...
- `archive/adr/ADR-036-project-key-atomic-reservation.md` (NEW, Accepted) — KEY = `<PREFIX>-<Issue#>` (GitHub atomic Issue numbering 위임). 6 결정: KEY 형식 / cfp-reserve.yml Form / phase:reservation label / reservation-cleanup.yml workflow / story-init.yml concurrency 안전망 / Migration (기존 KEY rename 금지).
...
- `archive/adr/ADR-037-plugin-version-bump-rule.md` (NEW, Accepted) — Option β core (12 surface category) + Wrapper-coupling trigger 3종 (T1 contract MAJOR / T2 agent topology / T3 family invariant ADR supersede) + Option α (Conventional Commits CI enforcement).
...
- `archive/adr/ADR-033-docker-first-infra-engineering.md` (NEW) — CFP-128 carrier. 7 결정: (1) InfraEngineerAgent default 출력 = Dockerfile + compose.yml + .dockerignore (1st-class). (2) K8s manifests = `presets/k8s/` (codeforge-develop) opt-in. (3) systemd / launchd / PaaS = legacy (consumer overlay opt-in only). (4) SecurityTest 1st-layer = trivy + hadolint 추가. (5) CONDITIONAL deputy 매트릭스 — Docker 관련 cell annotation update. (6) Migration = ADR Accepted 후 신규 Story 만 의무, 기존 in-flight grandfathered (ADR-031 §14 freeze pattern). (7) Consumer 측 follow-on Epic = mctrader 등 컨테이너화 코드 작업 별도 Epic 의무 (consumer 워크스페이스 수행).
- `archive/adr/ADR-014-operational-risk-ssot-distribution.md` (Modify) — frontmatter `amendments: [ADR-033]` + 본문 "Amended by" section (§7.4 OpRiskArch mandate 4 항목 확장 — container restart policy / volume DR / health check tuning / network mode boundary).
...
- `archive/adr/ADR-033` (Modify) — Status `Proposed` → `Accepted`. effective date = Phase 2 PR merge timestamp.
...
- `archive/adr/ADR-031-lane-spawn-evidence-trail.md` (Modify) — frontmatter `status: Proposed → Accepted`, `related_files` 갱신 (`phase-gate-mergeable.yml` → `lane-evidence-check.yml`). §상태 갱신 (CFP-126 Phase 1 PR #59 + Phase 2 PR #232 merged). §결정 1 storage location 4 candidate → (a) §14 freeze (12 field YAML schema explicit). 다른 3 candidate 명시적 superseded.
...
- `archive/adr/ADR-026-post-merge-automation.md` (NEW) — 4 결정 (Wrapper Orchestrator post-merge automation 의무 / Cross-repo PAT / Telemetry only / Disable-by-flag + main 직접 push 금지). Sonnet decider (CFP-74-001) pick=alpha, Codex round 2 audit (gpt-5.5 high, ADR conflict 0/7).
...
- `archive/adr/ADR-030-live-entry-gate-policy.md` (NEW) — 5 결정 (gate:live-entry-pass label 정의 / Live touching Story 식별도 mechanism / phase-gate-mergeable.yml validation / 3-condition AND consumer-side SSOT / fast-pass 영향 차단). mctrader debut audit P0 (Codex gpt-5.5 high 2026-05-04) 해소.
...
- `archive/adr/ADR-029-phase-execution-visibility-expansion.md` (NEW) — 5 결정 (sub-step event narration 의무 / format 표준 + sanitize policy / stop discipline cross-reference / verbosity opt-out / Lane plugin 변경 불요). 사용자 directive (2026-05-05) "phase 와 내부 진행단계를 완료 시마다 출력해주어야 한다" 해소.
...
- `archive/adr/ADR-020-cross-repo-epic-pattern.md` (Modify) — Amendment 2 신설:
...
- `archive/adr/ADR-028-superpowers-integration-policy.md` (NEW) — 6 결정 (SSOT 위치 / contract / path override / 변환 표 / helpers 소유권 / Phase 2-7 batch open). Sonnet decider Option B + Codex 3 mod + Sonnet 2 mitigation 통합.
- `archive/adr/ADR-017-skill-override-path-enforcement.md` (Modify) — Amendment 1: agent md `Edit/Write(docs/superpowers/**)` 권한 표기 lint 추가. effective date = Phase 1 PR merge 직후.
...
- `archive/adr/ADR-027-consumer-adoption-protocol.md` (NEW) — 5 결정 freeze: bootstrap 검증 책임 = wrapper overlay/hooks/, 3-trigger enforcement (Story phase / UserPromptSubmit / SessionStart), bypass = HOTFIX_BYPASS_CODEFORGE env, cross-platform (POSIX + Windows), consumer-guide.md = 절차 SSOT.
...
- `archive/adr/ADR-015-stateful-test-category.md` — carrier ADR (5 결정 + 거부된 대안)
...
- `archive/adr/ADR-014-operational-risk-ssot-distribution.md` — carrier ADR (OperationalRiskArchitectAgent SSOT 분배 + §7.4 5 sub-item + §11.6 CONDITIONAL + design-output v1→v2 BREAKING 결정 5종)
...
- `archive/adr/ADR-012-wrapper-claudemd-ssot-boundary.md` §3: "3 named SSOT exceptions" → "4 named SSOT exceptions" (carrier ADR pattern)
...
- `archive/adr/ADR-011-inter-plugin-contract-drift-detection.md` — drift detection 정책 동결 (live fetch + 정규화 5단계 + Archived skip + PR/push trigger only)
...
- `archive/adr/ADR-008-inter-plugin-contract-versioning.md` §5.1 신규 단락 — Deprecated → Archived 전환 트리거 3 조건 정의 (consumer 부재 + 후속 MAJOR 1+ release + canonical/`sibling sync` 또는 wrapper 단독)
...
- `archive/adr/ADR-009-wrapper-only-decomposition.md` — 신규 ADR (status: Adopted). ζ arc 결정 영속 기록
...
- `archive/adr/ADR-008-inter-plugin-contract-versioning.md` — SemVer-style versioning 룰 (v1.x compat / v2.0 BREAKING)
...
### Changed
- `agents/ArchitectAgent.md` frontmatter — `docs/change-plans/**` + `archive/adr/**` Edit/Write 추가, `docs/**` 블랭킷 deny 제거
...
- `archive/adr/ADR-002-docsagent-inherit-footer-pattern.md` — §3.2 path example 오타 정정
...
### Breaking
- **3 lane × 2 vendor = 6 워커 → 2 워커로 통합** ([ADR-001](archive/adr/ADR-001-review-agent-unification.md)). consumer overlay에 `agents/Claude{Design,Code,SecurityTest}ReviewAgent.md` 또는 `Codex...` 파일이 있다면 마이그레이션 필요
...
### Added
- `archive/adr/ADR-001-review-agent-unification.md` (첫 ADR)
...
- **ADR → `archive/adr/ADR-NNN-<slug>.md`** (flat, frontmatter `category:`)
...
- **CODEOWNERS**: `archive/adr/**`·`docs/change-plans/**`·`docs/stories/**` → architect team / `docs/domain-knowledge/**` → domain expert team

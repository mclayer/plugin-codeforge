...
**의미축 분리 원칙 (ADR-072 §결정 8 ↔ 본 §결정 6.2)**: 본 `owner_axis_kind` 는
**axis 분류 영역** (policy / evidence / consult — boundary axis 의 mandate scope
구분). ADR-072 §결정 8 의 `owner_deputy_kind: "production_evidence"` 는 **SubAgent
...
|---|---|---|---|
| `owner_deputy_kind` (ADR-072 §결정 8) | SubAgent identity (누가 owner) | `production_evidence` (8 → 9 SubAgent 확장) | CFP-Z review-verdict-v4 v4.6 MINOR |
...
### 결정 6.3: Amendment 2 §결정 3 ↔ ADR-072 §결정 2 5번째 cell 3-way 충돌 처리
...
- ADR-072 §결정 4 (boundary axis 1줄 명시 — 본 Amendment 양 방향 cross-ref)
- ADR-072 §결정 8 (`owner_deputy_kind` SubAgent identity field — 본 §결정 6.2 `owner_axis_kind` axis 분류 field 와 disjoint semantic axis. 양 field 동시 carrier CFP-Z review-verdict-v4 v4.5 → v4.6 MINOR bump 단일 PR atomic add 의무)
...
|---|---|---|---|
| §7.4.1 DR | **primary** | backup / restore / failover policy 설계 시점 결정 | ProductionEvidence consult (실측 evidence — ADR-072 §결정 2 정합) |
...
InfraOperationalArchitect (rename 후) 와 ProductionEvidenceDeputyAgent ([ADR-072](ADR-072-production-evidence-deputy-and-epic-cutover-gate.md) §결정 1 3번째 CONDITIONAL SubAgent) 의 책임 경계 axis 계승 (Amendment 3 §결정 6.1 boundary axis 무변경 — rename 만 반영):
...
- ProductionEvidenceDeputy = **production cutover evidence gate** owner (production state 실측 — CONDITIONAL, Live touching OR production cutover Story 한정, ADR-072 §결정 3)

**dual-spawn 가능성 + wrapper-self-app N/A 명문화** (Epic #1026 ProductionEvidence boundary 단락 SSOT):
- **consumer production cutover Story** 에서 InfraOperationalArchitect + ProductionEvidence **dual-spawn 가능** — 영역 disjoint (design-time policy vs runtime-evidence). ADR-072 §결정 3 "SubAgent spawn both 의무" (production cutover = 9 SubAgent: 6 permanent + LiveOps + LiveOrdering + ProductionEvidence) 정합. 한쪽만 spawn 금지 (boundary axis 정합 검증 의무).
- **wrapper-self-app 시 ProductionEvidence N/A** (ADR-072 §결정 6 + [ADR-005](ADR-005-plugin-self-application-na-standardization.md) `plugin-meta-na` 정합 — codeforge dogfood Epic = production cutover 부재. CFP-954 precedent). 본 CFP-676 자체 = wrapper governance Story (code 0 + runtime behavior 0) → InfraOperationalArchitect (6 permanent 상시) 만, ProductionEvidence N/A.
...
- ADR-072 §결정 1/§결정 3/§결정 6 (ProductionEvidenceDeputy ↔ InfraOperationalArchitect disjoint axis + wrapper-self-app N/A)
- ADR-072 mirror cross-ref 단락 (CFP-676 — 양 방향 cross-ref)
...
| Cross-ref | ADR-072 ProductionEvidence (별도 deputy, disjoint axis — Amendment 4 §결정 1 SSOT) | ADR-087 §결정 5 6-step + ADR-088 §결정 3 ProductionEvidence ownership 이관 |
...
ADR-072 §결정 1 (ProductionEvidence Deputy 신설) 의 ownership 위치 = wrapper-side deputy (Amendment 4 §결정 1) → ADR-088 §결정 3 (CFP-1059 Story-1 carrier) 으로 정식 이관. 본 ADR-014 Amendment 5 = ownership transfer declarative cross-ref only — ADR-014 §결정 1~5 본문 변경 0건, ADR-072 §결정 1 본문 변경 = ADR-072 자체 Amendment 4 carrier scope.

...
CFP-20 (`docs/orchestrator-playbook.md` §14) 가 §0 Live Progress mechanism 도입 — Orchestrator 가 `.claude-work/progress/<KEY>.md` file 에 7-lane × phase 진행을 hierarchical 형식으로 기록. Trigger SSOT 표 (§14.5) 에서 **lane-level event 만 `terminal narration ✅`** (사용자 가시), sub-step event (SubAgent spawn / return / 병렬 dispatch / R9 subset) 는 `❌ (file only)`.
...
- Credential, secret, token, 개인식별정보(PII), 원문 SubAgent 산출물의 민감 payload 는 narration 에 포함 금지.
...
- Per-deputy narration content 표준 (SubAgent 별도 작성자 책임 — 본 ADR §결정 2 sanitize policy 만 강제)

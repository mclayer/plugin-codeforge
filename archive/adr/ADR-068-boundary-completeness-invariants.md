...
related_stories:
  ...
  - CFP-2351  # Amendment 6 carrier — 신규 I-8 standing invariant-surface invariant (Epic #2346 진단축 D, S4 #2350 의 enabler). long-lived mutable structure (WAL/버퍼/큐/캐시/카운터/ledger) 의 bound/lifetime/ordering invariant 가 상시 문서 docs/system-invariants.md 에 색인돼야 함 + impl PR 이 새 구조 추가 시 surface 확장 의무. ADR-119 §결정 10 ② generative invariant sweep 의 standing 기록처 instantiation, S4 falsifier cross-ref. 7 → 8 invariants ratchet 강화. review-verdict-v4 self-check field 추가 = S4(#2350) single-owner 위임 (본 Amendment 6 review-verdict-v4 무수정).
  - CFP-2478  # Amendment 7 carrier — I-7 chief-author cross-ADR claim consistency 의 CI-time 기계화 carrier 실현 (Amendment 5 가 예약한 Wave 2 슬롯 충족) + cross-document 정합 sweep CI-time 일반화 (Epic CFP-2476 E2 = Codex 주장→증거 감사기). 신규 invariant 0 — I-1~I-8 본문 의미 변경 0. 2-layer (A 결정론 grep PR-time warning / B Codex LLM advisory 변경분 diff). E1 execution-dispatch-pattern-v1 재사용. disjoint = 기존 lint 2종 (ADR-074 / check_adr_citation_slug.py) 보존 (U-1 (a) 위임). content-anchor citation (U-2). CFP-2457 4 drift 케이스 distinct fixture. Phase 1 design lane = ADR codify, Phase 2 = mechanical wire.
absorbed_issues:
  - 438
related_adrs:
  - ADR-008
  ...
  - ADR-074  # 신설 cross-ref (CFP-2478 Amendment 7 — disjoint 경계 sibling: ADR-074 = CLAUDE.md narrative ↔ ADR amendment_log length 한 쌍, §결정 8 author-time self-check ↔ CI-time lint 분리. 본 Amendment 7 cross-document sweep 은 그 명시 공백 (ADR↔ADR cross-ref / phantom ID ownership / enum 리터럴 / content-anchor / 버전 표기) 만 cover — ADR-074 흡수·sunset 0. ADR-074 본문 0건 변경)
  - ADR-119  # 신설 cross-ref (CFP-2478 Amendment 7 — 본 cross-document sweep = ADR-119 검증-후-단언 §결정 8 declarative-only 의 mechanical 자동화 carrier 후보. ADR-119 amendment 불요 — wave_2_wire_carrier 가 '후속 lint CFP' 예약 (verified ADR-119:49), 본 CFP 가 예약 carrier 실현. ADR-119 본문 0건 변경)
  - ADR-070  # 신설 cross-ref (CFP-2478 Amendment 7 — layer B Codex LLM claim-audit finding = `[hypothesis]` default → ground-truth falsify 후 채택 (D1/D3 verify-before-trust). Amendment 11 §B3 execution-dispatch-pattern-v1 (E1) 재사용. ADR-070 본문 0건 변경)
  - ADR-081  # 신설 cross-ref (CFP-2478 Amendment 7 — layer B dispatch = ADR-081 Amendment 11 §A3 execution dispatch (adversarial-review/task file-redirect + Codex sandbox) 재사용. ADR-081 본문 0건 변경)
  - ADR-119  # 신설 cross-ref (CFP-2351 Amendment 6 — I-8 = ADR-119 §결정 10 ② generative invariant sweep 의 standing 기록처 instantiation. ADR-119 §결정 10 ② "실패 경로의 long-lived mutable 구조 열거 + bound/lifetime/ordering invariant 명시 + 코드 보존 여부 실측" 의 상시 surface 화. 본 ADR-068 = I-8 invariant 신설, ADR-119 본문 0건 변경)
  - ADR-015  # 신설 cross-ref (CFP-2351 Amendment 6 — I-8 의 accumulation/lifetime-class flag = ADR-015 Amendment 1 §결정 1 accumulation/lifetime-class soak (capacity 회계 monotone·미회수 / 무한 누적 / lifetime cap) 정합. 본 ADR-068 = I-8 invariant 신설, ADR-015 본문 0건 변경)
  - ADR-014  # 신설 cross-ref (CFP-2351 Amendment 6 — I-8 의 accumulation/monotone/미회수 명시 flag = ADR-014 Amendment 7 §7.4.7 outcome-signal ③ 발현조건 임계 (accumulation/lifetime-class 리스크 발현 누적량) 와 cross-ref. 본 ADR-068 = I-8 invariant 신설, ADR-014 본문 0건 변경)
amendments:
  - amendment_id: 1
    cfp: CFP-528
    date: 2026-05-13
  ...
  - amendment_id: 6
    cfp: CFP-2351
    date: 2026-06-19
    scope: "신규 I-8 standing invariant-surface invariant 추가 — 시스템의 long-lived mutable structure (프로세스 수명 동안 상태를 누적·유지하는 in-process WAL/버퍼/큐/캐시/카운터/ledger 등) 의 bound/lifetime/ordering invariant 가 상시 문서 (`docs/system-invariants.md`) 에 색인돼야 한다. impl PR 이 새 long-lived mutable structure 를 추가하면 그 PR 이 invariant-surface 를 확장할 의무. runtime-failure 진단 (S4) 의 falsifier 는 이 surface 를 cross-ref 해 invariant sweep 의 enumeration 완전성을 보강한다. I-8 의무 항목 (declarative): 각 구조당 {구조명·위치 / 종류 (buffer/queue/cache/counter/ledger/WAL 등) / invariant (bound: 상한·backlog vs lifetime 구분 / lifetime: 회수·reclaim 여부 / ordering: 순서·watermark) / 코드상 보존 (또는 위반) 지점}. 누적·monotone·미회수 구조는 명시 flag (ADR-015 Amendment 1 accumulation/lifetime-class soak + ADR-014 Amendment 7 §7.4.7 outcome-signal ③ 발현조건 임계 cross-ref). ADR-119 §결정 10 ② instantiation — 진단면 generative invariant sweep 의 standing 기록처. S4 (#2350) 의 falsifier 가 invariant-surface 입력으로 본 surface 사용. review-verdict-v4 self-check field 추가 = S4 (#2350, review-verdict-v4 single-owner) 위임 또는 Phase 2 defer — 본 Amendment 6 review-verdict-v4 무수정 (병렬 force-push 충돌 회피). enforcement = Phase 1 declarative-only (I-8 declaration + template; impl-PR-extends-invariant-surface mechanical lint + review-verdict-v4 self-check field = 별 carrier Phase 2 defer, ADR-119 §결정 8 / ADR-060). wrapper-self declarative (runtime 0, 면제). boundary completeness 7 invariants (I-1~I-7) → 8 invariants ratchet 강화. Epic #2346 진단축 D incident evidence (ADR-119 §결정 10 line 190 incident 근거: 전 lane PASS 후 제품 사망, 표면 증상 진단으로 약 10h 3회 헛수정 — 'WAL accumulator 회수 안 됨' 상시 구조적 속성 기록처 부재로 장애 중 재도출)."
    status: applied
    ref: "본문 §결정 1 I-8 declaration + §결정 2 Tier A/B/C I-8 row + §결정 6 표 5번째 row + Amendment 6 section + 변경이력 entry + 신규 templates/system-invariants.md"
    sunset_justification: "N/A — ratchet 강화 방향 (7 → 8 invariants, scope 축소 0건). I-1/I-2/I-3/I-4/I-5/I-6/I-7 본문 의미 변경 0건 — I-8 disjoint append only (standing invariant-surface axis 분리, write-time verification semantic 의 standing-doc 색인 면). is_transitional false 유지 (permanent governance). ADR-058 §결정 5 / ADR-064 §결정 7 top-down ratchet 정합 (강화 방향만 amendment). Amendment 1 (CFP-528 I-5) + Amendment 3 (CFP-1087 I-6) + Amendment 5 (CFP-1565 I-7) invariant 신설 precedent verbatim 답습. review-verdict-v4 self-check field binding = S4 위임 (본 Amendment 무수정) — mechanical self-check 명시적 defer. retroactive 면제 (Amendment 6 발효 후 신규 long-lived mutable structure 추가 PR 한정)."
  - amendment_id: 7
    cfp: CFP-2478
    date: 2026-06-30
    scope: "I-7 (chief-author cross-ADR scope/fact claim consistency) 의 **CI-time 기계화 carrier 실현** + cross-document 정합 sweep CI-time 일반화 (Epic CFP-2476 E2). 신규 invariant 0 — I-1~I-8 본문 의미 변경 0건, I-7 의 author-time 사람 self-check (verdict field-only, Amendment 5) 를 **CI-time 전수 결정론 lint** 로 보완(author self-check 와 disjoint layer, ADR-074 §결정 8 author-time self-check ↔ CI-time lint 분리 답습). 본 Amendment = `chief-author-crossref-consistency-check` mechanical_enforcement_action 의 Wave 2 mechanical wire carrier 지정 (Amendment 5 progress_note 의 '별 sub-CFP Wave 2 carrier' 슬롯 = CFP-2478 이 충족, evidence-checks-registry `chief-author-crossref-consistency` entry `sibling_dependencies.TBD-Wave-2-sub-CFP` → CFP-2478 치환). 2 layer codify (mechanism 분리 = Story §4.2 판정, 통합 금지): **(A) 결정론 cross-document sweep** (CI workflow PR-time, warning tier) — cross-ADR cross-ref 번호 존재 + phantom ID ownership (ADR-NNN ↔ 그 ADR 이 실 소유한 invariant/§결정 ID) + enum 리터럴 SSOT 대조 (정본 SSOT 명확한 enum 한정) + 버전 표기 parity + citation **content-anchor** 대조 (line-number 직접 정합 제외 — U-2). 재현 가능 → 자동 tier 정당 (CE-5). **(B) Codex LLM claim-audit** (Orchestrator inline dispatch, ADR-052 touchpoint 계열 advisory, 변경분 diff 한정) — substantive 단정 식별 + semantic misquote (번호 맞고 §결정 의미 틀림, citation_slug L1/L2 잔여) 대조. 비결정 → finding = `[hypothesis]` default, Orchestrator own dir Read falsify 후 채택 (자동 차단권 없음, ADR-070 D1/D3 + ADR-077 I-4). 실행 dispatch = E1 execution-dispatch-pattern-v1 (ADR-070 Amendment 11 §B3 + ADR-081 Amendment 11 §A3 + concept execution-based-review-verification) 재사용 — Codex CLI 자체 sandbox (read-only 기본 / network-off / .git 보호), lane worker own-Bash 아님. disjoint 경계 (U-1 = (a) 위임·disjoint 채택): 기존 lint 2종 (ADR-074 narrative↔amendment_log / check_adr_citation_slug.py L1 번호존재+L2 ADR-057 misquote deny) 보존 — 본 sweep 은 명시 공백 (ADR↔ADR cross-ref / phantom ID ownership / enum 리터럴 / content-anchor / 버전 표기) 만 cover, 흡수·sunset 0. 오탐 처리 = verbatim-claim 한정 (over-extraction 차단 CE-1) + P2 자동차단 금지 (FP 20~30%+ = 폐기 임계 CE-6) + abstention 채널. enforcement = Phase 1 design lane = ADR codify + Change Plan (본 Amendment) — 실 mechanical wire (scripts/check-adr-cross-ref-consistency.sh + scripts/lib + workflow yml + bats RED→GREEN + evidence-registry status deferred-followup → warning 전환 + label-registry-v2 MINOR + plugin bump 6.50.0 → 6.51.0 + marketplace sync) = Phase 2 PR (ADR-119 §결정 8 / ADR-060 4-tier promotion 경로 정합). ADR-119 amendment 불요 (wave_2_wire_carrier 가 '후속 lint CFP' 예약 — verified ADR-119:49, 본 CFP 가 예약 carrier 실현, cross-ref backref 만). CFP-2457 4 drift 케이스 (ADR-039 entry 번호 mis-cite / ADR-052 enum 리터럴 stale / ADR-077 phantom 'I-4' = 실제 ADR-068 소유 / citation 줄번호 오기) = distinct regression fixture (Phase 2 bats). ratchet 강화 방향 (I-7 author-time → +CI-time mechanical layer, scope 약화 0)."
    status: applied
    ref: "본문 §결정 6 표 chief-author-crossref-consistency-check row progress_note 갱신 (Wave 2 carrier 지정) + Amendment 7 section + 변경이력 entry + mechanical_enforcement_actions chief-author-crossref-consistency-check progress_note 갱신"
    sunset_justification: "N/A — ratchet 강화 방향 (I-7 author-time self-check → +CI-time 결정론 mechanical layer 보완 + cross-document sweep CI-time 일반화, scope 축소 0건). I-1~I-8 invariant 본문 의미 변경 0건 — 본 Amendment 는 신규 invariant 가 아니라 I-7 의 mechanical wire carrier 실현 (Amendment 5 가 예약한 Wave 2 슬롯 충족) + cross-document sweep 2-layer codify. is_transitional false 유지 (permanent governance). ADR-058 §결정 5 / ADR-064 §결정 7 top-down ratchet 정합 (강화 방향만 amendment). ADR-074 §결정 8 (author-time self-check ↔ CI-time lint 분리) precedent 답습 — author self-check (Amendment 5 verdict field) 보존 + CI-time lint 추가 (disjoint layer). doc-only fast-path ADR-054 (Phase 1 design lane = ADR codify, Phase 2 = mechanical wire full-lane). retroactive 면제 (Amendment 7 Phase 2 wire 발효 후 신규 cross-document drift 한정)."
mechanical_enforcement_actions:
  ...
  - action: chief-author-crossref-consistency-check
    status: deferred-followup
    progress_note: "ADR-068 Amendment 5 (CFP-1565) 신설 시점 — Wave 1 declaration-only (ADR-082 §결정 6 + ADR-070 §D5 retain pattern 답습). verdict field-only enforcement (`chief_author_crossref_consistency_self_check_passed: bool` + `findings[].type: 'chief-author-crossref-inconsistency'`). evidence-checks-registry entry `chief-author-crossref-consistency` (status deferred-followup, bypass_label hotfix-bypass:boundary-wording 재사용 — ADR-068 owner family). **Amendment 7 (CFP-2478, Epic CFP-2476 E2) = 본 action 의 Wave 2 mechanical wire carrier 지정** — author-time verdict field self-check (Amendment 5) 는 보존하고, CI-time 결정론 cross-document lint (layer A) 를 disjoint 추가 (ADR-074 §결정 8 author-time↔CI-time 분리 답습). Phase 1 (design lane) = ADR codify (Amendment 7) + Change Plan; Phase 2 PR = 실 mechanical wire (scripts/check-adr-cross-ref-consistency.sh + scripts/lib (ADR-061 Python SSOT) + workflow yml (self-app byte-identical ADR-005) + bats fixture RED→GREEN (CFP-2457 4 drift distinct fixture) + evidence-registry status deferred-followup → warning 전환). semantic misquote (번호 맞고 §결정 의미 틀림) = layer B Codex LLM advisory (자동 차단권 없음, ADR-070 D1/D3). blocking-on-pr 승격 후보 — pattern_count >= 2 재발 시 follow-up CFP MUST promote (ADR-082 §결정 6 정합)."
    target_section: §결정 1
  - action: invariant-surface-extension-check
    status: deferred-followup
    progress_note: "ADR-068 Amendment 6 (CFP-2351) 신설 시점 — Wave 1 declaration-only (ADR-082 §결정 6 + ADR-070 §D5 retain pattern 답습). I-8 standing invariant-surface 의무 = docs/system-invariants.md 색인 + impl PR 이 새 long-lived mutable structure 추가 시 surface 확장. review-verdict-v4 self-check field 추가 = S4 (#2350, review-verdict-v4 single-owner) 위임 — 본 Amendment 6 review-verdict-v4 무수정 (verdict field-only enforcement binding 도 S4 defer). 실 mechanical wire (impl-PR-extends-invariant-surface lint scripts/check-invariant-surface.sh + scripts/lib + workflow yml + bats fixture) = 별 sub-CFP Wave 2 carrier (ADR-119 §결정 8 / ADR-060 4-tier promotion 경로). blocking-on-pr 승격 후보 — pattern_count >= 2 재발 시 follow-up CFP MUST promote (ADR-082 §결정 6 정합). wrapper-self declarative (runtime 0, 면제 — ADR-005 plugin-meta-na)."
    target_section: §결정 1
---
...
**I-8: Standing invariant-surface (Amendment 6 — CFP-2351, 2026-06-19)**
- 정의: 시스템의 **long-lived mutable structure** (프로세스 수명 동안 상태를 누적·유지하는 구조 — in-process WAL / 버퍼 / 큐 / 캐시 / 카운터 / ledger 등) 는 그 **bound / lifetime / ordering invariant** 가 상시 문서 (`docs/system-invariants.md`) 에 색인돼야 한다. impl PR 이 **새 long-lived mutable structure 를 추가** 하면 그 PR 이 invariant-surface 를 확장할 의무. I-1~I-7 이 chief author 의 **write-time 발화 1회** 를 검증하는 axis 라면, I-8 = 구조적 속성을 **상시 standing 문서** 에 색인하는 axis (1회 발화가 아닌 영속 surface). runtime-failure 진단 (S4) 의 falsifier 는 이 surface 를 cross-ref 해 invariant sweep 의 **enumeration 완전성** 을 보강한다 (ADR-119 §결정 10 ② generative invariant sweep — "실패 경로의 long-lived mutable 구조 열거 + bound / lifetime / ordering invariant 명시 + 코드 보존 여부 실측" [cited_adr: ADR-119 §결정 10 ②, cited_value: "실패 경로의 long-lived mutable 구조 열거 + bound / lifetime / ordering invariant 명시 + 코드 보존 여부 실측", verify_status: verified — worktree Read ADR-119 line 206] 의 standing 기록처 instantiation).
- 동기 (incident): "이 WAL accumulator 는 회수 안 됨" 은 상시 구조적 속성인데 기록처가 없어 장애 중 재도출됐다 (Epic #2346 진단축 D — 전 lane PASS 후 제품 사망, 표면 증상 진단으로 약 10h 3회 헛수정 [cited_adr: ADR-119 §결정 10, cited_value: "consumer 수집기가 전 lane PASS 했으나 제품 사망 ... 약 10h 3회 헛수정", verify_status: verified — worktree Read ADR-119 line 190 incident 근거]).
- I-8 의무 항목 (declarative) — 각 long-lived mutable structure 당:
  1. **구조명·위치** — 구조 식별자 + 코드상 위치 (file)
  2. **종류** — buffer / queue / cache / counter / ledger / WAL 등 enum
  3. **invariant** — (a) **bound**: 상한값 + **backlog cap vs lifetime cap 구분** (backlog = 처리 대기 큐 상한 / lifetime = 프로세스 수명 누적 상한 — 혼동 시 monotone 누적이 backlog 로 오인됨) (b) **lifetime**: 회수 (reclaim) 여부 — 회수됨 / 미회수 (monotone) (c) **ordering**: 순서 보장 / watermark 정의
  4. **코드상 보존 (또는 위반) 지점** — invariant 가 enforce 되는 (또는 위반되는) file:line
- **명시 flag**: 누적·monotone·미회수 구조는 `accumulation/lifetime-class? = Y` 명시 flag 보유 의무 (ADR-015 Amendment 1 §결정 1 accumulation/lifetime-class soak — capacity 회계 monotone·미회수 / 무한 누적 / lifetime cap [cited_adr: ADR-015 Amendment 1 §결정 1, cited_value: "accumulation / lifetime-class 리스크 (capacity 회계 monotone·미회수 / 무한 누적 / lifetime cap)", verify_status: verified — worktree Read ADR-015 line 166-168] + ADR-014 Amendment 7 §7.4.7 outcome-signal ③ 발현조건 임계 [cited_adr: ADR-014 Amendment 7 §결정 1, cited_value: "③ 발현조건 임계 = accumulation\\lifetime-class 리스크가 드러나는 누적량", verify_status: verified — worktree Read ADR-014 line 508] cross-ref).
- Trigger: impl PR 이 새 long-lived mutable structure 를 추가 (또는 기존 구조의 bound/lifetime/ordering invariant 변경) → `docs/system-invariants.md` 색인 row 부재/미갱신 detect → I-8 위반. (review-verdict-v4 self-check field binding = S4 (#2350) single-owner 위임 — 본 Amendment 6 review-verdict-v4 무수정, mechanical self-check field 명시적 defer.)
- Mitigation: 새 구조 추가 PR 이 `docs/system-invariants.md` 표에 4 항목 row append (또는 기존 row 의 invariant 갱신). template = `templates/system-invariants.md` (consumer-side standing surface).
- Justification 조건 (면제): short-lived (단일 요청/트랜잭션 수명, 프로세스 수명 누적 0) 구조 = scope 외 (long-lived 가 아님). 불변 (immutable) 구조 = scope 외 (mutable 가 아님).
- Exemption: wrapper-self (codeforge dogfood) = runtime 0 (governance Story, in-process long-lived structure 부재) → declarative 면제 (ADR-005 `plugin-meta-na` 정합) — Story §1 명시 선언.
- Verification format: **standing-invariant-surface-row** — long-lived mutable structure 별 (a) name + location (file) (b) kind (buffer/queue/cache/counter/ledger/WAL 등) (c) bound invariant (backlog vs lifetime 구분) (d) lifetime invariant (reclaim 여부) (e) ordering invariant (f) 코드 보존 지점 (g) accumulation/lifetime-class? (Y/N) 7-key 정합 + `docs/system-invariants.md` 색인 존재. I-1~I-7 write-time 발화 검증 axis 와 **disjoint axis** — I-8 = 영속 standing 문서 색인 면 (1회 발화가 아닌 상시 surface 유지). dedup 비교차 (same anchor_id 양 literal 후보 충돌 회피).
...
- I-8 (Amendment 6, CFP-2351) self-check = **review-verdict-v4 self-check field binding 명시적 defer** — 본 Amendment 6 은 review-verdict-v4 무수정 (병렬 force-push 충돌 회피). I-8 verdict-level field (예: `invariant_surface_extension_self_check_passed`) 신설 + finding type literal 추가 = **S4 (#2350, review-verdict-v4 single-owner) 위임 또는 Phase 2 defer**. 본 Amendment 6 = I-8 declaration + `templates/system-invariants.md` template + impl-PR-extends-invariant-surface 의무 codify 만 (declarative anchor only). impl PR 이 새 long-lived mutable structure 추가 시 `docs/system-invariants.md` 색인 row append/갱신 의무 self-check 는 chief author write-time 가 아닌 **impl PR 시점** 영역 (S4 falsifier cross-ref surface).
...
- I-8 (Amendment 6) cross-validate: finding type literal + verdict field = **S4 (#2350) 위임** (본 Amendment 6 review-verdict-v4 무수정). 설계 lane perspective — Change Plan §3/§5/§11 이 새 long-lived mutable structure 도입 시 `docs/system-invariants.md` 색인 row append/갱신 누락 발견 flag (설계 문서 ↔ standing invariant-surface 정합 감사). 실 finding type literal = S4 carrier 가 review-verdict-v4 single-owner 로 신설.
...
- I-8 (Amendment 6) cross-validate: finding type literal + verdict field = **S4 (#2350) 위임** (본 Amendment 6 review-verdict-v4 무수정). code lane perspective — impl 이 새 long-lived mutable structure (WAL/버퍼/큐/캐시/카운터/ledger) 를 추가했으나 `docs/system-invariants.md` 색인 row append/갱신 누락 시 flag (impl ↔ standing invariant-surface 정합 검증 — bound/lifetime/ordering invariant 가 코드 보존 지점과 일치하는지 cross-validate). 실 finding type literal = S4 carrier 가 review-verdict-v4 single-owner 로 신설.
...
| `chief-author-crossref-consistency-check` | `deferred-followup` | §결정 1 (I-7 신설, Amendment 5) + Amendment 7 (CFP-2478 Wave 2 carrier 지정) | verdict field `chief_author_crossref_consistency_self_check_passed: bool` (review-verdict-v4 v4.12) + DesignReview/CodeReview `findings[].type: "chief-author-crossref-inconsistency"`. ADR-068 Amendment 5 신설 시점 Wave 1 declaration-only — evidence-checks-registry entry `chief-author-crossref-consistency` (status deferred-followup, bypass_label `hotfix-bypass:boundary-wording` 재사용 — ADR-068 owner family). **Amendment 7 (CFP-2478) = Wave 2 mechanical wire carrier 지정** (Amendment 5 의 '별 sub-CFP Wave 2 carrier' 슬롯 = CFP-2478 충족): author-time verdict self-check (Amendment 5) 보존 + CI-time 결정론 cross-document lint (layer A) disjoint 추가 (ADR-074 §결정 8 author-time↔CI-time 분리 답습). Phase 1 (design lane) = ADR codify (Amendment 7) + Change Plan; Phase 2 PR = scripts/check-adr-cross-ref-consistency.sh + scripts/lib (ADR-061) + workflow yml (ADR-005 byte-identical) + bats RED→GREEN (CFP-2457 4 drift distinct fixture) + evidence-registry status deferred-followup → warning 전환. semantic misquote (번호 맞고 §결정 의미 틀림) = layer B Codex LLM advisory (자동 차단권 없음). pattern_count >= 2 재발 시 follow-up CFP MUST promote (ADR-082 §결정 6 정합). |
| `invariant-surface-extension-check` | `deferred-followup` | §결정 1 (I-8 신설, Amendment 6) | I-8 standing invariant-surface 의무 = `docs/system-invariants.md` 색인 (template `templates/system-invariants.md`) + impl PR 이 새 long-lived mutable structure 추가 시 surface 확장. **verdict field + finding type literal = S4 (#2350, review-verdict-v4 single-owner) 위임** — 본 Amendment 6 review-verdict-v4 무수정 (병렬 force-push 충돌 회피). ADR-068 Amendment 6 신설 시점 Wave 1 declaration-only. 실 mechanical wire (impl-PR-extends-invariant-surface lint scripts/check-invariant-surface.sh + scripts/lib + workflow yml + bats) = 별 sub-CFP Wave 2 carrier (ADR-119 §결정 8 / ADR-060 4-tier promotion). pattern_count >= 2 재발 시 follow-up CFP MUST promote (ADR-082 §결정 6 정합). wrapper-self declarative (runtime 0 면제 — ADR-005). |
...
본 ADR-068 frontmatter `mechanical_enforcement_actions[]` 6 entry 모두 4 field 보유 (Amendment 1 `dimensional-empirical-grounding` + Amendment 3 `audit-gate-pointer-existence-check` + Amendment 5 `chief-author-crossref-consistency-check` + Amendment 6 `invariant-surface-extension-check` 포함).
...
- **2026-06-19 Amendment 6 (CFP-2351 — I-8 standing invariant-surface invariant 신설)**: 시스템의 long-lived mutable structure (in-process WAL / 버퍼 / 큐 / 캐시 / 카운터 / ledger) 의 bound/lifetime/ordering invariant 가 상시 문서 (`docs/system-invariants.md`) 에 색인돼야 함 + impl PR 이 새 구조 추가 시 invariant-surface 확장 의무. I-8 의무 항목 7-key (name+location / kind / bound (backlog vs lifetime 구분) / lifetime (reclaim 여부) / ordering / 코드 보존 지점 / accumulation/lifetime-class? Y/N). ADR-119 §결정 10 ② generative invariant sweep ("실패 경로의 long-lived mutable 구조 열거 + bound/lifetime/ordering invariant 명시 + 코드 보존 여부 실측", verified worktree Read ADR-119 line 206) 의 standing 기록처 instantiation — S4 (#2350) 의 falsifier 가 invariant-surface 입력으로 cross-ref, enumeration 완전성 보강 (S4 enabler). accumulation/lifetime-class flag = ADR-015 Amendment 1 §결정 1 (accumulation/lifetime-class soak, verified line 166-168) + ADR-014 Amendment 7 §7.4.7 outcome-signal ③ 발현조건 임계 (verified line 508) cross-ref. **review-verdict-v4 무수정** — I-8 verdict-level field + finding type literal 추가 = S4 (#2350, review-verdict-v4 single-owner) 위임 (병렬 force-push 충돌 회피). enforcement = Phase 1 declarative-only (I-8 declaration + `templates/system-invariants.md` template; impl-PR-extends-invariant-surface mechanical lint = 별 carrier Phase 2 defer, ADR-119 §결정 8 / ADR-060). wrapper-self declarative (runtime 0 면제, ADR-005). ratchet 강화 방향 (7 → 8 invariants, ADR-058 §결정 5 정합). I-1~I-7 본문 의미 변경 0건 — I-8 disjoint append only (standing invariant-surface axis 분리). Epic #2346 진단축 D (전 lane PASS 후 제품 사망, 표면 증상 진단 약 10h 3회 헛수정 — 'WAL accumulator 회수 안 됨' 상시 구조적 속성 기록처 부재). `mechanical_enforcement_actions[]` entry `invariant-surface-extension-check` (deferred-followup) append. CFP-528 Amendment 1 / CFP-1087 Amendment 3 / CFP-1565 Amendment 5 invariant 신설 precedent verbatim 답습. doc-only fast-path ADR-054.
- **2026-06-30 Amendment 7 (CFP-2478 — I-7 CI-time 기계화 carrier 실현 + cross-document 정합 sweep, Epic CFP-2476 E2)**: Amendment 5 (CFP-1565) 가 I-7 신설 시 예약한 "별 sub-CFP Wave 2 carrier" 슬롯을 CFP-2478 이 충족 — I-7 의 author-time 사람 self-check (verdict field, v4.12 보존) 를 CI-time 결정론 cross-document lint 로 보완 (disjoint layer, ADR-074 §결정 8 author-time↔CI-time 분리 답습) + cross-document 정합 sweep 2-layer codify. **신규 invariant 0** (boundary completeness 8 invariants I-1~I-8 본문 의미 변경 0 — 본 Amendment 는 invariant 신설 아닌 I-7 mechanical wire carrier 실현). 2 layer (mechanism 분리, 통합 금지): **(A) 결정론 grep** (CI workflow PR-time warning) = cross-ADR cross-ref 번호 존재 + phantom ID ownership (case #3 = I-7 직접 기계화) + enum 리터럴 SSOT 대조 (정본 명확 한정 — H6) + 버전 parity + citation **content-anchor** (line-number 직접 정합 제외 — U-2). **(B) Codex LLM** (Orchestrator inline dispatch advisory, 변경분 diff) = substantive 단정 식별 (verbatim 한정 — over-extraction 차단 CE-1) + semantic misquote. finding = `[hypothesis]` default → ground-truth falsify 후 채택 (ADR-070 D1/D3). 실행 dispatch = E1 execution-dispatch-pattern-v1 (ADR-070 Amd11 §B3 + ADR-081 Amd11 §A3) 재사용 — Codex sandbox (read-only/network-off/.git 보호), lane worker own-Bash 아님. disjoint (U-1 (a) 위임) = 기존 lint 2종 (ADR-074 narrative↔amendment_log / check_adr_citation_slug.py L1/L2) 보존, 본 sweep 은 명시 공백만 cover (흡수·sunset 0). 오탐 = P2 자동차단 금지 (FP 20~30%+ = 폐기 임계 CE-6) + abstention 채널. CFP-2457 4 drift 케이스 (ADR-039 entry 번호 / ADR-052 enum / ADR-077 phantom 'I-4' = 실제 ADR-068 소유 / citation 줄번호) = distinct regression fixture (Phase 2 bats). ADR-119 amendment 불요 (wave_2_wire_carrier '후속 lint CFP' 예약 실현 — verified ADR-119:49, cross-ref backref 만). `chief-author-crossref-consistency-check` mechanical_enforcement_action progress_note 갱신 (Wave 2 carrier 지정) + §결정 6 표 row 갱신. **review-verdict-v4 무수정** (Amendment 5 가 이미 v4.12 field 추가). enforcement = Phase 1 design lane (ADR codify + Change Plan) / Phase 2 = mechanical wire (scripts/check-adr-cross-ref-consistency.sh + lib + workflow + bats + registry status deferred-followup → warning 전환 + plugin bump 6.50.0 → 6.51.0 + marketplace sync). ratchet 강화 방향 (I-7 author-time → +CI-time mechanical layer, scope 약화 0). related_adrs ADR-074/119/070/081 backref 추가. CFP-2458 #7 ADDITIVE 선례 (신규 mechanism class). doc-only fast-path ADR-054.
...
- **ADR Amendment carrier 발의 의무** — RACI 미codify 영역을 codify 하는 별도 follow-up CFP (또는 본 Story 내 ADR Amendment) 발의. ADR-086 §결정 1 axis 분석 + §결정 2 5-checklist self-app 의무 (Deputy 신설 결정 framework cross-ref).
...
- **Amendment 1 (v4.4, CFP-528)**: I-5 별도 boolean field `dimensional_empirical_self_check_passed` 신설 (별도 disjoint check)
- **본 Amendment 2 (CFP-1086)**: `boundary_completeness_self_check_passed` scope expansion — 5 invariants (I-1 ~ I-4 + Amendment 2 chief tie-break ladder 3 단계 mechanism) 모두 통과 시 true. I-5 별도 field 무변경 (Amendment 1 disjoint invariant 보존).
...
## Amendment 6 — CFP-2351 I-8 standing invariant-surface invariant 신설

**날짜**: 2026-06-19 KST

> **형식 주의**: Amendment 5 (I-7) body section 방식 답습 — frontmatter `amendments:` block (amendment_id 6) + 본 `## Amendment 6` body section + §결정 1 I-8 declaration + §결정 2 Tier A/B/C I-8 note + §결정 6 표 5번째 row + 변경이력 entry. **review-verdict-v4 무수정** (I-1~I-7 은 각자 review-verdict-v4 self-check field 를 추가했으나, I-8 의 verdict-level field + finding type literal 추가는 S4 (#2350, review-verdict-v4 single-owner) 위임 — 병렬 force-push 충돌 회피).

### 동기

Epic #2346 진단축 D — runtime-failure 진단 시 시스템의 long-lived mutable structure (in-process WAL / 버퍼 / 큐 / 캐시 / 카운터 / ledger) 의 **상시 구조적 속성** (bound / lifetime / ordering invariant) 이 어디에도 색인돼 있지 않아 장애 중 매번 재도출됐다. "이 WAL accumulator 는 회수 안 됨" 은 코드를 바꾸지 않는 한 변하지 않는 상시 속성인데, 기록처가 없어 진단이 표면 증상 (lease-thrash 등) 에서 출발 → 약 10h 3회 헛수정 (ADR-119 §결정 10 incident 근거 — "consumer 수집기가 전 lane PASS 했으나 제품 사망 ... 약 10h 3회 헛수정" [cited_adr: ADR-119 §결정 10, cited_value verbatim, verify_status: verified — worktree Read ADR-119 line 190]).

기존 I-1~I-7 invariants = chief author 의 **write-time 발화 1회** 를 검증하는 axis (API contract docstring / cross-module propagation / guard placement / wording SSOT / dimensional empirical / audit-gate-pointer / cross-ADR claim). **구조적 속성을 상시 standing 문서에 색인하는 axis 부재** — 1회 발화 검증만으로는 "프로세스 수명 동안 누적·유지되는 구조의 bound/lifetime/ordering 이 지금 코드에서 어떻게 보존되는가" 라는 영속 surface 가 남지 않는다.

본 Amendment 6 = I-8 신설 — long-lived mutable structure 의 bound/lifetime/ordering invariant 를 상시 문서 (`docs/system-invariants.md`) 에 색인 + impl PR 이 새 구조 추가 시 surface 확장 의무.

### I-8 정의 (§결정 1 cross-ref)

§결정 1 I-8 declaration verbatim 적용. 각 long-lived mutable structure 별 7-key (name+location / kind / bound (backlog vs lifetime 구분) / lifetime (reclaim 여부) / ordering / 코드 보존 지점 / accumulation/lifetime-class? Y/N) standing-invariant-surface-row + `docs/system-invariants.md` 색인 존재 의무. template = `templates/system-invariants.md` (consumer-side standing surface).

### ADR-119 §결정 10 ② instantiation (진단면 generative invariant sweep 의 standing 기록처)

I-8 = ADR-119 §결정 10 ② 진단면의 instantiation:

- ADR-119 §결정 10 ② "generative invariant sweep — 실패 경로의 long-lived mutable 구조 열거 + bound / lifetime / ordering invariant 명시 + 코드 보존 여부 실측" [cited_adr: ADR-119 §결정 10 ②, cited_value: "generative invariant sweep — 실패 경로의 long-lived mutable 구조 열거 + bound / lifetime / ordering invariant 명시 + 코드 보존 여부 실측", verify_status: verified — worktree Read ADR-119 line 206]
- I-8 = 그 sweep 이 **매 진단마다 재열거** 하지 않도록 long-lived mutable 구조 + invariant 를 **상시 standing 문서** 에 미리 색인하는 기록처. S4 (#2350) 의 falsifier 가 invariant-surface 입력으로 본 surface 를 cross-ref 해 sweep 의 **enumeration 완전성** (구조 누락 0) 을 보강한다 [cited_adr: ADR-119 §결정 10 ②, cited_value: "S4 (#2350) 가 3번째 rung ('문제정의 오류 → 요구사항 lane 재진입') 을 신설", verify_status: verified — worktree Read ADR-119 line 208].
- S4 가 정의하는 falsifier 입력 ({코드, 증상, outcome-contract, invariant-surface} 4-tuple) 중 **invariant-surface 입력 = 본 I-8 surface** — 4-tuple 의 구체 구조 자체는 S4 carrier (#2350) 영역, 본 Amendment 6 = invariant-surface 의 standing 기록처 + 색인 의무만 codify (S4 의 enabler).

### accumulation/lifetime-class flag cross-ref

누적·monotone·미회수 구조의 명시 flag (`accumulation/lifetime-class? = Y`) 는 두 sibling ADR 와 정합:

| Source | cited_value | verify_status |
|---|---|---|
| ADR-015 Amendment 1 §결정 1 (accumulation/lifetime-class soak) | "accumulation / lifetime-class 리스크 (capacity 회계 monotone·미회수 / 무한 누적 / lifetime cap)" | verified — worktree Read ADR-015 line 166-168 |
| ADR-014 Amendment 7 §7.4.7 outcome-signal ③ | "③ 발현조건 임계 = accumulation\lifetime-class 리스크가 드러나는 누적량" | verified — worktree Read ADR-014 line 508 |

I-8 의 `accumulation/lifetime-class? = Y` flag = ADR-015 (테스트 측 soak 지속 시간 도출) + ADR-014 (운영 AC 측 발현조건 임계 선언) 의 **설계 정보원** — 같은 구조의 누적 속성을 standing surface 에 색인해 양 sibling 이 cross-ref 하는 단일 출처화.

### review-verdict-v4 binding = S4 위임 (본 Amendment 6 무수정)

- I-1~I-7 은 각자 review-verdict-v4 self-check field (예: I-7 = `chief_author_crossref_consistency_self_check_passed`) 를 추가했으나, **I-8 의 verdict-level field + finding type literal 추가는 S4 (#2350, review-verdict-v4 single-owner) 위임 또는 Phase 2 defer**.
- 본 Amendment 6 = review-verdict-v4.md **무수정** (병렬 force-push 충돌 회피 — S4 가 같은 contract file 의 single-owner). I-8 declaration + `templates/system-invariants.md` template + impl-PR-extends-invariant-surface 의무 codify 만.
- mechanical self-check binding 명시적 defer — Tier A/B/C 의 I-8 note 모두 "S4 위임" 으로 declare.

### enforcement = Phase 1 declarative-only

- 본 Amendment 6 = I-8 declaration + template (`templates/system-invariants.md`). impl-PR-extends-invariant-surface mechanical lint + review-verdict-v4 self-check field = **별 carrier Phase 2 defer** (ADR-119 §결정 8 declarative-only 정합 [cited_adr: ADR-119 §결정 10 enforcement/경계, cited_value: "Phase 1 declarative-only (§결정 8 정합) — 본 §결정 10 = umbrella anchor, mechanical lint 미신설. 실 wire 는 child Story S2~S5", verify_status: verified — worktree Read ADR-119 line 213] + ADR-060 4-tier promotion 경로 warning-tier entry → 승격 gate).
- **wrapper-self declarative** (codeforge dogfood) — runtime 0 (governance Story, in-process long-lived structure 부재) → I-8 면제 (ADR-005 `plugin-meta-na` 정합). consumer 측 = `templates/system-invariants.md` 로 자기 시스템에 standing surface 유지.

### ratchet 강화 방향 (ADR-058 §결정 5 / ADR-064 §결정 7 정합)

- 강화 방향: 7 → 8 invariants 추가 (scope 축소 0건)
- I-1/I-2/I-3/I-4/I-5/I-6/I-7 본문 의미 변경 0건 — I-8 disjoint append only (standing invariant-surface axis 분리 — write-time 1회 발화 검증 axis 와 별도 axis, 영속 standing-doc 색인 면)
- `is_transitional: false` 유지 (permanent governance — ADR-058 §결정 7 governance default presumption 정합)
- sunset_justification = N/A (ratchet 강화 방향, Amendment 1/3/5 invariant 신설 precedent 답습)

### Mechanical enforcement actions binding (§결정 6 영역)

본 Amendment 6 = Wave 1 declaration-only — `mechanical_enforcement_actions[]` entry `invariant-surface-extension-check` (status deferred-followup) append. 실 mechanical wire (`scripts/check-invariant-surface.sh` + scripts/lib + workflow yml + bats fixture + review-verdict-v4 self-check field = S4 위임) = 별 sub-CFP Wave 2 carrier. bypass_label = `hotfix-bypass:boundary-wording` 재사용 (ADR-068 owner family — 신규 label 신설 0건, dimensional-empirical-grounding / chief-author-crossref-consistency deferred entry 가 기존 label 재사용한 precedent 답습). pattern_count >= 2 재발 시 follow-up CFP MUST promote (ADR-082 §결정 6 + ADR-070 §D5 retain pattern 답습).

### Cross-ref

- ADR-119 §결정 10 ② (generative invariant sweep — I-8 = 그 standing 기록처 instantiation, S4 falsifier invariant-surface 입력. 본문 0건 변경)
- ADR-015 Amendment 1 §결정 1 (accumulation/lifetime-class soak — I-8 의 accumulation/lifetime-class flag 정보원, 테스트 측. 본문 0건 변경)
- ADR-014 Amendment 7 §7.4.7 outcome-signal ③ (발현조건 임계 — I-8 의 accumulation/lifetime-class flag 정보원, 운영 AC 측. 본문 0건 변경)
- ADR-119 §결정 8 (Phase 1 declarative-only — mechanical wire defer 정합)
- ADR-060 (4-tier promotion 경로 — invariant-surface-extension-check warning-tier → blocking 승격 별도 CFP)
- ADR-073 §결정 1 (verify-before-assert primitive — impl PR 색인 row 갱신 시 코드 보존 지점 direct verify directly-analogous)
- ADR-068 Amendment 1 (CFP-528 — I-5 invariant 신설 precedent) + Amendment 3 (CFP-1087 — I-6 invariant 신설 precedent) + Amendment 5 (CFP-1565 — I-7 invariant 신설 precedent verbatim 답습)
- ADR-058 §결정 5 / ADR-064 §결정 7 (top-down ratchet 정합)
- ADR-082 §결정 6 + ADR-070 §D5 (declaration-only Wave 1 retain pattern 답습)
- ADR-005 (plugin self-application NA — wrapper-self declarative 면제 정합)
- 신규 `templates/system-invariants.md` (consumer-side standing invariant-surface template)
- S4 (#2350, review-verdict-v4 single-owner) — I-8 verdict-level field + finding type literal binding 위임 carrier
...
### Amendment 6 (CFP-2351 — I-8 standing invariant-surface)

- `archive/adr/ADR-068-boundary-completeness-invariants.md` (Amendment 6 — frontmatter `amendments[]` amendment_id 6 + `mechanical_enforcement_actions[]` `invariant-surface-extension-check` entry + `related_stories` CFP-2351 + `related_adrs` ADR-119 / ADR-015 / ADR-014 + 본문 §결정 1 I-8 declaration + §결정 2 Tier A/B/C I-8 note + §결정 6 표 5번째 row + 본 `## Amendment 6` section + 변경이력 entry, CFP-2351)
- `templates/system-invariants.md` (신규 — consumer-side standing invariant-surface template, long-lived mutable structure 의 bound/lifetime/ordering invariant 색인. CFP-2351)
- `docs/system-invariants.md` (consumer 측 standing surface 실 instance — template 로부터 생성, consumer-guide cross-ref. wrapper-self 면제 = runtime 0)
- `docs/inter-plugin-contracts/review-verdict-v4.md` (I-8 verdict-level field + finding type literal = **S4 (#2350) 위임** — 본 Amendment 6 무수정, single-owner 충돌 회피)
- `skills/root-cause-decision/SKILL.md` (S4 #2350 — 3번째 rung '문제정의 오류 → 요구사항 lane 재진입' + falsifier invariant-surface 입력으로 본 I-8 surface cross-ref. 본 Amendment 6 무수정, S4 carrier 영역)
...
본 cross-ref note = mechanical_enforcement_actions[] entry 0건 추가 — ADR-082 §결정 6 + ADR-070 §D5 + CFP-898 / CFP-899 / CFP-900 precedent declaration-only Wave 1 retain pattern 답습. 본 CFP-1059 Story-1 = declaration carrier (declarative scope), mechanical wire = Phase 2 PR (별도 CFP — Wave 2 후속 carrier).
...
## Amendment 7 — CFP-2478 I-7 CI-time 기계화 carrier 실현 + cross-document 정합 sweep (Epic CFP-2476 E2)

**날짜**: 2026-06-30 KST

> **형식 주의**: 본 Amendment 는 **신규 invariant 0** — Amendment 1/3/5/6 (I-5/I-6/I-7/I-8 신설) 과 다르다. I-7 (Amendment 5) 의 author-time 사람 self-check 를 **CI-time 결정론 lint 로 보완** (disjoint layer) + cross-document 정합 sweep 을 codify 하는 carrier 실현 amendment 다. boundary completeness invariant 개수 = 8 유지 (I-1~I-8 무변경). frontmatter `amendments:` block (amendment_id 7) + 본 body section + §결정 6 표 chief-author-crossref-consistency-check row progress_note 갱신 + 변경이력 entry. **review-verdict-v4 무수정** (Amendment 5 가 이미 v4.12 에 `chief_author_crossref_consistency_self_check_passed` field 추가 — 본 Amendment 는 그 author-time field 의 CI-time 보완 layer 만 codify, verdict schema bump 0).

### 동기

Amendment 5 (CFP-1565, 2026-06-01) 가 I-7 (chief-author cross-ADR scope/fact claim consistency) 를 신설하며 enforcement 를 **Wave 1 declaration-only (verdict field-only)** 로 두고, 실 mechanical wire 를 "별 sub-CFP Wave 2 carrier" 로 예약했다 (Amendment 5 progress_note + evidence-checks-registry `chief-author-crossref-consistency` entry `sibling_dependencies.TBD-Wave-2-sub-CFP` 슬롯). 그 Wave 2 carrier 슬롯이 **CFP-2478 (Epic CFP-2476 E2 = Codex 주장→증거 감사기 + cross-document 정합) 으로 충족** 된다.
...
기존 lint 2종 (ADR-074 narrative↔amendment_log length / `check_adr_citation_slug.py` L1 번호존재 + L2 ADR-057 misquote deny) 은 모두 이 4 case 를 **명시적 미탐** 으로 남긴다 [verified: check_adr_citation_slug.py:32-38 "semantic correctness = reviewer responsibility, not lint scope" / ADR-074 scope = CLAUDE.md↔amendment_log 한 쌍]. 즉 ADR↔ADR cross-ref / phantom ID ownership / enum 리터럴 / citation anchor / 버전 표기 = 거버넌스 cross-document coherence 공백.

### 결정 — 2-layer cross-document 정합 + I-7 CI-time 보완 (mechanism 분리, 통합 금지)

본 cross-document sweep 은 **두 layer 로 분해** 한다 (단일 mechanism 통합 금지 — 결정론 영역에 LLM 주입 = 비용·재현성 손해, Story §4.2 판정 + concept CE-2/CE-5):

**Layer A — 결정론 cross-document sweep (CI workflow PR-time, warning tier)**:
- 적용 대상 (closed set): (a) cross-ADR cross-ref 번호 존재 — `ADR-NNN` 인용 → 파일 실존 (check_adr_citation_slug.py L1 disjoint 확장) (b) **phantom ID ownership** — `ADR-NNN <ID>` 인용 시 그 ID (`I-N` / `§결정 N` 등) 가 대상 ADR 의 실 소유인지 (case #3, I-7 의 직접 기계화 — ID→소유 ADR 매핑 데이터 모델 = Phase 2 설계) (c) enum 리터럴 SSOT 대조 — **정본 SSOT 명확한 enum 한정** (case #2; 정본 부재 enum 은 결정론 검사 불가 — Story H6) (d) 버전 표기 parity (기존 check-3way-version-parity.sh 재사용/확장) (e) citation **content-anchor** 대조 — 인용 텍스트 존재 grep (line-number 직접 정합 제외 — U-2 결정).
- 재현 가능 (grep 결정론) → 자동 tier 정당 (concept CE-5). 첫 도입 = warning (ADR-060 §결정 5). blocking 승격 = 별 carrier (pr_cumulative_min:20 / failure_threshold:0).

**Layer B — Codex LLM claim-audit (Orchestrator inline dispatch, ADR-052 touchpoint 계열 advisory, 변경분 diff 한정)**:
- 적용 대상: (a) substantive 단정 식별·파싱 — **원문 verbatim 단정 한정** (감사기 생성 inferred claim 배제, over-extraction 차단 — concept CE-1) (b) semantic misquote — 번호 맞고 §결정 *의미* 가 인용과 다름 (check_adr_citation_slug.py 가 명시 미탐으로 남긴 잔여).
- 비결정 (LLM nondeterminism, concept CE-5) → finding = `[hypothesis]` default, Orchestrator own working dir 직접 Read falsify 후만 채택 (자동 차단권 없음 — ADR-070 D1/D3 verify-before-trust + ADR-077 I-4 fact-check marker 무검증 승격 금지). mismatch 또는 재현 실패 → reject.
- 실행 dispatch = E1 (CFP-2477) **execution-dispatch-pattern-v1** 재사용 (ADR-070 Amendment 11 §B3 + ADR-081 Amendment 11 §A3 + concept execution-based-review-verification 7-step SSOT) — Codex CLI 자체 sandbox (read-only 기본 / network-off / `.git` 보호 / OS 격리), lane worker own-Bash 직접 실행 아님 (OWASP LLM06 최소권한). dispatch context = Orchestrator top-level inline (subagent owner 면 ADR-039 재귀 가드로 silent skip → 게이트 연극화; skip 시 `[codex-sandbox-fallback: subagent_recursion_blocked]` marker 의무).

### disjoint 경계 (U-1 = (a) 위임·disjoint 채택)

기존 cross-document lint 2종을 **보존** 한다 (흡수·sunset 0):
- **ADR-074** (CLAUDE.md narrative ↔ ADR `amendment_log[]` length, same-line strict, PR-time, warning) — scope = CLAUDE.md ↔ amendment_log 한 쌍. 본 sweep 의 amendment-번호 정합은 ADR-074 에 위임 (본 감사기 제외). ADR-074 §결정 8 (author-time self-check ↔ CI-time lint layer 분리) precedent 답습 — author self-check (I-7 verdict field, Amendment 5) 보존 + CI-time lint (layer A) disjoint 추가.
- **check_adr_citation_slug.py** (L1 번호존재 / L2 ADR-057 misquote deny) — 본 감사기 layer A 는 그 명시 공백 (phantom ID ownership / enum 리터럴 / content-anchor / 버전 표기) 만 cover. L1/L2 ↔ layer A 출력 중복 detect 행 정리 = Phase 2.

본 sweep = 두 lint 의 **명시된 공백** 을 정확히 메우는 신규 lint 영역 (중복 0). disjoint codify = "번호 의미·phantom ID ownership·enum·content-anchor·버전 표기 정합" (신규) vs 기존 2종.

### 오탐 처리 (FP calibration = 채택 생존선)

- substantive 단정 파싱 = 원문 verbatim 한정 (over-extraction = FP 1차 진원 — concept CE-1).
- P2 급 finding 자동 차단 금지 (기록 후 진행) — FP 20~30%+ = 도구 폐기 임계 (concept CE-6, static analysis cry-wolf 실증).
- 차단·FIX 승격 권한 = evidence (file:line) 동반 + ground-truth 재검증 통과 finding 한정. abstention 채널 필수 (검증 불가 추상 주장 = substantive 아님으로 skip).
- 검사연극 두 의미 구분: (가) 감사기 자신 FP (layer A 결정론 + layer B verbatim-한정+재검증 전처리) / (나) 대상 hollow-check (family Story B mutation 영역 인접, mechanism 다름).

### ADR 판단 (신규 ADR vs amendment)

- **ADR-068 Amendment 7 (본)** — I-7 의 mechanical wire carrier 실현이지 신규 거버넌스 결정 아님 → amendment 적격 (신규 ADR 불요). CFP-2458 #7 ADDITIVE 선례 (trim 된 시점-touchpoint 와 disjoint 신규 mechanism = ratchet 강화).
- **ADR-119 amendment 불요** — wave_2_wire_carrier 가 "후속 lint CFP (ADR-060 4-tier 경로)" 예약 [verified: ADR-119:49]. 본 CFP 가 예약 carrier 실현 → cross-ref backref 만 (frontmatter related_adrs).
- **ADR-070 / ADR-081 cross-ref only** — execution-dispatch-pattern-v1 (E1) 재사용, 본문 0건 변경.
- **ADR-052 touchpoint #8 vs 별도 ADR** — layer B 가 ADR-052 touchpoint 계열 (Codex 신호원 origin 재사용, advisory). 신규 touchpoint enum 추가 시 ADR-052 Amendment + playbook 절차 = Phase 2 조건부 (실 wire 시점). Phase 1 (본 amendment) 은 mechanism class codify 만.

### enforcement (Phase 1 design lane = ADR codify, Phase 2 = mechanical wire)

- **Phase 1 (design lane, 본 PR)**: ADR-068 Amendment 7 (본) + Change Plan (`docs/change-plans/cfp-2478-codex-claim-evidence-audit.md`) + concept (`claim-to-evidence-audit.md`, 요구사항 lane 이미 landed). review-verdict-v4 무수정. plugin bump 0 (doc-only Phase 1 — ADR-054 fast-path).
- **Phase 2 (구현 lane, 별 PR)**: `scripts/check-adr-cross-ref-consistency.sh` (bash thin wrapper) + `scripts/lib/check_adr_cross_ref_consistency.py` (ADR-061 Python SSOT) + `templates/github-workflows/*.yml` + `.github/workflows/*.yml` (ADR-005 byte-identical) + bats fixture RED→GREEN (CFP-2457 4 drift distinct fixture — AC-9, CFP-1334 §8.4) + evidence-checks-registry `chief-author-crossref-consistency` entry status deferred-followup → warning 전환 (또는 신규 `adr-cross-ref-consistency` entry append + sibling) + label-registry-v2 MINOR (필요 시 — hotfix-bypass:boundary-wording 재사용으로 신규 label 0건 가능) + plugin bump 6.50.0 → 6.51.0 MINOR + marketplace atomic sync (ADR-063).

### ratchet 정합

ratchet 강화 방향 (I-7 author-time self-check → +CI-time 결정론 mechanical layer 보완 + cross-document sweep CI-time 일반화) — scope 약화 0, invariant 개수 8 유지 (I-1~I-8 본문 의미 변경 0). ADR-058 §결정 5 / ADR-064 §결정 7 top-down ratchet 정합. doc-only fast-path ADR-054 (Phase 1 = ADR codify).

### Cross-references

- ADR-068 Amendment 5 (CFP-1565 — I-7 declaration, 본 Amendment 7 의 carrier 실현 대상)
- ADR-074 §결정 8 (author-time self-check ↔ CI-time lint 분리 precedent)
- ADR-119 §결정 8 (declarative → mechanical 승격 = ADR-060 4-tier 경로) + wave_2_wire_carrier 예약 실현
- ADR-070 Amendment 11 §B3 + ADR-081 Amendment 11 §A3 (execution-dispatch-pattern-v1, E1 재사용)
- concept `claim-to-evidence-audit` (CE-1~CE-6) + `execution-based-review-verification` (X-1~X-6)
- ADR-082 §결정 6 + ADR-070 §D5 (declaration-only Wave 1 → Wave 2 mechanical wire promote pattern)
- CFP-2458 #7 ADDITIVE 선례 (trim 시점-touchpoint 와 disjoint 신규 mechanism)

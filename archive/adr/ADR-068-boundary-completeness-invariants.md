...
- I-1/I-2/I-3/I-4/I-5/I-6/I-7 본문 의미 변경 0건 — I-8 disjoint append only (standing invariant-surface axis 분리 — write-time 1회 발화 검증 axis 와 별 axis, 영속 standing-doc 색인 면)
- `is_transitional: false` 유지 (permanent governance — ADR-058 §결정 7 governance default presumption 정합)
...
- ADR-060 (4-tier promotion 경로 — invariant-surface-extension-check warning-tier → blocking 승격 별 CFP)
...
- **ADR-052 touchpoint #8 vs 별 ADR** — layer B 가 ADR-052 touchpoint 계열 (Codex 신호원 origin 재사용, advisory). 신규 touchpoint enum 추가 시 ADR-052 Amendment + playbook 절차 = Phase 2 조건부 (실 wire 시점). Phase 1 (본 amendment) 은 mechanism class codify 만.

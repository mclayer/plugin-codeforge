...
amendment_log:
  - amendment: 1
    carrier_story: CFP-2183
    date: 2026-06-12
    scope: >-
      D1 결정 변경 — lane repo 8개 GitHub archive (삭제 금지) → 사용자 직접 지시 (2026-06-12) 로
      삭제 전환. 사유: repo 목록 정리 (archive 유지 권고 기각). 트레이드오프 명기: 구 repo
      PR/issue 이력 소멸 + archive 가역성 포기 — git 이력은 D2 subtree 흡수로 모노레포 전량
      보존 (lane commit 368 + blame 연속성, 코드/문서 손실 0). direction: 결정 변경.
      A-0 번호 게이트 불요 (신규 ADR 아님 — Amendment, ADR-054 doc-only fast-path 1 PR).
    sunset_justification: 'N/A — is_transitional: false (영구 기록). 삭제 = 비가역 사실 기록.'
related_adrs:
...
# ADR-118: codeforge 모노레포 통합 — 8 lane repo wrapper 흡수 + 9-plugin 정체성 유지

> 번호 이력: 최초 draft = ADR-117 → 설계리뷰 iter1 P0-1 (main 기존재 `ADR-117-fable-5-surgical-model-tier.md` 충돌) → ADR-118 renumber (origin/main 실측: 118/119 미사용). 재발 차단 절차 = Change Plan §2 A-0 (commit 직전 번호 가용성 재확인).
...
Accepted (2026-06-11 KST — CFP-2152 carrier).
...
| # | 결정 | 근거 |
|---|---|---|
| D1 | lane repo 8개 = GitHub **archive** (삭제 금지) → **Amendment 1: 사용자 지시 삭제 전환** | 이력·이슈 보존, 가역성 → Amendment 1 로 트레이드오프 수용 (아래 로그) |
...
| D6 | ADR 예약 2건 = 본 ADR-118 (신규) + ADR-063 Amendment (source 필드 형식, **S5 carrier**) | family scope 9-plugin 불변 → Amendment 로 충분 [cross-adr-claim-verify: cited_adr=ADR-063 §결정 18-B (Amendment 7, CFP-1059), cited_value="family scope = wrapper + 8 lane = 9 plugin (7→9 확장 codify)", verify_status=verified (설계리뷰 iter1 + ADR-063 frontmatter 직접 Read — 본 Epic 은 9-plugin scope 불변이므로 Amendment 로 충분)] |
...
6. **DD-6 rollback**: merge 전 = branch 폐기 / merge 후 = main 의 PR-B merge commit 1개를 `git revert -m 1 <PR-B merge SHA>` (**단일 revert** — PR-B 가 merge-commit 방식이라 main 측 도입 변경 전체가 1 commit). S6 전 전 구간 lane repo 원본 생존 = 완전 가역.
...
### Amendment 1 — D1 archive → 삭제 전환 (CFP-2183, 2026-06-12)

- **변경**: D1 "lane repo 8개 = GitHub archive (삭제 금지)" → **사용자 직접 지시 (2026-06-12) 로 8개 repo 삭제**. direction: 결정 변경.
- **사유**: repo 목록 정리 — archive 유지 권고를 사용자가 기각.
- **트레이드오프 (명시 수용)**:
  - 구 lane repo 의 PR / issue 이력 소멸 (GitHub UI 접근 불가) — wrapper 문서 안 좌표형 인용 (`plugin-codeforge-<lane>#NN`) 은 평문 역사 기록으로만 잔존.
  - archive 가역성 포기 (un-archive 불가) — 단 git 이력은 D2 subtree 흡수로 모노레포가 전량 보존 (lane commit 368 + blame 연속성), 코드/문서 손실 0.
- **후속 조치 (본 carrier CFP-2183)**: 삭제 전 wrapper repo 의 깨질 참조 (URL · 클릭형) 전수 제거 + 평문 역사 서사는 모노레포 경로 (`plugins/<lane>/`) 병기 보존 (기록 재작성 금지).
- **절차**: 기존 ADR Amendment + 문서만 = ADR-054 doc-only fast-path 1 PR. A-0 번호 게이트 불요 (신규 ADR 번호 미사용 — Amendment).

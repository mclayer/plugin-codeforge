...
본 ADR = 약화 0건. **강화 방향** — 요구사항 lane 에 능동 발굴 forcing function 추가 + 사용자 확정 gate 추가 + terminal/counter/advisory 정직 라벨 추가. 기존 어떤 §결정/게이트도 대체·약화하지 않는다 (짝 Amendment 3건 모두 direction: strengthen — ADR-071 Amd15 = sunset_justification: null / ADR-077 Amd1·ADR-125 Amd3 = frontmatter amendment_log 근거 문자열(non-null, ADR-125 Amd2 strengthen+non-null 선례 허용)). is_transitional: false (permanent governance anchor). 원복은 별도 Story 의 명시 결정으로만 가능하며 그 경우에도 ADR-058 §결정 5 (약화 시 sunset_justification 3-tuple metric/who/how) 를 따른다. ADR-064 §결정 7 top-down self-application ratchet 정합.

## 해소 기준

N/A — permanent policy.

본 ADR 은 요구사항 lane 사용자 대화 프로세스 도메인의 1st-class normative anchor — codeforge 가 deprecate 되지 않는 한 영구 유효. Amendment 는 evidence-gated symmetric ratchet (강화·약화 양방향 + 양방향 evidence 의무, ADR-058 §결정 5 + ADR-064 §결정 7 self-application 정합).

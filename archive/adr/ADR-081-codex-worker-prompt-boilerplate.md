...
related_stories:
  ...
  - CFP-2884 # Amendment 15 — 신규 §결정 D16 promptfile 언어 구획 규약 (3-구획: 지시문 영어 / 인용 원문 verbatim / 상향 한글 요약 병기) + promptfile UTF-8 round-trip fail-closed 배선 (ADR-170 §결정 2 row 7 동형 승계 + TH-A 단일 anchor 재정착 + ADR-123 carve-out 확인). dogfood wrapper-self.
related_adrs:
...
4. **PYTHONUTF8 은 codex 자신에 무효** — codex 실행 사슬 = node launcher → Rust native binary [verified: codex-cli 0.144.5 버전 고정 프로브 — CLI 버전 갱신 시 재검증 의무]. env 실효 대상 = 우리 쪽 Python 조립·검증 helper 한정. codex→model shell env 전달 = `shell_environment_policy` config 소관 (옵션 실재 [verified: `codex exec --help`]) — env 상속 실효 실측 = EXEC_BLOCKED 미결 (침묵 ≠ 상속 성공).
5. 규범 재사용 자산 — ADR-170 §결정 2 표 row 7 + L376: "argv 는 ASCII path 만, 한국어 실값·content 는 UTF-8 파일 내부" (구 ADR-039 §결정 7 (b), 재제정 CFP-2869) [verified: 본 worktree 실독]. 축 A 와 구조 1:1 동형 — 승계 인용은 ADR-170 을 가리킨다 (ADR-039 직접 인용 = stale pointer 금지).
...
1. **3-구획 언어 경계 (판정 규칙 = 사람 아닌 규칙으로 판정 가능)**:
   ...
   - **구획 B** — 인용 원문 = delimited untrusted block **내부**: 기존 대상 (diff + commit 메시지 헤더) + **Story §1 사용자 원문 신규 편입** (기존 경계의 대상 집합 확장 — 신규 delimiter 0, 방어 mechanism 신설 0. "비신뢰" = injection 축 author-provenance 의미론 — 사용자 원문 = 외부 저작 입력이라 분류 타당하며, 무결성은 internal-docs `story-section-1-immutable.yml` required check 가 별도 보장 [verified: PR required check `check-section1` 가동]). **원문 그대로 보존 — 번역·재서술·요약 대체 금지** (감사 ground-truth 보존 — §5 AC 가 §1 원문을 덮는지 감사하는 lane 임무의 전제, ADR-166 1차 소스 원칙 동형). **negative-list 명시**: 한글 commit 메시지 (repo 기본)·한글 파일명 diff = 구획 B — 영어 강제 오적용 금지 (injection 방어 구획과 정면 충돌 회피).
...
7. **§D15 A1.5 `-c` 한정의 encoding env 정밀화** — §D15 의 "`-c` 는 `model_reasoning_effort` 한정" 은 sandbox-위험 override 차단 목적. 본 항은 `-c shell_environment_policy.*` (codex→model shell env-전달 정책 — sandbox 수위 아님) 한정 추가를 **Phase 2 실측 실증 조건부**로 허용한다: (ii) 계층 미상속이 실측으로 실증되고 그 계층이 실사고 경로로 판정될 때만 배선 (미실증 상태 기본 = 미배선 — 추정 lock-in 금지). sandbox 계열 override 금지·위험 플래그 금지는 불변 (약화 0).
...
- §결정 D16 신설 — 3-구획 언어 경계 (A 영어 강제 + whitelist verbatim 예외 / B Story §1 편입 + 번역 금지 + negative-list / C verbatim 무변경 + 한글 요약 병기) + 축 A round-trip fail-closed (앵커 provenance + BOM RED + env 계층 재배치) + ADR-170 row 7 동형 승계 + TH-A 단일 anchor 재정착 + ADR-123 carve-out 확인 + ADR-001 비침해 declare + `-c` encoding env 조건부 정밀화 + lint 3축 — A1 SSOT
...
- Phase 2 wire 목록 (carrier CFP-2884): agent md 3-구획 절 + touch point pointer / whitelist 실물 / round-trip 검증 helper + self-test (fixture 3종 — 조립측 provenance-discriminating 포함) / lint 3축 / TH-A 재정착 sweep / CFP-2851 test 동반 갱신 (same-PR 원자) / living-arch `codeforge-review.md` interfaces 갱신

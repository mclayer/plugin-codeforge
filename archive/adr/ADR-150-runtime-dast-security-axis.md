...
- **DAST 실 구동 = consumer `test.yml`(또는 별 `dast.yml`, QADeveloperAgent, ADR-048 CI-native 정합)**: (a) 앱 CI 기동(docker compose up / 로컬 바이너리) → (b) 스캐너(ZAP baseline/full/api-scan) 실행 → (c) 결과 파싱. 신규 codeforge 러너 부활 = ADR-048 재충돌 금지(StatefulTest deprecated 유지).
...
- **ADR-146 Amendment 로 착륙**: security-attack oracle = 새 mechanism + 신규 게이트/좌표/cross-field 검사 → 별 컨텍스트/결정/결과 블록 필요 → 기각, 신규 ADR-150(§결정 1, A2-5 구조).

...
> **category cross-ref (CFP-2753 정규화)**: primary `security` — 본 결정 성격(접근통제/merge trust boundary, ADR-058 §결정7 security ADR default presumption, 규칙4 단일 primary). 구 compound `governance/security` 의 secondary 축 `governance`(SDLC governance auto-wire 정책)은 정보 손실 방지 위해 본문 cross-ref 로 보존. [verified: ADR-153 Amendment 1 A1-1]

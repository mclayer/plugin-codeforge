...
- `doc frontmatter/section schema (CFP-28 — strict)` 2 context = source workflow 가 repo 어디에도 물리 부재 → **wrapper-self only**.
...
| `strict` | `true` | 형상별 override 여지 | merge 전 base 최신화 강제 (consumer-guide L1853 예시 정합) |
...
3. `scripts/lib/workflow_branch_protection_drift_check.py` — **orphan** (부재 workflow 의 run-block helper, 어떤 `.yml`/`.sh` 도 참조 안 함 — `grep -rl ... --include=*.yml --include=*.sh` = 0건).

...
amendment_log:
  ...
  - amendment_id: 3
    ...
    date: 2026-06-16  # KST per ADR-079 §결정 2
    decisions_touched: ["§결정 1"]
    nature: ratchet-up  # A-prime 운영 모델 — Confluence 를 "공식 읽기 + 사람 편집 표면"으로 격상하되 작성 정본 = git 유지 (편집은 PR 역류). git SoR-work ↔ Confluence SoR-docs disjoint axis 무손상 확장 (forbid scope 축소 0, ADR-058 §결정 5 강화 방향 면제)
    sunset_justification: null  # ratchet-up — Confluence 격상이 작성 정본을 옮기지 않음 (역전 아님). git = SoR-work invariant 보존, Confluence 사람 편집은 git PR 제안(역류, lossy-tolerant)으로만 정본 반영. ADR-058 §결정 5 ratchet 강화 방향 면제 정합. ADR-013 §결정 1 KEEP/MOVE 의미 약화 0건. 약화 방향 (git=SoR-work → Confluence=SoR-work 역전 / backward 직접 git write) 차단
    summary: |
      A-prime 운영 모델 cross-ref (CFP-confluence-aprime) — §결정 1 git SoR-work ↔ Confluence SoR-docs disjoint axis 위에 Confluence 의 "공식 읽기 + 사람 편집 표면" 격상 명시. 핵심 invariant: Confluence 가 사람 편집 가능 표면으로 격상되더라도 **작성 정본 = git** (역전 아님). 사람의 Confluence 편집은 git PR 제안(역류, lossy-tolerant — git 이 최종 정본)으로만 반영. 운영 mechanism (forward 자동 git→Confluence + backward Confluence 편집 → git PR 제안) 의 SSOT = ADR-103 Amendment 3 §결정 7 (본 ADR = git=SoR-work 보존 cross-ref 1-2문장). ADR-103 Amendment 3 동반 발의 (axis disjoint — 본 ADR-100 = git SoR-work 보존 / ADR-103 = sync mechanism). ratchet 강화 방향 (사람 편집 표면 격상 + 작성 정본 git 명시, forbid scope 축소 0).
---
...
- **A-prime 운영 모델 정합 (Amendment 3 cross-ref)** — Confluence 가 단순 readable mirror 에서 "공식 읽기 + 사람 편집 표면"으로 격상되되, **작성 정본은 git 유지**(역전 아님). 사람의 Confluence 편집은 git PR 제안(역류, lossy-tolerant — git 이 최종 정본)으로만 반영되므로 git SoR-work ↔ Confluence SoR-docs disjoint axis 무손상. 운영 mechanism (forward 자동 git→Confluence + backward Confluence 편집 → git PR 제안) SSOT = ADR-103 Amendment 3 §결정 7.
...
- `docs/adr/ADR-103-git-confluence-sync-mechanism.md` — narrow allow wire / sync agent / doc-locations.yaml confluence path 실 결정 owner (W4 MERGED). Amendment 3 §결정 7 = A-prime 운영 모델 mechanism SSOT (forward 자동 git→Confluence + backward Confluence 편집 → git PR 제안), 본 ADR-100 Amendment 3 동반 발의 (axis disjoint — git=SoR-work 보존)
- `docs/project-config-schema.md` — §결정 3 atlassian.* schema 신설 (token = `*_env` reference, deploy 1password env-key precedent 정합, 실 wire = Phase 2)

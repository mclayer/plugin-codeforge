...
| 2 | **보존 기간 안에서만** | green→blue 복귀가 데이터 손실 0 으로 가능한 보존 기간 **안에서만** 자동. CFP-1059 / ADR-087 §결정 5 blue 보존 default 3시간 정합 (`[empirical-source: ADR-087 §결정 5 L142 — brainstorm Phase 1 합의 default, consumer override 가능 project.yaml deploy.retention_hours]`). consumer overlay 확장 가능(더 길게) / 축소 불가(더 짧게 금지 — ADR-064 §결정 7) | 보존 기간 초과 = 자동 rollback 금지 → user-decision layer → hotfix 흐름(rollback-protocol Step 5b forward-fix path) (EC-1) |

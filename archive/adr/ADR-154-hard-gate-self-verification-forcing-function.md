...
- ★**상호 참조 대응표(배선면 = Change Plan §8.QC-MECH)** — 대응표 정본은 **§8.QC-MECH 의 `MECH-X` 절**이 보유한다(중복 표 2벌 금지 · §결정 9 재codify 0 동형). 요지: **A2-2↔MECH-2** / **A2-3↔MECH-7·MECH-1** / **A2-4(M-1)↔MECH-4 ①~⑤** / **A2-5(M-2)↔MECH-5 D-1~D-5** / **A2-6↔MECH-8·MECH-4⑥** / **A2-7↔MECH-5 D-4·MECH-9** / **A2-8↔MECH-3·MECH-7·MECH-9** / **A2-9↔MECH-6**. **A2-1(형태 판정)은 배선면 대응 없음**(ADR 단독).
- ★**천장 동시-변경 불변식**: A2-8 의 정직 천장과 §8.QC-MECH MECH-9 의 정직 천장은 **같은 문면을 양쪽에 보유**한다 — **한쪽에서만 천장을 완화해 인용하는 것을 금지**하며, 바꾸려면 **두 문서가 함께** 바뀌어야 한다. 한쪽만 완화하는 것 자체가 본 ADR 이 겨냥하는 class(*선언과 실상태의 조용한 괴리*)의 문서-축 발현이다.

...
related_files:
  ...
  - scripts/lib/check_deferred_item_recovery.py          # Amendment 1 — structured DEFER row scan (ADR-045 §D-11 enum anchored)
...
2. **(b) 로컬 check 스크립트** — `scripts/lib/check_deferred_item_recovery.py`(structured DEFER row scan) + thin `scripts/check-deferred-item-recovery.sh`(Phase 2).
...
### A1.5 — structured DEFER row schema (ADR-045 §D-11 enum 재사용 — 자유텍스트 회피)

거버넌스 게이트에서 자유텍스트 NLP 파싱은 false-positive/negative 양산 → **enum-anchored structured row** 채택. ADR-045 §D-11 4-option closed-set 재사용:

- `disposition` enum = `{ CLOSE_AS_OBVIATED | CLOSE_AS_SENTINEL | PROMOTE | DEFER }` (ADR-045 §D-11 (1) L671-674) + observe-only 표현은 `CLOSE_AS_SENTINEL`/`DEFER` + 사유, tracking-externalized 는 `PROMOTE`/`CLOSE_AS_OBVIATED` + Issue 링크에 매핑. `[verified: ADR-045 §D-11 L671-674, L692 5-column table]`
- retro.md §deferred 섹션 = 5-column 답습 `disposition | item | tracking(Issue link) | rationale | source(§4/§8 ref)`.

**dual-source AND (theater 차단)**: 선언 DEFER row ∧ (실 tracking Issue 존재 OR observe-only 사유 텍스트 존재). 선언만 있고 backing 없음 = theater → WARN (AC-1). EC-3(observe-only 사유 누락 → WARN) / EC-4(tracking 선언 but 실 Issue 없음 → WARN).
...
- change-plan = **작성**(면제 아님) — Phase 2 코드(스크립트/워크플로/hook/registry) 동반이라 doc-only 아님. `docs/change-plans/cfp-2470-deferred-item-recovery.md` (Phase 1 PR #2472, wrapper repo — 기존 `cfp-1336-cross-repo-label-sync.md` 선례 동형 위치). doc-locations dogfood variant(internal-docs `wrapper/change-plans/`)는 MOVE-path SSOT 이나 본 Phase 1 PR scope 가 plugin-codeforge 라 wrapper repo 적재 (CFP-2456 선례). `[근거: Story §7 "doc-only 아님 → 정식 설계 lane + change-plan 작성 대상"]`
...
- [ADR-045](ADR-045-story-retro-mandatory-trigger.md) — §D-11 batch closure 4-option enum (Amendment 1 structured DEFER row schema 재사용)

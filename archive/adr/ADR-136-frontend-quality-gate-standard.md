...
amendments:
  ...
  - amendment: 5
    date: 2026-07-21  # KST +09:00
    cfp: CFP-2780
    summary: "D1 css-lint 게이트 warning-tier → required 승격 실행(§결정5/§14.5 forward-ref 실현). §결정16 신설. 하위결정 A~D + ADR-060 3-tuple 조작화."
related_adrs:
...
- actionlint 를 전체 workflow 에 실행(overall exit code 무시 — `|| true`)하고, **class 개별 error-string 정규식으로 stdout+stderr 를 grep count** 한다. class C 의 count == 0 이면 GREEN, mutation 재삽입 시 count >= 1 이면 RED(class-scoped).
...
- **partition-axis (§14.2 안①)**: 판정 = **class-scoped grep count**(overall actionlint exit code 무시 — shellcheck 부채로 non-zero 나와도 무관). class 개별 count 0/≥1 대조가 primary, error-string regex 는 secondary 진단 라벨(F6 정합).
...
- **tier (결정 14 §14.5 정합)**: ratchet-UP-only(약화 방향 인용 불가 — 약화 시 ADR-058 §결정5 / ADR-064 §결정7 symmetric evidence 절차). day-1 착지 = **warning-tier**(`continue-on-error`, ADR-060 §결정5 warning-first). 개별 게이트의 L1(blocking) 승격은 결정 14 §14.5 B evidence-gate(ADR-060 3-tuple: PR누적≥20 / failure=0 / sibling merged) 종속 — 별도 named carrier Story(CFP-TBD 금지).
...
### 결정 16 — D1 css-lint required 승격 (Amendment 5, carrier CFP-2780)

§결정5/§14.5 가 예약한 "D1 required 승격 = ADR-060 evidence-gate 통과 별도 follow-up Story" 를 실행한다. 승격 1급 대상 = wrapper self(dogfood).

**하위결정**
- A (job name): 현 job name 유지 + 즉시 required 등록(7일 green 적립). "warning-tier" literal = cosmetic 부채(rename = permanent-pending + chicken-egg 이중 함정 → deferred, 관찰-only).
- B (parity): 양 copy(active `.github` + template `templates/github-workflows`) 동시 `continue-on-error` 제거(Option A). CONSUMER_ONLY_WORKFLOWS 추가(B)·라인단위 예외(C) 반박 — parity invariant 보존 + 신규 machinery 0.
- C (discriminating, F1): 메인 css-lint job + css-lint-test job 둘 다 required(7→9-tuple). css-lint-test = wrapper-ONLY required(consumer wave 미포함 — consumer 는 `if: github.repository` false → skip → permanent-pending 회피). 근거 = 메인 job 은 CSS 0 wrapper 에서 구조적 always-green(discriminating power 0), 실 teeth 는 css-lint-test 에만 존재(hollow-gate 회피, §14.1 execution-liveness).
- D (consumer wave): consumer hard-block 승격 scope-OUT — 후속 per-consumer wave(branch-protection opt-in + honest 7일-green + ADR-060 evidence-gate + css-lint-test 미포함). template de-masking 이 consumer masked-green 을 구조 해소.

**ADR-060 §결정6 3-tuple 조작화(§14.5 종속)**
- (a) PR누적 ≥ 20: css-lint.yml 최초 merge(CFP-2505 Phase2)부터 **merged-PR** 카운트(run 카운트 아님 — ADR-060(a) 단위 = PR merge). Phase 2 firsthand assert(60 run 에서 추론 금지). 경험적 near-certain(3주+ dense PR)이나 assert 의무.
- (b) bypass 외 failure = 0: css-lint = hotfix-bypass 채널 부재 → 순수 failure=0. 등록 eve 재확인(AC-7 동형).
- (c) sibling merged: sibling = CFP-2505(도입 Story), merged(css-lint 문맥 sibling 미정의 → 설계 lane 정의).
- evidence 산출물: (i) run URL / (ii) failure=0 출력 / (iii) PR≥20 카운트 = Phase 2 제출 / (iv) outage runbook = InfraOp §7.4 DR 제공 / (v) audit-comment · (vi) sticky-comment = **N/A**(bypass 채널 부재 → ADR-145 non-applicable 선언).

**cited precondition(미확장)**: ADR-130 §결정4(path-filter 금지)/§결정6(7일+unique+무차별배포금지+wave) = precondition, amend 불요. ADR-145 6→7 narrowing = 표면 동형이나 override 메커니즘 비적용(승격 = 사전 승인 ratchet-UP, 상반 선례 부재).

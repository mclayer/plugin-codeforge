...
**범위**: 기존 `repos[].components` 예약필드(§결정1 schema L82 "implementation 시 권장 — target repo 결정 fallback")의 실 라우팅 격상을 **Phase 2 구현 영역으로 codify** 한다. 본 Amendment 는 *예약필드 활성화 선언* + cross-ref 만 담고, 실 소비 자동화(`story-init.yml` 의 `components` 소비 라우팅)는 **별 carrier = CFP-2423 (Epic CFP-2418 Story 3)** 가 구현한다 (본 ADR-069 §"Phase 2 (mechanism 구현)" carrier 의 multi-repo 라우팅 sub-scope 구체화).

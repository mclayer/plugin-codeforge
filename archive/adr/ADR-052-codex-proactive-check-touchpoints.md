...
| **dispatch 주체** | **구현리뷰 lane worker (sub-agent) 가 Bash 로 Codex 호출** (재귀 가드 = subagent→**Agent tool** 호출 한정, Bash 호출은 미발동) — **ADR-039 inline whitelist 신규 entry 불요** (entry 6 merge-time 흡수 아님 — D6.e). ⚠ **CFP-2828 reconcile**: 예시 `node codex-companion.mjs review` = 잔존 dormant scope-out 채널(§5.5-2); CFP-2828 이후 CodexReviewAgent `codex exec` 직접 전환·`Bash(node *)` 은퇴로 종전 "CodexReviewAgent 동형 proven channel" 근거 소멸 (Amendment 16 시점 = 유효했음, 현재 무효) — touchpoint #8 = 미배선 playbook-only 절차(활성 holder 0), node 권한 원천 = 배선 시 lane worker context. 정본 = §결정 D6.e |
...
touchpoint #7 (Amendment 15) 은 **merge-time** 에 lane PL 부재라 Orchestrator top-level inline dispatch 가 필요했고, Codex 를 Agent tool 로 spawn 하면 재귀 가드 silent skip 되므로 ADR-039 Amendment 6 inline whitelist 6번째 entry 가 필요했다. touchpoint #8 은 **구현리뷰 lane-time** — CodeReviewPL 이 active 하고, 구현리뷰 lane worker(sub-agent) 가 **Bash(`node codex-companion.mjs review`)** 로 Codex 를 호출하는 proven channel 을 보유 [verified — Amendment 16(2026-06) 시점 CodexReviewAgent.md `Bash(node *)` grant + node invocation 실증; **CFP-2828(ADR-081 Amd14 §결정 D15) 이후 CodexReviewAgent 는 `codex exec` 직접 dispatch 로 전환하며 `Bash(node *)` 은퇴** — mutation peer touchpoint #8 의 `node codex-companion.mjs review` 잔존 채널(§5.5-2 scope-out — 본 Story 미이관) doc = `docs/orchestrator-playbook.md` §3.9-ter/§3.10.8. **정직 표기(ADR-119)**: touchpoint #8 은 현재 **미배선 playbook-only 절차**(활성 agent holder 0 — grep empty, companion 실물도 wrapper repo 부재)라 node dispatch 권한을 실제 보유한 주체는 부재하다; (배선 시) 권한 원천은 해당 lane worker context 이지 CodexReviewAgent frontmatter 아님 — CFP-2828 node grant 소멸로 종전 "CodexReviewAgent 동형 proven channel" 근거는 무효]. 재귀 가드 = "subagent → **Agent tool** 호출 금지" (L474) 한정이라 Bash 호출 미발동. 따라서 mutation peer dispatch = lane worker spawn → Codex via Bash = ADR-039 §결정 1 binary always-spawn 정합 (수정 작업 = lane worker 가 수행) + inline whitelist 신규 entry 0 (entry 6 merge-time 흡수 아님, entry 7 신설 아님).

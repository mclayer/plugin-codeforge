...
★★★★ **stale 고지 (FIX Iter 11) — 아래 2엔진 차분은 43행 시절 산출이며 60행에서 재실행하지 않았다.**
...
★★★ **2엔진 차분 실행 결과 (firsthand — FIX Iter 9 최종 43행 재실행 · ★ 60행 미재실행)**: (iv) 표 **전 43행**을
...
- ★ **판별 행 0 이다 (정직 기재)** — 이 순서를 바꿔도 뒤집히는 행이 **없다**(firsthand — ★ **FIX Iter 11 최종 60행 재실행**: 발행일 정규화를 (iii) leg 뒤로 미룬 변주에서 뒤집힌 행 **0**).

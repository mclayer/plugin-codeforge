...
firsthand @ `405abc701`: `git show <ref>:<이 파일> | grep -c skill` → **1**(위 scope 문장 그 자체뿐).
...
- **기계 판정 분해**: ①③ = `normative` (frontmatter·registry 구조 파싱). ② = **`declared`** —
  ...
  **교체 대상을 이름으로 적어 놓고 (iii) 사본만 교체**했고, Iter 12 의 `호출`→`출현` 정정도 (iii) 만 만졌다
  ...
  2 → 4 로 늘어난다**(정의 줄 1이 사라지고 이력 인용 **3**이 생겼다 — `2 − 1 + 3 = 4`).
  산출 = `git show <ref>:<이 파일> | grep -o '<그 문자열>' | wc -l` —
  `e1794df9c` **2** · `529bdefad` **2** · `cc598c55f`(Iter 13 판) **4**.
  ★★ **이 계수는 자기 포함 열거다 (FIX Iter 14 정정, R14 3자 중 2자 수렴)** — 세는 문장 자신이 대상에
  포함된다(신설 인용 3곳 중 하나가 바로 이 계수를 적는 문장이었다). 직전 판의 `3` 은 그 자기 포함분을
  빠뜨린 값이며, *"계수만 보고 오독하지 말라"* 고 경고하는 자리에 **틀린 계수를 앵커로 놓았다.**
  ★★★ **그래서 고정 정수가 아니라 「명령 + immutable ref」 로 적는다** — 최종 문면은 산출 명령을
  `<그 문자열>` **플레이스홀더**로 적어 자기 포함을 끊었고, 그래서 HEAD 계수는 **4 에서 멎는다**.
  자기 포함 계수는 *"현재 판"* 에 고정할 수 없다. 검증자는 위 ref 로 고정해 센다.
  ★★ **FIX Iter 15 정정 (R15 Claude P2-2)** — 직전 판은 이 자리에 *"Iter 14 저작 중 산출 명령을
  리터럴로 적자 계수가 즉시 **5** 로 움직였다(firsthand)"* 라고 적었으나, **`5` 는 어느 ref 에서도
  재현되지 않는다**: 본 파일을 건드린 전 리비전을 전수 계수하면 값 공간은 `{1, 2, 3, 4}` 이고 **5 는
  공집합**이다(재현 = 아래 규율의 형식으로 각 리비전에 적용). 사라진 **저작 중간 상태**를 가리키는
  값이라 원리적으로 반증 불가하며, **맨 정수를 금지하는 문단 자신이 맨 정수를 남긴** 형상이었다.
  결론(*자기 포함 계수는 현재 판에 고정할 수 없다*)은 `2 → 4` 이동만으로 지지되므로 **무손상**이다.

  ★★★ **규율 — 자기참조 grep 주장은 immutable ref 없이 적지 않는다
  (설계 FIX Iter 15 신설, R15 Codex F-B / Claude P2-1 수렴)**. 이 문서 안의 문자열을 세는 주장은
  **세는 문장 자신이 대상에 포함**되고 정정이 착지하는 순간 값이 이동하므로, 「현재 판」 기준 정수는
  **저작 시점부터 이미 재현 불가**다. ⇒ 형식 =
  `firsthand @ <ref>: git show <ref>:<이 파일> | grep -c '<토큰>' → <N>`.
  ★ **본 판에서 형제 5건에 적용**(각 ref 는 *정정이 착지하기 직전 판* = 그 주장이 참이던 마지막
  리비전이며, 전건 그 ref 에서 재현 확인):

```
skill                            @ 405abc701 -> 1     (HEAD 5)
adr_number: 999                  @ 759d10c58 -> 1     (HEAD 5)
발행일 :=                         @ 3fb612449 -> 0     (HEAD 4)
adr-admission                    @ 2c002066c -> 0     (HEAD 7)
invariant-check|CONSUMER_ONLY    @ 80352157b -> 0     (HEAD 9)   # grep -cE
# 대조군 (ref-pin 을 이미 보유하던 site — 형태가 옳으면 값이 맞는다)
<그 문자열>                       @ cc598c55f -> 4     (HEAD 4)  일치   # (iv-L) 인용 토큰.
                                                                   # 리터럴로 적으면 자기 포함이 된다 (아래 ★)
mechanical_enforcement_actions   @ bb2778865 -> 3     (문면 3)   일치
#   ★ 이 행만 정의역이 다르다 — 본 파일이 아니라 repo 트리(scripts/ hooks/ .github/ tests/)를 센다.
```

  ★★★ **그리고 그 자기 포함을 본 판이 실물로 다시 겪었다 (firsthand, 재현 가능)** — 위 대조군 첫 행에
  토큰을 **리터럴로** 적자 같은 순간 `grep -c` 가 **4 → 5** 로 움직였고, `<그 문자열>` 플레이스홀더로
  되돌리자 **4 로 복귀**했다. 철회한 *"즉시 5"* 와 달리 이것은 **재현 규칙**이다(리터럴을 넣었다 빼면
  누구나 4↔5 를 왕복시킬 수 있다) — 사라진 저작 중간 상태가 아니다. ⇒ 자기 포함 계수를 다루는
  **유일한 안정 해는 정수 pin 이 아니라 ① ref 고정 ② 플레이스홀더 ③ 줄 선두 앵커** 세 형태뿐이며,
  이 문단은 그 셋을 모두 행사한다. **자기참조 주장을 쓰는 판은 그 주장으로 자기 값을 움직인다** 는 것이
  이 계보에서 **두 번째 재현 가능한 실증**이다(Iter 13 `2 → 4` · 본 판 `4 ↔ 5`).
  ★ 방금 철회한 *"Iter 14 저작 중 5"* 는 재현 불가라 **실증에 세지 않는다** — 철회한 값을 근거로 되쓰지 않는다.

  ★★ **이 열거는 하한이다 — 5건이 전수라는 보장은 없다.** **R15 Codex 심사**의 기계 추출이 본 문서군의
  재현 명령 후보 **상한 171**(거짓양성 포함, 그 심사 자신이 상한으로 declare)을 냈는데 본 저작의
  자기 식별은 **21** 이었다. ⇒ **정본은 위 형식(규율)
  자신이고 이 목록은 그 규율의 관측 사례**다. 목록을 정본으로 두면 한 칸 옆에서 재발한다 —
  이 문서가 `배선` witness 열거와 회피 경로 열거에서 이미 두 번 겪은 형상이다.
  **계수만 보고 "미교체" 로 읽지 말 것** —
...
★★★ **계수 규칙 정본 = (iv-0) 「산출 명령」 펜스의 *기대 열(`GREEN|RED|OUT`) 한정 정규식* 줄
(설계 FIX Iter 14, R14 3자 수렴 P1 — ★ 앵커 형식은 설계 FIX Iter 15, R15 3자 중 2자 수렴 P1)** — 이 2단 명령은 그
정본과 **같은 값을 내야 하며**, 갈리면 그 정규식이 이긴다. 직전 판까지 이 자리의 문자군이
...
두 벌 있으면 하나는 반드시 낡는다** — (iv-0) 「산출 명령」 펜스의 *「행 id 만으로 세면 안 된다」* 주석이
이 함정을 이름으로 경고하고 **바로 아래 줄**이 교정판을
갖고 있었는데도 **경고를 적은 판이 경고 대상 사본을 고치지 않았다.** 그래서 정본을 하나로 못박는다.

★★ **좌표 앵커 금지 규율 (설계 FIX Iter 15 일반화, R15 3자 중 2자 수렴 P1)** — **이 문서 안의 다른
자리를 가리킬 때 줄 번호(`:NNN`)를 쓰지 않는다.** 좌표는 편집마다 밀리므로 포인터가 **조용히** 낡고,
그러면 위 tie-break 조항이 **판정 불가**가 된다(재현자가 정본을 찾아가면 계수 규칙이 아니라 무관한
슬롯 정의를 만나 *"정본이 없다"* 고 결론한다). 직전 판의 `:1403` 이 그 실물이다 — firsthand:

```
# 정본(= 기대 열 한정 정규식)의 실제 줄번호 대 선언된 좌표 :1403
git show 592038552:<이 파일> | grep -n 'GREEN|RED|OUT'   -> 1413   # (A) 저작 시점 — 이미 10줄 어긋남
git show d288c1850:<이 파일> | grep -n 'GREEN|RED|OUT'   -> 1431   # HEAD    — 어긋남 28줄
git show d288c1850:<이 파일> | sed -n '1403p'            -> `eol := 줄끝 바이트` (무관한 슬롯 정의)
```

즉 **저작 시점(`592038552`)에 이미 10줄 어긋나 있었고**(그때도 `:1403` 은 계수 규칙이 아니라 그 위
산문이었다), 같은 Iter 14 의 후속 커밋들이 정본을 **18줄 더 밀어** 어긋남이 **28줄**이 됐다. 즉 **위 문단이 이름 붙인 실패(*"둘이 있으면 하나는
반드시 낡는다"*)를 그 문단이 신설한 기제가 자기 자신에게 재생산**했다. ⇒ 가리킬 것은 **절 라벨 +
그 자리의 리터럴 내용**이며, 새 좌표로 바꾸는 것은 같은 실패를 한 번 더 심는 것이다.
★ **본 판에서 자기 좌표를 전수 치환했다** (형제 site 를 남기면 그것이 다음 라운드 finding 이다):

```
# 자기 좌표 참조 전수 — 백틱으로 감싼 :NNN / :NNN-NNN 꼴 토큰
grep -nE '`:[0-9]{2,4}(-[0-9]{2,4})?`' <이 파일>
#   @ d288c1850 -> 매치 13건 / 10줄. 그 중 **이 파일 자신을 가리키는 것** = 4줄
#     (위 3줄 + frontmatter related_files 주석 지시 1줄). 나머지 9건은 타 파일 좌표
#     (fix-event-v1.md · fix-ledger-sync.yml · evidence-checks-registry.yaml · ADR-070) = 대상 아님.
#   정정 후 -> 지시자로 쓰인 자기 좌표 = 0. 남는 매치 2건은 바로 위 문단의 **이력 인용**
#     (*"직전 판이 :1403 을 가리켰다"*)이지 지시자가 아니다.
#   ★ 이 2건은 본 판이 만든 것이다 — 이력을 적으면 그 이력이 다시 grep 에 잡힌다(자기 포함).
#     그래서 판정 술어는 「매치 수」가 아니라 **「지시자로 쓰였는가」** 여야 한다.
```
...
Iter 13 이 신설한 **2행**(50d · 58b)을 포함해 **전 leg(51종)을 하나씩 다시 껐다**.
★ **FIX Iter 14 정정 — 직전 판은 `52종` / `나머지 48` 이었다 (R14 Codex F-4).** 정본 정의역 =
**(0-c) 표 본문 행수**이며 그 값은 **51**(firsthand — 아래 `산출 명령` 의 awk 1줄. 본 문서는
`"leg N종" ≡ "표 본문 N행"` 규약을 쓴다 — Iter 12 가 *"구 기재 「leg 47종」은 그 시점 표가 이미
48행이라 stale"* 로 이미 이 등식을 못박았다).

★★ **FIX Iter 15 정정 (R15 Codex F-C) — 직전 판의 *"`52` 는 정의역이 다르다"* 를 철회한다.**
직전 판은 심사 하네스가 보고한 `52` 를 *"자기 leg 분해 기준이라 정의역이 다르다"* 로 처분해 **참값의
지위**를 부여했다. 그러나 그 문장은 **산출 명령 없는 단정**이었고, 명령을 붙여 보면 **어떤 일관 규칙도
52 를 내지 않는다**:

```
# ★ 아래 3 계수는 전부 **이 파일 자신을 세는 자기참조**다 — 위 (D) 규율대로 immutable ref 에 고정한다.
#   ref = d288c1850 (R15 심사 대상 판). HEAD 에서도 전건 동일하게 재현된다(51 / 1 / 4).
#   <이 파일> = git show d288c1850:archive/adr/ADR-181-verification-domain-deficit-normative.md
# (a) 규정 연산 1개당 1 leg  =  (0-c) 표 본문 행수
awk '/^\| leg-off \| \*\*67행\*\*/{f=1;getline;getline} f&&/^\|/{n++} f&&!/^\|/{exit} END{print n}' <이 파일>
  -> 51
# (b) 열거된 대체 독법을 전부 leg 으로 셈
#   b-1  셀 안에 (A)/(B) 를 병기한 행 수
awk '/^\| leg-off \| \*\*67행\*\*/{f=1;getline;getline} f&&!/^\|/{exit} f&&/^\|/&&/\(A\)/&&/\(B/{n++} END{print n+0}' <이 파일>
  -> 1        (`PUBDATE` = 실행일 행 단독)
#   b-2  (0-c) 절이 펜스로 열거한 CBODY 독법 수
grep -cE '^ +C[1-9] ' <이 파일>
  -> 4        (C1~C4, 그 중 C2 가 ★ 규정)
#   => 51 + 1 + 3 = 55   (규정 독법까지 따로 재계상하면 57)
# 52 를 내는 규칙 = 없음
```

⇒ `52` 는 **셀 안에 타이핑된 대체 독법만 세고 펜스로 열거된 것은 세지 않은** 값이라, 정의역이 아니라
**대체 독법이 어디에 적혔는가(셀 대 펜스)** 라는 **표기 위치의 부산물**이다. 게다가 본 문서의 Iter 11
규율(*"연산이 둘 이상 성립하면 전부 열거하고 **하나를 규정으로 지정**"*)상 `PUBDATE` 행도 `CBODY` 도
규정을 1개씩 지정했으므로 **둘 다 1 leg** 이고 합은 **51** 이다 — 즉 (b) 는 본 문서의 규약과도 어긋난다.
★ 철회해도 잃는 것이 없다: **51 결론은 무손상**이고, 없어지는 것은 *"52 에도 참인 정의역이 있다"* 는
근거 없는 양보뿐이다. 그 양보를 남기면 다음 판 저자가 *"문서가 정의역을 인정했다"* 를 근거로 51 을 52 로
되돌리는 **ratchet-in 경로**가 열린다(본 문서가 스스로 경고한 형상). 정본 = **51**, 다른 수를 병기하지 않는다.

★ **문서 안 다른 자리의 `52 leg` 표기 처분 (R15 Claude P2-3)** — 행 `58b` 의 신설 근거와 그 서술 사본이
*"검증필 하네스(65/65 · 52 leg 전건 일치)"* 라 적는다. 이는 **그 시점 심사 하네스의 자기보고를 인용한
이력**이지 본 문서의 leg 계수가 아니다. 두 자리는 **같은 문장의 사본**이고 그 중 하나는 (iv) 표 안이라
byte 고정 대상이므로, 한쪽만 라벨하면 **사본이 갈린다**(이 문서가 반복해서 고발한 형상). ⇒ 개별 라벨
대신 **여기서 한 번 정의역을 declare** 한다: **본 문서가 쓰는 leg 계수는 `51` 단일**이며, 인용된 `52` 는
하네스 자기보고값이다.
정본 = 아래 **`67행` 열**. **변화한 leg 은 4 뿐**이며(`PATH_KEYS drop workflow` 3→5 ·
`LADDER_WIRED` 3→4 · `WF_BRANCH` 1→2 · `WIRE_WF` 1→2) 나머지 47 leg 은 65행 값과 **동일**하다 —
...
★ **나머지 62행은 네 슬롯 전부 기본값**이며, 그 사실을 여기 적는 것이 (iv-0) *"산문 서술 금지"* 의 이행이다.
★★ **FIX Iter 14 정정 (R14 Claude P1-2)** — 직전 판은 이 단정 줄이 `60` 이었다. Iter 13 은 3줄 아래
정정 문단(*"표가 67행이 되었으므로 정본은 62"*)과 산출 명령 주석만 갱신하고 **이 단정 줄은 남겼다** —
같은 판이 봉합한 class(*"교체 대상을 이름으로 적어 놓고 한 사본만 교체"*)를 그 봉합과 **같은 판에서
재생산**한 실물이다. 한 수치를 고치면 그 수치의 형제를 전부 찾아 **같은 커밋에서** 고친다.
...
#   ★ 기대 열 앞의 `★` 를 허용하지 않으면 **행 36(`★ **OUT**`) 하나가 조용히 빠져 66** 이 된다
#     (firsthand — 이 계수 규칙 자신이 born-broken 이었고 자기 실행으로 잡았다.
#      ★ FIX Iter 14 정정: 직전 판의 `64` 는 65행 시절 값(65−1)이며 Iter 13 연쇄에서 미갱신이었다)
...
(firsthand @ `759d10c58`: `git show <ref>:<이 파일> | grep -c 'adr_number: 999'` → **1** = 선언 문장뿐, 골격·상수 0건).
...
stub checker := ladder 를 `len(mea) >= 1` 로만 구현 (경로 키·실재·배선 3연언지 전부 삭제)
  ...
  ★ FIX Iter 13 최종 67행 -> rows=67  match=57  mismatch=10     # 신설 58b 가 추가로 탈락 (50d 는 stub 에서도 GREEN)
     탈락 행 = 43 · 44 · 46 · 46b · 48 · 49 · 50c · 52 · 53 · 58b   # ★ FIX Iter 14 — 58b 누락 정정 (계수 10 ↔ 열거 9)
```
...
- ⇒ **`twin 규약 = byte-parity` 는 전건 성립이 아니다.** parity 게이트는 `story-init.yml` 에 **적용된 적이 없고**
  ...
  **0회** 언급했다(firsthand @ `80352157b`: `git show <ref>:<이 파일> | grep -cE 'invariant-check|CONSUMER_ONLY'` = **0**).
...
@ `3fb612449`: `git show <ref>:<이 파일> | grep -c '발행일 :='` → **0**(당시 `발행일` 3 hit 전부 사용처). 그 결과 세 독법이
...
3. **`date:` 는 문서의 사실상 앵커** — frontmatter `date` 는 그 ADR 이 언제 발효했는지의 SSOT 이며
   **실측상 대체로 amendment 로 갱신되지 않는다**(ADR-067 은 amendment 4회 후에도 `date: 2026-05-13`).
   면제가 "그 결정 이후 얼마나 오래 미건설로 남아 있는가" 를 재려면 앵커는 **결정 시점**이어야 한다.
   ★★ **FIX Iter 14 정정 — 직전 판의 *"불변 앵커 … amendment 로 갱신되지 않는다"* 는 단정이었고
   코퍼스가 반례를 보유한다 (R14 Claude P1-3).** firsthand: `ADR-051` 의 `date:` 가
   `2026-05-09`(커밋 `c22f12399`) → `2026-05-13`(커밋 `8f407c8cf`)로 **이동했고**, 그 줄의 주석이
   스스로 `# Amendment 1 — Draft → Accepted 전환 (CFP-506)` 이라 적는다. 즉 근거 3은
   **단일 양성 사례의 일반화**였다 — 술어가 강제하는 성질이 아니라 **관행**이다.
   택일 결론(= `date:` 채택)은 근거 1·2 만으로도 유지되므로 무손상이나, 아래 천장이 따라붙는다.

> ★★★ **천장 (`declared`) — 15번째 회피 경로: 앵커를 앞으로 옮기면 상한 창이 재개된다**
> (설계 FIX Iter 14 신설, R14 Claude P1-3 주입 실증).
> **본 술어는 `date:` 의 불변성을 검사하지 않는다.** 만기를 그대로 둔 채 `date:` 한 줄을 미래로 옮기면
> `발행일 + 180` 상한 창이 **재개**되며, 다음 판에서 또 옮길 수 있으므로 **반복 가능 = 무한**이다
> (주입 실증: 행 40 원형 `RED/over-cap` → `date:` 만 이동 시 `GREEN` → 재이동 시 다시 `GREEN`).
> base 대조도 막지 못한다 — `b5` 는 frontmatter **키 집합**만 보고 **값을 비교하지 않는다.**
> ★ 이것은 위 근거 1이 amendment 독법을 기각한 사유(*"180일씩 나눠 적는 무한"*)와 **같은 무한**이
> 채택 독법에서 **다른 문으로** 열려 있는 것이다. 차이는 "새 amendment 를 적으면 자동 연장" 대
> "`date:` 한 줄을 고치면 연장" 뿐이다.
> **표 내 판별 = 0 · 천장 문면 = 이 문단.** 판별 행을 신설하지 않는 이유 = 코퍼스 도달이 낮고
> (pin 코퍼스에서 `date:` 이동 파일 = **1**), (0-c) 자기적용이 *"코퍼스 출현이 낮으면 문면 고정만"*
> 이기 때문이다. **base↔head `date:` 값 비교는 Phase 2 소관** — carrier `#2985` / 만기 `2026-09-15`.
...
> ★★★ **천장 (`declared`) — 16번째 회피 경로: 「항목 수준」 정량자(ALL/ANY)는 표가 판별하지 않는다**
> (설계 FIX Iter 15 신설, R15 Claude P2-4 주입 실증).
> 행 `58b` 는 **한 항목 안의 경로 키** 축(ALL/ANY)을 닫았다. 그러나 **항목 사이** 축 —
> `mea` 리스트의 *항목들* 을 ALL 로 볼 것인가 ANY 로 볼 것인가 — 은 (iv) 표가 **전혀 행사하지 않는다**:
> **표 67행 중 `K` 항목이 2개 이상인 행 = 0** 이라 — ★ *"항목 1개인 행"* 의 수는 술어에 따라
> **20~23 으로 갈리지만**(무앵커 `- ` 계수 23 · `K` 소유 계수 20 · R15 심사 보고 21),
> **≥2 인 행이 0** 이라는 사실은 시도한 세 술어 **전부에서 불변**이고 판별 0 을 지지하는 것은 후자다 —
> 사다리 행 전건에서 `item=ALL` 과 `item=ANY` 의 뒤집힘 집합이 **완전 동일**하다
> (`key=ALL` 에서 둘 다 {49} · `key=ANY` 에서 둘 다 {49, 58b}). ⇒ **항목 축 판별 = 0.**
> **주입 실증** — `58b` 의 두 경로 키를 **두 항목으로 분할**하면 `item=ALL` 은 `RED/ladder-unwired`,
> `item=ANY` 는 **GREEN** 이다. 즉 **`58b` 가 막은 조합이 그대로 열린다.** 방향은 **fail-open**
> (죽은 선언이 살아있는 선언에 업혀 통과 — 본 Story 표적 그 자체)이고, 기전은 `58b` 근거가 이미
> 지목한 `break`(pick-first)가 **항목 루프에도** 있는 경우다.
> ★ 주입 2×2 산출은 R15 Claude peer 산출이며 **본 저작이 재구현하지 않았다**(술어 5번째 구현 미실행 —
> 아래 정직 목록과 같은 잔여). 아래 코퍼스 실측만이 본 저작 firsthand 다.
> **표 내 판별 = 0 · 천장 문면 = 이 문단. 판별 행을 신설하지 않는다** — 코퍼스 도달이 **0** 이기 때문이며,
> 이는 (0-c) 자기적용의 *"코퍼스 출현이 낮으면 문면 고정만"* 에 해당해 **15번째 경로와 같은 처분**이다.
> 근거는 도달 0 이라는 결과만이 아니라 **왜 0 인지**다 (firsthand @ HEAD · 정의역 `archive/adr/ADR-*.md`):

```
mea 비-빈 파일 48 · 항목 169                  (방법 A census 와 동상)
그 중 항목이 2개 이상인 파일 = 27
★ 그러나 **경로 키를 보유한 항목은 169 중 5 뿐**이고, 그 5가 한 파일에 2개 이상 모이는 경우는
  ADR-072 **1파일**뿐이다(두 항목이 같은 경로쌍 — script_path + workflow_path, 실재·배선 전건 참).
  나머지 파일은 경로 키 보유 항목이 0 또는 1 이라 ALL 과 ANY 가 **정의상 동치**다.
⇒ 항목 정량자가 갈리는 파일 = 0   (사다리 두 단 실재(blob 존재) · 배선(출현) 양쪽에서 확인)
```

> 즉 이 축은 **현 코퍼스에 피해 0** 이지만, *선언이 강제를 못 갖는* 형상이 수용 기준인 (iv) 표 자신에 남는다.
> 항목 축 ALL 은 산문(층 3)에는 규정돼 있으나 **그 층은 수용 기준이 아니다** — 수용 기준에 판별이 0 이라는
> 사실을 여기 적는 것이 `INV-D` 이행이다. **항목 루프 정량자 고정은 Phase 2 소관** —
...
- **자기적용 (실측 기준)**: 본 ADR frontmatter 는 ③ 형식(`carrier=#2985 expiry=2026-09-15
  ...
  append 해 놓고 **ADR 문면에서 한 번도 언급하지 않았다**(firsthand @ `2c002066c`: `git show <ref>:<이 파일> | grep -c 'adr-admission'` → **0**).
...
| (0-c) ablation 표 본문 행 | **51 (무변경)** — 신설 leg **0** | 전 **51** leg 재실행 후 **값이 이동한 leg = 4** (`PATH_KEYS drop workflow` 3→5 · `LADDER_WIRED` 3→4 · `WF_BRANCH` 1→2 · `WIRE_WF` 1→2), 나머지 **47** leg 불변 (★ FIX Iter 14 — 직전 판은 **같은 셀 안에서** `51` 과 `52`/`48` 을 동시에 적었다. 정본 = 표 본문 행수 51 · `4 + 47 = 51`) |
...
같은 파일 frontmatter `related_files` 의 `fix-event-v1.md`·`dev-process-event-v1.md` 두 주석이 이미
실버전을 정확히 적고 있어 **자기모순**이기도 했다(좌표 앵커 금지 규율 — 설계 FIX Iter 15).

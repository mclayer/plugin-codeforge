...
```
  ...
  **유일한 안정 해는 정수 pin 이 아니라 ① ref 고정 ② 플레이스홀더 ③ 줄 선두 앵커** 세 형태뿐이며,
  ...
  이 계보에서 세 번째 실증이다(Iter 13 `2 → 4` · Iter 14 저작 중 · 본 판 `4 ↔ 5`).

  ★★ **이 열거는 하한이다 — 5건이 전수라는 보장은 없다.** 같은 라운드의 기계 추출이 본 문서군의
  재현 명령 후보 **상한 171** 을 냈는데 본 저작의 자기 식별은 **21** 이었다. ⇒ **정본은 위 형식(규율)
...
슬롯 정의를 만나 *"정본이 없다"* 고 결론한다). 직전 판의 `:1403` 이 그 실물이다 — **저작 시점
(`592038552`)에도 계수 규칙 펜스가 아니라 그 위 산문이었고**, 같은 Iter 14 의 후속 6커밋이 **14줄**을
더 밀어 완전 무관한 `eol :=` 슬롯 정의로 이동했다. 즉 **위 문단이 이름 붙인 실패(*"둘이 있으면 하나는
...
> **표 67행 중 `K` 항목이 2개 이상인 행 = 0** 이라(항목 수 분포 = {0항목 44행, 1항목 23행}),

...
| 재현기 | 구성 |
|---|---|
| **R** | (iv) 표의 각 행 바이트를 위 골격에 넣어 파일을 만들고, **경계(b1~b5) → `ADRQ` 정의역 → (iii) leg** 순으로 평가해 `(verdict, exit 사유)` 를 표 기대와 대조한다. 구현 능력 = Python `re`(또는 동등) + YAML 파서 + NFC 정규화. **변주는 전부 플래그로 노출**한다 — `상수 A 삽입 여부` · `평가순서(경계 우선 / 정의역 우선)` · `행37 base 독법(1줄 / 실 frontmatter)` · `행36 기대(GREEN / OUT)` · `leg-off(b2·b5 등)`. **문면에 없는 조각을 메우지 않는 것이 유일한 구성 규칙**이다(Change Plan §8.2 `H-5c`) |
...
```
재현기 R: 상수 A 삽입 · 경계 우선 · 행37 base=1줄 · 행36 기대=OUT

...
- ★★ **수용 기준 축소(B안)가 이 항의 severity 를 올렸다** — 유령 leg 이 `declared` 로 강등된 뒤
  `b4` 는 **frontmatter 밖으로 밀려난 키를 잡는 유일한 축**이다. 그 축의 술어가 미정의면
...
| `b4` — 종단 이후 FM-형 줄 검사 제거 | 33 · **47** | RED → **GREEN** | ✓ 2 |
| `b5` — base 키 보존 미검사 | **42** | RED → **GREEN** | ✓ 1 |
| `N0` 묶음 (b1~b5 동시) | 14 · 20 · 33 · 34 · 35 · 41 · 42 · 47 | ★ **GREEN 화는 33 · 41 · 42 · 47 넷뿐** — 14 · 20 은 `OUT`, 34 · 35 는 `RED/mea-missing` | — |

★★ **직전 판이 틀린 지점** — 묶음 off 로 GREEN 이 되는 것은 **33 · 41 · 42(+ 신설 47)** 이고,
**14 · 20 은 GREEN 이 되지 않으며**(`OUT`) **34 · 35 도 되지 않는다**(`mea-missing`). 즉 표에 적힌
*"7행이 RED → GREEN"* 은 **네 행에 대해 거짓**이었다. `H-6`(*"sub-leg 을 개별로 꺼라"*)이 없었으면
이 거짓은 또 한 라운드 생존했을 것이고, **실제로 `H-6` 을 신설한 그 판이 이 표를 묶음으로 적었다.**

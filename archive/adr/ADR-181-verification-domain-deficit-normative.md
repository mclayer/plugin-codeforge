...
★★ **갈림길이 실측으로 해소됐다 — 층 2 는 load-bearing 이 아니다.** 두 심사자(`ClaudeReviewAgent` ·
`CodexReviewAgent`)가 **서로를 보지 못한 상태에서 각각 본 ADR 텍스트만으로** 판정 술어를 재구현했고,
**둘 다 (iv) 표 전건 재현**에 도달했다. 즉 재현자가 실제로 소비한 것은 **층 1(결정표) + 층 3(정규식 텍스트)**
이며 층 2 는 그 경로에 등장하지 않았다. ⇒ 강등해도 잃는 판별력이 **0** 이다.
...
신설 행 **41**(FM 종단 부재) · **42**(base 키 소실) 은 둘 다 ERE **GREEN**(firsthand — `sh ere.sh`)
...
실제로 두 심사자가 각자 그런 구현을 만들어 **전건 재현**에 도달했으므로, 이 능력 기술은
**단일 산출물에 의존하지 않는다**(그것이 층 2 를 강등할 수 있었던 근거이기도 하다).

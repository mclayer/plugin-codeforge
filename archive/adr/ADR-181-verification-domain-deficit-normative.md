...
**(iv) 표 31행 정의역에서 19행이 뒤집힌다**(★ FIX Iter 7 재실행. Iter 6 판 29행 정의역 = **17행** ·
Iter 5 판 25행 정의역 = **14행**.
...
3. ★★ **주석 본문 문자군 — 불변식은 "개행에 도달하지 못한다" 이고, 표기는 엔진별이다**
   ...
   - ★ **오전사 비용 실측 (firsthand)**: ERE 가지의 `.` 을 `[^\n]` 으로 옮겨 적으면 정상형 주석의
     ...
     **(iv) 표 31행 정의역에서 19행이 뒤집힌다**(★ FIX Iter 7 재실행 — Iter 6 판 29행 = **17행** ·
     Iter 5 판 25행 = **14행**). ★ `CBODY` off 의 의미는 **ERE 문면 오전사**로 (0-c) 에서 못 박았다 —
...
| **주석 본문군** | 캡처 `c` | ★ **`.`** (행 단위 레코드 = 개행 도달 불가) | ★ **`[^\n]`** (+ `DOTALL` 금지) | 입력 `anb⏎axb`: ERE `a.b` -> **2** ⁄ ERE `a[^\n]b` -> **1** ⁄ Py `a[^\n]b` -> **2** | ★★ 서로 옮겨 적으면 **17행 뒤집힘 · 행 1 born-red** | **19행** (행 1 포함 — 31행 정의역. 29행 정의역 재산출 = **17**, FIX Iter 7 L3-ⓒ 재실행) |

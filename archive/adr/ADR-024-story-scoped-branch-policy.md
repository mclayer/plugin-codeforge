...
amendments:
  ...
  - by: "CFP-2469"
    date: "2026-06-30"
    scope: "Amendment 20 — Amendment 2 §결정 C step 2 (gh api PUT branch protection) 수동→operator-token 자동 전환 명문화 (Epic CFP-2468 Track W/W1, paired ADR-132 carrier). consumer dead-gate(게이트 workflow 존재하나 required_status_checks 미등록 = merge 차단력 0) 출고 차단 — bootstrap stage + reapply 가 operator gh auth 토큰으로 GET-merge-PUT idempotent 배선 (신규 scripts/wire-branch-protection.{sh,ps1} SSOT 호출). **§결정 A core contexts 삭제 불허 invariant 무손상** — GET-merge-PUT 의 append-only 가 자동 준수 (setup-branch-protection.sh _validate_core4 동형). **권한 invariant**: 자동화는 operator gh auth 만 사용 (ADR-066 §결정 2 PAT 6-scope 무손상 — Administration:write 미요구, codeforge PAT 미사용). operator 권한 부족(403) = WARN graceful degrade (hard-fail 아님, ADR-027 §결정 2 warning-inject-only + drift-preview fallback). **§결정 B drift-check.yml 대체 명문화**: §결정 B 가 선언한 templates/github-workflows/branch-protection-drift-check.yml 은 never-built (firsthand 2026-06-29 KST: templates + .github/workflows 양쪽 0건) → ADR-132 §결정 8 readiness check (check_bootstrap.py branch protection 점검, WARN tier) 가 drift 재감지 safety-net 대체. 죽은참조 3종(부재 workflow + 부재 check-branch-protection-drift.sh + orphan scripts/lib/workflow_branch_protection_drift_check.py) 정리 = Phase 2 코드 델타 (본 amendment = 대체 결정 명문화). FORM(b) setup-branch-protection.sh 무손상 (write 로직 = 신규 wire-* 분리, SoD). additive only — Phase 1 = ADR amendment + change-plan SSOT, Phase 2 = wire-*/reapply 스크립트 + bootstrap stage + readiness check + dead-ref cleanup. ratchet 강화 방향 (강제력 추가 — dead-gate 출고 차단, weakening 0). ADR-082 §결정 9 verify-at-write-time: worktree base commit 4482b798 / `git show origin/main:archive/adr/ADR-024-...md | grep '^## Amendment'` actual max body Amendment = 19 (CFP-2374, 2026-06-20) → next slot = 20 직접 verify 후 write (frontmatter `by:` array 는 chronological out-of-order = stale, body `## Amendment N` header 가 authoritative slot source — linter slot-collision 교정 반영)."
    sunset_justification: "N/A — is_transitional: false (permanent governance policy). Amendment 20 = Amendment 2 §결정 C step 2 수동→자동 전환 (강제력 추가, dead-gate 출고 차단) — operator gh auth 경유라 ADR-066 §결정 2 PAT scope invariant 무손상 + §결정 A core-contexts-삭제-불허 invariant 무손상. ratchet-UP 강화 방향 (consumer merge 차단력 충전, 약화 영역 0건 — codeforge PAT scope 미확장 / core contexts 미삭제 / enforce_admins 우회 미정상화). ADR-058 §결정 5 + ADR-064 §self-application top-down ratchet 정합. paired ADR-132 (메커니즘 SSOT) + ADR-027 Amendment 13 (mechanical/advisory layer 구분)."
related_files:
  - CLAUDE.md
...
mechanical_enforcement_actions:
  ...
  # ADR-072 §결정 5 의 2 evidence-checks-registry entry (production-cutover-deputy-spawn-evidence +
...
- warning tier 첫 도입 (ADR-060 §결정 5 정합 — 모든 신규 entry 는 warning 시작 강제). blocking-on-merge 격상 = empirical evidence 누적 후 별도 CFP carrier 영역 (ADR-060 승격 gate AND condition 통과 의무).
...
- **per-plugin scope 누적 카운터** (단일 plugin 5 entry 각 1회 = 5회 분산이지만 근본은 동일 plugin 의 체계적 회피) — 별도 CFP carrier
- **blocking-on-merge tier escalation** — 별도 CFP, ADR-060 승격 gate AND condition 통과 후
- **bypass narrative audit trail mechanical enforce** (`[bypass-justification]` PR 코멘트 marker) — 별도 CFP
- **cross-repo bypass counter extension** (codeforge-internal-docs / marketplace) — 별도 CFP, EC-1 정합
- **carrier Issue 코멘트 append** (이미 발의된 Issue 에 추가 PR 정보 append) — 별도 CFP
...
- warning tier 첫 도입 (ADR-060 §결정 5 정합 — 모든 신규 entry 는 warning 시작 강제). blocking-on-pr 격상 = empirical evidence 누적 후 별도 CFP carrier 영역.
...
**§결정 6.A.2 와 disjoint invariant**: 동일 PR 가 §결정 6.A.2 (per-entry) + §결정 6.A.3 (per-plugin) 양 trigger 시 양 carrier Issue 각 발의 (signature aggregation 금지, 각 carrier 가 별도 evaluation 영역).
...
- warning tier 첫 도입 (ADR-060 §결정 5 정합 — 모든 신규 entry 는 warning 시작 강제). blocking-on-merge 격상 = empirical evidence 누적 후 별도 CFP carrier 영역 (ADR-060 승격 gate AND condition 통과 의무, Story-2 #861).
...
- warning tier 첫 도입 (ADR-060 §결정 5 정합 — 모든 신규 entry 는 warning 시작 강제). blocking-on-pr 격상 = empirical evidence 누적 후 별도 CFP carrier 영역 (ADR-060 승격 gate AND condition 통과 의무).
...
| 신규 entry | family position | 의미 |
|---|---|---|
| `hotfix-bypass:prod-cutover-deputy-evidence` | **45번째 (raw)** (raw active concrete grep count: 44 + 1 new = 45) | ADR-072 §결정 5 evidence-checks-registry 2 entry (`production-cutover-deputy-spawn-evidence` + `epic-cutover-gate-evidence-quad-check`) warning-tier bypass channel. Wave 4 sub-Epic #882 Story-3 ProductionEvidenceDeputy mandate first activation declare layer 의 mechanical lint conditional skip 채널. registry-side late codify — gh-side 는 CFP-954 PR-time 이미 등록 (1 hit verified 2026-05-19 KST). |
...
**self-meta loop 회피**: 본 lint workflow / script 자체의 PR 에 `hotfix-bypass:prod-cutover-deputy-evidence` 부착 시 본 lint step skip. **carrier_story self-exempt**: 본 entry 도입 carrier (CFP-1000) Story file 자체의 Phase 1 PR 은 carrier 영역 bootstrap-exempt 정합 (ADR-062 §결정 8 self-application precedent / CFP-722 §결정 27 carrier-Story exemption 동형). ADR-072 §결정 3 D3 consensus wrapper-self-app exemption 2-tier (Tier-1 declare-time + Tier-2 runtime) 와 disjoint scope — exemption ≠ bypass channel (exemption 외 영역에서만 bypass 발효).
...
bidirectional drift 잔여 34 entries (gh→registry 5 + registry→gh 29) reconcile sweep = 별도 CFP #1004 (post-merge open) 이관. ADR-064 §결정 1 (CFP scope unitary) 정합 — 한 CFP 안 "Tier-A 경량 → Tier-B full" 단계 채택 금지. 별도 CFP 분리 (CFP-1000 Tier-A + CFP-1004 Tier-B) 가 independent brainstorm + independent Story + independent PR invariant 보존.

추가 OOS 영역 (CFP #1004 또는 별도 CFP carrier):
...
- warning tier 첫 도입 (ADR-060 §결정 5 정합 — 모든 신규 entry 는 warning 시작 강제). 본 entry = ADR-072 §결정 5 `production-cutover-deputy-spawn-evidence` + `epic-cutover-gate-evidence-quad-check` 2 entry warning-tier 동반. blocking-on-pr 격상 = empirical evidence 누적 후 별도 CFP carrier 영역 (ADR-060 승격 gate AND condition 통과 의무).
...
- ADR-072 §결정 1 + §결정 5 (CFP-954 carrier — ProductionEvidenceDeputy mandate first activation declare layer + Epic close-time quad-check evidence aggregation)
...
CFP-1000 (Tier-A) 가 1 registered-but-not-declared entry (`prod-cutover-deputy-evidence`) registry-side late codify 으로 Tier-A 영역 close — Tier-B 영역 (35 registry→gh missing + 5 gh→registry missing + 1 naming mismatch = 41 raw drift, Issue body CFP-1000 retro 2026-05-19 morning verification 값) 은 별도 CFP carrier 분리 (ADR-064 §결정 1 CFP scope unitary 정합 + CFP-963 retro escalation candidate). 본 Amendment 11 = CFP-1006 carrier 영역 Tier-B Wave 1 (gh→registry missing 5 entries + 1 naming mismatch).
...
2. **`hotfix-bypass:epic-cutover-quad-check`** (registered-but-not-declared): gh repo pre-existing + `docs/evidence-checks-registry.yaml:1441` + `ADR-072:36` provenance. registry §3 yaml 미선언. Operational scope = production-touching Epic-cutover PR 4-tuple evidence quad (live_touching / production_cutover_touching / marketplace_publish_touching / consumer_impact_blast_radius) check skip. Backing carrier = ADR-072 §결정 1/§결정 5 (Wave 4 sub-Epic #882 Story-3, CFP-954). Workflow = `production-cutover-evidence.yml`. wrapper-self-app Tier-1 declare-time 면제 invariant 보존.
...
`CODEFORGE_CROSS_REPO_PAT` 가 `Issues: write` on `mclayer/plugin-codeforge` 획득하기 전까지 (ADR-066, 사용자 secret-domain) CI mechanism 은 future registry→gh drift self-heal 불가. 본 Amendment 12 backfill = one-time manual reconcile. Wave 3 sync-drift lint (별도 CFP, ADR-064 §결정 1 unitary) 는 PAT remediate 전까지 detection-only. 문서화된 accepted residual — 본 Story blocker 아님.
...
- ADR-064 §결정 1 (CFP scope unitary — Wave 3 sync-drift lint 별도 CFP)
...
- ADR-058 §결정 5 sunset_justification (ratchet-up 강화 방향)
- ADR-064 §결정 5 CFP scope unitary (per-plugin-cumulative-counter 단일 entry 승격, sibling sub-decisions 6.A.4/6.A.5 별도 CFP)
...
#### Wave 3 (post Phase 2 PR merge 별도 task — memory housekeeping)
...
따라서 본 Amendment 18 = mechanical 신설 declarative codify (existing-reuse) — 별도 lint / workflow / script 신설 0건. ADR-076 / ADR-082 / ADR-086 precedent (`mechanical_enforcement_actions: []` declaration-only Wave 1) 와 disjoint — 본 entry 는 `existing-reuse` (이미 wired) status.

#### Wave 2 (Phase 2 PR scope — none, 별 carrier 0)

본 §결정 6.A.10 = phase-gate-mergeable.yml inline OR-gate 분기 (이미 wired) — 별도 lint / sentinel / evidence-check entry 신설 영역 외. Wave 2 carrier 0 (mechanical layer 완결).
...
- Phase 2 carrier 0 — mechanical layer 는 Story-1 + Story-2 에서 이미 완결 (existing-reuse status, 별도 lint / workflow / script 신설 0건)
...
- **ADR-026 Amendment 5 (CFP-900)**: 5번째 fast-pass source `.github/` content sanity orthogonal warning layer — fast-pass OR-gate 확장 sibling carrier (별도 axis, orthogonal warning vs additive source)
...
## Amendment 20 — Amendment 2 §결정 C step 2 수동→operator-token 자동 전환 (consumer dead-gate 출고 차단, CFP-2469)

> Epic CFP-2468 (codeforge 강제력·검증 균질성 복구) Track W/W1. Paired carrier = **ADR-132** (consumer branch-protection 자동 wire 메커니즘 SSOT) + **ADR-027 Amendment 13** (mechanical/advisory layer 구분). 본 Amendment = ADR-024 branch policy 측 연결 (step 2 수동→자동 명문화 + §결정 B drift-check.yml 대체).

### 컨텍스트

Amendment 2 §결정 C 의 운영 규칙은 step 1 (manifest 합성 + drift preview = `setup-branch-protection.sh` FORM b, 자동화됨) + **step 2 (`gh api -X PUT .../protection` = 현재 수동 operator)** 로 구성된다. consumer 16 repo 중 15개가 step 2 미수행 → 게이트 workflow 가 PR 마다 돌지만 `required_status_checks.contexts[]` 미등록 = **merge 차단력 0 (dead gate)**. step 2 를 수동으로 둔 그 설계가 강제력 부재(dead-gate 출고)를 낳았다.

### 결정 A — step 2 수동→자동 전환 (operator gh auth, ADR-066 무손상)

step 2 (branch protection PUT) 를 **자동 배선** 으로 전환한다. bootstrap stage + reapply 가 신규 `scripts/wire-branch-protection.{sh,ps1}` SSOT 를 **operator gh auth 토큰** 으로 호출 (GET-merge-PUT idempotent). 메커니즘 본체 SSOT = ADR-132.

- **권한 invariant 무손상**: 자동화는 operator 가 bootstrap/reapply 를 실행하는 그 셸의 `gh auth` 토큰만 사용 — codeforge PAT (`CODEFORGE_CROSS_REPO_PAT`) 미사용/미변경. ADR-066 §결정 2 PAT **6-scope** (Administration:write 부재) 무손상. operator 권한 부족(403) = WARN graceful degrade (hard-fail 아님 — ADR-027 §결정 2 warning-inject-only + drift-preview fallback).
- **§결정 A core contexts 삭제 불허 invariant 무손상**: GET-merge-PUT 의 append-only 가 자동 준수 (`setup-branch-protection.sh` `_validate_core4` 동형 — core contexts 는 union 보존, 삭제 0).
- **FORM(b) `setup-branch-protection.sh` 무손상**: write 로직은 신규 wire-* 에 분리 (SoD — preview ↔ write 분리, FORM(b) 의 ZERO-write 보장 보존).

### 결정 B — §결정 B drift-check.yml 대체 (never-built 죽은참조 정리)

Amendment 2 §결정 B 가 선언한 `templates/github-workflows/branch-protection-drift-check.yml` 은 **never-built** (firsthand 2026-06-29 KST: templates + `.github/workflows` 양쪽 0건). related_files L98 + 본 §결정 B + manifest L6 + `setup-branch-protection.sh` L301 + consumer-guide L1859 가 SSOT 로 인용하나 실파일 부재 = **죽은참조**. 동반 orphan = `scripts/lib/workflow_branch_protection_drift_check.py` (부재 workflow 의 run-block helper, 어떤 `.yml`/`.sh` 도 참조 안 함).

**대체 결정**: ADR-132 §결정 8 readiness check (`check_bootstrap.py` 의 branch protection 점검, WARN tier) 가 drift 재감지 safety-net 을 *대체* 한다. never-built weekly-cron 실체화는 불요 (기능 중복). 죽은참조 3종 정리 = Phase 2 코드 델타 (본 Amendment = 대체 결정 명문화 + 죽은참조 식별).

### Sunset justification

강화 방향 ratchet (step 2 수동→자동 = 강제력 추가, dead-gate 출고 차단 / §결정 B never-built 죽은참조 정리 = readiness check 대체). operator gh auth 경유라 ADR-066 §결정 2 PAT scope 무손상 + §결정 A core-contexts-삭제-불허 무손상. 약화 0 (codeforge PAT scope 미확장 / core contexts 미삭제 / enforce_admins 우회 미정상화). ADR-058 §결정 5 비대상 (강화 방향). paired SSOT = ADR-132 (메커니즘) + ADR-027 Amendment 13 (layer 구분).

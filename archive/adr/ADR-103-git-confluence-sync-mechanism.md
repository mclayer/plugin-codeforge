...
- `scripts/confluence_forward_sync.py` — Amendment 3 (§결정 7) forward 동기화 스캐폴드 (ADR-061 외부 .py, --build-manifest / 기본 sync 모드, secret 부재 dry-run)
- `.github/workflows/confluence-forward-sync.yml` — Amendment 3 (§결정 7-B) forward CI workflow (on push main, paths docs/** + archive/adr/**, required check 아님, secret 미설정 dry-run)

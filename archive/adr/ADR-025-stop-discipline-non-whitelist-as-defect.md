...
- **정의역**: fence 등재 named form id 에 한한다. 표의 **무기명 예시 행**(named form id 를 갖지 않는 행 — §결정 7 표의 선행 예시 행들)은 fence 정의역 밖이므로 본 요건 대상이 아니다(정의역 명시 축소 + 근거 = §A4-8 B-2 (b) 경로). **보존의 정도 = 판정·분류 무변경까지이며 byte 무접촉이 아니다** — `"Phase 1 완료, Phase 2 시작할까요?"` 행에 `over-ask` cross-ref 1문장이 additive 로 덧붙었다(본 요건을 그 행에 적용한 것이 아니라, 기명 form 으로의 포인터만 부여). 종전 이 자리의 `무접촉 보존` 표기는 그 additive 편집과 어긋났으므로 정정한다(설계리뷰 iter3 X-2).

...
amendment_log:
  ...
  - amendment_id: 4
    ...
    scope: "CFP-2944 carrier — §결정 7 illegal-stop 표 행 추가 + **form-set fence 신설**(illegal-stop named form inventory v1 = over-halt · over-ask · vague-pause · status-report-then-halt · limit-signal-halt 5 id, 기계 SSOT). 행 추가 = 신규 명명 2종(status-report-then-halt = 정보성 보고 편승 정지 / limit-signal-halt = 한도류 신호 발 의지적 정지) + 기왕 명명돼 있었으나 자기 행이 없던 over-halt · over-ask 의 **자기 행 부여**(신규 금지 창설 0 — 양자는 §결정 1/§결정 2/§결정 5 이래 이미 whitelist 밖 defect, 가시화만). 완료 기준은 행수·서수가 아니라 술어 — **fence 등재 전 id 가 각각 자기 행 + discriminant 3항 + tier 라벨 보유**(§A4-2 행 단위 구조 요건). 본 amendment 가 저작한 각 신규 행은 discriminant 3항을 primary 로 명기하고 예시는 보조(비망라), negative control 동반 — 정의역 = 본 amendment 저작 행 한정이며 Story CFP-2944 검사 술어 D3 과 동일 정의역이다(기왕 착지한 vague-pause 행은 negative control 이 부재하나 Amendment 3 저작물이라 본 술어 정의역 밖 — 종전 이 자리의 '각 행은' 전칭은 fence 등재 vague-pause 행에서 거짓이었고 자기 acceptance 술어보다도 넓었으므로 정정, 설계리뷰 iter3 X-3). §결정 10 = **기존 분류 재사용, 신규 enum 0** — status-report-then-halt → 기존 subclass policy_violation_vague_pause / limit-signal-halt → stop-event-v1 기존 reason_class policy_violation_rate_limit_induced. §결정 6 whitelist 5종 **무변경**(세션 한도 미등재 유지 = 합법화 방지). consumer mirror(docs/consumer-guide.md §7.1) 선행 drift(vague-pause 행 부재 실측) 동반 해소 — §결정 9 가 선언한 mirror 의무의 이행. 강화(ratchet↑) 방향(수치는 본 amendment base = 착지 시점 origin/main 대비 델타의 확정 기록이며 현재시제 행수 assert 가 아니다 — §A4-5 자기참조 절대수치 금지 정합): base 6행 중 4행은 byte 동일, 2행은 순수 additive 주석 수정('Phase 1 완료, Phase 2 시작할까요?' 행 = over-ask cross-ref 1문장 추가 / vague-pause 행 = 본문의 over-halt 에 backtick anchor 표기 부여) — 양 행 모두 판정·분류·defect 사유 문언 무변경이라 금지 범위 축소 0(ratchet 방향 보존). 4 subclass·whitelist 5종 무변경, illegal 표는 행 추가만. 종전 이 자리의 '기존 6행 … 무변경' 표기는 위 2행 수정과 어긋났으므로 정정한다(설계리뷰 iter3 X-2 — 실측: base 6행 대비 byte 동일 4). 아울러 over-ask 행의 ask-trigger 3종 인용을 ADR-071 §20.1 표 데이터 3행의 셀 문언 verbatim 으로 교체한다 — 종전 인용은 같은 3종의 압축 렌더였고 출처를 §결정 20 본문에 귀속했으나, 그 문자열의 실 소재는 §결정 20 본문(표 형식, 해당 문자열 0-hit)이 아니라 ADR-071 frontmatter amendment_log scope 필드였다(C6 full-block 인용 규율 위반, 설계리뷰 iter3 X-1)."
    status: proposed
...
| "세션 한도라 여기서 멈추겠습니다" 류 한도류 신호 발 정지 (limit-signal-halt — Amendment 4, CFP-2944) | **한도류 신호를 근거로 한 의지적 정지** — vague-pause 와 동일 축(A2). discriminant 3항: **잔여작업 有 ∧ 결정 payload = 0 ∧ volitional 발화**(한도류 신호는 정지의 *계기*일 뿐 payload 가 아니다 — 신호를 인용해도 사용자에게 요구하는 결정은 0). 대상 = 선제적 정지 제안("리셋 후 재개하시죠") · 자식 리밋을 Story 종료로 오처리 · 제어 회복 후 재확인 질문·무발화 정지. 신호 판정 = ADR-109 Amendment 2 판별식 D(`D-out-1`·`D-out-2`), 6-literal 열거는 비망라 fast-path. 세션 한도는 §결정 6 whitelist 5종에 **미등재이며 등재하지 않는다**(합법화 방지). negative control(정당 구분선): `D-out-2` 의 **1회 통지**(whitelist #1 User environment 변경 의무 — 통지는 정당, "통지 후 무기한 대기"만 부당) / 통지 후 이어갈 가용 잔여작업이 실제로 0(whitelist #1 정지 성립) / 세션이 **실제로 사망**한 비의지적 종료(축 C — ADR-110 · ADR-071 §결정 24 복구 경로). `[advisory]` — runtime hard-deny 불가(한도 순간 토큰 발화 불가 구간은 정의역 밖) |
| 잔여작업 有인데 **무발화**로 정지 (over-halt — Amendment 4 에서 자기 행 부여, CFP-2944) | **decision-null pause (silent form)** — discriminant 3항: **잔여작업 有 ∧ 결정 payload = 0 ∧ volitional**. 3항째는 본 form 이 무발화라 발화 유무로 평가할 수 없고 §A4-3 판별자(**"그 시점에 다음 tool call 을 할 수 있었는가" — 할 수 있었는데 안 함 = 축 A**)로 평가한다. 이는 [ADR-144](ADR-144-orchestrator-autonomy-stop-taxonomy.md) §결정 2 가 over-halt 를 **silent form = 축 A2** 로 배정한 것의 표층 적용이며 taxonomy 개정 0 이다(§결정 1 배정규칙의 첫 분기는 발화 유무를 키로 쓰므로 무발화 form 에 그대로 적용되지 않는다 — 그 간극을 §A4-3 판별자가 메운다. ADR-144 개정 필요 여부는 본 amendment 범위 밖 · 관찰 기록). negative control(정당 구분선): 다음 tool call 이 **불가능**했던 정지 = 축 B(비의지적 stall — force-resume) 또는 축 C(세션 사망 — 복구 경로) / whitelist 5종 발동 정지 / 작업 단위 완료 후 종료(whitelist #5). **신규 금지 창설 0** — over-halt 는 §결정 2 이래 이미 whitelist 밖 defect 였고 본 행은 그 사실에 자기 행·tier 를 부여해 가시화할 뿐이다(§A4-4 동형). `[advisory]` — 무발화 turn-end 라 runtime hard-deny 불가 |
| "다음 진행할까요?" 류 확인 질문 (over-ask — Amendment 4 에서 자기 행 부여, CFP-2944) | **확인-요구 정지 (축 A1 — payload > 0)** — discriminant 3항: **잔여작업 有 ∧ 사용자 확인·승인을 요구하는 결정 payload > 0 ∧ ask-trigger 3종 미해당**. ask-trigger 3종 = **① 요구 애매 = 사용자 요구 자체에 애매함이 있어 진행 방향이 안 잡힘 — 명확화 필요 ② 진짜 가치 trade-off = 확실히 짚어야 하는 가치 결정 — default 비자명 + 사용자 선호가 결과를 가름 (제품 방향·우선순위 등) ③ 비가역·고비용 = 중대 결함 대응 / 대거 삭제 / rollback / 배포·외부 발송**([ADR-071](ADR-071-orchestrator-user-dialog-convergence.md) **§20.1 ask-trigger 3종 표의 데이터 3행 — 셀 문언 verbatim**. 원문이 표 형식이라 표 마크업만 제거해 인라인 직렬화했다: 열 구분자 = `　=　`, 행 구분자 = `①②③` 마커. 문언 자체는 무손실·무압축이며 생략 표기 0. 본 amendment 는 이 enum 에 member 를 추가하지도 빼지도 않는다). A2 3 form 과 달리 payload 부호가 반대(> 0)이므로 정당/부당을 가르는 것은 **3항째**다([ADR-144](ADR-144-orchestrator-autonomy-stop-taxonomy.md) §결정 1 축 A1 = "ask-trigger 정당 + §결정 7 illegal 부당·기명"). 본 표의 확인-질문 예시 행들은 동 form 의 변형이다(**판정·분류 무변경** — 단 byte 무접촉은 아니다: `"Phase 1 완료, Phase 2 시작할까요?"` 행에 본 form 을 가리키는 cross-ref 1문장을 additive 로 덧붙였고, 나머지 확인-질문 예시 행은 byte 동일. 덧붙인 문장은 주석이며 defect 사유 문언·금지 범위를 바꾸지 않는다). negative control(정당 구분선): ask-trigger 3종 해당 발화(payload > 0 ∧ 정당) / decider escalation(whitelist #4) / User environment 변경 1회 통지(whitelist #1) / 작업 단위 완료보고(whitelist #5, terminal). **신규 금지 창설 0** — over-ask 는 §결정 1·§결정 5 이래 이미 defect 분류였고 본 행은 가시화만. `[advisory]` — plain-text turn-end 라 runtime hard-deny 불가 |
...
**행 단위 구조 요건 (존재 검사와 짝 — 운용정의)**: fence 등재 **각** id 는 §결정 7 표에 **자기 행**을 가지며, 그 행 **안에** ① discriminant 3항 ② tier 라벨 을 함께 둔다. 축별 3항 = **축 A2**: 잔여작업 有 ∧ 결정 payload = 0 ∧ volitional / **축 A1**: 잔여작업 有 ∧ payload > 0 ∧ ask-trigger 3종 미해당.

- **동거 상속을 충족으로 보지 않는다** — 다른 form 의 행에 얹혀 tier·discriminant 를 상속하는 형태는 행 단위 평가가 성립하지 않는다. 본 amendment 초안이 정확히 그 상태였다: `over-halt` 은 vague-pause 행에 동거했고 `over-ask` 은 무기명 예시 행의 cross-ref 로만 존재해, **존재 검사는 GREEN 인데 구조 검사가 RED** 로 착지할 뻔했다(설계리뷰 iter2 F-A).
- **축 참조는 3항을 대신하지 않는다** — "vague-pause 와 동일 축(A2)" 류 참조는 사람 독해에 충분하나 행 단위 평가에서는 미달이다. 본 FIX 직전 `limit-signal-halt` 행이 그 형태였고, 3항을 문자로 요구하느냐 축 참조를 인정하느냐에 따라 같은 행의 판정이 갈렸다 — 술어를 운용정의 없이 두면 verdict 가 독자마다 달라진다. 본 항이 그 운용정의다.
- **정의역**: fence 등재 named form id 에 한한다. 표의 **무기명 예시 행**(named form id 를 갖지 않는 행 — §결정 7 표의 선행 예시 행들)은 fence 정의역 밖이므로 본 요건 대상이 아니다(정의역 명시 축소 + 근거 = §A4-8 B-2 (b) 경로). **보존의 정도 = 판정·분류 무변경까지이며 byte 무접촉이 아니다** — `"Phase 1 완료, Phase 2 시작할까요?"` 행에 `over-ask` cross-ref 1문장이 additive 로 덧붙었다(본 요건을 그 행에 적용한 것이 아니라, 기명 form 으로의 포인터만 부여).
...
**실현 사례 (예고 → 발현, firsthand)**: B-2 의 첫 dry-run 은 **존재 축**(§A4-2 검사 방향 ①) 술어를 인벤토리에 넣고 평가했으나 **구조 축**(§A4-2 행 단위 구조 요건) 술어는 넣지 않았고, 그 결과 `over-ask` 의 RED 를 놓친 채 착지할 뻔했다 — 잡아낸 것은 dry-run 이 아니라 설계리뷰다. 인벤토리에서 빠진 술어는 §7.16 표에 행이 없고 **"빠졌다"는 사실 자체도 어느 표면에도 남지 않는다**(Story `CFP-2944` Z-12 가 원리적 미해소로 예고한 한계의 실물 발현). 따라서 B-2 의 실효는 **인벤토리 완전성에 상한이 걸리며 그 완전성을 본 절이 보장하지 않는다** — B-2 는 평가한 술어의 결과를 표면화할 뿐 평가 대상 선정의 누락은 표면화하지 못한다.

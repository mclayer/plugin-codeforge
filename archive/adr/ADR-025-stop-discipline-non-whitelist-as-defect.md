...
| "다음 진행할까요?" 류 확인 질문 (over-ask — Amendment 4 에서 자기 행 부여, CFP-2944) | **확인-요구 정지 (축 A1 — payload > 0)** — discriminant 3항: **잔여작업 有 ∧ 사용자 확인·승인을 요구하는 결정 payload > 0 ∧ ask-trigger 3종 미해당**. ask-trigger 3종 = **① 사용자 요구 자체 애매 = 명확화 필요 / ② 확실히 짚어야 하는 진짜 가치 trade-off = default 비자명 + 사용자 선호가 결과를 가름 / ③ 비가역·고비용 = 중대 결함 대응·대거 삭제·rollback·배포·외부 발송**([ADR-071](ADR-071-orchestrator-user-dialog-convergence.md) §결정 20 verbatim — 본 amendment 는 이 enum 에 member 를 추가하지도 빼지도 않는다). A2 3 form 과 달리 payload 부호가 반대(> 0)이므로 정당/부당을 가르는 것은 **3항째**다([ADR-144](ADR-144-orchestrator-autonomy-stop-taxonomy.md) §결정 1 축 A1 = "ask-trigger 정당 + §결정 7 illegal 부당·기명"). 본 표의 확인-질문 예시 행들(무접촉 보존)은 동 form 의 변형이다. negative control(정당 구분선): ask-trigger 3종 해당 발화(payload > 0 ∧ 정당) / decider escalation(whitelist #4) / User environment 변경 1회 통지(whitelist #1) / 작업 단위 완료보고(whitelist #5, terminal). **신규 금지 창설 0** — over-ask 는 §결정 1·§결정 5 이래 이미 defect 분류였고 본 행은 가시화만. `[advisory]` — plain-text turn-end 라 runtime hard-deny 불가 |
...
- **정의역**: fence 등재 named form id 에 한한다. 표의 **무기명 예시 행**(named form id 를 갖지 않는 행 — §결정 7 표의 선행 예시 행들)은 fence 정의역 밖이므로 본 요건 대상이 아니며 무접촉 보존한다(정의역 명시 축소 + 근거 = §A4-8 B-2 (b) 경로).

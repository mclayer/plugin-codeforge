...
amendments:
  ...
  - amendment_id: 13
    ...
    summary: |
      ...
      §결정 22 = §결정 15(over-halt = 3-touchpoint 미해당 발화 정지) + §결정 20(over-ask = ask-trigger 미해당 확인 질문) 를 전환 지점에 적용 강화(매핑, 재정의 아님). §결정 18(session-swap reflex) 와 disjoint 축 — cross-ref only, session-swap 재codify 0. **closed-enum 무손상 명제(핵심)**: 본 §결정은 §결정 15 3-touchpoint / §결정 20 ask-trigger 3종 enum 에 member 를 추가하지 않는다. "Story 전환 진행 확인" 은 어느 enum 에도 미등재(= 발화 정당 사유 아님)이며, 본 §결정은 그 비-등재 상태를 전환 지점에서 surface + 예방강제할 뿐 → §15.5/§16.6 closed-enum 확장 규약 미발동(touchpoint 추가 0, trigger 추가 0, 사용자 burden 증가 0). D2 = UserPromptSubmit 예방 reminder(ADR-115 hook-frame 재사용) + Stop force-continue REJECT(ADR-115 §결정 2 block 금지 #10412/#55754 + 정당/부당 정지 hook 구분 불가 → over-suppression = active harm). ASM-2 platform-limit 재확인 — runtime 발화 前 hard-block 불가(PreToolUse(AskUserQuestion) 미지원 anthropics/claude-code#15872 CLOSED/NOT_PLANNED, 무발화 over-halt = 어떤 hook 도 미감지 = AC-3 documented blind-spot). D2 = warning-tier 예방/surface only, hollow-detection gate 아님.
...
related_stories:
  ...
  - CFP-2567 # Amendment 13 carrier (§결정 22 신설 — Epic 내 Story 전환 자율 진행, 전환 지점 over-halt/over-ask 방지. D1 doc 국소 명시 + D2 UserPromptSubmit 예방 reminder hook(skip-offer 동형), §결정 15/20 전환 지점 적용 강화 = enum member 추가 0, §결정 18 disjoint cross-ref, third strengthen-direction, Stop force-continue REJECT ADR-115 §결정 2)
related_adrs:
...
- mechanical lint (over-asking 자동 검출) = platform inherent limit (turn-final hook 부재, runtime 발화 시점 block 불가) — behavioral directive only, 별 follow-up CFP scope.
...
- **§결정 18 (session-swap reflex)** — **disjoint 축, cross-ref only, 재codify 금지**. session-swap("context 가득 → 별 세션")은 §결정 18 anti-pattern 7종이 이미 codify. 전환 경계에서 co-occur(§18.4 incident 처럼)하나 별 축 — 본 §결정은 over-halt/over-ask 절반만 신규 커버하고 session-swap 절반은 §결정 18 로 위임(중복 codification 회피, disjoint-axis invariant 보존).
...
- §15.5 / §16.6 closed-enum 확장 규약(강화 ratchet + 별 CFP + Story §1 사용자 승인 의무)은 **미발동** — touchpoint 추가 0, trigger 추가 0, 사용자 발화 burden 증가 0.
...
| lever | tier | 실효 범위 | 한계 |
|---|---|---|---|
| (a) D1 정책 명시 | 정책 | **가장 강함** — 전환 지점에 norm 을 못박아 Orchestrator self-governance 근거 제공 | 기계 강제 아님(behavioral) |
| (b) UserPromptSubmit 예방 reminder (본 hook) | warning(예방) | 매 user turn 에 전환 자율 norm 을 salient 하게 유지 (execution-backed test) | autonomous run 중 Story 간 전환에는 **re-fire 안 함**(그 사이 user prompt 부재) / 감지·차단 안 함 / payload 에 Epic/Story state 없음 |
...
- **무발화 over-halt = 어떤 hook 도 미감지** — tool call·발화 전무 = hook event 자체가 없음 (근본 blind-spot, AC-3).
...
- D2 = **신규 sibling hook** `hooks/story-transition-autonomy-reminder`(bash) + `.py`(self-contained stdlib, pattern A). skip-offer-reminder 확장 아님 — 별 mother ADR(§결정 22 vs §결정 21) + 별 governance 축, one-concern-per-hook 정착 패턴. shared reminder-base 추출 금지(YAGNI, fail-isolation).
...
**Amendment 11 (CFP-2374) / Amendment 12 (CFP-2392) / Amendment 13 (CFP-2567) = 강화 방향 `sunset_justification: null` family** — skip-offer PATH 폐지 (§결정 21) / MEMORY.md 슬림화 mechanism deferred 해제 resolve (§18.7 resolved_carrier ADR-129) / Epic 내 Story 전환 autonomy surface + 예방 reminder (§결정 22). Amendment 9/10 의 weaken-direction 과 달리 셋 다 정식성·자율 진행 강화 방향 → ADR-058 §결정 5 약화 evidence-gate 비대상. §결정 22 = §결정 15/20 enum member 추가 0 (전환 지점 적용 강화만, closed-enum 무손상) — cognitive layer count 불변 (D2 hook = mechanism 추가).
...
- [CFP-2567](https://github.com/mclayer/plugin-codeforge/issues/2567) — Amendment 13 carrier Issue (§결정 22 신설 — Epic 내 Story 전환 자율 진행, 전환 지점 over-halt/over-ask 방지, D1 doc 국소 명시 + D2 UserPromptSubmit 예방 reminder hook skip-offer 동형, §결정 15/20 전환 지점 적용 강화 enum member 추가 0, Stop force-continue REJECT)
...
- hooks/story-transition-autonomy-reminder(.py) — Amendment 13 D2 신규 sibling hook (skip-offer-reminder 동형 UserPromptSubmit 예방 reminder)

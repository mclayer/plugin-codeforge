...
- **신규 hook 근거 (별 sibling 정당)**: §결정 18(session lifecycle reflex) ⊥ §결정 22(transition autonomy) **disjoint** (§22.2 line "§결정 18 disjoint, cross-ref only, 재codify 금지" 무손상) + one-concern-per-hook(§22.7) — session-swap-handoff priming 은 transition-autonomy 와 별 concern 이므로 기존 hook TEXT 확장 아닌 신규 sibling 이 정합.

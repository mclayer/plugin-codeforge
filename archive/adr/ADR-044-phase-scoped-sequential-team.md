...
- **정직 한계**: 본 단락은 **저작-시점 self-assessment** 이며 ADR-167 §결정 7:97 상 재해석 여부의 의미 판정 최종 권위는 **설계리뷰 lane** 이다. "명시 검토 0건" 을 남기지 않기 위한 기록이지 리뷰 판정의 대체가 아니고, 재제정 의무의 해소(clear)를 주장하지도 않는다(clear 조건 = §결정 1 말미 — 구본 `Superseded` 전이). 참고 선례(규범 근거 아님, 관행 관측): `reinterpretation: true` entry 3건을 보유한 [ADR-147](ADR-147-ci-runner-topology-selfhosted-migration.md):46,60,67 도 `status: Accepted` 로 즉시 재제정되지 않았다.

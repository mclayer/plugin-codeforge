...
**결정 A1-2 — 검증 scope = codeforge family 7 plugin 한정 (codex 제외)**

> **CFP-2433 Amendment 3 fact-correction 반영** (2026-06-27): 본 prose 의 carrier facts 가 정정되었다 — (1) codex source repo = `openai/codex-plugin-cc` (registry name `openai-codex` ≠ GitHub source repo, MARKETPLACE_REPO override 맵 SSOT) (2) `superpowers` 엔트리 제거 (CFP-2249/ADR-122 의존 0 + 실 source `.version` 부재) (3) "9 plugin / codeforge family 7" → 실 entry 10 (codeforge 9 = CFP-1219 deploy/deploy-review 반영 + codex). **결정 내용 (codex/superpowers 가 family atomic upgrade scope 제외 = 2nd-class) 은 무변경** — facts 만 SSOT-정합. 상세 = Amendment 3.

`scripts/check-codeforge-version-drift.sh` `PLUGIN_MARKETPLACE` map = **10 entry** (codeforge family 9 — wrapper `codeforge` + 8 lane plugin, CFP-1219 deploy/deploy-review 포함 — + `codex`, source repo `openai/codex-plugin-cc`). `codex` 는 **외부 marketplace** = codeforge atomic upgrade 대상이 아니다 (독립 lifecycle). 무필터 10-entry 검사 시 codex 독립 drift 가 **false transaction-fail → 불필요 전체 rollback** 을 유발한다. 따라서 0 drift invariant 검증 scope = **codeforge family 7 plugin 한정** (atomic upgrade roster — `codeforge` + `codeforge-{requirements,design,review,develop,test,pmo}`, 결정 A1-3 7회 invocation 명단) — codex 제외 보장 의무 (false rollback = 0).
...
| `.github/workflows/<x>.yml` (단, `templates/github-workflows/<x>.yml` 짝 존재 시) | **wrapper 귀속** | self-app mirror = wrapper runtime workflow (F-D5 dedup 규칙 — `templates/` SSOT 와 쌍 1회 평가) |
| `.github/**` (templates 미러 짝 없음), `marketplace.json` 등 repo-meta | 비귀속 (별도 게이트) | version-bump 게이트 surface 아님 — `invariant-check` / `marketplace-parity` 등 별도 채널 |

**과확장 금지 invariant**: 변경 파일이 `plugins/<lane>/**` 에만 있으면 wrapper bump 요구 0 (역도 동일 — wrapper root surface 변경이 lane bump 요구 0, 단 coupling 전파 결정 A2-4 예외).

**unmapped-path 결정성 (deterministic 단일 규칙 — S2 lint 결정성 보장)**: 귀속 매핑은 두 closed 표만으로 outcome 이 결정된다 — (i) 위 **귀속 표** (경로 → plugin) + (ii) **closed exempt 표** (`archive/**` / `.github/**`·`marketplace.json` 등 repo-meta = 명시 비귀속). 둘 중 어디에도 매칭되지 않는 경로 (= unmapped) 의 outcome 은 **단일 규칙**으로 확정한다:

> **unmapped path = fail-closed = wrapper 귀속 보수 평가** (= wrapper bump 요구). silent skip 금지 (ADR-083 fail-closed-unknown 정합).

"또는" 분기 없음 — unmapped 는 무조건 wrapper 귀속(보수). **명시 exempt 는 closed exempt 표(ii)에 등재된 경로만** 면제 (등재 확장 = 본 ADR amendment 강화 방향 의무, runtime ad-hoc 면제 금지). 즉: `archive/**` 면제 = exempt 표 등재 근거 / 신규 unmapped 경로 등장 시 = wrapper 보수 bump 요구 (후속 amendment 로 귀속 표 또는 exempt 표에 명시 등재 전까지). 이로써 S2 lint 의 모든 경로가 deterministic single outcome (wrapper / lane-N / exempt) 으로 분류된다.

**diff_signal 2단 매핑 표 (path → §결정 1 surface category → bump rank — S2 fixture assert 단위, F-D4)**: 귀속 plugin 확정 후 diff_signal 은 변경 파일을 §결정 1 12-surface table 의 category 로 매핑해 rank 를 산출한다. glob → surface category → rank 가 deterministic 하도록 명시 (wrapper root scope 기준 — lane scope 는 `plugins/<lane>/` prefix 붙여 동형 적용):

| glob (귀속 후) | §결정 1 surface row | 기본 diff_signal rank | 비고 (rank 상향 조건) |
|---|---|---|---|
| `agents/**` (lane: `plugins/<lane>/agents/**`) | (a) Agent file | patch (minor edit) | 추가 = minor / 삭제·역할 재정의 = major (T2 동반, 결정 A2-4) |
| `skills/**` | (b) Skill file | patch | 추가 = minor / 삭제·redefine = major |
| `hooks/**` | (c) Hook script | patch (config-only) | 선택 hook 추가 = minor / required hook 추가·behavior break = major |
| `templates/**` | (d) Template (workflow/Form) | patch (comments only) | 선택 workflow·Form 추가·additive schema = minor / required workflow·schema break = major |
| `CLAUDE.md` (lane: `plugins/<lane>/CLAUDE.md`) | (g) CLAUDE.md SSOT semantic | patch (typo/clarity) | additive guidance = minor / 기존 artifact invalidate = major |
| `docs/**` | (g) 파생 (SSOT 문서) | patch | semantic 추가 = minor / invalidate = major (author 판정) |
| `commands/**` | (j) Slash command | patch (wording/help) | 추가 = minor / 삭제·behavior break = major |
| `scripts/bootstrap-*.sh` | (i) Bootstrap script | patch (comments/help) | 선택 setup step = minor / 기존 install 실패 유도 = major |
| `scripts/**` (기타) | (i) 파생 (스크립트) | patch | behavior 변경 시 author 판정 (minor/major) |
| `.claude-plugin/plugin.json` | (l) Marketplace mirrored field | (자체 = actual_bump source) | description/author 변경 = minor / version 변경 = bump 자체 |
| inter-plugin contract 파일 (`plugins/*/` contract) | (e)/(f) contract (ADR-008) | contract MAJOR → major / MINOR → minor | T1 coupling 동반 (결정 A2-4) |

- rank 산출 = 변경 파일들의 surface rank 중 `max` (단일 PR 다중 surface 시 최고 rank). **기본 rank = 보수 하한 (patch)** — 추가/삭제/break 판정으로 minor·major 상향. 상향 판정의 자동화 한계(예: agent "역할 재정의" 의미 판정)는 S2 가 휴리스틱(파일 삭제 = major / 신규 파일 = minor / 기존 파일 edit = patch) 으로 근사 + author override 경로 보유.
- **fixture assert 단위**: 각 행의 (glob, 변경 종류) → 기대 rank 가 deterministic 하므로 S2 의 RED→GREEN fixture 가 행 단위로 작성 가능 (예: `plugins/codeforge-design/agents/X.md` *삭제* → major / *신규* → minor / *edit* → patch).

**mirror 쌍 dedup 규칙 (`templates/github-workflows/**` ↔ `.github/workflows/**` — F-D5)**: ADR-005 byte-identical mirror invariant 에 의해 wrapper workflow 는 `templates/github-workflows/<x>.yml` (consumer 배포 SSOT) ↔ `.github/workflows/<x>.yml` (wrapper self-app copy) 쌍으로 함께 변경된다. 본 version-bump 게이트 자신(`check-plugin-version-bump.yml`)도 이 쌍이다. diff_signal 귀속:

> mirror 쌍 두 파일 **모두 wrapper 귀속** (`templates/**` = 결정 A2-3 wrapper / `.github/**` = repo-meta exempt 이나 **`templates/github-workflows/` 미러인 `.github/workflows/<x>.yml` 은 예외적으로 wrapper 귀속**으로 승격 — self-app copy 가 wrapper runtime workflow 이므로). 단 **rank 산출 시 쌍을 1회만 카운트 (dedup)** — 동일 논리 파일의 mirror 가 중복 trigger 로 rank 를 이중 가산하지 않도록, `templates/github-workflows/<x>.yml` 의 surface rank ((d) Template) 를 SSOT 로 1회 평가하고 `.github/workflows/<x>.yml` 은 그 mirror 로 간주(중복 가산 0).

요지: (1) `.github/workflows/` 중 `templates/github-workflows/` 에 짝이 있는 파일 = wrapper 귀속(면제 아님) / 짝 없는 self-app meta-only `.github/**` (예: branch-protection helper) = repo-meta exempt. (2) mirror 쌍은 dedup — rank 1회 평가. S2 lint 은 `templates/github-workflows/*.yml` basename 집합으로 `.github/workflows/` 매칭 여부를 판정해 귀속·dedup 결정.
...
**lane-only 분리 PR 의 coupling escape hatch (base §결정 2 상속 ambiguity 해소 — F-D3)**: lane plugin 변경(예: contract MAJOR)이 **wrapper root surface 무변경 PR (lane-only diff)** 으로 들어올 때 wrapper MAJOR 강제 여부를 deterministic 확정:

> **T1/T2 coupling 은 동일 PR 안에서 wrapper root surface 가 함께 변경될 때만 wrapper MAJOR 를 강제한다 (in-PR coupling).** lane-only 분리 PR (wrapper root 파일 0 변경) = wrapper bump 요구 0 — lane plugin 자체 MAJOR 만 평가 (결정 A2-2 per-plugin scope). wrapper MAJOR 전파는 **후속 wrapper PR (wrapper `CLAUDE.md` "Inter-plugin Contract" 참조 / "Development Agent Team" 표 갱신 동반)** 시점에 그 PR 의 surface 변경으로 자연 발생.

근거: §결정 2 의 T1/T2 전파 근거 자체가 "wrapper `CLAUDE.md` contract reference / agent topology 표 영향" 이다 — 그 wrapper 문서가 실제 변경되는 PR 에서 diff_signal/coupling_signal 로 잡히는 것이 정합이며, wrapper 파일을 건드리지 않은 lane-only PR 에 wrapper MAJOR 를 소급 강제하면 결정 A2-3 "과확장 금지(false-positive)" 와 충돌한다. 즉 coupling = in-PR 결합 시 전파, cross-PR 소급 강제 없음. 단 **MAJOR atomic (결정 A2-9)** 은 별개 축 — lane MAJOR bump 자체가 9-plugin 동시 MAJOR atomic cross-check 를 발동(ADR-063 §결정 18-B), 이는 coupling escape hatch 와 무관하게 적용.
...
3. **`#2294`/`#2298` 류 (linter `.yml` / manifest 수정)**: 변경 파일이 wrapper surface 귀속 (`templates/**` / `scripts/**`, 그리고 F-D5 규칙으로 `templates/github-workflows/` 짝이 있는 `.github/workflows/<x>.yml`) → diff_signal = 해당 surface rank (대개 PATCH — comments/config-only). `fix:` commit_signal patch 와 정합 → PATCH bump 의무. 단 **`templates/` 미러 짝이 없는 self-app meta-only `.github/**`** (예: branch-protection helper) 단독 = repo-meta exempt → 면제 (no-surface-touch).
...
- **blocking 승격 trigger**: ADR-060 §결정 5 default warning + 승격 gate (PR 누적 ≥ N + bypass 외 failure_threshold + evidence sample 누적) 충족 후 별도 amendment (또는 Epic 후속 Story) 로 `required` context 전환. 승격 = ratchet 강화 방향 (warning → blocking, ADR-058 §결정 5 정합).
...
- **lane CHANGELOG 은 해당 lane 검사에만 종속 가능**: `plugins/<lane>/CHANGELOG.md` 는 per-plugin self-owned (ADR-092 §결정 1 + Amendment 1 lane 8종 보존). lane bump 평가 시 lane CHANGELOG entry 동반은 ADR-092 §결정 2 drift detection (lane 8종) 영역 — 본 version-bump 게이트와 cross-ref (commit-signal 근거는 아님, 별도 detection 축).
...
- **cross-check roster source (authoritative — S2 결정성)**: 9-plugin roster 의 **authoritative source = 디렉터리 실측 enumeration** — wrapper root (`.claude-plugin/plugin.json` = `codeforge`) + `plugins/codeforge-*/.claude-plugin/plugin.json` glob (현재 8 lane). roster = `{codeforge} ∪ {plugins/codeforge-*/ 디렉터리명}`. 이 source 는 모노레포 디렉터리 구조(ADR-118 D3)에서 직접 derive 하므로 별도 상수 유지 drift 와 무관.
  - **`TOPOLOGICAL_ORDER` 비의존 (decoupling 의무)**: `scripts/lib/walk_plan.py` `TOPOLOGICAL_ORDER` 를 roster source 로 **사용하지 않는다**. 실측 (worktree `scripts/lib/walk_plan.py:94-104`): `TOPOLOGICAL_ORDER` 는 이미 9-plugin (CFP-1199 F1 — `codeforge-deploy` + `codeforge-deploy-review` 추가 완료) 이나, ADR-063 §결정 19 *본문 prose* 는 여전히 "현 walk-infra 7-plugin DAG 가정" stale 텍스트를 보유 (CFP-1199 코드 확장이 §19 prose 에 미반영). version-bump 게이트를 별도 lifecycle 의 Python 상수에 결합하면 향후 roster drift 리스크 → 디렉터리 enumeration 을 authoritative 로 고정해 결합 회피.
  - **ADR-063 §결정 19 prose stale = 별도 follow-up 지목** (본 ADR 비수정·비의존): §19 본문의 "7-plugin DAG 가정" 텍스트 ↔ `walk_plan.py` 9-plugin 실측 drift 정정 = 별도 follow-up CFP (ADR-063 영역, 본 Amendment scope 외). 본 Amendment 는 그 stale 을 고치지 않되 **roster source 로 의존하지 않음을 명시** (디렉터리 enumeration authoritative).
...
- **Tier routing (ADR-063 §결정 19)**: MAJOR bump 은 Tier 무관 family bundle 경로 강제 (§결정 19 "MAJOR bump 시 Tier 2 lane 도 Tier 1 bundle walk 강제 routing"). v2 cross-check = 이 routing 의 PR-time mechanical 검증 surface. (Tier routing 의 *개념*만 §19 인용 — roster source 는 위 디렉터리 enumeration, §19 prose 의 stale 7-plugin 텍스트 비의존.)
...
## Amendment 3 — drift-check 매핑 fact-correction (registry-name ↔ source-repo decouple + superpowers 제거) (CFP-2433, 2026-06-27)

> **Carrier**: CFP-2433 (drift-check codex/superpowers marketplace 매핑 정정). **Source**: `scripts/check-codeforge-version-drift.sh` 의 `PLUGIN_MARKETPLACE` 매핑이 marketplace registry name 과 GitHub source repo path 를 단일 문자열로 conflate — codex 엔트리(`[codex]="openai-codex/marketplace"`)가 잘못된 GitHub repo (`openai-codex/marketplace`, 404) 를 가리켜 codex 의 실 drift (installed 1.0.4 < latest 1.0.5) 를 silent masking 했다. 본 Amendment = 그 코드 정정(decouple + superpowers 제거)과 동시에 §A1-2 carrier prose 의 정정된 facts 를 SSOT-정합화하는 **fact-correction amendment** (direction = clarify, 결정 내용 무변경).

### Amendment 3 컨텍스트 — firsthand 실측 (research-before-claims)

| 항목 | origin/main 실측 (6.40.0 기준) | 판정 |
|---|---|---|
| `[codex]="openai-codex/marketplace"` | `marketplace_version()` 의 `repos/openai-codex/marketplace/...` fetch = 404 (실 source repo = `openai/codex-plugin-cc`) | codex 실 drift silent masking (fetch-failed 흡수) |
| codex registry vs source repo | cache dir = `openai-codex/codex/1.0.4` (registry name `openai-codex`) ≠ GitHub source repo `openai/codex-plugin-cc` | 단일 `mp` 문자열로 동시 만족 불가 = decouple 필요 |
| `[superpowers]="claude-plugins-official/marketplace"` | 실 source (anthropics/claude-plugins-official) 의 superpowers entry `.version` 부재 (source=url obra/superpowers.git, sha only) | 정정해도 항상 fetch-failed → 제거 대상 (CFP-2249/ADR-122 의존 0) |
| §A1-2 carrier prose | "9 plugin (codeforge family 7 + codex + superpowers)" | CFP-1219 deploy/deploy-review 미반영 stale (실 10 entry / codeforge 9) |

### Amendment 3 결정

**결정 A3-1 — §A1-2 carrier prose fact-correction (결정 내용 무변경)**

§A1-2 본문이 이 Story 가 정정·제거하는 두 사실을 verbatim carry 했다 — ① `codex openai-codex/marketplace` (= 정정되는 wrong 매핑) ② `superpowers claude-plugins-official/marketplace` (= 제거되는 엔트리) ③ "9 plugin / codeforge family 7" count-prose (= CFP-1219 미반영 stale). 코드 정정과 동시에 ADR prose 를 갱신하지 않으면 ADR 가 이미 deprecated 된 매핑을 SSOT 로 박제 → governance fact drift 영속. 따라서 §A1-2 prose 를 정정한다:

- `codex openai-codex/marketplace` → `codex` source repo `openai/codex-plugin-cc` (registry name `openai-codex` ≠ source repo, `MARKETPLACE_REPO` override 맵 SSOT).
- `superpowers claude-plugins-official/marketplace` 언급 → **제거** (codex 단독 외부 marketplace 로 정정).
- "9 plugin (codeforge family 7 + ...)" → "10 entry (codeforge family 9 + codex)".

**§A1-2 결정 내용 (codex/superpowers 가 family atomic upgrade 0-drift scope 에서 제외 = 2nd-class) 은 무변경** — Amendment 는 그 결정의 carrier prose 의 facts 만 정정 (mapping repo + count). 즉 strengthen/weaken 아닌 fact-correction.

**결정 A3-2 — 코드 decouple mechanism (registry-name ↔ source-repo 분리)**

`scripts/check-codeforge-version-drift.sh` 에 `MARKETPLACE_REPO` override 맵 (`[openai-codex]="openai/codex-plugin-cc"`) 신설 + `marketplace_version()` 안에서만 consult (`local repo="${MARKETPLACE_REPO[${mp%/marketplace}]:-$mp}"`). `installed_version()` (cache-key 파생) 과 install hint 의 `${mp%/marketplace}` 는 **무손상** — codex cache 좌표 `openai-codex/codex` 보존 (silent-regression 가드). 9 codeforge plugin 은 registry name `mclayer` 미등재 → `mclayer/marketplace` fallback (fetch 경로 byte-동일, 무회귀).

**결정 A3-3 — superpowers 엔트리 제거**

`[superpowers]="claude-plugins-official/marketplace"` 엔트리 삭제 + 주석 카운트 정정 (11→10 entry / 9→10 plugin). `installed_version()`·`marketplace_version()` 함수 본문 무관 — 엔트리 제거만으로 bare loop 에서 자연 배제. `check-no-superpowers.sh` 가드 (`superpowers:[a-z]` 라이브 호출 grep) 와 충돌 0 (map 키 ≠ skill-call 패턴).

**결정 A3-4 — direction = clarify (fact-correction, 약화 0)**

- `is_transitional: false` 보존. amendment direction = **clarify (fact-correction)** — weaken 아님 (결정 내용 무변경, carrier facts 만 SSOT-정합). ADR-058 §결정 5 약화 차단 logic 비대상 (mapping/count fact 정정은 invariant 약화 0).
- **T3 trigger 미발동**: ADR-037 = family invariant ADR 이나 본 amendment = additive fact-correction (supersede 아님) → 결정 A2-4 boundary 에 의해 wrapper MAJOR 강제 0 (Amendment 1/2 선례 §A1-5/§A2-4 정합).
- **version bump**: script behavior 정정 (silent false-negative → 정확 보고) = 의도 동작 복원 → `fix:` PATCH (결정 A2-6 commit_signal × diff_signal — `scripts/**` wrapper 귀속 → diff_signal patch). ADR additive amendment (h) = MINOR signal 이나 fact-correction = PATCH author 판정. expected = max(patch, patch, none) = PATCH → 6.40.0 → 6.40.1.

### Amendment 3 영향

- **wrapper plugin (codeforge)**: `scripts/check-codeforge-version-drift.sh` (override 맵 + superpowers 제거 + count 정정) + 본 ADR §A1-2 fact-correction + 신규 self-test → 6.40.0 → 6.40.1 PATCH. marketplace version sync (ADR-063).
- **6 lane plugin**: 영향 0 (drift-check = wrapper-root surface, lane plugin 자체 surface 무변경).
- **0 drift invariant (Amendment 1)**: 검증 scope 정의 (codeforge family 7 한정) 무변경 — codex 제외 보장 의무 유지. 본 Amendment 는 codex fetch 정확성만 복원 (codex 가 family scope 진입하는 것 아님 — 무필터 bare 검사에서 codex drift 가 정확히 *보고*되되 family atomic transaction 판정에는 여전히 미산입).

### Amendment 3 self-application (ratchet 검증)
...
> **v2 flow (Amendment 2 — normalization-before-max + β 비대칭 명시)**: 아래 다이어그램은 base 단순 `max() == actual` 표기를 v2 로 정밀화한 SSOT 다 (S2 구현 기준). 핵심: **① commit_signal 강등(A2-6 no-surface-touch normalization) 은 `max()` *입력*을 정규화하는 단계이지 `max()` 연산자나 비교 연산자를 바꾸는 것이 아니다.** ② 최종 비교는 등치(`==`)가 아니라 **β lenient 비대칭(A2-5: actual ≥ expected → PASS / actual < expected → FAIL)** 이다. S2 가 이 두 노드(정규화 + β 비교)를 생략하고 naive `max() == actual` 만 구현하면 본 PR 류(`docs:` prefix + 귀속 surface 0 변경)를 false-positive FAIL 시킨다(v1 template line 180-184 재현).

```
PR open
   │
   ├── Conventional Commits prefix 추출 → commit_signal_raw
   ├── PR diff 분석 → 결정 A2-3 귀속 SSOT → 결정 1 surface table → diff_signal
   ├── (wrapper PR 만) T1/T2/T3 검사 → coupling_signal
   ├── plugin.json version bump 추출 → actual_bump
   │
   ▼
① 입력 정규화 (A2-6 — max() 연산자 불변, 입력만 보정)
   │   변경 파일 중 wrapper version-bump surface 귀속 파일 0개 (no-surface-touch)?
   │     ├── YES → commit_signal := none (강등)   # docs:/fix: prefix 여도 귀속 surface 0 = bump 면제
   │     └── NO  → commit_signal := commit_signal_raw (유지)
   ▼
② expected = max(commit_signal(정규화 후), diff_signal, coupling_signal)
   ▼
③ β lenient 비대칭 비교 (A2-5 — '==' 아님)
   │
   ├── actual_bump == downgrade            → FAIL (forward-only 위반, §결정 5)
   ├── actual_bump ≥ expected              → PASS (over-bump 허용)
   └── actual_bump <  expected             → FAIL (under-bump 차단)
        ├── "expected MAJOR but got MINOR — diff includes (a) agent file deletion"
        ├── "expected PATCH but got none — commit fix: 이나 귀속 surface 있음, bump 누락"
        └── (대조) "docs: prefix 이나 archive/** only → commit none 강등 → expected none = actual none → PASS"
```

...
| `.github/**` (templates 미러 짝 없음), `marketplace.json` 등 repo-meta | 비귀속 (별 게이트) | version-bump 게이트 surface 아님 — `invariant-check` / `marketplace-parity` 등 별 채널 |
...
- **cross-check roster source (authoritative — S2 결정성)**: 9-plugin roster 의 **authoritative source = 디렉터리 실측 enumeration** — wrapper root (`.claude-plugin/plugin.json` = `codeforge`) + `plugins/codeforge-*/.claude-plugin/plugin.json` glob (현재 8 lane). roster = `{codeforge} ∪ {plugins/codeforge-*/ 디렉터리명}`. 이 source 는 모노레포 디렉터리 구조(ADR-118 D3)에서 직접 derive 하므로 별도 상수 유지 drift 와 무관.
  ...
  - **ADR-063 §결정 19 prose stale = 별도 follow-up 지목** (본 ADR 비수정·비의존): §19 본문의 "7-plugin DAG 가정" 텍스트 ↔ `walk_plan.py` 9-plugin 실측 drift 정정 = 별 follow-up CFP (ADR-063 영역, 본 Amendment scope 외). 본 Amendment 는 그 stale 을 고치지 않되 **roster source 로 의존하지 않음을 명시** (디렉터리 enumeration authoritative).

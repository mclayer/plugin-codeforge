...
5번째 measurement channel 추가 = ADR-163 amendment 의무. 본 closed enumeration 가 future "X tool 호출도 ledger record" 류 압박을 차단.
...
- `docs/adr/ADR-163-codeforge-measurement-channel-architecture.md` (본 file)
...
### 대안 C — Privacy policy 를 본 ADR-163 §결정 추가 (sibling ADR-043 미신설)
...
**(C) §결정 11 ROI-gate supersede 명문화** — §결정 11(post-merge-counters.jsonl 30+ run ROI gate prerequisite)를 Epic #2686 directive 가 supersede(Amd1 CFP-2393 §근거 byte-동형 template [verified: ADR-163:455] — "Epic directive 가 ROI deferral 을 supersede 한 선례"):

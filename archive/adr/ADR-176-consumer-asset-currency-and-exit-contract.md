...
- **(C4) 런타임 전제 — 정의역 2분할 (자산 분류 기준).** 초판은 단일 전제로 적었으나, 그 근거(self-hosted fleet degrade)가 닿는 정의역은 **consumer 로 실제 배포되는 자산**뿐이다. 정의역을 명시하지 않으면 wrapper 내부 검증 하네스까지 침묵으로 구속되어 실행 가능한 오라클이 사라진다(CFP-2978 AC-10 에서 실제 발생).
  - **(C4-a) consumer 배포 자산** = `templates/consumer-scripts.manifest` 등재분 + consumer repo 로 실제 배치되는 스크립트·workflow. 런타임 전제 = `python3`(하한 3.10, **stdlib only**) + `git` + `curl` 3종. `yq`·`gh`·`uv`·PyYAML 등 3rd-party 의존 **금지** — self-hosted 러너 이미지(`ci-runner-infra` `ci-runner-linux/Dockerfile`, base `ubuntu:22.04`)에 `yq`·`gh` 가 **부재**하고 [verified — Dockerfile 실독 + live run 4/4 교차 일치], 의존을 늘리는 설계는 그 fleet 에서 **조용히 degrade** 한다.
  - **(C4-b) wrapper 검증 자산** = consumer 에 배포되지 **않는** 검증 하네스(manifest **미등재** ∧ wrapper CI `ubuntu-latest` 또는 lane-time 로컬에서만 실행). **C4-a 의 정의역 밖**이다 — wrapper 는 `requirements.txt`(`pyyaml`·`pytest`)를 보유하고 20+ workflow 가 `pip install pyyaml` step 을 이미 수행한다 [verified]. 단 **의존 부재 = fail-closed meta-error(exit 2)** 를 의무화한다(`scripts/check-workflow-yaml-parse.sh` L20-24 형) — 침묵 GREEN 금지.
  - **분류 승격 게이트 (비협상)**: C4-b 자산이 이후 `consumer-scripts.manifest` 에 등재되는 순간 **C4-a 로 승격**되어 3rd-party 의존 허용을 잃는다. 등재 PR 은 그 자산의 의존을 **재판정할 의무**를 진다. 이 승격 규칙 없이 C4-b 를 인용하면 C4-a 우회 통로가 된다 — "검증용" 이라는 자기 라벨만으로 면제되지 않는다.
...
- **(E3) 소비층은 판정을 은폐하지 않는다.** **shell 층 및 `env:` 매핑과 `$GITHUB_ENV` 기입을 포함한 모든 선언·전달 층**이 callee 의 최종 분기에 도달하지 못하게 막는 선주입 — **판정 산출을 은폐하는 형태(`determined` 부재 · 조용한 기본값 대입)** — 을 **금지**한다. 이들 층의 역할은 *best-effort 유도 시도*이고, 유도 실패 시 아무것도 주입하지 않아 py 의 정책 authority 에 위임한다. 흡수(OR-true, `continue-on-error`)로 인해 verdict 가 비차단에 머무는 경우, 그 **비차단성은 명시 선언 대상**이며 "통과"로 계상할 수 없다. **↔ (E4) 구분자**: 본 조가 금지하는 것은 *은폐하는* 선주입이다. (E4) 가 향후 blocking 승격 시점에 허용할 수 있는 bypass 수령(`determined: false` **명시 emit** + 감사 마커)은 판정을 **산출로 드러내므로** 은폐가 아니며 본 조의 금지 대상이 아니다. 구분자는 **주입 층이 아니라 산출의 가시성**이다.
  - **선언 층 포함 근거 (1차 확장 — 결정 반전 아님).** 정본 `scripts/lib/check_parallel_work_sentinel.py` L128 (`resolve_prefix()`) 이 `os.environ.get("STORY_KEY_PREFIX", "")` 로 **주입 출처를 구분하지 않고** 읽으므로, shell 대입 0줄이어도 workflow 의 `env:` 매핑(정본 `.github/workflows/parallel-work-sentinel-check.yml` L44 workflow-level / L65 step-level)에 같은 키를 넣으면 shell 선주입과 **동일한 은폐 효과**가 성립한다. "shell 층" 한정 문면은 그 경로를 narrow-reading 우회로 남기므로, 층을 선언 층까지 확장해 의도와 문면을 일치시킨다. [verified firsthand — 좌표는 `origin/main` 커밋 `ecfe62d6378a64de6f644ca1919386504ae4f469` 기준 실측]
  - **`$GITHUB_ENV` 포함 근거 (2차 확장 — 같은 narrow-reading class 의 재발).** 1차 확장이 명명한 `env:` 매핑은 **세 번째 채널을 여전히 남겼다**. 선행 step 이 `$GITHUB_ENV` 가 가리키는 파일에 기입하면 그 키는 **후속 step 의 프로세스 env** 에 실려 위와 동일한 은폐 효과를 낸다 [source: docs.github.com/en/actions/reference/workflows-and-actions/workflow-commands — *"The step that creates or updates the environment variable does not have access to the new value, but all subsequent steps in a job will have access."*]. 이 경로는 **두 검사 사이로 빠져나간다** — `env:` 키 술어는 `run:` 문자열 내부를 보지 않고, CFP-2978 AC-1c leg ① 은 "대입 구문만 카운트"라 `echo "K=V" >> "$GITHUB_ENV"` 를 명시 배제한다. wrapper 실 선례가 실재한다 — `.github/workflows/css-lint.yml` **L128-129** 2개소 [verified firsthand — `origin/main` 커밋 `ecfe62d6378a64de6f644ca1919386504ae4f469`]. 나아가 step 으로 실행되는 **action**(composite / JS `core.exportVariable()` / Docker) 도 같은 파일에 기입하므로 [source: github.com/actions/toolkit `packages/core/README.md` — *"Since each step runs in a separate process, you can use `exportVariable` to add it to this step and future steps environment blocks."*], 본 조의 정의역은 **층 열거가 아니라 "센티널 프로세스의 env 에 키를 싣는 모든 경로"** 로 닫는다. 열거는 예시이며 미열거 경로의 면제 근거가 되지 않는다. 채널 전수 판정·층별 검출 완결성 = CFP-2978 Change Plan §8.F.
- **(E4) warning-tier 게이트에 bypass 우회로를 신설하지 않는다.** bypass 의 존재 이유는 merge 를 막는 게이트를 hotfix 로 뚫는 것인데, 차단하지 못하는 게이트에 대한 우회로는 운영 이득 0 + 공격 표면 순증이다. 향후 blocking 승격(ADR-171 evidence-gated 별 carrier) 시점에 도입한다면 `determined: false` **명시 emit**(부재 금지) + workflow/runner env 에서만 수령(PR 통제면 파생 금지) + 감사 마커 + "bypass 는 판정을 대체하지 않음(원장 충족 계상 금지)"를 동반한다. **↔ (E3) 구분자**: 본 조가 서술하는 향후 bypass 는 (E3) 가 금지하는 *은폐형 선주입*이 **아니다** — 구분자는 명시 emit·감사 마커의 유무이지 주입 층이 아니다. 따라서 **(E4) 를 근거로 (E3) 의 선언·전달 층 확장을 되돌리는 인용을 금지한다**(본 조는 수령 조건을 규정할 뿐 은폐를 허용하지 않는다).

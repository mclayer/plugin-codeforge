...
amendment_log:
  ...
  - amendment: 4
    ...
    summary: "S4 auto-rollback signal producer 소실 disposition NOTE (tombstone 아님). (a) S4 producer scripts/check_rollback_signal.py + rollback-signal-monitor.yml 삭제 — CFP-2782 delete-set (ADR-105 auto-rollback TOMBSTONE 결합). (b) deploy-review 성능 verdict input source 소실(§결정 C 명시 수용 포섭 — deploy-review lane 물리 제거로 1 input channel 소멸). (c) self-improving loop 재정의(GitHub-native delegation 하 operational signal source 재배치) = 후속 CFP defer(ADR-121 §결정 E 후속-defer + ADR-106 S6-carrier defer 정합). tombstone 아님 — loop 개념(operational metric → PMO)은 delegation-agnostic, §결정 1~5(회로 4단계 / ADR-045 §D-9 disjoint 답습 / KPI append-only / loop closure 3원칙) 본문 0건 변경. §결정 3 closed enum(latency_burn_rate 등) 자체 무변경(producer 파일 삭제이지 enum 정의 변경 아님). AC-9 충족."
    direction: disposition
    sunset_justification: "loop-policy 약화 0 — 자기개선 loop §결정 1~5 본체 무변경(loop 개념 delegation-agnostic). 제거 대상 = deploy 실행 machinery(S4 auto-rollback signal producer check_rollback_signal.py + rollback-signal-monitor.yml)로 상위 ADR-105 TOMBSTONE + ADR-121 배포 완전 위임에 종속된 dead-input cleanup. loop signal source 재배치(GitHub-native operational signal)는 후속 CFP defer(원 S6-carrier defer 정합) — 본 Amendment 는 소실 disposition 기록만. ADR-058 §결정 5 정합(정책 약화 아닌 upstream-driven input 채널 소실 기록)."
---
...
| 2026-07-22 | CFP-2782 | **Amendment 4** — S4 auto-rollback signal producer 소실 disposition NOTE (tombstone 아님). S4 producer `scripts/check_rollback_signal.py` + `rollback-signal-monitor.yml` 삭제 (CFP-2782 delete-set, ADR-105 TOMBSTONE 결합) + deploy-review 성능 verdict input source 소실(§결정 C 수용) + self-improving loop 재정의(GitHub-native delegation) 후속 CFP defer. loop 개념 delegation-agnostic → §결정 1~5 본문 0건 변경, §결정 3 closed enum 무변경. disposition (dead-input cleanup, upstream ADR-105/ADR-121 종속). |

...
- **consumer overlay = 별도 changelog 미보유** — consumer overlay (`.claude/_overlay/`) 는 자체 changelog 를 두지 않는다. consumer 는 wrapper changelog walk 의 **입력** (설치된 plugin version 집합) 으로 참여하며, 별도 changelog SSOT 를 신설하지 않는다 (consumer = 정책 축소 불가 + 신규 SSOT 신설 비대상).
...
- **tier = warning** (ADR-060 4-tier enum 의 `warning` — advisory). drift 검출 시 경고 emit, PR gate block 미발동 (§결정 0 carry-over 2 — PR-gate layer disjoint). 승격 (warning → blocking) 은 ADR-060 승격 gate (PR 누적 ≥ 20 + bypass 외 failure = 0 + sibling Story merged) 충족 후 별도 amendment.
...
- consumer overlay 별도 changelog 미보유 declare — consumer 정책 축소 불가 + 신규 SSOT 폭발 차단.

...
> **INV-S1 digest 분기의 실행 범위 (설계리뷰 R5 — 정직 고지 정밀화)**: 현행 3파일에 `cfp-split` 마커가 **0** 이라 `anchor_delta = ∅` 가 상시 성립하고 digest 분기는 **무변경 정본 배터리에서 미진입**이다(미발화를 통과로 계상하지 않는다 — CP §13.2). 다만 이를 *"INV-S1 검출력 미검증"* 이라 기술하는 것은 **과대**다 — 위 M-SPLIT / M-LOSS 행이 판별력을 실증했다(무손실 **GREEN** / 소실 **RED**). 정확한 한정은 **"본 배터리 한정 미실행"** 이다.

...
- **guard 배치 mechanism 결정 (설계 lane 확정 — AC-2 outcome-binding 충족)**:
  - **근본 사실 (#846 root)**: subagent 는 wrapper `CLAUDE.md` 를 자동 상속하지 않고 **자기 agent `.md` system prompt** 로 동작한다. 따라서 Orchestrator-layer(wrapper CLAUDE.md)에만 두면 subagent 미도달 = 재무장 미차단.
  - **채택 (2-layer)**:
    1. **subagent-reach layer (guaranteed)** — **7 agent `.md` body 각각에 disambiguation 1줄**. agent `.md` 가 곧 subagent 의 system prompt 이므로 **보장 도달**(AC-2 "≥1 subagent-read layer surface" outcome 충족).
    2. **governance SSOT (Orchestrator-facing)** — 본 ADR-141 Amendment 1(SSOT) + wrapper `CLAUDE.md`(L79 "Sonnet/Haiku 세션 중단" + L98-99 "전 에이전트 opus 단일 tier" 두 문장에 scope 명확화)에 disambiguation 명문화.
...
- **haiku 429**: haiku 는 opus 와 별 pool·별 429 거동 → ADR-109(opus-scoped 429 mitigation) 미적용, fallback tier 부재(단일 tier 폐기 상속). 리스크 낮음(경고) — 운영 관찰 대상.

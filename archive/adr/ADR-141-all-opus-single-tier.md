...
4. **SHARED SSOT prose 일반화** — `docs/architecture/codeforge-family.md` tier 서술 + 각 lane architecture mirror(ADR-078) → 누적 "haiku 7 + sonnet 10 + fable 10 (opus 14)".
...
7. **§8 roster-integrity 게이트 신설** — count=10 fable + 정확 파일 대조 + **SecurityTestPL NOT fable** + drift 0 검증. RED→GREEN discriminating(한 파일 model flip → 게이트 FAIL) + self-test enroll + hard-gate cross-seal(born-broken 회피 — CFP-2635/881/2762 계보). **기존 agent-model-tier roster lint 부재 firsthand 확인**(check-stakes-tier-gating[dead]/check-tier-downgrade-guard[registry]/check-tier-honesty[stop-lever] 3종 모두 frontmatter roster 미검증) → 신규 gate.

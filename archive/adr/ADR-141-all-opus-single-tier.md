...
- **guard 배치 mechanism 결정 (설계 lane 확정 — AC-2 outcome-binding 충족)**:
  ...
  - **채택 (2-layer — 양 layer 모두 subagent 도달 = over-determined coverage)**:
    ...
    2. **guaranteed reinforcement** — **7 agent `.md` body 각각에 disambiguation 1줄**. agent `.md` 가 곧 subagent 의 system prompt 이므로 **system-prompt-embedded 보장 도달**(agent 별 명시). AC-2 "≥1 subagent-read layer surface" outcome 충족 — 실제로는 CLAUDE.md·agent md 두 layer 모두 subagent-read 이므로 over-determined.
...
- **sonnet 429**: sonnet 은 opus 와 별 pool·별 429 거동 → ADR-109(opus-scoped 429 mitigation) 미적용, fallback tier 부재. Amendment 1 A1-6 haiku 429 note 와 동형 — 리스크 낮음(경고), 운영 관찰 대상.

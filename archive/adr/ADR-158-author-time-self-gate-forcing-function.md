...
mechanical_enforcement_actions: []  # declaration-only Phase 1 — ADR-082 §결정 6 / ADR-155·156 retain pattern 답습. 실 self-gate runner(authoring_self_gate.py) + advisory lane-exit self-check wiring + (조건부)신규 lint + self-test(독립 oracle + 대칭 fail-closed) + inventory enroll = 동일 Story Phase 2 carrier(CFP-2689 §8-§11). pattern_count >= 2 재발 시 follow-up CFP MUST promote advisory→mechanical hook (ADR-084 precedent)
---

...
**경계 무변경**: branch-protection 7-tuple 무변경(결정 8) — internal-docs 게이트는 다른 repo(독립 저장소). advisory `authoring_self_gate.py` 존치(defense-in-depth, 다른 시점 커버). wrapper `ac-traceability-matrix.yml` 무변경.

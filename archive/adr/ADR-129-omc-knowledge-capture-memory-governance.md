...
related_adrs:
  ...
  - ADR-051   # skill 형식 SSOT (skills/<slug>/SKILL.md subdir + frontmatter name/description + trigger 명시 의무 (ADR-051 §결정 4))
...
| **(b) total file** | MEMORY.md 전체 ≤ 24.4KB | ADR-071 §18.2 `[verified]` cap SSOT |
...
`size > 24.4KB` 시:
- **oldest-first** — 가장 오래된 entry 부터 슬림화 (ADR-071 §18.3 normative 답습).
...
**ADR-051 정합**: 본 ADR 의 skill 산출물 = `skills/knowledge-capture-gate/SKILL.md` subdir form + frontmatter `name`/`description` + trigger 명시 의무 (ADR-051 §결정 4 — trigger declaration; "≥20줄" 은 §결정 1(a) CLAUDE.md inline→skill 추출 판정 기준이지 skill 형식 요건 아님).

...
related_adrs:
  ...
  - ADR-145-ac-traceability-zero-drop-gate  # "심사 완결" 분모 = §5.3 AC `coverage_required` **열** 재사용 (새 완결 술어 발명 = 2-SSOT 금지). 판정은 literal `=Y` 가 아니라 §결정 4-1 **D-1 정규화 술어** 경유 — literal 배선은 실 corpus 분모를 99/107 Story 에서 0 으로 붕괴시켜 폐기됨. Risk5(tier 오분류 = 강제 약화) 동형 경고 상속
  ...
  - ADR-171-evidence-enforceable-promotion-framework  # 신규/확대 검사 warning-first(:172) + 신규 entry 등록 4 필수 동반(:174 — mechanical_enforcement_actions / owner_adr·carrier_adr 분리 / self-application 무한루프 회피 판정 / 실 script+workflow+discriminating self-test) + 승격 3-AND(§결정 6). required-host 충돌 해소 = 본 ADR §결정 10
...
related_files:
  - plugins/codeforge-review/templates/review-pl-base.md  # Phase 2 1차 착지면 — §2 정의역 결속 1문 / §3 판정표 conjunct / `### lane PASS 종결 술어` 절 신설(### 형제·§3.x namespace 밖 — F3 처분) / :133 정정 / :205 열거 개정
  ...
  - scripts/lib/review_exit_criteria.py  # Phase 2 신규 — 실행 SSOT (AC-23 실행층 전환): normalize_coverage_cell·count_checklist_required·count_checklist_examined leaf
  - tests/scripts/_review_exit_fixtures.py  # Phase 2 신규 — fixture 빌더 (_ac_matrix_fixtures.py 형판 sibling)
  - tests/scripts/fixtures/review_exit_criteria/  # Phase 2 신규 — census baseline 스냅샷 (reach-baseline.json)
related_stories:
  - CFP-3011
...
**★ 위 수치의 산출 근거 (F7 처분 — 수치 리터럴 단독 금지)**: 이 단락의 백분율·라운드 수(67.3% / 32.7% / 29.2% / 5.3% / 113 / 5.65 / 1-of-20)는 **기계 명령 산출물이 아니다** — Story CFP-3011 §1 의 **수기 전수 분류표**(20 Story × FIX 라운드별 발원 class 배정, 재량 ±5% 정직 선언)가 유일 원천이며, 재현 절차 = 그 분류표의 class 배정을 동일 기준으로 재수행하는 것이다. 따라서 본 ADR 은 이 수치를 **구조 진단의 정성 근거로만** 쓰고 AC·성공 기준·게이트 임계로 쓰지 않는다 (§측정 절의 비목표화 선언과 동일 취급). 반면 본 ADR 이 배선 근거로 쓰는 census 수치(§결정 4-1 corpus 계수 · §결정 5 heading·embed 계수)는 **전건 산출 명령 + 기준 SHA 를 그 자리에 병기**한다 — 명령 없는 수치는 증거가 아니다.
...
lane PASS = 다음 동시 충족의 명시 판정. 착지면 = review-pl-base 의 **신규 `### lane PASS 종결 술어 (심사 정의역 결속 — ADR-183)` 절** (이하 "종결 술어 절"). 배치 세부는 Change Plan §3.3.

**★ 실행층 전환 선언 (AC-23 최초 발동 — 설계리뷰 3회차 2-strike 성립, 사용자 결정 2026-08-19 verbatim "실행층 전환")**: 분모 정규화(D-1)·토큰 해소·checklist 축 분모 단위의 세부 규칙 **정본 = Phase 2 실행 SSOT** — `scripts/lib/review_exit_criteria.py`(`normalize_coverage_cell` / `count_checklist_required` / `count_checklist_examined`) + `tests/scripts/_review_exit_fixtures.py` + `tests/scripts/test_review_scrutiny_domain.py`·`tests/scripts/test_review_exit_criteria.py` + **mutant 계약 M1~M17** (M17 = Iter 5 ratchet 판별자 — DR4-6). 본 문서는 그 정본을 **참조**한다 (산문 재기술 금지 — 같은 산문 층 4번째 처방 금지). 산문에 남는 것 = **fail-closed 방향 3개**(① 미해소 = `UNDECIDABLE` RED ② 분모 0 = RED ③ 표면-존재 4분기 폐쇄) + 착지 좌표 + 포인터. 근거 = 3라운드 공통 근인("ADR 산문으로 정의역/값 규칙을 기술하는 층 자체가 매 라운드 자기 정의역을 새 사각으로 재생산" — Story §9.10) + 형판 선례(`check_ac_traceability_matrix.py` 가 동일 문제를 코드 4-값 enum + 가드로 이미 폐쇄).

**★ heading 레벨 + 번호 namespace 절단 (설계리뷰 v2 FIX Iter 2 / F3, P1)** — 종결 술어는 **4 리뷰 lane 보편** 명제인데, base 의 `§3.x` 번호는 `### Adversarial debate`(base:201) 블록이 소유하고 그 블록은 `:203`/`:205` 로 **DesignReview 한정 scope** 를 선언한다. 따라서 신규 절을 그 블록의 `####` 자식으로 넣거나 `§3.4` 로 명명하면 **이름·구조가 멤버십을 신호**해 4-lane 보편 술어가 문면상 design 전용으로 정당 해석된다 (AC-1·AC-2 충돌). 처분 3항 — 전건 동시 적용:

1. **heading 레벨 = `###` 형제 절** — `### Adversarial debate` 블록(§3.3 종료) **직후**에 배치해 물리 인접(참조 근접성)은 유지하되 부모 scope 상속을 절단한다.
2. **번호 namespace 절단** — 절 이름에 `§3.x` 번호를 쓰지 않는다 (`§3.4` 명명 금지 — 이름 자체가 Adversarial debate SOP 멤버십을 신호). 정본 이름 = 위 heading 문면.
3. **본문 첫 줄 scope 선언 의무** — 절 본문 첫 줄에 다음을 적는다: *"본 절은 4 리뷰 lane(요구사항리뷰·설계리뷰·구현리뷰·보안테스트) 전건 적용 — 상위 `### Adversarial debate` 절의 DesignReview 한정 scope(:203/:205)를 상속하지 않으며, Adversarial debate SOP(§3.0~§3.3) 의 구성원도 아니다."* 아울러 §결정 5 의 base:205 열거 개정문에 "종결 술어 절은 Adversarial debate SOP 비소속 4-lane 공통 절"을 명문화한다 (heading 레벨 조정 ∧ scope 예외 명문 **둘 다** — 단일 방어 의존 회피).
...
1. **심사 완결** — 해당 lane 적용분 §5.3 AC 중 **`coverage_required` 가 그 lane 을 커버 대상으로 지시하는 행** 전수 심사 ∧ packet checklist 전수 심사, **두 축 각각의 선언**. 분모 = ADR-145 `coverage_required` 재사용 (새 완결 술어 발명 = 2-SSOT 금지).
   - **★ 두 축은 등식 정의역이 다르다 (설계리뷰 3회차 FIX Iter 3 / P1-1)**: 등식 `examined_count == required_count`(§결정 4-2)의 정의역은 **§5.3 AC 행 축 단독**이다. checklist 축은 그 등식 안에 들어가지 않으며 **별 선언**(`checklist_examined` / `checklist_required` 쌍 emit + 동일 등식 형식)으로 판정한다. 두 축을 한 등식에 섞으면 **딜레마가 성립**한다 — checklist 항목을 분자에 더하면 `분자 > 분모` 가 상시 성립해 **항상-RED**, 분모에도 더하면 정의역이 ADR-145 `coverage_required` 밖으로 나가 2-SSOT, 아예 빼면 checklist 축이 **측정되지 않는 vacuous** 축으로 남는다. ∴ **같은 형식·별 등식 2벌**이 유일한 비모순 해다 (같은 *규칙* 두 벌 금지와 무관 — 규칙은 하나, 정의역이 둘). **checklist 축 fail-closed floor (Iter 4 F-2 처분 — fail-closed 방향 ② 의 checklist 축 인스턴스)**: `checklist_required == 0` → **RED**. 분모 항목 단위·판별자의 정본 = 실행 SSOT `count_checklist_required` + `F-CHK-*` fixture — 후보 산식 5개가 5~154 로 갈리는 단위 결정을 **산문으로 확정하지 않는다** (산문 확정 = 4번째 같은-층 처방). checklist 표면 원리적 부재 = `null` 비적용 positive declare (침묵 통과 금지 — 축 B-1 형판 상속). 집행력 = `declared` (§결정 11 Iter 4 재판정 — Phase 2 M13 kill 실증 시 승격).

**★ 분모 술어 — 결정 근거 요지 + 실행층 전환 (Iter 2 P0 결정 자체는 존속, Iter 4 는 정본 위치만 이전)**

- **literal `Y` 배선 확정 기각 (결정 존속 — 요지)**: 게이트 파서는 그 열을 읽지 않고(`_scan_ac_table` 추출 컬럼 4개, workflow·sh hit 0), 정본 스키마 의미는 **list** 이며(`templates/story-page-structure.md:202` · `scripts/lib/ac_id.py:124`), 실 corpus 에서 literal `Y` 배선 하 분모 0 Story = **99/107 (92.5%)** — `0 == 0` 이 등식을 충족해 AC-18 이 금지한 vacuous-PASS 가 재입장한다. **산출 명령 + 기준 SHA + 값 공간 3-정의역 병기 + "동일 명령이 아니라 동일 결론 3회" 정직 선언의 정본 = Change Plan §2 재현 명령 블록**(internal-docs `2e9e514d`) — 본 문서는 수치 상세를 재기술하지 않는다.

- **D-1 분모 정규화 (`declared` — Iter 4 실행층 전환, Phase 2 승격 조건부)**: 판정 골격 = `strip` + `casefold` 후 **4-값 폐쇄 enum** — `AFFIRM`(lane-agnostic 긍정 = 전 lane 커버 지시) / `NEGATE`(제외) / `LIST_RESOLVED`(list-form 해소 — 해소 판정의 정본은 실행 SSOT) / `UNDECIDABLE`. **fail-closed 방향 ① (산문 잔존 불변식)**: 미해소 표기·빈 셀·`[]`·열 부재 = `UNDECIDABLE` → **RED**. 값 집합·delimiter·lane→토큰 매핑의 **정본 = 실행 SSOT** `normalize_coverage_cell` + `F-DENOM-*`·`F-TOKEN-*` fixture — 본 문서는 재기술하지 않는다. 재발명 0 = `check_ac_traceability_matrix.py:103` `APPLIC_UNDECIDABLE` 형판 재사용.

**★ 토큰 해소 — 실행층 이전 (`declared`, Iter 4 F-3·F-5 처분 — 구 축 C 1·2항의 매핑 표·delimiter 열거 전체 제거, 폐쇄 결정 자체는 존속)**: 폐쇄 매핑·delimiter 의 **정본 = 실행 SSOT** `normalize_coverage_cell` + `F-TOKEN-*` fixture. **문서 층 불변식 (fail-closed 방향 ① 의 토큰 축 인스턴스)**: 매핑은 폐쇄이며 미해소 = `UNDECIDABLE` → **RED**. **부분해소 셀**(해소 토큰 ≥1 ∧ 미해소 토큰 ≥1)의 판정 정본 = 실행 SSOT — `F-TOKEN-PARTIAL`(부분해소 → RED) + `M14`(해소 판정 약화 시 그 fixture 만 kill, `F-TOKEN-UNRESOLVED` 는 RED 생존 — 비대칭 assert)가 **행동으로 고정**한다 (F-3 처분: 양화사 규칙 문장을 산문에 다시 쓰지 않는다). 집행력 = **`declared`** — 직전 판의 `mechanical floor` 는 리뷰 F-3 이 반증(부분해소 GREEN 흡수 독법 실재)했으므로 하향하며, 승격 조건 = Phase 2 코드 + fixture + mutant 착지 ∧ M14 kill 실증 (ADR-171 3-AND 경로). ※ **DR4-1 정정 (Iter 5 — 기재 진실성)**: 직전 판(Iter 4)이 이 자리에 적었던 단정 — 매핑 표의 `implementation` 7 이 열-오귀속이며 그 열-한정 계수가 0 이라는 것 — 은 **거짓**이었다 — 열-한정 실측 참값 = `implementation` **7** · `security` **3** (발원 = CFP-2949 §5.3 — `wrapper/stories/CFP-2949.md` 헤더 `| id | statement | source | verification | coverage_required | phase | tier |` 표의 JSON-인용 list 셀 `["implementation","security"]`×3 · `["design","implementation"]`×2 · `["implementation","integration"]`×1 · `["implementation"]`×1, 직접 Read + 표-블록 파서 재현 일치). dr3 F-5 의 "0" 은 **JSON 인용부호 미strip 계측 산물**이다 (bare 리터럴만 매칭한 파서). 구 매핑 표의 7 은 참이었고, 표 제거의 사유는 "오귀속이라서"가 아니라 **정본 위치 이전 (실행층 전환)** 이다. 열 한정 실측의 정본 = Change Plan §2 **토큰 축 열-한정 census** (Iter 5 실등재).

- **0-cell lane 정직 declare (구 축 C 3항 존속 축약 — Iter 5 DR4 정정: `security` 제외)**: 열-한정 실측(인용부호 strip — CP §2 토큰 census)상 `code` · `requirements`(-review) 리터럴 = **0회** — 구현리뷰·요구사항리뷰 **2 lane** 의 분모는 day-1 전량 AFFIRM 경로로만 형성되고 list-form 축은 그 2 lane 에 대해 vacuous 하다 (그래도 분모 0 이면 축 B-3 가 RED — 종결 술어 비적용 아님). `security` 는 list-form **3회 실재** (전건 CFP-2949 §5.3 `["implementation","security"]` — Iter 4 의 "3 lane 전건 0회" declare 는 인용부호 미strip 계측 산물의 승계였다, DR4-1 파생 정정). **F-7 처분 (AC-2 긴장의 계약면 등재 — Iter 5 동반 정정)**: AFFIRM 은 lane-agnostic 이므로 **code·requirements-review 2 lane** 의 분모는 동일 집합이 되고, security lane 은 list-form 3셀만큼 분모가 갈릴 수 있다 (실질 변별은 여전히 design 지배 — 열-한정 `design` 248 대 `security` 3). AC-2 의 lane 단위 개별 식별은 정의역 계약(§결정 1~2) 층에서 충족되며, 분모 계수 층의 day-1 미변별 선언은 위 2 lane 에 한정한다.
- **fixture corpus (구 축 C 4항 축약)**: fixture 는 corpus 실토큰·실측 오염 셀로 구성한다 — 구성 목록의 정본 = 실행 SSOT fixture 정의 (Change Plan §8).
...
| B-4 | present | — (∧ `required_count >= 1`) | 등식 판정 진입 (§결정 4-2) | `APPLIC_SURFACE_PRESENT`(:102) |
  ...
  - **분기 집합은 폐쇄**다 — 위 4행이 `signature × claim × required_count` 조합을 전부 덮으며, 어느 조합도 판정 없이 통과하지 않는다. 현 corpus 99/107 이 정확히 B-3 형상이므로 이 floor 없이는 신설 등식이 실질적으로 항진이다 (계수 명령 정본 = Change Plan §2 재현 명령 블록 — 구 P2-1 블록은 Iter 4 실행층 전환으로 그쪽에 일원화).
  - **분기 계수 정정 (Iter 4 F-1 — 기록 진실성 정정)**: 리뷰 처방은 형판을 **3분기**로 기술해 `:102`(`APPLIC_SURFACE_PRESENT`) 를 누락했다 (실 undercount) — 본 표는 4-값 전건을 승계하며, **형판 이름 귀속은 처방과 동일하다** (처방 원문은 RED 를 `APPLIC_UNDECIDABLE`(:103) 에, `APPLIC_NO_AC_SURFACE`(:100) 를 PASS 분기에 정확히 귀속했다 — dr2 원문 대조; 내구 기록 = Story §9.10 F-1 판정 [ephemeral scratchpad 앵커의 내구 앵커 교체 — Iter 5 DR4-8]). 직전 판이 이 자리에 적었던 "리뷰 처방의 형판-이름 오귀속을 실측 기각했다"는 취지의 서술은 **거짓 귀속**이라 제거한다. 오귀속의 발생 지점(chief 오독 vs 중계 왜곡)은 중계 packet 미보존으로 **확인 불가** (Story §9.10 판정 승계).
...
심사 완결의 분모는 Story §5.3 AC 표의 `coverage_required` 열이므로(§결정 4-1), 그 열의 **AFFIRM→NEGATE 전환 또는 열 자체 제거는 lane 내 판정으로 불가**하며 carrier/ADR 경유를 의무화한다 (§결정 8-4 "열거 ratchet 비대칭"을 분모 축에 확장 — 추가·NEGATE→AFFIRM 확대는 자유, **축소 방향만** 제약). 검출 술어 = **PR diff 상 §5.3 AC 행의 `coverage_required` 값이 §결정 4-1 D-1 정규화 기준으로 축소 방향(AFFIRM→NEGATE / lane 토큰 제거 / 열 삭제)으로 변화** (결정론). 입력면이 git diff 라 **day-1 non-vacuous** 하다 — `closed_axes` 류의 이력 의존 vacuity 가 없다는 점이 채택 근거다.

- **B안 = 확정 기각 — 라운드 간 `required_count` non-decreasing (근거 3항)**: ① **day-1 입력면 부재** — 전 라운드 값의 유일 입수원이 §9 verdict embed 인데 embed 보유 Story **11/591**, `required_count`·`closed_axes` 기존 emit **0** (산출 명령 = §결정 5 census 블록) → 원리적 무발화 = F1 이 고발한 vacuous class 재생산 ② **정상 정정에 거짓 RED** — 분모는 중복 AC 통합·오분류 정정·lane 별 적용분 차이로 **정당하게 감소**한다 ③ **역효과** — 방어책 자체가 AC 표 패딩(분모 과대 계상) 유인을 만든다. §9 분기 선택 표에 기각 행으로 등재.
- **강제 지점 정직 — 양방향 over-claim 금지 (chief firsthand 검증)**: `coverage_required` 는 §5.2 **derived 필드**로 ac-traceability 게이트의 §5-parse **Hop1 에서 재검증되지 않는다** (ADR-145:80 — "derived 필드 verification/coverage_required/phase = §결정6 파생, Hop1 미재검증"). 동 ADR:83 은 이를 "미강제 잔여가 아니다 — 계약(requirements-output-v1)·Hop2·review 층에서 실제 강제되며 … 강제 지점 이동, 미강제 아님" 이라 선언하는데, **그 선언을 그대로 인용하면 본 도메인에서 over-claim 이 된다** — 실측: 계약 층 강제의 실체는 `scripts/lib/ac_id.py:124` 의 **list format-only 검사 1건**이고, 그 검사는 `if "coverage_required" in record` 가드 뒤에 있는데 §5.3 markdown 경로의 record 는 `_scan_ac_table`(:248) 이 그 컬럼을 **추출하지 않으므로**(`validate_ac_record` 호출 = 동 파일 `:437`, 입력 record = `:295`/`:321` 산출물) 가드가 항상 거짓 → **그 경로를 아예 타지 않는다**. 따라서 정확한 서술은 다음 둘 다의 부정이다: "분모 축소 완전 무방비"(거짓 — ratchet + review 층 실재) / "다른 층이 강제하므로 안전"(거짓 — markdown 표는 format check 미도달). **실 강제 강도 = 본 조항 ratchet(PR diff 결정론) ⊕ 계약 층 format-only(record 입력 경로 한정) ⊕ review-tier semantic.**
- **C안 정직 declare — 등식이 재지 않는 두 번째 것**: 등식은 분모 축소를 못 잡는 데 더해 **분모 *구성의 타당성*도 재지 않는다** — "어떤 AC 가 이 lane 의 분모에 들어야 옳은가" 는 review-tier(declared) 이며 기계강제로 격상하지 않는다 (ADR-154 INV-5 ceiling).
...
- **2-control 상속 (ADR-154 §결정 5)**: 완결 판정 장치에 positive-control(분모 AFFIRM 행 1개를 examined 에서 의도 제거 → 반드시 RED, curated 1-mutant 상시) ⊕ internal-control identity-probe 를 건다 (측정 exit code 재사용 금지 — 파싱 결과 축).
  - **★ identity-probe 축 = 행 수 ⊕ 버킷 5-계수 (Iter 2 P1-2 채택 → Iter 4 consult 이의 #2 정밀화 — 처분의 역전이 아니라 실행층 정밀화)**: probe 가 `len(ac_rows)` 만 emit 하면 **D-1 정규화 함수가 통째로 죽어도 행 수는 보존**되어 미검출이다. 따라서 probe 축 = `len(ac_rows)` ⊕ **버킷 5-계수**(`affirm` / `negate` / `list_resolved` / `undecidable` / `absent`) ↔ sentinel 대조. **5-계수 근거 1줄**: 직전 판 4-계수는 정규화 enum 의 충실한 histogram 이 아니어서 `LIST_RESOLVED → AFFIRM` 붕괴가 행 수·4-계수 **양쪽에서 불변** = 미검출 (M16 blind spot) — 5-계수 정렬 시에만 M16 이 죽는다. 세부(붕괴 mutant·생존 assert)는 실행 SSOT (Change Plan §8 M13~M16 표).
  - **음성 대조군 의무 (축 확장의 정당화 조건)**: 정규화를 상수 반환으로 치환한 mutant 에서 버킷이 `(n,0,0,0,0)` 로 붕괴해 **불일치 RED** 가 나야 한다 — 같은 mutant 에서 행 수는 보존되므로 `len`-only probe 는 GREEN 이다. 이 대조군이 RED 를 내지 못하면 축 확장은 정당화되지 않는다 (존재-assert 단독 금지 형판).
...
- **★ lane 필터 의무 — cross-lane 오귀속 차단 (P1, TestContractArch consult)**: 현행 §3.2 술어의 grep 대상은 Story §9 **전체**이고 lane 필터가 없다. 이 상태로 4 lane 에 얹으면 (i) design debate 0 인 Story 에서 잔여 3 lane 이 항진 GREEN 이 되고, (ii) 반대로 design debate 가 있는 Story 에서는 **구현리뷰·보안테스트가 design lane 의 재발로 escalation 되는 cross-lane 오귀속**이 발생한다. 따라서 각 lane 의 재발 계수는 **그 lane 이 스스로 생산한 기록만** 정의역으로 한다 — 설계리뷰 = 자기 transcript heading / 잔여 3 lane = 자기 verdict `closed_axes[]` (Phase 2 델타 조건).
- **★ 현행 술어의 발화 실적 = 0 + EOL 앵커 취약 (P1, 정직 실측)**: heading 11건을 `sort | uniq -c` 하면 **전건 계수 1** 이고, 한 Story 가 2 heading 을 가진 사례(CFP-688)조차 서로 다른 anchor 다 → `>= 2` 조건은 **설계리뷰 lane 에서도 한 번도 발화한 적이 없다**. 원인 일부 = base:287 의 `<anchor_id>\s*$` **EOL 앵커**가 후행 괄호텍스트 heading(예: `F-DR-820-blanket (iter 1 FIX + iter 2 regression PASS)`)을 못 잡는다 — 실측 `grep -rhE '^### Debate transcript: [^ ]+\s*$' wrapper/stories/ | wc -l` → **7 / 11** (4건 미매치). Phase 2 델타에 **후행텍스트 tolerant 파싱 + 그 형상의 음성 fixture** 를 동반한다. 이 실측은 위 표의 "설계리뷰 = 도달 예"를 무효화하지 않는다(입력면이 실재하고 술어가 실행된다) — 단 **발화 실적 0** 을 declare 한다 (ADR-139 INV-L2: 미발화 = INCONCLUSIVE ≠ 재발 0).
- **★ AC-11 born-tension 기록**: AC-11 은 "기존 anchor_recurrence 술어의 정의역 **확대**로 착지"를 요구하는데, 그 술어의 입력면이 design 전용 산출물이라는 사실은 요구사항 시점에 알려지지 않았다. 규범을 설계리뷰 1 lane 으로 좁히는 정직 축소안은 **AC-11 문면 위반**이라 기각하고, 위 **2-표면 명시**(동일 판별 기준을 두 관측면으로 실현 — 신규 판별 체계 발명 0)를 AC-정합 해소로 채택한다. 대안(AC-11 재작성)은 요구사항 lane 재진입 비용이 과대하다.
...
- **required-host 충돌 해소 (이의 #3, A안 채택)**: 기존 host 확대 1순위(ADR-181 §결정 7)와 warning-first(ADR-171:172)가 required-tier host(`doc frontmatter/section schema`·`ac-traceability-matrix` — branch protection 8-tuple) 위에서 충돌한다. 해소 = required host 안에 **exit code 무기여 warning 서브체크**로 착지 — 신규 실패 모드는 **`scripts/lib/check_doc_section_schema.py` 의 `total_fails` 합산식**(현행 좌표 `:745` — `:746` 은 그 값을 소비하는 `if total_fails:` 이며 리뷰 지적 전 판의 `:746` 은 off-by-one 오기였다)에 **넣지 않는** 별도 warning 리스트로 분리(`::warning::` emit only). ★ 인용 정본 = **심볼명 + 파일**(`total_fails` in `check_doc_section_schema.py`) — 라인 리터럴은 후속 편집에 드리프트하므로 보조 표기다 (재현 명령 = `grep -n 'total_fails' scripts/lib/check_doc_section_schema.py`). 승격은 ADR-171 3-AND 별도 경로.
- **부속 전액 상속**: 신규/확대 검사 = ADR-181 §결정 5 admission 3항 + ADR-171 §결정 5 warning-first + 등록 4 필수 동반 + ADR-154 2-control + 5-piece chain(기존 host 확대 시 ①~④ 신규 0, ⑤ discriminating self-test + registry row 는 여전히 의무) + ADR-180 §결정 6 배선 형상 제약 5종. 확대 시 (가) 지정 mutant RED ∧ (나) 형제 site 회귀 0 실증 (ADR-181 §결정 7 승계). 두 admission 목록을 **전문 인용**한다 (부분 인용 = 상속 누락, 설계리뷰 v2 FIX Iter 2 / F5 정정):
  - **ADR-181 §결정 5 admission 3항** — ① `docs/evidence-checks-registry.yaml` 대응 entry 존재(tier = `warning` 로 태어남) ② 미건설 자산의 현재형 기술 금지(미래형 + carrier + 만기만) ③ `mechanical_enforcement_actions: []` 는 **carrier Issue 번호 + 만기일 주석을 그 키와 같은 물리 줄의 trailing 주석으로** 병기할 때만 적법.
  - **ADR-171 §결정 5 등록 4 필수 동반 (ADR-171:174 verbatim 실측)** — ① frontmatter `mechanical_enforcement_actions[]` entry(신규 lint carrier 시 — ADR-040 Amd3 §결정 7.A) ② `owner_adr`(정책 SSOT ADR) / `carrier_adr`(framework host) 분리 기재 ③ **self-application 무한루프 회피 판정** ④ 실 script + workflow + **discriminating self-test** 동반(Phase 분리 시 status 로 정직 표기). 본 ADR 의 D-contract·D-instance 두 확대는 Phase 2 착지이므로 registry row 의 status 필드에 그 사실을 정직 표기한다(②의 미래형 규율 정합).
...
- **Iter 4 집행력 라벨 재판정 (AC-23 실행층 전환)**: 정본이 Phase 2 실행 SSOT 로 이전된 축 — **D-1 분모 정규화 · 토큰 해소(구 축 C) · checklist 축 floor** — 의 라벨은 `mechanical floor` → **`declared`** 하향한다 (리뷰 F-3 이 구 축 C 라벨을 이미 반증 — 부분해소 GREEN 흡수 독법 실재). 승격 조건 = Phase 2 코드 + fixture + mutant 착지 ∧ **M13~M17 kill 실증** (M17 = Iter 5 추가 — DR4-6, ADR-171 3-AND 경로). 축 B 표면-존재 4분기 · examined 등식 · §결정 4-2a ratchet 등 **산문 골격이 자족 판정 가능한 항은 기존 라벨 유지**.

### 결정 12 — 경계 (disjoint/보완 선언)

1. **#2948 (리뷰 실행 파이프라인)**: 본 ADR 은 심사 자격과 종결 술어만 정의하며 리뷰 실행 채널을 지정·변경하지 않는다. #2948 파이프라인은 본 술어의 입력 생산자이고, 술어는 어느 생산 채널의 finding 에도 동일 적용된다. 파일면 실측 — wrapper cfp-2948 브랜치는 review-pl-base 미접촉(ADR 5개뿐), 실 경합 = internal-docs #2746 Change Plan 계획 문면(review-pl-base:101 §2 매트릭스 1행)으로 본 ADR 착지 절(§3 판정표 + 신규 `### lane PASS 종결 술어` 절)과 다른 절 — 선착 merge 후 rebase 정합.
...
- `scripts/lib/review_exit_criteria.py` + `tests/scripts/_review_exit_fixtures.py` + `tests/scripts/fixtures/review_exit_criteria/` — Phase 2 실행 SSOT (AC-23 실행층 전환 — D-1·토큰·checklist 세부 규칙 정본)

...
amendment_log:
  - amendment: 1
    date: 2026-08-17
    carrier_story: CFP-2995
    reinterpretation: false          # 결정 재해석 아님 — 기록된 실측 사실의 오류 정정
    summary: >-
      F-12 표의 JWT 행과 그 결론 문면을 정정한다. 종전 서술은 판별 변수를 서명 세그먼트 길이로
      단일 귀속했으나 실 semantics 는 최대 연속 토큰별 독립 판정 + 길이·엔트로피 논리곱의 존재
      양화다. 방향은 과비관(실제보다 커버리지를 나쁘게 기술)이라 안전 과대주장은 아니나, 이 행이
      잔여 회부 근거로 인용되고 있어 정밀화한다. 결정 2(차단 = secret-class 9룰) 판정은 무접촉.
    scope:
      - "F-12 표 JWT 행 2개 — 무서명·단서명 행 + 표준 HS256 행의 우발성 귀속"
      - "표 아래 첫 번째 번호 항목의 단일 원인 귀속 + 경계 서술"
      - "구 (ii) 재기술 문단의 비차단 열거 원소"
      - "별 파일 tests/scripts/test_bundle_pre_push_redaction.sh Part 5 천장 선언 1줄"
    residual_unchanged: [a, b, c, e]   # 잔여 declare ⓐⓑⓒⓔ 무변 (AC-16)
---
...
3. **③ 은 유일한 원문형 예외이며 그 사실을 여기서 명시한다** — ★ **단 ③ 의 '폐쇄' 는 저작 규율이지 기계 강제가 아니다**: 출처 `ADR-178:135` 는 구조 열거 + 4항 denylist 이지 텍스트 allowlist 가 아니고 구성요소에 "의미 단위 요약 1줄(추상)" 이라는 자유 산문이 있으며, 같은 절 `:136` 이 **"완화 전건 = 저작 규율(기계 강제 0)"** 을 자기선언한다. ⇒ **③ 에 대한 AC-31 의 기계 보장 = 0 이며 ③ 내용 통제는 AC-32 단독에 위임**된다. — 아래 (b) 채택으로 ⑩ 이 신설되어 진행 노트가 참조형으로 내려갔고, 남은 원문형은 ③ 1건뿐이다. ③ 은 이미 ADR-178 §결정 6-4 가 값공간을 폐쇄한 필드라 자유 본문이 아니다.
   ...
   **구 (ii) 는 흡수되지 않고 존치**: "닫힌 14-rule **밖** PII 미커버(전화번호·주소·실명·카드번호 등)" 는 신 (iii) 과 **별개 사실**이다. 신 (iii) 은 rule **안**의 2종이 차단 축에서 빠졌다는 것이고, 구 (ii) 는 rule **밖**은 애초에 안 본다는 것이다. ⇒ PII 축은 **밖은 미커버 · 안은 비차단**이라 어느 쪽에서도 차단 기여가 없다.
   ...
   | JWT 계열 입력 전반 — 표준 · 서명 절단 · `alg=none` 무서명을 포함한 모든 변형. 정의역은 다른 룰의 문맥 표지 없이 토큰열이 단독으로 주어진 입력이다 | 탐지기 문자 클래스의 최대 연속 토큰을 각각 독립 판정 (점은 그 클래스 밖이라 입력은 점 경계에서 갈리며 표준 JWT 는 세 토큰, 서명 세그먼트가 비면 두 토큰이다) — 길이 하한 이상이면서 엔트로피 하한 이상인 토큰이 하나라도 있으면 그 토큰에 대해 `cloud_key`, 하나도 없으면 없음 | `cloud_key` 가 발화한 경우에 한해 `cloud_key`, 그 밖에는 없음 | **그런 토큰이 하나라도 존재하면 차단, 하나도 존재하지 않으면 통과 (룰 0 발화). 헤더 문맥 · 자격증명 키 문맥 · env-dump 전체 제외 문맥에 감싸인 입력은 이 조건과 무관하게 그 문맥 룰이 선행 발화해 탐지·차단되며 그 판정은 아래 두 행과 전체 제외 규칙이 말한다** |
   | JWT — 표준 HS256, 서명 43자 | `cloud_key` | `cloud_key` | 차단 (위 행 술어의 실례 — 문맥 표지 없는 단독 입력이고 payload 토큰이 두 조건을 충족한다. **우발성의 소재는 서명이 아니다** — 아래 ①) |
   | `token: <JWT>` | `api_key_credential` · `cloud_key` | 둘 다 | 차단 |
   | `Authorization: Bearer <JWT>` | `authorization_header` | 해당 | 차단 |

   ⇒ 두 사실을 분리해 적는다.
   ① **표준 JWT 는 차단된다 — 그러나 JWT 를 아는 룰이 있어서가 아니다.** `_RE_CLOUD_GENERIC` 의 문자 클래스에 점이 없어 입력은 **최대 연속 토큰**으로 갈리고(표준 JWT 는 header · payload · 서명 세 토큰이고 서명 세그먼트가 비면 두 토큰이다) **각 토큰이 독립으로** 판정된다. 발화는 **존재 양화**다 — 어떤 토큰이든 하나가 `len >= CLOUD_GENERIC_MIN_LEN` **∧** `_shannon_entropy >= CLOUD_ENTROPY_MIN` 을 충족하면 그 토큰이 치환되어 `cloud_key` 가 발화하고, 그런 토큰이 **하나도 없으면 룰 0 발화**다. 최댓값 선택이 **아니다** — 가장 긴 토큰이 엔트로피 하한에 미달해도 다른 토큰이 두 조건을 함께 충족하면 발화한다. ⇒ 종전의 「HS256 서명 세그먼트가 걸릴 뿐」과 「서명 40자에서 발화 시작」은 **둘 다 거짓**이다: 표준 JWT 는 payload 토큰만으로 두 조건을 충족하므로 **서명이 0자여도(`alg=none`) 발화한다.** 판별 변수는 서명 길이가 아니다. 단 「표준 JWT 의 payload 가 길이 하한을 넘는다」는 **RFC 적합성 명제가 아니라 현실 claim set 에 대한 경험적 명제**다 — RFC 7519 상 모든 claim 이 OPTIONAL 이고 `{}` 도 유효한 Claims Set 이다. **그리고 이 술어의 정의역은 다른 룰의 문맥 표지 없이 토큰열이 단독으로 주어진 입력이다** — 헤더 문맥 · 자격증명 키 문맥 · env-dump 전체 제외 문맥에 감싸인 입력은 그 문맥 룰이 **선행 발화해 탐지·차단**되며 위 토큰 조건이 그 판정을 좌우하지 않는다.
   ...
   ⇒ 구 (ii) 를 "밖 = PII" 로 읽으면 안 된다. 정정된 문면: **밖 = PII 축 전량 + secret class 중 URL basic-auth 자격증명 · raw hex secret**. **JWT 는 이 열거에서 제거한다** — JWT 의 차단 여부는 서명 길이가 아니라 최대 연속 토큰별 독립 판정으로 갈리며, 문맥 표지 없이 단독으로 주어진 입력에서 어떤 토큰이든 하나가 길이 하한 이상이면서 엔트로피 하한 이상이면 `cloud_key` 로 차단되고 그런 토큰이 하나도 없으면 룰 0 발화다(위 ① 참조). 헤더 문맥 · 자격증명 키 문맥 · env-dump 전체 제외 문맥에 감싸인 입력은 그 문맥 룰이 선행 발화해 탐지·차단되므로 이 열거 밖이다. 그리고 **안에 있는 것 중 일부는 우발적 길이 매칭에 기대고 있다**. 이는 완화가 아니라 **declare** 다 — 아래 잔여 회부 참조.
...
## Amendment 1 — F-12 JWT 서술 정정 (CFP-2995)

**자기적용 규약 (본 Amendment 를 구속한다)**: 본 Amendment 는 CFP-2995 §4.5 열거 규칙의 정의역
안에 있다 — JWT 어휘와 비탐지 주장 어휘를 **같은 줄에** 담는 순간 그 줄 자신이 정정 대상 사이트가
되어 전역 술어 의무를 진다. ⇒ 본 Amendment 는 (i) 종전 거짓 문면을 **verbatim 재생산하지 않고**
(ii) 정정 대상의 **재위치 앵커 문자열을 재생산하지 않으며**(재생산하면 앵커 유일성이 깨진다)
(iii) 두 어휘를 함께 담는 줄에는 전역 술어 전문을 싣는다.

**인용 규약**: 좌표가 아니라 구조 단위로 식별한다 — 본 파일은 매 amendment 마다 frontmatter 가
자라 본문 전체가 하류로 이동하므로 라인 인용이 성립하지 않는다(선례 = ADR-147 Amendment 5).

**정정 형태 = 본문 in-place 정정 + 본 섹션의 변경 기록**. append-only 로는 정정이 성립하지 않는다 —
거짓 문장이 원 위치에 남으면 그 위치를 읽는 독자와 검사기가 여전히 거짓을 읽는다. ADR-058 §결정 11
이중 게이트 충족: (가) silent 삭제 금지 = 본 섹션이 대상·변경 내용을 전수 열거 (나) 사용자 명시
비준 = CFP-2995 §1 요구 4 가 정정을 직접 지시. 결정-사실 복구가능성 = git history.
효력 있는 결정 텍스트(결정 2)는 **비대상**.

**정정 대상 4 + 인접 1**

1. **F-12 표의 무서명·단서명 JWT 행** — 셀 단위 정정. 표 행 전체를 한 줄로 인용하면 셀 구분자가
   인용문에 들어가 표가 깨지므로 셀 단위로 적는다.
   - 셀 1 (페이로드) — 입력 부류를 JWT 계열 전반으로 올리되 **정의역을 다른 룰의 문맥 표지 없이
     토큰열이 단독으로 주어진 입력으로 한정**한다. 종전 셀 1 은 미발화 조건을 서명 길이로 단일
     귀속했고 그것이 거짓이었다.
   - 셀 2 (발화 룰) — 최대 연속 토큰별 독립 판정으로 재서술. 길이 하한과 엔트로피 하한을 함께 넘는
     토큰이 하나라도 있으면 그 토큰에 대해 `cloud_key`, 하나도 없으면 없음.
   - 셀 3 (차단 교집합) — 셀 2 의 발화 여부에 종속.
   - 셀 4 (판정) — 조건부 판정으로 재서술: 그런 토큰이 하나라도 존재하면 차단, 하나도 존재하지
     않으면 통과. 그리고 **헤더 문맥과 자격증명 키 문맥과 전체 내용 제외 문맥에 감싸인 입력은 그
     문맥 룰이 선행 발화해 탐지·차단되며 위 토큰 조건이 그 판정을 좌우하지 않음**을 함께 싣는다.
   ⇒ **셀 2·3·4 를 무변으로 두면 행이 하나의 술어가 되지 못한다.** 셀 1 만 전역화하면 조건을 충족하는
   입력에 대해 셀 1 은 발화를 함의하는데 셀 4 는 통과를 단언해 **정면 모순**이 된다.
   ⇒ **정의역 한정을 문면에서 빼면 형제 행이 다루는 입력까지 이 행이 지배하게 되어 다시 거짓이 된다** —
   근거 범위가 주장 범위를 덮어야 한다는 요구가 이 행에도 그대로 걸린다.
2. **F-12 표의 표준 HS256 행 (인접 결함, 동반 정정)** — 판정 셀의 「우발적」 귀속처가 틀렸다. 43자
   서명이 없어도 차단되므로 우발성의 소재는 서명이 아니라 payload 다. 1 의 정정이 이 행을 **그 술어의
   실례**로 만들므로 그 관계를 명시하고 잘못된 귀속을 제거한다. 이 행을 **표 전체의 일반 규칙으로
   승격시키지 않는다** — 승격은 형제 행이 다루는 입력까지 한 술어로 덮어 거짓을 만든다.
3. **표 아래 첫 번째 번호 항목** (표준 JWT 의 차단 근거를 서술하는 문단) — 단일 원인 귀속과 경계
   서술을 존재 양화 술어로 교체. 최댓값 선택 가설의 명시적 부정과 정의역 한정을 포함한다.
4. **같은 절의 구 (ii) 재기술 문단** — 비차단 열거에서 JWT 원소를 제거하고, 그 자리에 JWT 의 차단
   여부를 결정하는 술어와 그 정의역을 둔다. 원소만 지우면 그 문단이 JWT 탐지 여부를 더는 진술하지 않게 된다.
5. **별 파일 `tests/scripts/test_bundle_pre_push_redaction.sh` 의 Part 5 천장 선언 1줄** — 같은
   오류가 self-test 주석에 복제돼 있다. 같은 술어로 교체하며 ADR 참조는 남긴다.

**실행 대조 (AC-14)**: 위 정정문이 함의하는 탐지 여부는 CFP-2995 §4.5 축 고정 표의 입력 부류
전건과 **문맥 래핑 3 부류의 조건충족·미달 쌍**에 대해 실 탐지기 실행 결과와 일치하며, 어느 입력에서도
함의가 미결정으로 남지 않는다. 문맥 부류가 정의역에 없으면 정의역을 넓히는 서술이 그 자리에서
검증되지 않으므로, **입력 집합은 서술이 주장하는 범위를 덮어야 한다.**

**양상 한정 (AC-15)**: 「표준 JWT 의 payload 가 길이 하한을 넘는다」는 RFC 적합성 명제가 아니라
현실 claim set 에 대한 경험적 명제다. 구조적 필연으로 서술하면 같은 종류의 부정확을 반대 방향으로
재도입한다.

**임계 수치 비고정**: 정정문은 길이·엔트로피 하한을 **상수 이름**으로만 지시하고 수치 리터럴을 담지
않는다. 상수는 tunable 이며 수치를 문면에 박으면 다음 조정에서 문면이 다시 거짓이 된다.

**호출부 완화의 검증 상태 (정직 기술)**: 종전 「호출부 완화」 단락이 인용한 `PARSE_TIMEOUT_S` 포화
가드는 CFP-2995 이전까지 이를 실행시키는 테스트가 없었다 — 미검증 완화가 천장 주장을 떠받치고 있었다.
CFP-2995 가 결정론적 주입으로 가드 발동을 실증하고 대조군을 세웠다. 추가 실측: 실 입력으로도 포화가
도달 가능하다(약 124 KB 의 역추적 유발 입력이 `BYTE_CAP`·`LINE_CAP` 어느 쪽도 건드리지 않고 timeout 을
트립). 단 실 포화 경로는 머신 속도 의존이라 결정론적 게이트로 부적격이며 게이트는 주입 대역을 쓴다.

**잔여 declare — 해소 아님 (AC-16)**: ⓐ 엔진 `audit` 의 열화 미보고 · ⓑ 비-L1 소비자(capture-time
hook 경로) 미수혜 · ⓒ rule 단위 해상도 부재 · ⓔ 신 (iii) — **전건 무변**. ⓓ(정상 완료도
`undecidable` 로 계상되는 가용성 비용)는 **발생이 테스트로 관측 가능해졌을 뿐** 설계 선택 자체는
잔존한다. ⇒ 본 Amendment 가 해소한 것은 **F-12 서술의 정확성 + ⓓ 의 관측 가능성**이며 그 밖은 없다.

**결정 2 무접촉**: 차단 판정을 secret-class 9룰로 협착한 판정은 바뀌지 않는다. `SCAN_BLOCKING_RULES`
와 탐지기 정규식은 CFP-2995 에서 byte-identical 로 동결되며 AC-12 가 이를 기계 보존한다.

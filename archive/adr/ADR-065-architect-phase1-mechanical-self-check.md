...
related_stories:
  ...
  - CFP-1565  # Amendment 6 carrier — 12th item commit-time 선제-lint 에 claude-md-line-cap 추가 (F-005 evidence: CFP-1522 retro CLAUDE.md 322>320 line-cap 초과 미사전검출). Amendment 4 row 10 선제-lint family 확장 (check-doc-section-schema + check-adr-sunset-criteria → + check-claude-md-line-cap)
related_adrs:
  ...
  - ADR-012   # CFP-1565 cross-ref — Amendment 1 §결정 6 CLAUDE.md line-cap ≤320 SSOT (row 12 commit-time 선제-lint check-claude-md-line-cap.sh source)
  - ADR-068   # CFP-1565 cross-ref — sibling carrier same Story (Amendment 5 I-7 chief-author cross-ADR claim consistency, ADR-065 §결정 1 row 12 = commit-time 선제-lint family 확장 sibling)
amendments:
  - amendment: 1
    date: 2026-05-15
    cfp: CFP-685
  ...
  - amendment: 6
    date: 2026-06-01
    cfp: CFP-1565
    summary: "§결정 1 표(11-row) 에 row 12 append — Phase 1 산출물 commit 직전 CLAUDE.md 변경 시 `bash scripts/check-claude-md-line-cap.sh` 로컬 선제 실행 (PASS 확인). Amendment 4 row 10 선제-lint family 확장 (check-doc-section-schema.sh + check-adr-sunset-criteria.sh → + check-claude-md-line-cap.sh). F-005 evidence: CFP-1522 retro — CLAUDE.md 322>320 line-cap 초과 가 commit 직전 chief author self-detect 안 되고 사후 CI warning 도달. CLAUDE.md line-cap ≤320 SSOT = ADR-012 Amendment 1 §결정 6 (CFP-506). §결정 11 신설 — Amendment 6 narrative (F-005 incident reference + Amendment 4 선제-lint family 정합 + cascade obligation invariant + ADR-068 Amendment 5 sibling carrier same Story). §결정 1 row 1-11 본문 변경 0 (row 12 append 만), §결정 2-10 변경 0. mechanical_enforcement_actions[] = 기존 `claude-md-line-cap` evidence-check entry (CFP-506 / ADR-060 framework, warning tier) 의 commit-time 선제 실행 cross-ref only — 신규 evidence-checks-registry entry 0건 (기존 안전망이 cover). ratchet 강화 방향만 (11→12 ratchet 확장 — 12th item 제거 / claude-md-line-cap cross-ref 해제 발의 차단). doc-only fast-path ADR-054 (src/tests 무변경)."
    is_transitional: false
    sunset_justification: "N/A — permanent policy 의 ratchet 강화 (Amendment 1/2/3/4/5 family pattern 정합). ADR-064 §self-application top-down ratchet 정합. 약화 방향(12th item 제거 / claude-md-line-cap 선제-lint cross-ref 해제) 발의 차단."
mechanical_enforcement_actions:
  ...
  - action: claude-md-line-cap
    status: existing-warning-cross-ref
    progress_note: "ADR-065 Amendment 6 (CFP-1565) 신설 시점 — 신규 evidence-checks-registry entry 0건. 기존 `claude-md-line-cap` check (CFP-506 / ADR-012 Amendment 1 §결정 6 / ADR-060 framework, `scripts/check-claude-md-line-cap.sh`, warning tier) 의 commit-time 선제 실행 cross-ref only. 본 Amendment 6 이 row 10 (Amendment 4) 의 선제-lint family (check-doc-section-schema.sh + check-adr-sunset-criteria.sh) 에 row 12 = check-claude-md-line-cap.sh 추가 — chief author 가 CLAUDE.md 변경 PR commit 직전 line-cap ≤320 self-detect. 신규 lint script / 신규 workflow yml / 신규 evidence-checks-registry entry 0건 (기존 안전망이 cover). status 승격 trigger 없음 — `existing-warning-cross-ref` (별도 CFP 가 claude-md-line-cap tier 승격 발의 시에만 status 갱신). row 12 = behavioral mandate (chief author commit-time manual 선제 실행)."
    target_section: §결정 1 row 12 (Amendment 6 CFP-1565) / §결정 11 (신설)
---
...
| 12 | Phase 1 산출물 commit 직전 CLAUDE.md 변경 시 `bash scripts/check-claude-md-line-cap.sh` 로컬 선제 실행 (PASS 확인) — Amendment 4 row 10 선제-lint family 확장 (Amendment 6 CFP-1565) | CLAUDE.md 변경 PR 시 commit 직전 line-cap ≤320 self-detect (`bash scripts/check-claude-md-line-cap.sh` exit 0 PASS 확인). 초과 시 다른 단락 압축으로 상쇄 후 commit (CLAUDE.md line-cap ≤320 SSOT = ADR-012 Amendment 1 §결정 6 / CFP-506). 사후 CI warning 도달 전 chief author self-detect (F-005 CFP-1522 retro evidence — 322>320 미사전검출). CLAUDE.md 무변경 PR = NA 명시적 PASS |
...
codeforge-design `templates/change-plan.md` (현 `plugins/codeforge-design/templates/change-plan.md`, 구 plugin-codeforge-design repo 삭제됨 2026-06-12) 에 `§13. Phase 1 산출물 self-check 결과 (ADR-065 / CFP-438)` 섹션 추가. chief author 가 7 항목 결과 (PASS / NA / FIX) 를 명시 의무.
...
### 결정 11 — 12th item CLAUDE.md line-cap commit-time 선제-lint (Amendment 6, CFP-1565)

§결정 1 표(11-row, post-Amendment-5) 에 row 12 append 로 chief author self-check 항목을 12개로 확장. 신규 12번째 항목 = **"Phase 1 산출물 commit 직전 CLAUDE.md 변경 시 `bash scripts/check-claude-md-line-cap.sh` 로컬 선제 실행 (PASS 확인)"** — Amendment 4 row 10 선제-lint family 확장 (check-doc-section-schema.sh + check-adr-sunset-criteria.sh → + check-claude-md-line-cap.sh).

#### 11.1 F-005 Incident reference (CFP-1522 retro)

CFP-1522 작업이 CLAUDE.md 를 322 line 으로 증가 — `claude-md-line-cap` warning tier (CAP=320, ADR-012 Amendment 1 §결정 6 / CFP-506) 초과. chief author 가 commit 직전 self-detect 하지 못하고 사후 CI warning 도달. row 10 (Amendment 4) 의 선제-lint family (`check-doc-section-schema.sh` + `check-adr-sunset-criteria.sh`) 는 ADR/doc section schema + sunset criteria 만 cover — CLAUDE.md line-cap 영역 미포함. row 12 신설이 이 gap 보완.

**핵심 gap**: warning tier CI 가 사후 감지 channel — chief author 의 **commit-time** forcing function 부재. row 12 신설이 line-cap 영역으로 선제-lint family 확장.

#### 11.2 Amendment 4 선제-lint family 정합

Amendment 4 row 10 = 선제-lint behavioral mandate (touched ADR/doc 에 두 lint 로컬 선제 실행). 본 Amendment 6 row 12 = 동일 family 의 CLAUDE.md line-cap 영역 확장 — behavioral mandate only (chief author commit-time manual 선제 실행). 두 row 모두 사후 CI 도달 전 self-detect 목적.

#### 11.3 CLAUDE.md line-cap SSOT

`bash scripts/check-claude-md-line-cap.sh` = ADR-012 Amendment 1 §결정 6 (CFP-506) CLAUDE.md ≤320 line-cap warning tier lint. exit 0 PASS / exit 1 cap 초과 / exit 2 meta-error. 초과 시 다른 단락 압축으로 상쇄 후 commit (line 수 net-zero 또는 감소 방향).

**Cascade obligation**: CAP 값 (현재 320) 이 ADR-012 Amendment 1 §결정 6 에서 변경되는 PR 은 본 row 12 wording 영향 영역 — script 내 CAP 상수가 SSOT. cascade 검출 = manual review 의존 (별도 follow-up CFP carrier).

#### 11.4 mechanical_enforcement_actions[] = 기존 claude-md-line-cap cross-ref

신규 evidence-checks-registry entry 0건. 기존 `claude-md-line-cap` check (CFP-506 / ADR-060 framework, warning tier) 의 commit-time 선제 실행 cross-ref only (Amendment 4 의 `existing-warning-cross-ref` 패턴 정합). 신규 lint script / 신규 workflow yml / 신규 mechanical action name 0건.

#### 11.5 ADR-068 Amendment 5 sibling carrier (same Story)

본 Amendment 6 = CFP-1565 sibling carrier — ADR-068 Amendment 5 (I-7 chief-author cross-ADR scope/fact claim consistency invariant) 와 동일 Story Phase 1 PR. 두 Amendment 모두 chief author commit-time self-check 강화 axis (ADR-068 = cross-ADR claim verify / ADR-065 = CLAUDE.md line-cap 선제-lint) — disjoint axis, same forcing function lifecycle (chief author Phase 1 commit 직전 self-check). CFP-1522 retro F-003 (ADR-068 Amendment 5 evidence) + F-005 (본 Amendment 6 evidence) sibling.

#### 11.6 row 1-11 본문 변경 0 invariant + sunset_justification

§결정 1 row 1-11 본문 변경 0 (row 12 append 만). §결정 2-10 변경 0. Amendment 1/2/3/4/5 family pattern 정합 — additive ratchet only. `is_transitional: false` 보존 + `sunset_justification: "N/A — permanent policy 의 ratchet 강화..."` quoted-string-form 의무. 약화 방향(12th item 제거 / claude-md-line-cap 선제-lint cross-ref 해제) 발의 차단 (ADR-058 §결정 5 / ADR-064 §self-application top-down ratchet).
...
### Amendment 6 (CFP-1565) 신설/변경 파일

- **이 ADR 본문** (`docs/adr/ADR-065-architect-phase1-mechanical-self-check.md`) — frontmatter `amendments[]` amendment 6 entry + `mechanical_enforcement_actions[]` `claude-md-line-cap` entry (existing-warning-cross-ref) + `related_stories` CFP-1565 + `related_adrs` ADR-012·ADR-068 + 본문 §결정 1 표 row 12 + §결정 11 narrative section + 본 sub-section
- [`scripts/check-claude-md-line-cap.sh`](../../scripts/check-claude-md-line-cap.sh) — **무수정** (기존 CFP-506 / ADR-012 Amendment 1 §결정 6 lint, row 12 cross-ref target)
- [`docs/adr/ADR-068-boundary-completeness-invariants.md`](ADR-068-boundary-completeness-invariants.md) — sibling carrier (Amendment 5 I-7 chief-author cross-ADR claim consistency, same Story CFP-1565)

**신규 lint script / 신규 workflow yml / 신규 ADR / 신규 evidence-checks-registry entry / 신규 mechanical_enforcement_actions[] action name = 0건** (기존 `claude-md-line-cap` evidence-check entry commit-time 선제 실행 cross-ref only — 기존 안전망이 cover). plugin.json bump = ADR-068 Amendment 5 sibling carrier (same Story PR) 와 atomic 처리.

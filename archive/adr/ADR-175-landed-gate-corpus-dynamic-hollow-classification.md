...
**★ opt-in 취소 계상 (W — 수령 3항. 선언 3 = 열거 3. ★`W-2`·`W-3` 은 *미수령 번호* 이며 결손이 아니다 — 원 packet 번호를 보존한 것이고 그 번호로 명명된 술어는 양 문서에 존재한 적이 없다[전역 0 hit]. 재번호 대신 번호 보존을 택한 이유 = 수령 packet 대조 가능성 유지. 설계리뷰 DR2-M11)**: **W-1** `ever_declared` high-water mark(취소가 **D-2 를 자동 트립**) · **W-4** 취소는 하한을 면제하지 않는다(**두 겹 차단**) · **W-5** `withdrawn[]` **판정기 미투입**(reconciler 전용).

...
- ★**mandate 편차 정직 기재**: 본 계약의 초판은 설계 lane 6 permanent deputy 중 **4 미수령** 상태에서 통합됐고, 그 결과 **회부한 축에서 실제로 P0 가 발생**했다. 1차 개정이 APIContractArchitectAgent 수령분(판별자 계약 · sidecar manifest — **실행 재현 기반**)을, 2차 개정이 **ModuleArchitectAgent**(corpus 경로·형상·dependency direction) + **InfraOperationalArchitectAgent**(운영 리스크 6-sub · §11.6 멱등)를 반영했다. ⇒ **4 미수령 중 3 수령·반영**, 잔여 **1 = TestContractArchitectAgent**(대조군 blinded 섭동 · arm 별 분모 하한 분해 · arm-L 표본 타당성)이며 **후속 수령분으로 반영**한다. ★**미수령이 실제로 P0 를 낳았다는 사실은 사후에도 확증됐다** — ModuleArch 수령분이 **required 게이트 정의역 침범**(`check_ac_traceability_matrix` 가 corpus `.py` 심볼을 편입해 AC↔named-test 판정을 fail-open 시키는 경로 — Change Plan MECH-8)을, InfraOp 수령분이 §결정 10 리터럴 근거의 오류를 각각 반증했다. *"수령 전 통합" 은 회부 표기로 갈음되지 않는다.*

...
- ★★**축 정합 구속(IC-6 축 짝짓기)의 잔여 — 3항 (설계리뷰 DR3-M8 신설. ★본 항은 §8.QC-MECH MECH-9 와 동일 문면을 보유한다 — 천장 동시-변경 불변식)**
  - ⓐ **corpus 착지 전 강제 게이트 0** — 배선면의 *"⑤ baseline sanity 로 기계 관측된다"* 는 **검출** 주장이지 **착지 전 정적 강제** 주장이 아니다. **검출 시점 = corpus 최초 실행**이며, 그전까지 축 어긋난 pairing 은 문서 규약으로만 막힌다.
  - ⓑ ★**그 검출은 축 *불일치* case 한정이다. 축이 *일치* 하는 정상 경로는 오히려 `I-11` 을 부른다** — **축 짝짓기 성립 ≡ arm-H 의 kill·clean 관측 동일**이고, 그것이 §결정 4 ⑤ `I-11` 협착(DR3-M2)의 근거였다. ⇒ 배선 문면을 *"축을 맞추면 안전하다"* 로 읽으면 실측과 반대다 — 축을 맞추는 것은 **필요**하되 그 자체가 판정 안전을 주지 않으며, 안전은 협착된 `I-11` 문면이 준다.
  - ⓒ **협착 후 arm-H 축의 잔여 challenge 는 `I-7`(`DELIVERED`) 단일 축에 집중**된다(§결정 4 ⑤ 약화 축 기재의 승계). day-1 에서 그 단일 축이 성립하는 근거는 **terminal-marker 편입 후의 kill ↔ empty ① 갈림 하나**이므로, 그 갈림이 사라지는 게이트에서는 arm-H 판정이 성립하지 않는다.
  - ★**일반화 금지** — 위 ⓑ 는 **HGSV + H-1 조합에서 실행 확증**된 것이다. 다른 게이트에서 terminal 문면이 중화 축에 의존하면 kill·clean 이 갈려 `I-11` 이 애초에 발동하지 않을 수 있다.

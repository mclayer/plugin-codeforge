...
related_adrs:
  ...
  - ADR-163  # measurement channel architecture (sibling — privacy concern 분리)
...
**Accepted (2026-05-09)** — carrier_story = CFP-283. ADR-163 sibling Phase 1 PR. Privacy policy SSOT 단일화 — narration (ADR-029) + ledger (ADR-163) sanitize 통합.
...
- **Privacy = cross-cutting concern**. ADR-163 architectural decision + ADR-043 policy decision 분리가 maintainability ↑.
...
4. **Cross-host telemetry 통합 정책 부재** — wrapper-vs-consumer ledger isolation (ADR-163 §결정 9) 부재 시 raw event leak 위험.
...
위반 시 (default false 인데 ledger write 발생) = `policy_violation` (defect). lint = TestContractArch §8.4 후보 5 (opt-in default false invariant — ADR-163 §검증 채널 cross-ref).
...
**Capture 시점 sanitize = Allow-list ONLY**. stop-event-v1 schema field (ADR-163 §결정 2 enumerated + CFP-1744 Amendment 1) 외 field capture 금지:
...
추가 field capture = MINOR change (optional field) 또는 BREAKING change (필수 field) → ADR-163 amendment 의무 + 본 ADR-043 §결정 2 갱신 의무 (cross-ref). optional field 추가 = MINOR (ADR-008 §결정 2 정합 — v1.0 reader 가 skip 가능).
...
### 대안 A — Privacy 정책을 ADR-163 §결정 5-6 inline (별도 ADR-043 미신설)
...
- ADR-163 = architectural decision, ADR-043 = policy decision — SSOT 분리가 maintainability ↑
...
- ADR-163 §결정 11 ROI gating 패턴 정합 (30+ post-merge-counters run 후)
...
2. **Allow-list ONLY enforcement lint** — stop-event-v1 schema 18 field 외 추가 시 lint FAIL (BREAKING change → ADR-163 + ADR-043 amendment 의무).
...
5 후보 모두 = follow-up CFP (Phase 2 enforcement 도입 시 동반 land — 분리 land = partial enforcement 의미 없음, Phase 1 trust model 정합 — ADR-163 §검증 채널 SSOT 정합). 후속 CFP 번호 = post-merge-counters.jsonl 30+ run ROI 평가 후 발의 시 할당 (current 미배정).
...
- Telemetry hook 구현 (sqlite write) — ADR-163 §결정 12 deferred
- Token attribution model — ADR-163 §결정 3 spawn-event 보류 와 동반 deferred
...
- **ADR-163** (measurement channel architecture) — **sibling Phase 1 PR**. ADR-163 §결정 5-6 verbatim cross-ref. architecture vs policy 분리.
...
- `docs/domain-knowledge/orchestrator-discipline/measurement-channel.md`
- `docs/adr/ADR-163-codeforge-measurement-channel-architecture.md` (sibling)
...
- `hook_decision`: enum `{"record-only"}` optional (non-sensitive) — non-blocking marker. ADR-115 §결정 2 block 금지 binding constraint. closed-set 1-value, 확장 시 별도 CFP + 본 Amendment 갱신 의무.
...
- **명시 경계**: 본 Amendment 는 **stop-event runtime 의 raw session_id bug 수정을 위임하지 않는다** — 그것은 별도 follow-up (3문 게이트 통과 시 발의). 본 Amendment 는 spawn-event 가 sha256 을 쓴다는 것만 mandate.
...
- **privacy = cross-cutting concern** (본 ADR 컨텍스트) — spawn-event 가 동일 정책 inherit 가능하도록 ADR-163 와 분리한 설계 의도의 실현 첫 사례.
...
- §결정 2 stop-event-v1 18 field Allow-list 무변경 (spawn-event 는 별도 channel Allow-list).
...
- stop-event-v1 runtime raw session_id bug = 본 Amendment 미위임 (별도 follow-up).
...
- §결정 2 stop-event-v1 18 field + spawn-event-v1 19 field Allow-list 무변경 (self-context = 별도 record type, allow-list 확장만).
...
- **wrapper-self dogfood scope = always-on** — codeforge family 자기 개발 계측이 Story 목적. always-on = **checkout-identity 파생**(consumer overlay 의 user-settable bool 아님). ADR-163 §결정 6 "wrapper dogfood always-on = Phase 2 follow-up" 를 CFP-2687 이 carrier.
...
- §결정 3 의 **deny-regex pattern 집합 자체는 무증감**이다 — 본 (C) 는 적용 순서와 매치 해석만 규정한다. **기수를 리터럴로 적지 않는다** (설계리뷰 2회차 파생 수치 전수 스캔 산출) — 정본 = §결정 3 표, 재현 = `awk '/^### 결정 3 —/,/^\*\*Match → redact\*\*/' <본 파일> | grep -cE '^\| [^-|]'` **에서 헤더 1행을 뺀 값**.
  - **★ 승계된 계수 불일치 정직 declare (본 Amendment 가 처음 검출)**: 위 재현식을 실제로 실행하면 §결정 3 표의 pattern 은 **6종**(API key/credential · GitHub PAT · GitHub fine-grained PAT · 한국 주민번호 · Email · Hex≥32)이다. 그런데 본 파일에는 `deny-regex 확장(6→7)`(Amendment 5 · CFP-2850)과 `deny-regex 7종 무변경`(Amendment 6 · CFP-2985)이 남아 있고 **§결정 3 표에는 7번째 행이 실재하지 않는다**. 본 Amendment 초판은 그 `7종` 을 **검증 없이 3 site 에 승계**했다 — 리터럴 계수는 이렇게 amendment 사슬을 타고 번진다. 처분 = **본 Amendment 착지분에서만 리터럴 제거**(위). **선행 amendment 문면은 바이트 무접촉** — 타 Story(CFP-2850 / CFP-2985) 소유 구조라 본 Amendment 의 정의역 밖이며(`## Amendment 2` 무접촉 규율 동형), 정정은 별 carrier 가 소유해야 한다. **본 declare 는 불일치를 해소하지 않는다 — 관측·귀속만 한다.** 실 통제(집행되는 pattern)는 §결정 3 표 6종이며 본 채널은 어차피 폐쇄 어휘라 deny-list 가 원리적 no-op 이므로 **본 Amendment 의 privacy 결론에는 영향이 없다.**
...
- §결정 3 deny-regex **pattern 집합 무변경** (본 Amendment 는 순서·해석만 규정, pattern 증감 **0**). 기수는 §결정 3 표가 정본 — 위 (C) 의 승계 계수 불일치 declare 참조.

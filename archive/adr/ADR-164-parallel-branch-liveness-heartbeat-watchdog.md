...
### §결정 6 — D4 lane 단위 stale 임계 baseline (수치 전부 proposal) + 예상-idle ≠ hung
...
**참조 (기존 — 본 ADR 이 제약·상속하는 자산)**

- `archive/adr/ADR-139-background-wait-liveness-gate.md` — 가장 가까운 선행. liveness ⊥ granularity + INV-L1/L2/L4. 본 ADR 이 observer-death 케이스로 확장하는 기점.
- `archive/adr/ADR-155-dev-process-observability-substrate.md` — heartbeat 가 재사용하는 CODE/PATTERN(agent-emit 스타일 · 2-layer local-SoT+projection) 출처. SCHEMA 는 미접촉(§결정 1).
- `archive/adr/ADR-163-codeforge-measurement-channel-architecture.md` — dev-process = 9th Tier-3 channel. 본 ADR 은 그 amendment 를 유발하지 않음(non-amendment, §결정 1).
- `archive/adr/ADR-043-codeforge-telemetry-privacy-policy.md` — 14-rule redaction floor 상속(heartbeat egress 우회 불가, AC-8).
- `archive/adr/ADR-099-atlassian-allow-redefinition.md` / `archive/adr/ADR-100-confluence-jira-doc-ssot-recognition.md` — Jira write = `addCommentToJiraIssue` 1종 narrow-allow. verdict=GitHub → §A1-1 무약화(AC-11).
- `archive/adr/ADR-115-runtime-hook-enforcement.md` — record-only · non-blocking · exit0 상속(AC-8).
- `archive/adr/ADR-157-infra-resource-manifest-drift-gate.md` — watchdog credential/workflow = infra-resource-baseline.yaml 선언 의무(미선언 = fail-closed FAIL).
- `docs/inter-plugin-contracts/dev-process-event-v1.md` — 무변경 대상 계약(§결정 1 non-amendment). MANIFEST kind:contract/registry entry 신설 0.
- `skills/jira-progress-mirror/SKILL.md` — Arc B-2 heartbeat relay 확장 대상(Phase 2).
- `scripts/jira-channel/deny-scan.sh` · `scripts/jira-channel/echo-guard.sh` · `scripts/jira-channel/progress-format.sh` — egress backstop(email/RRN gap 주의 §결정 6) / sentinel SSOT(⟦cf-orch⟧, HEARTBEAT 토큰 신설) / heartbeat-format.sh 형제 선례. 무변경 재사용.
- `.github/workflows/confluence-forward-sync.yml` — watchdog credential 패턴 선례(Atlassian token as GH secret / secret-absent=dry-run / continue-on-error / ubuntu-latest hosted).
- `docs/architecture/codeforge-family.md` — 경계 + 데이터흐름 갱신(Phase 1 co-land).

**신설 (Phase 2 — 실 배선 대상, 본 ADR 은 설계 SSOT)**

- `.github/workflows/branch-liveness-watchdog.yml` — 외부 watchdog cron(ubuntu-latest hosted, §결정 2).
- `scripts/check-branch-liveness.sh` → `scripts/lib/check_branch_liveness.py` — 3-state 판정 SSOT(thin wrapper → Python, ADR-061).
- `scripts/lib/emit_branch_heartbeat.py` — heartbeat emit 모듈(agent-emit 스타일, non-blocking exit0).
- `scripts/jira-channel/heartbeat-format.sh` — comment-body 마이크로포맷 SSOT(progress-format.sh 형제).
- `docs/infra-resource-baseline.yaml` — watchdog workflow + read-only Jira credential 선언(ADR-157 drift gate).
- `mclayer/codeforge-internal-docs:wrapper/change-plans/cfp-2772-parallel-branch-heartbeat-watchdog.md` — 실 설계 상세 SSOT(dogfood-out).
- `mclayer/codeforge-internal-docs:wrapper/stories/CFP-2772.md` — carrier Story(§5 AC surface).

...
- **freeze 보존**: `branch_key` 를 신규 상관 ID 로 넣으면 §5.1 "4 상관 ID freeze 변경 = amendment 의무" + §4 "필수 field 추가 = MAJOR". amendment cascade(ADR-042/155/043 + _ROW_KEYS 18→20 parity + self-test assert + §9 mining filter + dormant Port B 활성 + heartbeat-전용 seq-MUST invariant)를 유발하나, 얻는 것은 도달-불가 LOCAL 기록뿐.
...
- **dev-process-event-v1 / ADR-042/155/043 = 무변경**(§결정 1 non-amendment). MANIFEST kind:contract/registry entry 신설 0.

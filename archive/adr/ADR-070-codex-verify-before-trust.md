...
**3-enum exhaustive invariant**: 4번째 path 발생 = D1 expansion 거절된 대안 영역 (자동 retry / 외부 verify proxy / multi-source consensus 등). 본 ADR-070 scope 외 — 별도 follow-up CFP carrier 영역 (CFP-946-B 도 미포함).
...
- (D1.exp-A) **substitution path 4번째 enum value 추가** (auto-retry / multi-source consensus / 외부 verify proxy 등) — 본 Story-A scope 외 (CFP-946-A §5.3 Out-of-Scope 영역). 별도 follow-up CFP carrier 분리. 3-enum exhaustive retain.
...
4. **Story §10 marker `[codex-rescue-fallback: <fail-mode>]` (또는 disjoint marker) 채택 권장** — reactive 채널 fail-mode tally audit trail (proactive 채널 `[codex-sandbox-fallback]` 의 disjoint variant, 또는 동일 marker 의 reactive scope value codify — Wave 2 mechanical lint 도입 시 별도 CFP carrier 결정 영역). 사용자 자율 선택 — codeforge 강제 0.
...
`codex-network-scope-presence` lint (evidence-checks-registry entry, ADR-060 Amendment 14 §결정 28 carrier) 의 mechanical detection scope = proactive 6 touchpoint spawn prompt 한정 (CFP-963 Story §6.3 OOS row L374 derived default 정합) — reactive 영역 mechanical lint 확장 = 별도 CFP carrier 분리 (Wave 2). ADR-064 §결정 1 (CFP scope unitary, "경량 → full" 단계 채택 금지) 정합. CFP-963 Phase 2 mechanical lint 패턴 답습 (Phase 1 declarative + Phase 2 mechanical = 별도 CFP).
...
본 Wave 2 carrier = CFP-1003 retro 발의 시점 (또는 사용자 directive 시점) 별도 CFP 분리. 본 Amendment 5 Wave 1 declarative-only scope retain.
...
- (Amendment 5-D) **review-verdict-v4 신규 `reactive_codex_invocation_count` contract field 신설** — contract MINOR bump + sibling sync (ADR-008/010) + Phase 2 PR = doc-only fast-path 이탈. ADR-070 §D5 + ADR-081 §D5 + ADR-082 §결정 6 declaration-only retain precedent 위배 → declaration-only retain + Wave 2 mechanical lint (별도 CFP carrier) 채택.
...
- (Amd9-B) **fail-mode 9-enum 에 disposition 흡수** (10번째 value 추가) — axis mismatch (cause ↔ disposition disjoint). 별도 §결정 D7 disposition 3-enum 채택.
...
- (Amd10-E) **fail-mode 9-enum 에 mutation disposition 흡수** — axis mismatch (cause ↔ disposition disjoint, ADR-070 Amd 9 Amd9-B 선례). 별도 §결정 D8 채택.

...
related_adrs:
  ...
  - ADR-125  # 요구사항리뷰 lane — §결정2 "branch-protection required contexts 무변경" 선례는 phase-gate INTERNAL 흡수 전제. 본 ADR 은 fail-closed 비호환(anchor=warning-tier + fast-pass bypass 실측)으로 이 선례를 override 하고 신규 required job 정당화 (CONFLICT-C, §결정 3). §5.6 RO-1 = ADR-125 Amd2-style additive disjoint 축 신설 선례
...
**Amendment 5** (2026-07-13 KST, CFP-2659 — #2657 backfill, CFP-2653 follow-up · 등록 트랙 #2649/#2642/#2632 종결자): **§결정 11** 신설 — marker-format false-red(F1) 제거 + Hop3 `.py`-only disclosed residual(F2 defer) + 6→7 등록 완결. (1) **F1 = 어댑터 inline-JS 마커 3정규식(story_uri/none/rtm)이 markdown 장식(키 bold `**`/list `- `·`* `/값 trailing `**`) 마커를 놓쳐 both-absent false-red** — 저자 규약 준수 PR 오차단(실측 2/7 = CFP-2646/2636 P2 PR), required 등록 시 born-broken required(CFP-2530/2535 계보). ★**born-hollow 제약(1급)**: 마커파싱이 어댑터 JS 라 현 self-test(`test_check-ac-traceability-matrix.sh` 전 case 가 core `.py` 구동) 0 케이스 도달 → regex-only fix = mutation-kill 불가 = born-hollow(§결정5 self-test 의무 + ADR-136 execution-liveness 위반, 대안 Option A 기각 근거와 동형). ∴ **마커-파싱 pure 모듈 `scripts/lib/ac_pr_markers.py` 추출**(`ac_id.py` 형제 leaf, §결정7) → 어댑터 = thin orchestrator(execFileSync arg-array + body-file, injection-safe) → 기존 bash+python self-test 가 실 파서 구동(born-hollow 봉인). 장식 tolerance = **line-start marker-anchored bounded**(false-green 산문 악화 금지, 불균형 `story_uri**:` = malformed·PASS 아님) + **ReDoS bounded-quantifier/per-line-anchor 가드**(pre-auth fork-PR 도달, SecurityArch 신규). (2) **F2 = defer** — Hop3 `.py`-only scope disclosed residual(§결정5 grep-ban ⊥ `.sh` naive 확장 = bashlex/tree-sitter-bash 신규의존 or grep-BANNED, 현 dormant) + named carrier = 본 §결정11(C) 조건부-활성 트리거(CFP-2590 TBD-naive 회피). (3) **6→7 등록 = §결정10(D) 7-conjunct → 8-conjunct**(marker-format canary plain/키bold/list+bold 각 실 PR PASS append). live=7-tuple 완결 미선언(Phase-2 gated, premature-declaration 재범 금지, ADR-119). **A2-5 = Amendment 5**(신규 mechanism 0 — 마커추출 testability 리팩터링 + scope 정정 false-red 제거, F2-defer 가 new-mechanism-0 prong 보존). ratchet↑ 방향, 약화 surface 0. **§결정1-10 무supersede·무rewrite**(append-only). **Codex debate(F1 decoupling) = err1920 sandbox degrade 정직 표기 — firsthand fallback(RefactorAgent advocacy + TestContractArch/SecurityArch 확정 + firsthand ADR anchor).**

**Amendment 6** (2026-07-18 KST, CFP-2671 — CFP-2659 Amendment 5 §결정11 follow-up, C1/C2 비차단 refinement 정산): **§결정11(B) tolerance clause bidirectional 명문화 + F2 dormant 재확인** (신규 §결정 0 / 신규 standalone ADR 0 / doc-only). 근거 = 제안-필요성 게이트(ADR-119 §결정9) 통과: §결정11(B) clause 가 C2 shipped 후 "trailing" 만 기록 = SSOT drift(bidirectional 실구현과 불일치), doc-only 1-clause 저비용, ADR 정합성은 관찰자 무관 내재 필요.
> - **§결정11(B) 정정**: "값 캡처 후 **trailing** `**` strip" → "값 캡처 후 **양방향(leading+trailing) bounded** `**` strip (방향당 ≤2, 정규식 `\*{0,2}` tolerance 와 대칭)". 근거: value-only-bold(`story_uri: **url**`)는 정당 CommonMark 이며 trailing-only strip 시 leading `**` 잔존 → false-red(정상 PR 오차단). 정규식(`\S+` greedy + bounded `\*{0,2}` + per-line `^` anchor)은 **무변경** → ReDoS 가드 자연 보존, fail-closed 경계(빈-값→None, 3+ asterisk→bounded dirty) 불변. 구현 = strip helper `_strip_trailing_decor`→`_strip_bold_decor` rename + 대칭 leading loop 추가(pure 함수, 계약 표면 무변경).
> - **F2(Hop3 `.sh` born-missing scope) = dormant 재확인**: Amendment 5 의 조건부-활성 트리거(§결정11(C)) 무변경. 활성 3중 조건 미충족 상태 지속 (`collect_test_symbols` `.py`-only 무터치).
> - **C1(defense-in-depth CI 배선) = mechanism 델타 0**: `ac-traceability-self-test.yml` 에 파서 pytest 스텝 1개 추가(중복커버 위 추가 관측층, 새 갭 봉인 아님 — 정직 천장). 신규 workflow/required context/게이트/카테고리/계약 schema 0. required contexts(live 7-tuple) 무변경. **§결정1-11 무supersede·무rewrite**(append-only — clause 정정 declare + F2 재확인만).
...
- **CONFLICT-C divergence 정당화**: ADR-125 §결정2 "branch-protection required contexts 무변경" 선례는 phase-gate **INTERNAL 흡수**(요구사항리뷰 gate 를 기존 check-gate 로 매핑)를 전제했다. 본 게이트는 그 흡수 경로가 **fail-closed 비호환**(anchor=warning-tier + fast-pass bypass 실측)이므로 선례가 성립하지 않는다 → 신규 required job 이 정당하다. ADR-125 선례 override 는 **비호환 근거에 한정**되며 required contexts 를 보수적으로 유지하려는 ADR-125 의 INTERNAL-흡수 선호를 전면 폐기하지 않는다.
...
- **A 채택의 born-broken 안전 전제 (핵심 정당화 — 2 guard 구분)**: 즉시 required 의 실패 모드는 둘이며 각각 별도 guard 로 차단된다 —
...
### 결정 11 — marker-format false-red 제거 (F1: 마커-파싱 pure 모듈 추출 born-hollow 봉인) + Hop3 `.py`-only scope disclosed residual (F2 defer) + 6→7 등록 완결 (Amendment 5, CFP-2659)

**배경 (firsthand origin/main 79f14597 실측 + node regex 재현)**: §결정9(C)가 어댑터를 thin router(마커 추출·forward)로 규정했으나, 마커 **추출 자체**는 어댑터 inline-JS(`github-script` step)의 3 정규식에 잔존한다 — `story_uri: /story_uri:\s*(\S+)/` / `ac_applicability: none /ac_applicability:\s*none\b\s*[—–-]?\s*(.*)/` / `rtm_uri: /rtm_uri:\s*(\S+)/`. 이 3 정규식은 **markdown 장식 마커를 놓친다**: (i) 키 장식 `**story_uri**:`(bold) / `- **story_uri**:`(list+bold) → `/story_uri:/` NOMATCH → both-absent 오판 → false-red FAIL(실측 최근 표본 2/7 = CFP-2646/2636 P2 PR live-FAIL), (ii) 값 장식 `**story_uri: <url>**` → `(\S+)` 캡처에 trailing `**` 혼입 → dirty URI. 저자는 마커 규약을 지켰으나(bold/list = 정당한 PR-body markdown 습관, 규약 SSOT 는 plain 지정하나 매체가 장식 허용) 파서가 못 읽은 것 = **게이트 결함(false-red)** 이지 정상 enforcement 아님. required 등록(6→7) 순간부터 정상 PR 을 repo전체 영구 오차단 → born-broken required(CFP-2530/2535 계보). ∴ F1 은 "개선"이 아니라 **등록의 born-broken 안전 전제** — story_uri-verify(CFP-2609) / story_uri-absent(CFP-2634) / born-invalid(CFP-2644) / §5-parseability(CFP-2653) 4연속 false-red 클래스의 **4번째·마지막 = marker-format 장식**. [verified: origin/main `.github/workflows/ac-traceability-matrix.yml` github-script step 3 regex + node 재현 NOMATCH/dirty-capture]

**★born-hollow 제약(1급 — F1 설계 지배 제약)**: 마커 파싱이 어댑터 JS 에 위치 → 현 self-test(`tests/scripts/test_check-ac-traceability-matrix.sh` 전 case 가 `$CORE`=core Python CLI 를 이미-파싱된 flag 로 구동)가 **regex 파싱 스텝을 0 케이스 구동**(어댑터 yml = L2 byte-diff check 만, JS 미실행) → regex-only fix = mutation-kill 불가 = **born-hollow**(§결정5 self-test 의무 + ADR-136 execution-liveness 위반). 이는 **대안 Option A(어댑터 short-circuit)를 §결정9(C)/대안절이 기각한 근거와 동형**("self-test 하네스가 core Python 만 mutation-변조 → 실행-backed 테스트 0 = born-hollow"). 정확한 anchor(요구리뷰 P2 citation-hygiene 정정 — Story §4.2 `[verified: ADR-145 L237]` mis-anchor 교정): born-hollow 근거 = **본 ADR 대안절 "(Amendment 2) Option A 어댑터 short-circuit" 문단 + §결정8(D) 적용성 verdict core 단일소유 + §결정9(C) Option B thin-router**(L237 아님). [verified: self-test.sh 565줄 전 case `case_pass/case_fail "$CORE"`, adapter-JS 마커 regex 구동 case 0]

**A2-5 판정 (Amendment vs 신규 ADR — §결정8/9/10 A2-5 구조 답습, 양 prong 반증) = Amendment 5 (ADR-145 무supersede)**:
- **Amendment prong = 채택**: 신규 mechanism 0 — 신규 workflow 0(기존 `ac-traceability-matrix.yml`/core 수정), 신규 required context 0(§결정3/8/9/10 이 이미 선언한 동일 6→7), 신규 게이트/카테고리 0, 신규 계약 schema 0(마커 = adapter-routed INPUT, 계약 필드 아님). F1 = 기존 마커-추출 로직의 **testability 리팩터링 + 장식 tolerance scope 정정**(false-red 제거) — `scripts/lib/ac_pr_markers.py` 추출은 기존 `scripts/lib/ac_id.py`(leaf 파서, §결정7) 의 isomorphic 형제(신규 verdict-owner·신규 축 0). Amendment 1/2/4 와 동class(scope 정정 ratchet↑).
- **신규 ADR prong = 기각**: Epic CFP-2602 "게이트 G당 1 신규 ADR"은 신규 게이트 G(G1=145/…/G6=151)에만 적용. CFP-2659 = G1(ADR-145) 자신의 marker-format 정련 + 등록 완결 carrier, 신규 게이트 0 → 새 G 슬롯 아님. ADR-133 atomic-claim 신규 번호 소요 0.
- **F2-defer 가 new-mechanism-0 prong 보존**: F2 를 실배선(§결정5 mechanism 에 `.sh` resolution 추가)하면 신규 mechanism 도입 → Amendment prong 흔들림. F2 = defer(아래 (C)) → 순수 disclosed-residual(doc-only) → Amendment 5 새 mechanism 0 유지.

**(A) F1 메커니즘 = 마커-파싱 pure 모듈 추출 (born-hollow 봉인, decoupling refactor — RefactorAgent advocacy + Codex debate err1920 degrade → firsthand fallback)**:
- **추출**: PR-body 마커 파싱을 pure leaf 모듈 `scripts/lib/ac_pr_markers.py`(신규 — `ac_id.py` 형제, network-0 pure, argv/stdin·파일 입력)로 추출. `parse_pr_body(body) → {story_uri, none_declared, none_reason, rtm_uri, both_absent}`. thin wrapper 신규 subcommand(`check-ac-traceability-matrix.sh --parse-pr-body <file>` — key=value/JSON stdout) 로 노출(additive, 기존 gate CLI 2-value exit 계약 무침범 — 별도 subcommand).
- **어댑터 강등 = thin orchestrator**: 어댑터 github-script = (i) `pr.body` 를 파일 write → (ii) `execFileSync`(arg-array, **셸 미경유** — injection-safe, SecurityArch S7-1/S7-3) 로 모듈 호출 → 파싱된 마커 수령 → (iii) 기존 cross-repo fetch(I/O, 어댑터 잔존) → (iv) 기존 gate 실행. 파싱(pure)=core-side / fetch(I/O)=adapter — §결정7 Ports&Adapters 무손상, core network-0 보존.
- **DC-4/§결정8(D) 무손상**: 마커 **추출**만 pure 모듈 이동, 적용성 **verdict**(precedence·위장·none 판별)은 core `classify_ac_source`/`run()` 단일소유 잔존. `both_absent` = 기계적 추출 사실(마커 0개)이지 verdict 아님 → Option A(어댑터 verdict 재범) 아님(§결정8 D/§결정9 C 봉인 유지).
- **byte-identical 상승작용(DC-5)**: regex 를 YAML 밖으로 빼면 inline-JS 표면↓ → 양 사본(`.github/` ↔ `templates/`) diff 표면↓ → L2 byte-identical 유지 용이.
- **born-invalid 재발 금지(Amd3 상속)**: `run:` 블록에 리터럴 `${{ }}` 재유입 금지, actionlint N-class(Amd3 (2)) green 유지.

**(B) 장식 tolerance 범위 + false-green 경계 (DC-1 대칭 — TestContractArch/SecurityArch 확정)**:
- **허용 장식 집합(marker-anchored bounded)**: 3 마커(story_uri/none/rtm) × {키 bold `**`, list-marker `- `/`* `, 값 bold `**...**`, 주변 공백}. 정규화 = **line-start marker-anchored** — 각 라인의 선행 list/bold/공백 strip 후 마커 매치, 값 캡처 후 trailing `**` strip. 임의 산문 매칭 금지.
> **Amendment 6 (CFP-2671) 정정**: 위 "trailing `**` strip" 은 실제로 **양방향(leading+trailing) bounded** strip 이다 (value-only-bold `story_uri: **url**` 처리 — 상단 Amendment 6 참조). 정규식 무변경.
- **false-green 경계(요구리뷰 이월 — 악화 금지)**: 현 L82 가 산문("여기서 `story_uri:` 필드가 필요")에 mild false-green. 수정은 **line-start anchored bounded** 로 산문 false-green **악화 금지**(anchor 강화로 오히려 저감). 불균형/malformed 토큰(`story_uri**:` 등)은 **malformed 처리(PASS 대상 아님)** — 균형 장식만 normative PASS(AC-11, §5.5 default, DC-1 false-green 경계 보존).
- **ReDoS 가드(SecurityArch S7-5 신규 발견)**: 어댑터 job 은 fork PR 에서 token 확인 **이전** 실행 → 마커 파서가 저자제어 body 에 **pre-auth 노출**. tolerance 정규식 = **bounded 수량자(`\*{0,2}`)** + **per-line `^` anchor** + 인접/중첩 unbounded 수량자 금지(catastrophic backtracking 회피). §8 에 adversarial-timing bound case 포함. [source: OWASP ReDoS / regex catastrophic backtracking]

**(C) F2 = defer (Hop3 `.py`-only scope disclosed residual, named carrier — §결정5 grep-ban tension)**:
- **현 사실**: core `collect_test_symbols`(L458+, **L474** `if not name.endswith(".py"): continue`)가 `.py` ast def/class/async-def symbol 만 수집. §8 RTM 이 `.sh` 테스트를 normative-AC 명명 테스트로 지정하면 Hop3(Phase 2) born-missing false-FAIL. **현 dormant**(어떤 §8 도 `.sh` 를 normative-AC 명명 테스트로 안 부름 — 게이트 자신의 `.sh` self-test 는 Hop3 대상 아님). [verified: core L474 origin/main]
- **판정 = defer**: (i) §결정5(born-missing = 실 symbol/node resolve, **grep 금지**, CFP-2545 false-oracle) ⊥ naive `.sh` 확장 — `.sh` 는 ast-등가 stdlib 부재 → non-stdlib shell 구조 파서(bashlex/tree-sitter-bash = 신규 의존) 또는 grep(§결정5 BANNED) 필요. naive 확장 = §결정5 정면 위반. (ii) 트리거 3중 동시(Phase2 ∧ `.sh` 명명 ∧ 실 RTM) 현 부재 = dormant → ADR-119 §결정9 제안-필요성 게이트(깨짐·강제요인 0, 이득<비용) → 실배선 부적격.
- **disclosed residual codify(DC-8 — silent 약화 금지)**: Hop3 resolution scope = **`.py` ast symbol only** 를 명시 disclose. **named carrier = 본 §결정11(C) 조건부-활성 트리거**: *IF 향후 RTM 이 non-`.py`(예: `.sh`) 테스트를 normative-AC 명명 테스트로 지정하면, THAT Story 가 (a) non-grep 구조 파서(bashlex/tree-sitter-bash)로 Hop3 resolution 확장 OR (b) RTM 이 `.py` symbol 명명 하도록 규약 강제 — 그 시점 A2-5 재검(신규 mechanism 개입).* 이는 CFP-2590 TBD-naive 회피(조건부 트리거 = 완전 명세된 durable carrier, "추후 생성 placeholder" 아님) + §결정9 speculative-Issue 미발의(dormant, 강제요인 0) 양립. 별도 GitHub Issue carrier 는 §결정9 상 불요(원할 시 Orchestrator 재량 — 본 clause 가 carrier SSOT).

**(D) 6→7 등록 완결 = marker-format canary conjunct 추가 (§결정10 D 7-conjunct → 8-conjunct)**:
- §결정10(D) ordering invariant 7-conjunct 에 **(8) marker-format canary**(plain / 키bold / list+bold 마커 각 실 PR PASS 실측 — story_uri 도착·후속 fetch 입력 clean) append. 근거 = 4연속 false-red defer 교훈(story_uri→born-invalid→§5-parseability→marker-format) = **비적용 canary PASS ≠ 등록 안전** 재확인. **5번째 hidden-blocker 저감**: marker-format 넘어 **잔여 governance PR 형상 표본**(marketplace sync / Epic close / sibling)까지 canary battery 확대 권고(disclosed, non-exhaustive).
- **live=7-tuple 완결 미선언**(ADR-119 premature-declaration 재범 금지, 4연속 교훈): 등록 = Phase 2 post-merge, 8-conjunct 전수 실측 gated. 등록 실행 = human/Orchestrator gh api POST `/branches/main/protection/required_status_checks/contexts` append(6 무손상) → 즉시 GET == 정확히 7-tuple. forged machine-test/self-register 금지(§결정9 H/§결정10 D 상속). rollback-ready 6-tuple 선캡처. 미충족 시 defer(#2649/#2632 계승). doc(CLAUDE.md 7-tuple doc-ahead — origin/main 실측)↔live parity = 승격 시점.

**불변 무손상**: fail-closed 핵심(적용-미추적=FAIL, §결정4 AC-7) + F1 repo-guard + F-AC7-a 빈-AC FAIL + presence/mapping 천장(§결정1) + `AC_ID_RE` sub-letter(§결정4) + 적용성 verdict core 단일소유(§결정8 D) + I-APPLIC anti-degradation + non-suppressible conditional(§결정9 D) + none-마커 4-조합 mutation-kill(none-무사유/none-위장/둘다부재→FAIL · 비적용→PASS, §결정9) + injection 가드(§결정9 G env-var 전달) + born-validity(Amd3) + §5-authoring forcing(§결정10) 전부 무손상. 본 Amendment = marker-format false-red 제거(scope 정정) + 마커-파싱 testability 리팩터링(born-hollow 봉인) + F2 residual disclose + 등록 conjunct 추가이지 검출력 silent 약화 0(evidence-gated ratchet↑). **§결정1-10 무supersede·무rewrite**(append-only).
...
### Amendment 5 결과 (CFP-2659)

- **marker-format false-red 제거 + born-hollow 봉인**: 어댑터 마커 3정규식의 장식-마커 미인식(키bold/list/값 trailing `**`) false-red 를 **마커-파싱 pure 모듈(`ac_pr_markers.py`) 추출**로 정정 — 기존 self-test(core `.py` 구동)로는 도달 불가하던 마커 파싱을 **실 구동·mutation-kill(G-DECOR 장식-strip 무력화→RED / G-CLEANCAP trailing-`**`-strip 무력화→dirty URI)** 하는 discriminating self-test 로 봉인. Option A(어댑터 short-circuit born-hollow) 기각 근거의 동형 해소 — parsing pure-core / fetch adapter-I/O(§결정7) + verdict core 단일소유(§결정8 D) 무손상.
- **false-green 경계 보존 + ReDoS 가드**: tolerance = line-start marker-anchored bounded(산문 false-green 악화 금지, 불균형 `story_uri**:` = malformed·PASS 아님, AC-11) + pre-auth fork-PR 도달 파서의 bounded-quantifier/per-line-anchor(catastrophic backtracking 회피, SecurityArch 신규 §7 residual). none-마커 §결정9 4-조합 mutation-kill 무손상.
- **F2 defer + disclosed residual**: Hop3 `.py`-only scope 를 §결정5 grep-ban tension·dormancy 근거로 defer, `.py`-only 를 명시 disclose(DC-8) + 조건부-활성 named carrier(§결정11 C) codify — CFP-2590 TBD-naive 회피, §결정9 speculative-Issue 미발의 양립. F2-defer 로 Amendment 5 new-mechanism-0 순수 유지.
- **6→7 등록 8-conjunct + 완결 미선언**: §결정10(D) 7-conjunct 에 marker-format canary(plain/키bold/list+bold) append(4연속 false-red defer 교훈 — 비적용 canary PASS ≠ 등록 안전). live=7-tuple 완결 = Phase-2 post-merge 8-conjunct gated, 미충족 시 defer(premature-declaration 재범 금지, ADR-119). doc(CLAUDE.md 7-tuple doc-ahead)↔live parity = 승격 시점.
- **Codex debate degrade 정직**: F1 decoupling(pure 모듈 추출 vs 유지) Codex↔Claude debate(ADR-138 blanket_designrefactor) 시도 — Codex err1920-class sandbox degrade(repo 접근 실패, zero evidence). fail-open 안 함, firsthand fallback(RefactorAgent advocacy + TestContractArch/SecurityArch 독립 확정 + firsthand ADR anchor). 교차모델 다양성만 미확보.
...
- **(Amd5) Story**: `<internal-docs>/wrapper/stories/CFP-2659.md` (§3 ADR-145 Amd5 ref · §7 marker-format false-red + 마커-파싱 pure 모듈 추출 설계 서사 · §8 Test Contract) · Change Plan: `<internal-docs>/wrapper/change-plans/2026-07-13-cfp-2659-ac-traceability-marker-format-false-red.md`
- **(Amd5) 수정(Phase 2)**: `scripts/lib/ac_pr_markers.py`(신규 — PR-body 마커 파싱 pure leaf `parse_pr_body`: 장식-tolerant line-start marker-anchored + trailing-`**` clean-capture + bounded/anchored ReDoS-safe, `ac_id.py` 형제) · `scripts/check-ac-traceability-matrix.sh`(신규 `--parse-pr-body` subcommand additive — 기존 gate 2-value exit 계약 무침범) · `.github/workflows/ac-traceability-matrix.yml`(어댑터 = thin orchestrator: inline-JS 3정규식 제거 → `execFileSync` 모듈 호출 arg-array + body-file, injection-safe) · `templates/github-workflows/ac-traceability-matrix.yml`(L2 byte-identical) · `tests/scripts/test_check-ac-traceability-matrix.sh`(F1 discriminating self-test: plain/키bold/list+bold/값bold-cleancap/공백/rtm/none-장식→인식, both-absent→FAIL, 산문→미매치, 불균형→malformed, ReDoS-bound + Mutation G-DECOR/G-CLEANCAP; §결정9 none 4-조합 회귀 무손상)
- **(Amd5) F2 defer carrier**: 본 §결정11(C) 조건부-활성 disclosed residual(non-`.py` RTM 명명 트리거 시 활성 — Hop3 `.py`-only scope, 별도 GitHub Issue 불요 §결정9)
- **(Amd5) 코드-외 조치(Phase 2 post-merge)**: 6→7 등록 — 8-conjunct(7 기존 + marker-format canary plain/키bold/list+bold 각 실 PASS) 전수 실측 후 Orchestrator gh api POST append + 즉시 GET==7 + rollback-ready. 미충족 시 defer(#2649/#2632 계승). `docs/security/branch-protection-audit.md` HELD→완결 갱신(등록 완결 시점)

...
- **disclosed residual codify(DC-8 — silent 약화 금지)**: Hop3 resolution scope = **`.py` ast symbol only** 를 명시 disclose. **named carrier = 본 §결정11(C) 조건부-활성 트리거**: *IF 향후 RTM 이 non-`.py`(예: `.sh`) 테스트를 normative-AC 명명 테스트로 지정하면, THAT Story 가 (a) non-grep 구조 파서(bashlex/tree-sitter-bash)로 Hop3 resolution 확장 OR (b) RTM 이 `.py` symbol 명명 하도록 규약 강제 — 그 시점 A2-5 재검(신규 mechanism 개입).* 이는 CFP-2590 TBD-naive 회피(조건부 트리거 = 완전 명세된 durable carrier, "추후 생성 placeholder" 아님) + §결정9 speculative-Issue 미발의(dormant, 강제요인 0) 양립. 별 GitHub Issue carrier 는 §결정9 상 불요(원할 시 Orchestrator 재량 — 본 clause 가 carrier SSOT).
...
- **(Amd5) F2 defer carrier**: 본 §결정11(C) 조건부-활성 disclosed residual(non-`.py` RTM 명명 트리거 시 활성 — Hop3 `.py`-only scope, 별 GitHub Issue 불요 §결정9)

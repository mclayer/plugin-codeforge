...
**본 결정 = 엔진 enum sibling(contract surface).** 정책 신설(설계 리팩터링 결정 방식 격상)의 carrier = **ADR-138**(별 ADR — 정책 결정 기록은 엔진 ADR 이 아님). producer/consumer 실 dispatch 배선 + verdict judge(ArchitectAgent chief) + anchor scope(per-Story) 는 ADR-138 소유 — 본 §결정 12 는 그 경계를 명시한다.

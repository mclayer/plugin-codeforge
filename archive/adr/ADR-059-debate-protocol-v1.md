...
related_adrs:
  ...
  - ADR-138  # blanket_designrefactor 정책 신설 carrier (설계 리팩터링 debate 격상) — §결정11 blanket_refactor 와 sibling enum
related_files:
...
amendment_log:
  ...
  - id: 4
    ...
    summary: "설계 리팩터링(구조 3축 + repo-분해 escalation) consumer 추가 — dispatch_mode enum 에 blanket_designrefactor 추가(divergence 감지 없이 per-Story 무조건 발동, cross_module_signal block 불요; role_assignment={codex:proponent, claude:opponent} 기존 optional 필드 재사용; divergence_type structural per-lane keying 3-way 변별 — DesignLane 설계산출물 / blanket_refactor 실코드중복 / blanket_designrefactor 설계 리팩터 구조 축). 기존 가드(anti-sycophancy 4 + convergence_quality_invariant 3-tuple + anchor-recurrence ≥2 + min3/max5) 전부 상속, 신규 정의 0. debate-protocol-v1 v1.3→v1.4 MINOR bump 동반. 정책 신설(격상 결정) carrier = ADR-138(별 ADR), 본 Amendment 4 = 엔진 enum sibling. blanket_refactor(ADR-137) 무변경."
amendments:
  - id: 1
    date: 2026-05-13
    carrier_story: CFP-533
  ...
  - id: 4
    date: 2026-07-01
    carrier_story: CFP-2543
    section_ref: "§결정 12 — 설계 리팩터링 consumer + blanket_designrefactor"
related_stories:
  ...
  - CFP-2543  # Amendment 4 carrier (설계 리팩터링 debate 격상, blanket_designrefactor enum)
...
### 결정 12 — 설계 리팩터링 (구조 3축) consumer 추가 (Amendment 4, CFP-2543)

debate-protocol-v1 의 lane-agnostic 재사용성(§결정 5·10)을 소비해 **설계 리팩터링 전용 consumer(설계-lane 구조 축)** 를 추가한다. 설계 lane RefactorAgent 의 구조 리팩터링 3축((a)decoupling/(b)pattern/(c)interface-separation) + repo-분해 escalation advocacy 의 반박·수용 판정을 Codex(발제·proponent)↔Claude(반대·opponent) min 3 / max 5 라운드 적대 토론으로 도출하는 계약 표면을 연다.

**본 결정 = 엔진 enum sibling(contract surface).** 정책 신설(설계 리팩터링 결정 방식 격상)의 carrier = **ADR-138**(별도 ADR — 정책 결정 기록은 엔진 ADR 이 아님). producer/consumer 실 dispatch 배선 + verdict judge(ArchitectAgent chief) + anchor scope(per-Story) 는 ADR-138 소유 — 본 §결정 12 는 그 경계를 명시한다.

**(1) blanket_designrefactor dispatch_mode — invocation semantics 만**: `dispatch_mode` enum 에 `blanket_designrefactor` 를 additive 추가. 발동 방식(activation-manner)만 인코딩 — `divergence_detected` 신호 없이 무조건 debate 를 켠다(§결정 7 blanket_cross_module_designlane / §결정 11 blanket_refactor 동형: 설계 구조 리팩터는 "발제↔반대" 대립이 도메인 본질). per-Story 무조건 발동. `cross_module_signal` block 불요(무조건 발동이라 수치 신호 없음 — dead surface 회피, blanket_refactor 선례 동형). 우선순위(SSOT = §결정 7): blanket 군에 total-order 삽입 = `blanket_cross_module_designlane > blanket_refactor > blanket_designrefactor > auto_on_divergence > mechanical_fast_path_inline > user_request_only`. 세 blanket 은 lane-disjoint(설계리뷰 lane / refactor lane / 설계 lane)라 실질 충돌 없으나 total-order 결정론 보장.

**(2) role_assignment 재사용(신규 필드 0)**: 설계 리팩터링 lane = `role_assignment: {codex: proponent, claude: opponent}`. §결정 11 에서 신설한 기존 optional 필드를 재사용 — 신규 필드 0. role_lock(입장 고정)과 orthogonal(direction 배정).

**(3) divergence_type = `structural` 재사용**: 신규 divergence_type 값 추가 0. registry §2.1 per-lane keying 이 의미 확정 — *"structural (blanket_designrefactor) = 설계 리팩터 구조 축(a/b/c + repo-분해) divergence, `<설계 요소>::<구조 축>` anchor(실코드 `<file>:<line>` 아님 — 설계-time line 없음)"*. DesignLane structural(설계 산출물) / blanket_refactor structural(실코드 중복) / blanket_designrefactor structural(설계 리팩터 구조 축) 3-way per-lane 구분.

**(4) 가드 상속 — 신규 정의 0**: trigger schema 표면만 확장하고 round/termination schema 무변경 → anti-sycophancy 4종 + convergence_quality_invariant 3-tuple + anchor-recurrence ≥2 즉시 escalation + min 3 / max 5 라운드 전부 상속. 어느 것도 설계 리팩터링 전용 재정의 0.

**(5) anchor scope per-Story (ADR-138 소유, cross-ref)**: 설계-time = 매 Story 신규 설계 → anchor scope = per-Story(anchor-recurrence ≥2 = Story §9 내 escalation). cross-Epic drop-ledger 불요 — 그건 ADR-137 Epic-close 실코드 중복 전용(content-hash cross-Epic 영속). 설계-time per-Story 에 cross-Epic 재발 개념 부적합. blanket_refactor(ADR-137) cross-Epic 배선과 혼동 금지.

**(6) blanket_refactor(ADR-137) 무변경**: refactor lane enum·verdict judge(PMOAgent)·cross-Epic drop-ledger 무변경. 본 Amendment 는 설계 lane 축만 추가.

**SemVer**: debate-protocol-v1 v1.3 → v1.4 MINOR bump. 기존 enum 값·필드 의미 변경 0(순수 additive strengthening, ADR-008 §결정 2/§결정 10). 정책 신설 carrier = ADR-138.

...
mechanical_enforcement_actions:
  ...
  - action: evidence-registry-structure-verify
    status: warning               # ADR-060 §결정 5 첫 도입 = warning (continue-on-error, non-required). blocking 승격 = evidence-gated 별 후속 CFP. self-entry 는 §32.E self-application — 게이트 자체 script/workflow 실존 → carrier_absent==false, self-flag 안 됨

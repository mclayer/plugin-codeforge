...
related_adrs:
  ...
  - ADR-115  # gate 절대-준수 강제 표준 = hook injection (필터 2 논거)
...
- **gate 절대-준수 강제의 사내 표준 = hook injection** (PreToolUse blocking) — [`ADR-115`](ADR-115-runtime-hook-enforcement.md) `[verified]`. 필터 2 의 "gate 는 skill 이 아니라 hook/anchor 로" 논거.
...
- **hot** = 매 spawn / 매 턴 / 매 lane-진입마다 무조건 로드·참조되는 핫패스.
- **cold** = 특정 상황·이벤트에서만 드물게 참조되는 절차형.
...
**load-scope layer 어휘 (1회 명시)**: wrapper-global / lane-specific / agent-init-inline 3-layer 중 본 매트릭스는 layer-orthogonal 하게 적용되며, **agent-init-inline layer (spawn 시 무조건 로드되는 agent 정의 본문) 만 §결정 4 corollary 의 전용 적용 범위**다.
...
- gate 강제가 필요한 지침의 표준 경로 = **hook injection** ([`ADR-115`](ADR-115-runtime-hook-enforcement.md) PreToolUse blocking) 또는 **CLAUDE.md anchor inline 유지** (ADR-051 anchor 판정 — §결정 6). skill 은 게이트 캐리어가 아니다.
...
**분기 룰 (a vs b)**: 반복 블록이 소수 agent 에 산재 + 분량 짧음 → (a). 다수 agent 공통 + 분량 큼 (drift 위험 / 중복 라인 임계 초과) → (b).
...
**판정 합성 순서 (전체 파이프라인 — 기존 판정자 먼저, 새 필터 다음)**:

1. ADR-051 anchor-vs-reference 판정자 — anchor 면 CLAUDE.md inline 유지, 판정 종료.
2. 본 ADR 필터 2 (gate-type?) → gate 면 INELIGIBLE (hook/anchor 경로 — §결정 3).
3. 본 ADR 필터 1 (hot path?) → hot 이면 INELIGIBLE.
4. §결정 5 ROI 게이트 — net-positive 시 적격 확정.
5. 대상이 agent-init-inline layer 면 §결정 4 corollary 적용 (runtime skill 대신 대안 2종).
...
- ADR-051 (위치·크기·anchor 축) 위에 빈도 × 활성화 축이 합성되어 추출 판정 파이프라인 완결 (§결정 6 순서).

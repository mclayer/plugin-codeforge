...
related_adrs:
  ...
  - ADR-043   # Amendment (본 carrier) — self-context proxy field 3종 allow-list 추가 (numeric/enum only, opt-in default-false 상속)
...
**disjoint-axis 선언 (INV-C 보존)**: 본 ADR 의 READ/synthesis/verbose-return 축은 ADR-039 §결정2 inline-write whitelist(현행 6-entry closed, WRITE 축)와 **완전 disjoint axis(다른 차원)**. **whitelist 7번째 entry 신설 0** — §결정2 closed enumeration 무변경. (ADR-039 §결정9 D3 / §결정16 / §결정17 의 "disjoint axis, N번째 entry 신설 0, closed enumeration 무변경" 어법 답습.)
...
- **미래 재개방**: platform 이 foreground-synchronous 반환 하드캡을 문서화하거나 feature 로 제공 시 = ADR-060 evidence-gate 경유 별 CFP + 본 §결정3 amendment. 현행 = `/feedback` feature-gap 후보로만 기록(추정 배선 금지).
...
- **자매 ADR amendment (본 carrier)**: (a) **ADR-043 Amendment** — §결정4 self-context proxy 3 field allow-list 추가(numeric/enum only, opt-in default-false 상속, T-INFO-5 정합, MINOR). (b) **ADR-039 §결정9 cross-ref stub** — Orchestrator-self Read-axis realization = 본 ADR(disjoint, whitelist 무변경). (c) **ADR-044 §결정11 cross-ref stub** — holder generalization(lane-PL→Orchestrator) = 본 ADR.

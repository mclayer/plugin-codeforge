...
- **job pod 하드닝**(hook extension template 로 강제): `serviceAccountName: ci-job-noperm`(RoleBinding 0) + `automountServiceAccountToken: false` + NetworkPolicy 로 apiserver egress 차단(이중). Story W-4 의 "러너 automount false" 는 kubernetes mode 와 모순 — **job pod 한정으로 재정의**한다(러너 pod 은 토큰 필수, SecurityArch·InfraOp 수렴). **★ 조건 포인터(Amendment 3 앵커 사실 HT-1 참조 — 부기, 결정 무변경)**: 위 SA 축 2 종(`serviceAccountName` + `automountServiceAccountToken`)의 **적용 성립 조건 = 앵커 사실 HT-1 3-way**(reachability 축 ⊥ merge 축) — **(0)/(2) 분기에서는 본 SA 축 2 종이 부재**하다(NetworkPolicy leg 은 ns-level 이라 분기와 독립 잔존). **실확정 = Change Plan §11.4b M-18 L0-pre + L0, 미실행.**
...
**앵커 사실(HT-1)** — `ci-runner-infra` `ci-runner-arc/manifests/hook-configmap.yaml` 의 `data["pod-template.json"]` 이 job pod 에 대해 `runAsNonRoot: true` · `runAsUser: 1001` · `fsGroup: 1001` · `seccompProfile.type: RuntimeDefault` · `allowPrivilegeEscalation: false` · `capabilities.drop: ["ALL"]` 를 **자발 선언**한다 [verified — P0-1 정정 커밋 `4edf150` 기준 L46-64 구간; 구 문면의 L40-59 좌표는 자산 정정으로 shift, 선언 내용 무변]. 배선 기재 실재 — `ci-runner-arc/install.sh:86` 이 apply 하고 values 가 `ACTIONS_RUNNER_CONTAINER_HOOK_TEMPLATE` env 를 배선한다 [verified]. **★ 전달 기전 성립 조건(설계리뷰 §9.5 P0-1 확정 — 선언 도달의 1차 조건)**: upstream hook 은 이 env 값을 **파일 경로**로 소비한다 — `readExtensionFromFile()` = `yaml.load(fs.readFileSync(process.env[ENV_HOOK_TEMPLATE_PATH], 'utf8'))` [source: actions/runner-container-hooks `packages/k8s/src/k8s/utils.ts`] ∧ GitHub Docs "set the `ACTIONS_RUNNER_CONTAINER_HOOK_TEMPLATE` environment variable in the runner container spec to **point to the path of the YAML file** mounted in the runner container" [source: docs.github.com — deploying-runner-scale-sets-with-actions-runner-controller]. 본 앵커 작성 시점의 구 배선(`configMapKeyRef` — ConfigMap **내용**을 env **값**으로 주입)은 이 계약과 불일치 → `readFileSync(<내용 문자열>)` throw → `prepare-job` 실패 → **template 이 hook 에 도달 자체가 불가(born-broken, fail-loud)**였다. ⇒ HT-1 하드닝의 **성립 조건 = ConfigMap 파일 mount ∧ env 값 = 그 mount 경로**이며, 실 자산은 이 형상으로 정정 완료 [verified — `4edf150`: env 값 = `/home/runner/pod-template/pod-template.json` + ConfigMap volume 파일 mount, `configMapKeyRef` 제거]. **★ 조건부 등급(2차 조건 — 4차 등급 정정)**: 전달 성립 후 container hook 이 이 선언을 그대로 적용하는지 override·병합하는지의 등급은 **main 소스에서 override 확정 / chart 0.12.1 번들 hook 동일성 미확인**이다(구 등급 표기는 설계리뷰 §9.5 **P1-1**(과소증거) 로 폐기) — `mergeContainerWithOptions` 는 `name`·`image` skip(warning) / `env`·`volumeMounts`·`ports` mergeLists append / **나머지 키 = catch-all `else { base[key] = value }`**, `mergePodSpecWithOptions` 는 `containers` 필터·append / `volumes` mergeLists / **나머지 키 = catch-all `else { base[key] = value }`** ⇒ `securityContext`(pod-level·container-level 양쪽)·`nodeSelector`·`priorityClassName`·`serviceAccountName`·`automountServiceAccountToken` 은 **extension 이 override 확정** [source: actions/runner-container-hooks `main` packages/k8s/src/k8s/utils.ts — catch-all verbatim]. **★ 축 분리(상속 금지)**: **reachability 축(전달 성립 — 판정 = M-18 L0-pre)** 과 **merge 축(override 성립 — 판정 = M-18 L0)** 은 **disjoint** 이며 **한쪽 결론을 다른 쪽으로 상속시키지 않는다**. ⇒ 이하 두 정정은 **"template 이 도달·적용될 때 성립"** 조건부이며 **실행 결과 단정이 아니다**. 실확정 = Change Plan §11.4b **M-18 L0-pre**(전달 성립 — (0) 도달 불가 판별) 선행 + **L0**(job pod `-o yaml` — (1) 적용 ↔ (2) override 실패 판별) — **둘 다 미실행**. **정직 한정**: 벤더 소스 확인은 upstream `main` 브랜치 기준이며 chart 0.12.1 이 pin 하는 러너 이미지 번들 hook 과의 동일성은 **미확인** — 단 GitHub Docs 문면(버전 무관 서술)과 env 이름 계약이 함께 path 를 가리키므로 **path semantics 자체는 버전 종속으로 보지 않는다**.

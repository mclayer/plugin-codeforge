...
★**조건부 성립**★ — 본 amendment 의 실현은 **C-1**(workflow agent 도구 호출이 PreToolUse 를 발화하는가)과 **U13**(P3 auto-planned 경로가 `Workflow` **도구 호출**로 표면화되어 PreToolUse 에 도달하는가)에 조건부다. 두 전제는 **미검증**이며 1회 probe run 으로 해소한다(부정 시 채택 철회 — ADR-173 §결정 3).

...
related_adrs:
  ...
  - ADR-037  # plugin version bump rule — 기존 skill 본문 편집 = §결정1 (b) Skill file PATCH + 신규 wrapper script(scripts/**, status A) = MINOR signal → Phase 2 plugin.json MINOR bump 대상 (A1-6 정정: 신규 skill 0 논거는 기존 본문 편집·신규 script row 미고려 오류)
...
- **plugin.json MINOR bump 대상 (A1-6 정정)**: 기존 skill 본문 편집(research-request-gate SKILL.md execute/inject 절 확장 = Phase 2)은 ADR-037 §결정1 (b) Skill file "minor edit = PATCH" 대상이고, 신규 wrapper script(scripts/check-research-corroboration-contract.sh, status A)는 ADR-037 surface table 신규 script = MINOR signal + feat 커밋 = commit_signal minor → 종합 expected = **MINOR bump** (Phase 2 6.47.0→6.49.0 carrier). ADR-063 marketplace version sync 동반. **초안의 '신규 skill 0 → skills/ 자동 발견 → bump 불요' 논거는 기존 skill 본문 편집 row 와 신규 script row 를 미고려한 ADR-037 오적용** — no-surface-touch 면제 불가(skills/** + scripts/** 둘 다 wrapper 귀속 surface). [verified: ADR-037 surface table + check-plugin-version-bump-self.sh 실 게이트 run 28371195841 VERDICT=FAIL]
...
- wrapper plugin.json bump = **MINOR 대상 (정정)** — 기존 skill 본문 편집(ADR-037 §결정1 (b) PATCH) + 신규 wrapper script(scripts/**, status A = MINOR signal) → expected MINOR(Phase 2 6.47.0→6.49.0). 초안의 'bump 불요'(신규 skill 0 논거)는 기존 본문 편집·신규 script 미고려 ADR-037 오적용 — marketplace 4필드 중 version 변경 동반(ADR-063 sync). [verified: ADR-037 surface table + 실 게이트 VERDICT=FAIL]

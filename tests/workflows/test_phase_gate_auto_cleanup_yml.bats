...
#   - fixture_file: tests/workflows/test_phase_gate_auto_cleanup_yml.bats
#   - assertion_classification:
#       - T-1/T-1b: discriminating — on.pull_request.types + on.issues.types trigger schema (FAIL without schema)
#       - T-2/T-2b: discriminating — concurrency group namespace disjoint (FAIL on phase-gate-auto-cleanup absent)
#       - T-3/T-3b/T-3c: discriminating — hotfix-bypass guard label presence (FAIL without bypass guard)
#       - T-4/T-4b/T-4c/T-4d: discriminating — removeLabel API + try/catch 404 + KST timestamp (FAIL without)
#       - T-5/T-5b: discriminating — safety-first gate absent alert (FAIL if auto-fix merged)
#       - T-6/T-6b: regression_guard — GITHUB_TOKEN only (D-4 decision invariant, FAIL if PAT introduced)
#       - T-7/T-7b: regression_guard — EC-2 cross-repo orphan guard (FAIL without Related marker check)
...
#       tests/workflows/test_phase_gate_auto_cleanup_yml.bats
#     bats tests/workflows/test_phase_gate_auto_cleanup_yml.bats
#     # Expected: marker lint WARN (markers absent -> [WARN] 0/5 in bare state)
#     git stash pop
#     bats tests/workflows/test_phase_gate_auto_cleanup_yml.bats
...
#   $ git stash push --include-untracked -- tests/workflows/test_phase_gate_auto_cleanup_yml.bats
#   $ bash scripts/check-bats-red-green-proof.sh tests/workflows/test_phase_gate_auto_cleanup_yml.bats

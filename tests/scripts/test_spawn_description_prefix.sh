...
echo "FAIL: $FAIL"
if [ "$FAIL" -eq 0 ]; then
  echo "✓ All $PASS cases pass — T-1(INV-1 exit-0-always) + T-2(INV-2 tz-invariance) + T-4(INV-3 lint-inert) + T-3(hook↔detector seam) + B(no-epoch production smoke) + C(empty-content nonconformant) 입증"
  exit 0
else

...
── 정직 천장 (ADR-119 — 이 검사가 **못 하는** 것) ───────────────────────────────
  ...
  ⑦ **참조와 뒤집기를 구분하지 못한다 — 반대 방향의 오발화.** Amendment 안에서
     ...
     A1-3 `:275` · A5-4-c `:585` 의 *"「신규 플래그 0건」 정신"* 인용만으로 걸렸다 [ceiling: 인용부호 안은 ADR-172 원문 리터럴 — 본 모듈의 주장이 아니라 mention 이다]
  ...
  ⑧ **행번호 포인터(`:NNN`)의 stale 은 정의역 밖 — 좌표가 편집에 따라 썩는다.**
     정정 포인터를 `:412` 같은 행번호로 앵커하면, 같은 브랜치의 후속 커밋이 앞쪽에
     문단을 삽입할 때 좌표가 밀려 **다른 줄을 가리키게 된다**. 이 검사는 그것을 보지
     못한다 — 위 「절 ID 문법」이 `A<n>-<m>` · `§결정 N` 두 형태만 담아 `:NNN` 은
     **값 공간 밖**이고, 그래서 읽지도·해소하지도·위반으로 세지도 않는다.
     실물(ADR-172 A5-3 기재, ArchitectPL 실측): 그 근거표의 인용 좌표 8 개가 저작
     시점 `4f5f74c5e` 에는 실내용과 일치했는데, 후속 6 커밋이 21~35 줄 밀어 정본
     HEAD 에서 재현 불가가 됐다 — `:416` 은 빈 줄이 됐고 `:289`-`:306` 은 다른
     Amendment 구간으로 옮겨갔다(본 모듈 firsthand 재확인: `c2182d242` 에서
     `:306`·`:416` 둘 다 빈 줄).
     ★ 인접한 ⑤ 와 **기전이 다르다** — ⑤ 는 *지목과 인용의 구분 불가*가 사유이고,
     ⑧ 은 *좌표가 시간에 따라 썩음*이 사유다. ⑦ 과도 다르다 — ⑦ 은 읽고 나서
     구분을 못 하는 것이고, ⑧ 은 **애초에 읽지 못한다**.
     ⇒ ⑧ 의 완화는 검사 확장이 아니라 **저작 규율**이다: 절 ID · verbatim 인용문 ·
       불변 SHA 로 앵커한다. 이 천장은 산문으로만 두지 않고
       `test_correction_pointer_cochange.py::test_ceiling8_line_number_pointer_is_outside_detection_domain`
       이 assert 로 각인한다 — 검사기를 좌표 축으로 넓히면 그 테스트가 RED 가 되어
       본 목록의 갱신이 강제된다(각인은 검출이 아니라 **넓힐 때 알려주는 장치**다).

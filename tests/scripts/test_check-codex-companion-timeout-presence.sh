...
# CFP-2545 Phase 2 (+ 구현리뷰 FIX iter1) — Discriminating self-test for
...
#   항상 **runnable option-first** 가드 `timeout --kill-after=<K> <N>` 로 감싸야 함 (ADR-081 §D14).
...
#
# 2-축 결박 (구현리뷰 FIX iter1 F-2 blind spot 봉인):
...
echo "═══════════════════════════════════════════════════════════════════════════"
echo " CFP-2545: codex-companion timeout-presence lint — 축 A (grep oracle)"
echo "═══════════════════════════════════════════════════════════════════════════"
echo

# 1: GREEN — option-first (env-default) 가드 존재
run_case "A1: GREEN option-first (env-default)" \
...
# 2: GREEN — option-first (리터럴) task --write
run_case "A2: GREEN option-first (리터럴) task --write" \
  'timeout --kill-after=30 300 node "$CMD" task --write "y"' \
  0 "task --write dispatch 도 runnable option-first 가드 필수"

# 3: RED — duration-first 오배열 (broken, GNU timeout exit 127)
run_case "A3: RED duration-first 오배열 (broken)" \
...
run_case "A4: RED duration-first 리터럴 (broken)" \
...
run_case "A5: RED mutation — 가드 제거" \
...
run_case "A7: RED --kill-after 누락" \
  ...
  1 "--kill-after 부재 = detached node 좀비 위험 + 가드 불완전 → lint RED"

# 8: RED — hollow-gate I-3 (home 실존 + dispatch 발화 0건)
run_tree_case "A8: RED hollow-gate I-3 (home 실존 + 발화 0건)" \
  1 "home 실존하나 dispatch 발화 0건 → 발화가 스코프 이탈 가능 → exit 1 (항상 GREEN 방지)" \
  yes "이 파일에는 dispatch 발화가 없다 — companion 은 prose 로만 언급."

# 9: GREEN — consumer no-op (home 부재 + 발화 0건)
run_tree_case "A9: GREEN consumer no-op (home 부재)" \
  0 "consumer degradation: plugins/codeforge-review/agents/ 부재 → honest no-op exit 0 (spurious RED 미발생, byte-identical parity 안전)" \
  no "이 문서는 codex companion 을 prose 로만 언급. 실행 dispatch 발화 0건."

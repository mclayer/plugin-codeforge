...
#   TC-1 stale:           CLAUDE.md "Amendment 2 (CFP-NNN)" 참조 + ADR amendment_log 길이=4 → exit 1 + drift msg
#   TC-2 latest:          CLAUDE.md "Amendment 4 (CFP-NNN)" 참조 + ADR amendments 길이=4   → exit 0
#   TC-3 no-amendment-log: CLAUDE.md "Amendment 1 (CFP-NNN)" 참조 + ADR frontmatter 에 amendment_log/amendments 없음 → exit 1
#   TC-4 multi-Amendment: CLAUDE.md 안 다수 ADR cross-ref 동시 detect (각각 verify)
...
(Amendment 2 (CFP-631) §결정 11 신설 — mirrored field description PR-time mechanical proactive lint mandate.)
CLAUDEEOF
...
Some policy = [ADR-999](docs/adr/ADR-999-test-adr.md)
(Amendment 1 (CFP-500) §결정 9 신설.)
CLAUDEEOF
...
ADREOF
    ...
    # 두 블록을 20줄 이상 분리하여 ±5 context window 크로스매치 방지
    cat > "$TEST_DIR/CLAUDE.md" <<'CLAUDEEOF'
# CLAUDE.md fixture for TC-4

## Section A

**Evidence-enforceable** = [ADR-060](docs/adr/ADR-060-evidence-enforceable.md)
(Amendment 2 (CFP-455) §결정 3 current_tier required 전환.)
...
**Test ADR** = [ADR-999](docs/adr/ADR-999-test-adr.md)
(Amendment 1 (CFP-500) §결정 9 신설.)
CLAUDEEOF
...
Missing ADR = [ADR-777](docs/adr/ADR-777-missing.md)
(Amendment 1 (CFP-500) §결정 1.)
CLAUDEEOF

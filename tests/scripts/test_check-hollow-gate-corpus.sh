...
#             · 카운터 **정의 본체** 1행 (`fail_case()` 정의의 `FAIL=` → `PASS=`) — 전 실패가
#               PASS 로 계상되는데 총계는 그대로다. 정의 위치는 좌표가 아니라 **재현 규칙**으로
#               적는다: `grep -n '^fail_case() {' <본 파일>` (13회차 F-CR30-9 — 종전 좌표
#               `:135` 는 편집 누적으로 **dangling** 이 됐다).
#             · pin 직전 **tally 보정** 1행 삽입 — 케이스 소실을 상쇄해 은닉한다.
#             · 카운터 **호출 site 1토큰** relabel (`fail_case` → `pass_case`) — **전 호출 site**
#               가 대상이고 relabel 가드는 0.
#               ★ **계수를 주석에 리터럴로 적지 않는다** (13회차 F-CR30-4). 종전 문면
#               「117 site(`fail_case` 80 · `pass_case` 37)」는 **같은 파일의 선언 상수**
#               `HGC_DECLARED_COMPOSITION` 과 모순한 채 3 리비전 생존했고, 직전 회차 리뷰가
#               그 값을 「정본」으로 명시 인증하기까지 했다. 리터럴은 커밋과 함께 이동하는데
#               주석은 따라가지 않는다. ⇒ 계수 정본 = **재현 명령**이며 현재값은 §0
#               `HGC_DECLARED_COMPOSITION` 의 `census=` 성분이 SSOT 다(§13.5 가 매 실행 실측과
#               대조하므로 **주석이 아니라 실행 중 값**이 정본이다). 재현:
#                 awk '/^[[:space:]]*fail_case /{f++} /^[[:space:]]*pass_case /{p++} \
#                      /^[[:space:]]*skip_case /{s++} END{print f+0, p+0, s+0}' <본 파일>
...
#               site 를 **인라인 전개**해 `fail_case` 호출을 1 줄이고, **도달 불가 블록**에
#               1 줄을 넣어 census 를 되돌린다. rc=0 ·
#               ★ **재현 규칙 정정 (13회차 F-CR30-7)** — 보상 블록은 반드시 **다중행**이어야
#               한다. 종전 문면의 단일행 형 `if false; then fail_case …; fi` 는 **재현되지
#               않는다**: 그 한 줄은 줄머리가 `if` 라 census 술어(`^[[:space:]]*fail_case `)에
#               걸리지 않아 보상이 성립하지 않고 오히려 census 가 내려가 RED·rc=1 이 된다.
#               보상이 성립하는 형은 `fail_case` 호출이 **자기 줄의 줄머리**에 오는 다중행이다:
#                   if false; then
#                     fail_case "…"
#                   fi
#               §8.14.6 이 「좌표 대신 **재현 규칙**을 정본으로」를 채택했으므로 **규칙 자체의
#               정확성**이 그 약속의 대상이다 — 규칙이 틀리면 좌표보다 나을 것이 없다.
...
#   ★ 정직 천장 (전건 실측 — 추론 0):
#     [반증시도: 완료(부분) — 「pin 이 구성 변화에 눈이 없다」는 앵커 **대상 교체**로 반증을
#      시도했고 §13.5 가 그 산물이다(총량 보존형 5종 중 rc-도달 5 · stdout-도달 1 · 미도달 다수).
#      「(3) 공유 앵커의 상호 보호 무효」는 **미시도** — 값을 함께 이동시키는 편집이 두 소비자를
#      정합 이동시킨다는 것은 구성상 자명하나 반례 탐색을 수행하지는 않았다]
...
# ★ `skip_case` 는 SKIP 을 총량 tally 에 계상한다 (F-CR28-4). 호출 site 수는 위 재현 명령의
#   세 번째 값이며 §0 `HGC_DECLARED_COMPOSITION` 의 `census=` 세 번째 성분과 같다.
#   ★ **종전 문면 「호출 site 0 건(휴면)」은 거짓이었다** (13회차 F-CR30-4). §13.5 신설 커밋이
#     baseline leg 의 NOT_APPLICABLE 분기로 호출 site 를 만들었는데 주석이 따라가지 않았고,
#     그 계수가 **바로 같은 파일의 선언 상수 세 번째 성분**이라 파일이 자기 선언과 모순한 채
#     3 리비전 생존했다. 「휴면」을 전제로 세운 아래 존치 논증도 그만큼 약해진다 — 이 헬퍼는
#     이제 실제로 발화한다.
#   **존치**를 택한다 — 사유: ⓐ SKIP 은 요약행·tally 가 이미 1급으로 다루는 결과 범주라 헬퍼만
#   지우면 표현이 불완전해지고, ⓑ 무엇보다 이것은 §0 (4) **relabel 표면의 한 원소**일 뿐이라
#   삭제해도 relabel 의 **목적지 한 종류가 줄 뿐 표면 자체는 그대로**다. 「한 얼굴을 지워
#   닫았다」는 것이 정확히 본 Story 가 두 회차 연속 반증당한 열거-확장 처방이므로 되풀이하지 않는다.
...
#   건전성이 아니라 **하네스 자신의 선행 가드**(본 파일의 NOT_RUN pyyaml preflight — 재현:
#   `grep -n 'import yaml' <본 파일>`. pyyaml 부재면 즉시 exit 1 이라 core 의 DEP 경로가 애초에
#   도달 불가다)였고, ★ 종전 좌표 `:124` 는 **오지시**였다(13회차 F-CR30-9).
...
#     판별력으로 계상한다 — 「분기 중화」와 「분기 도달 전 사망」은 **rc 차분 단독으로는**
#     구별되지 않는다.
#     [반증시도: 완료 — 정의역 한정으로 정정. rc 차분 하나만 보면 참이지만 **관측면을 늘리면
#      구별된다**(crash 0 conjunct 가 바로 그 산물 — 양 팔 Traceback 계수를 함께 본다).
#      「원리적 불가」가 아니라 **단일 관측면의 한계**였다]
...
if [ -z "$m7_base" ] || [ -z "$m7_mut" ]; then
  ...
  #   이었다. 「불변식 중화」와 「조기 사망」은 **rc 차분 단독으로는** 구별되지 않는다.
  #   [반증시도: 완료 — 「구별 불가」를 **정의역 한정으로 정정**했다. rc 차분 하나만 보면 참이나
  #    **관측면을 늘리면 구별된다**: Traceback 계수 ∧ 종점 announce 도달을 conjunct 로 붙이면
  #    조기 사망 팔이 거부된다(바로 아래 가드가 그 산물). ⇒ 「원리적 불가」가 아니라 **단일
  #    관측면의 한계**였다]
  ...
  #     [반증시도: 미수행 — 단 **닫는 조건은 확정**돼 있다(참값이 다른 2번째 관측 = abort 무력화
  #      표면 신설). 「원리적 불가」가 아니라 **관측 1개라는 현 구성의 한계**이며, 구성이 바뀌면
  #      닫힌다. 비용 사유로 미착수임을 명시한다]
...
#   의미를 정하는 표면).
#   ★★ **「3 성분을 구조적 class 로 정의한다」는 총괄 주장을 철회한다** (13회차 F-CR30-1).
#     세 성분은 **각자 다른 정의역**을 가지며 그 정의역은 **표기**로 정해진다 — 역할이 아니다.
#     성분별로 분해해 적는다(총괄 주장 1줄이 셋의 차이를 덮고 있었다):
#     census   — 줄머리 토큰 **문면** 계수 3종 (`fail_case` / `pass_case` / `skip_case`).
#                정의역 = **줄머리 표기**이지 호출 site 가 아니다(아래 천장 ⓒ).
#     helper   — `pass_case() {` 부터 `skip_case() {` 까지의 **위치 창** 안 본체 해시.
#                ★ **끝 경계가 있다** — 창 **밖**의 재정의는 정의역 밖이다. **ATK-B**
#                (9번째 얼굴, 13회차): `skip_case() {` 행 **뒤**에 `fail_case` 를 **1행
#                재정의**한다(bash 는 후정의가 이긴다). 즉 **1행 추가로 카운터를 갈아끼우면서
#                본 앵커에 흔적을 남기지 않는다.**
#                  · 구현 자체 실측(정적) = 행 델타 **1** · 서명 3 성분 **전건 불변**.
#                    재현: `skip_case() {` 행 뒤에 `fail_case()` 재정의 1행을 넣고 서명을
#                    재산출하면 `census`·`helper`·`loopdecl` 이 모두 무처치본과 같다.
#                  · 전 하네스 rc=0 · tally 무차분 · `✗` 0 · stdout 차분 0 은 **13회차 리뷰의
#                    실행 측정**이며 본 회차 구현은 재현하지 않았다(인용).
#                ★ 왜 `census` 가 못 잡나: 재정의 행의 줄머리는 `fail_case(` 라 census 술어
#                  `^[[:space:]]*fail_case ` (뒤에 **공백**)에 걸리지 않는다.
#                ⇒ 처방은 **술어 확장이 아니다**(T6 — 확장은 두 회차 연속 반증당했다). 여기서
#                하는 것은 **정의역을 정확히 적는 것**뿐이다: 본 성분이 보증하는 것은
#                「창 **안**의 본체가 그대로다」이지 「카운터 구현이 그대로다」가 아니다.
#     loopdecl — **반복 항목 선언면** 해시 (`for … in` 헤더행 ∧ 이어지는 인용 항목행).
#                ★ **술어가 고르는 기준은 「역할」이 아니라 「이름 모양」이다** — 12회차 전수 실측:
#                  잡는 것 **6 블록** = `f`(:222 NOT_RUN 가드 — **케이스 생성 아님**) · `pair`(:732) ·
#                    `ann_case`(:1101) · `tdep_case`(:1446) · `tok`(:1657) · `word`(:1736)
#                  빠지는 것 **2 블록** = `m7_r`(:939) · `m8_a`(:1036) — 사유는 역할이 아니라
#                    **변수명에 숫자가 있어 `[a-z_]+` 에 안 걸리는 것**이다.
#                ⇒ 현재 커버리지 손실은 **0**(빠진 2개가 마침 케이스 생성 loop 이 아니다) 이지만
#                  **그 일치는 우연**이다. 숫자·대문자를 포함한 이름의 케이스 생성 loop 이 신설되면
#                  **조용히 빠진다**. 또 `m8_a` 처럼 **항목이 변수 확장**인 loop 은 헤더를 잡아도
#                  항목이 선언면에 없어 항목 변경을 못 본다. Story 잔여 등재 — 「전건이 덮인다」로
#                  적지 않는다(11회차 「N곳 과소계수」 계보).
#
...
#     ⓐ **rc-도달**(rc=1 로 차단) : R1(호출 site 1토큰) · U2(삭제+tally 보정) · U2B(**이종**
#        net-zero swap — 치환으로 종류별 개수가 바뀌는 형) · XPOST(스냅샷-후 주입) ·
#        N5(loop 항목 파라미터 1문자)
#        ★ **「net-zero swap」을 한 덩어리로 적으면 거짓이다** (13회차 F-CR30-3). 같은 이름이
#          **두 구성**을 덮고 있었다: 종류별 개수가 바뀌는 **이종** 치환은 여기 ⓐ 이고,
#          종류가 보존되는 **동종 1:1** 치환은 아래 ⓒ **미도달**이다. 종전 열거는 뒤엣것까지
#          ⓐ 로 계상했다.
...
#          호출을 1 줄이고 **도달 불가 블록**에 `fail_case` 를 **줄머리로** 1 줄 넣어 census 를
#          되돌린다 ⇒ 서명 3 성분 전건 불변 · rc=0.
#          ★ 재현 규칙 = **다중행**(단일행 `if false; then fail_case …; fi` 는 재현되지 않는다 —
#            13회차 F-CR30-7. 정확한 형은 §0 XINL 항 참조).
#          ★★ **기전 서술 정정 (13회차 F-CR30-6)** — 종전 문면 *「census 는 호출 site 의
#            **존재**를 셀 뿐 도달 가능성을 보지 않는다」* 는 부정확하다. census 는 **호출 site 를
#            세지 않는다**. 술어가 세는 것은 **줄머리 토큰 문면**이며 실행 가능한 코드일 필요조차
#            없다 — 문자열 리터럴 안의 평문 한 줄, here-doc 본문, 주석이 아닌 임의의 텍스트가
#            줄머리에 그 토큰을 두기만 하면 계상된다(13회차 실측 **XSTR**: 호출을 인라인 전개한
#            뒤 **문자열 상수 안 평문 1줄**로 census 를 복원 → rc=0 · 서명 불변 · stdout 무차분).
#            ⇒ 보상재가 **코드일 필요가 없다**는 뜻이고, 이는 「도달 불가 코드」 서술보다 **한
#            단계 더 약한** 요구다. 정적 선언면 계수의 구조적 한계이고 얼굴을 하나 더 열거해서
#            닫히지 않는다. 닫혔다고 적지 않는다.
#            [반증시도: 완료 — 술어 정의역을 문자열 리터럴·도달불가 블록·실호출 3형에 각각
#             적용해 **3형 모두 계상**됨을 실측(실행 경로는 1형뿐). 「호출 site 계수」 해석은
#             그 실측으로 반증됐다]
#        · **동종 1:1 치환 (net-zero swap 중 종류 보존형)** — 실 케이스 1건의 계상 줄을 무력화
#          하고 **같은 종류**의 공허 케이스 1건을 투입하면 census 는 종류별 개수(multiset
#          cardinality)만 보므로 **원리적으로 불가시**다. 요약행·구성 앵커 줄이 무처치본과
#          **byte 동일**이고 `✗` 0 이다(13회차 실측). 보상 기교조차 필요 없다.
#          ★ **천장의 천장 (과대 서술 금지)**: 이것은 「검사 무력화」가 **아니다**. 치환 대상은
#            `then` 분기의 계상 줄이고 `else` 실패 경로는 살아 있다. 실증된 명제는 「케이스
#            1건을 tally 에서 걷어내고 동종 공허 케이스로 대체해도 어떤 기계 소비자도 발화하지
#            않는다」까지다.
#          [반증시도: 부분 — 술어 수준 반증은 **정적으로 성립**한다(종류별 개수만 세므로 동종
#           치환에 불변). 전 하네스 rc·요약행 byte 동일 관측은 **13회차 리뷰의 실행 측정**이고
#           본 회차 구현이 재현하지 않았다 ⇒ 그 부분은 인용이지 자체 실측이 아니다]
...
# 선언 정합 판정을 **이름 있는 술어**로 둔다 — 인라인 비교였을 때는 §13.6 T-SIG 가 판정 경로를
#   행동으로 탐침할 방법이 없었다(항진화를 관측할 표면 부재). 동작은 종전과 동일하다.
hgc_decl_match() { [ "$1" = "$HGC_DECLARED_COMPOSITION" ]; }

# ★ 판정 **전체**(대상 파일 → 서명 산출 → 술어)를 한 함수 안에 넣는다. 목적은 하나 —
#   **호출 site 에 인자 자리를 남기지 않는 것**이다.
#   14회차 리뷰가 실측한 열림: 술어를 이름 있는 함수로 뽑는 봉합이 술어 **안**의 구멍을 덮으면서
#   **호출 site 의 인자 자리**를 새로 열었다. 2-arm 실측 —
#     ARM1 교란만(헬퍼 창 주석 1행)                          → PASS=75 FAIL=1 rc=1 (§13.5 RED, 대조군 생존)
#     ARM2 교란 + 호출부 인자 1-토큰 치환(현재서명 → 선언상수) → PASS=76 FAIL=0 rc=0, T-SIG 6 leg 전원 PASS
#   즉 b·c·d 는 **서명 산출 함수**를, e·f 는 **술어**를 각각 따로 쟀고, 그 둘을 **잇는 자리**는
#   어느 leg 에도 없었다. 인자를 함수 안으로 넣으면 그 자리 자체가 사라진다.
# ★ 산출값을 전역 `hgc_verdict_actual` 에 기록하고 **문면도 그 전역을 인쇄한다** — 판정이 비교한
#   표현식과 인쇄하는 표현식을 같은 것으로 묶는다. 종전에는 판정이 인자값을, 문면이 `$hgc_cur_sig`
...
hgc_composition_verdict() {
  ...
  hgc_decl_match "$hgc_verdict_actual"
}

if hgc_composition_verdict; then
  pass_case "구성 앵커(선언 정합): 실측 구성 서명 [$hgc_verdict_actual] == §0 선언 [$HGC_DECLARED_COMPOSITION] (두 값을 나란히 찍는다 — 판정이 비교한 바로 그 표현식을 인쇄하므로 문면과 판정이 어긋날 수 없다) — 총량 보존형 구성 변경 중 **카운터 본체 · 호출 site relabel · tally 보정 · 스냅샷-후 주입 · loop 항목 파라미터**는 본 서명을 움직이므로 선언 미갱신이면 본 검사가 발화한다. ★ 천장 4종(전건 실측 — 봉인 아님): ⓐ 선언을 **함께** 갱신하는 2-편집은 통과한다(얻은 것은 「구성 이동이 diff 한 줄로 드러남」) ⓑ **카운터 정의 본체 1행(D2)에는 rc 가 따라오지 않는다** — 본 검사의 RED 를 세는 카운터가 바로 그 편집의 표적이라 FAIL 이 0 으로 계상되고 rc=0 이다. D2 에 대해 본 검사가 주는 것은 **stdout 가시성**뿐이다 ⓒ assert 술어 리터럴 교체(N4)와 **줄머리 보상 1행으로 census 를 되돌리는 형(XINL·XSTR)**은 **미도달** — census 가 세는 것은 호출 site 가 아니라 **줄머리 토큰 문면**이라 보상재가 코드일 필요조차 없다 ⓓ **동종 1:1 치환(net-zero swap)은 미도달** — census 는 종류별 개수만 보고 **케이스 동일성**을 보지 않으므로, 실 케이스 1건을 걷어내고 같은 종류의 공허 케이스 1건으로 갈아끼우면 요약행·본 서명이 **byte 단위로 무차분**이다(13회차 F-CR30-3 — 종전 문면은 이것을 잡는 것으로 열거했고 그 열거가 거짓이었다)"
else
  fail_case "구성 앵커(선언 정합): 실측 [$hgc_verdict_actual] ≠ §0 HGC_DECLARED_COMPOSITION [$HGC_DECLARED_COMPOSITION] — 케이스 집합의 **구성**이 바뀌었는데 선언이 갱신되지 않았다(총량은 보존됐을 수 있다). 의도적 변경이면 §0 선언을 실측 서명으로 갱신하라"
fi
...
#     · leg 순서를 **self-test 안 리터럴로 pin**(core 의 `LEG_ROLES` 를 읽어오면 좌우변이
#       동시 파생돼 항진한다 — 그 사유가 그 자리에 그대로 적혀 있다)
#       [재현] grep -n 'LEG_ROLES 를 읽어오면' "$0"
#     · 6축 이름을 **리터럴 pin**(core 의 `CENSUS_AXES`/`LOWER_BOUND_AXES` 파생 금지 —
#       "상수를 좁힐 때 양변이 함께 좁아져 검사가 항진한다")
#       [재현] grep -n '6축 이름을 \*\*self-test 안 리터럴로 pin\*\*' "$0"
#     · `FALS_REQUIRED_VOCAB` 를 `FALS_VOCAB` 와 **독립 리터럴**로 두고 **어긋남 자체를
#       신호**로 삼음("파생 금지가 이 배선의 전부다")
#       [재현] grep -n '^FALS_REQUIRED_VOCAB=' "$0"
#   ★ 위 3 항은 **줄번호를 적지 않는다**. 15회차 봉합이 본 파일에 +372 행을 넣으면서 종전에
#     적어 둔 좌표 3건(`:983`/`:1046`/`:2303`)이 통째로 어긋났고, 어긋난 좌표가 **실재하는 다른
#     줄**을 가리켜 감사자가 「인용이 참」으로 오독할 수 있는 상태가 됐다(본 회차 구현 자기검출).
#     이 Story 가 이미 4회 도달한 결론과 같다 — **정본은 재현 명령이지 좌표 리터럴이 아니다**.
...
#   있고(T-ANN · T-WIRE · T-WIRE-E · T-MUT — **4종 전부 선행 finding 의 처방으로 신설된 자기
#   대조군**), Story §8.14.7b 는 「신설물 **자신**에 born-RED 대조군 동반 필수」를 명문화까지
#   했다. 그런데 정작 §13.5 가 **자기 대조군 없이** 착지했다. 13회차 실측 2종이 각각
#   **1 논리행**으로 완전 무성이었다:
#     · A-TAUT  — 판정 비교를 항진화(선언 상수 대신 현재 서명 자신과 대조) → rc=0 · 3성분 불변 · ✗0
#     · A-SIGFN — 서명 산출을 상수 반환으로 치환                          → rc=0 · 3성분 불변 · ✗0
# ★ 왜 §14 총량 pin 처럼 「짝 소비자」로 덮이지 않는가. §14 에는 §15 종료 가드라는 짝이 있어
#   **블록 삭제**가 총계를 어긋내 rc=1 을 낸다. 그러나 §13.5 는 **판정만 항진화하면 케이스가
#   그대로 발화**해 총계가 보존된다 — 삭제가 아니라 **무력화**라서 총량 축에 걸리지 않는다.
# ★ 형태 = **양성 ∧ 음성 공존** (T-ANN 이 세운 규율). 서로의 맹점을 덮는다:
#     「항상 불변 / 항상 일치」 변이 → 양성 leg(b·c·d·e) 이 잡고,
#     「항상 이동 / 항상 불일치」 변이 → 음성 leg(a·f) 이 잡는다.
#   실측 대조 행렬 — **고정 열거를 정본으로 두지 않는다**(14회차 F-CR31-7). 종전 문면은
#   arm 을 라벨로만 적어 두어 **재현이 불가능**했고, 리뷰가 문면 그대로 실행하니 선언 집합
#   `{a,c,d}` 가 **재현되지 않았다**(실제 `{§13.5, a}`). ⇒ 각 arm 을 **처치 규칙**으로 적고,
#   RED 집합은 **그 규칙을 실행해 얻은 값**만 적는다. 아래는 구조 `82c3728aa` 에서 16 arm
#   직렬 실행 실측이다(구현 firsthand · LAB 격리 · 전후 null-control 2회 GREEN):
#     [처치 규칙]                                                    → [RED leg 집합]
#     null-control  무변형                                           → {} (79/0/1)
#     M-g1  verdict 안 인자를 §0 선언 상수로 치환(=리뷰 ARM2)         → {g}
#     M-g2  hgc_decl_match 를 `return 0` 항진화                       → {f, g}
#     M-g3  서명 산출을 **선언값** 상수 반환으로 치환                 → {b, c, d, g}
#     M-g3b 서명 산출을 **무관값** 상수 반환으로 치환                 → {구성앵커, b, c, d, h}
#     M-g4  생산자 대입(`hgc_cur_sig=`)을 선언 상수로 치환            → {} ★ 아래 ⓒ 참조
#     M-x2  §13.5 호출부를 `if true` 로 치환(판정 미호출)             → {a, b, c, d}
#     M-x3  §13.5 호출부에 `|| true` 부착(호출하되 rc 무시)           → {} ★ 열린 축, ⓔ
#     M-c1  탐침 b 생성을 **빈 파일**로 치환                          → {b}   (F-CR31-6 봉합)
#     M-b1  FALS_VOCAB 를 5→1 감산                                    → {T-FALS-d}
#     M-b2  FALS_VOCAB 에서 `내재` **단독** 제거                      → {T-FALS-d}
#     A-TAUT  판정을 P1 변수 자신과 대조(항진)                        → {e}
#     A-TAUT2 판정을 P2 변수 자신과 대조(항진)                        → {e, g}
#     A-NONCE 서명 산출에 입력 무관 per-call nonce 후행               → {구성앵커, a, b, c, h}
# ★ **M-g3 ↔ M-g3b 가 「양성 ∧ 음성 공존」의 실측 정당화다.** 같은 축(서명 산출 상수화)인데
#   상수가 **선언값**이면 양성 leg(g)이, **무관값**이면 음성 leg(h)이 잡는다. 한쪽만 두면
#   나머지 한 값이 통째로 빠져나간다 — 이 규율은 종전까지 논증으로만 서 있었고 여기서
#   **한 축을 두 값으로 갈라** 실측으로 섰다.
# ★ **성분별 분해(per-cell)** — ~~위 A-SIGFN 이 b 에서만 RED 이고 c·d 가 초록인 것이 그
#   실증이다~~ → **철회**(14회차 F-CR31-5, 3 관측자 독립 수렴). 그 arm 은 per-cell 을
#   **구별하지 못한다**: 탐침 b·c·d 는 각각 정확히 한 성분만 움직이므로 **가려줄 다른 이동이
#   애초에 없고**, 뭉쳐 재도 판정이 동일하다. 분해 그 자체는 이득이 아니었다.
#   ⇒ 실 이득은 **불변 conjunct**(대상 성분 이동 ∧ 나머지 두 성분 불변)가 만든다 — 그것이
#   붙고 나서야 「전 성분이 무너진 탐침」(M-c1 빈 파일)이 양성에서 걸러진다. per-cell 은
#   **conjunct 의 전제**이지 그 자체로 판별력이 아니다.
# ★ 정직 천장 (알고 안 닫음):
#   ⓐ **T-SIG 자신의 판정 경로에는 대조군이 없다.** 본 블록의 비교를 같은 방식으로 무력화하면
#      조용하다. 회귀는 **한 층 안쪽으로 이동했고 0 이 되지 않았다** — 이 Story 가 반복해서
#      관측한 형(봉합이 자기 대조군을 낳고 그 대조군이 다시 열린다)이 여기서도 그대로다.
#      [반증시도: 미수행 — 층 이동 자체는 실측했으나(§13.5 → §13.6) 다음 층의 무성 여부는
#       재지 않았다. 「닫혔다」로 읽지 말 것]
#   ⓑ 탐침이 겨누는 것은 **서명 산출 함수와 선언 정합 판정**이지 §13.5 의 baseline(merge-base)
#      leg 이 아니다. 그 leg 은 본 PR 에서 구조적 무발화(⊘ SKIP)라 대조군을 세울 실행면이 없다.
#   ⓒ **생산자 대입 자리(P1)는 「검출」이 아니라 「레버리지 소멸」로 처분했다.** 14회차에 봉합이
#      P2(verdict 내부 재산출)를 만들면서 같은 사실의 생산자가 2개가 됐고, M-g4(P1 을 선언
#      상수로 치환)가 **전건 GREEN**이었다 — 청정 상태에선 선언값 == 실측이라 관측면이 없다.
#      ⇒ 처분 = **모든 판정 소비자를 P2 로 통일**해 P1 에 소비자를 0 으로 만든 것이다. 그래서
#      M-g4 는 **지금도 GREEN 이고 그것이 정상**이다(변이가 무해해졌다는 뜻이지 검사가 눈멀었다는
#      뜻이 아니다). ★ **이 형은 born-RED 로 검증되지 않는다** — 검증 수단은 「P1 을 값으로 읽는
#      판정 소비자가 0」이라는 **구조 대조**(`grep -n 'hgc_cur_sig'` 전수 → 대입 1 · 주석 n ·
#      (e) 의 교란 대상 1, 판정 입력 0)다. 「born-RED 로 닫았다」로 적지 말 것.
#      [반증시도: 완료 — P1 소비자 전수를 grep 으로 확인했고 판정 입력 0 을 실측했다. 다만
#       「앞으로도 0」은 보장이 아니라 규약이다: 누군가 P1 을 다시 판정에 끌어들이면 축이 되살아나며
#       그것을 막는 기계 장치는 없다]
#   ⓓ **봉합은 인접 축을 연다 — 이번 회차에 같은 손으로 두 번 실측했다.** ① 인자 자리를 닫자
#      생산자 대입 자리가 열렸다(ⓒ) ② P1·P2 를 통일하자 **(e) 의 교란 정의역이 좁아져** 항진
#      구현이 다른 live 변수를 참조하는 형을 놓칠 뻔했다(M-e1 = 그 근접 실패의 GREEN 재현).
#      ⇒ **일반형: 대조군은 자기가 지목한 표현식만 덮는다.** 판정을 이루는 표현식 목록
#      (입력파일 → 서명산출 → 생산자대입 → 인자 → 술어 → 선언상수 → 판정소비 → 인쇄 → 집계 → rc)
#      에서 **지목되지 않은 원소가 다음 회차의 공격면**이다. 처방은 「대조군 하나 더」가 아니라
#      **원소를 열거하고 각각에 지목이 있는지 대조**하는 것이며, **봉합할 때마다 전 arm 재측정**이
#      필요하다(문면만 고치면 정의역이 어긋난 채 남는다). ★ 위 열거 자체도 **하한**이다.
#   ⓔ **판정 소비 배선 축 — 부분만 닫혔다.** 「§13.5 가 이 판정을 실제로 소비하는가」는 (g)(h) 가
#      덮지 못한다(그 leg 들은 verdict 함수를 **직접** 부른다). 실측 2형:
#        · M-x2 `if true` (호출 자체 소멸) → **RED {a,b,c,d}**. `hgc_verdict_actual` 이 공백으로
#          남아 a 의 NOT_RUN 가드가 fail-closed 로 잡는다 — **통일의 부수 효과로 닫혔다**.
#        · M-x3 `|| true` (호출은 하되 rc 무시) → **GREEN. 열린 채다.** 함수가 실제로 돌아
#          산출값이 서므로 위 가드가 서지 않는다.
#      ⇒ **「소비 배선 축을 닫았다」로 쓰지 말 것.** 닫힌 것은 「미호출」 하위 축 하나뿐이다.
#      M-x3 형의 일반 해법은 「`✗` 발화 수와 `FAIL` 계수의 정합을 독립 assert」인데 그것은
#      **잔여 #12 의 닫는 조건**으로 이미 등재돼 있고 본 회차 범위 밖이라 미착수다.
#      [반증시도: 완료 — M-x2·M-x3 두 형을 각각 실행해 한쪽만 닫혔음을 실측했다. 열린 쪽을
#       닫는 시도는 **하지 않았다**(범위 밖, 잔여 #12 로 이관)]
echo ""
echo "── T-SIG: 구성 앵커 자신의 판별력 (born-RED 대조군) ─────────────────────────"

# 서명 문자열에서 성분 1개를 뽑는다 (형식 = `census=f/p/s helper=H loopdecl=L`).
tsig_field() { printf '%s' "$2" | sed -n "s/.*$1=\([^ ]*\).*/\1/p"; }

tsig_probe="$TEST_TMP/tsig-probe.sh"

# (a) 음성 대조군 — 무변형 사본의 서명은 현재 서명과 같아야 한다.
...
#   상수로 치환된 것이다(A-SIGFN 계열).
#   ★ 탐침 사본은 **해시 전용**이다 — 실행하지 않으므로 구문 유효성을 요구하지 않는다.
#   ★ census 탐침 행을 `printf ... 'fail_case'` 로 조립하는 이유: 탐침 **자신의 소스**가 줄머리
#     카운터 호출로 보이면 본 파일의 census 가 탐침 때문에 부풀어 오른다.
for tsig_case in \
  "b:census:줄머리 카운터 호출 1행 추가" \
  "c:helper:카운터 헬퍼 정의 창 안 1행 삽입" \
  "d:loopdecl:반복 항목 선언 헤더 1행 추가"; do
  tsig_id="${tsig_case%%:*}"
  tsig_rest="${tsig_case#*:}"
  tsig_comp="${tsig_rest%%:*}"
  tsig_desc="${tsig_rest#*:}"
  ...
  case "$tsig_id" in
    b) cp -- "$hgc_self_src" "$tsig_probe"
       printf '  %s "T-SIG census probe"\n' 'fail_case' >> "$tsig_probe" ;;
    c) awk '{print} /^pass_case\(\) \{/{print "# T-SIG helper probe"}' \
         "$hgc_self_src" > "$tsig_probe" ;;
    d) cp -- "$hgc_self_src" "$tsig_probe"
       printf 'for tsig_probe_item in alpha beta; do\n' >> "$tsig_probe" ;;
  esac
...
# (e) 양성 — **판정이 §0 선언 상수에 의존함**을 실증한다.
#   ★ 왜 「다른 인자 → 불일치」 탐침으로는 부족한가: 청정 상태에서는 선언값 == 현재 서명이라
#     `[ 인자 = 선언 ]` 과 `[ 인자 = 현재서명 ]` 이 **외연적으로 같은 함수**다. 인자만 흔드는
#     탐침은 두 구현을 구별하지 못한다.
#     [반증시도: 완료 — 인자-only 탐침(선언값+sentinel 주입)을 LAB 격리 드라이버에서 실행 →
#      청정본·항진본이 **둘 다** 불일치를 반환해 구별 실패. ⇒ 인자가 아니라 **비교 대상 변수**를
...
#     [반증시도: 완료 — 항진 대상을 달리한 두 변형을 LAB 격리 shim 에서 직렬 실행:
#      항진 대상 = P1(`hgc_cur_sig`) → rc=1 · RED = **{(e)} 단독** / 항진 대상 =
#      P2(`hgc_verdict_actual`) → rc=1 · RED = **{(e), (g)}**. ★ 「둘 다 (e) 단독」이 아니다 —
#      본 주석의 잠정 문면이 그렇게 적혀 있었고 **실측이 그것을 정정했다**(P2 형은 (g) 도 잡는다).
#      그리고 소비자 통일 직후의 「교란 대상 1개」 상태를 복합 변이로 재현하니 P1 항진형이
#      **rc=0 · PASS=79 FAIL=0 SKIP=1 · RED 0** 으로 전건 통과했다 — 그 관측이 「교란 대상
#      1개면 충분」을 반증한다. 「서명 사본을 담은 새 변수를 추가하는 형」은 **미수행** —
#      열거의 하한 지위는 그대로다]
...
# (f) 음성 — 선언값과 다른 값은 불일치여야 한다. 「무엇이든 일치」 변이를 잡는다((e) 의 맹점).
if hgc_decl_match "${HGC_DECLARED_COMPOSITION}-tsig-sentinel"; then
  fail_case "T-SIG-f (판정 비-항상일치): 선언값과 명백히 다른 값이 **일치**로 판정됐다 — 비교가 항상-참으로 죽었다"
else
  pass_case "T-SIG-f (판정 비-항상일치): 선언값과 다른 값을 불일치로 판정 — 비교가 「무엇이든 일치」가 아니다(음성 · (e) 의 맹점을 덮는다)"
fi

# (g)(h) = **판정 끝단 대 끝단** 짝 (F-CR31-1 봉합). b·c·d 는 서명 산출 함수를, e·f 는 술어를
#   각각 **따로** 잰다. 그 둘을 잇는 자리 — 대상 파일 결박 · 산출값이 술어에 **인자로 들어가는
#   자리** — 는 6 leg 어디에도 없었고, 그래서 호출부 1-토큰 치환(14회차 ARM2)이 6 leg 전원 초록
#   인 채로 §13.5 를 항진시켰다. (g)(h) 는 `hgc_composition_verdict` 를 **통째로** 호출해 그
#   전 경로를 한 번에 탐침한다.
# ★ 형태는 §13.6 이 세운 규율 그대로 **양성 ∧ 음성 공존**이다. 서로의 맹점을 덮는다 —
#   (g) 단독은 자기-보호를 못 하고(판정이 「항상 불일치」면 통과), (h) 단독은 자기-건전하지
#   않다(판정이 「항상 일치」면 통과).

# (g) 양성 — 구성이 **실제로 이동한** 사본을 판정 대상에 걸면 판정은 **불일치**여야 한다.
#   일치가 나오면 판정 경로 넷(파일 결박 · 서명 산출 · 함수 안 인자 · 술어) 중 하나가 상수로
#   치환된 것이다.
tsig_g_saved_src="$hgc_self_src"
cp -- "$tsig_g_saved_src" "$tsig_probe"
printf '  %s "T-SIG-g verdict probe"\n' 'fail_case' >> "$tsig_probe"
hgc_self_src="$tsig_probe"
tsig_g_verdict="불일치"
if hgc_composition_verdict; then tsig_g_verdict="일치"; fi
tsig_g_seen="$hgc_verdict_actual"
hgc_self_src="$tsig_g_saved_src"
if [ "$tsig_g_verdict" = "불일치" ]; then
  pass_case "T-SIG-g (판정 끝단 대 끝단 · 양성): 구성이 이동한 사본을 판정 대상에 걸자 판정이 **불일치** [$tsig_g_seen] — 판정 경로(대상 파일 결박 → 서명 산출 → **인자** → 술어)가 상수로 치환되지 않았다는 양성 증거. ★ 덮는 것: 함수 안 인자를 선언 상수로 바꾸는 1-토큰 치환(14회차 ARM2 재현형) · 술어 항진화 · 서명 산출의 선언값 상수화. ★ 덮지 **못하는** 것: 이 판정을 §13.5 가 **실제로 소비하는가**(호출 site 배선) — §13.6 정직 천장 ⓔ 참조"
else
  fail_case "T-SIG-g (판정 끝단 대 끝단 · 양성): 구성이 **이동한** 사본에도 판정이 [$tsig_g_seen] 로 **일치** — 판정 경로가 입력을 읽지 않는다(파일 결박 · 서명 산출 · 인자 · 술어 중 하나가 상수). 이 상태에서 §13.5 선언 정합 leg 은 어떤 구성 이동에도 초록이다"
fi

# (h) 음성 — 실제 대상에 대해서는 **일치**여야 한다. 「무엇이든 불일치」 변이를 잡는다((g) 의 맹점).
if hgc_composition_verdict; then
  pass_case "T-SIG-h (판정 끝단 대 끝단 · 음성): 실제 대상에 대해 판정이 **일치** [$hgc_verdict_actual] — 판정이 「무엇이든 불일치」가 아니다. (g) 와 짝으로만 의미가 있다: 양성 leg 은 판정을 항상-불일치로 죽여도 통과하고, 음성 leg 은 항상-일치로 죽여도 통과하므로 **둘이 공존해야** 판정 경로가 닫힌다"
else
  fail_case "T-SIG-h (판정 끝단 대 끝단 · 음성): 실제 대상인데 판정이 **불일치** [$hgc_verdict_actual] ≠ §0 선언 [$HGC_DECLARED_COMPOSITION] — 판정이 항상-불일치로 죽었거나 §0 선언이 실측과 어긋났다(후자면 §13.5 도 함께 RED 다)"
fi

# ═══════════════════════════════════════════════════════════════════════════════
# 13.7. T-FALS = 「불가」 라벨의 반증시도 필수 필드 (F-CR30-5 기계 장치)
# ═══════════════════════════════════════════════════════════════════════════════
# ★ 무엇이 반복됐나. 규율 ⓐ(**반증 시도 후에만 「불가」를 부여한다**)는 이미 문서에 있었다.
#   그런데 12회차가 그 규율을 신설한 **직후** 스스로 위반했고(§0 「원리적 미도달」), 13회차에
#   또 위반이 나왔다(잔여 #18ⓑ 「원리적으로 닫히지 않는다」 → EXIT trap 3-arm 대조로 반증).
#   ⇒ **규율을 한 번 더 쓰는 것은 처방이 아니다.** 두 번 연속 같은 자리에서 실패했다.
# ★ 그래서 여기서 하는 것은 **라벨을 붙이는 자리에서 필드를 기계적으로 요구**하는 것이다.
#   「불가」 계열 어휘를 품은 주석 블록은 같은 블록 안에 `[반증시도: …]` 를 동반해야 한다.
#   ★ **「미수행」도 유효한 값이다** — 이 장치가 강제하는 것은 *반증에 성공했음*이 아니라
#     *시도 여부를 명시했음*이다. 침묵하던 단정을 **선언된 열림**으로 바꾸는 것이 전부다.
#     (닫힘을 위조하도록 요구하면 그것이야말로 본 Story 가 겨냥한 hollow 패턴이다.)
# ★ 정직 천장 — **어휘 집합은 「하한」이다**(§0 공격면 열거와 같은 지위). 새 표현은 빠져나간다.
#   ★★ 이것은 추정이 아니라 **본 회차 실측**이다: 어휘 초안(원리적 / 구조적 불가 /
#     닫히지 않는다)으로 스캔했을 때 **정작 F-CR30-5 를 촉발한 §15 의 그 주장이 걸리지
#     않았다** — 그 자리는 「순차 프로그램에 **내재**」로 적혀 있었기 때문이다. 어휘를 넓혀
#     (`내재` 추가) 잡았지만, 이 사건 자체가 **문면 기반 술어의 한계를 그 자리에서 실증**한다.
#     ⇒ 「이 장치가 「불가」 단정을 봉인한다」로 읽지 말 것. 재는 것은 **등재된 어휘의 표기**이지
#     주장 일반이 아니다.
#     [반증시도: 완료 — 초안 어휘로 스캔 시 §15 주장 **미검출**을 실측했고, 그 실측이 「어휘
#      집합이 주장 일반을 덮는다」를 반증했다. 어휘 확장은 그 반례 1건을 흡수했을 뿐 일반적
#      완전성을 주지 않는다]
#   ★ 블록 단위 요구라 **큰 주석 블록은 필드 1개로 충족**된다(정의역 거칢). 문장 단위 결박은
#     하지 않았다 — 그 정밀도를 주장하지 않는다.
echo ""
echo "── T-FALS: 「불가」 주장의 반증시도 필드 (born-RED 대조군) ──────────────────"
...
fals_scan() {
  awk -v vocablist="$FALS_VOCAB" -v field="$FALS_FIELD" '
    BEGIN { nv = split(vocablist, vocab, "|") }
    function flush_block() { if (has_claim && !has_field) print block_start; has_claim=0; has_field=0; in_block=0 }
    { line=$0; sub(/^[[:space:]]+/,"",line)
      if (line ~ /^#/) {
        if (!in_block) { in_block=1; block_start=NR; has_claim=0; has_field=0 }
        for (v=1; v<=nv; v++) if (index($0, vocab[v]) > 0) has_claim=1
        if (index($0, field) > 0) has_field=1
      } else if (in_block) flush_block()
    }
    END { if (in_block) flush_block() }
  ' "$1"
}

# 합성 fixture 2종 — 같은 주장 문면에 **필드만** 다르다(단일 변수 대조).
printf '# 이 축은 원리적으로 닫히지 않는다\n# 부연 서술 한 줄\nfals_fixture_code=1\n' > "$TEST_TMP/fals-unfielded.sh"
printf '# 이 축은 원리적으로 닫히지 않는다\n# [반증시도: 3-arm 대조 실행 --> rc=1 로 반증됨]\nfals_fixture_code=1\n' > "$TEST_TMP/fals-fielded.sh"

fals_unf="$(fals_scan "$TEST_TMP/fals-unfielded.sh" | grep -c . || true)"
fals_fld="$(fals_scan "$TEST_TMP/fals-fielded.sh" | grep -c . || true)"

# (a) 양성 — 필드 없는 주장을 검출해야 한다. 술어가 「무엇도 검출 안 함」이면 여기서 RED.
if [ "$fals_unf" -ge 1 ]; then
  pass_case "T-FALS-a (양성): 반증시도 필드 없는 「불가」 주장 fixture 를 ${fals_unf}건 검출 — 술어가 「무엇도 검출하지 않음」이 아니다"
else
  fail_case "T-FALS-a (양성): 필드 없는 주장 fixture 를 검출하지 못했다(검출 ${fals_unf}건) — 술어가 죽었다. 본체 위반 0 을 커버리지로 읽을 수 없다"
fi

# (b) 음성 — 필드가 붙으면 검출 0 이어야 한다. 술어가 「무엇이든 검출」이면 여기서 RED.
if [ "$fals_fld" -eq 0 ]; then
  pass_case "T-FALS-b (음성): 같은 주장에 반증시도 필드를 붙인 fixture 는 검출 0 — 술어가 「무엇이든 위반」이 아니다((a) 의 맹점을 덮는다)"
else
  fail_case "T-FALS-b (음성): 필드를 붙였는데도 ${fals_fld}건 검출 — 술어가 항상-위반으로 죽었다"
fi
...
#   ★ 정직 천장 — 현 배선에서 창은 0 이 아니다. 남는 것은 두 자리다:
...
#                 ⓑ 동일 주입 + `trap … EXIT` 가 **최종 라이브 tally** 재평가 → **rc=1(닫힘)**
#                 ⓒ trap 有 · 주입 없음 → rc=0(항진 아님 — 대조군 성립)

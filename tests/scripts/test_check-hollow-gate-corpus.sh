...
#
#   ★ **공격면은 3곳이다** — F-CR27-1 정정. 종전 열거는 2곳이었고 **공유 앵커인 리터럴
#     자신이 빠져 있었다**:
...
#     (1)(2) 만 놓고 보면 서로의 삭제를 덮는 것이 맞다(X3·X4 실측). 그러나 (3) 을 공격면에
#     넣으면 그 상호 보호는 성립하지 않는다.
#   ★ 정직 천장 (전건 실측 — 추론 0):
#       · **silencing 1-편집으로 거짓 초록이 성립한다.** 케이스 1건을 지운 뒤 본 리터럴을
#         1행(`66→65`) 고치면 rc=**0** · `FAIL=0` · ✗ **0건** · **경고 0줄**이다(리뷰 M-B 실측).
#       · 종전 문면 「1-편집으로는 조용해지지 않는다」와 「거짓 초록 = 2-편집 / 바이트 동일 =
#         3-편집」 비용 모델은 **실측으로 반증되어 삭제**했다. 실제로는 2-편집 경로(X5:
#         (1)(2) 동시 삭제)가 stdout 에 종료 가드 경고 1줄을 **남기는** 반면 1-편집 경로(M-B)는
#         **아무것도 남기지 않는다** — 더 싼 경로가 더 조용하다(비용 모델 **역전**).
#       · ⇒ 본 pin 이 실제로 주는 것은 **무력화 불가능성이 아니라 리뷰 가시성**이다:
#         케이스 증감이 **named constant 의 명시적 diff 를 강제**한다. 원 회귀 class(우발적
#         케이스 소실)는 그대로 잡는다 — 리터럴 미갱신 삭제는 RED(M-A 실측 rc=1).
#   ★ **기계 판정으로는 종결 불가 (확정)**: 기대값 앵커를 하네스 밖(CI 워크플로·registry)으로
#     옮겨도 그 값 역시 **같은 커밋에서 1행 수정 가능**하다 — 어떤 in-repo 앵커든 동일하다.
#     따라서 종전의 「밖에 두면 종결된다」는 **거짓**이고 삭제했다. 이설이 바꾸는 것은 판정
#     가능성이 아니라 **diff 가시성·승인 표면**(CODEOWNERS 분리 등)뿐이며, 그것은 **닫힘이
#     아니라 별 축의 부분 완화**다 — 두 축을 섞어 쓰지 않는다.
#     Story §8.11.8 #12 로 등재(닫는 조건 = **무효** 표시).
...
# ═══════════════════════════════════════════════════════════════════════════════
# 기대치의 사유·공격면 3곳·정직 천장은 §0 `HGC_EXPECTED_CASE_TOTAL` 정의 주석이 SSOT.
...
#   ★ 단 그 성질은 **공유 앵커(리터럴) 를 건드리지 않는 편집에 한정**된다 — 리터럴을 함께
#     갱신하면 두 소비자가 정합 이동해 조용해진다(F-CR27-1 / §0 정직 천장).
echo ""
echo "── 케이스 총량 pin (은닉 소실 검출) ─────────────────────────────────────────"
hgc_cases_before_pin=$((PASS+FAIL+SKIP))
if [ "$hgc_cases_before_pin" -eq $((HGC_EXPECTED_CASE_TOTAL-1)) ]; then
  pass_case "케이스 총량 pin: 등재 케이스 ${hgc_cases_before_pin}건 = 선언 상수 $((HGC_EXPECTED_CASE_TOTAL-1))건 — 케이스 증감은 §0 HGC_EXPECTED_CASE_TOTAL 의 명시적 diff 없이는 통과하지 못한다 (상수를 함께 갱신한 소실은 본 검사를 통과한다 — 검출이 아니라 리뷰 가시성)"
else
  fail_case "케이스 총량 pin: 등재 케이스 ${hgc_cases_before_pin}건 ≠ 기대 $((HGC_EXPECTED_CASE_TOTAL-1))건 — 케이스가 tally 에서 조용히 사라졌거나(loop 항목 삭제·조기 continue·블록 제거) 신설분이 기대치에 미반영이다. 의도적 증감이면 §0 HGC_EXPECTED_CASE_TOTAL 을 함께 갱신하라"
fi
...
# ★ 「상호 보호」는 리터럴을 건드리지 않는 편집에 한정된다 — §0 정직 천장 참조(F-CR27-1).

...
#
#   ★ **상호 보호** — 본 리터럴의 소비자는 **2곳**이고 서로의 삭제를 덮는다:
#       (1) §14 총량 pin 케이스    — 케이스 소실/증가를 FAIL 로 발화
#       (2) §15 종료 가드 conjunct — pin 블록 **자신의 삭제**를 rc=1 로 발화
#     (1) 만 지우면 → 총계가 기대치에 미달해 (2) 가 RED. (2) 만 지우면 → 케이스 소실은 (1) 이
#     여전히 RED. **1-편집으로는 조용해지지 않는다.**
#   ★ 정직 천장: 이것은 **종결이 아니라 1-편집 무력화의 봉쇄**다. (1)(2) 를 **동시에** 지우면
#     초록이 복원된다 — 실측(X5): rc=**0** · `FAIL=0` · RED 0.
#     ★ 다만 「조용하다」로 뭉뚱그리지 않는다 — X5 에서도 **stdout 에 종료 가드 경고 1줄은 남는다**
#       (경고 `echo` 블록은 지워지지 않았다). CI 판정자가 **rc** 이므로 **거짓 초록은 2-편집으로
#       성립**하고, baseline 과 **바이트 동일**해지려면 그 경고까지 **3-편집**을 요한다.
#       이 구분은 F-CR25-7 이 「악용 미실증(부재 단정 아님)」을 정확히 남겼던 규율의 계승이다.
#     하네스 내부 pin 은 원리적으로 이 회귀를 끝내지 못한다(pin 을 지키는 pin 이 다시 필요) —
#     종결하려면 기대 총량 앵커를 **하네스 밖**(CI 워크플로·registry)에 두어야 하며 본 회차 범위
#     밖이다. Story §8.11.8 #12 로 등재.

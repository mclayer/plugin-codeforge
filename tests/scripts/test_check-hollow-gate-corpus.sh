...
#   실측 대조 행렬(구현 firsthand, LAB 격리 드라이버):
#     청정 → 6/0 GREEN · A-SIGFN → b 단독 RED · A-TAUT → e 단독 RED ·
#     비교-항상참 → f 단독 RED · 서명-항상이동 → a·c·d RED.
# ★ **성분별 분해(per-cell)** — 서명 3 성분을 한 덩어리로 재면 **한 성분만** 상수화한 변이가
#   다른 성분의 이동에 가려진다. 위 A-SIGFN 이 b 에서만 RED 이고 c·d 가 초록인 것이 그 실증이다.
# ★ 정직 천장 (알고 안 닫음):

#!/usr/bin/env bash
# CFP-426 / CFP-428 / ADR-040 Amendment 3 — unit test for check-worktree-first-pre-checkout.sh (actual logic)
# Tests:
...
SCRIPT_REL="scripts/check-worktree-first-pre-checkout.sh"
SAMPLE_REL="templates/.git-hooks/pre-checkout.sample"
...
if [ "$STATUS" -eq 0 ] && echo "$OUT" | grep -q "\[worktree-first-pre-checkout\] OK"; then

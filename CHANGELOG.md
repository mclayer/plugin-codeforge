...
- [CFP-1646] **ADR-024 Amendment 16 — §결정 6.A.8 신설 Issue label timing discipline (backlog `phase:*` reflex 차단) + `hotfix-bypass:backlog-issue-label` 103번째 family member + label-registry-v2 v2.77 → v2.78 MINOR bump** (full-lane per src 동반, doc-only fast-path ADR-054 미적용 — workflow + script + bats Wave 2 carrier 동반, Issue #1646 USER-UTTERANCE-VERBATIM CFP-1111 follow-up #1198/#1199 phase:설계 reflex incident origin)
  - **§결정 6.A.8 신설** (Issue label timing discipline) — 즉시 lane 진입하지 않는 backlog / 예약 / follow-up CFP Issue 발의 시 `phase:*` label 부착 금지. 미착수 backlog 관례 = unlabeled (Issue #1112 / #1169 prior art pattern). 본 invariant = "발의-time 에 lane spawn 자동 follow 가정 차단".
  ...
  - **Bypass channel** = `hotfix-bypass:backlog-issue-label` 103번째 (raw active concrete grep count post-append, 102 + 1 = 103) family member — §결정 6.A per-entry namespace 영구 확장. 기존 single audit lint `scripts/check-bypass-audit-comment.sh` reuse (CFP-389 prior art, 신규 audit lint 도입 0건).
  - **Behavioral directive normative 승격 첫 carrier** (CLAUDE.md "Orchestrator 정책 적용 범위 + behavioral directive (normative)" 단락 정합) — 사용자 directive 를 personal memory file 저장 만으로 갈음 금지. 본 Story = memory `feedback_no_phase_label_on_backlog_issues.md` (ephemeral + consumer 비전파 + single-session scope = structural enforcement 불가) → ADR-024 normative SSOT 승격 첫 사례. 향후 mandate 의 template precedent.
  - **Wave 1 declarative (본 Phase 1 PR scope)**: ADR-024 Amendment 16 본문 + frontmatter `amendments[]` row + `mechanical_enforcement_actions[]` row + `amended_by/amended_date` 갱신 / label-registry-v2 v2.77 → v2.78 MINOR bump + `hotfix-bypass:backlog-issue-label` 103번째 family member append + amendment_log row append / MANIFEST.yaml `"2.77"` → `"2.78"` sync / evidence-checks-registry `backlog-issue-phase-label-forbid` warning-tier entry append (ADR-060 §결정 5 default warning mode) / `templates/github-workflows/backlog-issue-label-forbid.yml` Wave 1 stub + `.github/workflows/` self-app byte-identical (ADR-005) / internal-docs Change Plan `plugin-codeforge/change-plans/cfp-1646-backlog-issue-label-invariant.md` (chief author cross-repo write).
  - **Wave 2 (Phase 2 PR scope)**: workflow body actual lint wire (3-AND predicate detection) + `scripts/check-backlog-issue-label.sh` thin-wrapper (ADR-061) + `scripts/lib/check_backlog_issue_label.py` (ADR-061 multi-line Python external file) + `tests/bats/check-backlog-issue-label.bats` RED→GREEN stash proof (CFP-1334 mandate) + `tests/fixtures/backlog-issue-label/{pass,fail}/` boundary fixture pair (CFP-963 Codex TP#4) + evidence-checks-registry `status: deferred-followup` → `warning` 전환.
  - **Wave 3 (Phase 2 PR merge 후 별 task)**: memory 3건 retire — `feedback_no_phase_label_on_backlog_issues.md` (normative 승격 완료 → ephemeral memory retire) + `feedback_brainstorming_why_first.md` (skill `codeforge:codeforge-brainstorm` body L63/66/100/140-145 4 anchor + 9 grep hits HIT verified → retire only, skill body 보강 carrier 불필요) + `feedback_codex_collaboration_patterns.md` (ADR-052 + ADR-070 + ADR-081 codified → retire only) + MEMORY.md index 3 line 제거.
  - **chief tie-break ladder 1단계 RACI lookup** — ModuleArchitect primary axis (boundary axis unified — label timing discipline = module-level governance boundary). §결정 6.A.N per-entry namespace 패턴 답습 정합 = single axis advocate sufficient → deputy fan-out skip via mechanical_fast_path_inline precedent (Amendment 15 CFP-1607 답습). Option A (§결정 6.A.8 sub-decision) 채택 vs Option B (신규 §결정 7 separate axis) reject — axis disjoint criteria 모호 + Amendment 4/5/6/7/8/9/10/11/13/14/15 모두 §결정 6.A.N add-only pattern 답습 정합.
  - **ADR-082 §결정 9 verify-before-write + ADR-070 D1 verify-before-trust + ADR-040 Amendment 6 §결정 7.J.3 worktree absolute path 강제 directive 정합 적용** — worktree base e1e2b751 (CFP-1632 HEAD = origin/main identical) `git -C <worktree_abs_path> show HEAD:<path>` direct verify. filesystem Read tool stale main repo path snapshot 회피 (FIX iter-1 self-fault recovery carrier — Orchestrator premise resolution SendMessage). META self-application ADR-082 §결정 1-K 4-step numeric claim verify-before-write mandate 22nd applied case (raw active concrete grep count post-append = 102 + 1 = 103 정합) + §결정 1-G amendment-slot pre-reservation mandate 5th applied case + §결정 1-K numeric claim write-time strict claim mandate 3rd applied case.
  - **PMOAgent retro flag** (별 carrier — Phase 2 PR merge 후 retro spawn 시): `stale_fact_inheritance` + `worktree_path_mismatch` pattern_count 누적 — Orchestrator initial turn Grep verify stale (main repo path 사용, 18 commit behind origin/main, SessionStart hook warning 무시) + ArchitectPL inline verify falsified (worktree path 미사용 / hallucinated). 양쪽 모두 ADR-082 §결정 9 + ADR-073 Amendment 2 violation. ADR-045 §D-9 escalation territory (pattern_count ≥ 2 reach Mandatory) — sister carrier 후속.
  - **Files**: `docs/adr/ADR-024-story-scoped-branch-policy.md` (frontmatter amendments[] Amendment 16 row + amendment_log[] entry + `amended_by`/`amended_date` + `mechanical_enforcement_actions[]` row + 본문 `## Amendment 16` section) / `docs/inter-plugin-contracts/label-registry-v2.md` (frontmatter version `"2.77"` → `"2.78"` + amendment_log row append + `hotfix-bypass:backlog-issue-label` 103번째 family member entry append) / `docs/inter-plugin-contracts/MANIFEST.yaml` (label_registry row `"2.77"` → `"2.78"` sync) / `templates/github-workflows/backlog-issue-label-forbid.yml` (Wave 1 declarative stub, Phase 2 carrier hydrate) / `.github/workflows/backlog-issue-label-forbid.yml` (self-app byte-identical, ADR-005) / internal-docs `plugin-codeforge/stories/CFP-1646.md` (Story §3/§7/§11 append) / internal-docs `plugin-codeforge/change-plans/cfp-1646-backlog-issue-label-invariant.md` (Change Plan chief author cross-repo write) / `CHANGELOG.md` (본 entry append)
  - **Cross-ref**: Issue #1646 USER-UTTERANCE-VERBATIM (CFP-1111 follow-up #1198/#1199 origin) / sister CFP-1645 (memory codify carrier 병렬 session 1/3, file 영역 disjoint) / sister CFP-1647 (ADR-082 Amd 27 sub-scope 1-P Wave 2 mechanical wire 병렬 session 3/3, file 영역 disjoint) / prior art Amendment 15 (CFP-1607 chronological direct precedent — mechanical_fast_path_inline single-axis deputy fan-out skip 답습) / prior art Amendment 13/14 (CFP-1510/CFP-1306 §결정 6.A.N sub-decision 신설 패턴 답습) / prior art Amendment 11 (CFP-1006 raw count convention 답습) / Issue #1112 / #1169 (backlog Issue unlabeled prior art pattern) / Issue #1198 / #1199 (phase:설계 reflex incident origin)

...
**사용 대상**: DocsAgent (생성·섹션 갱신 단독), 모든 에이전트 (파일 경로 + 섹션 번호 참조해 read-only fetch via `Read`)
...
DocsAgent는 Action이 처리 못하는 후속 갱신만 담당.
...
**타이밍**: 파일 생성 시점엔 placeholder (요구사항 레인 진입 전 비어있음). DomainAgent가 Analyst·Researcher와 **병렬 실행** 후 결과 반환하면 RequirementsPL이 DocsAgent 경유로 §2 채움 — 따라서 **Analyst·Researcher는 §2를 입력으로 참조하지 않음** (독립 관점 보장). §5·§6과 같은 사이클에 동시 기록.
...
[`impl-manifest.md`](impl-manifest.md) 스키마 따름. DocsAgent가 §8.5에 테이블 기록 → Phase 2 PR에 commit → `subissue-from-impl-manifest.yml` Action이 자동으로 file 단위 sub-issue 생성 (DocsAgent의 sub_issue_write 직접 호출 불필요).
...
DocsAgent가 append-only 관리 (행 삭제·수정 금지). "현재 사이클" count는 RESET 마커 이후 iteration만 합산. §10 commit이 main에 push되면 `fix-ledger-sync.yml` Action이 자동:

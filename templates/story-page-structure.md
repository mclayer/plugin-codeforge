...
- (consumer scope) application-BC 어휘 사전 owner-path = `docs/domain-knowledge/domain/<area>/lexicon.md` + `concept-dictionary.md` (DomainAgent build+maintain, CFP-2453/ADR-091 Amd3 — 4-way drift-sync: lane-self-write-ownership-matrix.yaml canonical ↔ lane-self-write SKILL.md ↔ 본 표 ↔ lint regex)
...
change-plan §8.9 미러 cross-ref (본문 상세 SSOT = change-plan.md §8.9). §8 burden-flip 표준(feasible 동적 검증 default 수행 — do-it-unless-proven-infeasible)의 **보안 축** — G4-fuzz(oracle=기능 crash/robustness)와 달리 oracle=보안 취약 재현(attack — injection 실행·인증 우회·민감데이터 노출·설정 취약). 배포/기동 앱을 실제 구동해 공격을 능동 재현하는 실행-보안 검증(single `dast` axis — §8.8 4기법 loop 미복제). 상주 실행 서비스(공격 표면 존재) Story 는 §8.9.0 applicability 표(`dast` 1 행 `DO|N/A` + `g_boundary_check`)로 판정 후 DO 시 12 산출물 계약 필드(target·attack_surface·scanner_or_harness·payload_class·oracle·repro_seed·execution_budget·pass_condition·status·auth_mode·environment_ref·observed_result)를 §8 개발 서사에 포함하고, 실 실행은 consumer `test.yml`/`dast.yml`(QADeveloperAgent, ADR-048 CI-native)로 배선한다(신규 codeforge 러너 부활 금지). 2 cross-field 선언-정합(declared-consistency, detection 아님): active/destructive payload ⟹ non-prod/ephemeral environment_ref (blast-radius) / authenticated 표면 ∧ unauthenticated auth_mode ⟹ infeasibility_reason (silent false-negative 차단). deployable service 0(runtime-inert) 시 §8.9.x aggregate N/A (substantive 30자 이상 reason, 3축 AND). soak/restart/replay = G2 단일소유(§8.5 disjoint) — `g_boundary_check` 로 경계 무침범 확인. 정직 천장(§8.9.5): 게이트는 presence/구조 + 2 cross-field 선언-정합까지만 — 검출력(=G3)/완결성/사유타당성/g_boundary_check 실준수는 review·advisory·G3 로 defense-in-depth.

#### §8.10 dark-path 로스터 (default-off flag 활성화 — oracle=activation ⊥ G4 robustness ⊥ G5 attack — CONDITIONAL — CFP-2624 / ADR-152)

change-plan §8.10 미러 cross-ref (본문 상세 SSOT = change-plan.md §8.10). §8 burden-flip 표준(do-it-unless-proven-infeasible)의 **dark-path 활성화 축** — "코드가 default-off flag(예: mctrader `COMPACT_TIERED=0`) 뒤에 숨어 landing(repo 착지)했다는 사실만으로 done(요구 충족)으로 인정"하는 병을 차단한다: **landing ≠ done**. default-off flag 뒤 도달 가능한 product 코드는 그 flag 를 실제 ON 으로 켜서 행사하는 **discriminating test(discriminating-B)** + ON-state 관측 assertion 을 선언해야 done(flag-OFF-only 통과 = dark-path 미행사). oracle 로 축이 갈린다: 활성화(flag ON 행사) ⊥ G4-fuzz(robustness) ⊥ G5-DAST(attack). discriminating-**B**(product 활성화 축) ⊥ discriminating-**A**(게이트 self-test mutation-kill, G6 소유). §8.9(DAST 점유) → 다음 자유 번호 **§8.10**(single `dark_path` axis — §8.8 4기법 loop 미복제, ADR-152 §결정2). 관련 코드 있는 Story 는 §8.10.0 applicability 표(`dark_path` 1 행 `DO|N/A` + `g_boundary_check`)로 판정 후 DO 시 6 산출물 계약 필드(flag_identifier·default_state·activation_test_ref·on_state_assertion·discriminating_basis·status)를 §8 개발 서사에 포함하고, 실 활성화 test 는 consumer `test.yml`(QADeveloperAgent, ADR-048 CI-native)로 배선한다. 2 cross-field 선언-정합(declared-consistency, detection 아님): status=activated ⟹ activation_test_ref non-empty ∧ on_state_assertion substantive(≥15자) (빈 stub 활성화 위장 차단) / status=infeasible ⟹ infeasibility_reason(≥30자). default-off product flag 도입 0 시 §8.10.x aggregate N/A (substantive 30자 이상 reason, 3축 AND). soak/restart/replay=G2·fuzz=G4·attack=G5 disjoint — `g_boundary_check` 로 경계 무침범 확인. 정직 천장(§8.10.5): 게이트는 presence/구조 + 2 cross-field 선언-정합까지만 — 검출력(discriminating-B 실행사)/완결성/사유타당성/g_boundary_check 실준수는 review·advisory·G3 로 defense-in-depth.

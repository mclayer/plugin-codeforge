...
> **설계 lane sub-domain owner 가이드 (CFP-1026 S2 — ADR-042 Amendment 7 / `deputy-mandate` skill cross-ref)**: 설계 lane 의 §3 "도입할 설계" 본문 (Change Plan §3 SSOT = codeforge-design `templates/change-plan.md`, 본 Story template §3 = 관련 ADR 인용만) 은 deputy 영역별로 구조화한다:
> - **code 축 (CodeArchitect — Sonnet single-mandate advocacy)**: layered / hexagonal / clean / DDD bounded context / module boundary / dependency direction
> - **data 축 (DataArchitect — Opus)**: entity / aggregate / VO / DB schema / event schema / DTO / API contract data / persistence model / 데이터 흐름 (+ §11 migration)
>
> 설계 서사 본문은 §7 (ArchitectAgent (chief author) → ArchitectPLAgent 검수) 미러링 + Change Plan §3 에 작성 — 본 Story template §3 은 관련 ADR 인용 전용 (selective additive guidance, consumer 기존 Story 작성 흐름 무영향).
...
- §1 목적 / §3 도입할 설계 / §4 API 계약 / §7 보안 설계 요약 / §9 분기 선택 요약 미러링 (5-10줄)
  ...
  - **§7.4 운영 리스크 미러링 시 primary/shell 분류 명시 (CFP-1026 S2 — ADR-014 Amendment 4 §결정 2)**: §7.4 = **primary 4-sub** (DR §7.4.1 / Clock §7.4.3 / Env §7.4.5 / Container §7.4.6 — 설계 시점 정책 값 결정 가능) vs **cross-ref shell 2-sub** (Cancel-on-disconnect §7.4.2 / Rate limit §7.4.4 — evidence-driven, policy 값 측정 후 결정). shell 2-sub 는 §8.6 IntegrationTest contract pointer 로 측정 대상 명세 (정책 값 공백이나 pointer 존재 의무 — 본 template §8.6 evidence-driven 3-axis pointer schema 참조).
...
##### §8.6 evidence-driven 3-axis pointer schema (CFP-1026 S2 — ADR-014 Amendment 4 §결정 2)

§7.4 **cross-ref shell 2-sub** (Cancel-on-disconnect §7.4.2 / Rate limit §7.4.4) 는 설계 시점에 policy 값을 결정할 수 없는 evidence-driven sub. 이 경우 §8.6 contract 에 아래 3-axis pointer 추가 (정책 값 미결정이나 측정 대상 명세는 완전 — 공란 금지):

```yaml
# §8.6 evidence-driven 3-axis pointer (cross-ref shell sub: §7.4.4 Rate / §7.4.2 Disconnect)
evidence_driven_pointers:
  - shell_sub: "§7.4.4 Rate limit"        # 또는 "§7.4.2 Cancel-on-disconnect"
    axis_1_phase1_measurement_target: "무엇을 측정하는가 — 예: 거래소 rate-limit 임계 (req/sec) / disconnect 후 cancel latency (ms)"
    axis_2_phase2_measurement: "실측 수행 — Phase 2 IntegrationTest 측정 (FIX 루프와 disjoint)"
    axis_3_phase1_followup_policy: "실측값으로 policy 결정 — Phase 1 follow-up PR (예: backoff threshold / cancel timeout)"
    integration_test_contract_ref: "본 §8.6 coverage_targets[] 중 측정 시나리오 이름"
```

> **disjointness 명문화 (ADR-014 Amendment 4 §결정 2)**: **§7.4.4/§7.4.2 policy 공백 ↔ FIX root-cause decision table = disjoint axis** (측정 대상이지 실패 원인 아님). audit gate = DesignReviewPL 이 §8.6 cross-ref pointer **존재만 mandatory check** — **policy 값 공백 자체는 PASS**, pointer 누락만 FIX. Phase 2 측정 (Axis 2) 은 측정 단계이지 실패 분류 아님 (FIX 루프 disjoint). 신규 inter-plugin contract registry entry 0 — 기존 `reconcile-protocol-v1` / `test_verdict` contract cross-ref only (sibling sync 미발생, ADR-010 §결정 2 무관).

**면제 조건**: 컴포넌트 경계 0개 Story, doc-only Story.
면제 시 `N/A — <근거 30자 이상>` 필수 (lint 강제). evidence-driven 3-axis pointer = §7.4 cross-ref shell sub 보유 Story 만 의무 (그 외 생략 가능 — selective additive schema).

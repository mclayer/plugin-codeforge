...
> **superpowers 비의존 (다음 MINOR 버전부터)**: codeforge 는 더 이상 `superpowers@claude-plugins-official` 에 의존하지 않습니다 ([ADR-122](../archive/adr/ADR-122-superpowers-dependency-removal.md)). brainstorming / writing-plans 는 codeforge 자체 skill (`codeforge:brainstorm` / `codeforge:writing-plans`) 로 자립합니다 — consumer breaking 0. consumer 는 여전히 superpowers 를 opt-in 설치할 수 있습니다 (overlay 확장).
...
### 1n. 배포 — consumer GitHub Actions/Environments 완전 위임 (CFP-2227 / [ADR-121](../archive/adr/ADR-121-deprecate-deploy-lanes.md))

> **배포 2 lane 물리 제거 완료 ([ADR-121](../archive/adr/ADR-121-deprecate-deploy-lanes.md) / CFP-2782, 2026-07-22)** — 구 배포 실행 + 배포 후 검토 2 lane (ADR-087 + ADR-088) 은 sunset(2026-07-13 KST) 경과 후 Wave 2 (Epic #2217 S5/S6) 로 물리 제거됐다. 배포는 consumer repo 의 GitHub Actions + Environments 로 완전 위임된다. 신규 consumer 는 아래 "배포 위임 모델"만 따른다.
...
### 1o. Operational Story — 운영 가능성 검증 및 시스템 불변식 선언 (CFP-2361 PS4 / [ADR-014 Amendment 7](../archive/adr/ADR-014-operational-risk-ssot-distribution.md) + [ADR-015 Amendment 1](../archive/adr/ADR-015-stateful-test-category.md) + [ADR-068 I-8](../archive/adr/ADR-068-boundary-completeness-invariants.md))

Operational Story = **운영 부하(throughput/scale) 또는 장기 불변식(accumulation/lifetime-class)** 을 다루는 Story. 매 impl PR 이 **도입 불변식을 명시 + soak 테스트로 검증**하는 declarative 설계 + behavioral 검증의 2층 구조. wrapper-self 는 declarative-only (운영 부하 0), consumer 는 full impl 책임.

#### 운영 Story 식별 (label: `operational:true`)

Story 가 다음 중 1+ 해당하면 `operational:true` label 부착:

- **component:runner / component:ingestion / component:persistence** 중 1+ (producer/consumer 역할 — [domain-knowledge consumer taxonomy](domain-knowledge/) 참조)
- **외부 stream 의존** (WebSocket / SSE / long-poll / persistent TCP / gRPC stream)
- **in-memory mutable 장기 보유** (cache / queue / buffer)
- **background worker / queue consumer / stream aggregator**
- **외부 API 호출 + rate-limit / 재시도 / timeout 정책** (예: trading signal 수집, 주문 submit, 이벤트 batch)

`operational:true` label 이 설계 stage 에서 change plan §7.4.7 + §8.5.1 심화 의무 trigger (ADR-014 Amendment 7).

#### Change Plan 에서의 운영 선언 (§7.4.7 + §8.5.1)

**§7.4.7 Operational throughput/scale** — 3요소 선언 의무 (ADR-014 Amendment 7 §7.4.7):

| 요소 | 정의 | 예시 | 설계 시점 hard | 실측 defer |
|---|---|---|---|---|
| ① **terminal downstream sink** | 최종 적재처 경로 (ingest 경계 proxy 아님, outcome 도달점) | object-store parts / DB committed rows / message offset | ✓ 확정 필수 | — |
| ② **monotone progress metric** | sink 의 단조 증가 지표 (동결 = outcome 정지 신호) | written parts count / committed row count / consumer offset | ✓ 확정 필수 | — |
| ③ **발현조건 임계** | accumulation/lifetime 리스크가 드러나는 최소 누적량 | `≥ 8 MiB/shard` flush / `≥ N rows/window` | ✓ 임계 존재 + 축 확정 | 숫자 = `[empirical-source: TBD]` 허용 |

**§8.5.1 Soak 구동 종점** (ADR-015 Amendment 1) — accumulation/lifetime-class Story 는 soak 테스트 구동 시간 **반드시 발현조건 기반 도출** (고정 단창 금지):

- **manifestation-derived**: 발현조건 임계 (§7.4.7 ③) 도달까지 구동 → 그 임계가 곧 soak 종점
- **duration floor fallback**: 임계 정량화 불가 시 최소 지속 시간 (예: `≥ 30분`) + "발현조건 미상" 리스크 명시

#### Impl PR 에서의 불변식 확장 (docs/system-invariants.md)

각 operational impl PR 이 **새 장기-수명 mutable 구조(accumulation/lifetime-class) 를 도입하면**, consumer repo 의 `docs/system-invariants.md` 에 **operating 불변식 row 를 append** 의무:

```markdown
# System Invariants — 운영 불변식 표

| component | invariant | measurement | threshold |
|---|---|---|---|
| order-batch-accumulator | buffer 미회수 누적 용량 | accumulated_memory_bytes | ≤ 512 MiB per shard |
| trade-signal-dedup-window | cache eviction rate | eviction_events_per_min | ≥ 1 (live) |
| historical-snapshot-loader | snapshot loading latency p95 | p95_latency_ms | ≤ 2000 ms (SLA) |
```

**docs/system-invariants.md 위치**: `<owner-repo>/docs/system-invariants.md` (doc-locations.yaml entry: `system_invariants`, CFP-2361 신설).

**작성 의무**: ArchitectAgent (Change Plan author) 이 설계 시점에 불변식 row 작성 spec 을 명시하면, DeveloperAgent / QADeveloperAgent 가 impl PR 에서 실제 row append. Story §11 data-migration 섹션에 "impl PR 이 system-invariants.md 갱신" 명시 (§11.7 N/A 선택 불가, 운영 Story 는 필수).

#### Soak Test 실행 (Story §8.5.1)

**적용 조건**: §8.5.0 applicability table 에서 long-running connection / cache / worker / restart-aware 중 1+ Y:
...
| 적용 조건 | Y/N | 근거 |
|---|---|---|
| Long-running connection (WebSocket/SSE/gRPC stream) | Y | order-stream fetcher WebSocket sustained |
| Stateful in-memory cache (>1 upd/sec sustained, >5min retention) | Y | dedup window 6시간 retention |
| Background worker / queue consumer | N | sync HTTP only |
| Process restart-aware | Y | graceful shutdown + state recovery |
```

→ 1+ Y 시, **§8.5.1 soak 테스트 계획 작성** 의무.

**테스트 fixture** (consumer 책임): `tests/infra/<story-key>/` 디렉터리에 soak harness 배치. QADeveloperAgent 가 impl PR 에서 시각:

```bash
# 예: tests/infra/MCT-123/soak-order-batch-accumulator.py
# 시뮬레이션: N시간 지속 + 매분 ordering batch 도착 + accumulation 임계 도달 점검

import asyncio

async def soak_order_batch_accumulator():
    """Manifestation-derived: § ≥ 8 MiB/shard 누적까지 구동"""
    accumulated_bytes = 0
    TARGET_BYTES = 8 * 1024 * 1024  # 8 MiB from §7.4.7

    async with OrderStreamConnector() as stream:
        async for batch in stream:
            accumulated_bytes += sys.getsizeof(batch)
            # Assertion: memory usage ≤ 512 MiB (system-invariants.md threshold)
            assert accumulated_bytes <= 512 * 1024 * 1024
            if accumulated_bytes >= TARGET_BYTES:
                logger.info(f"Manifestation reached: {accumulated_bytes} bytes")
                break
```

**검증 채널**: ADR-015 Amendment 1 + Change Plan §8.5.1 + `docs/system-invariants.md` standing surface.
...
**canary promotion criteria** (구 warning-tier 자동 lint = ADR-121 / CFP-2782 배포 canary machinery 물리 제거로 teardown): 구 배포 canary promotion lint workflow(PR-open + workflow_dispatch 2-trigger, ADR-072 §결정 5 production-cutover-evidence.yml byte-pattern 답습)가 PR 의 4-tuple measurement source + family_7_atomic_canary_pin three_way_match + enum closed-set invariant 를 자동 verify 했다. 해당 lint workflow + 조건부 skip bypass label 은 CFP-2782 로 삭제됨. **본 consumer channel(ADR-076 §결정 9 Stable/Beta/Canary) promotion criteria 개념 자체의 disposition = 구현리뷰 escalate 대상** — ADR-076 은 본 removal Story 의 ADR 판단 집합 밖이라 "개념 존치(enforcement 소실만)" vs "consumer channel canary 전면 teardown" 이 미확정. 4-tuple evidence quad 측정 개념은 consumer Story 책임으로 잔존(ProductionEvidenceDeputy = codeforge-design CONDITIONAL deputy, ADR-072 relocate).
...
In-lane brainstorming (DomainAgent / RequirementsPL 가 lane 내부에서 호출) 과는 다른 단계입니다.
...
```bash
# 단계 1: Plugin install (7 critical = wrapper + 6 lane)
/plugins install codeforge@<marketplace>
...
| (b) | plugin 7 critical (wrapper + 6 lane) 미설치 | `~/.claude/plugins/installed_plugins.json` parse |
...
integration_test:
  ...
  sink_probes:                              # presence 필수 (부재 = fail-closed)
    - name: committed_ticks
      ...
      threshold: 1000                       # manifestation-derived 조기 PASS (선택) — ≥1000 ticks 시 조기 종료
      duration_floor_seconds: 1800          # soak 창 max horizon / deadline ceiling (항상 필수, > 0)
...
- **soak duration (floor = 필수 horizon, threshold = 선택 조기 PASS — 상호배타 아님)**:
  - **`duration_floor_seconds`** (항상 필수, > 0): soak 창 max horizon / deadline ceiling. soak-runner 가 이 값으로 deadline(=START+floor)을 세워 timeout·무한 CONTINUE hang 을 방지한다(ADR-139 축2 정합). 미설정 = 무한 대기/CONTINUE = 게이트가 겨냥한 deadlock hang 유발 → 반드시 지정.
  - **`threshold`** (선택, manifestation-derived 조기 PASS): 실패-class 가 드러나는 최소 누적량(예: `≥1000 ticks` / `≥ 8 MiB flush`)에 floor 만료 **전** 도달하면 조기 PASS. [§1o Operational Story](#1o-operational-story--운영-가능성-검증-및-시스템-불변식-선언-cfp-2361-ps4--adr-014-amendment-7--adr-015-amendment-1--adr-068-i-8) §7.4.7 ③ 발현조건 임계와 동일. `threshold: null` = floor 만료 시점 net 순증(freeze) 판정. "blanket 고정 창 금지"(§결정7)는 floor 를 발현조건에 맞춰 도출하라는 의미지 floor 생략이 아니다.
...
Consumer 프로젝트에서 요구사항을 GitHub Issue Form으로 입력하면 플러그인이 0 core (wrapper-only) + distributed agent (6 lane plugin) + `role: dev` 동적 roster · 8 레인 구조로 자율 실행:

```
요구사항 → 설계 → 설계 리뷰 → 구현 → 구현 리뷰 → 통합 테스트 → 보안 테스트
```
...
**Skill 호출** (`codeforge:*` 등) = Inline (file write 아님 — meta wrapper). Skill 내부 individual tool call (Read / Edit / Write / mcp__github__\* / Agent / Bash) level 에서 spawn 분류 발동.

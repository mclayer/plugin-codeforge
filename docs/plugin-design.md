...
- **Generic SW 개발 오케스트레이션 플러그인** — 0 core 에이전트 (wrapper-only) + 6 lane plugin (codeforge-{requirements,design,develop,test,pmo,review}) 의 23 agent 가 자기 owner path 분산 보유 + `role: dev` 동적 roster · 7 레인 · FIX 루프 구조를 consumer 프로젝트 전반에 재사용
...
- 추상 구조·관계 (0 core + 6 lane plugin org chart — 각 plugin 의 agent listing 은 해당 plugin repo 참조, 7 레인, PL/sub 계층, `role: dev` 동적 roster 디스커버리)
...
- `agents/*.md` — ζ arc (CFP-31~CFP-40) 완료로 **0개** (wrapper-only). 역사: v0.9 review 워커 6 → 2 통합, v0.11-v0.14 설계 deputy 5인 구조 완성, v0.15 owner doc direct write 이관 (CFP-26), v0.17 review 추출 → codeforge-review (CFP-29), v2.0 requirements 추출 → codeforge-requirements (CFP-37), v3.0 test 추출 → codeforge-test (CFP-38), v4.0 develop 추출 → codeforge-develop (CFP-39), v5.0 design 추출 → codeforge-design + DocsAgent final delete (CFP-40). 현재 agent md 는 각 6 lane plugin repo 가 SSOT
...
## 6. 분류 가이드 (0 core (wrapper-only) + 6 lane plugin agent + preset)

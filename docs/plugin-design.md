...
- **Generic SW 개발 오케스트레이션 플러그인** — 0 core 에이전트 (wrapper-only) + 8 lane plugin (codeforge-{requirements,design,review,develop,test,deploy,deploy-review,pmo}) 의 agent 가 자기 owner path 분산 보유 + `role: dev` 동적 roster · 10 레인 · FIX 루프 구조를 consumer 프로젝트 전반에 재사용
...
- 추상 구조·관계 (0 core + 8 lane plugin org chart — 각 plugin 의 agent listing 은 해당 plugin repo 참조, 10 레인, PL/sub 계층, `role: dev` 동적 roster 디스커버리)
...
- `CLAUDE.md` — core 오케스트레이션 규칙 (10 레인 구조)

...
- **Generic SW 개발 오케스트레이션 플러그인** — 24 core 에이전트 + `role: dev` 동적 roster · 7 레인 · FIX 루프 구조를 consumer 프로젝트 전반에 재사용
...
- 추상 구조·관계 (24 core 에이전트 org chart, 7 레인, PL/sub 계층, `role: dev` 동적 roster 디스커버리)
...
## 6. 분류 가이드 (24 core 에이전트 + preset)

현재 버전에서 각 에이전트의 core 비중 · overlay 예상 내용:

### Group A (Overlay 거의 없음 · core 100%)

**ArchitectPLAgent** (설계 lane PL · 5-deputy 오케스트레이터) · **ArchitectAgent** (chief author · Change Plan 집필) · CodebaseMapperAgent · RefactorAgent · **SecurityArchitectAgent** (threat model · trust boundary 설계) · **TestContractArchitectAgent** (§8 Test Contract QA perspective contributor) · **DataMigrationArchitectAgent** (데이터 무결성 advocate · §11 author input — CFP-21) · DesignReviewPLAgent · CodeReviewPLAgent · SecurityTestPLAgent · ClaudeReviewAgent · CodexReviewAgent · DeveloperPLAgent · RequirementsPLAgent · RequirementsAnalystAgent · ResearcherAgent (워커 통합 후 lane-agnostic 2종 — [ADR-001](adr/ADR-001-review-agent-unification.md))

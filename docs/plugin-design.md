...
- **Generic SW 개발 오케스트레이션 플러그인** — 19 core 에이전트 + codeforge-review plugin 5 agent (CFP-29 Phase 1 추출) + `role: dev` 동적 roster · 7 레인 · FIX 루프 구조를 consumer 프로젝트 전반에 재사용
...
- 추상 구조·관계 (19 core + codeforge-review 5 agent org chart, 7 레인, PL/sub 계층, `role: dev` 동적 roster 디스커버리)
...
- `agents/*.md` — 19 core agent md (process 15 + generic Dev 3: DeveloperAgent·DataEngineerAgent·InfraEngineerAgent + QADev). v0.9에서 review 워커 6 → 2 통합 ([ADR-001](adr/ADR-001-review-agent-unification.md)). v0.11에서 ArchitectPLAgent·SecurityArchitectAgent 추가 (설계 lane 4-deputy 구조 도입 — CFP-17). v0.12에서 TestContractArchitectAgent 추가 (설계 lane 5-deputy 구조 — CFP-18). v0.14에서 DataMigrationArchitectAgent 추가 (설계 lane 5-deputy 구조 — CFP-21). v0.15에서 4 single-owner doc(change-plans/adr/domain-knowledge/retros)을 owner agent direct write로 이관 (CFP-26 Phase 0a — DocsAgent scope 축소). v0.16에서 owner doc 템플릿 2건(domain-knowledge / retro) 신설 + lint 2건(doc-frontmatter / doc-section-schema, warning 모드) + redistribution lint CI integration (CFP-27 Phase 0b — Non-BREAKING). **v0.17에서 5 review agent + base + 3 checklist을 별도 plugin [`codeforge-review`](https://github.com/mclayer/plugin-codeforge-review)로 추출 (CFP-29 Phase 1 — BREAKING) + Inter-plugin Contract `review_verdict v1` 동결 ([ADR-008](adr/ADR-008-inter-plugin-contract-versioning.md))**
...
- `CLAUDE.md` — core 오케스트레이션 규칙 (7 레인 구조)
...
## 6. 분류 가이드 (19 core 에이전트 + codeforge-review plugin 5 agent + preset)

현재 버전에서 각 에이전트의 core 비중 · overlay 예상 내용:

### Group A (Overlay 거의 없음 · core 100%)

**ArchitectPLAgent** (설계 lane PL · 5-deputy 오케스트레이터) · **ArchitectAgent** (chief author · Change Plan 집필) · CodebaseMapperAgent · RefactorAgent · **SecurityArchitectAgent** (threat model · trust boundary 설계) · **TestContractArchitectAgent** (§8 Test Contract QA perspective contributor) · **DataMigrationArchitectAgent** (데이터 무결성 advocate · §11 author input — CFP-21) · DeveloperPLAgent · RequirementsPLAgent · RequirementsAnalystAgent · ResearcherAgent

**[codeforge-review plugin] (별도 — CFP-29 Phase 1 추출)**: DesignReviewPLAgent · CodeReviewPLAgent · SecurityTestPLAgent · ClaudeReviewAgent · CodexReviewAgent (워커 통합 후 lane-agnostic 2종 — [ADR-001](adr/ADR-001-review-agent-unification.md))

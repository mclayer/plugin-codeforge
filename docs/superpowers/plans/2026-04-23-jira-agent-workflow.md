...
## Task 1: 사전 준비 — GitHub for Jira 앱 (사용자 수동)

**Files:** 외부 시스템만 변경

**Note (2026-04-23 확인)**: Jira custom status 추가는 불가한 것으로 확인 → **labels-only 방식으로 전환**. 스펙 §3.4, §8.1 참조. 관리자 작업은 GitHub for Jira 앱 설치 1건으로 축소.
...
- [ ] **Step 1.3: 기본 transition ID 확인** (참고용, labels-only이므로 수집 불필요)

기본 3개 status의 transition ID는 `해야 할 일=11`, `진행 중=21`, `완료=31`. 이미 migration 작업 때 확인됨. Story 생성·진행·완료 시 이 3개만 사용.

**게이트**: Step 1.2 완료 전까지 Task 2 이후 진행 금지.
...
Jira 기본 3-state 유지(`해야 할 일 → 진행 중 → 완료`) + phase label로 단계 표현. 한 Story는 현재 단계 label 1개만 보유(교체 방식).

```
[생성] status=해야 할 일 → 즉시 phase:요건 label + 진행 중 전이
   ↓
[phase:요건 → phase:설계] (Architect Change Plan 확정 시 label 교체)
   ↓
[phase:설계 → phase:구현] (구현 병렬 스폰 시)
   ↓
[phase:구현 → phase:리뷰-step1] (Step1 진입)
   ↓
[phase:리뷰-step1 → phase:테스트-step2] (Step1 PASS)
   ↓
[PR merged] → status=완료 (GitHub for Jira 자동 전이), 마지막 label 유지(감사용)
```

### Transition ID (기본 3개만 사용)
- `해야 할 일` → 11
- `진행 중` → 21
- `완료` → 31

### FIX 루프
- **Step1 P0/P1** 또는 **Step2 FAIL** 시: phase label 되돌림 `phase:리뷰-step1|phase:테스트-step2 → phase:구현` + `fix:step1-retry` 또는 `fix:step2-retry` label 추가 + 코멘트 `[FIX #N] <Agent>: <원인>`
...
### 코멘트 권한
- **직접 기록(9)**: PMAgent, PMOAgent, ArchitectAgent, DeveloperPLAgent, EngineerPLAgent, ReviewPLAgent, RefactorAgent, ClaudeReviewAgent, CodexReviewAgent
...
Run:
```bash
grep -n "Jira 워크플로우" /Users/1111971/workspace/mctrader/CLAUDE.md
```
Expected: "## Jira 워크플로우 (MCTRADER 프로젝트)" 섹션 헤더 발견.
...
- Modify: `.claude/agents/ReviewPLAgent.md`
- Modify: `.claude/agents/RefactorAgent.md`
- Modify: `.claude/agents/ClaudeReviewAgent.md`
- Modify: `.claude/agents/CodexReviewAgent.md`
...
Run:
```bash
for a in PMAgent PMOAgent ArchitectAgent DeveloperPLAgent EngineerPLAgent ReviewPLAgent RefactorAgent ClaudeReviewAgent CodexReviewAgent; do
...
- Modify: `.claude/agents/TestAgent.md`
...
Run:
```bash
for a in RequirementsAnalystAgent ResearcherAgent QADeveloperAgent FrontendDeveloperAgent BackendDeveloperAgent DataEngineerAgent ServerEngineerAgent TestAgent DocsAgent; do
...
mcp__atlassian__transitionJiraIssue(
  cloudId="9dfd8e80-9cc3-4f30-adca-905478a2d628",
  issueIdOrKey="<Epic 키>",
  transition={"id": "21"}   # 진행 중
)
```
Expected: 성공.

- [ ] **Step 6.3: Story 생성 + Epic link + phase:요건 label + 진행 중 전이**
...
mcp__atlassian__createJiraIssue(
  ...
  additional_fields={
    ...
    "customfield_10014": "<Epic 키>"  # Epic link 필드 (프로젝트별 다를 수 있음, 실패 시 생략)
  },
  transition={"id": "21"}   # 진행 중
)
```
Expected: `MCTRADER-<M>` 반환. `getJiraIssue`로 `status.name == "진행 중"` + labels에 `phase:요건` 확인.

참고: Epic link 필드는 team-managed 환경에서 `customfield_10014` 또는 다른 ID일 수 있음. 실패 시 해당 필드 제거하고 Story만 생성 — smoke test 목적상 link는 선택.
...
- 9개 결정/PL 에이전트: 직접 코멘트 권한 + Jira 코멘트 규약 섹션
  (PM/PMO/Architect/DevPL/EngPL/ReviewPL/Refactor/Claude+CodexReview)
- 9개 오케스트레이터 경유 에이전트: TL;DR 출력 규약 섹션
  (Analyst/Researcher/QADev/Frontend+Backend/DataEng+ServerEng/TestAgent/Docs)

관리자 1회 설정(별도): GitHub for Jira 앱 설치 완료 (custom status는 현 Jira 구독 제약으로 labels-only로 전환)
...
- [ ] CLAUDE.md에 "## Jira 워크플로우 (MCTRADER 프로젝트)" 섹션 존재 (labels-only 방식 기술)

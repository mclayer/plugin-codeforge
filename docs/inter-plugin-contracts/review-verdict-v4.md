  ...
  findings:                          # v3 그대로 (배열, severity/category/file/evidence/suggestion) + anchor_id NEW + type NEW (v4.3)
    - severity: P0 | P1 | P2
      ...
      type: <finding_type_enum>      # NEW v4.3 (optional) — finding 유형 literal
                                     ...
                                     # "audit-gate-pointer-missing": ADR-068 Amendment 3 §결정 1 I-6 위반 — §8.6 audit gate finding 4-form pointer scope existence verify 실패 (v4.7). boundary-completeness 와 disjoint axis (별 verdict field boolean audit_gate_pointer_self_check_passed)

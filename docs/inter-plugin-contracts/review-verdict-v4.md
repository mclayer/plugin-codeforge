...
related_adrs:
  ...
  - ADR-041  # Phase-scoped sequential team SSOT (본 v4 carrier)
authors:
  - CFP-137 (2026-05-09) — review-verdict v3 → v4 MAJOR bump (Sonnet decider 영역 정식 제거 + worker_dialog_rounds 추가)
---

# review_verdict v4 — Inter-plugin Contract (CFP-137 / ADR-041)
...
신규 field `worker_dialog_rounds` 추가 — Adversarial debate (5 권장 패턴 — ADR-041 §결정 5) measurable verification.
...
- ADR-041 carrier: `docs/adr/ADR-041-phase-scoped-sequential-team.md`
...
| `worker_dialog_rounds` | (없음) | **NEW** — Adversarial debate SendMessage round count (ADR-041 §결정 5) |
...
review_verdict:
  ...
  worker_dialog_rounds: <int>        # NEW — Adversarial debate SendMessage round count
                                     ...
                                     # >= 2 권장 (ADR-041 §결정 5 Adversarial measurable verification)
...
본 v4 는 wrapper Phase 1 PR (CFP-137 Wave 2) merge 시점 즉시 cutover. consumer scope 0건 (mctrader debut audit 까지) 으로 backward compat 면제. ADR-041 §결정 4 정합.
...
## 10. Adversarial debate measurable verification (ADR-041 §결정 5)

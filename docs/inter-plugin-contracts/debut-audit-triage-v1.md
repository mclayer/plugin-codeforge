...
| 2 | `category:agent-gap` | ArchitectPL | 신규 agent ADR + lane plugin CFP / 책임 매트릭스 row 추가 / phase 분리·통합 ADR. **R1-R4 measurable signal ([ADR-021](../../archive/adr/ADR-021-phase-gap-measurable-signal.md))** detection trigger 시 발화 |
...
`category:agent-gap` 발견은 [ADR-021](../../archive/adr/ADR-021-phase-gap-measurable-signal.md) R1-R4 measurable signal trigger 시 발화 — `scripts/check-debut-audit-signals.sh` 자동 detection (Phase 2).

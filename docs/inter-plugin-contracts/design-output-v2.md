...
## 9. Wrapper sibling marker (CFP-46 PR-F)

**상위 SSOT 위치**: 본 sibling 은 [`mclayer/plugin-codeforge-design/docs/inter-plugin-contracts/design-output-v2.md`](https://github.com/mclayer/plugin-codeforge-design/blob/main/docs/inter-plugin-contracts/design-output-v2.md) canonical 의 verbatim mirror.

- canonical 변경 시 본 sibling sync PR 후속 의무 (ADR-010)
- 본 sibling 은 Read-only 참조용. 변경은 canonical 에서만 수행
- 본 sync PR-F (CFP-46) 가 v2 sibling 신설 — `**상위 SSOT 위치**:` marker 는 CFP-42 inter-plugin-contracts lint 의 sibling identity 검사용

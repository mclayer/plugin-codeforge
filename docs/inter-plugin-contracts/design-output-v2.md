...
- **[ADR-015 — Stateful test category](https://github.com/mclayer/plugin-codeforge/blob/main/docs/adr/ADR-015-stateful-test-category.md)** (CFP-47) — TestContractArchitectAgent mandate 에 §8.5 추가. 신규 deputy 미도입. design-output additive minor in-place bump.
...
3. **6 deputy 통합 표 (§4) 의 OpRiskArch row 의 Primary 책임 컬럼에 §7.4.6 추가** — restart policy / volume DR / health check / network mode 4 항목.

#### Carrier ADR (v2.2)

- **[ADR-033 — Docker-first Infra Engineering](https://github.com/mclayer/plugin-codeforge/blob/main/docs/adr/ADR-033-docker-first-infra-engineering.md)** (CFP-128) — InfraEngineerAgent mandate Docker-first 재정의 + ADR-014 §7.4 OpRiskArch mandate 4 항목 amend (container restart policy / volume DR / health check / network mode). 신규 deputy 미도입. design-output additive minor in-place bump.

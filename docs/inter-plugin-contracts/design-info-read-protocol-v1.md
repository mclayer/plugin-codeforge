...
### 2.1 1차 소스 선언 + 소비자 그룹 분류 (G1-G5 — ADR-166 §결정 2)
...
### 2.2 mandatory 선행 read 의 형태 (부분집합 anchor + 2-doc ceiling + 4 H2 fallback — ADR-166 §결정 3)
...
### 3.1 reader-side 배선 (in-band 주입 + 계약 등록 — dead-letter 방지 — ADR-166 §결정 4)
...
### 3.2 read 추적 marker + advisory ceiling (정직 천장 — ADR-166 §결정 5)
...
### 3.3 rollout 순서 + scope (freshness 선결 조건 — ADR-166 §결정 6)
...
### 3.4 소비자 in-band 주입 요지 (배선 표면 참조용)
...
2. **anchor**: floor = arc42 §5 Building Block View + 그룹별 권장 anchor (§2.2 표). 5-anchor 부재 doc = 4 H2 closed-enum(모듈/경계/인터페이스 계약/데이터 흐름) fallback. 전문 pre-embed 금지.
...
ADR-008 §결정 2 정합 — additive/optional 변경(소비자 그룹 additive 편입 / 그룹별 권장 anchor 추가 / marker optional 필드 추가) = MINOR bump, 기존 의무 의미 변경 = MAJOR bump. v1.0 = 초판 (ADR-166 §결정 2-6 verbatim 미러). 약화 방향(G2 축소 / mandatory 해제 / marker 폐지) = ADR-058 §결정 5 sunset_justification 의무.

### 4.2 Sibling sync 정책

kind:registry — wrapper 단일 원본 (ADR-118 D5), sibling repo verbatim mirror 없음 (sibling sync 면제 — ADR-010 §결정 2 + ADR-008 §결정 2). completeness SSOT = `MANIFEST.yaml` registries row. plugin.json version bump 동반해도 본 registry 자체는 mirrored field(name/version/description/author) 무관 = marketplace_sync 대상 아님.

...
authors:
  ...
  - ArchitectAgent (CFP-2543 — v1.4 MINOR bump, 설계 리팩터링 consumer + blanket_designrefactor dispatch enum. ADR-059 Amendment 4 동반)
version_history:
  ...
  - { version: "1.4", date: 2026-07-01, carrier: CFP-2543, change: "MINOR bump — dispatch_mode enum 6번째 value blanket_designrefactor 추가 (설계 lane RefactorAgent 구조 리팩터 활동 per-Story 무조건 발동, divergence 감지 없이, cross_module_signal block 불요) + role_assignment 재사용({codex:proponent, claude:opponent}, 신규 필드 0) + divergence_type structural per-lane keying 3-way (DesignLane 설계산출물 / blanket_refactor 실코드중복 / blanket_designrefactor 설계 리팩터 구조 축) + trigger.lane design-lane 재사용(신규 lane enum 0). 기존 enum 값·필드 의미 변경 0 (additive strengthening, ADR-008 §결정 2). ADR-059 Amendment 4 동반. CFP-2543 (정책 carrier = ADR-138)." }
related_adrs:
  ...
  - ADR-138  # blanket_designrefactor 정책 carrier (설계 리팩터링 debate 격상)
...
producers:
  ...
  - "codeforge-design/ArchitectAgent  # blanket_designrefactor 설계 리팩터링 debate verdict judge (CFP-2543 / ADR-138) — 3분기 verdict(now/defer/drop) 판정 + Change Plan §3 착지 author. dispatch 주체 아님: 실 debate dispatch = Orchestrator top-level inline(ADR-039 §결정18 merge-time Codex adversarial 전용, 재귀 가드). ArchitectAgent self-spawn 불가 → producer=verdict judge 의미로 배선."
consumers:
  ...
  - "codeforge-design/RefactorAgent  # refactor debate transcript 수신 (Epic CFP-2533 Story C 실배선, ADR-137 §결정5) — 엔진 소비. (d) Reusability 측정 축 종착 transcript consumer."
  - "codeforge-design/RefactorAgent  # 설계 리팩터링 debate transcript 수신 (CFP-2543 / ADR-138) — 구조 3축 advocacy input provider(verdict 주체 아님). 엔진 소비."
---
...
| **blanket_designrefactor** | ADR-059 Amendment 4 (CFP-2543) — 설계 lane RefactorAgent 구조 리팩터 활동 전면 적용. trigger = 설계 리팩터링 구조 축(a/b/c + repo-분해 escalation) advocacy = **per-Story 무조건 발동** (divergence 감지 없이, cross_module_signal block 불요). role_assignment={codex:proponent, claude:opponent}. verdict judge = ArchitectAgent chief. anchor = `<설계 요소>::<구조 축>` per-Story scope (cross-Epic drop-ledger 불요 — ADR-137 전용). 정책 carrier = ADR-138. 우선순위 = blanket 군 (§결정 7 SSOT). |
...
| **refactor lane (구현-리팩터링)** | ADR-059 Amendment 3 (CFP-2534) — 실제 머지된 코드를 대상으로 중복·재사용 리팩터링 지점을 Codex(찬성)↔Claude(반대) 적대 토론으로 도출하는 소비 lane. divergence_type = `structural` 재사용 (per-lane keying 으로 의미 확정). producer/consumer 실배선 = Story C (실배선 완료, ADR-137). |
...
trigger:
  ...
  dispatch_mode: blanket_cross_module_designlane | blanket_refactor | blanket_designrefactor | auto_on_divergence | mechanical_fast_path_inline | user_request_only  # REQUIRED (v1.4 — blanket_designrefactor: CFP-2543 / ADR-059 Amendment 4; 설계 리팩터링 per-Story 무조건 발동, cadence per-Story; blanket_refactor: CFP-2534 / ADR-059 Amendment 3)
...
- Refactor (CFP-2534 blanket): `structural` **재사용** — 실제 머지 코드의 중복·재사용 divergence, `<file>:<line>` anchor. divergence detection 없이 자동 발동 (`blanket_refactor`). per-lane keying 으로 DesignLane structural 과 의미 구분 (DesignLane = 설계 산출물 structural / Refactor = 실코드 중복·재사용 structural). role_assignment = {codex: proponent, claude: opponent}. producer/consumer 실배선 = Story C (실배선 완료, ADR-137).
- DesignRefactor (CFP-2543 blanket): `structural` **구조 축** — 설계 리팩터 구조 축(a/b/c + repo-분해 escalation) divergence, `<설계 요소>::<구조 축>` anchor (실코드 `<file>:<line>` 아님 — 설계-time line 없음). divergence detection 없이 per-Story 무조건 발동 (`blanket_designrefactor`). per-lane keying 으로 DesignLane structural(설계 산출물) / Refactor structural(실코드 중복) 과 의미 구분 (DesignRefactor = 설계 리팩터 구조 축). role_assignment = {codex: proponent, claude: opponent}. verdict judge = ArchitectAgent chief. 정책 carrier = ADR-138.
...
- [ADR-138](../../archive/adr/ADR-138-design-refactoring-debate-elevation.md) — blanket_designrefactor 정책 carrier (설계 리팩터링 debate 격상)

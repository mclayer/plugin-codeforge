...
- date: 2026-05-17
  ...
  # CFP-2490 신설 — ADR-135 §결정 2/5 (Epic CFP-2481 E2 — own-branch force-push pre-flight HEAD-pin 가드 L2 CI 사후 detect bypass channel)
  ...
  - name: hotfix-bypass:force-push-base-advance
    category: hotfix-bypass
    color: "fef2c0"
    description: "hotfix-bypass: force-push-base-advance warning-tier mechanical lint 조건부 skip + audit comment 자동 발의 (CFP-2490 / ADR-135 §결정 2/5 — Epic CFP-2481 E2, own-branch force-push pre-flight HEAD-pin 가드 L2 CI 사후 detect bypass channel). own-branch force-push 가 stale base 위에서 sibling commit 을 ancestry corruption(덮어쓰기)하는 사고(#1027, CFP-967/991 pattern_count 2)를 PR head 가 base(origin/main) advance 미반영(base-advance/divergence)한 채 열린 상태로 사후 표면화 (warning). 검사 명제 = git ancestry 실측(PR-body-proxy 아님, 위조 곤란): head..base BEHIND count > 0 (base-advance) 또는 base 가 head 의 ancestor 아님 (divergence) → exit 1 WARN. 차단 불가 (force-push 발생 후 detect) — opt-in L1 local pre-push hook(templates/.claude/hooks/pre-push.sh.sample, PRE_PUSH_BASE_CHECK=1)만 진짜 pre-flight 차단 (ADR-135 §결정 2 한계 정직 기술, ADR-119 검사연극 금지). graceful-degradation: SHA 부재(shallow checkout/fetch 실패) → exit 0 advisory degrade / git·python3 미설치 → exit 2 SETUP fail-closed. 본 Story 산출물: scripts/lib/check_force_push_base_advance.py (Python SSOT, git output parse only + SHA-shape anchored simple regex = ReDoS-safe, subprocess list-arg shell=False) + scripts/check-force-push-base-advance.sh (bash thin wrapper, ADR-061 convention) + templates/github-workflows/force-push-base-advance-check.yml + .github/workflows/force-push-base-advance-check.yml (byte-identical mirror, ADR-005 — Change Plan §5 #4 양쪽 명시) + tests/scripts/test_check-force-push-base-advance.sh (self-contained discriminating test, 7 fixture base-advance/divergence/clean/force-vs-non-force/graceful/SETUP + always-PASS mutation 생존 0). injection-safe (pull_request NOT pull_request_target / permissions contents:read / base·head SHA env-injection only = run inline 보간 0, fork-PR injection 차단). warning tier: exit 0=PASS(or graceful skip)/1=WARN/2=SETUP (continue-on-error: true 정합, PR 차단 금지). ADR-060 §결정 5 default warning mode 정합 — blocking 승격은 evidence-gated 별 carrier (ADR-135 §결정 5, recurrence count 2 < threshold 3). de-bloat 재발 방지 = live workflow + detect_command path 실존 + discriminating self-test 3-종 동반 (ADR-135 §결정 6 / CFP-2381 reconcile gate, spawn-prompt-head-pin theater 제거 전례 답습 회피). audit-trailed exception channel (ADR-024 Amendment 6/8/16 §결정 6.A lint chain inherit — bypass 부착 시 `### Bypass reason` PR body 섹션 의무). 101번째 hotfix-bypass:* family member (ADR-108 §결정 2(B)/§결정 3 forcing function parity mandate — description text N번째 = append-time `grep -c '^  - name: hotfix-bypass:' docs/inter-plugin-contracts/label-registry-v2.md` post-append raw active count 정합. canonical = raw-grep post-append (NOT monotonic-ratchet). firsthand 실측: post-append = 101)."
    single_active: false
    attach_owner_plugin: "Orchestrator (CFP-2490 force-push-base-advance warning-tier bypass — own-branch force-push base-advance/divergence 사후 detect lint 임시 skip 시 부착, audit-trailed exception channel 의무) / ArchitectAgent / DeveloperPLAgent"

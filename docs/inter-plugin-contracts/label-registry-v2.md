...
date: 2026-05-24  # CFP-1495 v2.65: Sub-B S2.5 of Mega-Epic CFP-1415 (Confluence-as-derived-mirror governance standardization, Sub-B Sub-Epic CFP-1490 마지막 carrier 5/5) — confluence-drift-detection lint bypass channel 신설 (1 신규 entry: hotfix-bypass:confluence-drift-detection 89번째 hotfix-bypass:* family member, CFP-1495 / ADR-103 §결정 1 영역 carrier — Confluence-mirror drift scheduled detection MCP-direct staleness check mechanical lint warning-tier bypass channel). docs/confluence-ia-tree.yaml 49 page staleness heuristic 2-axis (timestamp delta + title mismatch) detection, mark engine path #1320 활성 영역 = exempt (3-anchor stamp 부착). MCP-direct deviation declare (ADR-103 §결정 1 / CFP-1146 W5-S17 precedent): ATLASSIAN_API_TOKEN secret 부재 시 silent skip per page. ADR-082 §결정 6 retain pattern 답습 (Wave 1 declare + wire 동시 — sister #1498 IA tree S2.1 precedent). marketplace-drift-detection.yml byte-pattern precedent 답습 (CFP-627 / ADR-063 Amendment 3 §결정 13 — 24h cron + workflow_dispatch dual trigger, continue-on-error: true warning tier). Wave 1 mechanical wire: scripts/lib/check_confluence_drift.py (Python SSOT per ADR-061) + scripts/check-confluence-drift.sh (bash thin wrapper) + templates/github-workflows/confluence-drift-detection.yml + .github/workflows/confluence-drift-detection.yml (self-app byte-identical per S1.3 #1457 lesson) + tests/scripts/cfp-1495/cfp-1495-confluence-drift.bats (5+ TC RED→GREEN stash proof, ADR-082 §결정 11.A precedent) + evidence-checks-registry confluence-drift-detection entry (warning tier Active, owner_adr ADR-103). MINOR bump: 1 신규 entry, kind:registry sibling sync 면제 (ADR-010 §결정 2 + ADR-008 §결정 3 row append), plugin.json bump 0 = marketplace_sync_declared: false (script + workflow 신설 = governance behavior 변경이나 plugin.json mirrored field 무변경). MANIFEST.yaml row "2.64" → "2.65" ratchet 동반. ADR-108 §결정 3 forcing function parity mandate — description text "89번째" 명시 의무 (raw active concrete grep count post-append = 88 + 1 = 89 정합). Sub-B Sub-Epic CFP-1490 5/5 child Story 완료 마지막 carrier (sibling: #1491 IA tree + #1492 ADR-103 Amd N + #1493 ADR migration + #1494 change-plans migration). pattern_count >= 3 재발 시 follow-up CFP MUST promote to blocking tier.
date: 2026-05-24  # CFP-1500 v2.63: Wave 2-B of CFP-1389 (Sub-CFP B CFP-1436 mechanical wire) — mid-spawn-drift-detection lint bypass channel 신설 (1 신규 entry: hotfix-bypass:mid-spawn-drift-detection 87번째 hotfix-bypass:* family member, CFP-1500 / ADR-082 Amendment 16 §결정 1 layer 1 sub-scope (1-F) + ADR-073 Amendment 12 paired sibling — Mid-spawn rebase auto-detection protocol mechanical lint warning-tier bypass channel. CFP-1436 Wave 1 declarative stub workflow (PR #1475 merged 2026-05-24) → Wave 2-B 실 mechanical wire: scripts/lib/check_mid_spawn_drift_detection.py (Python SSOT per ADR-061, line-by-line parse CodeQL ReDoS guard CFP-1497 PR #1499 sentinel verbatim 답습) + scripts/check-mid-spawn-drift-detection.sh (bash thin wrapper) + templates/github-workflows/mid-spawn-drift-detection-check.yml (Wave 1 stub hydrate → live PR-time pull_request + workflow_dispatch dual trigger split per CFP-954 D2 consensus) + .github/workflows/mid-spawn-drift-detection-check.yml (self-app byte-identical) + tests/scripts/cfp-1500/cfp-1500-mid-spawn-drift.bats (9 TC RED→GREEN stash proof verified 12/12 GREEN, ADR-082 §결정 11.A precedent) + evidence-checks-registry mid-spawn-drift-detection entry (warning tier Active, owner_adr ADR-082 + ADR-073). META self-application 3rd applied case: 본 PR 의 ArchitectAgent spawn 자체가 [PRE-SPAWN-ORIGIN-MAIN-SHA: c4e306e319c14fde3d2a0302d6d3ec8424656527] block 사용 = CFP-1489 META precedent 답습 (Wave 2-A 1st applied case → CFP-1497 Wave 2-C 2nd applied case → 본 CFP-1500 Wave 2-B 3rd applied case). 3-layer mechanical defense forcing function 완결 (CFP-1389 parent Epic): preventive pre-spawn pin (CFP-1489) + preventive slot reservation (CFP-1497) + reactive mid-spawn drift (본 CFP-1500). MINOR bump: 1 신규 entry, kind:registry sibling sync 면제 (ADR-010 §결정 2 + ADR-008 §결정 3 row append), plugin.json bump 0 = marketplace_sync_declared: false (mirrored field 변경 0). MANIFEST.yaml row "2.62" → "2.63" ratchet 동반. ADR-108 §결정 3 forcing function parity mandate META self-app 6th applied case (CFP-1346 1st + CFP-1384 2nd + CFP-1429 84번째 3rd + CFP-1489 85번째 4th + CFP-1497 86번째 5th + 본 CFP-1500 87번째 6th = post-append raw active concrete grep count = 86 + 1 = 87 정합). pattern_count >= 3 재발 시 follow-up CFP MUST promote to blocking tier.

...
- date: 2026-05-17
  ...
  - name: hotfix-bypass:backlog-issue-label
    category: hotfix-bypass
    color: "fef2c0"
    description: "hotfix-bypass: backlog-issue-phase-label-forbid warning-tier mechanical lint 조건부 skip + audit comment 자동 발의 (CFP-1646 / ADR-024 Amendment 16 §결정 6.A.8 신설 carrier — Issue label timing discipline mechanical enforce bypass channel, evidence-checks-registry backlog-issue-phase-label-forbid entry 의 bypass channel). backlog 상태 식별 3-AND predicate (state:open + assignee:none + linked_pr:none) 만족 = backlog (lane 진입 X, `phase:*` label 부적합) → lint 발화 (warning tier). 한 조건이라도 false = 착수 가능성 신호 (lint 발화 부적합, false-positive 차단). behavioral directive normative 승격 carrier — CFP-1111 follow-up #1198/#1199 phase:설계 reflex incident (사용자 지적 '두개의 이슈 현재 작업 중 아닌가?') origin, memory `feedback_no_phase_label_on_backlog_issues.md` ephemeral → ADR-024 normative SSOT 승격 (CLAUDE.md 'Orchestrator 정책 적용 범위 + behavioral directive (normative)' 단락 첫 carrier — memory 만으로 갈음 금지 invariant). ADR-060 §결정 5 default warning mode 정합 (first introduction = warning tier). Wave 1 declarative (registry §3 entry append + workflow stub) / Wave 2 mechanical wire (workflow body actual lint + scripts/check-backlog-issue-label.sh thin-wrapper + scripts/lib/check_backlog_issue_label.py ADR-061 multi-line Python external file + bats RED→GREEN stash proof CFP-1334 + boundary fixture pair CFP-963 Codex TP#4) — Phase 2 PR scope. BYPASS_BACKLOG_ISSUE_LABEL=1 env unconditional skip. bypass audit-trailed exception channel (ADR-024 Amendment 6/8 §결정 6.A 5 lint chain inherit). sibling carrier — memory `feedback_brainstorming_why_first.md` (skill `codeforge:codeforge-brainstorm` body L63/66/100/140-145 4 anchor + 9 grep hits HIT verified → retire only) + `feedback_codex_collaboration_patterns.md` (ADR-052/070/081 codified → retire only). ADR-082 §결정 9 verify-before-write mandate 적용 — worktree base e1e2b751 (CFP-1632 HEAD = origin/main identical) `git -C <worktree_abs_path>` direct verify (filesystem Read tool stale main repo path 회피, FIX iter-1 self-fault recovery carrier). META self-application ADR-082 §결정 1-K 4-step numeric claim verify-before-write mandate 22nd applied case. **103번째 hotfix-bypass:* family member (ADR-108 forcing function parity mandate — description text N번째 명시 의무, raw active concrete grep count post-append = 102 + 1 = 103 정합 — CFP-1632 v2.77 102번째 retro-batch-adr-draft-pre-publish → CFP-1646 = 103번째 / v2.78 ratchet up 완료)**."
    single_active: false

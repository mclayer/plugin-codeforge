...
| 시각 | ISO8601 string | required | machine event 층(직렬화 값·미래 machine export) = UTC strict — Z suffix 필수, `2026-04-29T12:34:56Z` (CFP-295 / Issue #302 — +00:00 / bare datetime 불허). **§10 markdown 표 시각 칸(표시 표면) = display layer — ADR-079 §결정 2 초-KST `YYYY-MM-DDTHH:MM:SS+09:00` 정본** (ADR-079 §결정 5 V-3 dual-clock 재배정). 두 층 disjoint co-exist — §14 Lane Evidence dual-layer (ADR-079 §결정 9, story-page-structure §14 field #4/#5) 동형. (주: `fix-ledger-sync.yml` Action mirror 는 §7.1 ② 실측상 `시각` 을 소비하지도 자체 시각값을 emit 하지도 않으므로 machine carrier 아님 — 코멘트 시각 = GitHub `created_at` 플랫폼 메타데이터. 현 machine 층 물리 carrier = S6 참조.) |

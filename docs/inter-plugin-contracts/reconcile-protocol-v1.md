...
authors:
  ...
  - ArchitectPLAgent (CFP-898 Wave 4 sub-Epic Epic CFP-858 S1 — §4.3 (i) trigger 신설 발동 + §4.10 dependency_bundle_integrity_binding block 신설. ADR-076 Amendment 2 §결정 2 11번째 row append + §결정 6 fail-closed clause sub-section carrier. mctrader-data#81 14 failing checks evidence)
version_history:
...
### 4.10 Wave 4 sub-Epic #1 Story-1 multi-version channel pin declare layer (v1.7 — CFP-906 §4.3 (i) trigger 신설 발동, ADR-076 §결정 9 + ADR-016 Amendment 3 + ADR-063 Amendment 6 §결정 17 carrier)
...
```yaml
multi_version_channel_pin_binding:   # v1.7 신설, CFP-906 §4.3 (i) 발동 (Wave 4 sub-Epic #1 Story-1, Epic CFP-882)
  carrier_story: CFP-906
  carrier_adr: "ADR-076 §결정 9 + ADR-016 Amendment 3 + ADR-063 Amendment 6 §결정 17"
  status: schema_declared_phase1   # Phase 1 = schema binding declare / Phase 2 = runtime UpgradeAgent multi-channel dispatch (Wave 4 sub-Epic #1 Story-2 carrier)
  ...
  registry_channel_matrix:
    ...
    story_1_scope_declare_only: "본 Story-1 = declare layer SSOT only. 실 marketplace.json channels[] field 추가 = Wave 4 sub-Epic #1 Story-2 (runtime UpgradeAgent multi-channel dispatch) carrier 영역. marketplace_sync_declared: false (Story-1 = ADR/contract MINOR bump only, plugin.json bump 0 — declare-layer scope)."
  three_way_channel_invariant:
    ...
    runtime_carrier: "Wave 4 sub-Epic #1 Story-2 (별 CFP — runtime UpgradeAgent multi-channel dispatch + scripts/check-3way-version-parity.sh 의 channel 차원 확장)"
  ...
  channel_drift_detection:   # OpRiskArch §7.4 §1 Story-2 carrier (Story-1 declare-only placeholder)
    required: true
    runtime_carrier: "Wave 4 sub-Epic #1 Story-2 (별 CFP) — channel-drift-detection workflow (ADR-063 Amendment 3 §결정 13 marketplace-drift-detection.yml precedent 답습 — 24h cron + workflow_dispatch + Issue auto-create)"
    ...
    failure_mode_default: "warning-first (ADR-060 §결정 5 default + §4.8 version_handshake fallback orthogonality 답습)"
    story_1_scope: "declare placeholder only. evidence-checks-registry.yaml `channel-drift-detection` entry append = Wave 4 sub-Epic #1 Story-2 carrier (Story-1 영역 외)."
...
Phase 1 (CFP-906) merge 시 본 §4.10 binding block 활성 (schema declare — 3-tier channel taxonomy + family atomic channel invariant + 3-way channel invariant + partial activation invariant + canary tier authority advisory + channel drift detection placeholder + channel demotion warning). Phase 2 (별 PR, Wave 4 sub-Epic #1 Story-2/Story-3/Story-4/Story-5 sequential carrier) merge 시 runtime UpgradeAgent multi-channel dispatch + marketplace.json channels[] field 실 등록 + `scripts/check-3way-version-parity.sh` 의 channel 차원 확장 + ProductionEvidenceDeputy canary tier activation + `channel-drift-detection.yml` workflow self-app + `docs/evidence-checks-registry.yaml` `channel-drift-detection` entry (warning-tier) + promotion criteria mechanical lint (Story-4) + downgrade invariant declarative carrier (Story-5) sequential 활성.

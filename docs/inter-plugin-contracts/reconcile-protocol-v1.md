...
sunsetted_carrier_cfp: CFP-1186 (CFP-1111-Wave-4-Story-11)
...
version_history:
  ...
  - version: "1.14"
    ...
    change_type: status_transition
    summary: "status Deprecated → Sunsetted. CFP-1186 (CFP-1111 Wave 4 Story-11) carrier — sunset 실행. §4.3(k)/(l)/(m) + §4.13/§4.14 + §4.8 sunset_justification 영역 모두 SATISFIED: (1) walk-entries (Story-10 CFP-906/954/991/1014) 가 §4.3 area semantics carry (2) imperative-walker-protocol-v1 §2 walk_result 4-value enum (SUCCESS/SUCCESS_WITH_DEGRADATION/PARTIAL_FAILURE/FAILED) 이 §4.13 result_fidelity_binding result enum semantics carry (3) §4.14 canary_compatibility_check_binding + §4.8 version_handshake_3way_binding 효용이 walker schema 안 channel/version 분리 invariant 로 carry. β2 audit (#1113) 9/9 LOSSLESS 확인 — 본 contract 의 모든 binding block 효용이 imperative-walker-protocol-v1 (후속 carrier) 로 lossless 이전됨. ratchet 강화 보존 (carrier-preserved sunset, ADR-097 §결정 3 정합). §4.x 본문 삭제 금지 — Sunsetted = retired/superseded 상태, historical record 영구 보존."
    backward_compat: "PATCH bump (Sunsetted marker only, semantic 변경 0)"
  - version: "1.14.1"
    date: 2026-08-12
    cfp: CFP-2926
    change_type: cross_reference_relabel
    summary: "§4 `upgrade_agent_binding.spawn_model` 의 원인-귀속 라벨 정정 — '재귀 spawn 금지 platform inherent' → 'codeforge 정책'. platform 은 nested subagent spawn 을 허용하므로(depth ≤ 5) 구 라벨은 사실오류였다. 위상 SSOT = ADR-170 §결정 19 (`lead → Story-teammate → lane PL → SubAgent`, depth 0→1→2) + ADR-044 Amendment 7. ADR-039 → ADR-170 재제정 cross-reference 재지향 동반. field 추가·삭제 0 · schema shape 무변경 · Sunsetted status 무변경. CFP-2875 imperative-walker-protocol-v1 v1.2.0→v1.2.1 (cross-reference 재지향 PATCH, semantic 0) 선례 답습."
    backward_compat: "PATCH bump (라벨 문면 정정 only, semantic 변경 0 · consumer 영향 0)"
...
related_adrs:
  ...
  - ADR-072   # ProductionEvidenceDeputy mandate + EPIC cutover gate evidence quad — §4.14 §결정 5 production cutover gate semantic SSOT (CFP-991 Story-4 carrier — Amendment 3 carrier)
...
mechanical_implementation_binding:
  ...
  upgrade_agent_binding:
    ...
    spawn_model: "Orchestrator default subagent one-shot (ADR-170 §결정 1 — worker 자가-spawn 금지는 codeforge 정책이며 platform inherent 아님, ADR-170 §결정 19 / ADR-044 Amendment 7)"
...
multi_version_channel_pin_binding:   # v1.7 신설 (CFP-906 §4.3 (i) 발동, Wave 4 sub-Epic #1 Story-1) / v1.8 runtime active (CFP-932 Story-2), Epic CFP-882
  ...
  consumer_channel_field:
    ...
    tier_semantic:
      ...
      canary: "preview + production-impact tier. HIGH risk class (production cutover). admin tier 권장 (consumer-side 책임). canary tier 선언 시 Wave 4 sub-Epic #1 Story-3 ProductionEvidenceDeputy spawn trigger 영역 (ADR-072 §결정 1 정합)."
...
```yaml
canary_compatibility_check_binding:   # v1.11 신설, CFP-991 §4.3 (l) 발동, ADR-072 amendment_log Amendment 3 + ADR-076 §결정 9.2/9.6 + ADR-070 §결정 D6 carrier
  carrier_story: CFP-991  # Wave 4 sub-Epic #1 Story-4 (Epic CFP-882)
  carrier_adrs:
    - "ADR-072 amendment_log Amendment 3 (§결정 1 표 wrapper governance row append + §결정 5 표 row append)"
  ...
  enabled:
    ...
    workflow_self_app_exemption: "production_cutover_touching=true AND repo=wrapper AND code_change=0 triple-AND fast-PASS (Tier-1 exemption, ADR-072 §결정 6 정합)"
  ...
  promotion_criteria_4tuple:   # ADR-076 §결정 9.6 SSOT 4 industry exemplar verbatim cite — Chrome 3-channel primary + npm dist-tag / Rust 3-channel / K8s 3-stage 보조
    ...
    sub_fields:
      ...
      monitoring:
        measurement_source: "consumer production-side monitoring metric (Prometheus rate / drainage rate / WAL sample — ADR-072 §결정 5 evidence quad 정합)"
        gate_state_enum: ["pass", "fail", "n_a"]
        evidence_origin_annotation_required: true
      testing:
  ...
  canary_consumer_evidence_origin:   # RefactorAgent C-1 (open_extension: false closed-set enum 명시 강화) — SecurityArch T-1.1 wrapper Tier-1 declare-time bypass mitigation core field
    ...
    semantic:
      wrapper_self: "wrapper PR scope (Tier-1 declare-time exemption) — code 0 risk class, developer self-service. 사용 영역 = ADR-072 §결정 6 wrapper-self-app N/A invariant 정합 (영구 fast-PASS scope)"
  ...
  out_of_scope:
    ...
    - wrapper_self_app_runtime_4tuple_measurement   # ADR-072 §결정 6 wrapper-self-app N/A invariant 정합 (영구 fast-PASS scope)

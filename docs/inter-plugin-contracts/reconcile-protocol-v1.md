...
overlay_reconcile_implementation_binding:   # v1.4 신설, CFP-745 §4.3 (g) 발동
  ...
  base_acquisition_binding:
    ...
    integrity: "Story-3 §11.4 tarball checksum verify 재사용 (base corrupt = §7.4.1(a) abort + 명시 / 부재 = marker 부재 fallback). checksum verify 완료 전 per-file 3-way merge 진입 금지 (partial-state 0)"
    base_absent_first_reconcile: "snapshot 부재 (첫 reconcile) = wholesale_mirror_with_user_visible_loss_report (3-way 불가 시 2-way wholesale + loss report). 다음 reconcile 부터 snapshot = base 확보 (3-way 정상)"
  failure_modes:   # Change Plan §7.4.1 (a)-(h) overlay reconcile DR 8 failure mode SSOT cross-ref
    - "(a) base 부재/corrupt → wholesale_mirror fallback OR abort-before-touch (silent loss 0)"

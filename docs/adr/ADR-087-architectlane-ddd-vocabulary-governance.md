...
# ADR-087 — ArchitectLane DDD vocabulary governance
...
본 ADR 의 핵심 forcing function. 6 Story acceptance criteria 각각 "어휘 emit ↔ spawn/review/ADR criteria 변경" 검증항목 1건 이상 의무. **본 ADR Phase 2 PR5 LAND gate** = mctrader ADR-031 golden-path worked example (S6) 의 FINAL VERDICT 섹션이 다음 5 영역 박제로 evidence enumeration:
...
3. **Change Plan DDD field 변경 evidence** — §bounded_context_boundary + §affected_aggregates 안 explicit BC + Aggregate 박제
...
5. **ADR acceptance criteria 변경 evidence** — 본 ADR-087 § 결정 N + golden-path before/after diff 가 mctrader ADR-031 의 4-Layer 모델 (line 499-524) 위에 OHS (Layer 2 data /v1 REST API endpoint) + ACL (Layer 1 거래소 어댑터 → market Protocol 구현) 시연
...
- `docs/adr/ADR-080-agent-role-terminology-deputy-subagent.md` (S1 amendment 후보, DDD pattern 매핑 박제)
...
1. **ADR-080 DDD pattern 매핑 amendment** — deputy / SubAgent terminology 의 DDD pattern (Authority Pair / Domain Service / Subdomain Specialist) 매핑 박제. ratchet 강화 방향 (terminology 정확도 향상).

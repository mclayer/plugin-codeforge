...
### 결정 8: mirrored field 변경 시 sibling PR 동행 정책 (Amendment 2 / CFP-409)

CFP-391 retro follow-up — wrapper plugin.json mirrored field 변경 시 별도 `mclayer/marketplace` sibling PR open·merge 의무 부담 검토 결과:

**검토 안 비교**:

| 안 | 내용 | 판정 |
|---|---|---|
| (a) | wrapper PR 안에 `mclayer/marketplace/.claude-plugin/marketplace.json` 변경 포함 (단일 cross-repo PR) | **불가** — GitHub PR 은 단일 repo 내부만 지원, cross-repo PR 기능 부재 |
| (b) | 자동 sibling PR open Action (wrapper PR merge trigger → GitHub App / PAT 로 marketplace repo 에 PR 자동 open) | **deferred** — ROI 부정적. frequency × complexity 산정 시 구현·운영 부담 (GitHub App 인증 / PAT 보안 / 실패 retry / drift 감지) > 절약 시간 (3개월 3회 발동 빈도) |
| (c) | 현재 정책 유지 + CFP-408 `scripts/sync-contract-bump.sh` Stage 3 활용 권고 | **채택** |

**채택안 (c) 내용**:
- ADR-016 §결정 3 (Sync trigger) 정책 **무변경** — mirrored field 변경 시 sibling PR 동행 의무 그대로 유지
- 운영자 부담 경감은 [`scripts/sync-contract-bump.sh`](../../scripts/sync-contract-bump.sh) (CFP-408) Stage 3 marketplace plan emitter 가 담당
  - Stage 3 = `mclayer/marketplace/.claude-plugin/marketplace.json` mirrored field 갱신 plan emit
  - 현재 plan emitter only (사용자 수동 PR open) — Phase 2 follow-up CFP 에서 full automation 검토 가능

**근거**:
- (a) 는 GitHub 플랫폼 제약으로 즉시 기각
- (b) 는 구현 부담이 빈도 대비 과도 — 별도 CFP 발의 시 재검토 가능 (안 (c) 운영 후 ROI 재평가 base 확보)

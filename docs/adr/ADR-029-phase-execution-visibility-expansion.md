...
amendment_log:
  - date: 2026-05-09
    carrier: CFP-283
    section: "§결정 2"
    summary: "Sanitize policy 적용 범위 확장 — narration (stderr) → narration + telemetry ledger unified SSOT (ADR-043 §결정 3 정합)"
related_adrs:
  - ADR-042  # measurement channel architecture (sibling — sanitize scope expansion trigger)
  - ADR-043  # telemetry privacy policy (unified sanitize SSOT — §결정 3)
---
...
> **Amendment 1 (2026-05-09, CFP-283 carrier)** — sanitize policy 적용 범위 확장: 본 §결정 2 sanitize policy = narration (stderr) **+ telemetry ledger (ADR-043 정합) 양쪽 적용**. ADR-043 §결정 4 unified SSOT cross-ref. CFP-283 measurement channel (stop-event-v1) 신설로 ledger sanitize 도 동일 정책 inherit — privacy = cross-cutting concern (narration + ledger 동일 정책). future ledger (spawn-event-v1 land 시) 도 동일 적용 의무. ADR-043 §결정 3 = sanitize Deny-list regex 6 pattern SSOT, 본 §결정 2 = format / 한국어 lane / stderr-only invariant SSOT. 양쪽 SSOT 변경 시 sync 의무.

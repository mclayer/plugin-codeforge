...
- **Lifecycle / retention**: N most-recent (default N=5, configurable via consumer `.claude/_overlay/project.yaml` `upgrade.snapshot_retention_count`). per-MAJOR-version retention 영역은 Wave 4 E1 carrier.
...
- **Atomicity boundary**: **per-plugin** (Story-1 scope). codeforge family 7 plugin atomic upgrade = Wave 2 Story-4 carrier 영역 (per-family atomicity 확장). per-file atomicity = too granular (rollback 빈번), per-family = Wave 2 carrier — per-plugin 이 본 Story-1 의 atomicity unit.
- **Completion criterion**: ADR-053 §D2 cross-ref — codeforge plugin self 변경 시 marketplace sync + consumer install + version drift PASS 3 조건 AND. **본 ADR 의 transaction completion = ADR-053 D2 의 prerequisite 가 충족된 시점**.
...
### 결정 7 — Transaction completion criterion = ADR-053 D2 prerequisite

본 ADR §결정 3 transaction completion (atomic unit 완결) = ADR-053 §D2 (codeforge 변경 시 consumer 배포 포함) 의 prerequisite 가 충족된 시점.

**ADR-053 §D2 verbatim 의 3 조건 AND**:
- marketplace sync PR open · merge 완료 (ADR-016 mirrored field 4종 atomic — ADR-063 §결정 5)
- consumer install 완료 (`/plugins install` 또는 동등)
- `bash scripts/check-codeforge-version-drift.sh` PASS (no drift)

**본 ADR transaction completion criterion**: 위 3 조건 모두 AND 충족 = transaction 의 atomic unit 완결. 미충족 시 다음 Story 작업 진입 차단 (ADR-053 §D2 의 blocking semantic 보존).
...
### 결정 8 — codeforge family scope 7 plugin atomic 의 Story-1 atomicity unit 분리

ADR-016 §결정 1 정합 — wrapper + 6 lane plugin = 7 plugin atomic unit. 본 ADR §결정 3 transaction 의 per-plugin atomicity boundary 는 **Story-1 scope 한정 결정** — per-family (7 plugin atomic) 영역은 Wave 2 Story-4 carrier 분리.

**rationale**: 본 Story-1 = semantic SSOT only, runtime 0건. per-family atomicity 의 mechanical implementation (`scripts/atomic-upgrade-7-plugins.sh`) 은 Wave 2 carrier. 본 Story-1 의 transaction atomicity 정의는 per-plugin 으로 fix — per-family 확장은 Wave 2 Story-4 진입 시점 본 ADR Amendment trigger (ratchet 강화, ADR-058 §결정 5 정합).
...
- **Wave 2 Story-4**: 7 plugin atomic upgrade — 본 ADR §결정 8 의 atomicity boundary 확장 carrier.
...
- atomicity boundary 확장 (per-plugin → per-family — Wave 2 Story-4 carrier 시점)

...
amendments:
  ...
  - amendment_id: 24
    ...
    date: 2026-05-25
    summary: "§결정 1 layer 1 sub-scope (1-M) 신설 — own-author synthesis 보고 vs actual git commit gap verify mandate (F-DR-003 carrier). axis disjoint from 1-A (cross-repo state Amd 2) / 1-B (Issue body authorship Amd 2) / 1-C (user-utterance verbatim block 형식 Amd 5) / 1-D (cross-repo label-write authority Amd 14) / 1-E (PRE-SPAWN-ORIGIN-MAIN-SHA pre-spawn pin Amd 15) / 1-F (mid-spawn periodic origin re-pin Amd 16) / 1-G (amendment-slot pre-reservation strict claim Amd 17) / 1-H (Orchestrator §10 FIX Ledger resolution source/evidence Amd 18) / 1-I (pre-spawn-prompt-finalize window race shortening Amd 19) / 1-J (cross-repo worktree target authority Amd 21) / 1-K (numeric claim write-time strict claim mandate Amd 22) / 1-L (spawn prompt fact verify upstream-inherited stale fact super-class Amd 23). 본 sub-scope 1-M = **own-author synthesis 보고 vs actual artifact (git commit) gap own-author verify axis** — author 자체가 자기 작성 결과를 잘못 claim (예: PL verdict packet 'Artifacts written: <file>' 보고 했으나 실 git commit 0건 — worktree only modified/untracked, ArchitectPL synthesis 단계 안 작업 결과 verify 부재) 차단. axis 분리 vs Amd 23 1-L = upstream-inherited stale fact (4 sub-source: 사용자 발화 / sibling Issue body / sister PR commit message / 별 carrier retro file) ↔ 본 Amd 24 1-M = own-author synthesis 결과 vs actual commit gap (자기 작업 결과 self-verify, downstream of own write 영역). 1-L = upstream input verify (synthesis input 측), 1-M = downstream output verify (synthesis output ↔ artifact gap). axis 인접하나 verify 대상 disjoint (input vs output, upstream-inherited vs own-author). axis 분리 vs Amd 5 1-C (사용자 발화 content 전달 형식 verbatim 의무): 1-C = upstream content 전달 형식 / 1-M = own-author synthesis 결과 vs actual artifact gap. axis 분리 vs Amd 18 1-H (Orchestrator §10 FIX Ledger resolution source/evidence verify): 1-H = Orchestrator monopoly §10 FIX Ledger row resolution field claim source/evidence verify (fix-event-v1 contract level Orchestrator self-write monopoly axis) / 1-M = ArchitectPL / Dev / lane PL synthesis 보고 vs actual git commit gap (lane agent self-write own-author axis, fix-event-v1 contract 영역 외). 4-tuple primitive: (a) **synthesis 보고 시점 actual git commit verify 의무** — author 가 verdict / 산출 / 완료 보고 작성 시점 `git -C <worktree> log --oneline origin/main..HEAD` direct execute + actual commit list 확인 의무 (worktree modified/untracked 영역 commit 0건 미포함 — claim 'Artifacts written' = actual commit hash 매핑 의무) / (b) **review-verdict-v4 packet 안 optional `artifact_commits[]` field 영역** — commit hash array (40-char hex), future MINOR bump (v4.10 carrier 별 sub-CFP) / (c) **verify-before-trust 4-layer 정합 — Story §14 Lane Evidence row append 시 actual commit verify** — lane outcome row append 시 `git log` actual commit hash 인용 의무 (ADR-073 verify-before-assert + ADR-082 §결정 1 layer 1 sub-scope 1-M dual binding) / (d) **`synthesis_vs_commit_gap_verified: <bool>` field annotation 의무** — synthesis packet / verdict packet / 완료 보고 안 명시 (write-time semantic truth verify, annotation 부재 시 sentinel 발화). 동인 = CFP-1523 carrier F-DR-002 P0 finding (Phase 1 ArchitectPL verdict packet 'Artifacts written: ...' 보고 ≠ actual git commit, DesignReviewPL audit verify-before-trust direct git status verify 후 detect, FIX iter 1 dispatch). pattern_count 1 deferred-followup carrier (Wave 1 declaration-only mandate, mechanical wire Wave 2 별 carrier — pattern_count 누적 시 carrier 별 발의). PMOAgent CFP-1523 retro carry-over (memory `project_cfp_1523_complete.md` 안 finding 3 박제). Wave 1 = declaration-only behavioral mandate (`mechanical_enforcement_actions[]` 신규 entry `synthesis-vs-commit-gap-check` warning-tier deferred-followup append). Wave 2 mechanical wire (Python SSOT lint per ADR-061 `scripts/lib/check_synthesis_vs_commit_gap.py` + `scripts/check-synthesis-vs-commit-gap.sh` bash thin wrapper + `templates/github-workflows/synthesis-vs-commit-gap-check.yml` workflow + `.github/workflows/` byte-identical mirror + `tests/scripts/cfp-synthesis-vs-commit-gap.bats` fixture RED→GREEN stash proof per §결정 11.A + label-registry-v2 `hotfix-bypass:synthesis-vs-commit-gap-check` MINOR bump + evidence-checks-registry warning-tier initial registration ADR-060 §결정 5 정합 + review-verdict-v4 v4.9 → v4.10 MINOR carrier 별 sub-CFP `artifact_commits[]` optional field schema 신설) = 별 sub-CFP carrier 분리 (CFP-1437/1436/1435/1559/1578/1601/1590 Wave 1→Wave 2 split precedent 답습). False-positive 완화 guards (code-span EXEMPT / quoted-text EXEMPT / templates/** EXEMPT / docs/stories/§9 transcript EXEMPT) = Wave 2 lint design 시 결정 — Phase 1 declarative scope 외. 본 Amendment 24 자체가 META-self-applied (§결정 10.D 19th applied case + Amendment 17 §결정 1-G strict pre-reservation claim mandate 5th applied case after Amd 18 CFP-1342 + Amd 21 CFP-1578 + Amd 22 CFP-1601 + Amd 23 CFP-1590): 본 Amendment 번호(24) 가 target ADR-082 frontmatter `amendments:` Read verify (worktree HEAD `f2e78b1` origin/main amendments[] max=23 — CFP-1590 Amd 23 merge 후 base, 정확 next-slot for CFP-1589 = 24) 후 결정 + ADR-RESERVATION amendments_reserved[] row pre-append + commit `98ebb8c` + push (verified-via: `git show origin/main:docs/adr/ADR-082-write-time-self-write-verification-mandate.md` frontmatter amendments[] 2026-05-25 KST 기준 origin/main HEAD `f2e78b1` PRE-SPAWN-ORIGIN-MAIN-SHA pinned + ADR-082 §결정 11.A direct extract sed amendments[] max=23 verify). doc-only fast-path ADR-054 Cat 1 단일 PR (script / workflow / bats wire 0건, ADR Amendment + CLAUDE.md cross-ref + ADR-RESERVATION row + evidence-checks-registry yaml row append + CHANGELOG only)."
    direction: strengthening
  ...
  - amendment_id: 25
    ...
    date: 2026-05-25
    summary: "§결정 1 layer 1 sub-scope (1-N) 신설 — numeric claim write-time verify Wave 2 mechanical enforcement wire (ADR-082 sub-scope 1-K Amendment 22 CFP-1601 carrier 의 Wave 1 declaration-only → Wave 2 mechanical lint enforcement wire). Issue #1612 HIGH FU-A from CFP-1601 retro carrier — 1-K Wave 1 declarative mandate 의 mechanical enforce. axis disjoint from 1-A (cross-repo state Amd 2) / 1-B (Issue body authorship Amd 2) / 1-C (user-utterance verbatim block 형식 Amd 5) / 1-D (cross-repo label-write authority Amd 14) / 1-E (PRE-SPAWN-ORIGIN-MAIN-SHA pre-spawn pin Amd 15) / 1-F (mid-spawn periodic origin re-pin Amd 16) / 1-G (amendment-slot pre-reservation strict claim Amd 17) / 1-H (Orchestrator §10 FIX Ledger resolution source/evidence Amd 18) / 1-I (pre-spawn-prompt-finalize window race shortening Amd 19) / 1-J (cross-repo worktree target authority Amd 21) / 1-K (numeric claim write-time strict claim mandate Amd 22 Wave 1 declaration-only) / 1-L (spawn prompt fact verify upstream-inherited stale fact super-class Amd 23) / 1-M (own-author synthesis 보고 vs actual git commit gap Amd 24). 본 sub-scope 1-N = **1-K Wave 1 declaration-only behavioral mandate 의 Wave 2 mechanical enforcement wire** (lint script Python SSOT per ADR-061 + bash thin wrapper + workflow yml hydrate + .github/workflows/ byte-identical mirror per ADR-005 + bats fixture RED→GREEN stash proof per §결정 11.A + label-registry-v2 MINOR bump + evidence-checks-registry warning-tier initial registration per ADR-060 §결정 5). axis 분리 vs Amd 22 1-K = 1-K mandate 자체 (4-step verify-before-write 의무 + 6 numeric claim closed-set dimension) ↔ 본 Amd 25 1-N = 1-K Wave 2 mechanical enforcement scope (declaration → mechanical lint enforce, ADR-060 §결정 5 'first introduction = warning mode' default). 1-N = mechanical wire SSOT 영역 (lint script + workflow + bats fixture binding) — 1-K = behavioral mandate SSOT 영역 (4-step + 6 dimension enum). 4-tuple primitive: (a) **lint script SSOT** — `scripts/lib/check_numeric_claim_write_time.py` Python SSOT (ADR-061 multi-line Python convention, > 5줄 → 외부 .py file 의무) + `scripts/check-numeric-claim-write-time-verify.sh` bash thin wrapper (`python -m scripts.lib.check_numeric_claim_write_time \"$@\"` shim 형식) / (b) **6 dimension detection logic** — line count (`wc -l` / `grep -c` line-occurrence) / file count (`find ... | wc -l` / `git diff --shortstat`) / API count (`grep -c mcp__github__` / agent spawn count) / pattern_count (cross-Story ADR-045 §D-9 base) / commit count (`git rev-list --count`) / row count (yaml row / md table row / contract field) — 각 dimension 별 numeric claim regex pattern + source command pattern map (Python dict SSOT) / (c) **FP guard 4종 enumeration (Wave 2 lint design)** — code-span EXEMPT (backtick block) + quoted-text EXEMPT (`>` blockquote / `'/'` quote) + templates/** EXEMPT (canonical example pattern) + docs/stories/§9 transcript EXEMPT (Round 0 verbatim cite) + PER_BLOCK_SCAN_CAP=50 line CodeQL ReDoS guard (CFP-1497 PR #1499 sentinel verbatim 답습, nested quantifier regex 절대 금지) / (d) **workflow + bypass channel + evidence-checks-registry binding** — `templates/github-workflows/numeric-claim-write-time-verify.yml` workflow + `.github/workflows/` byte-identical mirror per ADR-005 + `continue-on-error: true` warning tier + `hotfix-bypass:numeric-claim-write-time-verify` bypass label per ADR-024 Amendment 6/8 §결정 6.A 5 lint chain inherit + evidence-checks-registry warning-tier initial registration `numeric-claim-write-time-verify` per ADR-060 §결정 5 default. 동인 = CFP-1601 Issue #1612 HIGH FU-A retro carry-over — 1-K Wave 1 declaration-only mandate 의 mechanical enforce 부재 영역 (Wave 1 → Wave 2 split precedent CFP-1437/1436/1435/1559/1578/1601/1590/1589 11번째 instance). Wave 1 = 본 Amd 25 sub-scope 1-N declarative wire SSOT + evidence-checks-registry warning-tier initial registration entry append + label-registry-v2 hotfix-bypass family MINOR bump declarative anchor (Phase 1 PR — 본 carrier). Wave 2 = actual script + workflow + bats fixture file 실 write (Phase 2 PR 별 sub-carrier — DeveloperPL + QADev spawn). pattern_count ≥ N 추가 reach 시 warning → blocking-on-pr 승격 = Wave 3 별 carrier 분리 (ADR-060 §결정 6 promotion gate AND 3/3 — PR 누적 ≥ 20 + bypass 외 failure = 0 + sibling Story merged). False-positive 완화 guards 4종 (code-span / quoted / templates/** / docs/stories/§9) + PER_BLOCK_SCAN_CAP=50 ReDoS guard = Wave 2 lint design 시 결정 (Phase 1 declarative scope 외, 본 Amendment summary 안 enumeration only). 본 Amendment 25 자체가 META-self-applied (§결정 10.D 20th applied case + Amendment 22 §결정 1-K numeric claim write-time strict claim mandate 1st applied case META first applied for Wave 2 wire carrier — 본 Amendment 번호(25) 의 source command `grep -c '^  - amendment_id:' docs/adr/ADR-082-...md` actual = 41 (line occurrence count) vs semantic intent (max amendment_id slot value = 24) divergence 인식 + source command 정밀화 `grep -oE '^  - amendment_id: [0-9]+' | tail -1` 정확 next-slot = 25 verify after CFP-1589 Amd 24 merge + Amendment 17 §결정 1-G strict pre-reservation claim mandate 6th applied case after Amd 18 CFP-1342 + Amd 21 CFP-1578 + Amd 22 CFP-1601 + Amd 23 CFP-1590 + Amd 24 CFP-1589 + 본 carrier): 본 Amendment 번호(25) 가 target ADR-082 frontmatter `amendments:` Read verify (origin/main HEAD `5b5c9f7b` amendments[] max=24 — CFP-1589 Amd 24 merge 후 base, 정확 next-slot for CFP-1612 = 25) 후 결정 + ADR-RESERVATION amendments_reserved[] row pre-append (status active 직접 전환, ArchitectAgent chief author precedent — ADR-082 Amendment 24 row CFP-1589 직전 carrier 정합) + mid-flight collision recovery 3rd applied case (initial spawn pinned origin/main `f2e78b16` + amendments[] max=23 → CFP-1589 Amd 24+1-M merge 후 base shift to `5b5c9f7b` + max=24 → renumber Amd 24→25 + sub-scope 1-M→1-N on rebase, ADR-082 §결정 1-L spawn packet fact verify-before-trust mandate 1st applied case recursive dogfooding — 본 spawn packet 안 'Amendment 24 + sub-scope 1-M' claim 이 target ADR-082 frontmatter Read verify 후 stale at planning 발견, 정확 next-slot = 25 + 1-N 정정). doc-only fast-path ADR-054 Cat 1 단일 PR (ADR Amendment + ADR-RESERVATION row + evidence-checks-registry yaml row + label-registry-v2 MINOR + MANIFEST.yaml version bump + CHANGELOG entry, script/workflow/bats wire 0건 = Phase 2 별 sub-carrier)."
    direction: strengthening
  ...
  - amendment_id: 26
    ...
    date: 2026-05-25
    summary: "§결정 1 layer 1 sub-scope (1-O) 신설 — PR commit message + PR body 안 numeric claim write-time strict claim mandate (axis disjoint from 1-K Amd 22 write-time governance docs scope + 1-N Amd 25 Wave 2 mechanical wire scope; 본 1-O = PR commit message + PR body layer 3rd axis). ADR-045 §D-9 Mandatory escalation 산물 (Pattern 2 meta-self-application-accuracy-violation pattern_count 2 reach: (i) CFP-1601 §13.C row 3 claim '11' vs actual 15 — Story scope, sub-scope 1-K territory / (ii) CFP-1612 PR #1631 commit message + PR body 안 claim '29 / ~470 / ~155' vs canonical actual '30 / 605 / 183' — PR commit msg + PR body layer, NEW scope codify 의무 영역, 본 1-O 영역). axis disjoint from 1-A (cross-repo state Amd 2) / 1-B (Issue body authorship Amd 2) / 1-C (user-utterance verbatim block 형식 Amd 5) / 1-D (cross-repo label-write authority Amd 14) / 1-E (PRE-SPAWN-ORIGIN-MAIN-SHA pre-spawn pin Amd 15) / 1-F (mid-spawn periodic origin re-pin Amd 16) / 1-G (amendment-slot pre-reservation strict claim Amd 17) / 1-H (Orchestrator §10 FIX Ledger resolution source/evidence Amd 18) / 1-I (pre-spawn-prompt-finalize window race shortening Amd 19) / 1-J (cross-repo worktree target authority Amd 21) / 1-K (numeric claim write-time strict claim mandate Amd 22 — write-time governance docs scope: ADR text / Change Plan / Story / spawn packet wording / Issue body) / 1-L (spawn prompt fact verify upstream-inherited stale fact super-class Amd 23) / 1-M (own-author synthesis 보고 vs actual git commit gap Amd 24) / 1-N (1-K Wave 2 mechanical wire scope — lint script + workflow + bats Amd 25). 본 sub-scope 1-O = **PR commit message + PR body 안 numeric claim write-time strict claim mandate** (PR-level artifact layer, axis 3rd). axis 분리 vs 1-K Amd 22 = scope target docs (governance docs in-tree: ADR / Change Plan / Story / spawn packet / Issue body) ↔ 본 Amd 26 1-O = scope target PR-level artifact (commit message body + PR description body, git/GitHub layer). axis 분리 vs 1-N Amd 25 = mechanism (mechanical lint enforce, scripts/lib + workflow + bats wire) ↔ 본 Amd 26 1-O = declaration-only Wave 1 (mechanical wire 확장은 별 sub-CFP Wave 2 carrier). same 6 numeric closed-set dimension (line count / file count / API count / pattern_count / commit count / row count) inherit from 1-K Amd 22 + same 4-step verify-before-write mandate inherit ((a) source command identify (b) direct execute (c) claim↔actual cross-verify (d) write only on match). Wave 1 declaration-only — Wave 2 mechanical wire (current `scripts/lib/check_numeric_claim_write_time.py` Python SSOT scope 확장 to PR commit msg + PR body detection layer, ADR-060 framework evidence-gated initial registration warning-tier) = 별 sub-CFP carrier 분리 (precedent ADR-082/070/077/078/097 답습 + CFP-1437/1436/1435/1559/1578/1601/1590/1589/1612 12번째 instance Wave 1→Wave 2 split). 동인 = CFP-1612 retro Pattern 2 meta-self-application-accuracy-violation pattern_count 2 reach: (i) CFP-1601 §13.C row 3 (1-K write-time mandate codify carrier 자체 안 self-violation — Story scope, 1-K territory) + (ii) CFP-1612 PR #1631 commit message + PR body (1-N Wave 2 mechanical wire carrier 자체 안 self-violation — PR scope, 1-O NEW scope codify 의무). 두 instance 의 axis disjoint sub-scope 영역 (Story vs PR) 동일 root pattern (META self-application accuracy violation) — ADR-045 §D-9 threshold 2 reach = Mandatory escalation. META-self-applied (§결정 10.D 21st applied case + Amendment 17 §결정 1-G strict pre-reservation claim mandate 7th applied case after Amd 18 CFP-1342 + Amd 21 CFP-1578 + Amd 22 CFP-1601 + Amd 23 CFP-1590 + Amd 24 CFP-1589 + Amd 25 CFP-1612 + 본 carrier + Amendment 22 §결정 1-K numeric claim write-time strict claim mandate 2nd applied case after Amendment 25 first wave 2 wire carrier — 본 Amendment 번호(26) 의 source command `grep -oE '^  - amendment_id: [0-9]+' docs/adr/ADR-082-...md | tail -1` actual = 'amendment_id: 25' (Amd 25 CFP-1612 점유) → 정확 next-slot = 26 verify after CFP-1612 Amd 25 merge): 본 Amendment 번호(26) 가 target ADR-082 frontmatter `amendments:` Read verify (origin/main HEAD `d1c629f0` amendments[] max=25 — CFP-1612 Amd 25 merge 후 base post-rebase, 정확 next-slot for CFP-1637 = 26) 후 결정 + ADR-RESERVATION amendments_reserved[] row pre-append (status active 직접 전환, ArchitectAgent chief author precedent — ADR-082 Amendment 25 row CFP-1612 직전 carrier 정합). doc-only fast-path ADR-054 Cat 1 단일 PR (ADR Amendment + ADR-RESERVATION row + Story file + Change Plan only, script/workflow/bats wire 0건 = Wave 2 별 sub-CFP carrier — `scripts/lib/check_numeric_claim_write_time.py` Python SSOT scope 확장 to PR commit msg + PR body detection layer)."
    direction: strengthening
  ...
  - amendment_id: 27
    ...
    date: 2026-05-26
    summary: "§결정 1 layer 1 sub-scope (1-P) 신설 — sub-scope 1-O (Amendment 26, CFP-1637) Wave 2 mechanical enforcement wire (declaration-only behavioral mandate 의 Wave 2 mechanical lint enforcement wire, declaration → mechanical enforce promotion path). Issue #1647 HIGH FU-A from CFP-1637 retro carrier — sub-scope 1-O Amd 26 CFP-1637 Wave 1 declaration-only mandate (PR commit message + PR body layer numeric claim write-time strict claim mandate) 의 mechanical enforce 부재 영역 codify. axis disjoint from 1-A (cross-repo state Amd 2) / 1-B (Issue body authorship Amd 2) / 1-C (user-utterance verbatim block 형식 Amd 5) / 1-D (cross-repo label-write authority Amd 14) / 1-E (PRE-SPAWN-ORIGIN-MAIN-SHA pre-spawn pin Amd 15) / 1-F (mid-spawn periodic origin re-pin Amd 16) / 1-G (amendment-slot pre-reservation strict claim Amd 17) / 1-H (Orchestrator §10 FIX Ledger resolution source/evidence Amd 18) / 1-I (pre-spawn-prompt-finalize window race shortening Amd 19) / 1-J (cross-repo worktree target authority Amd 21) / 1-K (numeric claim write-time strict claim mandate Amd 22 — write-time governance docs scope) / 1-L (spawn prompt fact verify upstream-inherited stale fact super-class Amd 23) / 1-M (own-author synthesis 보고 vs actual git commit gap Amd 24) / 1-N (1-K Wave 2 mechanical wire scope Amd 25, governance docs layer) / 1-O (PR commit msg + PR body declaration-only Wave 1 scope Amd 26). 본 sub-scope 1-P = **1-O Wave 2 mechanical enforcement wire scope** (mechanism axis — declaration mandate 1-O ↔ mechanical lint enforce 1-P disjoint complement layered, 1-K ↔ 1-N pattern 답습). axis 분리 vs 1-N Amd 25 = scope target (governance docs in-tree: ADR text / Change Plan / Story / spawn packet / Issue body) ↔ 본 Amd 27 1-P = scope target (PR commit message body + PR description body, git/GitHub layer — 1-O scope 확장). 1-N + 1-P = 1-K + 1-O 각각의 Wave 2 mechanical wire pair (axis = mechanical wire SSOT scope, scope target = governance docs / PR-level artifact disjoint complement). 4-tuple primitive: (a) **lint script SSOT scope 확장** — `scripts/lib/check_numeric_claim_write_time.py` Python SSOT (CFP-1612 wired, ADR-061 multi-line Python convention 정합) 의 detection scope 확장 to PR commit message body + PR description body layer (current scope = Story file + Change Plan governance docs in-tree; 본 Amd 27 1-P scope 확장 = PR commit msg + PR body source ingestion 추가, `gh pr view --json title,body,commits` 또는 git log range subprocess primitive) + `scripts/check-numeric-claim-write-time-verify.sh` bash thin wrapper 의 `--scope pr-commit-msg` + `--scope pr-body` flag 추가 (or unified --scope all default) / (b) **6 dimension detection logic 재사용** — 1-K + 1-N + 1-O 동일 6 numeric closed-set dimension (line count / file count / API count / pattern_count / commit count / row count) Python dict SSOT regex+source command pattern map 재사용 (PR-level artifact layer detection regex = governance docs regex 와 동일 — `\\d+\\s*(commits?|files?|lines?|LOC)` 등 universal patterns) / (c) **PR-level source ingestion 추가** — workflow trigger 확장 (PR open + sync event) + `gh pr view --json title,body,commits` source ingestion + commit message body inspection (`git log --format=%B <range>`) + PR description body inspection + FP guard 4종 재사용 (code-span EXEMPT + quoted-text EXEMPT + templates/** EXEMPT + docs/stories/§9 transcript EXEMPT) + PER_BLOCK_SCAN_CAP=50 ReDoS guard 재사용 / (d) **workflow + bypass + evidence-checks-registry binding 갱신** — `templates/github-workflows/numeric-claim-write-time-verify.yml` workflow trigger 확장 (PR open + PR sync 추가) + `.github/workflows/` byte-identical mirror per ADR-005 + 기존 `continue-on-error: true` warning tier 유지 + 기존 `hotfix-bypass:numeric-claim-write-time-verify` bypass label per ADR-024 Amendment 6/8 §결정 6.A 재사용 (bypass label 신설 0건, scope expansion only) + evidence-checks-registry entry `numeric-claim-write-time-verify` target_section 갱신 (`§결정 1 layer 1 sub-scope (1-K + 1-N + 1-O + 1-P)` 4 sub-scope 정합, label-registry MINOR bump 0건). 동인 = Issue #1647 HIGH FU-A from CFP-1637 retro carry-over — 1-O Wave 1 declaration-only mandate (Amd 26) 의 mechanical enforce 부재 영역. Wave 1 → Wave 2 split precedent CFP-1437/1436/1435/1559/1578/1601/1590/1589/1612/1637 13번째 instance (CFP-1612 = 11th + CFP-1637 = 12th + 본 CFP-1647 = 13th sequential lineage extension). Wave 1 = 본 Amendment 27 declarative SSOT (Phase 1 PR — ADR Amendment + ADR-RESERVATION + mechanical_enforcement_actions entry 갱신 + Story + Change Plan + CHANGELOG only, script 갱신 0건). Wave 2 = actual mechanical wire (Phase 2 PR 별 sub-carrier — DeveloperPL + QADev spawn): Python SSOT scope 확장 + workflow PR trigger 추가 + bats fixture (PR commit msg + PR body fixture pair RED→GREEN stash proof per §결정 11.A) + evidence-checks-registry entry target_section 갱신. pattern_count ≥ N 추가 reach 시 warning → blocking-on-pr 승격 = Wave 3 별 carrier 분리 (ADR-060 §결정 6 promotion gate AND 3/3). 본 Amendment 27 자체가 META-self-applied (§결정 10.D 22nd applied case + Amendment 17 §결정 1-G strict pre-reservation claim mandate 8th applied case after Amd 18 CFP-1342 + Amd 21 CFP-1578 + Amd 22 CFP-1601 + Amd 23 CFP-1590 + Amd 24 CFP-1589 + Amd 25 CFP-1612 + Amd 26 CFP-1637 + 본 carrier + Amendment 22 §결정 1-K numeric claim verify-before-write 3rd applied case after Amd 25 + Amd 26 + Amendment 23 §결정 1-L spawn prompt fact verify-before-trust 2nd applied case for spawn packet Amendment 27 + sub-scope 1-P claim verify + Amendment 24 §결정 1-M synthesis vs commit gap verify 1st applied case for inline self-verify): 본 Amendment 번호(27) 가 target ADR-082 frontmatter `amendments:` Read verify (origin/main HEAD `e1e2b751` amendments[] max=26 — CFP-1637 Amd 26 merge 후 base, source command `grep -oE '^  - amendment_id: [0-9]+' docs/adr/ADR-082-...md | tail -1` actual = `amendment_id: 26` → 정확 next-slot for CFP-1647 = 27) 후 결정 + ADR-RESERVATION amendments_reserved[] row 27 pre-append (status active 직접 전환, ArchitectAgent chief author precedent — ADR-082 Amendment 26 row CFP-1637 직전 carrier 정합)."
    direction: strengthening
...
amendment_log:
  ...
  - amendment_id: 27
    ...
    note: "CFP-1647 carrier — Issue #1647 HIGH FU-A from CFP-1637 retro carrier. sub-scope 1-O Amd 26 CFP-1637 Wave 1 declaration-only mandate (PR commit message + PR body layer numeric claim write-time strict claim mandate) 의 Wave 2 mechanical enforcement wire (declaration → mechanical lint enforce promotion path). 4-tuple primitive: (a) `scripts/lib/check_numeric_claim_write_time.py` Python SSOT (CFP-1612 wired) detection scope 확장 to PR commit msg + PR body layer (current scope = Story + Change Plan governance docs in-tree; 본 Amd 27 1-P scope 확장 = PR-level artifact ingestion) + bash thin wrapper `--scope pr-commit-msg` + `--scope pr-body` flag 추가 (or unified scope) per ADR-061 / (b) 6 dimension detection logic 재사용 (line/file/API/pattern/commit/row count) Python dict SSOT regex+source command pattern map 재사용 / (c) workflow trigger 확장 (PR open + sync event 추가) + `gh pr view --json title,body,commits` source ingestion + commit message body inspection `git log --format=%B <range>` + PR description body inspection + FP guard 4종 재사용 (code-span / quoted / templates/** / docs/stories/§9) + PER_BLOCK_SCAN_CAP=50 ReDoS guard / (d) `templates/github-workflows/numeric-claim-write-time-verify.yml` + `.github/workflows/` byte-identical mirror per ADR-005 trigger 확장 + 기존 `continue-on-error: true` warning tier 유지 + 기존 `hotfix-bypass:numeric-claim-write-time-verify` bypass label 재사용 (bypass label 신설 0건 — scope expansion only) + evidence-checks-registry entry `numeric-claim-write-time-verify` target_section 갱신 (`§결정 1 layer 1 sub-scope (1-K + 1-N + 1-O + 1-P)`). Wave 1 declarative SSOT (본 Amendment 27 Phase 1 PR) = ADR-082 §결정 1 layer 1 sub-scope 1-P append + mechanical_enforcement_actions[] entry `numeric-claim-write-time-verify` target_section 갱신 + ADR-RESERVATION amendments_reserved[] row 27 pre-append + Story + Change Plan + CHANGELOG entry append. Wave 2 actual mechanical wire (Phase 2 PR 별 sub-carrier — DeveloperPL + QADev spawn): Python SSOT scope 확장 + workflow PR trigger 추가 + bats fixture (PR commit msg + PR body fixture pair RED→GREEN stash proof per §결정 11.A) + evidence-checks-registry entry target_section 갱신. precedent: CFP-1601 (1-K declaration) → Wave 2 CFP-1612 (1-N mechanical wire) → CFP-1637 (1-O scope 확장 declaration) → Wave 2 본 CFP-1647 (1-P mechanical wire) = 13번째 Wave 1→Wave 2 split lineage extension. pattern_count ≥ N 추가 reach 시 warning → blocking-on-pr 승격 = Wave 3 별 carrier 분리 (ADR-060 §결정 6 promotion gate AND 3/3 — PR 누적 ≥ 20 + bypass 외 failure = 0 + sibling Story merged). 본 Amendment 27 자체가 META-self-applied (§결정 10.D 22nd applied case + Amendment 17 §결정 1-G strict pre-reservation claim mandate 8th applied case + Amendment 22 §결정 1-K numeric claim verify-before-write 3rd applied case + Amendment 23 §결정 1-L spawn prompt fact verify-before-trust 2nd applied case + Amendment 24 §결정 1-M synthesis vs commit gap 1st applied case): 본 Amendment 번호(27) 가 target ADR-082 frontmatter `amendments:` Read verify (origin/main HEAD `e1e2b751` amendments[] max=26 — CFP-1637 Amd 26 merge 후 base, source command `grep -oE '^  - amendment_id: [0-9]+' docs/adr/ADR-082-...md | tail -1` actual = `amendment_id: 26` → 정확 next-slot for CFP-1647 = 27) 후 결정 + ADR-RESERVATION amendments_reserved[] row pre-append (status active 직접 전환, ArchitectAgent chief author precedent — ADR-082 Amendment 26 row CFP-1637 직전 carrier 정합). doc-only fast-path ADR-054 Cat 1 단일 PR (ADR Amendment + ADR-RESERVATION row + Story + Change Plan + CHANGELOG entry only, script/workflow/bats wire 0건 = Wave 2 별 sub-CFP carrier)."
related_stories:

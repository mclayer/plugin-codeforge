...
**Status:** Proposed
**Date:** 2026-05-05
**Related:** ADR-013 (dogfood-out), ADR-016 (marketplace), ADR-020 (cross-repo Epic), ADR-022 (Sonnet decider), ADR-024 (story-scoped branch policy)
**CFP:** CFP-96 (Epic, #198), CFP-97 (#199) + CFP-98 (#200) Phase 2 implementation
...
- consumer `.github/workflows/` 11종 (CFP-99 close 후 14종) sync
...
**Primary — Story phase 진입**: 기존 `phase-gate-mergeable.yml` + `phase-label-invariant.yml` workflow. CFP-96 Phase 4 에서 #143 fix (doc-only PR fast-pass) + #144 fix (CI terminal state classification) 적용.

**Secondary — UserPromptSubmit hook (NEW, CFP-98)**: consumer `.claude/settings.json` 에 등록, wrapper `overlay/hooks/userprompt-reminder.{sh,ps1}` 호출. 검출 패턴 regex `(구현|만들|수정|짜|fix|implement|refactor|create|add)` 매치 + 활성 Story 미특정 ∨ phase label 부재 시 stdout 으로 reminder 출력 → Claude Code 가 LLM context 에 inject. **Block 아님 — warning inject only.**

**Tertiary — SessionStart hook (강화, CFP-97 + CFP-98)**: 기존 `regen-agents.sh` 호출 + 신규 `check-bootstrap.{sh,ps1}` 강화. 부재/불일치 시 stdout 으로 안내 + LLM context inject. **Claude Code hook 자체는 session 차단 권한 없음** — LLM 이 첫 reasoning turn 에 reminder 받아 사용자에게 dependency 미충족 surface + 후속 작업 이전에 install 안내 (enforcement = LLM 측 책임).
...
- bypass 후 후속 audit issue 자동 생성 (post-bypass audit, CFP-100 Phase 4 에서 detail)
...
consumer `.claude/settings.json` 에 OS 분기 등록 (또는 wrapper 가 `$OSTYPE` / `$env:OS` 분기). 단위 테스트 양 platform CI 의무 (CFP-97 task).

### 결정 5: consumer-guide.md 가 SSOT

Consumer 절차 SSOT = `docs/consumer-guide.md`. 본 ADR 은 결정만 freeze, 절차/명령어 SSOT 는 consumer-guide. CFP-100 Phase 4 에서 consumer-guide §X "CI terminal state classification" + §Y "bootstrap protocol" 추가.

## 결과

- mctrader-hub Phase 6 verify (CFP-102, #204) 후 protocol 정합 확인
...
- 6 child Story: CFP-97~102 (`codeforge-internal-docs/wrapper/stories/CFP-9{7,8,9}.md` + `CFP-10{0,1,2}.md`)
- ADR-013 (dogfood-out): 본 ADR spec/plan 위치 internal-docs override
- ADR-020 (cross-repo Epic): Mode B hub-centralized

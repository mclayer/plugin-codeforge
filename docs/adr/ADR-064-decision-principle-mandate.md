...
**ADR-071 Amendment 1 cross-link (CFP-695, 2026-05-15)**: 본 결정 9 Question quality 3-check 의 시점 (Orchestrator stop-time 발화 직전) 와 직접 연동되는 mechanical version = [ADR-071 §결정 12](ADR-071-orchestrator-user-dialog-convergence.md) Layer 2 self declare strict subschema (3 항목 표 형식 fix — `사용자가 답해야 할 것` / `묻기 직전 derived default 시도 여부` / `가치 판단 vs 사실 판단`). Question quality 3-check 결과를 표 schema 의 2번째 항목 (`done` / `skipped` / `value-judgment` enum) 에 visible 하게 노출. PR-time mechanical lint = ADR-071 §결정 13 Phase 2 carrier `dialog-declare-schema` (warning tier, advisory only).

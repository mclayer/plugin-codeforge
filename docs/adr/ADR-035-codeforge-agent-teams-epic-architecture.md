...
## CFP-135 Foundation Story — change-plan + 6 SubAgent 면제 근거
...
또한 CFP-135 = **doc-only Story** (Phase 2 implementation 부재, ADR-027 Amendment 1 정합) — §7 (보안 설계) / §11 (데이터 마이그레이션) / §13 (Live Operational Discipline) 모두 N/A. 따라서 **6 SubAgent gathering skip 정합** (SubAgent mandate 매트릭스 의 active row 0). CodebaseMapper / Refactor / SecurityArch / OpRiskArch / TestContractArch / DataMigrationArch 6 SubAgent 모두 mandate 영역 (코드 경로 매핑 / refactor 영역 / trust boundary / DR / Test Contract / migration) 본 Story 에서 비활성. ArchitectAgent (chief author) 가 직접 본 ADR 작성 + Stage 0 spec verbatim 보존 invariant 우선 — Wave 2 (CFP-137 worktree 인프라) / Wave 3 (CFP-139 GitOpsAgent) 진입 시 SubAgent gathering 재개 trigger (§7 / §11 / §13 sub 활성).

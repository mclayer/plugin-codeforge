...
|---|---|---|---|
| 1 | 사용자 dialog | `AskUserQuestion` / 확답 step / 정보 요청 답변 (Yes/No / 옵션 선택) | Subagent one-shot 이라 continuous dialog 자체 mechanism 불가능 (ADR-009 §결정 + CLAUDE.md "플랫폼 제약") |
| 2 | TodoWrite scratchpad | progress visualization marker write (ADR-038) | TodoWrite = file write 아님, Orchestrator scratchpad / meta progress channel — 수정 작업 enumeration 미포함 |
...
### 결정 5 — Lane plugin / 6 deputy / inter-plugin contract = 0 변경
...
- **ADR-031** (lane-spawn evidence trail) — §14 row append ownership 무변, mechanism 만 spawn (§결정 3).
...
- **ADR-038** (progress visualization TodoWrite) — TodoWrite = Orchestrator scratchpad / meta progress channel. 본 ADR 의 §결정 2 Inline whitelist entry 2 source.
...
- `docs/orchestrator-playbook.md` §3.0 (신설)
- `CLAUDE.md` "오케스트레이션 규칙" § "Default subagent context (수정 작업)"
- `docs/consumer-guide.md` § "Subagent default (codeforge orchestration)"
- `docs/hotfix-playbook.md` (1줄 ADR-039 cross-ref)
- `docs/domain-knowledge/orchestrator-discipline/spawn-default.md`
- `docs/change-plans/cfp-275-orchestrator-subagent-default.md`

...
wrapper Story 가 sibling repo (lane plugin) 에 mechanical mirror sync PR 을 open 시, sibling PR 은 자체 설계리뷰 evidence 가 없다. wrapper Story 의 설계리뷰 PASS 결과는 wrapper PR 에만 기재 (comment + label) 되어 sibling PR 의 `phase-gate-mergeable` workflow 가 인식하지 못한다.

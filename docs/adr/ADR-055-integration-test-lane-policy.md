...
amendments:
  - by: "CFP-992"
    date: "2026-05-18"
    scope: "Amendment 3 §결정 10 신설 — Amendment 2 §결정 3 v2 item 7/8 (Story Suite 자동생성 + Baseline 자동승격) layout 표준의 pytest `--import-mode=importlib` 의존 명세. MCT-202 동일 basename 3곳 rootdir module collision 영구 해소."
...
- 본 Story-3 = baseline v1 (declarative-only, story_keys [CFP-906, CFP-932, CFP-954] cross-Story consistency check 3 entry) — Story-4 = v2 (promotion criteria 4-tuple executable) / Story-5 = v3 (downgrade asymmetry invariant + Wave 4 sub-Epic close final pin) incremental promotion
- `docs/doc-locations.yaml` 15번째 entry `integration_test_baseline` 신설 (ADR-041 §결정 1 정합) — location SSOT (`tests/integration/stories/<EPIC-KEY>/` glob pattern + frozen-SHA pin discipline)
...
- baseline yaml self-hash invariant + append-only stratified pin pattern (Story-4 carrier — OpRiskArch §G.4 권고)
=======
## Amendment 3 (CFP-992, 2026-05-18) — Baseline 자동승격 layout `--import-mode=importlib` 의존 명세

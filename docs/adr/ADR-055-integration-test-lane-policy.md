...
amendments:
  - by: "CFP-992"
    date: "2026-05-18"
    scope: "Amendment 4 §결정 10 신설 — Amendment 2 §결정 3 v2 item 7/8 (Story Suite 자동생성 + Baseline 자동승격) layout 표준의 pytest `--import-mode=importlib` 의존 명세. MCT-202 동일 basename 3곳 rootdir module collision 영구 해소. (CFP-954 Amendment 3 와 번호 충돌 회피 — Amendment 4 재번호)"
    sunset_justification: "N/A — ADR-055 = permanent policy (is_transitional: false, ## 해소 기준 N/A — permanent policy). sunset 미충족 사유 부재 — 본 amendment 는 기존 baseline 자동승격 layout 표준의 전제 조건 (importlib import mode) 명문화 (CFP-822 N=3 MCT-202 instance, basename collision 재발 방지)."
---
...
2. **원인-결과 분리 불가**: Story별도 실행 시 어느 Story의 변경이 기존 서비스를 깨뜨렸는지 파악하기 위해 수동 bisect 필요. Epic 완료 후 1회 실행하면 이 정보가 구조적으로 확보된다.
...
### Amendment 3 — Epic-level baseline first activation (CFP-954, 2026-05-18)

**carrier_story**: CFP-954 (Wave 4 sub-Epic #882 Story-3 — ProductionEvidenceDeputy mandate first activation + IntegrationTestAgent Epic-level reactivation).

**핵심 변경**:
- Epic-level integration test baseline 자동 승격 첫 사례 — `tests/integration/stories/<EPIC-KEY>/baseline-v<N>-<carrier-key>.yaml` immutable append-only naming convention (DataMigrationArch §G.5)
- 본 Story-3 = baseline v1 (declarative-only, story_keys [CFP-906, CFP-932, CFP-954] cross-Story consistency check 3 entry) — Story-4 = v2 (promotion criteria 4-tuple executable) / Story-5 = v3 (downgrade asymmetry invariant + Wave 4 sub-Epic close 최종 고정) incremental promotion
- `docs/doc-locations.yaml` 15번째 entry `integration_test_baseline` 신설 (ADR-041 §결정 1 정합) — location SSOT (`tests/integration/stories/<EPIC-KEY>/` glob pattern + frozen-SHA 고정 규율)
- Story-level vs Epic-level disjoint axis — Story-level integration test (`tests/integration/<story-key>/`) ≠ Epic-level baseline (`tests/integration/stories/<EPIC-KEY>/`). 양 layer 동시 존재 가능 (Story-level scope = component boundary / Epic-level scope = cross-Story consistency check)
...
**Amendment 3 미결 사항 (후속 CFP)**:
- baseline v2/v3 promotion script (Story-4 carrier, executable baseline 진입)
- baseline yaml self-hash invariant + append-only 계층 고정 패턴 (Story-4 carrier — OpRiskArch §G.4 권고)

## Amendment 4 (CFP-992, 2026-05-18) — Baseline 자동승격 layout `--import-mode=importlib` 의존 명세

### 배경

Amendment 2 §결정 3 v2 item 7 (Story Suite 자동생성: `tests/integration/stories/<EPIC-KEY>/<STORY-KEY>/`) + item 8 (Baseline 자동승격: `tests/integration/baseline/` merge) 의 baseline 승격 layout 표준은 pytest module import 메커니즘 의존성을 명시하지 않았다.

MCT-202 에서 IntegrationTestAgent baseline 승격이 `test_eager_cleanup_cascade.py` 동일 basename 을 3곳 (원본 `tests/integration/test_eager_cleanup_cascade.py` + baseline `tests/integration/baseline/MCT-202/test_eager_cleanup_cascade.py` + Story Suite `tests/integration/stories/EPIC/MCT-202/test_eager_cleanup_cascade.py`) 에 동시 존재시켰다. pytest default `prepend` import mode 는 rootdir 기준 module name 을 `__init__.py` 부재 시 basename 만으로 산출 → 3 동일 basename 이 `import file mismatch` (pytest 양 OS exit 2) 충돌을 일으켰다. Amendment 2 의 자동승격 layout (item 7/8) 이 동일 basename 다중 dir 배치를 구조적으로 유발하므로, baseline 승격 Story 재발 시 동일 충돌이 반복된다.

근본 해소: `pyproject.toml` `[tool.pytest.ini_options] addopts = "--import-mode=importlib"`. importlib mode 는 sys.path 조작 없이 file path 기반 unique module 생성 — 동일 basename 다중 dir 양립.

### §결정 10 신설 — baseline 자동승격 layout 의 importlib 의존 명세

Amendment 2 §결정 3 v2 item 7 (Story Suite 자동생성) + item 8 (Baseline 자동승격) 의 layout 표준은 다음 import 메커니즘 전제 위에서만 충돌-free:

- **consumer repo `pyproject.toml` 의무**: `[tool.pytest.ini_options]` 에 `addopts` 가 `--import-mode=importlib` 포함. IntegrationTestAgent 가 baseline 승격 / Story Suite 자동생성 수행 전, consumer `pyproject.toml` 의 본 설정 부재 시 = `infra_setup` failure_type (§결정 9, FIX 담당 InfraEngineerAgent, ArchitectPL 판정 불필요).
- **layout invariant**: item 7/8 의 3-tier dir (`tests/integration/` 원본 + `tests/integration/baseline/<EPIC-KEY>/` + `tests/integration/stories/<EPIC-KEY>/<STORY-KEY>/`) 가 동일 basename 을 의도적으로 다중 배치하므로, `--import-mode=importlib` (또는 동등 file-path-unique module 생성) 가 baseline 자동승격 layout 표준의 **전제 조건**. default `prepend` / `append` mode 는 동일 basename 다중 dir 에서 `import file mismatch` (양 OS pytest exit 2) — baseline 승격 차단.
- **검증 시점**: IntegrationTestAgent deployability 검증 (Amendment 2 §결정 3 v2 item 6) 4단계 직전, `pyproject.toml` import-mode 설정 존재 확인을 0단계로 추가 (consumer overlay `integration_test.required_pytest_import_mode: importlib` 기반 — 미설정 = `infra_setup` FIX).

### Amendment 3 미결 사항 (후속 CFP)

- **codeforge-test plugin**: IntegrationTestAgent mandate 에 baseline 승격 전 `pyproject.toml --import-mode=importlib` 0단계 검증 추가 — 별도 CFP
- **consumer overlay schema**: `integration_test.required_pytest_import_mode` 키 추가 (consumer overlay SSOT) — 별도 CFP

...
> verified-via: `git -C <worktree> log --all --oneline -- 'docs/adr/ADR-*.md' | grep -i atlassian` (worktree HEAD `fee741e`) — exit 0, 3 매치 (`9ccb64d` ADR-099 신설 #1150 / `402ae0d` ADR-RESERVATION 예약 / `3dc9084` CFP-29 check-no-atlassian allowlist chore). **3 매치 모두 제거 predecessor 의 carrier ADR 아님** (ADR-099 = 본 Epic-A lint 역전 본체 / RESERVATION = 번호 예약 / CFP-29 = allowlist chore commit, ADR 본문 무관 — 커밋 메시지 "atlassian allowlist" 가 grep 매칭). predecessor v0.7→v0.8 Atlassian 제거의 formal ADR carrier 0건 재확인 (ADR-099 §결정 4-A 결론 정합, DesignReviewPL F-DR-1102-1 evidence precision 정정).

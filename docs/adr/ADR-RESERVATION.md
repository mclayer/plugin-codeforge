...
| 97 | CFP-1111-W1-S2 | reserved | 2026-05-21 KST (GitOpsAgent sequential append per ADR-RESERVATION write boundary — paradigm replacement governance anchor SSOT, ADR-064 §결정 1 CFP scope unitary 면제 carrier. CFP-1111 Wave 1 Story-2 carrier (7-slot bundle 6/7). codeforge 의 paradigm-level replacement (declarative → imperative full-shift 등) governance anchor — ADR-064 §결정 1 "한 CFP 안 경량→full 단계 채택 금지" 의 면제 영역 정의 + paradigm replacement scope boundary + multi-CFP sequential coordination normative SSOT codify. ADR-064 Amendment N (면제 channel 신설) sibling carrier. parent_epic CFP-1111. status `reserved → active` 전환 = ArchitectAgent commit time (ADR-083 row 83 precedent 정합 — chief author scope). ADR file = `ADR-097-paradigm-replacement-governance-anchor.md` 예정.) |

...
| 92 | CFP-1111-W1-S2 | reserved | 2026-05-21 KST (GitOpsAgent sequential append per ADR-RESERVATION write boundary — changelog SSOT location anchor. CFP-1111 Wave 1 Story-2 carrier (7-slot bundle 1/7). codeforge family 7 plugin × consumer overlay 의 changelog SSOT location / generation ownership / drift detection lint normative SSOT codify. ADR-095 (9 ADR sunset metric 표준화) sister within W1-S2 (changelog metric mining 의존). parent_epic CFP-1111. status `reserved → active` 전환 = ArchitectAgent commit time (ADR-083 row 83 precedent 정합 — chief author scope). ADR file = `ADR-092-changelog-ssot-location.md` 예정.) |

| 93 | CFP-1111-W1-S2 | reserved | 2026-05-21 KST (GitOpsAgent sequential append per ADR-RESERVATION write boundary — 4-field 완료 보고 schema (walker walk_result + 4-field) SSOT. CFP-1111 Wave 1 Story-2 carrier (7-slot bundle 2/7). 완료 보고 walker walk_result + 4-field schema (touched_files / atomic_invariants / verify_via / lane_outcomes) normative SSOT codify. ADR-094 (Fallback 정책) sister within W1-S2 (구형 consumer 보고 schema 호환 의존). parent_epic CFP-1111. status `reserved → active` 전환 = ArchitectAgent commit time (ADR-083 row 83 precedent 정합 — chief author scope). ADR file = `ADR-093-completion-report-4field-schema.md` 예정.) |

| 94 | CFP-1111-W1-S2 | reserved | 2026-05-21 KST (GitOpsAgent sequential append per ADR-RESERVATION write boundary — Fallback 정책 (consumer 구형 버전 유지 호환 범위) SSOT. CFP-1111 Wave 1 Story-2 carrier (7-slot bundle 3/7). consumer 구형 버전 (min_prerequisite_version 미만) 유지 시 wrapper fallback 정책 / 호환 범위 / degraded mode normative SSOT codify. ADR-096 (min_prerequisite_version manifest schema) sister within W1-S2 (manifest schema 의존). parent_epic CFP-1111. status `reserved → active` 전환 = ArchitectAgent commit time (ADR-083 row 83 precedent 정합 — chief author scope). ADR file = `ADR-094-consumer-legacy-version-fallback-policy.md` 예정.) |

| 95 | CFP-1111-W1-S2 | reserved | 2026-05-21 KST (GitOpsAgent sequential append per ADR-RESERVATION write boundary — 9 ADR sunset metric 표준화 SSOT. CFP-1111 Wave 1 Story-2 carrier (7-slot bundle 4/7). ADR-058 §결정 5 sunset_justification 3-tuple (metric / who / how) 의 metric 영역 표준화 — 9 ADR (현존 is_transitional=true) sunset metric 형식 통일 / 집계 dashboard / cron 자동 측정 normative SSOT codify. ADR-092 (changelog SSOT) cross-ref (metric mining source). parent_epic CFP-1111. status `reserved → active` 전환 = ArchitectAgent commit time (ADR-083 row 83 precedent 정합 — chief author scope). ADR file = `ADR-095-sunset-metric-standardization.md` 예정.) |

| 96 | CFP-1111-W1-S2 | reserved | 2026-05-21 KST (GitOpsAgent sequential append per ADR-RESERVATION write boundary — min_prerequisite_version manifest schema SSOT. CFP-1111 Wave 1 Story-2 carrier (7-slot bundle 5/7). consumer overlay `.claude/_overlay/project.yaml` + plugin `plugin.json` 의 min_prerequisite_version manifest schema field 신설 / semver 호환 enforce / mismatch lint normative SSOT codify. ADR-094 (Fallback 정책) sister within W1-S2 (Fallback trigger 의존). parent_epic CFP-1111. status `reserved → active` 전환 = ArchitectAgent commit time (ADR-083 row 83 precedent 정합 — chief author scope). ADR file = `ADR-096-min-prerequisite-version-manifest-schema.md` 예정.) |
...
| 98 | CFP-1111-W1-S2 | reserved | 2026-05-21 KST (GitOpsAgent sequential append per ADR-RESERVATION write boundary — UpgradeAgent runtime ownership SSOT (PMO 흡수 vs 신규 codeforge-upgrade lane plugin). CFP-1111 Wave 1 Story-2 carrier (7-slot bundle 7/7). CFP-703 Wave 2 Story-3 의 UpgradeAgent runtime — codeforge-pmo lane 흡수 vs 신규 codeforge-upgrade lane plugin 신설 의 decision framework + lane lifecycle invariant (ADR-023) + ownership boundary normative SSOT codify. ADR-076 (declarative reconciliation upgrade flow) cross-ref (UpgradeAgent runtime SSOT carrier). parent_epic CFP-1111. status `reserved → active` 전환 = ArchitectAgent commit time (ADR-083 row 83 precedent 정합 — chief author scope). ADR file = `ADR-098-upgrade-agent-runtime-ownership.md` 예정.) |

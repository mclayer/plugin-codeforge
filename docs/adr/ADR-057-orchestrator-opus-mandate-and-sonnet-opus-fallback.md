...
amendment_log:
  ...
  - by: "CFP-448"
    date: "2026-05-12"
    scope: "§결정 3 selective rollback (Amendment 3) — 6 agent 중 ChangeImpactAgent / CodebaseMapperAgent / RefactorAgent 3종 Opus → Sonnet 복귀, FeasibilityAgent / ContinuityAgent / DeveloperPLAgent 3종 Opus 유지. §결정 3 표 갱신 + sunset gate 2 measurement contract 분모 5종 → 8종 갱신 (ADR-057 §결정 3 = SSOT, CLAUDE.md L127 = mirror reference). 결정 1·2 본문 변경 0건."
    sunset_justification: "§결정 3 일부 revert 는 ratchet anti-pattern 이 아니라 axis-A (operational cost trade-off) + axis-B (ADR-042 §결정 2 invariant 정합) + axis-C (CL-6 사용자 확정 SSOT direction) 3축 evidence 기반 selective re-evaluation. CFP-379 → CFP-448 (2026-05-11 → 2026-05-12) 운영 evidence: (1) Codex review finding (CFP-379) 의 mandate 깊이 부족 우려는 CodebaseMapper / Refactor 2종 single-mandate advocacy agent 에서 mandate text 재정의 (description / 본문 role 정의 강화) 산출물 동시 의무 발화로 해소 — 단순 model field downgrade 금지. (2) ChangeImpactAgent 는 read+map structured output (no synthesis) = Sonnet sufficient 정합 (LangGraph contributor pattern). (3) Opus 유지 3종 (Feasibility / Continuity / DeveloperPL) 은 architecture 해석 / cross-Story pattern detection / supervisor synthesis (LangGraph supervisor pattern) — Sonnet cover 불가 영역으로 ADR-042 §결정 1 (e)/(f)/(a) tier criteria 정합. (4) sunset gate 1 ('Sonnet 잔류 agent 0건') 거리 증가 (5종 → 8종) 는 reverse direction 으로 보이나, 본 ADR 의 의도 (Orchestrator 차단 위험 구조적 해결) 는 결정 1 (Opus mandate) 으로 이미 달성 — gate 1 자체는 향후 Sonnet 모두 폐지될 때 발화하는 idealized end-state. 본 Amendment 3 의 selective rollback 은 operational reality (Opus token cost vs reasoning 품질 trade-off) 와 governance reality (ADR-042 §결정 2 invariant role 재정의 시그널) 의 균형 → ratchet 회피 evidence. (5) ADR-058 §결정 5 self-application 두 번째 사례 (Amendment 2 첫 사례 후) — Amendment 시 sunset_justification 의무 정합."
related_stories:
  - CFP-379
  - CFP-392
  - CFP-393
...
### 결정 3: ADR-042 Amendment 4 + Amendment 6 적용 (selective tier 할당 — Amendment 3 갱신)

본 ADR 이 ADR-042 Amendment 4 (carry) + Amendment 6 (CFP-448 Amendment 3 cross-ref) 를 carry. 최종 tier 할당 (**Amendment 3 갱신 후 SSOT** — selective rollback 결과):
...
| DeveloperPLAgent | **Opus 유지** | OPUS (a) supervisor synthesis (LangGraph supervisor pattern) — PL 동적 roster + FIX 1차 진단 |
| ChangeImpactAgent | **Sonnet (Amendment 3 rollback)** | read+map structured output (no synthesis) — Sonnet sufficient (LangGraph contributor pattern). axis-A cost 효율 + axis-B single-mandate 정합 |
...
6. ChangeImpactAgent (codeforge-requirements) — **Amendment 3 신규**
...
| 항목 | 내용 |
|---|---|
| **metric** | 월 50회 이상 Sonnet subagent spawn 발생 환경에서 3개월 연속 rate-limit fallback 발생률 < 1%. 분자 = §14 Lane Evidence `transcript` 필드의 `[rate-limit-fallback:sonnet→opus]` 태그 발화 건수 / 분모 = 월간 Sonnet 잔류 agent **8종 (Amendment 3 후 SSOT — DeveloperAgent · BackendDeveloperAgent · FrontendDeveloperAgent · IntegrationTestAgent · StatefulTestAgent · ChangeImpactAgent · CodebaseMapperAgent · RefactorAgent)** 의 spawn row 카운트. 측정 단위 = calendar month UTC (half-open `[month-N-start, month-N+1-start)`). minimum sample size gate (월 < 50 spawn 환경 = `sample_size_sufficient: false`, gate 발화 보류 — zero division 회피 + 거짓 PASS/FAIL 신호 차단). **Amendment 3 (CFP-448) 효과: 분모 5종 → 8종 확대 → KPI sample size 자연 회복 (CFP-393 §11 follow-up #1 mitigation)**. |
...
1. **§결정 3 표 갱신 — 3 agent Opus → Sonnet 복귀** (위 § "결정 3" 표 SSOT — 이미 본 Amendment 에 의해 갱신됨):
   - **Sonnet rollback (3)**: ChangeImpactAgent · CodebaseMapperAgent · RefactorAgent
   - **Opus 유지 (3)**: FeasibilityAgent · ContinuityAgent · DeveloperPLAgent
...
|---|---|---|---|---|---|---|
| ChangeImpactAgent | Sonnet sufficient (no synthesis) | single-mandate (AS-IS→DELTA 매핑) | mirror 갱신 후보 | finding 약함 | contributor tier | **Sonnet rollback** |
...
| DeveloperPLAgent | Opus tier (a) supervisor | supervisor synthesis | SSOT 정합 (Opus) | FIX 1차 진단 품질 finding 강함 | supervisor pattern (LangGraph) | **Opus 유지** |

**EC-9 tie-break 적용** (axis-A vs axis-B 충돌, CodebaseMapper + Refactor): axis-A 1차 우선 → rollback PASS + axis-B conditional constraint = mandate text 재정의 산출물 동시 의무 (EC-5 정합).
...
2. **gate 1 ("Sonnet 잔류 0건") 자체는 idealized end-state** — 모든 agent role 이 Opus 가 필요할 정도로 진화한 시점. 본 Amendment 3 은 그 종점이 아직 멀다는 운영 evidence (Sonnet sufficient role 존재 — ADR-042 §결정 2 invariant 정합) 를 반영.
...
4. **CFP-379 → CFP-448 운영 evidence** [HYPOTHESIS — 양적 evidence 부재, axis-A 운영 결정 근거]: Opus 6 agent 1 주일 운영 후 P0/P1 결함 카운트 0 + token cost overhead 누적 → Sonnet rollback 의 trade-off 정당화. quantitative monitoring 은 Amendment 2 KPI dashboard 가 자동 수집 (분모 8종 확대 후 sample size 자연 회복).
5. **ADR-042 Amendment 6 cross-ref** (Amendment 3 atomic): agent model tier 정책 SSOT 가 본 Amendment 3 과 동시 갱신 — drift 차단 (ADR-063 atomic invariant 정합 cross-plugin scope, marketplace single sync ordering).
...
### Cross-ref (ADR-042 Amendment 6)

본 Amendment 3 = ADR-042 Amendment 6 동시 발의 (CFP-448 atomic). cross-ref:
- 본 ADR §결정 3 표 = SSOT (Sonnet 잔류 명단)
- ADR-042 §결정 1 표 + Amendment 6 row = SSOT (각 agent tier criteria 정합)
...
- `plugin-codeforge-requirements/agents/ChangeImpactAgent.md` — model: claude-sonnet-4-6 (**Amendment 3 rollback**)

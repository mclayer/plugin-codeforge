...
**§결정 9 — Stop-time 300자 평문 정리**:

Orchestrator 가 사용자 dialog turn 종료 시 300자 ± 50자 평문 정리 step 의무.
...
- behavioral directive only — mechanical enforce 미시도 (turn-final hook 부재)
- 적용 범위: wrapper + 모든 consumer (CLAUDE.md L208 normative 정합)

**강제 강도**: behavioral directive only (ADR-039 §결정 2 Inline whitelist 첫 번째 entry "사용자 dialog" 의 평문 정리 step 부착 정합). mechanical enforce 미시도 — retro audit signal (PMOAgent retro file §wording-discipline 표) + sunset gate metric (frustration 발화 0건/3 Story 누적).
...
**trivial turn 정의 3 criteria AND**: (1) 응답 ≤ 1 줄 + (2) 의문 부재 + (3) 결정 부재. 3 criteria 모두 충족 시 Layer 1 + Layer 2 면제. Layer 3 / Layer 4 는 trivial turn 에서도 active.

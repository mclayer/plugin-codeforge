...
**0. 필수 의존성 확인 (모든 작업 선행 · 의무)**
   ...
   **0a-prime. worktree 정리 — eager 완료 시점 (primary) + 주기 backstop (CFP-427 / ADR-040 §결정 5)**
   ...
   - **완료-게이트 (phase:완료 worktree-clean self-check)**: 정상 완료 경로의 eager 누락 검출 (backstop 과 disjoint — 완료 worktree 는 0일령). `STORY_KEY=cfp-NNN bash scripts/check-worktree-completion-clean.sh` — 완료 처리 중 Story 의 sub-worktree(`cfp-NNN/lane/*`·`cfp-NNN/fix-iter-*`) 잔존 즉시 검출 + Story root(`cfp-NNN` flat)는 Phase 2 PR mergedAt non-null 일 때만 검출(open=보존). `phase:완료` transition precondition (§9.7.1) 의 (b) 로컬 check (ADR-040 Amendment 9 §결정 7.K / ADR-045 Amendment 13 §D-12). warning-tier advisory (required CI 불가 — worktree 클라우드 러너 미접근. **정의역 = 클라우드 러너 평면**이며, 세션 독립 로컬 스케줄 작업은 그 정의역 밖이지만 도입기 관측·보고까지라 required 승격 근거가 아니다 — [ADR-172](../archive/adr/ADR-172-local-scheduled-task-residue-observation.md) §결정 7·9).

   > 과거 SessionStart hook 동기/주기 호출은 **제거됨** — worktree 90+ 동기 스캔으로 세션 시작 지연. SessionStart async:true 는 무시되어 동기 실행 = 지연 회귀라 배제 (요구사항리뷰 PASS 확정 외부사실, hooks.md "SessionStart and Setup hooks do not support async"). 또 GitHub post-merge automation 은 클라우드 러너라 로컬 worktree 미접근 → **클라우드 러너 평면에서는** 로컬 세션 종료 시점(SessionEnd)이 backstop 의 deterministic 자동 트리거 지점이다. **"유일한" 은 더 이상 성립하지 않는다** — SessionStart detached lazy GC(ADR-169 §결정 4)와 **세션 독립 로컬 스케줄 작업**([ADR-172](../archive/adr/ADR-172-local-scheduled-task-residue-observation.md))이 추가 자동 트리거 축으로 존재한다. 단 후자는 도입기 **관측·보고까지**이며 삭제·집행 0 이라 backstop GC 를 대체하지 않는다. (문서-실배선 drift 복원 SSOT — 2026-06-12 stale worktree 230개 누적이 동인.)

   **0a-prime-2. 비대화형(사람 없는) 세션의 잔재 관측 호출 계약 (CFP-2949 / [ADR-172](../archive/adr/ADR-172-local-scheduled-task-residue-observation.md))**

   잔재 관측 스크립트를 **사람 없는 세션**(로컬 스케줄 작업 등)이 호출할 때 준수하는 규약. 대화형 세션의 §0a-prime / §3.5 절차를 대체하지 않는다 — 그 절차 위에 얹히는 호출 계약이다.

   | 항목 | 계약 |
   |---|---|
   | 작업 디렉터리 | **repo 절대경로로 `cd` 를 프롬프트 안에서 명시 고정**한다. 전 스크립트가 `git rev-parse --show-toplevel` 기반 cwd 의존이며 스케줄 작업의 folder 설정은 git 밖 등록 상태라 검증 표면이 없다 ⇒ **folder 설정에 위임 금지** |
   | 관측-only 강제 | `mode` = observe-only ∧ `GC_DRY_RUN=1` ∧ `TEMP_GC_DELETE_ENABLED` **unset**. **신규 플래그 0건** — 기존 3중 게이트를 그대로 재사용한다 |
   | exit code | `[residue-scan]` 계열은 **always exit 0** (advisory) ⇒ **exit code 를 성공·실패 오라클로 쓰지 않는다** |
   | 호출 실패 시 | 정본 미도달 · 스크립트 부재 · `cd` 실패는 **정직 중단 + 사실 보고**. 부분 성공을 전체 성공처럼 렌더하지 않는다 |
   | 출력 판독 | 하위 스크립트 출력 중 **verdict 어휘(`PASS`/`FAIL`/`OK`/`정상`/`문제없음`)가 포함된 줄은 인용 금지**, `warn=N` 같은 **수치 필드만 재서술**한다. 보고 본문은 `선언값 · 실측값 · 불일치` **사실 3-tuple** 로만 구성한다. 실측 오염원 1건 = `scripts/check-capture-gate-completion.sh` 가 `[capture-gate] PASS:` 를 직접 발화 |
   | 발화 주체 | 보고 채널 조회·발화는 **`scripts/lib/scheduled_task_reconcile.py` 프로세스 안에서만** 일어난다. LLM 세션은 채널에 쓰지 않는다 (ADR-172 §결정 5) |

   > **정직 라벨**: 본 계약은 **문서 = advisory** 다. 기계 강제는 스캐너 3중 게이트와 `permissions.deny` 축에만 있다 (ADR-172 §결정 4 · 결과 8).
...
- **raw 분석용 READ 축적** = read-worker offload default (ADR-142 §결정 1 L1 — advisory 승격, hard block 아님) — 단 trivial-read 6종(§3.0.2a CLOSED)은 carve-out 으로 spawn 강제 아님. 구 "trivial Read 1건도 spawn 의무"(CFP-275 AC-3 trivial-threshold-zero)는 hollow-gate(무조건 offload = spawn 고정비 순손실)로 판정돼 본 carve-out 으로 대체 (ADR-142 §결정 2)

#### §3.0.2a Trivial-read carve-out (CLOSED enumeration — ADR-142 §결정 2)

장수명 Orchestrator 의 raw 분석용 READ 축적은 read-worker subagent 에 offload 후 high-signal 요약만 수신함을 **default 경로로 승격**(§3.3 정합 · advisory prompt-mandate — per-read intent 감지 infeasible 이므로 hard deny 아님, ADR-039 §결정 9 ceiling). 단 아래 6종 trivial-read 는 offload 강제 대상이 아니며 Orchestrator 직접 `Read`/`Glob`/`Grep` 유지 (무조건 offload = spawn 고정비 순손실 = hollow-gate):

1. read-only Q&A 답변용 read (ADR-039 §결정 2 entry3)
2. status/progress report 합성용 read (entry4)
3. Story §9/§10/§14/phase inline write 직전 확인 read (§결정 15 entry5)
4. merge-time Codex dispatch 판정 read (entry6)
5. 단일 파일 1회성 사실 확인 (값 1개, N+ 턴 미잔존)
6. lane spawn 직전 packet 구성용 발췌 read

**위임 판단 기준** = read 결과가 holder N+ 잔여 턴 잔존 여부 (read-count 아님) — ADR-044 §결정 11(c) 상속. 위 목록 추가 = ADR-142 amendment 의무 (open-ended carve-out = hollow-gate anti-pattern).
...
#### §3.0.12b fable-리밋 opus failover (ADR-141 Amendment 6 / ADR-109 §결정1 Amendment 1)

fable 배정 subagent(Amendment 4 carve-out 10 역할 — 6 lane PL + ArchitectAgent + ResearcherAgent + PMOAgent + IntegrationTestAgent)를 spawn 한 결과가 **리밋 계열 신호**를 동반해 실패하면 Orchestrator(ADR-039 spawn monopoly — lane PL 자가-재spawn 불가)가 opus 로 fresh re-spawn 1회 수행. §3.0.12 "fallback tier 없음"(sonnet 축 rate-limit fallback machinery dead)은 **무변경** — 본 절은 fable-리밋 한정 예외 failover 만 추가한다("fallback"[sonnet 축, dead] ≠ "failover"[fable-리밋, 신규] — 별개 trigger·별개 SSOT·별개 태그).
...
- **미분류 오류(감지집합 미매칭)** = failover 미발동 → **task-failure 분류**(silent fallback 금지 — model-unavailable/floor-fail/로직 오류를 opus 재spawn 으로 은폐 차단, ADR-057 §결정4 동형). **경계(AC-5)**: session/usage-limit 계열(`session limit`/`usage limit`)은 감지집합 **IN-SET** 이라 '미분류' 아님 = AC-1 failover 대상(task-failure 오분류 대상 아님). 529(`529`/`overloaded`)은 감지집합 NOT-IN → ADR-109 §결정6 cooldown 경로(pool-agnostic overload).
- **Option A 즉시전환** = fable same-model 재시도 SKIP. ADR-109 §결정3 sequence 에서 **step1(fable same-model exp-backoff soak) bypass → step2(fable→opus substitution, 구 ADR-057 §결정2 dead slot re-tenant) 즉시 직행**. 근거 3층 = reset long-horizon(실관측 `resets 10:20pm` ≫ backoff budget) / fable·opus 별개 pool / Retry-After trap (ADR-141 A6-2). Option B(fable 1회 재시도 후 전환) 기각.
- **1-hop / fresh-spawn-only** = 새 `Agent` spawn + `model: opus` override(동일 입력 패킷). **SendMessage resume 금지**(원본 frontmatter `model: fable` resume 재해석 재실패 = CFP-2236 root cause). **per-spawn 1회** 독립 카운터(재진입/FIX 재spawn 시 시도마다 리셋 — Story 누적 아님, 무한 failover 차단).
- **비대상 3종**(failover 미발동, AC-3) = (a) Orchestrator 세션 자체 리밋(launch 고정 → 기존 대기/수동 handoff, ADR-110 축) (b) refusal(`stop_reason: refusal` — 수동 opus 재spawn 방어, CFP-2803) (c) 비-fable tier subagent 리밋(haiku 7 / sonnet 10 / opus — 기존 운영 관찰 note 무변경).
...
**dispatch 주체 = 구현리뷰 lane worker (sub-agent) → Codex via Bash (critical — #7 과 다름)**: touchpoint #7(merge-time)은 lane PL 부재라 Orchestrator top-level inline + ADR-039 entry 6 가 필요했으나, touchpoint #8 은 **구현리뷰 lane-time** — CodeReviewPL 이 active 하고, 구현리뷰 lane worker(sub-agent)가 **Bash 로 Codex 를 dispatch** 한다. 재귀 가드 = "subagent → **Agent tool** 호출 금지" 한정이라 Bash 호출은 미발동 → **ADR-039 inline whitelist 신규 entry 불요** (entry 6 merge-time 흡수 아님) — 이 결론은 dispatch 채널 종류와 무관하게 유효. dispatch 자체 = read-only adversarial probe (`codex exec --sandbox read-only`, ADR-081 §결정 D8 무손상). **node 채널 정직 표기 (CFP-2828 reconcile — [ADR-052 §결정 D6.e](../archive/adr/ADR-052-codex-proactive-check-touchpoints.md) SSOT)**: 예시 dispatch `node codex-companion.mjs review` 는 **잔존 dormant scope-out 채널**(§5.5-2 — 본 Story 는 touchpoint #8 을 codex exec 로 이관하지 않음; companion 실물은 wrapper repo 부재)이다. CodexReviewAgent 는 CFP-2828 이후 `codex exec` 직접 dispatch 로 전환하며 `Bash(node *)` 은퇴(frontmatter 실측 = `Bash(codex *)`+`Bash(timeout *)`)라 종전 "CodexReviewAgent 동형 proven channel" 근거는 소멸. 또한 touchpoint #8 은 **현재 미배선 playbook-only 절차**(활성 agent holder 0 — grep empty) — node dispatch 권한 원천은 (배선 시) 해당 lane worker context 이지 CodexReviewAgent frontmatter 아님.
...
- 코드 경로는 Story file §4에 요약. **raw 분석용 구체 내용 READ 는 장수명 Orchestrator 가 직접 축적하지 않고 read-worker subagent 에 offload 후 high-signal 요약만 수신함을 default 경로로 승격** (ADR-142 §결정 1 L1 — advisory prompt-mandate, hard block 아님). **trivial-read carve-out**(§3.0.2a CLOSED 6종: 읽기전용 Q&A·status·Story inline 직전 확인·merge Codex 판정·단일 1회성 사실확인·lane packet 발췌)은 예외로 Orchestrator 직접 `Read`/`Glob`/`Grep` 유지. 위임 판단 기준 = read 결과의 holder N+ 잔여 턴 잔존 여부(read-count 아님, ADR-044 §결정 11(c) 상속)
...
| 3 | sub-agent return 후 (+ 작업 중 상시) | 자기 sub-branch commit → sequential merge: `worktree-merge.sh cfp-NNN/<lane> cfp-NNN/<lane>/<sub1> cfp-NNN/<lane>/<sub2>`. **병기 (additive) — 작업 중 atomic 의미 단위 경계마다의 진행 커밋 (progress-commit, [ADR-178](../archive/adr/ADR-178-subagent-progress-commit-preservation.md))**: return 없는 종료에서도 성립하는 **상시 선행 적재**다 — return-종속 트리거만으로는 강제 종료 시 커밋 트리거가 정의되지 않는다(I-3 공백). 주체 = 산출 주체 워커 자신, 보존 최소 인정 단위 = local git commit(P0), 미완 표식 = 커밋 subject `[CFP-NNN][WIP]`. 신호-무관 상시 규율이며 진행 커밋은 정지 사유가 아니다 (ADR-178 §결정 2·5·7) |
...
**잔재 발견(residue discovery) + scratch TTL (CFP-2822 / ADR-169 세션 잔재 수명 규약 — [ADR-169](../archive/adr/ADR-169-ephemeral-residue-lifecycle.md) §결정 4)**: backstop(§3.5 등록 worktree scoped)과 **disjoint 한 상위집합** 스캔. 등록 worktree 는 backstop 전담 유지 — 아래는 그 사각지대(codeforge-scratch·workspace root `_wt-*`·홈 직하 미등록 git dir·harness Temp·stash) 커버.

- **discovery 스캐너**(`templates/scripts/check-workspace-residue-discovery.sh`, manifest 배포): multi-class walk(등록 worktree cross-check-only / workspace root discover / 홈 직하 git-dir / Temp `observe-only`) → orphan **3축 분류**(등록여부 / git존재 / 상태검사) + stash census + 용량 임계 → aging report(`[residue-scan] DONE: scanned=N flagged=M`, **always exit 0 advisory**, 기존 `[stale-check]` output contract 무접촉). **보존 트리거 = 상태 신호 한정**(dirty/unpushed/locked/worktree-pin/INCONCLUSIVE 1+ 양성 또는 판정 불능) — 등록·존재 여부 자체 ≠ 보존 사유(INV-1 data-loss 가드). **자동 삭제 강제 아님** — 보존항목은 사유+나이 가시화 + 임계 초과 재알림(dedup/backoff), stash 자동삭제 = Non-goal(git 무만료 = 의도적 사용자 데이터, INV-3).
- **scratch TTL purge**(`templates/scripts/check-codeforge-scratch-ttl.sh`, manifest 배포): codeforge-scratch 내부 age>TTL **순수 loose 파일**만 자동 purge — `.git` 보유 항목(clone/worktree/export)은 삭제 제외 → orphan 회부. 상태·메타파일은 scratch **밖** `~/.claude/worktree-gc-state/` 배치(자기소멸 회피).
- **크래시 보완 트리거**: SessionEnd best-effort eager 유지 + **SessionStart detached lazy GC**(fire-and-forget 즉시 return, 실 스캔 분리 프로세스) 다중화 — 트리거 단일화 invariant 를 "멱등/lock 실증 다중 wire" 로 완화(ADR-169 §결정 4 — ADR-040 §결정 5·ADR-128 §결정 3 단일-wire sub-claim scoped-supersede). 2차 트리거 활성화 전 **mkdir 원자 lock + cooldown + 멱등 remove 선행**(E10 double-delete 0). OS 스케줄러(ADR-110) = consumer opt-in 보조(주 트리거 아님).
- **생성위치 표준**: `git worktree add` target 이 `~/.claude/worktrees/<repo>/` 밖이면 위반 — PreToolUse 위치 가드(`WORKTREE_LOCATION_GUARD_TIER=warn|block`, 도입기 warn → 승격기 block) 1차 예방 ⊕ discovery 스캐너 2차 검출(AC-11 미등록 orphan count). "기계적 차단" ≠ 완전봉인(matcher 회피·난독화 잔여, over-claim 금지).
- **bypass env 3종**(disjoint, 각 사용 시 audit 1줄): `BYPASS_WORKSPACE_RESIDUE_SCAN`(discovery) / `BYPASS_CODEFORGE_SCRATCH_TTL`(scratch TTL) / `BYPASS_WORKTREE_LOCATION_GUARD`(위치 가드). Temp 삭제는 별도 축 `TEMP_GC_DELETE_ENABLED`(default-off — 미충족 시 관측·보고만, 제3자 소유 삭제 0 = INV-9). 정책 SSOT = ADR-169, 완료-게이트 wire = §9.7.1/§9.7.2 residue-clean self-check.
...
#### §3.5.2 Cross-repo worktree target authority verify (CFP-1578 / [ADR-168 §결정 1 sub-scope 1-J](../archive/adr/ADR-168-write-time-self-write-verification-mandate.md) (구 ADR-082 Amendment 21, 재제정 CFP-2840))

> **NORMATIVE — ADR-168 §결정 1 layer 1 sub-scope 1-J (구 ADR-082 Amendment 21, 재제정 CFP-2840) declarative anchor**. chief author / lane agent / Orchestrator 가 spawn prompt 작성 또는 직접 file write 직전 cross-repo worktree target authority verify-before-write 의무. mechanical wire (lint script + workflow + hook json sample + bats fixture) = 별 sub-CFP Wave 2 carrier — 본 §3.5.2 = behavioral directive + declarative anchor (Wave 1 declaration-only).
...
**Verify pattern (verify-before-trust 4-layer governance Layer 3 — ADR-168 §결정 1 sub-scope 1-J (구 ADR-082 sub-scope 1-J, 재제정 CFP-2840))**:
...
**Cross-ref**: [ADR-168 §결정 1 sub-scope 1-J](../archive/adr/ADR-168-write-time-self-write-verification-mandate.md) (구 ADR-082 Amendment 21, 재제정 CFP-2840) (declarative anchor SSOT) / [ADR-040](../archive/adr/ADR-040-worktree-convention.md) (namespace 표준) / [ADR-013](../archive/adr/ADR-013-codeforge-family-dogfood-out-policy.md) (dogfood-out path 분리: Story·Change Plan·retro = internal-docs, src·workflow·ADR·CLAUDE.md = wrapper) / evidence-checks-registry `worktree-target-authority-verify` (warning tier) / paired sibling CFP-1559 Amendment 20.
...
1. **codex exec dispatch 도 wall-clock 상한 + 마커 fallback 대상** — 모든 `codex exec` dispatch 발화는 **option-first** `timeout --kill-after=<K> <N>` prefix 로 감싼다 (GNU coreutils 는 duration-first `timeout <N> --kill-after=<K>` 에서 `--kill-after` 를 명령으로 오인 → exit 127 가드 무효; option 은 duration 앞에 필수). exit 124(wall-clock kill) → substitution `fallback_skip_with_marker` + Story §10 marker `[codex-sandbox-fallback: dispatch_stall_or_stream_timeout]`. **fail-mode enum #8 `dispatch_stall_or_stream_timeout` trigger = "codex exec TTY 0-byte" + "codex exec wall-clock 초과" 양쪽** (신규 enum value 신설 0 — closed-set 크기 무변경, 기존 #8 trigger 의미 유지). fail-open 금지 — exit 124 / verdict 미획득 → verdict=inconclusive (PASS 자동 승격 금지, PASS-only-if-explicit).

2. **Orchestrator liveness 게이트 (보조 2차 관측, ADR-039 소유)** — 호출부 `timeout`(1차 process wall-clock 상한)을 보조하는 liveness 게이트 = worker 진행 마커(out.json mtime + content grep) 부재가 max-wait 분 초과 시 stall 판정 → 다음 step 진행. worker 자가-spawn 금지(`plugins/codeforge-review/CLAUDE.md:46` "워커는 직접 다른 subagent 스폰 불가") → 게이트 spawn 주체 = **Orchestrator 고정**.
...
- **tier 정직** — detection `tier: [measurement]`(output-mtime/marker 관측) + recovery `[advisory]`(lead-owned discretionary). **`[물리강제]` 아님** (SubagentStop record-only [ADR-115](../archive/adr/ADR-115-runtime-hook-enforcement.md) + INV-L4 lead 판정). **force-resume 완전자동화 주장 금지** — force-resume 는 lead-owned discretionary 다. **full auto-wake-parent dispatcher(env=1)는 구현 금지** — substrate 부재(env=1) → DEFER-escalate (recurrence anchor `L3-delivery-gap::(a)`, ≥2 Story 재-제안 시 escalate, 자동 followup 발의 안 함).
...
**Orchestrator active-resume 결정론 절차 (CFP-2597 / [ADR-139](../archive/adr/ADR-139-background-wait-liveness-gate.md) INV-L4)**: LEAD 가 background 자식 대기 중 **wait-spin**(진행 marker 갱신 없이 동일 대기 반복)에 빠지면, 추측·ad-hoc 재개가 아니라 **결정론 절차**를 따른다 — (1) INV-L1~L3 로 stall 판정(wall-clock ceiling AND no-progress-marker, 0-byte 단독 ≠ stall), (2) 자식이 남긴 **peer transcript / verdict artifact 를 직독**(direct Read/stat — 자식 self-attestation 이나 memory 추정 불신, INV-L4 대기주체↔판정주체 분리), (3) artifact 실재+non-empty → outcome 확정 수집 / 부재·stall → honest INCONCLUSIVE 표식(INV-L2, PASS 자동승격 금지) 후 re-dispatch(max-retry cap 2) 또는 다음 step 진행. 직독 대상 artifact = review-lane 의 `peer_verdicts[].target`([review-verdict-v4](inter-plugin-contracts/review-verdict-v4.md) §19, verdict file dir 기준 상대) 도 포함 — LEAD 가 그 경로를 stat 해 완료를 반증 가능하게 확정한다. tier = detection `[measurement]` + recovery `[advisory]`(lead-owned discretionary), `[물리강제]` 아님.

**I-6.6 review-lane 이식 (CFP-2597 / [parallel-dispatch-protocol-v1](inter-plugin-contracts/parallel-dispatch-protocol-v1.md) I-6.6)**: parallel-dispatch-protocol-v1 **I-6.6**("batch N+1 의 dispatch 시점 = batch N 의 **all worker_outcomes PASS** 확인 후 — cross-batch state isolation")를 review-lane 에 이식한다 — review lane 이 peer worker 를 batch 로 dispatch 할 때 **batch N 의 양 worker_outcomes 가 PASS(또는 honest INCONCLUSIVE/degrade)로 확정된 뒤에만** batch N+1 을 dispatch 한다. 미확정(blind-wait) 상태에서 다음 batch 를 spawn 하면 batch 간 state leak + delivery-gap 진원이 된다(review-pl-base §3 "종합 발화 precondition" 정합). all worker_outcomes 확정 = §3.10.1 active-resume 결정론 절차(peer transcript 직독)로 판정.
...
> **ADR-082 cross-ref (CFP-776)**: Codex proactive check 의 finding evidence 신뢰는 외부 worker output verify layer (ADR-070). lane agent 가 §9 evidence / corpus enumeration write 시점 source/value verify 누락은 별 disjoint layer ([ADR-168 §결정 2](../archive/adr/ADR-168-write-time-self-write-verification-mandate.md) (구 ADR-082 §결정 2, 재제정 CFP-2840)) — Codex proactive check 와 verify 대상 disjoint (Codex output ↔ lane self-write write-time).
...
| **dispatch 주체** | 구현리뷰 lane worker (sub-agent) → Codex via Bash — **ADR-039 inline whitelist 신규 entry 불요** (재귀 가드 = subagent→Agent tool 한정, Bash 미발동). 예시 `node codex-companion.mjs review` = 잔존 dormant scope-out 채널(§5.5-2); CFP-2828 이후 CodexReviewAgent 는 `codex exec` 직접 전환·`Bash(node *)` 은퇴로 "CodexReviewAgent 동형" 근거 소멸, touchpoint #8 = 미배선 playbook-only 절차(활성 holder 0), node 권한 원천 = 배선 시 lane worker context (SSOT: [ADR-052 §결정 D6.e](../archive/adr/ADR-052-codex-proactive-check-touchpoints.md) / §3.9-ter) |
...
#### 설계 리팩터링 debate dispatch (CFP-2543 / [ADR-138](../archive/adr/ADR-138-design-refactoring-debate-elevation.md))
...
6. **가드 상속**: anti-sycophancy 4종 + convergence_quality_invariant 3-tuple + anchor-recurrence ≥2 + min3/max5 전부 debate-protocol-v1 상속(신규 정의 0). token budget = ADR-059 상속 cap(~50K, 신규 cap 도입 0). EC(Codex 미가용) = §3.13 Wave 4 EC-1 동형(skip + Story §10 row + 사용자 통지 후 정상 진행, ArchitectAgent chief 단독 통합 fallback).

상세 SSOT = ADR-138. dispatch_mode enum SSOT = debate-protocol-v1 v1.4 §2.1.
...
> **ADR-082 cross-ref (CFP-776)**: 본 §3.14 = Orchestrator ↔ user 대화 표현 layer. lane agent §9 evidence / corpus enumeration write-time source/value verify 는 별 disjoint layer ([ADR-168 §결정 1](../archive/adr/ADR-168-write-time-self-write-verification-mandate.md) (구 ADR-082 §결정 1, 재제정 CFP-2840) 4-layer 표) — 사실 verify layer ↔ 대화 표현 layer 분리 (ADR-073 ↔ ADR-071 분리 패턴과 동형). schema 재사용 금지.
...
> **설계 진입 전용 advisory sibling predicate — `user-final-sign-off-resolved` (ADR-159 결정 3 / CFP-2725, Phase 1 advisory)**: **설계 lane 진입 preflight** 시, 위 체크 2(§1-6 채움) 와 **sibling** 으로 `user-final-sign-off-resolved` 를 advisory 확인한다 — 요구사항리뷰 PASS 후 사용자 최종 확정 발화가 Story file 에 verbatim 기록(presence)돼 있는지. ground-truth = 확정 발화 presence (**advisory ceiling** — 단일 사용자 환경에서 author 로 "사용자 행위 vs Orchestrator 행위" 구분 불가라 "확정 기록의 실재" presence 까지만 정직 주장, ADR-159 결정 6). anchor = `phase:요구사항-리뷰 → phase:설계` 전이 경계 (§9.7.1). **Phase 1 = advisory ONLY (기계 fail-close 아님)** — 신규 phase 라벨·gate AND·required context 채택 금지(advisory ceiling 위반, ADR-127 §결정 9.4 bypass-valve 금지 정합 — 사용자 명시 skip(ADR-159 결정 2)은 gate satisfied 조건이지 bypass 아님). 미해소 시 = 확정 요청 relay 후 재확인이지 "생략 후 진행" 아님. 확정 대기 stop = 정당 멈춤(payload>0, ADR-144 A1 / ADR-071 §결정 22 carve-out).
...
### §3.17 Orchestrator-authored Issue body pre-publish verify mandate (CFP-1016 / [ADR-168 §결정 1 sub-scope 1-B](../archive/adr/ADR-168-write-time-self-write-verification-mandate.md) (구 ADR-082 Amendment 2, 재제정 CFP-2840))
...
**verify-before-trust 의무** (Wave 1 behavioral mandate, ADR-168 §결정 1 layer 1 sub-scope 1-B (구 ADR-082 §결정 1 layer 1 sub-scope 1-B, 재제정 CFP-2840)):
...
| lint output verbatim 인용 | lint output 의 source state 자체 verify (lint regex FP 가능성 — citation ≠ assertion 분리, ADR-168 §결정 4 (구 ADR-082 §결정 4, 재제정 CFP-2840)) |
...
**Story-level forcing function** (Wave 1 mechanical, ADR-168 §결정 1 sub-scope 1-B (구 ADR-082 Amendment 2 alternative (a), 재제정 CFP-2840)):
...
- ADR-085 §결정 1 5-layer disjoint 표 (ADR-168 §결정 1 (구 ADR-082 §결정 1, 재제정 CFP-2840) 4-layer 표 verbatim 답습 + 5번째 row Multi-session coordination 신설) — coordination axis disjoint complement.
...
|---|---|---|---|
| `phase:요구사항-리뷰` | `gate:requirements-review-pass` | CFP-2326 / ADR-125 | Phase 1 PR — 요구사항리뷰 lane 진행 중. `phase-gate-mergeable.yml` required.gates 내부 분기(`phase:요구사항-리뷰 → gate:requirements-review-pass`)로 enforce, branch protection contexts 무변경 (ADR-125 §결정 2) |
...
| `phase:요구사항-리뷰` | `phase:요구사항-리뷰` | `phase:요구사항` | `gate:requirements-review-pass` (verdict PASS 시 부착) | RequirementsPL verdict PASS + RequirementsReviewPLAgent spawn 직전 (요구사항 → 요구사항리뷰 lane 전이, ADR-125) | Orchestrator (gate label = codeforge-review self-write) |
| `phase:설계` | `phase:설계` | `phase:요구사항-리뷰` | (`gate:requirements-review-pass` retain) | 요구사항리뷰 PASS + **`user-final-sign-off-resolved` 확정 presence (advisory sibling predicate, §3B.1 preflight — ADR-159 결정 3)** + ArchitectPLAgent spawn 직전 | Orchestrator |
...
- **worktree-clean self-check 미통과 (detected>=1)** = GitOpsAgent eager 정리 누락 → 정리 실행 후 재확인이지 "생략 후 진행" 아님 (ADR-127 정합). 본 self-check 는 transition precondition (label attach 직전)이지 Issue close 차단(reopen)이 아니다 — `retro-mandatory.yml` close-blocking auto-reopen 과 axis disjoint (ADR-045 Amendment 13 §13.C, #772 EC-5 정합). warning-tier 로컬 self-check (required CI 불가 — worktree 클라우드 러너 미접근 [**정의역 = 클라우드 러너 평면** — ADR-172 §결정 7], branch protection required contexts 무변경).
- **residue-clean self-check 미통과 (flagged 미가시화/미정리)** = 완료 Story 잔재(worktree+scratch+stash+orphan)가 aging report 에 미반영 → discovery Story-scoped 리포트 확인·정리 후 재확인이지 "생략 후 진행" 아님. 본 self-check 도 transition precondition (label attach 직전)이지 Issue close 차단이 아니다 — §D-12 worktree-clean 과 **disjoint 축**(worktree-clean=등록 worktree eager 미실행 검출 / residue-clean=잔재 전반 가시화). **자동 삭제 강제 아님** (가시화=INV-3, stash 자동삭제 Non-goal, git 무만료 = 의도적 사용자 데이터). warning-tier 로컬 self-check (required CI 불가 — 잔재 스캔 대상[~/.claude/worktrees·codeforge-scratch·workspace root·Temp] 클라우드 러너 미접근 [**정의역 = 클라우드 러너 평면** — ADR-172 §결정 7], branch protection 8-tuple 무변경). ADR-169 §결정 9.
...
> **완료 단계 = 정식 분류 (no new lane, ADR-128 §결정 1)**. merge 직후 "마무리(wrap-up)" 가 4개 독립 트리거로 흩어져 있다. 본 §9.7.2 = 그 **수렴·종결 단일 SSOT** (갭 B). 단일 자동 수렴 검증기는 **클라우드 평면에서 구조적 불가** (클라우드↔로컬 실행평면 비대칭 — 클라우드 러너가 로컬 worktree 미접근). **정의역 = 클라우드 평면**이며 세션 독립 로컬 스케줄러는 그 정의역 밖이다 ([ADR-172](../archive/adr/ADR-172-local-scheduled-task-residue-observation.md) §결정 7 = ADR-128 §결정 4 정의역 amendment — 약화되는 통제 0, 순수 정의역 명확화). **정의역을 좁힌 것이 새 기계 게이트를 만들어도 된다는 허가는 아니다** — 로컬 축은 도입기 **관측·보고까지**이며 판정·집행 0 이다. 따라서 **절차 명문화 + 기존 텔레메트리 관찰 조합**으로 수렴을 확인한다 (신규 기계 게이트 신설 0).
...
| 4 | Orchestrator `phase:완료` transition | 로컬 | precondition 5-조합 통과 확인 (§9.7.1 — terminal gate + `gate:retro-complete` + worktree-clean self-check + deferred-item-recovery self-check + residue-clean self-check) |

**수렴 확인 절차** (Orchestrator 완료 시점):
- 로컬 트리거 (3·4) = 완료 시점에 Orchestrator 가 한 곳에서 확인 (worktree-clean self-check + deferred-item-recovery self-check + residue-clean self-check + precondition 5-조합).
...
**Epic-close 구현-리팩터링 triage step (CFP-2541 / ADR-137)**: Epic-close 수렴 시점(Epic 하위 전 Story merged)에 **구현-리팩터링 triage** 1회 배치 발동 — 실머지 코드 중복·재사용 anchor 를 debate-protocol-v1 `blanket_refactor`(role_assignment={codex:proponent, claude:opponent}) 로 실측 후 3분기 verdict(now/defer/drop) 판정. **dispatch 주체 = Orchestrator top-level inline**(ADR-039 §결정18 merge-time Codex adversarial 전용, 재귀 가드) — PMOAgent 는 **verdict judge** 로 transcript 수신·판정만(self-spawn 불가, PMOAgent.md L288). verdict 착지: now → 리팩터 Story 정식 10레인 발의(INV-BP §8 강제) / defer → `EPIC-RESULTS-<EPIC_KEY>.md` `## §deferred` 5-column row(source=`triage-defer`) append(회수 강제) / drop → cross-Epic drop-ledger(`docs/refactor-triage/drop-ledger.md`) row + PMOAgent read/count(≥2 → 사용자 escalation). |anchor|=0 = triage skip. 상세 SSOT = ADR-137, verdict judge mandate = PMOAgent.md §4.3.

**recovery self-check 인자 EPIC-RESULTS 등록 (C2 dead-path 폐쇄)**: `phase:완료` precondition 의 deferred-item-recovery self-check(§9.7.1 (d))는 `check-deferred-item-recovery.sh` 를 호출할 때 **retro 파일뿐 아니라 `EPIC-RESULTS-<EPIC_KEY>.md` 도 인자 목록에 반드시 포함**한다 (`bash scripts/check-deferred-item-recovery.sh <retro-file>... EPIC-RESULTS-<EPIC_KEY>.md`) — triage-defer verdict 가 착지하는 `## §deferred` 5-column row(source=`triage-defer`)가 `_scan_retro_file` 스캔 대상에 도달해야 회수가 강제된다. EPIC-RESULTS 를 인자에서 누락하면 triage-defer row 가 silent-drop(dead-path)된다. 스크립트 로직 자체는 무변경(기존 5-column 스캔이 `source=triage-defer` row 를 enum 값 무관하게 파싱).
...
**residue-clean self-check pointer (CFP-2822 / ADR-169 §결정 9 — §결정 7 가시화 mechanism 의 완료-게이트 wire)**: `phase:완료` precondition 에는 worktree-clean self-check 와 함께 **residue-clean self-check** 도 포함된다 — "완료 Story 의 잔재(worktree+scratch+stash+orphan)가 가시화·정리됐는가" (discovery 스캐너 `--story-key cfp-NNN` 모드 리포트 확인 — `[residue-scan] DONE: scanned=N flagged=M`, flagged 보존항목별 사유+나이 aging 집계). worktree-clean self-check 와 **동형**(phase:완료 local-only warning-tier, required CI 불가 — 잔재 스캔 대상[~/.claude/worktrees·codeforge-scratch·workspace root·Temp] 클라우드 러너 미접근 [**정의역 = 클라우드 러너 평면** — ADR-172 §결정 7. 비대화형 호출 계약 = §0a-prime-2]) + **disjoint 축**(worktree-clean=등록 worktree eager 미실행 / residue-clean=잔재 전반 가시화). **자동 삭제 강제 아님**(가시화=INV-3, stash 자동삭제 Non-goal). 절차·판정 SSOT = `codeforge:worktree-lifecycle` skill + ADR-169. evidence-checks-registry `residue-clean-completion-gate` (warning-tier, workflow:null, `gate:residue-clean` label 미신설).
...
| CodebaseMapper | Orchestrator (flat) | existing codebase fact (as-is) | haiku (ADR-141 Amd1) |
...
## 15. Observability channel boundary (ADR-163 §결정 1 + Amendment 1 + Amendment 2, CFP-283 / CFP-2393 / CFP-2687)
...
- **§14 ↔ spawn-event-v1 boundary**: spawn-event-v1 신설 land ([ADR-163 Amendment 1](../archive/adr/ADR-163-codeforge-measurement-channel-architecture.md), CFP-2393 — 구 §결정 3 보류 supersede). §14 Lane Evidence(lane-coarse) 와 spawn-event(per-agent fine) 는 disjoint granularity. **§14↔spawn-event dedup script 신설 의무** (§14 row count 와 spawn-event row count 정합 검증 — read-time/aggregate 책임, append-time 아님) = Phase 2 precondition AC.
...
- **0 API call constraint** ([ADR-163 §결정 8](../archive/adr/ADR-163-codeforge-measurement-channel-architecture.md)) — telemetry instrumentation = local I/O only. Anthropic API / GitHub API / external service 호출 금지. measurement = measure 대상 amplify 금지 (CRITICAL invariant).
- **measurement-vs-fix scope boundary** ([ADR-163 §결정 10](../archive/adr/ADR-163-codeforge-measurement-channel-architecture.md)) — CFP-283 scope = measurement only. throttling / backoff / circuit breaker / rule-based hook = 별도 후속 CFP.
- **ROI gating** ([ADR-163 §결정 11](../archive/adr/ADR-163-codeforge-measurement-channel-architecture.md)) — Phase 2 enforcement 발동 prerequisite = post-merge-counters.jsonl 30+ run 누적 ([ADR-026 §결정 3](../archive/adr/ADR-026-post-merge-automation.md) 패턴 정합).

### 15.6 Cross-references

- [ADR-163](../archive/adr/ADR-163-codeforge-measurement-channel-architecture.md) — measurement channel architecture (본 §15 SSOT)
...
|---|---|---|---|
| (a) per-Issue body verbatim cite | Issue body wording 직접 인용 | `mcp__github__issue_read method=get` body field verbatim | ADR-168 §결정 1 layer 1 sub-scope 1-C (구 ADR-082 §결정 1 layer 1 sub-scope 1-C, 재제정 CFP-2840) USER-UTTERANCE-VERBATIM block 패턴 답습 |
...
| (c) axis disjoint discrimination | "비슷한 carrier 가 cover 한다" false-positive obviation | manual axis enumeration + Issue body intent ↔ carrier scope diff verify | ADR-168 §결정 12 (구 ADR-082 §결정 12, 재제정 CFP-2840) retro-time verify-before-trust 정합 batch closure 영역 |
| (d) sibling carrier cross-link via PR number | closure rationale 안 PR/Issue 번호 explicit cite | comment body 안 #NNN explicit cite + `mcp__github__pull_request_read` verify | ADR-168 §결정 9 (구 ADR-082 §결정 9, 재제정 CFP-2840) verify-before-cite 양방향 답습 batch closure 영역 |
| (e) sub-scope alphabet sequential verify | 본 §18.3 sub-scope (a)~(e) pre-write 위치 확인 | manual sub-scope alphabet sequential check before each verify step | ADR-168 §결정 1 (구 ADR-082 §결정 1, 재제정 CFP-2840) sub-scope codify 패턴 답습 |

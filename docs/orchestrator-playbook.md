...
보안 테스트: Orchestrator → SecurityTestPLAgent
             1차 layer (자동): Dependabot + CodeQL + Secret Scanning + Push Protection 결과 fetch
             2차 layer (병렬): ClaudeSecurityTest ∥ CodexSecurityTest → PASS/FIX
...
| **SecurityTestPLAgent** | 1차 layer = Dependabot/CodeQL/Secret Scanning 결과 fetch. 2차 layer = Claude/Codex 보안 리뷰 병렬 스폰 후 종합. 구현 테스트 PASS 이후 진입 |
| **Claude/CodexSecurityTestAgent** | 보안 대상 (OWASP·CWE·CVE·trust boundary·credential). 정규화 스키마, 독립 수행. 1차 layer 결과를 입력으로 활용해 high-level 분석에 집중 |
...
| 구현-리뷰 | CodeReviewPL FIX (원인=설계) | 설계 (Phase 1 follow-up PR) | Iter N / 구현-리뷰 / 원인=설계 / Change Plan 갱신 | `fix:구현-리뷰-retry` + `fix:설계-리뷰-retry` |
...
- **CodexDesignReview / CodexCodeReview / CodexSecurityTest**: 미설치 시 해당 리뷰 레인 **진입 불가** → 설치 안내 + 세션 중단
...
2. **Change Plan 소급 작성**: Architect가 hotfix 변경 diff를 소급해 Change Plan 작성 (§1-9 전부, 단 실구현은 이미 존재 상태). docs/change-plans/ commit
...
```markdown
---
type: issue-comment | story-section | change-plan | adr | ledger-append | label-update | pr-create
story: <KEY>
requester: <AgentName>
issued_at: <ISO 8601>

...
PMOAgent batch closure write-time 각 Issue 별 다음 5 sub-scope 의무 (ADR-045 §D-11 (2) 정합):
...
1. **Issue 별 `[PMO]` prefix comment + state transition** —
   - Issue 별 closure decision rationale comment 작성 (`[PMO] <decision_enum>: <rationale_with_verify_evidence>`)

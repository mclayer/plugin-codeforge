...
설계:        Orchestrator → ArchitectPLAgent → (CodebaseMapper ∥ Refactor ∥ SecurityArchitect 병렬) → ArchitectAgent (chief author) 통합 → ArchitectPLAgent 검수 → Change Plan 확정
...
| **설계 레인** | CodebaseMapper ∥ Refactor ∥ SecurityArchitect | 셋 다 원 소스(코드·ADR·Change Plan 초안) 직접 독해, 타 산출물 미참조 → 입력 독립. ArchitectAgent (chief author)가 교차 검토 → ArchitectPLAgent 검수 |
...
| 설계 | 150k | 260k | ~75k (Mapper ∥ Refactor ∥ SecurityArchitect, 각 ~25k) + ArchitectAgent (chief author) 10k + ArchitectPLAgent 5k | SecurityArchitect deputy 추가로 total +30k / peak +25k |
...
Orchestrator → (사용자 승인) → ArchitectAgent (chief author) 빠른 Change Plan (§1·§5·§8 축약) → DevPL 구현 → TestAgent → SecurityTestPL
 · CodebaseMapper·Refactor·SecurityArchitect deputy 생략, chief author 단독 (ArchitectPLAgent 검수 생략)
 · 설계 리뷰 생략

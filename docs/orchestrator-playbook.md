...
| 4 | **TodoWrite 스키마 가용** (non-blocking) | `ToolSearch("select:TodoWrite")` 성공 여부. 미로드 시 재시도 1회. 재시도 실패 시 **PASS** (lane 미차단 — ADR-038 §결정 7) + `⚠️ TodoWrite 스키마 미로드` 경고 출력. ADR-038 Amendment 1 §결정 8. |

...
5. 재스폰 이력은 Story §10 FIX Ledger가 아닌 §9(또는 해당 레인 작업 로그)에 append (FIX 루프와 구분)
...
|-----------|-----|-----------------|
| phase:요구사항 | §1-2 | RequirementsPLAgent 재스폰 → Domain·Analyst·Researcher **병렬 재스폰** (Never-skippable 3종 전원). 이전 산출물이 세션 컨텍스트에 없으면 전면 재실행 |
| phase:요구사항 | §5-6 | RequirementsPLAgent — 통합 명세서 재확정, "사용자 확인 필요" 해소 여부 체크. 일부 관점만 재보강 필요 시 해당 에이전트만 clarification 재스폰 |
| phase:설계 | §7 초안 | ArchitectAgent — Mapper·Refactor **병렬 재스폰** (이전 산출물 세션 외 유지 불가, §7 Change Plan 초안만 복원됨) |

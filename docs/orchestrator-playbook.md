...
| **DomainAgent 지식 공백 발견 시** | `docs/domain-knowledge/<area>/<topic>.md` — **DomainAgent 직접 write** (owner, CFP-26 Phase 0a). DocsAgent 의뢰 불필요 | (DocsAgent 스폰 트리거 해당 없음 — owner direct write) |
...
| **PMOAgent 회고 시** | `docs/retros/<sprint>.md` — **PMOAgent 직접 write** (owner, CFP-26 Phase 0a). Story file §11 요약 링크만 DocsAgent 의뢰 | (DocsAgent 스폰 트리거 해당 없음 — retro 본문은 owner direct write) |
...
| 설계 | 200k | 320k | ~125k (Mapper ∥ Refactor ∥ SecurityArchitect ∥ TestContractArch ∥ DataMigrationArchitect, 각 ~25k) + ArchitectAgent (chief author) 10k + ArchitectPLAgent 5k | DataMigrationArch deputy 추가로 total +25k / peak +25k (CFP-21) |
...
Orchestrator → (사용자 승인) → ArchitectAgent (chief author) 빠른 Change Plan (§1·§5·§8 축약) → DevPL 구현 → TestAgent → SecurityTestPL
 · CodebaseMapper·Refactor·SecurityArchitect·TestContractArch·DataMigrationArchitect deputy 생략, chief author 단독 (ArchitectPLAgent 검수 생략)
 · 설계 리뷰 생략
...
2. **Change Plan 소급 작성**: ArchitectAgent (chief author)가 hotfix 변경 diff를 소급해 Change Plan 작성 (§1-10 전부, 단 실구현은 이미 존재 상태). docs/change-plans/ commit
...
2. 파일 frontmatter type 별로 해당 처리:
   ...
   - `adr-draft` → **DocsAgent 처리 불가 (deny)** — Orchestrator가 ArchitectAgent를 스폰해 `docs/adr/ADR-NNN-<slug>.md` 직접 write (CFP-26 Phase 0a). 큐에 도달하면 skip + Orchestrator 경고
...
- **설계 lane packet recipient**: ArchitectPLAgent (Phase 2에서 ArchitectAgent (chief author) + 5 deputy(Mapper/Refactor/SecurityArchitect/TestContractArch/DataMigrationArchitect)에 forward — PL이 packet 분배 책임)
...
✅ 요구사항 — <S3 snippet>
🔄 설계 — 진행 중 (5/5 deputies, chief author 통합 중)
   ├─ ✅ CodebaseMapperAgent
   ├─ ✅ RefactorAgent
   ├─ ✅ SecurityArchitectAgent
   ├─ ✅ TestContractArchitectAgent
...
미정 데이터는 `?` placeholder (예: `Change Plan v? + ADR-? 신규 (deputy 5인)`).

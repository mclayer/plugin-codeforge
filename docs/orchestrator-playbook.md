...
최상위 Claude 세션(이하 **Orchestrator**)의 행동 SSOT. 사용자(Human)가 제공한 요구사항을 받아 23 core 에이전트 + role:dev roster를 조정하는 모든 규약을 담는다.
...
| **설계 레인** | CodebaseMapper ∥ Refactor ∥ SecurityArchitect ∥ TestContractArch | 넷 다 원 소스(코드·ADR·Change Plan 초안) 직접 독해, 타 산출물 미참조 → 입력 독립. ArchitectAgent (chief author)가 교차 검토 → ArchitectPLAgent 검수 |
...
- 에이전트별 누적 토큰 (23 core + preset/overlay-only `role: dev` 에이전트)
...
| 설계 | 175k | 290k | ~100k (Mapper ∥ Refactor ∥ SecurityArchitect ∥ TestContractArch, 각 ~25k) + ArchitectAgent (chief author) 10k + ArchitectPLAgent 5k | TestContractArch deputy 추가로 total +25k / peak +25k |
...
#### 에이전트별 작업 요약 (23 core + 스폰된 preset/overlay-only role:dev, 미참여 "-")
...
Orchestrator → (사용자 승인) → ArchitectAgent (chief author) 빠른 Change Plan (§1·§5·§8 축약) → DevPL 구현 → TestAgent → SecurityTestPL
 · CodebaseMapper·Refactor·SecurityArchitect·TestContractArch deputy 생략, chief author 단독 (ArchitectPLAgent 검수 생략)
 · 설계 리뷰 생략
...
- **설계 lane packet recipient**: ArchitectPLAgent (Phase 2에서 ArchitectAgent (chief author) + 4 deputy(Mapper/Refactor/SecurityArchitect/TestContractArch)에 forward — PL이 packet 분배 책임)

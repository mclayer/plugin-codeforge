...
- **비대상 3종**(failover 미발동, AC-3) = (a) Orchestrator 세션 자체 리밋(launch 고정 → 자동 모델 전환 구조 불가) (b) refusal(`stop_reason: refusal` — 수동 opus 재spawn 방어, CFP-2803) (c) 비-fable tier subagent 리밋(haiku 7 / sonnet 10 / opus — 기존 운영 관찰 note 무변경). **판정문 verbatim SSOT = `ADR-141` A6-3 + `ADR-141 A8-3` 1곳** — 본 절은 **pointer** 이며 (a) 의 remedy 처방을 여기서 재서술하지 않는다(3-site verbatim mirror 재발 차단, ADR-141 A8-5). **축 분리**: "failover 미발동" ≠ "작업 중단" — 정지 적법성은 `ADR-025` §결정 7 + `ADR-109` Amendment 2 판별식 D 소관(축 disjoint).
- **cascade count-in** = fable→opus hop = `cascade_depth` **1(COUNTS)**. opus 착지 후 opus 자기 within-model soak 미증가. opus soak(ADR-109 §결정2/3) 소진 후에도 리밋 = cascade ≥ 2 → **user manual resume only**(§결정5, 자동 무한 failover 금지). opus 착지 후 비로소 §결정2 exp-backoff / §결정3 step1·3·4 가 opus 를 same-model 로 재정박(AC-2 정합). **축 분리 cross-ref** — 그 `user manual resume only` 의 정의역은 **재시도 축 한정**(ADR-109 Amendment 2 (i)): 실패 호출의 재발행만 금지되고 남은 독립 작업의 전진은 계속된다(작업 진행 중단 아님, 신규 재시도 예산 0).
...
- **residue-clean self-check 미통과 (flagged 미가시화/미정리)** = 완료 Story 잔재(worktree+scratch+stash+orphan)가 aging report 에 미반영 → discovery Story-scoped 리포트 확인·정리 후 재확인이지 "생략 후 진행" 아님. 본 self-check 도 transition precondition (label attach 직전)이지 Issue close 차단이 아니다 — §D-12 worktree-clean 과 **disjoint 축**(worktree-clean=등록 worktree eager 미실행 검출 / residue-clean=잔재 전반 가시화). **자동 삭제 강제 아님** (가시화=INV-3, stash 자동삭제 Non-goal, git 무만료 = 의도적 사용자 데이터). warning-tier 로컬 self-check (required CI 불가 — 잔재 스캔 대상[~/.claude/worktrees·codeforge-scratch·workspace root·Temp] 클라우드 러너 미접근, branch protection 8-tuple 무변경). ADR-169 §결정 9.

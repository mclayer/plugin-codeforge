...
> **[POLICY-RETIRED — ADR-127 §결정 3]** 아래 Minimal Path 의 계획·설계리뷰·구현리뷰 생략 + 성능게이트 생략 + Claude peer 보안 생략은 폐지됐다. 운영 장애 단일 파일 버그도 정식 10 lane 무조건. 본문 historical 보존.
...
> **[POLICY-RETIRED — ADR-127 §결정 3]** 아래 Medium Path 의 deputy 생략 + 설계리뷰 생략 + Codex 구현리뷰 생략은 폐지됐다. 심각 운영 장애도 정식 10 lane 무조건. `severity:critical` 라벨은 우선순위 표기로만 존속(lane 생략 신호 아님). 본문 historical 보존.
...
- [`CLAUDE.md`](../CLAUDE.md) §"핵심 흐름" — 정식 풀 플로우 무조건 (Hotfix lane-skip 예외 폐지 — ADR-127 §결정 3)
- [`archive/adr/ADR-127`](../archive/adr/ADR-127-mandatory-full-flow-no-exemption.md) — Minimal/Medium lane-skip 폐지 SSOT
- `audit:post-hotfix` label = label-registry-v1 (§6 land_order 정정 사후 감사 용도 존속)
- `hotfix:minimal` / `hotfix:critical` label = label-registry-v1 (retired path — 부착 대상 없음)

## 5. 활성화 trigger

> **[POLICY-RETIRED — ADR-127 §결정 3]** Minimal/Medium lane-skip 경로는 활성화되지 않는다 — 운영 장애 시에도 정식 풀 플로우 무조건. 아래는 historical 기록.

- 첫 운영 장애 발생 시 본 playbook 채택 (retired — 정식 플로우로 대체)
- mctrader debut audit (Issue [#181](https://github.com/mclayer/plugin-codeforge/issues/181) P2-9) 까지 사용 사례 0 — 본 playbook 은 dormant (이후 ADR-127 로 lane-skip 경로 폐지)

## 6. cross-repo land_order post-merge 경로 (CFP-795 / ADR-026 Amendment 4 §결정 6)

> **본 경로는 운영 장애 hotfix 가 아니다** (그 경로 = §1 Minimal / §2 Medium = ADR-127 §결정 3 으로 폐지). 본 §6 은 cross-repo land_order safe-defect 정정 전용 PR-level CI 안전판으로 **존속**한다 — byte-equivalence·보안 non-touch 양면 검증을 거치는 land_order 정정 한정 (ADR-026 §결정 6 / ADR-127 §결정 8 보존군). 운영 장애 긴급 대응으로 오인용 금지.

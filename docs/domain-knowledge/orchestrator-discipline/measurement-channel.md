...
- **opt-in default false** (모든 telemetry channel 적용)
...
- **wrapper dogfood exception** = `CODEFORGE_DOGFOOD_TELEMETRY=1` explicit env flag 의무
...
- **wrapper dogfood**: codeforge family 자체 development scope (always-on exception with explicit env flag)
...
**Phase execution visibility (ADR-029) 와의 amends 관계** — §결정 2 sanitize policy = ADR-043 §결정 3 SSOT 통합. narration vs ledger 두 channel 모두 동일 sanitize 정책 inherit.

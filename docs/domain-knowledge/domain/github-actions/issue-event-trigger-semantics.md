...
- [ADR-036](../../../../archive/adr/ADR-036-project-key-atomic-reservation.md) — CFP-260 GitHub atomic Issue numbering. `KEY = PREFIX-${{ issue.number }}` 로 file-system scan + max+1 race 제거. opened event payload 의 `issue.number` 의존 — N+1 firing law 의 `opened` event 가 atomic 한 sequence position 보장 (label race 와 별 영역).

...
### 1.2 codeforge orchestration 에 영향 큰 deferred tool 5종
...
### 1.4 Critical path 의무 — `ToolSearch("select:TodoWrite")` 선제 로드
...
## 2. SessionStart hook 의 prompt-injection 모델
...
### 2.3 Anthropic 미공개 ABI 영역 (Researcher unknown #1)
...
## 3. codeforge 의 3 SessionStart hook 책임 분담
...
### 3.2 idempotent + side-effect 명시 패턴 공통
...
## 4. Researcher 3-tier enforcement model — codeforge mapping
...
### 4.2 (b) tier 의 책임 경계 (Amendment 2 §결정 9 본문 정합)
...
## 5. 관련 facts cross-reference

- ADR-037 drift hook 패턴 (선례 #1) — install path = plugin-installed
- ADR-040 worktree-gc hook 패턴 (선례 #2) — install path = consumer-installed
- ADR-038 Amendment 1 §결정 8 — runtime advisory tier 의무 (CFP-375)
- ADR-038 Amendment 1 §결정 7 — failure non-blocking 폴백
- ADR-038 Amendment 2 §결정 9 — SessionStart hook tier escalation (CFP-500, 본 entry 의 carrier ADR)
- ADR-058 — Amendment 2 amendment_log sunset_justification 3-tuple metric/who/how
- ADR-060 — evidence-enforceable framework (manual sampling → grep-based auto count 격상 후보)
- ADR-056 §결정 1·2 — domain-knowledge dir separation, 본 entry path 정합

...
**ADR 능동 일몰(active sunset)** = 일몰 *기준*을 충족한 단일 ADR 을, paradigm replacement 가 아닌 평시에 **실제로 일몰 상태로 전이**시키는 실행 절차. 효용을 대체 carrier 로 lossless 이전(carrier-preserved)하면서 `sunset_status: Sunsetted` frontmatter 플래그 set + archive 강등 + 역참조 안전 갱신 + RESERVATION 번호 잠금까지를 1 단위로 묶는다.

본 문서는 codeforge 의 sunset **기준·메트릭·약화 evidence-gate** 기계장치(ADR-058 / ADR-095 / ADR-097 / `check_adr_sunset_criteria.py`)를 **재정의하지 않고**, "기준을 충족한 단일 ADR 을 *실제로 일몰시키는 실행 절차*" 공백만 메우는 SSOT 다. 본 문서는 신규 워크플로 0 / 신규 스크립트 0 / 신규 ADR 0 (ADR-058 Amendment 로 연결만) — doc-only fast-path (ADR-054).

## 컨텍스트

### 왜 필요한가 — 기존 기계장치 gap (G1/G2/G3 공백)

codeforge 에는 sunset 기준·메트릭·약화 evidence-gate 기계장치가 이미 존재하나, "기준 충족 후 실제 일몰 *실행*" 의 표준 절차가 부재했다. 본 문서는 그 3개 공백(G1 단일·평시 실행 / G2 archive 강등 / G3 역참조 안전점검)을 declarative 로 메운다. 각 기계장치와의 경계는 §경계 참조.

### sunset 전례 — "실 retire 0건" 주장 정정
...
**carrier-preserved 의무**: 효용이 대체 carrier 로 lossless 이전됨을 enumeration. naive sunset (효용 소멸, carrier 부재) 은 ADR-058 §결정 5 약화 evidence-gate 대상 — 본 절차 비대상.
...
### 단계 4 — status 강등 (G2 체크리스트 참조)

frontmatter `sunset_status: Sunsetted` 설정. 상세 = 본 §핵심 규칙 G2.

### 단계 5 — 역참조 갱신 (G3 절차 참조)

본 ADR 을 인용하는 다른 ADR/contract/doc 의 cross-ref 를 carrier-preserved 포인터로 갱신. 상세 = 본 §핵심 규칙 G3.
...
#### G3.2 점검 절차 (Wave 1 — declarative, mechanical lint = 후속 carrier)
...
#### G3.3 mechanical enforcement = 후속 carrier (Wave 1 declarative-only)
...
| 단계 | 책임 |
|---|---|
| 트리거·후보 판정·`sunset_justification`·status 강등·역참조 (G1 단계 1-5) | ArchitectAgent (design lane chief author, ADR-070 chief author precedent) |
| RESERVATION 잠금 (G1 단계 6) | GitOpsAgent (ADR-050 monopoly write) |
...
### 기존 기계장치와 중복 0 (재정의 금지 경계)

codeforge 에는 ADR 일몰 **기준·메트릭·약화 evidence-gate** 기계장치가 이미 존재한다. 본 절차는 그것들을 **재정의하지 않고**, 그들이 다루지 않는 *실행 절차* 공백만 메운다.
...
| **ADR-095** | sunset metric 형식 표준 + K8s 시간 threshold (GA 12개월 / Beta 9개월) + bulk 집계 dashboard schema | 본 절차는 ADR-095 의 metric/threshold 를 **후보 판정(G1 단계 2)의 입력 신호**로 인용. metric 산정 규칙 신설 0. |
| **ADR-097** §결정 3 | bulk(9+ ADR 동시) sunset = carrier-preserved sunset 개념 + paradigm replacement 면제 channel | 본 절차 scope = **단일·평시 sunset** (1 ADR, paradigm replacement 아님). bulk sunset 은 ADR-097 면제 channel 이 SSOT — 본 절차는 아래 bulk 분기에서 "bulk = 본 절차 비대상, ADR-097 경로" 만 명시. |
| **`scripts/lib/check_adr_sunset_criteria.py`** | sunset *기준* mechanical lint (warning) — `is_transitional` 존재 / `## 해소 기준` 섹션 / 3-tuple / 모달 어휘 | 본 절차는 이 lint 가 검증하지 **않는** 영역(status 전이·archive 강등·역참조)을 다룬다. lint 와 disjoint — 본 절차의 G3 역참조 점검은 Wave 1 declarative-only (mechanical lint = 후속 carrier). |

**중복 신설 금지 invariant**: 본 문서는 신규 워크플로 0 / 신규 스크립트 0 / 신규 ADR 0 (ADR-058 Amendment 로 연결만). doc-only fast-path (ADR-054).

### bulk sunset 비대상 분기 (ADR-097 경로)
...
- 본 절차(G1-G3)는 그 bulk Epic 안의 **개별 ADR 강등(G2) + 역참조(G3)** 의 단위 실행 참조로 재사용 가능 — 단 자격/면제/atomic merge order 는 ADR-097 이 SSOT.

## 관련 ADR

- **ADR-058** (ADR sunset criteria mandate) §결정 5 — `sunset_justification` evidence-gate (본 절차 G1 단계 3 의 입력). §결정 9 ↔ 본 절차 cross-ref 양방향 정합 (ADR-058 §결정 9 가 본 실행 절차 doc 을 SSOT 로 지정, 본 절차가 ADR-058 evidence-gate 를 입력으로 수령).
- **ADR-095** (sunset metric 표준 + K8s threshold) — 후보 판정(G1 단계 2) 입력 신호 (GA 12개월 / Beta 9개월 threshold + bulk 집계 dashboard). metric 산정 규칙 신설 0.
- **ADR-097** (paradigm replacement governance) §결정 1-3 — bulk(9+ ADR) sunset = carrier-preserved sunset SSOT. 본 절차의 bulk 분기 경로 (§경계).
- **ADR-064** (decision principle mandate) — CFP scope unitary (본 carrier CFP-2061-S3 = 단일·평시 sunset 실행 절차 SSOT 신설, 별 ADR Amendment 0).
- **ADR-076 / ADR-083** (declarative reconciliation upgrade / consumer-applicability filter) — carrier-preserved sunset 집행 전례 (§컨텍스트 sunset 전례 표 + G2 archive 강등 체크리스트 전례). 본문 보존 + `sunset_status: Sunsetted` 플래그 + 역참조 carry 방식 직접 인용.
- **ADR-050** (ADR-RESERVATION 운영 레지스트리) — G1 단계 6 RESERVATION 번호 잠금 (`active → archived`, GitOpsAgent monopoly write).
- **ADR-041** (doc-locations SSOT) — 본 문서 위치 + ADR `archive/adr/` 경로 (G2 C-4 위치 유지 근거).
- **ADR-054** (doc-only fast-path) — 본 carrier classification 근거 (SSOT 문서 신설 + 기존 ADR Amendment 0 + src/tests 무변경).

## 변경 이력

| 일자 (KST) | 변경 | Carrier | 비고 |
|---|---|---|---|
| 2026-06-09 | 신설 (단일·평시 ADR 능동 일몰 실행 절차 SSOT — G1 6단계 + G2 archive 강등 체크리스트 + G3 역참조 안전 점검 + 기존 기계장치 중복 0 경계 + bulk 분기 + "실 retire 0건" 주장 정정). FIX iter — doc-section-schema 6 section (정의 / 컨텍스트 / 핵심 규칙 / 경계 / 관련 ADR / 변경 이력) 재배치 (정보 손실 0, G1/G2/G3 + 경계표 + 전례표 + RACI 전부 보존). | CFP-2061-S3 | parent Epic CFP-2061 de-bloat 거버넌스 영속화. ADR-058 §결정 9 ↔ 본 절차 cross-ref 양방향 정합. ADR-076/083 carrier-preserved 전례 verified-via Read `archive/adr/`. |

...
본 문서는 codeforge 의 sunset **기준·메트릭·약화 차단** 기계장치(ADR-058 / ADR-095 / ADR-097 / `check_adr_sunset_criteria.py`)를 **재정의하지 않고**, "기준을 충족한 단일 ADR 을 *실제로 일몰시키는 실행 절차*" 공백만 메우는 SSOT 다. 본 문서는 신규 워크플로 0 / 신규 스크립트 0 / 신규 ADR 0 (ADR-058 Amendment 로 연결만) — doc-only fast-path (ADR-054).

## 컨텍스트

### 왜 필요한가 — 기존 기계장치 gap (G1/G2/G3 공백)

codeforge 에는 sunset 기준·메트릭·약화 차단 기계장치가 이미 존재하나, "기준 충족 후 실제 일몰 *실행*" 의 표준 절차가 부재했다. 본 문서는 그 3개 공백(G1 단일·평시 실행 / G2 archive 강등 / G3 역참조 안전점검)을 declarative 로 메운다. 각 기계장치와의 경계는 §경계 참조.
...
codeforge 에는 ADR 일몰 **기준·메트릭·약화 차단** 기계장치가 이미 존재한다. 본 절차는 그것들을 **재정의하지 않고**, 그들이 다루지 않는 *실행 절차* 공백만 메운다.

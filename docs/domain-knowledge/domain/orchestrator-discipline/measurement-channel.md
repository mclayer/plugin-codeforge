...
codeforge orchestration 의 **4-channel observability boundary** SSOT. Tier 3 (persistent ledger) 의 stop-event-v1 + post-merge-counters.jsonl 구조와, 각 channel 의 책임 경계 (측정 대상 / 측정 제외 대상) 를 명시한다. ADR-163 measurement channel architecture + ADR-039 Phase 2 enforcement 의 데이터 prerequisite.
...
기존 codeforge observability stack 3-tier 중 Tier 3 가 가장 sparse — `post-merge-counters.jsonl` (ADR-026 lite scope) 만 존재했으며, stop-event-v1 ledger (ADR-025 §결정 10 deferred slot) 는 CFP-283 / ADR-163 신설로 채움. **spawn-event-v1 (per-agent token/cost attribution + replay) = CFP-2393 / ADR-163 Amendment 1 land** (구 Refactor B1 보류 해제 — Epic CFP-2391 S3, oh-my-claudecode MIT 차용). Tier 3 = 3 channel (post-merge-counters / stop-event-v1 / spawn-event-v1).

## 컨텍스트

본 page 는 ADR-163 (codeforge measurement channel architecture) + ADR-043 (codeforge telemetry privacy policy) 의 도메인 정의 cross-cutting reference. CFP-275 ADR-039 §결정 9 deferred 의 4 items 중 measurement channel slot only 처리 (CFP-283 carrier).
...
### 4-channel boundary (ADR-163 §결정 1 / playbook §15 SSOT)
...
- **§14 ↔ spawn-event-v1 boundary**: spawn-event-v1 land (ADR-163 Amendment 1, CFP-2393). §14 lane-coarse ↔ spawn-event per-agent fine = disjoint granularity. **§14↔spawn-event dedup script 신설 의무** (read-time/aggregate, Phase 2 precondition AC).
...
### Storage architecture (ADR-163 §결정 4 / DataMigrationArch substantive)
...
### ROI gating (ADR-163 §결정 11 / ADR-026 §결정 3 패턴)
...
**TodoWrite (ADR-038) 와의 boundary** — TodoWrite tool surface 자체가 file system / GitHub state mutation 미발화. ledger record 대상 아님 (boundary 차단). ADR-163 §결정 1 invariant.

**Stop discipline (ADR-025) 와의 amends 관계** — §결정 10 deferred slot (stop-event-v1) 가 ADR-163 §결정 2 로 채움. ADR-025 의 5 종 whitelist 무변 — stop-event-v1 = 측정 channel, whitelist 자체 변경 X.
...
**Lane-spawn evidence (ADR-031) 와의 boundary** — §14 lane coarse vs spawn-event-v1 sub-step. spawn-event-v1 보류 (ADR-163 §결정 3) — §14 schema 무변경 invariant 보존.
...
- [ADR-163](../../../../archive/adr/ADR-163-codeforge-measurement-channel-architecture.md) — measurement channel architecture (architectural decision SSOT)
...
- **2026-05-09** — ArchitectAgent 신규 작성 (CFP-283 carrier Story Phase 1 PR scope, ADR-163 §결정 12 동일 PR commit batch).

...
topic_slug: worktree-self-ownership-verify
title: "Worktree-first self-ownership verify — path-based 3-tuple (cwd/reflog/worktree-list)"
status: Active
updated: 2026-05-20
tags:
  - worktree-first
...
owner_adr: ADR-073-Amendment-3
sibling_cfp:
  - CFP-1038  # carrier ESC (plugin-codeforge#1038 PMO P1 ESC 정식 carrier)
  - CFP-983   # 후보 (c) 정식 carrier 관계 (disjoint axis — real parallel cross-session collision sub-domain)
  - CFP-1026  # sentinel #1 (brainstorm STAND-DOWN false-positive)
  - CFP-681   # sentinel #2 + #3 (cfp-1014 dup worktree + ArchitectPL Phase 3 자기 commit mis-flag)
---
...
### 본질 anchor — 5th layer staleness (spatial dimension)
...
### Path-based 3-tuple verify primitive (definition)

사용자 원문 prompt 의 3-tuple (cwd ↔ git config user.email / HEAD author ↔ session identity / `gh pr list head:<branch>` ownership) 은 **identity-based** — ResearcherAgent + FeasibilityAgent 통합 평가 결과 Solo-dev 환경 (단일 mccho-mclayer git config) 식별력 0 + edge case 다수 (signed commit GPG / detached HEAD / anonymous worktree / `CODEFORGE_SESSION_ID` env 부재). **path-based 대안 채택** (Amendment 3 §결정 1-D 정합) — 3 primitive 정의:

| ID | Verify check | 실행 cmd | PASS 조건 |
|----|---|---|---|
| **(a)** | cwd ↔ worktree path 일치 | `git rev-parse --show-toplevel` vs `git worktree list --porcelain` | string equality (path normalize: forward-slash `/` + lowercase drive-letter `c:`) |
| **(b)** | HEAD lineage ↔ session reflog membership | `git reflog show <branch> --all` + `git log <commit> --oneline` cross-check | reflog membership = true (본 session commit chain 추적 가능) |
| **(c)** | worktree list + reflog 2-source AND | `git worktree list --porcelain \| grep <branch>` parse `branch` field + reflog entry exists | 양 source PASS = self-ownership verify TRUE |
...
### 외부 fact 인용 — `git worktree list --porcelain` 산업 표준 verify primitive
...
### Subagent verdict re-verify mandate (multi-worktree self-confusion 영역 agent 도 보임)
...
### Mechanical enforcement chain (Wave 1 declaration / Wave 2 mechanical wire / Wave 3 auto-promote blocking)
...
#### detached HEAD (branch reflog 없음)
...
#### anonymous worktree (`git worktree add --detach` path)
...
#### Signed commit GPG + CODEFORGE_SESSION_ID env 부재
...
#### Reflog GC 90d window (`gc.reflogExpire=90 days` git default)
...
#### Path normalization (Windows drive-letter case + slash direction)
...
| ID | 통상 PASS 조건 | Fallback rule |
|---|---|---|
| **(a)** | string equality (path normalize) | normalize 후 mismatch 시 (a) FAIL → (c) backstop |
| **(b)** | reflog membership = true | reflog 90d GC 시 (b) FAIL → **(a)+(c) 2-source AND fallback** |
| **(c)** | 양 source PASS | (a) FAIL OR (b) FAIL OR (c) FAIL → parallel session verdict |

### Disjoint scope — real parallel session 영역 (#983 후보 (a)/(b)) 와 분리
...
| 날짜 (KST) | Carrier | 변경 |
|---|---|---|
| 2026-05-20 | CFP-689 | 신설 — ADR-073 Amendment 3 narrative SSOT (worktree-first self-ownership verify 3-tuple path-based, pattern_count 3 reach forcing function). FIX iter 1 = CFP-28 strict mode frontmatter (kind: domain → domain_fact + area/topic_slug/updated 필수 필드 append) + 6 필수 section schema 정합 (기존 8 section 본문 reuse + 재배치, 본문 의미 무손실). |

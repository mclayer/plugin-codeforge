...
ADR-083 이 applicability 필터(4-way repo-kind + positive whitelist)를 정의했으나, **closure 축이 1급으로 정의되지 않았고**, whitelist 의 normative 지위 역시 어느 ADR 에도 미정의 상태였다. ADR-130 이 그 공백을 메우고, 본 entry 가 도메인 narrative 로 명문화한다.

...
related_files:
  ...
  - docs/domain-knowledge/domain/production-cutover/rollback-protocol.md
---

# Production cutover — 4-evidence-quad 수집 절차

ADR-72 §결정 5 SSOT — EPIC CLOSED gate 가 의무하는 production 실측 evidence 4중 명시. **Live touching Epic 한정 의무** (wrapper governance Epic / doc-only Epic 자연 N/A — ADR-005 plugin self-application N/A 표준 정합).

## 1. Evidence quad 4중 정의

### 1.1 Production bucket prefix listing evidence

production storage (S3 / GCS / etc) bucket prefix listing verbatim output:
...
production storage WAL row sample:
...
### 1.3 L1 backlog drainage rate (1h sustained 측정)

drainage rate 의무 invariant: `drainage_rate / ingest_rate <= 1.0` (1h sustained window).
...
### 1.4 L2/L3 자연 cadence trigger 실측

5min OR 1h window trigger 실측 명시 (cadence_actual_window vs designed_window mismatch surface).
...
## 2. 사용자 ack quad 5중 (Story-1 anchor 정합)
...
## 3. 4 prerequisite measurement source mechanical anchor 4-tuple cross-ref
...
## 4. Wrapper-self-app N/A invariant
...
- rollback-protocol.md (7-step incident response)
- AWS Well-Architected Framework "Operational Excellence" pillar (`OPS09 Monitor operations`)
- Google SRE workbook "Postmortem culture" chapter
- Datadog APM "Service Level Objectives" precedent

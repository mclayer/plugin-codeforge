...
related_adrs:
  ...
  - ADR-045  # retro mandatory + §D-11 batch closure 4-option enum (CLOSE_AS_OBVIATED/SENTINEL/PROMOTE/DEFER) — structured-row 패턴 참조 선례 (값 비재사용; 게이트는 직교 {tracked, observed} 신설)
...
판정의 진정성은 **dual-source AND** 로 검증한다: (선언된 disposition row) ∧ (실제 tracking Issue 존재 OR 사유 텍스트 존재). 선언만 있고 실 backing 이 없으면 theater (연극 게이트). **게이트 전용 직교 2-value enum `{tracked, observed}` 를 신설**해 structured row 를 anchor 로 써 자유텍스트 파싱을 회피한다 (ADR-045 §D-11 batch-closure 5-column table 은 structured-row-over-free-text **패턴의 참조 선례**로만 인용 — closure enum *값*은 비재사용. 의미 축 직교: closure disposition ⊥ narrative deferred 판정).
...
- **ADR-045** §D-11 — retro batch closure 4-option enum (CLOSE_AS_OBVIATED / CLOSE_AS_SENTINEL / PROMOTE / DEFER) + 5-column closure summary table. structured-row 패턴의 **참조 선례** (값 비재사용 — 게이트는 직교 `{tracked, observed}` enum 신설; closure disposition 축 ⊥ narrative deferred 판정 축).

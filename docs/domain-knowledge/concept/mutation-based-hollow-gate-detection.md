...
sources:
  ...
  - https://mir.cs.illinois.edu/marinov/publications/ShiETAL19FlakyMutation.pdf                                  # Mitigating Effects of Flaky Tests on Mutation Testing — 19% killed mutants flaky-induced
...
mutation testing 은 **테스트가 결정론적** 이라고 암묵 가정하나 flaky test 가 이를 깬다. 자바 22개 프로젝트에서 **죽은 mutant 의 19% 가 적어도 하나의 flaky test 의 flakiness 에서 비롯** 되었고, 반복 실행 간 mutation score 가 평균 4%p 변동, mutant-test 쌍의 9% 가 unknown status (출처: ShiETAL19, mir.cs.illinois.edu; IEEE 10675738 flaky 중복제거).

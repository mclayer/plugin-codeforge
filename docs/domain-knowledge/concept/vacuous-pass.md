...
related_adrs:
  - ADR-119  # research-before-claims — 게이트 verdict = internal proxy 아닌 outcome ground-truth
  - ADR-136  # 게이트 실행-무결성 3요건 (blocking / full-scope / self-tested)
  - ADR-145  # ac-traceability zero-drop fail-closed 게이트 (대상 0건 fail-closed 선례)
tags:
  - codeforge
  - gate-integrity
  - false-green
...
- **In scope**: 품질 게이트가 "검사 대상 0건" 때문에 무력화되는 실패 모드의 정의·판정 기준·거부 규범. fail-open 방향(vacuous PASS)과 fail-stuck 방향(vacuous PENDING)을 같은 뿌리로 묶는다.
- **Out of scope**:
  - **테스트가 있으나 무력한** 경우 — mutation testing 영역(인접 개념, disjoint). vacuous PASS 는 그 **한 단계 앞**(대상 자체가 0).
  - 게이트 로직의 **정확성**(대상은 있으나 규칙이 틀린 경우) — 별개 축.
...
- **ADR-119 (research-before-claims)** — 게이트 verdict PASS 를 internal proxy 아닌 **outcome ground-truth** 로만 단정. vacuous PASS 는 이 원칙이 거부하는 대표 실패 모드(초록불 = proxy, 실제 검사 0건 = ground-truth 부재).
- **ADR-136 (게이트 실행-무결성 3요건)** — blocking / full-scope / self-tested. 그중 **full-scope**(대상 표면이 실재)와 **self-tested**(positive control)가 vacuous PASS 직접 방어.
- **ADR-145 (ac-traceability zero-drop fail-closed 게이트)** — "대상 0건 = fail-closed" 를 실제 required 게이트로 구현한 codeforge 내 선례.
- **CFP-2545 / CFP-2635** — `presence-grep = false oracle` → execution-backed 검증 전환 사례(개념의 실증 근거).

## 변경 이력

- 2026-07-13 KST — 초기 작성 (CFP-2661 Phase 1 설계 lane). vacuous truth 논리 뿌리 + 업계 관행(pytest / golangci-lint / Jest / ESLint / ruff) + codeforge 발현 3종 + 반증 가능성 기준·기본 규범 정립. 최초 작성본이 concept doc section/frontmatter schema(6 필수 섹션 + status·updated 필드) 미준수로 born-invalid → 본 재편으로 schema 정합(CFP-2661 Phase 1 CI FIX).

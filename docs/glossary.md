...
> **본 file = codeforge governance Bounded Context (BC) 의 Published Language SSOT** — DDD term 의 한국어/영어 병기 + definition + plugin-codeforge 적용 사례 박제. ADR-087 carrier story = CFP-1117-S1.
>
> **Published Language 분리** (ADR-087 §결정 4): codeforge governance BC ↔ mctrader application BC 의 동음이의 (Aggregate / Module / Repository 등) 충돌 차단. mctrader 측 SSOT = `mctrader-hub/docs/glossary.md` (downstream Epic, 별 CFP).
>
...
### codeforge governance BC 의 Authority / Specialist Pattern (ADR-087 §결정 1)
...
**plugin-codeforge 적용 사례**: 본 plugin = **codeforge governance BC** (Aggregate = supervised authority cluster). mctrader application = **별 BC** (Aggregate = DDD Aggregate root in domain model). Published Language 분리 (본 glossary SSOT + mctrader `docs/glossary.md` SSOT, ADR-087 §결정 4).

### Subdomain (Core / Supporting / Generic)

**영어**: Subdomain  
**정의**: 비즈니스 도메인의 하위 영역 분류. Core (경쟁 우위) / Supporting (지원 / 차별점 없음) / Generic (일반 / 외부 솔루션 우선).  
**plugin-codeforge 적용 사례**: codeforge governance BC 자체 = (a) **Core**: ArchitectLane decision-making (chief author 통합 + deputy advocacy synthesis) (b) **Supporting**: lane evidence tracking + retro automation (c) **Generic**: GitHub API integration + worktree management. Subdomain 분류 SSOT 는 본 ADR-087 = 분류 기준만 (downstream Epic 가 repo-by-repo 실 분류).

### Core Domain

**영어**: Core Domain  
**정의**: Subdomain 분류 중 비즈니스 경쟁 우위를 결정하는 핵심 영역. 최고 인재 / 정교한 모델 / 가장 많은 투자 의무.  
**plugin-codeforge 적용 사례**: ArchitectLane (codeforge-design plugin) 의 decision-making mechanism = codeforge governance BC 의 Core Domain. 본 ADR-087 의 vocabulary governance 가 Core Domain 의 discipline mechanism 부착.
...
**plugin-codeforge 적용 사례**: 본 glossary.md = codeforge governance BC 의 Ubiquitous Language SSOT. agent file 본문 + ADR + Change Plan + Story file 안 모든 DDD 어휘는 본 glossary 정의 verbatim 인용 의무. drift 차단 = `scripts/check-ddd-vocabulary.sh` (ADR-087 §결정 6 lint, S3 신설).
...
**plugin-codeforge 적용 사례**: 본 ADR-087 = codeforge governance BC 의 Strategic Design 결정. 15 agent 의 BC + Subdomain + Ubiquitous Language SSOT 명시화.
...
**영어**: Aggregate  
**정의 (codeforge governance BC, ADR-087 §결정 3 Layer A)**: **supervised authority cluster** — ArchitectPLAgent 가 6 deputy + chief author 산출물 통합하는 supervisor 의 metaphor only. Aggregate Root metaphor.  
...
**영어**: Aggregate Root  
**정의 (governance BC, ADR-087 §결정 3 Layer A)**: Authority Pair (ArchitectPL + Architect) 의 supervisor side metaphor. ArchitectPLAgent = supervised authority cluster 의 root.  
...
**정의 (codeforge governance BC, ADR-087 §결정 1)**: specialized judgment contributor — 7 permanent SubAgent + 3 sub-tuple 의 role.  
...
**plugin-codeforge 적용 사례**: codeforge governance BC 의 Tactical Design = 본 glossary 안 11 tactical term 그대로 채택. ADR-087 §결정 1 의 agent ↔ DDD pattern mapping 이 Tactical Design 의 codeforge governance 적용.
...
**영어**: Authority Pair  
**정의 (codeforge governance BC)**: ArchitectPLAgent (supervisor) + ArchitectAgent (chief author) 2-agent pair. plan consistency 책임 + Story 단위 산출물 (Change Plan + ADR draft) 의 final author authority. ADR-087 §결정 1 의 첫 DDD role enum.  
...
**정의 (codeforge governance BC, ADR-087 §결정 3 Layer A)**: ArchitectPLAgent 의 supervised authority cluster metaphor. **metaphor only** — literal DDD Aggregate Root 아님 (Codex Q2 합의: agent = process participant ≠ domain object).  
...
**정의 (codeforge governance BC, ADR-087 §결정 1)**: specialized judgment contributor. 7 permanent SubAgent (SecurityArch / InfraOpArch / TestContractArch / AggregateArch / APIContractArch / ModuleArch / DataArch) + 3 sub-tuple (CodebaseMapper / Refactor / ArchitectAnalyst).  
...
**정의 (codeforge governance BC, ADR-087 §결정 1)**: 3+1 CONDITIONAL deputy role — LiveOpsDeputyAgent / LiveOrderingDeputyAgent / ProductionEvidenceDeputyAgent + AggregateArch CONDITIONAL P2. "which subdomain under threat" 활성 시만 spawn.  
...
**정의 (ADR-087 §결정 2)**: ArchitectPLAgent 의 deputy spawn rationale 어휘 — perspective-contributor (보수 / 혁신 / 위협) 어휘 transition. **subdomain decision is at risk** 시 Subdomain Specialist spawn.  
...
**정의 (codeforge governance BC 자체 anti-pattern, ADR-087 §결정 7)**: 어휘 emit 만, decision flow 변경 0. agent 가 DDD 단어 emit 하면서 기존 implicit decision flow 유지 → restructure = document 만 향상 / runtime lesson 해소 = 0.  
**plugin-codeforge 적용 사례**: **ADR-087 §결정 7 forcing function 차단 대상**. INV-5 가 5 영역 박제 (Story field / deputy spawn rationale / Change Plan DDD field / review-verdict finding / ADR acceptance criteria) evidence enumeration 의무. Codex BIG CONCERN 정합.

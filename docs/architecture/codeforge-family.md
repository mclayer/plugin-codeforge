...
**Living Architecture 1차 소스 승격 — per-PR 최신성 게이트 + read protocol (ADR-078 Amd3 / ADR-112 Amd1 / ADR-166 / CFP-2813)**: 사용자 확정 진단("단편적으로 읽어 잘못된 정보" = layer 오용 — 현재상태 질문에 append-only 이력을 재구성해 읽음)의 2축 해소. **write/freshness 흐름 (a)**: PR 변경 표면 → 구조 표면 closed path-class 분류(`plugins/<X>/**` / wrapper 구조표면 / 비구조) → glob self-discovery 로 대응 arch doc 집합 도출(`plugins/<X>/docs/architecture/<X>.md` + family.md, mapping-miss = FAIL = 신규 plugin seed forcing) → per-doc closed-binary(frontmatter 외 본문 diff 갱신 OR `[living-arch-no-impact]` declare) → 위반 = job RED(blocking-on-pr(surfacing) day-1 — required 미편입·8-tuple 무변경, 구 게이트 2기가 warning 인 채 잊혀 제거된 rot evidence #1972/#2110 carve-out) → L3 substance = DesignReviewPL `living-architecture-not-updated`(기존 v4.16 채널). **read 흐름 (b)**: G1+G2 lane spawn(요구 4 + Mapper + 리뷰 PL 3 + ArchitectAnalyst) → agent .md/review-pl-base in-band 지시 → 자기주도 부분집합 read(floor = arc42 §5 Building Block, 2-doc ceiling, 5-anchor 부재 doc = 4 H2 fallback, 전문 pre-embed 금지 — context rot) → 산출물 `[Living-Arch-Read]` marker(advisory ceiling 정직 — read 기계 증명 아님). rollout 순서 invariant: (b) mandatory 발효 = (a) 게이트 활성 + 6 lane doc stale 실갱신(ArchitectAgent write monopoly) 이후 — stale 소스 중앙집중화 자기모순 방지. Orchestrator = read 강제 제외(ADR-142 L1 정합). consumer = capability-conditional(`docs/architecture/*.md` 보유 시 fallback 모드, 미보유 = honest no-op). 신규 required context 0. Phase 1 = 설계 SSOT(ADR 3건 + change-plan + Story §3/§7 + 본 doc), 실 게이트 5-piece chain + registry/인벤토리 row + 계약 파일 + 9 주입 표면 + 6 lane doc 실갱신 = Phase 2(Change Plan §5 22-항목).

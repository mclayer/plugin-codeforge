...
| **지속-liveness soak 게이트 G2 (ADR-148 / CFP-2613) — Phase 1 설계 landed** | 2-표면(선언 fail-closed CI-lint ⊥ 런타임 IntegrationTest boot-soak) + verdict-kernel seam(`evaluate_soak_sample` 순수함수 + reason-code enum 단일소스화) + INV-D1~D6 + 정직 천장 설계 완료(Phase 1 = 본 doc + ADR-148 + change-plan + Story §7). **미결(사용자 결정)** = 표면 A required 편입(7→8-tuple, 비가역+consumer mass-break=ask-trigger) — 설계리뷰 escalate, **권장 shadow-first**(blocking 승격하되 required 미편입 → bounded window 실 PR 신호 → 후속 CFP 편입, G1 즉시등록과 대조). **미결(Phase 2 구현)** = verdict-kernel 물리형태(templates/ 소script vs schema spec-embed, §3.2) / 표면 A blocking(exit1) 승격 + sink_probes[]·daemon_type schema + test-verdict-v2 v2.3 + fixture-daemon self-test 실배선. soak/restart/replay = G2 단일소유(ADR-146 §결정3 g2_boundary_check). 신규 required context 0(사용자 결정 유보) | 본 Story Phase 2 PR (CFP-2613 §8-§11) + required 편입 = 사용자 결정 후속 CFP |

...
| **backward 파생 sync 엔진 (Epic #2800 R2 — 문서 SoR 명목 역전, CFP-2829 S2) — Phase 1 설계 landed** | S2 = 엔진 + guardrail(flag CFP2829_BACKWARD_SYNC_ENABLED default OFF, 무활성) 완료 / live 저작 + 승인 loop = S3 / ADR-103·111 supersede + doc-locations immutable flip = S6 gated (DR-3). 실질 canonical = git-substrate 보존, 명목 저작 표면만 역전(INV-A PR-only, direct write 0). | Epic #2800 (R2) + 본 Story CFP-2829 (S2 landed / S3 / S6 후속 carrier) |

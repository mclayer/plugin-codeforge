...
infra_resources:
  ...
  resources:
    - id: <string>                    # 자원 논리 ID (env 키 아님) — plane B 는 이 ID 로만 참조
      namespace: <string>             # (선택) cross-repo 참조 시 필수 (§결정4). repo-local 자원은 생략. owner/repo 형식 = fetch allowlist.
      cross_repo_ref: <string>        # (namespace 선언 자원 필수, §결정4 ref-pin) pinned tag/SHA — moving HEAD(main 등) 금지

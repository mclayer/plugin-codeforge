...
**패턴 재사용(신규 발명 아님, AC-16)**: `_env` reference 규약이 이미 project.yaml 다수 위치(`atlassian.confluence.api_token_env` / `integration_test.db_probes[].connection_env` 등)에 정착 — `infra_resources` = 산재한 기존 규약의 중앙화 + (자원 ID · alias · mode) 필드 추가. baseline/승격 기제도 ADR-060 재사용(신규 발명 금지).
...
### `deploy` 섹션 = 제거됨 (ADR-121 / CFP-2782 — 배포 2 lane 물리 제거)

> 구 `deploy.*` block (CFP-1059 / ADR-087+088 — host_mapping / docker_hub / traefik / 1password / ssh_targets + auto_rollback / operational_monitor / self_improving_loop / canary)은 배포 2 lane 물리 제거(ADR-121 §결정 A, sunset 2026-07-13 KST 경과)로 삭제됐다. 배포는 consumer repo 의 GitHub Actions + GitHub Environments (dev/stg/prd)로 완전 위임된다 — 설정 절차 SSOT = [consumer-guide.md §1n 배포 위임 모델](consumer-guide.md). 대체 스키마 0 (project.yaml `deploy.*` 재정의 없음, 대체 모델은 project.yaml 밖에 산다 — ADR-121 §결정 2·3). schema 변경 7 원칙(ADR-089) + cross-layer 정책(ADR-090)은 배포 매커니즘과 독립으로 존치 (§consumer-guide.md §1n Schema/Cross-layer 단락 SSOT).

### `atlassian` 섹션 설명 (CFP-1215 / [ADR-100](../archive/adr/ADR-100-confluence-doc-ssot-recognition.md))

Atlassian suite 재결합 (Epic-A CFP-1146) 의 consumer overlay 영역. ADR-100 §결정 3 — project.yaml `atlassian.*` schema (token = `*_env` reference only, SecurityArch 권고, `integration_test.db_probes[].connection_env` env-key precedent 정합).
...
branch_protection:
  ...
  consumer_repo_lint:                # [선택] branch-protection-context-parity lint (CFP-1807 cross-repo parity carrier) 가 consumer repo 도 검사할지 결정
                                     # 부재 시: consumer repo 검사 비활성 (codeforge family 7 plugin 만 iteration)
...
- **secret 분류** (SecurityArch §7.1 정합):
  ...
  - precedent: project-config-schema.md `integration_test.db_probes[].connection_env` = `_env` suffix reference 패턴 (ADR-100 §결정 3 §secret boundary 정합).
...
- **`branch_protection.applicable`** (선택, enum, default `wrapper_only`): consumer-applicability 4-enum 분류.
  - **`wrapper_only`** (default) — codeforge family plugin repo (wrapper + 6 lane plugin = 7 plugin) 만 governance. wrapper `CLAUDE.md` "branch protection contexts SSOT" 표 가 SSOT. consumer 측 branch protection = 자율 (wrapper 검사 0건). 기존 동작 보존 — backward-compat invariant.

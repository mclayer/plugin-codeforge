...
| **codeforge-deploy** (신설 — CFP-1059 / ADR-087, Phase 1 declarative) | `docs/stories/<KEY>.md §12` 배포 manifest (DeployPLAgent self-write — Phase 1 declare, 실 lane plugin seed 신설 = S2 sub-Story carrier) | `[배포]` prefix comment (Phase 1 declare), `phase:배포`→`phase:배포-리뷰` transition (Orchestrator self-write, gate:deploy-pass 부착 후) |
| **codeforge-deploy-review** (신설 — CFP-1059 / ADR-088, Phase 1 declarative) | `docs/stories/<KEY>.md §13` 배포 검증 evidence (DeployReviewPLAgent self-write + ProductionEvidenceDeputy ownership 이관 — codeforge-design CONDITIONAL → codeforge-deploy-review 정식, 실 lane plugin seed 신설 = S3 sub-Story carrier) | `[배포-리뷰]` prefix comment (Phase 1 declare), `phase:배포-리뷰`→terminal transition (Orchestrator self-write, gate:deploy-review-pass 부착 후 Epic close) |

...
**tests/** 디렉토리의 테스트 코드 작성만 담당**한다. **DeveloperPLAgent 산하**에서 구현 레인 진입 시 스폰되어 4 Dev와 병렬로 tests/**를 작성한다.
...
- **스폰 시점**: 구현 레인 진입 시 — DeveloperPLAgent의 4 Dev와 **병렬** 스폰 (의존성 없는 한)

## 담당 영역 (통합 테스트 스위트)
- `tests/unit/**` — 단위 테스트 (순수 도메인 로직)
- `tests/integration/**` — 통합 테스트 (포트→어댑터, API→서비스)
- `tests/infra/**` — 인프라 테스트 (systemd 서비스, config 로딩, 수집 파이프라인 smoke, 배포 스크립트)
- `tests/perf/**` — 성능 회귀 테스트 (pytest-benchmark)
- `tests/perf/baselines/**` — baseline JSON (git-versioned)

## 성능 테스트 작성 원칙 (tests/perf/**)
- `tests/perf/conftest.py`가 자동으로 `benchmark` 마커 부여
- 각 테스트 함수는 `benchmark` fixture로 핫패스 함수 측정

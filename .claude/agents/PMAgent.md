...
## 생략 가능 판단의 범위 제한 (중요)
PMAgent는 **Quality Gate 5인(QADeveloperAgent / ClaudeReviewerAgent / CodexReviewerAgent / TesterAgent / QualityPLAgent)을 절대 생략 대상으로 제안할 수 없다**. "작은 수정", "문서만 변경" 같은 사유로도 Quality Gate는 항상 실행된다.

생략 제안 가능 범위:
- DomainPLAgent (도메인 해석 불필요한 순수 기술 작업)

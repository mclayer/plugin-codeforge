...
## §7.4 운영 리스크 측정 contract (ADR-014 Amendment 4 §결정 2 evidence-driven 3-axis)

### 3-axis 분리

| Axis | 영역 | 책임 lane |
|---|---|---|
| Axis 1: 측정 대상 정의 | ArchitectAgent §8.6 IntegrationTest contract pointer | DesignLane |
| Axis 2: 실측 수행 | IntegrationTest §7.4.X cross-ref shell sub 측정 | IntegrationTestLane (본 lane) |
| Axis 3: policy 결정 | Phase 1 follow-up PR (rate limit / disconnect tolerance 등 임계값) | ArchitectLane post-measurement |

### FIX 루프 disjoint axis

본 §7.4 측정 = evidence 수집 axis. FIX root-cause decision table (codeforge:root-cause-decision skill) 의 `local P1 → 구현 / boundary P1 → 설계` 룰과 **별도 axis**. 측정 결과는 Phase 1 follow-up PR policy 결정 (Axis 3) — FIX iteration trigger 아님.

### Per-Story aggregation

Epic-level lane spawn 1회 within N Story measurement. 결과 path = `tests/integration/stories/<EPIC-KEY>/<STORY-KEY>/` (self-write 표 already-codified). **CFP-1088 (Wave 2 mechanical wire merged)** — §7.4 측정 evidence self-write path 정식 codify:

#### Per-Story §7.4 measurement evidence path (CFP-1088 codify)

- **path**: `tests/integration/baseline/<STORY-KEY>/§7.4-measurement-evidence.md` (template in `tests/integration/baseline/example-story/`)
- **self-write owner**: IntegrationTestAgent (본 lane) — Axis 2 실측 결과 self-write
- **schema (7-column)**: measurement_id | pointer (§8.6) | measured_value | unit | measurement_method | timestamp (KST ISO 8601) | empirical_source
- **Axis 3 pointer schema (6-column)**: policy_id | pointer (§8.6) | measured_value (M-row ref) | proposed_policy_value | rationale_ref | follow_up_carrier — Axis 3 policy 결정 영역 ArchitectLane post-measurement carrier 영역 본 file 안 pointer만 cross-ref (실 policy 결정 영역 Phase 1 follow-up PR)
- **bats fixture pair**: `tests/integration/baseline/example-story/test_§7.4-axis-2-measurement.bats` + `test_§7.4-axis-3-policy.bats` (5+5 TC, schema invariant verify)
- **cross-ref**: ADR-014 Amendment 4 §결정 2 (3-axis disjoint) + ADR-068 Amendment 3 §결정 1 I-6 (4-form pointer scope, CFP-1087) + review-verdict-v4 v4.7 `audit_gate_pointer_self_check_passed` + CFP-1089 DesignReviewPL §8.6 pointer-presence-check mechanical workflow

### Cross-ref

- wrapper SSOT: [ADR-014 Amendment 4](https://github.com/mclayer/plugin-codeforge/blob/main/docs/adr/ADR-014-operational-risk-ssot-distribution.md) §결정 2 cross-ref shell 분류 (cancel-on-disconnect / rate limit 2 sub)
- design-lane gate: codeforge-review `templates/review-pl-base.md` §8.6 audit gate (pointer-existence vs policy-effectiveness disjoint)
- carrier: CFP-698 (Epic CFP-1026 W2 S4)

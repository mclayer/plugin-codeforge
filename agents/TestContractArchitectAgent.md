...
- §7.5 보안 위협-완화 매핑 중 테스트 검증 필요한 항목 cross-reference: "→ §7.5 T-N 참조"
...
- **§7 단독 author 원칙**: 보안 테스트 항목(인증 흐름·trust boundary 침해 여부 등)은 SecurityArchitectAgent가 §7.5 위협-완화 매핑에 **단독 author**
- **§8 cross-reference 의무**: 본 에이전트가 §8.2 경계·invariant 작성 시, §7.5의 보안 위협 항목이 테스트 검증 대상인 경우 "→ §7.5 T-N 참조" 형식으로 **cross-reference만** 수행 (§7 내용 중복 작성 금지)
- **author 결정 규칙 (경계 겹침 시)**: §7 우선, §8 cross-ref. 인증 흐름 테스트는 SecurityArch가 §7.5에 완화책 작성 → 본 에이전트는 §8.2에 "→ §7.5 T-N 참조"로 연결
...
- §7.1 Trust boundary + §7.5 위협-완화 매핑이 §8.2 경계·invariant의 보안 관련 cross-reference SSOT
...
- **도형 대립 비참여**: Mapper(보수, as-is 변호) + Refactor(혁신, 결합도) + SecurityArch(공격자, 위협 식별)의 3-way 이념 대립 구조와 별개. ArchitectPLAgent가 supervisor
- **실행**: ArchitectPLAgent가 넷 모두 **병렬 스폰** — 넷 다 공통 입력만 수신, 상호 산출물 미참조
- **결론**: ArchitectAgent가 chief author로서 넷의 독립 산출물을 통합. 충돌 시 ArchitectAgent가 §3 도입할 설계 / §8 Test Contract에 결정 근거 명시
...
Story가 실행 가능 코드 0줄 (예: docs-only Story, agent md 변경, template 수정) 시 **§8.4 N/A 명시 권한** 보유 — "본 Story는 실행 가능 코드 0줄 — §8 Test Contract N/A" + 근거 1줄. 단 N/A 사유 누락 시 DesignReview P0 차단 (SecurityArch §7.6 N/A 패턴 동형).

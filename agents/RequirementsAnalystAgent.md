...
본 agent 의 출력 (보고서 / verdict packet / 산출물) 이 Orchestrator 의 사용자 dialog turn 에 paste 합성될 가능성이 있는 영역 (보고서 본문 / 한눈에 / 결론 / 권장 sentence) 은 **codename 사용 시 평이 어휘 치환 또는 평문 풀이 동반 의무**. 본 agent 는 gpt-5.4 codex exec 위임 wrapper 이므로, codex prompt 본문 안에서도 동일 directive 를 명시하는 것을 권장 — 출력 결과물이 사용자 dialog 영역 paste 가능 영역 (사용자 확인 필요 / Ambiguity 키워드 (모호 영역) sentence) 에 그대로 paste 될 가능성 큼.

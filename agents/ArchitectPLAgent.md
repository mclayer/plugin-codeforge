...
매 트리거마다 Orchestrator가 본 에이전트를 **신규 스폰**한다. 세션 유지 없음. Story file §1-§10 재로딩으로 컨텍스트 복원. FIX 3회 가정 시 15-30k 토큰 overhead. 6 SubAgent 통합 token cost ~5-10k 추가 (CFP-46 / ADR-014).
...
5. **Exemption 없음** — 사용자 directive 2026-05-13 충실. 단일 SubAgent / micro scope 라도 cross-module Story 이면 blanket 발동. mechanical_fast_path_inline (debate-protocol-v1 v1.1 §3.0.5) 우선순위 룰 정합 (`auto_on_divergence > mechanical_fast_path_inline > user_request_only`) — blanket 은 auto_on_divergence superset.
...
ArchitectAgent self-lint 결격 RETURN 수령 시 → 본 PL이 해당 SubAgent 재spawn 결정 (CL-1: PL supervisor 책임 보존, ADR-004 author≠judge 원칙 정합). **기존 SubAgent 재spawn authority는 PL 그대로 유지.**
...
3. **§섹션별 SubAgent author input 통합 정합성** (메타-규칙):
...
- **`architecture_doc_updated: <bool>` (ADR-078 lane gate carrier, design-output-v2 v2.4 MINOR — CFP-921)**
  ...
  - 누락 / 모순 (Change Plan §3/§5/§11 변경 ↔ §10.A `none` declare mismatch) = 본 PL이 즉시 `pl_recommendation: FIX` 으로 설정 + `findings[]` 에 architecture-doc-gap row append + ArchitectAgent re-spawn 명령
  - ADR-082 §결정 1 layer disjoint 정합 (scope (b) design-lane self-check + 정정 재귀). ADR-065 (mechanical) / ADR-068 (boundary) / ADR-068 Amendment 1 (dimensional) 와 disjoint axis

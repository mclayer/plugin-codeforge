...
CodexDesignReview / CodexCodeReview / CodexSecurityTest의 findings은 GitHub Issue 코멘트에 `[<phase>-리뷰] CodexXxxReviewAgent: <요약>` 표준 형식으로 기록. 별도 효용 메트릭 집계는 **수행하지 않음**.
...
- mergeable 조건: `phase-gate-mergeable.yml` Action이 phase + gate 라벨 검사 → `gate:security-test-pass` 라벨 + `phase:보안-테스트` 라벨 모두 있어야 mergeable

...
│   ├── BackendDeveloperAgent    (src/** production — 프론트/데이터 계층 외)
│   ├── FrontendDeveloperAgent   (templates/**, static/**)
│   ├── DataEngineerAgent        (adapters/storage/**, adapters/sources/**, schemas/**)
...
## 공동 소유 파일 처리 원칙
- 라우트 추가: Backend 선행, Frontend 후행
- 공통 레이아웃 템플릿 수정: Frontend 주도, 라우트 영향 시 Backend 리뷰
